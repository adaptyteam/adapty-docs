...
:::warning
Adapty envoie les [achats Stripe et Paddle](#web-purchases-stripe-and-paddle) à Adjust avec le token de l'application correspondant à la plateforme sur laquelle l'acheteur a installé votre application. Si vous publiez une application Android et vendez sur le web, renseignez le champ **App Token** Android. Sans cela, l'achat web d'un utilisateur Android échoue avec l'erreur [Missing integration credentials](handle-integration-errors#missing-integration-credentials).
:::
...
### Connecter votre application à Adjust

Après avoir suivi les étapes décrites ci-dessus, ajoutez les deux méthodes suivantes à votre application. Elles établiront la communication entre votre application et Adjust :
...
2. **Pour recevoir les données d'attribution depuis Adjust** : Mettez à jour les données d'attribution avec la méthode SDK `updateExternalAttribution()` (appelée `updateAttribution()` avant la version 4.1 du SDK)
...
Adjust reçoit les achats [Stripe](stripe) et [Paddle](paddle) en même temps que vos achats App Store et Google Play. Un achat web se produit en dehors de votre application, il n'a donc pas de plateforme mobile propre. Adapty utilise à la place la plateforme de l'acheteur. Lorsque leur profil Adapty a été créé sur l'App Store ou Google Play, l'achat web est transmis à Adjust comme s'il provenait de ce store :

| Store du profil de l'acheteur | Ce qu'Adapty envoie avec l'achat |
|:------------------------------|:---------------------------------|
| App Store                     | Votre **App Token** et **OAuth Token** iOS, ainsi que les identifiants `idfa` et `idfv` de l'acheteur. |
| Google Play                   | Votre **App Token** et **OAuth Token** Android, ainsi que les identifiants `android_id` et `gps_adid` de l'acheteur. |

L'achat parvient à Adjust associé au même appareil que l'installation, ce qui permet à Adjust de l'attribuer à la campagne qui a amené l'utilisateur.

La page d'intégration Adjust ne comporte pas de champs distincts pour Stripe ou Paddle. Les achats web réutilisent les identifiants iOS et Android de la section [Connecter Adapty à Adjust](#connect-adapty-to-adjust).

Un acheteur qui n'a jamais ouvert votre application mobile n'a pas d'identifiant appareil Adjust, et son profil a été créé sur Stripe ou Paddle. Adapty envoie son achat à Adjust avec votre **App Token** iOS, sans identifiant publicitaire. Adjust n'a alors aucun identifiant permettant de relier cet achat à une installation.
