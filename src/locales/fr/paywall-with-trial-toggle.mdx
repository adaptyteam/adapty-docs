---
title: "Créer un paywall avec un bouton de trial"
description: "Basculez un paywall entre des produits avec et sans essai gratuit, et synchronisez le produit sélectionné avec le bouton."
metadataTitle: "Créer un paywall avec un bouton de trial | Flows et paywalls | Documentation Adapty"
---

Un bouton de trial vous permet de basculer un paywall entre des produits avec un essai gratuit et des produits sans. Mais le bouton lui-même n'est qu'un interrupteur : activé ou désactivé. C'est vous qui écrivez les règles qui affichent les bonnes cartes, et les règles qui changent le produit sélectionné.
...
## 1. Ajouter les cartes de produits \{#1-add-the-product-cards\}

1. Cliquez sur le menu Ajouter un élément **+** > **Paywall & Commerce** > **Products** et choisissez un preset.
2. Sélectionnez une carte — sur le canevas ou dans le panneau **Layers**. Assignez un produit à la carte et laissez le menu déroulant des offres sur **No offer**.
3. Répétez l'opération pour chaque plan que vous vendez — une carte par plan, sans l'essai.
4. Dupliquez chaque carte. Sur chaque copie, sélectionnez l'offre d'essai dans le menu déroulant des offres.

Chaque plan dispose maintenant de deux cartes pointant vers le même produit — une avec l'offre, et une sans.
...
1. Cliquez sur **+** > **Paywall & Commerce** > **Toggle** > **Trial toggle**.
...
2. Vérifiez que le bouton bascule appartient à un groupe sélectionnable du bon type. Cliquez sur une zone vide du canvas pour ouvrir les **Screen settings**. Sous **Selectable groups**, ouvrez `trial_toggle`, définissez **Group Type** sur **Toggle**, puis cliquez sur **Save**. La boîte de dialogue affiche la variable que nous utiliserons pour détecter si le bouton bascule est activé : `trial_toggle.isActive`.

3. Modifiez le libellé du bouton de bascule.

Laissez le bouton de bascule désactivé par défaut. Un bouton de bascule d'essai présélectionné risque d'entraîner un rejet par l'App Store en tant que pratique trompeuse — voir [Boutons de bascule et cases à cocher](builder-toggles).
