...
Un paywall rendu par Adapty est livré sous forme de flow dans le SDK v4, donc la règle ci-dessous couvre les deux cas.

La correspondance est exacte. Le SDK compare le code que vous passez avec les codes de localisation du flow caractère par caractère : il ne modifie pas la casse, ne remplace pas les tirets bas (`_`) par des tirets (`-`), et n'utilise pas de repli vers la balise de langue. Pour un flow avec une localisation `pt-br`, seul `pt-br` correspond : `pt-BR`, `pt_BR` et `pt-PT` ne correspondent pas.

Lorsque le code ne correspond à aucune localisation, le flow s'affiche silencieusement dans sa [locale par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — le SDK ne retourne pas d'erreur et n'enregistre pas d'avertissement.

Lorsque le code correspond, Adapty fusionne la localisation avec la localisation par défaut : les chaînes et ressources que la localisation correspondante ne définit pas proviennent de la localisation par défaut.

Omettre le code de langue n'est pas équivalent à demander la localisation par défaut du flow : le SDK substitue un `en` fixe. Un flow dont la langue par défaut est `de` s'affiche quand même en `en` s'il possède une localisation `en`, et ne bascule sur `de` que s'il n'en a pas.

:::warning
Transmettez le code de langue exactement tel qu'il est configuré dans le tableau de bord — des sous-balises en minuscules séparées par des tirets. Ne transmettez pas directement un identifiant de locale système : `CultureInfo.CurrentCulture.Name` retourne `pt-BR`, ce qui provoque un repli sur la localisation par défaut. Convertissez la valeur dans votre application avant de la transmettre.
:::

### Onboardings

Les onboardings sont localisés côté serveur, et les règles du serveur acceptent d'autres formats. Lorsque vous passez un `Locale` à [`GetOnboarding`](unity-get-onboardings) :
...
4. Si aucune correspondance n'est trouvée non plus, Adapty retourne le contenu dans la langue par défaut de l'onboarding
...
- **Flows et paywalls rendus par Adapty** : le SDK ne lit pas la langue du périphérique, résolvez-la dans votre app et [transmettez-la lors de la création de la vue](#choose-the-localization-of-a-flow). Le code de langue est facultatif — omettez-le et le flow s'affiche en `en`, ou dans sa langue par défaut si le flow n'a pas de localisation `en`.
- **Paywalls personnalisés (Remote Config)** : `GetFlow` retourne toutes les localisations configurées dans `flow.RemoteConfigs`. Chaque entrée est un `AdaptyRemoteConfig` avec un code `Locale` et un `Dictionary` de valeurs. Sélectionnez l'entrée correspondant à l'utilisateur, avec votre propre mécanisme de secours :
...
Adapty stocke ces codes `Locale` dans le format décrit dans [Standard de code de locale chez Adapty](#locale-code-standard-at-adapty). Le SDK ne fait pas correspondre les Remote Configs à une locale, c'est donc à votre application de déterminer quelle entrée appliquer.

### Choisir la localisation d'un flow \{#choose-the-localization-of-a-flow\}
