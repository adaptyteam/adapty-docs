...
Invoquée après la fin d'une navigation de paiement web, qu'elle ait réussi ou échoué.
...
Consultez [Suivre les vues des écrans de flow](ios-flow-screen-views) pour savoir comment les utiliser.

Un flow remonte également les valeurs que les utilisateurs saisissent et sélectionnent. Consultez [Traiter les données des flows](ios-flow-input) pour les lire.
...
:::important
Ce guide couvre la gestion des événements pour les achats, les restaurations, la sélection de produits et le rendu des paywalls. Vous devez également implémenter la gestion des boutons (fermeture du paywall, ouverture de liens, etc.). Consultez notre [guide sur la gestion des actions de boutons](handle-paywall-actions) pour plus de détails.
:::

Les paywalls configurés avec le [Paywall Builder](adapty-paywall-builder) n'ont pas besoin de code supplémentaire pour effectuer et restaurer des achats. Cependant, ils génèrent certains événements auxquels votre application peut répondre. Ces événements comprennent les pressions sur des boutons (boutons de fermeture, URLs, sélections de produits, etc.) ainsi que des notifications sur les actions liées aux achats effectuées sur le paywall. Découvrez ci-dessous comment répondre à ces événements.

Ce guide couvre uniquement **les paywalls construits dans l'ancien builder**, qui nécessitent le SDK Adapty v3.0 ou une version ultérieure.
...
Nous recommandons de fermer l'écran du paywall dans ce cas.

Il ne sera pas invoqué en mode Observer. Consultez la rubrique [iOS - Présenter les paywalls du Paywall Builder en mode Observer](ios-present-paywall-builder-paywalls-in-observer-mode) pour plus de détails.
