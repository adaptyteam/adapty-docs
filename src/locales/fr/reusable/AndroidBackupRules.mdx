...
Symptômes typiques de l'erreur : `Manifest merger failed: Attribute application@dataExtractionRules value=(@xml/your_data_extraction_rules)
...
#### 1. Ajouter le namespace `tools` à votre manifeste \{#1-add-the-tools-namespace-to-your-manifest\}
...
#### 2. Remplacer les attributs de sauvegarde dans `<application>` \{#2-override-backup-attributes-in-application\}
...
#### 3. Créer les fichiers de règles de sauvegarde fusionnées \{#3-create-merged-backup-rules-files\}
