...
description: "Guide de démarrage rapide pour configurer Adapty pour la gestion des abonnements intégrés dans React Native."
...
:::important
Ce guide vous concerne si vous disposez de votre propre système d'authentification. Vous y apprendrez comment gérer les profils utilisateurs dans Adapty afin de l'aligner avec votre système d'authentification existant.
:::
...
- Si votre application possède (ou possédera) une authentification backend, consultez la [section sur les utilisateurs identifiés](#identified-users).
...
- Vous fournissez un **customer user ID** pour faire le lien entre les profils Adapty et votre système d'authentification interne.
...
| **Persistance des données**  | Les données des utilisateurs anonymes sont liées à l'installation de l'app | Les données des utilisateurs identifiés persistent entre les installations de l'app |
...
Deux options s'offrent à vous pour identifier les utilisateurs dans l'application :
