...
Cette métrique suit le nombre d'utilisateurs ayant démarré l'essai après avoir vu un paywall. L'écran d'un [flow](adapty-flow-builder) compte comme une vue paywall ici.
...
### Vue du paywall -> Payé \{#paywall-view---paid\}

Cette métrique indique combien d'utilisateurs ont effectué un achat après avoir vu un paywall. Un écran d'un [flow](adapty-flow-builder) compte ici comme une vue de paywall.
...
<details>
    ...
    **Exemple** :
    ...
    - Le 8 janvier, 20 de ces utilisateurs avaient effectué un achat.
...
- Le 8 janvier, la conversion pour le 1er janvier = (20 / 100) × 100% = 20%
   ...
   Cela signifie que 50% des utilisateurs qui ont vu un paywall le 1er janvier ont effectué un achat, jusqu'au moment présent.
...
Si la plage de dates choisie ne suffit pas à afficher des résultats, une notification peut apparaître avec une date suggérée et une option pour ajuster automatiquement la plage de dates en un seul clic.
