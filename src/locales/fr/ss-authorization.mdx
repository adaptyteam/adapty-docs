...
Les requêtes à l'API côté serveur doivent être authentifiées avec votre clé API secrète, transmise dans un en-tête Authorization. Vous pouvez la trouver dans les [**App Settings**](https://app.adapty.io/settings/general). Le format de la valeur est `Api-Key {your-secret-api-key}`, par exemple `Api-Key secret_live_...`.
