...
| **customLayoutId**   | <p>optionnel</p><p>par défaut : `undefined`</p><p>SDK 4.1+</p> | L'identifiant personnalisé d'une mise en page définie dans le Flow Builder. Passez-le pour afficher cette mise en page spécifique plutôt que celle sélectionnée automatiquement par le SDK selon le type d'appareil et la taille d'écran. Si aucune mise en page ne correspond à l'identifiant, le flow se charge sans configuration de vue. |
| **customTags**       | optionnel | Définit un dictionnaire de tags personnalisés et leurs valeurs résolues. Les tags personnalisés servent de placeholders dans le contenu, remplacés dynamiquement par des chaînes spécifiques pour personnaliser le contenu dans le flow/paywall. Consultez la rubrique Tags personnalisés dans le Paywall Builder pour plus de détails. |
| **prefetchProducts** | optionnel | Activez cette option pour optimiser le timing d'affichage des produits à l'écran. Quand `true`, AdaptyUI récupère automatiquement les produits nécessaires. Par défaut : `true`. |
| **android.enableSafeArea** | optionnel | Android uniquement (ignoré sur iOS). Imbriqué sous la clé `android`. Quand `true`, la vue du flow applique les marges de la zone sécurisée. Par défaut : `true`. La valeur par défaut convient à la plupart des cas. |
