...
Un paywall rendu par Adapty est fourni sous forme de flow dans le SDK v4, donc la règle ci-dessous s'applique aux deux.

La correspondance est exacte. Le SDK compare le code que vous passez avec les codes de localisation du flow caractère par caractère : il ne modifie pas la casse, ne remplace pas les tirets bas (`_`) par des tirets (`-`), et ne revient pas au sous-tag de langue. Pour un flow avec une localisation `pt-br`, seul `pt-br` correspond : `pt-BR`, `pt_BR` et `pt-PT` ne fonctionnent pas.
...
Lorsque le code correspond, Adapty fusionne la localisation avec la localisation par défaut : les chaînes et les ressources non définies dans la localisation trouvée proviennent de la localisation par défaut.

Omettre le code de langue ne revient pas à demander la localisation par défaut du flow : le SDK substitue un `en` fixe. Un flow dont la langue par défaut est `de` s'affiche quand même en `en` s'il possède une localisation `en`, et ne bascule vers `de` qu'en son absence.

:::warning
Transmettez le code de langue exactement tel qu'il est configuré dans le tableau de bord — sous-balises en minuscules séparées par des tirets. Ne transmettez pas directement un identifiant de langue de plateforme : sur Android, `Locale.getDefault().toLanguageTag()` renvoie `pt-BR` ; sur iOS, `NSLocale.currentLocale.localeIdentifier` renvoie `pt_BR`. Les deux reviennent à la localisation par défaut. Convertissez la valeur dans votre application avant de la transmettre.
:::
...
Dans le SDK v4, vous ne passez pas de code de langue lors de la récupération d'un flow — `getFlow` renvoie le flow avec toutes ses localisations, et Adapty en applique une lors de la création de la vue du flow.

- **Flows et paywalls rendus par Adapty** : le SDK ne lit pas la langue du terminal, alors résolvez-la dans votre application et passez-la en tant que paramètre `locale` de `createFlowView`. C'est facultatif — si vous l'omettez, le flow s'affiche en `en`, ou dans sa [langue par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) si le flow ne possède pas de localisation `en`.
