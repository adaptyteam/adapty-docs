---
title: "Migrer le SDK Adapty Android vers la v4.0"
description: "Migrez vers le SDK Adapty Android v4.0 en remplaçant les API paywall par des API flow, qui restituent les flows en plus des anciens paywalls du builder."
metadataTitle: "Migration vers le SDK Adapty Android v4.0 | Documentation Adapty"
---

Le SDK Adapty Android 4.0 introduit les flows et renomme les API paywall en conséquence. Les nouvelles API fonctionnent avec les flows et restent compatibles avec les paywalls de l'ancien builder — aucune modification de configuration n'est nécessaire côté Adapty Dashboard.
...
- **Les onboardings sont dépréciés.** `AdaptyUI.getOnboardingView` et `AdaptyUI.getOnboardingConfiguration` sont marqués `@Deprecated` dans la version 4.0 — migrez vos onboardings vers des flows créés dans le [Flow & Paywall Builder](adapty-flow-builder).
...
Le type de retour de la récupération passe de `AdaptyPaywall` à `AdaptyFlow`, et le chargeur de configuration est renommé de `AdaptyUI.getViewConfiguration` en `AdaptyUI.getFlowConfiguration` (qui retourne `AdaptyUI.FlowConfiguration` au lieu de `AdaptyUI.LocalizedViewConfiguration`). Le paramètre `locale` sort de l'appel de récupération pour aller sur `getFlowConfiguration` : `AdaptyFlow` expose également `paywalls` — les variantes de paywall du flow, sous forme de valeurs `AdaptyFlowPaywall`. Les méthodes de paywall web prennent l'une de ces valeurs.
...
`logShowPaywall` est renommé en `logShowFlow` et prend désormais un `AdaptyFlow` au lieu d'un `AdaptyPaywall`. L'événement est toujours enregistré par rapport à la même variation, donc les métriques de funnel et de test A/B existantes continuent de fonctionner sans modification du tableau de bord.
...
Comme dans la v3, vous n'avez pas besoin d'appeler cette méthode lors de l'affichage des flows ou des paywalls rendus par Adapty — Adapty suit automatiquement ces vues.
...
:::note
Le SDK 4.1 renomme cette méthode en `updateExternalAttribution` et le type en `AdaptyExternalAttributionProvider`. Les anciens noms sont supprimés dans la version 4.1 sans dépréciation préalable. Si vous migrez directement vers la 4.1, consultez [Migrer vers v4.1](migration-to-android-sdk-41).
:::
