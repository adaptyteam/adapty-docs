...
:::important
Vous pouvez également définir un ID média personnalisé pour une image ou une vidéo en [arrière-plan](paywall-head-picture).
:::

Associez un ID média personnalisé à un élément image ou vidéo pour le remplacer au moment de l'exécution depuis le code de votre application. Utilisez cette option pour des [visuels personnalisés](get-pb-paywalls#customize-assets) — par exemple, afficher l'avatar choisi par l'utilisateur.

Le média que vous importez dans le Flow & Paywall Builder sert de solution de secours. Si votre code ne fournit pas de média pour cet ID au moment de l'exécution, c'est le média de secours qui s'affiche.
