...
| Prérequis | Version |
|---|---|
| React Native | 0.75 ou supérieur. [L'intégration SPM de React Native](https://reactnative.dev/blog/2026/08/11/react-native-0.87#experimental-swift-package-manager-support-for-ios) nécessite 0.87 ou supérieur. |
| iOS | 15.0 ou supérieur |
| Swift | 6.2 ou supérieur, inclus avec Xcode 26 |

:::info
Le SDK Adapty embarque sa propre version de la Google Play Billing Library — vous n'avez pas à l'ajouter à votre application.

À partir de la version 3.17.0, le SDK Adapty React Native utilise Billing Library v8 par défaut. Les versions 3.15.0 à 3.15.6 utilisent v7, et fonctionnent également lorsqu'une autre dépendance dans votre build Android élève la Billing Library à v8.
...
### Configurez votre projet iOS \{#set-up-your-ios-project\}

Adapty publie ses SDK iOS natifs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) uniquement sous forme de packages Swift — la v4 a abandonné la distribution CocoaPods ([le dépôt de specs CocoaPods passe en lecture seule en décembre 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Les deux options installent ces packages Swift ; elles diffèrent par ce qui pilote l'installation.
...
- **Intégration SPM de React Native (SDK 4.1 et versions ultérieures)** : Votre projet iOS abandonne CocoaPods, et React Native résout lui-même les packages Swift à partir du manifeste `Package.swift` fourni par Adapty. [React Native a ajouté cette intégration dans la version 0.87](https://reactnative.dev/blog/2026/08/11/react-native-0.87#experimental-swift-package-manager-support-for-ios) en préversion et ne la recommande pas encore pour la production.

<Tabs groupId="ios-package-manager" queryString>

<TabItem value="cocoapods" label="CocoaPods" default>

Déclarez les frameworks dynamiques dans votre cible `ios/Podfile` :
...
React Native lit le manifeste `Package.swift` d'Adapty et résout lui-même les SDK iOS natifs. Vous n'avez rien à ajouter à votre projet Xcode.

Conservez `ios/Podfile` en place, même si ce fichier n'installe aucun pod. Le CLI React Native localise votre projet iOS en cherchant le Podfile, donc sans lui, l'autolinking ignore entièrement iOS.

Si votre application utilise des pods autres que React Native, conservez-les dans le Podfile, exécutez `pod install`, et ouvrez le `.xcworkspace` plutôt que le `.xcodeproj`. Le workspace couvre les deux gestionnaires de paquets.

:::note
Sur un clone fraîchement créé ou lors d'un checkout CI, exécutez `npx react-native spm` une fois avant de lancer le build. Le projet Xcode re-synchronise les dépendances pendant le build, après qu'Xcode ait déjà résolu le graphe de paquets, donc un build démarré juste après une installation peut réussir avec Adapty manquant.
:::
