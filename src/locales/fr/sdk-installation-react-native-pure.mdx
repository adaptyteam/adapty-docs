...
:::important
À partir de la v4, le SDK Adapty pour React Native ne prend plus en charge l'installation de ses dépendances natives via CocoaPods. Si vous avez besoin de la v4 ou d'une version ultérieure (pour le [Flow & Paywall Builder](adapty-flow-builder)), suivez plutôt [SDK Adapty 4.0 : activer Swift Package Manager](#adapty-sdk-40-enable-swift-package-manager) ci-dessous.
:::
...
Le SDK React Native 4.0 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — nécessite **React Native 0.75 ou ultérieur**. Installez le SDK :
