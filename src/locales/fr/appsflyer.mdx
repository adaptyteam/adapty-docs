...
2. Attendez qu'AppsFlyer invoque ce callback. AppsFlyer attribue chaque installation sur ses propres serveurs, donc le résultat parvient à votre application de manière asynchrone plutôt qu'au lancement. Le SDK invoque à nouveau le callback à chaque session ultérieure.
3. Dans le callback, lisez l'identifiant AppsFlyer de l'utilisateur avec `getAppsFlyerUID` et transmettez-le à Adapty via `setIntegrationIdentifier()`. Les événements d'Adapty n'atteignent le bon utilisateur AppsFlyer qu'avec cette valeur.
4. Dans le même callback, transmettez les données d'attribution d'AppsFlyer à Adapty avec `updateExternalAttribution()`. Cela indique à Adapty quelle campagne a généré l'installation. La méthode s'appelait `updateAttribution()` avant la version 4.1 du SDK.
5. Attendez la résolution de `Adapty.identify()` au lieu de l'exécuter en parallèle des étapes 3 et 4. Adapty crée un profil anonyme à l'activation, puis bascule vers le profil identifié une fois que `identify()` est résolu. Un `appsflyer_id` défini pendant ce basculement ne le survit pas toujours.
