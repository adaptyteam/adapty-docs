...
Les offres s'appliquent uniquement aux abonnements : les produits de type **Lifetime**, **Non-Subscriptions** ou **Consumables** ne peuvent pas en bénéficier. Pour réduire le prix d'un achat unique, [créez un second produit au prix réduit et ciblez-le avec des audiences](#how-adapty-works-with-offers) à la place.

Avant de commencer à configurer les offres dans Adapty, assurez-vous des points suivants :
...
Dans le [Flow & Paywall Builder](adapty-flow-builder), vous associez une offre à un produit dans le panneau **Products** ou sur une fiche produit.

#### Dans le panneau Products \{#in-the-products-panel\}

- **Lors de l'ajout du produit** : Cliquez sur **+**, sélectionnez le produit et choisissez l'offre sous **Offer**. Les cartes qui utilisent le produit par la suite auront cette offre présélectionnée. Les étapes complètes sont décrites dans [Ajouter des produits dans le panneau Produits](paywall-product-block#add-products-in-the-products-panel).
- **Pour un produit déjà à l'écran** : Ouvrez le menu contextuel de l'entrée produit et sélectionnez [**Offer**](paywall-product-block#product-actions). Chaque carte, variable de texte et action utilisant ce produit bascule vers la nouvelle offre.

#### Sur une carte produit

1. Sur le canevas, sélectionnez la carte produit qui doit afficher l'offre.
2. Dans le panneau de droite, sous **Product**, sélectionnez le produit, puis choisissez l'offre dans le menu déroulant des offres. Si vous avez ajouté le produit avec une offre dans le panneau **Products**, cette offre est présélectionnée. Une offre différente ajoute une deuxième entrée pour le produit dans le panneau.

<ZoomImage id="offer-flow.webp" width="500px" alt="Paramètres de la carte produit dans l'onglet Design avec le menu déroulant des offres ouvert et Pay as you go sélectionné" />

### Ajouter une offre à un paywall \{#add-offer-to-paywall\}

:::info
Vous pouvez modifier les produits et les offres d'un paywall tant qu'il est à l'état **Draft**, ou dans les 30 jours suivant sa création s'il n'a encore aucune transaction. Passé ce délai, Adapty rejette la modification, et le tableau de bord désactive les champs de produits en expliquant pourquoi. Remplacer une offre par une autre sur un produit déjà présent dans le paywall est considéré comme une telle modification.

Pour modifier les produits ou les offres d'un paywall après ce délai, [dupliquez-le](duplicate-paywalls) et configurez la copie. Les flows ne sont pas soumis à cette restriction — vous pouvez y modifier les produits et les offres à tout moment.
:::
