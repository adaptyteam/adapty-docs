...
- **Flows créés dans le builder** : le SDK ne lit pas la locale de l'appareil, donc résolvez-la dans votre application et transmettez-la comme argument `locale` de `AdaptyUI.getFlowConfiguration`. L'argument est optionnel — omettez-le et le flow s'affiche dans sa [locale par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale).
- **Paywalls personnalisés (Remote Config)** : `getFlow` retourne toutes les localisations configurées dans `flow.remoteConfigs`. Chaque entrée contient un code `locale` et le contenu de la configuration (`jsonString`, ou le `dataMap` parsé). Sélectionnez l'entrée qui correspond à l'utilisateur, avec votre propre fallback :
