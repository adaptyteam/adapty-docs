...
Un paywall rendu par Adapty est livré sous forme de flow dans le SDK v4, donc la règle ci-dessous s'applique aux deux.

La correspondance est exacte. Le SDK compare le code que vous transmettez avec les codes de localisation du flow caractère par caractère : il ne modifie pas la casse, ne remplace pas les tirets bas (`_`) par des tirets (`-`), et ne se rabat pas sur la balise de langue. Pour un flow avec une localisation `pt-br`, seul `pt-br` correspond : `pt-BR`, `pt_BR` et `pt-PT` ne fonctionneront pas.

Lorsque le code ne correspond à aucune localisation, le flow s'affiche silencieusement dans sa [langue par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — le SDK ne renvoie pas d'erreur et ne journalise pas d'avertissement.

Lorsque le code correspond, Adapty fusionne la localisation avec celle par défaut : les chaînes et ressources non définies dans la localisation correspondante sont récupérées depuis la localisation par défaut.

Omettre le code de langue n'est pas la même chose que demander la localisation par défaut du flow : le SDK substitue un `en` fixe. Un flow dont la langue par défaut est `de` s'affiche quand même en `en` s'il dispose d'une localisation `en`, et ne bascule sur `de` qu'en son absence. Cela s'applique au SDK Android 4.0.1 et versions ultérieures — dans la version 4.0.0, omettre le code affiche la localisation par défaut du flow.

:::warning
Passez le code de locale exactement tel qu'il est configuré dans le tableau de bord — sous-balises en minuscules séparées par des tirets. Ne transmettez pas directement un identifiant de locale système : `Locale.getDefault().toLanguageTag()` renvoie `pt-BR` et `Locale.getDefault().toString()` renvoie `pt_BR`, et les deux reviennent à la localisation par défaut. Convertissez la valeur dans votre application avant de la transmettre.
:::
...
Notez que nous déconseillons cette approche, car il est difficile de prévoir ce que le serveur d'Adapty recevra exactement.

Si vous décidez quand même de l'utiliser, assurez-vous d'avoir couvert tous les cas d'utilisation pertinents.
