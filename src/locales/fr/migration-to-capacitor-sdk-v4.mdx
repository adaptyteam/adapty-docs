...
Le format du [fichier de secours](fallback-flows) a changé dans la version 4.0, puis à nouveau dans la version 4.1.1. Téléchargez à nouveau le fichier depuis **[Placements](https://app.adapty.io/placements)** > **Fallbacks** et intégrez-le à votre application, même si vous en aviez déjà téléchargé un pour une version bêta 4.0.
...
:::note
`customTimers` existe toujours, mais il n'affecte que les paywalls créés avec l'ancien Paywall Builder. Le [minuteur de compte à rebours](flow-timer) d'un flow fonctionne selon le comportement défini dans le Flow & Paywall Builder, donc un flow ignore ce que vous passez ici.
:::

| Paramètre | Description |
|---|---|
| `locale` | La localisation à utiliser pour afficher le flow. Elle a été déplacée ici depuis `getPaywall` — voir [getPaywall → getFlow](#getpaywall--getflow). |
...
L'API onboarding historique est dépréciée dans la v4 au profit du [Flow & Paywall Builder](adapty-flow-builder). Elle fonctionne toujours, mais sera supprimée dans une prochaine version — prévoyez donc la migration de vos onboardings vers le Flow & Paywall Builder.
