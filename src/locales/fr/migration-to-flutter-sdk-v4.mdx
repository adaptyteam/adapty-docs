---
title: "Migrer vers Adapty Flutter SDK v4.0"
description: "Migrez vers Adapty Flutter SDK v4.0 en remplaçant les API de paywall par des API de flow, qui affichent les flows en plus des anciens paywalls du builder."
metadataTitle: "Migration vers Adapty Flutter SDK v4.0 | Documentation Adapty"
---

Adapty Flutter SDK 4.0 introduit les flows et renomme les API de paywall en conséquence. Les nouvelles API fonctionnent avec les flows, et elles restent compatibles avec les paywalls de l'ancien builder — aucun changement de configuration n'est nécessaire côté Adapty Dashboard.

:::note
Ce guide couvre la migration 3.x → 4.0. Le SDK 4.1 modifie quelques API supplémentaires — l'attribution Adapty devient opt-in, les méthodes d'attribution externe sont renommées, et le format du fichier de paywall de secours change à nouveau. Une fois cette migration terminée, consultez [Migrer vers la v4.1](migration-to-flutter-sdk-41).
:::
...
Renommez la méthode et passez l'`AdaptyFlow` via le paramètre `flow`. Les autres paramètres (`loadTimeout`, `preloadProducts`, `customTags`, `customTimers`, `customAssets`, `productPurchaseParams`) sont inchangés, de même que les méthodes de vue `present`, `dismiss` et `showDialog` :

:::note
`customTimers` existe toujours, mais il n'affecte que les paywalls du Paywall Builder hérité. Le [minuteur de compte à rebours](flow-timer) d'un flow fonctionne selon le comportement défini dans le Flow & Paywall Builder, donc un flow ignore ce que vous passez ici.
:::
...
- Le nouveau callback optionnel `flowViewDidReceiveAnalyticEvent` rapporte les événements analytiques d'un flow, en commençant par une vue d'écran pour chaque écran qu'un utilisateur ouvre. Voir [Suivre les vues d'écran d'un flow](flutter-flow-screen-views).
...
L'ancienne API onboarding est dépréciée depuis la v4.0 au profit du [Flow & Paywall Builder](adapty-flow-builder). Elle fonctionne toujours, et votre IDE signale les symboles dépréciés via leurs annotations `@Deprecated` — aucun avertissement n'est émis à l'exécution. Ces symboles seront supprimés dans une prochaine version ; prévoyez donc de migrer vos onboardings vers le Flow & Paywall Builder.
