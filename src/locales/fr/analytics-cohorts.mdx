...
Chaque colonne suivante dans la ligne indique le nombre d'utilisateurs ayant renouvelé leur abonnement pour cette période. M3 correspond au mois 3 et signifie que les abonnés ont effectué 3 renouvellements consécutifs jusqu'à ce point, W7 correspond à la semaine 7, et Y2 correspond à l'année 2. Il arrive que P2 s'affiche dans les cohortes. P désigne la période d'abonnement. Adapty l'affiche à la place de W/M/Y lorsque plusieurs produits avec des périodes de renouvellement différentes sont présents dans la même cohorte.

En mode **par jours**, les en-têtes de colonnes affichent des horizons en jours : `7d`, `28d`, `183d`, etc. Une colonne en jours est cumulative : `183d` comptabilise les utilisateurs ayant effectué au moins une transaction payante dans les 183 premiers jours suivant l'installation, ou les revenus générés durant cette fenêtre — et non les paiements effectués le jour 183. Une cohorte n'a pas besoin d'atteindre cet âge pour que la colonne affiche des données : pour une cohorte plus jeune, la valeur reflète ce qui s'est passé jusqu'à présent et continue de croître.

:::note
Les colonnes de jours cumulatives s'appliquent aux valeurs absolues. Si vous affichez des valeurs relatives, les colonnes Revenus restent cumulatives, tandis que les colonnes Abonnements et Payeurs affichent la part de chaque colonne par rapport au total de la cohorte.
:::

Nous utilisons des dégradés de couleurs pour mettre en évidence les différences entre les valeurs des cohortes. Les valeurs les plus élevées ont des couleurs plus saturées.

L'image ci-dessous montre une cohorte type.
...
## Filtres, métriques, segments de cohorte et export en CSV \{#filters-metrics-cohort-segments-and-export-in-csv\}
...
Parmi ces filtres, seuls **Country, Store, Segment** et **Attribution** réduisent le nombre d'installations de la cohorte et toute métrique calculée à partir de ce nombre, comme l'ARPU. Filtrer par **Product duration, Product** ou **Paywall** réduit bien le Revenu, les Payers et les autres métriques liées aux achats, mais le nombre d'installations conserve tous les utilisateurs ayant installé l'application durant la période — ce qui peut faire paraître l'ARPU artificiellement bas, puisque le numérateur (revenu) diminue tandis que le dénominateur (nombre d'installations) reste inchangé.

<ZoomImage id="ad10de0-cohorts_filter.webp" width="700px" alt="Contrôles de filtre de cohorte" />

À droite du panneau de contrôle, un bouton vous permet d'exporter les données de cohorte au format CSV, que vous pouvez ensuite ouvrir dans Excel, Google Sheets ou importer dans votre propre système analytique.

L'export contient les lignes de cohortes et les totaux pour la métrique actuellement sélectionnée — sans colonne **Country** ni aucune autre ventilation. Pour segmenter les cohortes par pays, appliquez un filtre **Country** et effectuez un export par pays, ou demandez les données de cohortes avec un filtre pays via l'[API d'export](export-analytics-api).
