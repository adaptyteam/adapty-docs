...
| **onUrlPress**                        | Invoqué lorsqu'un utilisateur clique sur une URL dans votre flow.                                                                                                                                                                                                                                                |
| **onAndroidSystemBack**               | Présentation modale uniquement : invoqué lorsqu'un utilisateur appuie sur le bouton système **Back** d'Android.                                                                                                                                                                                                  |
| **onCloseButtonPress**                | Invoqué lorsque le bouton de fermeture est visible et qu'un utilisateur appuie dessus. Il est recommandé de fermer l'écran du flow dans ce gestionnaire.                                                                                                                                                         |
| **onPurchaseCompleted**               | Invoqué lorsque l'achat se termine, qu'il soit réussi, annulé par l'utilisateur ou en attente d'approbation. En cas d'achat réussi, fournit un `AdaptyProfile` mis à jour. Les annulations par l'utilisateur et les paiements en attente (par exemple, approbation parentale requise) déclenchent cet événement, et non `onPurchaseFailed`. |
| **onPurchaseStarted**                 | Invoqué lorsqu'un utilisateur appuie sur le bouton d'action « Acheter » pour lancer le processus d'achat.                                                                                                                                                                                                        |
| **onPurchaseFailed**                  | Invoqué lorsqu'un achat échoue en raison d'erreurs (par exemple, restrictions de paiement, produits invalides, problèmes réseau, échecs de vérification de transaction). Non invoqué pour les annulations par l'utilisateur ou les paiements en attente, qui déclenchent `onPurchaseCompleted` à la place.      |
| **onRestoreStarted**                  | Invoqué lorsqu'un utilisateur lance un processus de restauration des achats.                                                                                                                                                                                                                                     |
| **onRestoreCompleted**                | Invoqué lorsque la restauration des achats réussit et fournit un `AdaptyProfile` mis à jour. Il est recommandé de fermer l'écran si l'utilisateur possède le `accessLevel` requis. Consultez la rubrique [Statut de l'abonnement](react-native-listen-subscription-changes) pour savoir comment le vérifier.     |
| **onRestoreFailed**                   | Invoqué lorsque le processus de restauration échoue et fournit une `AdaptyError`.                                                                                                                                                                                                                                |
| **onProductSelected**                 | Invoqué lorsqu'un produit dans la vue du flow est sélectionné, vous permettant de suivre ce que l'utilisateur choisit avant l'achat.                                                                                                                                                                             |
| **onError**                           | Invoqué lorsqu'une erreur survient pendant le rendu de la vue et fournit une `AdaptyError`. Ce type d'erreur ne devrait pas se produire ; si vous en rencontrez une, merci de nous en informer.                                                                                                                  |
| **onLoadingProductsFailed**           | Invoqué lorsque le chargement des produits échoue et fournit une `AdaptyError`. Si vous n'avez pas défini `prefetchProducts: true` lors de la création de la vue, AdaptyUI récupérera lui-même les objets nécessaires depuis le serveur.                                                                         |
| **onAppeared**                        | Invoqué lorsque le flow s'affiche à l'utilisateur et fournit la `view` qui est apparue — voir [L'argument view](#the-view-argument). Sur iOS, également invoqué lorsqu'un utilisateur appuie sur le [bouton de paywall web](web-paywall#step-2a-add-a-web-purchase-button) dans un flow et qu'un paywall web s'ouvre dans un navigateur intégré. |
| **onDisappeared**                     | Présentation modale uniquement : invoqué lorsque le flow est fermé par l'utilisateur. Sur iOS, également invoqué lorsqu'un [paywall web](web-paywall#step-2a-add-a-web-purchase-button) ouvert depuis un flow dans un navigateur intégré disparaît de l'écran.                                                   |
| **onWebPaymentNavigationFinished**    | Invoqué après avoir tenté d'ouvrir un [paywall web](web-paywall) pour un achat, que ce soit avec succès ou non.                                                                                                                                                                                                  |
...
| **onRequestAppReview**                | Réservé aux demandes d'évaluation de l'application provenant d'un flow. Les flows ne déclenchent pas encore de demandes d'évaluation, vous n'avez donc pas besoin de l'implémenter.                                                                                                                             |
| **onRequestPermission**               | Réservé aux demandes d'autorisations système (comme les notifications push ou l'accès à la caméra) provenant d'un flow. Les flows ne déclenchent pas encore de demandes d'autorisation, vous n'avez donc pas besoin de l'implémenter.                                                                           |
| **onObserverPurchaseInitiated**       | Mode observateur uniquement : invoqué lorsqu'un utilisateur appuie sur le bouton d'achat dans un flow. Adapty n'effectue pas l'achat — réalisez-le avec votre propre code d'achat, puis signalez la transaction à Adapty. Voir [Gérer les achats en mode observateur](#handle-purchases-in-observer-mode) ci-dessous. |
...
L'argument `view` nécessite le SDK React Native 4.0.3 ou version ultérieure. Parmi tous les gestionnaires de flow, seul `onAppeared` reçoit une description de la vue elle-même — un objet `FlowEventView` avec les champs suivants :
...
| **variationId**    | La variante vers laquelle le flow a été résolu, pour attribuer vos propres analytics à un [test A/B](ab-tests).                                                                                                                                                                                    |
| **locale**         | La [localisation du flow](add-paywall-locale-in-adapty-paywall-builder) utilisée pour construire la vue. Elle peut différer de la locale que vous avez demandée si le flow ne dispose pas de cette localisation. Lisez-la pour aligner le reste de votre écran avec la langue dans laquelle le flow s'est affiché. Voir [Utiliser les localisations et les codes de locale](react-native-localizations-and-locale-codes). |
