---
title: "Capacitor - Installation et configuration du SDK Adapty"
...
- **AdaptyUI** : ce module affiche les flows, ainsi que les paywalls créés avec l'ancien builder. AdaptyUI est automatiquement activé avec le module principal.
...
Le [SDK Adapty pour Capacitor](https://github.com/adaptyteam/AdaptySDK-Capacitor/) exige les versions suivantes :
...
La compilation pour iOS nécessite **Xcode 26** ou une version ultérieure — le SDK iOS natif est compilé avec Swift tools 6.2. Sur Android, le SDK minimum est 24.

:::info
Le SDK Adapty intègre sa propre copie de la Google Play Billing Library — inutile de l'ajouter à votre application.

À partir de la version 3.17.0, le SDK Adapty Capacitor utilise la Billing Library v8 par défaut. Les versions 3.15.0 à 3.16.3 utilisent la v7, et fonctionnent également lorsqu'une autre dépendance dans votre build Android fait passer la Billing Library à la v8.
...
Sur iOS, le SDK Adapty récupère les SDK natifs Adapty via **Swift Package Manager uniquement** — le podspec CocoaPods a été supprimé ([le dépôt de specs CocoaPods passe en lecture seule en décembre 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Le projet iOS de votre application doit utiliser l'intégration SPM de Capacitor :

- Pour les nouvelles apps, ajoutez la plateforme iOS avec le gestionnaire de paquets SPM :

  ```sh
  npx cap add ios --packagemanager SPM
  ```

- Pour les apps existantes, migrez le projet iOS de CocoaPods vers SPM en suivant [le guide Capacitor pour utiliser SPM dans un projet existant](https://capacitorjs.com/docs/ios/spm#using-spm-in-an-existing-capacitor-project).

Vous mettez à jour depuis la v3 ? Consultez [Migrer le SDK Adapty Capacitor vers la v4.1](migration-to-capacitor-sdk-v4) pour la liste complète des changements d'API.
...
:::important
Attendez que `activate` soit résolu avant d'appeler toute autre méthode du SDK Adapty. Consultez [Ordre des appels dans le SDK Capacitor](capacitor-sdk-call-order) pour la séquence complète.
:::
...
- Si vous utilisez le [Flow & Paywall Builder](adapty-flow-builder), suivez le [guide de démarrage rapide](capacitor-quickstart-paywalls).
...
Si vous souhaitez utiliser le [Flow & Paywall Builder](adapty-flow-builder), vous aurez besoin du module AdaptyUI. Il est activé automatiquement en même temps que le module principal ; vous n'avez rien d'autre à faire.
...
Si vous utilisez [Adapty Attribution](user-acquisition), définissez `adaptyAttributionEnabled` sur `true` lors de l'activation du SDK. La valeur par défaut est `false` : sans ce paramètre, le SDK n'enregistre pas les installations et ne transmet pas les informations d'installation à votre application. Dans les versions du SDK antérieures à 4.1, Adapty Attribution est activé automatiquement.
...
:::note
Lorsque cette option est activée, le SDK ignore l'appel d'activation complet une fois qu'il a été activé. Ainsi, les modifications apportées aux paramètres d'activation ne prennent pas effet lors d'un rechargement à chaud. Pour appliquer de nouveaux paramètres d'activation, fermez complètement l'application et relancez-la.
:::
...
:::note
Ceci s'applique aux projets basés sur CocoaPods avec le **SDK 3.x**. Le SDK 4 s'installe sur iOS via Swift Package Manager uniquement (il n'y a pas de `Podfile`) et nécessite iOS 15.0 — définissez votre cible de déploiement à 15.0 dans Xcode.
:::
...
:::note
Cela s'applique aux projets basés sur CocoaPods avec le **SDK 3.x**. Le SDK 4 installe les SDK natifs via Swift Package Manager, donc il n'y a pas de `Podfile` à modifier.
:::
