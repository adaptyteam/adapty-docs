...
Dans SDK v4, vous ne passez pas de code de langue lorsque vous récupérez un flow — `getFlow` retourne le flow avec toutes ses localisations.

- **Flows construits dans le builder** : `createFlowView` ne prend pas de paramètre de locale, et le SDK ne lit pas la locale de l'appareil. Le flow s'affiche dans sa [locale par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — sur iOS, en `en` lorsque le flow dispose d'une localisation en anglais. Pour confirmer quelle localisation Adapty a appliquée, lisez `view.locale` sur le contrôleur renvoyé par `createFlowView`. Cela nécessite React Native SDK 4.0.2 ou une version ultérieure, et est `undefined` sur les versions antérieures.
- **Paywalls personnalisés (Remote Config)** : `getFlow` renvoie chaque localisation configurée dans `flow.remoteConfigs`. Chaque entrée contient un code `lang` et un objet `data`. Sélectionnez l'entrée correspondant à l'utilisateur, avec votre propre fallback :
