...
- **Flows créés dans le builder** : `createFlowView` ne prend pas de paramètre de locale, et le SDK ne lit pas la locale de l'appareil. Le flow s'affiche dans sa [locale par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — sur iOS, en `en` quand le flow a une localisation en anglais.
- **Paywalls personnalisés (Remote Config)** : `getFlow` renvoie toutes les localisations configurées dans `flow.remoteConfigs`. Chaque entrée contient un code `lang` et un objet `data`. Sélectionnez l'entrée qui correspond à l'utilisateur, avec votre propre fallback :
