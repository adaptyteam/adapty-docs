...
:::note
**API ou SDK ?** La plupart des applications envoient des données à Adapty Mail via le SDK Adapty, qui collecte automatiquement les e-mails et les achats — voir [Connecter Adapty à Adapty Mail](mail-collect-emails). Choisissez l'API lorsque votre application n'intègre pas le SDK Adapty, que les données se trouvent déjà sur votre serveur, ou que vous importez des abonnés depuis une autre source. Vous pouvez également utiliser l'API en parallèle d'un autre point d'entrée : Adapty Mail rapproche les données entrantes sur le `customer_user_id` et l'adresse e-mail, et conserve un seul profil par personne.
:::
...
Identifiez le profil par `external_profile_id`, `customer_user_id` ou `email` — envoyez au moins un de ces identifiants. La suppression est définitive : une sauvegarde ultérieure portant les mêmes identifiants ne recrée pas le profil.

Pour sécuriser une nouvelle tentative, identifiez le profil par `external_profile_id` ou `customer_user_id`. Répéter une suppression effectuée uniquement avec `email` renvoie `404`, car la suppression efface l'adresse stockée. Consultez la référence [Supprimer un profil](api-mail/operations/deleteProfile) pour plus de détails.
