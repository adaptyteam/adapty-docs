---
title: "Migrer le SDK Adapty React Native vers la v4.0"
description: "Migrez vers le SDK Adapty React Native v4.0 en remplaçant les API paywall par des API flow, qui affichent les flows en plus des anciens paywalls du builder."
metadataTitle: "Migration vers le SDK Adapty React Native v4.0 | Documentation Adapty"
---

Le SDK Adapty React Native 4.0 introduit les flows et renomme les API paywall en conséquence. Les nouvelles API fonctionnent avec les flows et restent compatibles avec les paywalls de l'ancien builder — aucune modification de configuration n'est nécessaire côté Adapty Dashboard.

:::note
Ce guide couvre la migration 3.x → 4.0. Le SDK 4.1 modifie quelques API supplémentaires — l'Attribution Adapty devient opt-in, les méthodes d'attribution externe sont renommées, et le format du fichier de secours change à nouveau. Une fois cette étape terminée, consultez [Migrer vers la v4.1](migration-to-react-native-sdk-41).
:::
...
[Le dépôt de specs CocoaPods devient en lecture seule en décembre 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/), donc à partir de la v4, les SDK natifs `Adapty`, `AdaptyUI` et `AdaptyPlugin` **ne sont plus récupérés en tant que sous-dépendances CocoaPods** — le podspec les récupère via **Swift Package Manager** (grâce au helper `spm_dependency`). Trois points à prendre en compte :

- **Xcode 26 ou version ultérieure** — le manifeste du package du SDK iOS natif déclare Swift tools 6.2, que les versions antérieures d'Xcode ne peuvent pas lire.
- **React Native 0.75 ou version ultérieure** — nécessaire pour le helper podspec `spm_dependency`. Sur une version plus ancienne, `pod install` échoue avec une erreur explicite ; mettez d'abord à jour React Native, ou restez sur `react-native-adapty` 3.x.
- **Frameworks dynamiques** — les dépendances SPM requièrent une liaison dynamique. La façon d'activer cela diffère selon que vous utilisez Expo ou React Native nu.

#### Expo

Ajoutez le plugin de configuration [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) et configurez les frameworks iOS en mode dynamique dans `app.json` (ou `app.config.js`) :
...
Si le build iOS échoue ensuite avec `'React/RCTBridge.h' file not found`, mettez à jour `expo-modules-autolinking` vers la version 57.0.10 ou ultérieure (Expo SDK 57) ou 56.0.22 ou ultérieure (Expo SDK 56) — consultez [l'entrée de dépannage](sdk-installation-react-native-expo#react-header-not-found) pour les détails et une solution de contournement.
...
Si vous avez précédemment intégré `Adapty`, `AdaptyUI` ou `AdaptyPlugin` en tant que sous-dépendances CocoaPods, supprimez d'abord toute ligne explicite `pod 'Adapty'`, `pod 'AdaptyUI'` ou `pod 'AdaptyPlugin'` de votre `Podfile`.

:::warning
Passer du linkage statique par défaut aux frameworks dynamiques peut entrer en conflit avec des bibliothèques qui ne prennent pas encore en charge les en-têtes modulaires, et est incompatible avec Flipper. Si vous rencontrez des problèmes de build, consultez cet [article sur l'intégration de Swift Package Manager avec les bibliothèques React Native](https://www.callstack.com/blog/integrating-swift-package-manager-with-react-native-libraries).
:::

Voir [Installer le SDK Adapty](sdk-installation-reactnative) pour la configuration complète.
...
| `products` | `flow.paywalls[i].productIdentifiers` | Les identifiants de produits se trouvent désormais sur chaque variante du flow, et non sur le flow lui-même. |
...
| `hasViewConfiguration` | supprimé en 4.0, rétabli en 4.1 | Retirez tout contrôle `hasViewConfiguration` de votre code lorsque vous êtes sur la 4.0. L'indicateur revient sur `AdaptyFlow` en 4.1 — voir [Migrer vers la v4.1](migration-to-react-native-sdk-41#hasviewconfiguration-returns-to-adaptyflow). |
...
Comme dans la v3, vous n'avez pas besoin d'appeler cette méthode lors de l'affichage de flows ou de paywalls rendus par Adapty — Adapty enregistre automatiquement ces vues.
...
L'ancienne API onboarding est dépréciée à partir de la v4.0 au profit du [Flow & Paywall Builder](adapty-flow-builder). Elle fonctionne toujours, mais sera supprimée dans une prochaine version — planifiez donc la migration de vos onboardings vers le Flow & Paywall Builder.
