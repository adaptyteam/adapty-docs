...
Un paywall rendu par Adapty est livré sous forme de flow dans le SDK v4, donc la règle ci-dessous couvre les deux.
...
Lorsque le code ne correspond à aucune localisation, le flow s'affiche silencieusement dans sa [langue par défaut](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — le SDK ne retourne pas d'erreur et n'enregistre pas d'avertissement.

Lorsque le code correspond, Adapty fusionne la localisation avec celle par défaut : les chaînes et ressources que la localisation trouvée ne définit pas sont héritées de la localisation par défaut.

Omettre le code de langue n'est pas la même chose que demander la localisation par défaut du flow : le SDK substitue un `en` fixe. Un flow dont la langue par défaut est `de` s'affiche quand même en `en` s'il possède une localisation `en`, et ne bascule vers `de` que si ce n'est pas le cas.

:::warning
Passez le code de langue exactement tel qu'il est configuré dans le tableau de bord — sous-balises en minuscules séparées par des tirets. Ne passez pas tel quel un identifiant de locale système : `navigator.language` renvoie `pt-BR`, ce qui fait basculer vers la localisation par défaut. Convertissez la valeur dans votre application avant de la transmettre.
:::
...
`view.locale` indique la localisation avec laquelle la vue a été construite. L'option `locale` et `view.locale` nécessitent le SDK Capacitor 4.1 ou une version ultérieure, et `view.locale` est `undefined` sur les versions antérieures. Le [gestionnaire `onAppeared`](capacitor-handling-events#the-view-argument) renvoie la même valeur.
- **Paywalls personnalisés (Remote Config)** : `getFlow` renvoie toutes les localisations configurées dans `flow.remoteConfigs`. Chaque entrée contient un code `lang` et un objet `data`. Sélectionnez l'entrée correspondant à l'utilisateur, avec votre propre solution de secours :
