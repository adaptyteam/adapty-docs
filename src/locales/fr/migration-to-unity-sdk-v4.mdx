...
:::note
`CustomTimers` existe toujours, mais il n'affecte que les paywalls créées avec le Paywall Builder legacy. Le [compte à rebours](flow-timer) d'un flow est géré par les paramètres définis dans le Flow & Paywall Builder, donc un flow ignore ce que vous passez ici.
:::
...
L'ancienne API onboarding est dépréciée dans la v4 au profit du [Flow & Paywall Builder](adapty-flow-builder). Elle fonctionne toujours, mais sera supprimée dans une future version — prévoyez donc la migration de vos onboardings vers le Flow & Paywall Builder.
