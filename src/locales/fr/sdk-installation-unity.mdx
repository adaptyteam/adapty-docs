...
Le SDK Adapty prend en charge iOS 13.0+, mais requiert iOS 15.0+ pour fonctionner avec les paywalls créés dans le Paywall Builder. Adapty SDK 4.1 — qui ajoute la prise en charge du [Flow Builder](adapty-flow-builder) — requiert **iOS 15.0+** pour l'ensemble de l'application : un validateur de build dans l'Unity Editor bloque le build iOS si la cible de déploiement est inférieure.
...
SDK 4.1 — qui ajoute la prise en charge du [Flow Builder](adapty-flow-builder) — est la première version stable de la ligne 4.x. Pour l'installer via le Unity Package Manager, ajoutez le tag de version à l'URL Git :
