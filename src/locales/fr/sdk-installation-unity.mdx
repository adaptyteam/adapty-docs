...
Le SDK Adapty prend en charge iOS 13.0+, mais nécessite iOS 15.0+ pour fonctionner avec les paywalls créés dans le Paywall Builder. Adapty SDK 4.1 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — exige **iOS 15.0+** pour l'ensemble de l'application : un validateur de build dans l'éditeur Unity bloque la compilation iOS si la cible de déploiement est inférieure. La compilation pour iOS avec le SDK 4.x nécessite également **Xcode 26** ou une version ultérieure — le SDK iOS natif qu'il ajoute en tant que Swift package est compilé avec Swift tools 6.2.

:::info
Adapty Unity SDK 4.0 et versions ultérieures fonctionne avec Google Play Billing Library v8.

La compatibilité avec une version de la Billing Library ne signifie pas qu'Adapty prend en charge toutes les fonctionnalités introduites par Google dans cette version. Avant d'adopter une nouvelle fonctionnalité de facturation Google Play, consultez [Produit dans le Play Store](android-products).
:::
