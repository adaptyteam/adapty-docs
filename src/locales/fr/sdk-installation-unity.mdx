...
Le SDK Adapty est compatible avec iOS 13.0+, mais nécessite iOS 15.0+ pour fonctionner avec les paywalls créés dans le Paywall Builder. Le SDK Adapty 4.1 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — requiert **iOS 15.0+** pour l'ensemble de l'application : un validateur de build dans l'éditeur Unity bloque le build iOS si la cible de déploiement est inférieure. La compilation pour iOS avec le SDK 4.x nécessite également **Xcode 26** ou une version ultérieure — le SDK iOS natif qu'il ajoute en tant que package Swift est compilé avec Swift tools 6.2.

:::info
À partir de la version 3.17.0, le SDK Adapty pour Unity utilise Billing Library v8 par défaut. Les versions 3.15.0 à 3.15.2 utilisent la v7, et fonctionnent également lorsqu'une autre dépendance dans votre build Android fait passer la Billing Library à la v8.
