...
Le SDK Adapty comprend deux modules clés pour une intégration transparente dans votre application Unity :

- **Core Adapty** : Ce SDK essentiel est requis pour qu'Adapty fonctionne correctement dans votre application.
- **AdaptyUI** : Ce module affiche les flows, ainsi que les anciens paywalls créés avec le builder.

:::tip
Vous souhaitez voir un exemple concret d'intégration du SDK Adapty dans une application mobile ? Consultez notre [exemple d'application](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Assets), qui illustre la configuration complète, notamment l'affichage des paywalls, les achats et d'autres fonctionnalités de base.
:::

## Prérequis \{#requirements\}

Le SDK Adapty prend en charge iOS 13.0+, mais nécessite iOS 15.0+ pour fonctionner avec les paywalls créés dans le Paywall Builder. Le SDK Adapty 4.1 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — requiert **iOS 15.0+** pour l'ensemble de l'application : un validateur de build dans l'Unity Editor bloque le build iOS si la cible de déploiement est inférieure. La compilation pour iOS avec le SDK 4.x nécessite également **Xcode 26** ou une version ultérieure — le SDK iOS natif qu'il ajoute en tant que package Swift est compilé avec Swift tools 6.2.

:::info
Le SDK Adapty intègre sa propre copie de la Google Play Billing Library — vous n'avez pas à l'ajouter à votre application.

À partir de la version 3.17.0, le SDK Adapty Unity utilise Billing Library v8 par défaut. Les versions 3.15.0 à 3.15.2 utilisent la v7, et fonctionnent également quand une autre dépendance de votre build Android fait passer Billing Library à la v8.
...
Le SDK 4.1 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — est la première version stable de la ligne 4.x. Pour l'installer via l'Unity Package Manager, ajoutez le tag de version à l'URL Git :

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

Si vous installez via le package Unity, téléchargez `adapty-unity-plugin-4.1.0.unitypackage` depuis la [version 4.1.0](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0).

Deux changements de configuration s'appliquent avec la version 4.x — le SDK natif iOS Adapty est déclaré comme package Swift distant et ne s'installe plus via CocoaPods :

- Mettez à jour l'External Dependency Manager vers la version **1.2.188 ou ultérieure** — les versions antérieures ne prennent pas en charge les dépendances Swift Package Manager. C'est la version que le SDK 4.1 déclare comme dépendance pair, donc Unity vous avertit si votre projet utilise une version plus ancienne.
- Les étapes CocoaPods ci-dessus (`iOS Resolver -> Install Cocoapods`, ouverture de `Unity-iPhone.xcworkspace`) s'appliquent uniquement au SDK 3.x. Avec le SDK 4.1, EDM ajoute automatiquement le package Swift au projet Xcode généré.
- Définissez la cible de déploiement iOS à **15.0 ou ultérieure**. Un validateur de build dans l'Unity Editor bloque le build iOS dans le cas contraire.

Consultez le [guide de migration](migration-to-unity-sdk-v4) pour la liste complète des changements dans la version 4.x.
...
:::note
Dans le SDK 4.1, les interfaces de listeners suivent la convention C# avec préfixe I — implémentez `IAdaptyEventListener` au lieu de `AdaptyEventListener` — et l'interface requiert une méthode supplémentaire, `OnReceivePromotedPurchase`. Consultez le [guide de migration](migration-to-unity-sdk-v4).
:::

:::important
Attendez le callback de complétion d'`Activate` avant d'appeler toute autre méthode du SDK Adapty. Voir [Ordre d'appel dans le SDK Unity](unity-sdk-call-order) pour la séquence complète.
:::
...
- Si vous utilisez le [Flow & Paywall Builder](adapty-flow-builder), commencez par [activer le module AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) ci-dessous, puis suivez le [guide de démarrage rapide](unity-quickstart-paywalls).
...
Si vous prévoyez d'utiliser le [Flow & Paywall Builder](adapty-flow-builder) et que vous avez installé le module AdaptyUI, vous devez l'activer. Vous pouvez le faire lors de la configuration :
...
Si vous utilisez [l'attribution Adapty](user-acquisition), définissez `SetAdaptyAttributionEnabled` sur `true` lors de l'activation du SDK. La valeur par défaut est `false` : sans ce paramètre, le SDK n'enregistre pas les installations et ne transmet pas les détails d'installation à votre application. Dans les versions du SDK antérieures à la 4.1, l'attribution Adapty est activée automatiquement.
