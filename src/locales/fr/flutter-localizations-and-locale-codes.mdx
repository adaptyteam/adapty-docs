...
Omettre le code de langue ne revient pas à demander la localisation par défaut du flow : le SDK substitue un `en` fixe. Un flow dont la langue par défaut est `de` s'affichera quand même en `en` s'il dispose d'une localisation `en`, et ne se rabattra sur `de` que si ce n'est pas le cas.

:::warning
Passez le code de langue exactement tel qu'il est configuré dans le tableau de bord — sous-balises en minuscules séparées par des tirets. Ne passez pas un identifiant de locale système tel quel : `Platform.localeName` renvoie `pt_BR` et `PlatformDispatcher.instance.locale.toLanguageTag()` renvoie `pt-BR`, et les deux tombent en repli vers la localisation par défaut. Convertissez la valeur dans votre application avant de la passer.
:::
