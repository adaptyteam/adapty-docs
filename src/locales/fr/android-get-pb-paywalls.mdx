...
| **locale**      | optionnel | L'identifiant de la [localisation du flow](add-paywall-locale-in-adapty-paywall-builder) à utiliser pour le rendu de la vue, sous forme de code de langue avec un ou deux sous-tags séparés par `-` (p. ex. `en`, `pt-br`). Si omis, la vue s'affiche en `en`, ou dans la localisation par défaut du flow si celui-ci ne dispose pas de version `en`. Voir [Localisations et codes de langue](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>optionnel</p><p>par défaut : `null`</p><p>SDK 4.1+</p> | L'identifiant personnalisé d'une mise en page dans la configuration de mises en page du flow. Passez-le pour afficher cette mise en page spécifique plutôt que celle que le SDK sélectionne automatiquement en fonction du type d'appareil et de la taille d'écran. Si aucune mise en page ne correspond à l'identifiant, l'appel échoue avec une erreur no-view-configuration. Le Flow & Paywall Builder n'attribue pas encore d'identifiants de mise en page personnalisés, laissez donc ce paramètre non défini. |
