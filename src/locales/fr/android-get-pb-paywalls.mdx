...
| **locale**      | optionnel | L'identifiant de la [localisation du flow](add-paywall-locale-in-adapty-paywall-builder) utilisée pour afficher la vue, sous forme de code de langue avec un ou deux sous-tags séparés par `-` (ex. : `en`, `pt-br`). S'il est omis, la vue s'affiche en `en`, ou dans la localisation par défaut du flow si celui-ci ne dispose pas de version `en`. Voir [Localisations et codes de locale](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>optionnel</p><p>défaut : `null`</p><p>SDK 4.1+</p> | L'identifiant personnalisé d'une mise en page définie dans le Flow Builder. Passez-le pour afficher cette mise en page spécifique plutôt que celle que le SDK sélectionne automatiquement en fonction du type d'appareil et de la taille de l'écran. Si aucune mise en page ne correspond à l'identifiant, le flow se charge sans configuration de vue. |
| **loadTimeout** | défaut : 5 sec | Cette valeur limite le délai d'attente de cette méthode. Si le délai est dépassé, les données en cache ou le contenu de secours local sont renvoyés. Notez que dans de rares cas, cette méthode peut expirer légèrement après le délai spécifié dans `loadTimeout`, car l'opération peut impliquer différentes requêtes en coulisses. |
...
| **locale**      | optionnel | L'identifiant de la [localisation du flow](add-paywall-locale-in-adapty-paywall-builder) dans laquelle afficher la vue, sous forme de code de langue avec un ou deux sous-tags séparés par `-` (par ex., `en`, `pt-br`). Si omis, la vue s'affiche en `en`, ou dans la localisation par défaut du flow si celui-ci n'a pas de version `en`. Voir [Localisations et codes de langue](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>optionnel</p><p>par défaut : `null`</p><p>SDK 4.1+</p> | L'ID personnalisé d'un layout défini dans le Flow Builder. Passez-le pour afficher ce layout spécifique plutôt que celui que le SDK sélectionne automatiquement en fonction du type d'appareil et de la taille de l'écran. Si aucun layout ne correspond à cet ID, le flow se charge sans configuration de vue. |
| **loadTimeout** | par défaut : 5 sec | Cette valeur limite le délai d'attente de cette méthode. Si le délai est dépassé, les données en cache ou le fallback local sont retournés. Notez que dans de rares cas, cette méthode peut expirer légèrement après le délai indiqué dans `loadTimeout`, car l'opération peut impliquer plusieurs requêtes en arrière-plan. |
