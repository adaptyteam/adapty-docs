...
:::info
Adapty React Native SDK 4.0.3 et versions ultérieures fonctionnent avec Google Play Billing Library v8.
...
:::important
À partir de la v4, le SDK Adapty React Native ne prend plus en charge l'installation CocoaPods de ses dépendances natives. Si vous avez besoin de la v4 ou d'une version ultérieure (pour le [Flow & Paywall Builder](adapty-flow-builder)), suivez plutôt [SDK Adapty 4.0 : activer Swift Package Manager](#adapty-sdk-40-enable-swift-package-manager) ci-dessous.
:::
...
### Adapty SDK 4.0 : activer Swift Package Manager

React Native SDK 4.0 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — nécessite **React Native 0.75 ou version ultérieure**. Installez le SDK :
...
Avec les [frameworks dynamiques activés pour Adapty SDK 4.0](#adapty-sdk-40-enable-swift-package-manager), la compilation iOS peut échouer dans `expo-updates`, `@expo/ui` ou un autre package contenant des sources Objective-C :
