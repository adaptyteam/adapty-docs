---
title: "Installer et configurer le SDK React Native Adapty dans un projet Expo"
description: "Guide étape par étape pour installer le SDK React Native Adapty dans un projet Expo pour les applications basées sur des abonnements."
metadataTitle: "Installation du SDK Adapty dans un projet Expo React Native | Documentation Adapty"
...
- **AdaptyUI** : Ce module affiche les flows, ainsi que les anciens paywalls créés avec le builder. AdaptyUI est automatiquement activé avec le module principal.

Si vous avez besoin d'un tutoriel complet sur la façon d'implémenter les achats intégrés dans votre application React Native, consultez [cet article](https://adapty.io/blog/react-native-in-app-purchases-tutorial/).

:::tip
Vous voulez voir un exemple concret d'intégration du SDK Adapty dans une application Expo ? Consultez nos exemples d'applications :
- [Exemple de build dev Expo](https://github.com/adaptyteam/AdaptySDK-React-Native/tree/master/examples/FocusJournalExpo) pour toutes les fonctionnalités, y compris les vrais achats et le Paywall Builder
- [Exemple Expo Go & Web](https://github.com/adaptyteam/AdaptySDK-React-Native/tree/master/examples/ExpoGoWebMock) pour tester avec le mode mock
:::
...
| Prérequis | Version |
|---|---|
| React Native | 0.75 ou ultérieur |
| iOS | 15.0 ou ultérieur |
| Swift | 6.2 ou ultérieur, inclus avec Xcode 26 |

:::info
Le SDK Adapty embarque sa propre version de la Google Play Billing Library — inutile de l'ajouter à votre application.

À partir de la version 3.17.0, le SDK Adapty React Native utilise la Billing Library v8 par défaut. Les versions 3.15.0 à 3.15.6 utilisent la v7, et fonctionnent également lorsqu'une autre dépendance de votre build Android fait passer la Billing Library à la v8.
...
:::important
Le SDK React Native Adapty v4 et versions ultérieures livrent leurs SDK iOS natifs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) sous forme de packages Swift. CocoaPods les installe toujours dans un projet Expo, mais les packages Swift se lient dynamiquement — votre build a donc besoin de frameworks dynamiques. Consultez [Activer les frameworks dynamiques pour le SDK Adapty v4](#enable-dynamic-frameworks).
:::
...
### Activer les frameworks dynamiques pour le SDK Adapty v4 \{#enable-dynamic-frameworks\}

Le SDK React Native 4.0 — qui ajoute la prise en charge des [flows](adapty-flow-builder) — nécessite **React Native 0.75 ou version ultérieure**. Installez le SDK :

```sh
npx expo install react-native-adapty@^4.0.0
```

v4 récupère les SDK iOS natifs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) en tant que Swift packages plutôt que des sous-dépendances CocoaPods ([le dépôt de specs CocoaPods passe en lecture seule en décembre 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Les Swift packages se lient dynamiquement, ce que vous activez dans Expo avec le plugin [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/). Ajoutez-le à `app.json` (ou `app.config.js`) :
...
Ensuite, installez le plugin et regénérez le projet natif :
...
Si le build iOS échoue avec `'React/RCTBridge.h' file not found`, consultez [l'entrée de dépannage correspondante](#react-header-not-found).
...
Si vous prévoyez d'utiliser le [Flow & Paywall Builder](adapty-flow-builder), vous avez besoin du module AdaptyUI. Il est activé automatiquement lors de l'activation du module principal ; vous n'avez rien d'autre à faire.
...
Si vous utilisez [Adapty Attribution](user-acquisition), définissez `adaptyAttributionEnabled` sur `true` lors de l'activation du SDK. La valeur par défaut est `false` : sans ce paramètre, le SDK n'enregistre pas les installations et ne transmet pas les détails d'installation à votre application. Dans les versions du SDK antérieures à 4.1, Adapty Attribution est activé automatiquement.
...
- Il **n'affiche pas** les flows ni les paywalls de l'ancien builder (AdaptyUI).
- Les modules natifs d'Adapty sont **complètement contournés** — même l'absence de fichiers SDK natifs dans la build Xcode/Android ou une clé API invalide ne déclenchera pas d'erreurs.

Pour tester de vrais achats et des paywalls Paywall Builder, utilisez un Expo Dev Client / une build de production où le mode mock est automatiquement désactivé.
::::
...
:::note
Lorsque cette option est activée, le SDK ignore l'appel d'activation s'il a déjà été activé, de sorte que les modifications apportées aux paramètres d'activation n'ont aucun effet lors d'un rechargement rapide. Pour appliquer de nouveaux paramètres d'activation, fermez complètement l'application et relancez-la.
:::
...
:::important
Cette configuration ne respecte les exigences de sauvegarde que pour Adapty, AppsFlyer et expo-secure-store.
...
#### La compilation iOS échoue avec `'React/RCTBridge.h' file not found` \{#react-header-not-found\}

Avec les [frameworks dynamiques activés pour le SDK Adapty v4](#enable-dynamic-frameworks), la compilation iOS peut échouer dans `expo-updates`, `@expo/ui`, ou un autre package avec des sources Objective-C :

```
error: 'React/RCTBridge.h' file not found
```

Le problème est causé par un bug dans le paquet `expo-modules-autolinking` (une dépendance d'`expo`). Les versions concernées sont 57.0.5–57.0.9 (Expo SDK 57) et 56.0.19–56.0.21 (Expo SDK 56) ; le bug est corrigé dans les versions 57.0.10 et 56.0.22.

Pour corriger le build, mettez à jour `expo-modules-autolinking` vers la version corrigée et régénérez le projet natif :
...
Si la version corrigée n'est pas encore disponible pour votre version d'Expo SDK, compilez React Native depuis les sources à la place. Cela contourne le bug au prix de builds iOS plus longs :
