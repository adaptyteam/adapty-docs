...
metadataTitle: "Variables | Flows & paywalls | Documentation Adapty"
---

Les variables vous permettent d'afficher du contenu dynamique dans vos flows — prix des produits, détails des offres et autres données qui s'adaptent au contexte de chaque utilisateur. Utilisez-les pour contrôler la visibilité des éléments et personnaliser le contenu des écrans.

Pour ouvrir le panneau des variables, cliquez sur l'icône **&#123; &#125;** dans le panneau de gauche. Le panneau comporte trois onglets :

- **[Personnalisées](#custom-variables)** : Variables que vous créez et gérez vous-même.
- **[Produit](#product-variables)** : Variables intégrées qui récupèrent les données localisées des produits et des offres depuis le store.
- **[Élément](#element-variables)** : Variables liées aux états des éléments sur le canvas.
...
Les variables de produit extraient directement les données localisées des stores. Utilisez-les dans les champs de texte pour afficher les prix, titres et détails d'offre localisés — ou dans les conditions, pour afficher ou masquer du contenu selon l'éligibilité à une offre. Lorsque vous insérez une variable pour un produit spécifique, le builder ajoute ce produit à la liste de l'écran dans le [panneau **Products**](paywall-product-block#add-products-to-a-screen).
...
`offer_price`, `offer_billing_period` et `offer_full_duration` nécessitent un produit associé à une offre. Sans offre, le flow [ne se publie pas](flow-common-issues#a-flow-wont-preview-or-publish).

Toutes ces valeurs proviennent du store, donc l'aperçu du builder n'en résout qu'une partie : les valeurs d'offre restent non résolues sur le canvas, et `is_free_trial`, `is_pay_up_front` et `is_pay_as_you_go` suivent l'offre que vous avez sélectionnée sur la fiche produit. Consultez [Aperçu des produits et offres](paywall-device-compatibility-preview#preview-products-and-offers) pour prévisualiser chaque type d'offre pendant la conception.
...
Les variables d'éléments capturent les choix de l'utilisateur — ce qu'il a sélectionné dans les quiz, l'onglet sur lequel il se trouve et si une bascule est activée.

Les types de variables d'élément dépendent du groupe :
- **Single choice** : Quiz à choix unique, onglets et bascules :
    - `selectedOptionId` : ID de l'élément à utiliser dans les conditions
    - `selectedOptionTitle` : Titre de l'élément à utiliser dans le texte dynamique
- **Multi-choice** : Quiz à choix multiple :
    - `selectedOptionIds` : IDs des éléments à utiliser dans les conditions
    - `selectedOptionTitles` : Titres des éléments à utiliser dans le texte dynamique
- **Toggle** : Éléments qui basculent entre activé et désactivé :
    - `isActive` : Valeur booléenne
- **Product** : Cartes produit :
...
Pour insérer une variable dans un élément de texte :
...
4. Sélectionnez une variable. Dans l'onglet **Product**, les produits déjà listés dans le panneau **Products** apparaissent en premier. Le reste de votre catalogue suit sous **Other products**.
