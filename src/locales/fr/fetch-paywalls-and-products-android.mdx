---
title: "Récupérer les paywalls et produits pour les paywalls Remote Config dans le SDK Android"
...
Avant de présenter les Remote Config et les paywalls personnalisés, vous devez récupérer les informations les concernant. Veuillez noter que cette rubrique concerne les Remote Config et les paywalls personnalisés. Pour obtenir des instructions sur la récupération des flows ou des paywalls personnalisés dans le **Flow & Paywall Builder** ou l'**old Paywall Builder**, consultez [Récupérer les flows & paywalls](android-get-pb-paywalls).
...
:::tip
Pour remplir ce cache avant que l'utilisateur n'accède au flow, appelez [`preloadFlows`](android-optimize-paywall-fetching#preload-placements) (SDK Android 4.1+) pour plusieurs placements à la fois. Un appel ultérieur à `getFlow` lit la copie préchargée uniquement avec `AdaptyPlacementFetchPolicy.ReturnCacheDataElseLoad`.
:::
