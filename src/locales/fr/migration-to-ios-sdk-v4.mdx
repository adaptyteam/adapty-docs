...
Dans le cadre de la migration vers StoreKit 2, le SDK Adapty iOS 4.0 supprime la prise en charge des achats intégrés promus sur l'App Store. La méthode déléguée `shouldAddStorePayment(for:)` et le type `AdaptyDeferredProduct` qu'elle reçoit ne sont pas disponibles dans la version 4.0.

:::warning
Cette suppression ne concerne que la version 4.0 — la prise en charge des achats intégrés promus est de retour dans le SDK 4.1 avec une nouvelle API. Si votre application s'appuie sur des achats intégrés promus, migrez directement de la version 3.x vers la 4.1. Voir [Achats intégrés depuis l'App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
