...
description: "Migrez vers le SDK iOS Adapty v4.0 en remplaçant les API paywall par des API flow, qui affichent les flows en plus des anciens paywalls builder."
metadataTitle: "Migration vers le SDK iOS Adapty v4.0 | Documentation Adapty"
---

Le SDK iOS Adapty 4.0 introduit les flows et renomme les API paywall en conséquence. Les nouvelles API fonctionnent avec les flows, et elles restent compatibles avec les paywalls de l'ancien builder — aucune modification de configuration n'est nécessaire côté Adapty Dashboard.
...
Adapty iOS SDK 4.0 abandonne la prise en charge de CocoaPods. Installez le SDK avec [Swift Package Manager](sdk-installation-ios#install-adapty-sdk). La v4 requiert également **Xcode 26** ou une version ultérieure, car son manifeste de package déclare Swift tools 6.2.

Si votre projet utilise encore CocoaPods, supprimez les pods `Adapty` et `AdaptyUI` de votre `Podfile`, exécutez `pod install` pour les retirer, puis ajoutez le package dans Xcode via **File → Add Package Dependency** en utilisant `https://github.com/adaptyteam/AdaptySDK-iOS.git`.
...
## Gestion des achats promus sur l'App Store temporairement supprimée \{#app-store-promoted-in-app-purchases-temporarily-removed\}

Dans le cadre de la migration vers StoreKit 2, le SDK Adapty iOS 4.0 supprime l'API permettant de gérer les achats intégrés promus sur l'App Store : la méthode delegate `shouldAddStorePayment(for:)` et le type `AdaptyDeferredProduct` qu'elle reçoit ne sont pas disponibles dans la version 4.0. Ces achats s'effectuent toujours et Adapty enregistre la transaction, mais la version 4.0 ne donne à votre application aucun moyen de les intercepter ou de les différer.

:::warning
Cette suppression ne concerne que la version 4.0 — la gestion est rétablie dans le SDK 4.1 avec une nouvelle API. Si votre application intercepte les achats intégrés promus, migrez directement de la version 3.x vers la 4.1. Consultez [Achats intégrés depuis l'App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
...
Les types retournés passent de `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` à `AdaptyFlow` / `AdaptyUI.FlowConfiguration`. Le paramètre `locale` quitte l'appel de récupération et se déplace vers `getFlowConfiguration` : `AdaptyFlow` expose également `paywalls` — les variantes de paywall du flow, sous forme de valeurs `AdaptyFlowPaywall`. Les méthodes de paywall web acceptent l'une de ces valeurs.
...
`logShowPaywall` est renommé en `logShowFlow` et prend désormais un `AdaptyFlow` au lieu d'un `AdaptyPaywall`. L'événement est toujours enregistré sur la même variation, de sorte que les métriques existantes des entonnoirs et des tests A/B continuent de fonctionner sans modification du tableau de bord.
...
:::note
SDK 4.1 renomme cette méthode en `updateExternalAttribution(_:provider:)` et le type en `AdaptyExternalAttributionProvider`. Les anciens noms sont supprimés dans la version 4.1 sans passer par une dépreciation. Si vous migrez directement vers la version 4.1, consultez [Migrer vers v4.1](migration-to-ios-sdk-41).
:::
