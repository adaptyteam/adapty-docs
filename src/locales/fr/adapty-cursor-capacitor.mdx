...
Une fois le skill ajoutée, exécutez `/adapty-cli` dans votre agent. Il vous guidera à chaque étape — y compris lorsqu'il faudra ouvrir le Dashboard pour connecter vos stores.
...
4. **Créez un flow ou un paywall et un placement** : Dans l'Adapty Dashboard, créez un flow (ou un paywall si vous construisez l'interface vous-même), puis assignez-le à un placement sur la page **Placements**. Dans le code, l'identifiant du placement est la chaîne que vous passez à `adapty.getFlow()`.
...
### Configurer selon l'avancement

Ces éléments ne sont pas nécessaires pour commencer à coder, mais vous en aurez besoin à mesure que votre intégration arrive à maturité :
...
- [**Paywalls créées manuellement**](capacitor-making-purchases) : vous construisez votre propre interface de paywall dans le code, mais utilisez toujours Adapty pour récupérer les produits et gérer les achats.
...
Vous ne savez pas laquelle choisir ? Consultez le [tableau comparatif dans le guide de démarrage rapide](capacitor-quickstart-paywalls).
...
Testez chaque achat dans le sandbox au fur et à mesure — n'attendez pas la fin. Consultez [Tester les achats en sandbox](test-purchases-in-sandbox) pour les instructions de configuration.
