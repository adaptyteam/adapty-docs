...
:::info
Par défaut, Adapty SDK 4.0.2 et versions ultérieures fonctionne avec Google Play Billing Library v8. Les versions antérieures du SDK fonctionnent avec v7.0.0 par défaut et sont compatibles avec la Billing Library jusqu'à la version 8.x — pour forcer une version ultérieure, ajoutez manuellement la [dépendance](https://developer.android.com/google/play/billing/integrate#dependency).
:::
...
Par défaut, AdaptyUI met en cache les médias (images, vidéos, etc.) pour améliorer les performances et réduire l'utilisation du réseau. Vous pouvez personnaliser ces paramètres en fournissant une configuration personnalisée.

Utilisez `AdaptyUI.configureMediaCache` pour remplacer la taille de cache et la durée de validité par défaut. Cette étape est facultative — si vous n'appelez pas cette méthode, les valeurs par défaut seront utilisées (100 Mo sur disque, 7 jours de validité).
