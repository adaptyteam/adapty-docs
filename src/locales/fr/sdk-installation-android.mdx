...
Le SDK Adapty comprend deux modules clés pour une intégration transparente dans votre application mobile :

- **Core Adapty** : Ce SDK essentiel est requis pour qu'Adapty fonctionne correctement dans votre application.
- **AdaptyUI** : Ce module affiche les flows, en plus des paywalls de l'ancien builder. AdaptyUI est automatiquement activé avec le module core.
...
:::info
Le SDK Adapty intègre sa propre copie de la Google Play Billing Library — vous n'avez pas besoin de l'ajouter à votre application.

À partir de la version 3.17.3 de la branche 3.x et de la version 4.0.2 de la branche 4.x, le SDK Adapty Android utilise la Billing Library v8 par défaut. Les versions 3.15.0 à 3.17.2 et 4.0.0 à 4.0.1 utilisent la v7, et fonctionnent également lorsqu'une autre dépendance de votre build élève la Billing Library à la v8.
...
:::important
Attendez que `Adapty.activate` se termine avant d'appeler toute autre méthode du SDK Adapty. Consultez [l'ordre des appels dans le SDK Android](android-sdk-call-order) pour la séquence complète.
:::

Configurez maintenant les paywalls dans votre application :

- Si vous utilisez le [Flow & Paywall Builder](adapty-flow-builder), suivez le [guide de démarrage rapide](android-quickstart-paywalls).
- Si vous créez votre propre interface de paywall, consultez le [guide de démarrage rapide pour les paywalls personnalisés](android-quickstart-manual).
