...
Quelle que soit la date à laquelle vous consultez le graphique, le nombre de référence (100 %) reste le même pour la période sélectionnée. En revanche, la rétention vers la période suivante peut augmenter avec le temps.
...
### Produits avec des durées différentes \{#products-with-different-durations\}
...
N'hésitez pas à combiner filtres et regroupements pour des analyses avancées. Obtenez de véritables insights à partir des données.
Variez :
...
5. L'attribution.
6. Le store.
7. Le flow et l'écran de flow.

Utilisez le contrôle #Absolu et %Relatif pour afficher les données souhaitées.
