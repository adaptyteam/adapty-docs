---
title: "Importar datos históricos a Adapty"
description: "Importa datos históricos a Adapty para obtener análisis detallados."
metadataTitle: "Importar datos históricos a Adapty | Documentación de Adapty"
---

Tras instalar el SDK de Adapty y publicar tu app, puedes acceder a tus usuarios y suscriptores en la sección [Profiles](profiles-crm). Pero ¿qué ocurre si tienes una infraestructura heredada y necesitas migrar a Adapty, o simplemente quieres ver tus datos existentes en Adapty?

:::note
La importación de datos no es obligatoria

Adapty otorgará automáticamente niveles de acceso a los usuarios históricos y restaurará sus eventos de compra en cuanto abran la app con el SDK de Adapty integrado. Para este caso de uso, no es necesario importar datos históricos. Sin embargo, importar los datos garantiza unas analíticas precisas si tienes un volumen significativo de transacciones históricas, aunque en general no es un requisito para la migración.

Una vez que el soporte importa el `apple_original_transaction_id` o el `google_purchase_token` de una transacción, Adapty asocia automáticamente las futuras notificaciones del servidor de App Store y Play para esa transacción al mismo perfil — el usuario no necesita abrir la app primero.
:::

:::warning
Si un webhook u otra integración otorga recompensas (por ejemplo, moneda virtual) en un evento de suscripción, activa **Exclude Historical Events** para esa integración en los [ajustes de integración](configuration) antes de iniciar la importación. Sin esto, las compras históricas importadas vuelven a disparar eventos de integración, y un webhook que otorga recompensas concede la recompensa por segunda vez para una compra que el usuario ya tiene. Consulta [Migrar integraciones a Adapty](migrate-integrations-to-adapty#exclude-historical-events).
:::

Para importar datos a Adapty:

1. Exporta tus transacciones a un archivo CSV (deben proporcionarse archivos separados para iOS, Android y Stripe). Consulta la [sección de formato del archivo de importación](importing-historical-data-to-adapty#import-file-format) a continuación para conocer los requisitos detallados.
...
4. Para las transacciones de iOS, asegúrate de que la sección **In-app purchase API** en [**App settings**](https://app.adapty.io/settings/ios-sdk) esté completada con el **Issuer ID**, el **Key ID** y la **Private key** (archivo .P8), incluso si usas StoreKit 1. Consulta las secciones [Provide Issuer ID and Key ID](app-store-connection-configuration#step-2-provide-issuer-id-and-key-id) y [Upload In-App Purchase Key file](app-store-connection-configuration#step-3-upload-in-app-purchase-key-file) para obtener instrucciones detalladas.
5. Comparte los enlaces con nuestro equipo por [correo electrónico](mailto:support@adapty.io) o a través del chat en línea en el Adapty Dashboard.

No te preocupes, importar datos históricos no creará duplicados, aunque esos datos se superpongan con entradas ya existentes en Adapty.
...
- [ ] Archivos separados para las importaciones de Android, iOS y Stripe.
...
- [ ] Los encabezados de las columnas son exactamente los que aparecen en la columna **Column name** de la tabla siguiente. Comprueba que no haya errores tipográficos.
- [ ] Las columnas que no son obligatorias pueden estar ausentes del archivo. No añadas columnas vacías para los datos que no tengas.
...
### Importar datos a Adapty \{#import-data-to-adapty\}

Por favor, contáctanos y comparte tus archivos de importación a través de [support@adapty.io](mailto:support@adapty.io) o mediante el chat en línea en el [Adapty Dashboard](https://app.adapty.io/overview).
