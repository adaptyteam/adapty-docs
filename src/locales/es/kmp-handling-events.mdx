...
Los flows y paywalls que renderiza Adapty no necesitan código adicional para realizar y restaurar compras. Sin embargo, generan algunos eventos a los que tu app puede responder. Esos eventos incluyen pulsaciones de botones (botones de cierre, URLs, selecciones de productos, etc.), así como notificaciones sobre acciones relacionadas con compras. A continuación aprenderás cómo responder a estos eventos.
...
En una situación normal, estos errores no deberían ocurrir, así que si te encuentras con uno, por favor, háznoslo saber.
...
Kotlin Multiplatform te entrega los parámetros del evento como una cadena JSON sin procesar, así que decodifica `paramsJsonString` antes de leer cualquiera de ellos.

<FlowScreenShowedParams />

Consulta [Rastrear vistas de pantallas del flow](kmp-flow-screen-views) para saber qué hacer con ellos.

Un flow también registra los valores que los usuarios escriben y seleccionan. Consulta [Procesar datos de flows](kmp-flow-input) para leerlos.
