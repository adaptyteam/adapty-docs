...
description: "Personaliza tu flow con un payload JSON de Remote Config."
metadataTitle: "Personalizar flow con Remote Config | Documentación de Adapty"
keywords: ['remote config', 'remote', 'flow builder', 'flow']
---

:::important
Esta guía cubre Remote Config para el Flow & Paywall Builder. Para paywalls clásicos creados sin él, consulta [Diseñar paywall con Remote Config](customize-paywall-with-remote-config).
:::
...
Para abrir el Remote Config de un flow, haz clic en el icono <Inline id="remote-config.svg" alt="Remote Config" /> que aparece encima de la vista previa de la pantalla en el builder.

<ZoomImage id="flow-remote-config-access.webp" width="300px" alt="Open remote config in the Flow & Paywall Builder editor" />

En la vista **JSON**, puedes introducir cualquier dato con formato JSON. El editor muestra una columna por cada idioma que hayas añadido:

<ZoomImage id="flow-remote-config-json.webp" width="700px" alt="JSON view of the flow remote config" />

:::warning
Si el Remote Config contiene JSON no válido, no podrás **guardar** ni **publicar** el flow. Consulta [Problemas comunes](flow-common-issues#a-flow-wont-preview-or-publish) para ver la lista completa de problemas que bloquean la vista previa y la publicación.
:::

Más adelante puedes acceder a estos datos desde el SDK a través del array `remoteConfigs` en `AdaptyFlow`. Adapty almacena una entrada `AdaptyRemoteConfig` por idioma; elige la que coincida con el idioma del usuario y lee el `dictionary` ya parseado o el `jsonString` en bruto para ajustar tu flow en tiempo de ejecución. A continuación encontrarás algunos ejemplos de cómo puedes usar un Remote Config.
...
Puedes combinar cualquiera de estos patrones o definir tus propias claves para probar textos, diseños o comportamientos alternativos.

A continuación, [crea un placement](create-placement) y añade el flow a él. Luego renderiza el flow en tu app: [iOS](present-remote-config-paywalls), [Android](present-remote-config-paywalls-android), [React Native](present-remote-config-paywalls-react-native), [Flutter](present-remote-config-paywalls-flutter), [Unity](present-remote-config-paywalls-unity), [Kotlin Multiplatform](present-remote-config-paywalls-kmp), o [Capacitor](present-remote-config-paywalls-capacitor).
