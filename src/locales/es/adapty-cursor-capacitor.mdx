...
4. **Crea un flow o paywall y un placement**: En el Adapty Dashboard, crea un flow (o un paywall si vas a construir la interfaz tú mismo), luego asígnalo a un placement en la página **Placements**. En el código, el ID del placement es el string que pasas a `adapty.getFlow()`.
...
- **Flows y placements adicionales**: Añade más llamadas `getFlow` con distintos IDs de placement.
...
Añade la dependencia del SDK de Adapty mediante npm y actívala con tu clave pública del SDK. Esta es la base: sin ella, nada más funciona.
