...
La interfaz principal del Flow Builder incluye todas las herramientas necesarias para añadir elementos visuales, editar sus propiedades y modificar la lógica del flow. Este artículo describe cada área de la interfaz: qué hace y dónde encontrarla.
...
* **Close** <Inline id="close.svg" alt="Close" />: Sale del editor de flows y vuelve a la página de flows.
* **App name** <Inline id="app.svg" alt="App" />: Identifica la app a la que pertenece el flow.
* **All flows** <Inline id="flows.svg" alt="Flows" />: Abre la lista de todos los flows de esta app.
* **Flow status**: El icono a la izquierda del nombre del flow indica el [estado del flow](builder-save-publish#flow-status) actual:
...
* **Save draft / Publish**: Haz clic en **Save draft** para guardar el progreso sin publicarlo (⌘ / Ctrl+S). Abre el desplegable <Inline id="collapse-layer.svg" alt="Open dropdown" /> para acceder al botón [**Publish**](builder-save-publish). Solo puedes añadir tu flow a un [placement](create-placement) después de publicarlo.
...
* Añade nuevas configuraciones regionales y edita las cadenas localizadas directamente.
* Selecciona la configuración regional predeterminada para tu flow.
* Controla el estado de traducción: cada fila aparece marcada como **Done** o **Missing**.
