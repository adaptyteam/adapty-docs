...
description: "Presenta flows y paywalls en apps de Flutter usando las funciones de monetización de Adapty."
...
Esta guía es para flows y paywalls renderizados por Adapty. Para presentar **paywalls de Remote Config**, consulta [Renderizar paywall diseñado por Remote Config](present-remote-config-paywalls-flutter).
...
### Cerrar el flow o paywall \{#dismiss-the-flow-or-paywall\}
...
Muestra esa vista de nuevo y el usuario vuelve a la pantalla donde estaba, con el estado que el flow había acumulado. Una vista conservada de esta forma se mantiene hasta que la descartes con `destroy: true`.
