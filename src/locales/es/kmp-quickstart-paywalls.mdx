---
title: "Habilitar compras con Flow & Paywall Builder en Kotlin Multiplatform SDK"
description: "Guía rápida para habilitar compras in-app con Adapty Flow & Paywall Builder."
...
- [**Placements**](placements) – dónde y cuándo muestras flows en tu app (como `main`, `onboarding`, `settings`). Asocias flows a placements en el dashboard y luego los solicitas por ID de placement en tu código. Esto facilita ejecutar pruebas A/B y mostrar diferentes flows a distintos usuarios.
...
|---|---|---|
| Adapty Flow & Paywall Builder | ✅ Fácil | [Creas un flow completo y listo para compras en el editor sin código](quickstart-paywalls). Adapty lo renderiza automáticamente y gestiona todo el proceso de compra, la validación de recibos y la administración de suscripciones. |
| Paywalls creados manualmente | 🟡 Media | Implementas la interfaz del paywall en el código de tu app, pero sigues obteniendo el objeto flow de Adapty para mantener flexibilidad en la oferta de productos. Consulta la [guía](kmp-quickstart-manual). |
...
1. Obtener el objeto `flow` a través del ID del [placement](placements) usando el método `getFlow`.

2. Crear la vista del flow con el método `createFlowView`. La vista contiene los elementos de interfaz y el estilo necesarios para mostrar el flow. Si el flow no tiene ninguna vista configurada, `createFlowView` devuelve un error — manéjalo en `onError`.
