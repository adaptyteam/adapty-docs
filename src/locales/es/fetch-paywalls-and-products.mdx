...
Antes de mostrar Remote Config y paywalls personalizados, necesitas obtener la información sobre ellos. Ten en cuenta que este tema hace referencia a Remote Config y paywalls personalizados. Para obtener orientación sobre cómo obtener flows o paywalls personalizados en el **Flow & Paywall Builder** o en el **old Paywall Builder**, consulta las <InlineTooltip tooltip="guías para obtener flows y paywalls en tu app">[iOS](get-pb-paywalls), [Android](android-get-pb-paywalls), [React Native](react-native-get-pb-paywalls), [Flutter](flutter-get-pb-paywalls), y [Unity](unity-get-pb-paywalls)</InlineTooltip>.
...
:::note
En la versión 4, el parámetro `locale` ha pasado de `getFlow` a `getFlowConfiguration` (que solo se usa al renderizar con AdaptyUI). En el caso de los paywalls personalizados, todos los locales disponibles se devuelven juntos en `flow.remoteConfigs` — elige el que coincida con el idioma del dispositivo del usuario o con la configuración de tu app.
:::

:::tip
Para llenar esa caché antes de que el usuario necesite el flow, llama a [`preloadFlows`](ios-optimize-paywall-fetching#preload-placements) (iOS SDK 4.1+) para varios placements a la vez. Una llamada posterior a `getFlow` leerá la copia precargada solo con `fetchPolicy: .returnCacheDataElseLoad`.
:::
...
| :-------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Flow      | Un objeto `AdaptyFlow` que contiene el placement, los identificadores (`id`, `variationId`), el nombre, sus variaciones de paywall (`paywalls`), un array `remoteConfigs` (una entrada por cada locale configurado) y un flag `hasViewConfiguration`. Para obtener los productos del flow, llama a `getPaywallProducts(flow:)`. |
