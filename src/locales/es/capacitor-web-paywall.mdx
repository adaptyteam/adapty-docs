...
1. `openWebPaywall({ paywallOrProduct: product })` que genera URLs por paywall y añade los datos del producto a las URLs también.
2. `openWebPaywall({ paywallOrProduct: paywall })` que genera URLs por paywall sin añadir los datos del producto a las URLs. Úsalo cuando los productos de tu paywall en Adapty sean distintos de los del paywall web.
:::
