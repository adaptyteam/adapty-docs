...
## Compras in-app promocionadas desde el App Store \{#in-app-purchases-from-the-app-store\}
...
`makePromotedPurchase` no recibe parámetros de compra — un producto promocionado proviene del App Store en lugar de un paywall, por lo que no tiene contexto de paywall. Devuelve el mismo [`AdaptyPurchaseResult`](https://capacitor.adapty.io/types/adaptypurchaseresult) que `makePurchase`.
