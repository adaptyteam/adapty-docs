...
description: "Guarda y usa datos de onboardings en tu app de Unity con el SDK de Adapty."
...
Cuando tus usuarios responden a una pregunta del quiz o introducen sus datos en un campo de texto, se invocará el método `OnboardingViewOnStateUpdatedAction`. Puedes guardar o procesar el tipo de campo en tu código.

Implementa el método `OnboardingViewOnStateUpdatedAction` en tu clase:

:::note
En el SDK 4.1, las interfaces de listener siguen la convención de prefijo I de C#: implementa `IAdaptyOnboardingsEventsListener` en lugar de `AdaptyOnboardingsEventsListener`. Los métodos no cambian. Consulta la [guía de migración](migration-to-unity-sdk-v4).
:::
