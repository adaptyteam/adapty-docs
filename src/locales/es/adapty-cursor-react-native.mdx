...
La skill de Adapty CLI permite que tu LLM configure tu app, productos, niveles de acceso, paywalls y placements directamente — sin necesidad de abrir el Dashboard en cada paso. Solo necesitas [conectar tus stores](integrate-payments) en el Dashboard.
...
4. **Crea un flow o paywall y un placement**: En el Adapty Dashboard, crea un flow (o un paywall si vas a construir la interfaz tú mismo), y luego asígnalo a un placement en la página **Placements**. En el código, el ID del placement es la cadena que pasas a `adapty.getFlow("YOUR_PLACEMENT_ID")`.
...
No son necesarios para empezar a escribir código, pero los necesitarás a medida que tu integración madure:
...
¿No sabes cuál elegir? Lee la [tabla comparativa en la guía de inicio rápido](react-native-quickstart-paywalls).
...
**Guías:**
- [Habilitar compras con Flow y Paywall Builder (inicio rápido)](react-native-quickstart-paywalls)
- [Obtener flows y su configuración](react-native-get-pb-paywalls)
- [Mostrar flows](react-native-present-paywalls)
- [Gestionar eventos de flow](react-native-handling-events-1)
- [Responder a acciones de botones](react-native-handle-paywall-actions)
...
:::tip[Checkpoint]
- **Expected:** El paywall aparece con los productos configurados. Al pulsar un producto se activa el diálogo de compra en sandbox.
- **Gotcha:** Flow vacío o error en `getFlow` → verifica que el ID del placement coincide exactamente con el del dashboard y que el placement tiene una audiencia asignada.
:::
...
- [`llms.txt`](https://adapty.io/docs/es/llms.txt): Lista todas las páginas con enlaces `.md`. Un [estándar emergente](https://llmstxt.org/) para hacer sitios web accesibles a LLMs. Ten en cuenta que para algunos agentes de IA (p. ej., ChatGPT) tendrás que descargar `llms.txt` y subirlo al chat como archivo.
