...
description: "Guía paso a paso para integrar Adapty en tu app React Native usando Cursor, Context7, ChatGPT, Claude u otras herramientas de IA."
...
Esta guía te lleva paso a paso por la integración de Adapty en tu app React Native con una herramienta de codificación con IA — le proporcionas la documentación correcta de Adapty en el orden correcto. Para un flujo completamente automatizado, usa el [skill adapty-sdk-integration](adapty-sdk-integration-skill-react-native) en su lugar.
...
Si prefieres configurarlo todo manualmente, esto es lo que necesitas antes de escribir código. Tu LLM no puede consultar los valores del dashboard por ti — tendrás que proporcionarlos tú mismo.

1. **Conecta tus stores**: En el Adapty Dashboard, ve a **App settings → General**. Conecta tanto App Store como Google Play si tu app es para ambas plataformas. Esto es necesario para que las compras funcionen.
...
4. **Crea un paywall y un placement**: En el Adapty Dashboard, crea un paywall en la página **Paywalls** y luego asígnalo a un placement en la página **Placements**. En el código, el ID del placement es el string que pasas a `adapty.getPaywall("YOUR_PLACEMENT_ID")`.
...
:::tip
Una vez que tengas los cinco, estarás listo para escribir código. Dile a tu LLM: "Mi clave SDK pública es X, mi ID de placement es Y" para que pueda generar el código correcto de inicialización y recuperación de paywall.
:::
...
[Context7](https://context7.com) es un servidor MCP que da a tu LLM acceso directo a la documentación actualizada de Adapty. Tu LLM obtiene automáticamente los documentos correctos según lo que preguntes — sin necesidad de pegar URLs manualmente.
...
:::warning
Aunque Context7 elimina la necesidad de pegar enlaces a la documentación manualmente, el orden de implementación es importante. Sigue el [proceso de implementación](#implementation-walkthrough) paso a paso para asegurarte de que todo funciona correctamente.
:::
...
Indica a tu LLM qué enfoque utilizas para las compras — esto afecta a las guías que debe seguir:
...
Añade la dependencia del SDK de Adapty usando npm (o yarn) y actívalo con tu clave pública del SDK. Esta es la base — sin ella, nada más funciona.

Tenemos guías de instalación separadas para proyectos Expo y React Native puro — elige la que se ajuste a tu configuración.
...
- **Esperado:** La app compila y se ejecuta en iOS y Android. Los logs de Metro bundler muestran el log de activación de Adapty.
- **Problema frecuente:** "Public API key is missing" → comprueba que has reemplazado el marcador de posición con tu clave real en **App settings**.
:::
...
:::tip[Checkpoint]
- **Esperado:** Aparece el paywall con tus productos configurados. Al tocar un producto se activa el diálogo de compra sandbox.
...
**Guías:**
- [Activar compras en tu paywall personalizado (quickstart)](react-native-quickstart-manual)
...
:::tip[Checkpoint]
- **Expected:** Tu paywall personalizado muestra los productos obtenidos desde Adapty. Al tocar un producto se activa el diálogo de compra en sandbox.
...
- [Implementar Observer mode](implement-observer-mode-react-native)
- [Reportar transacciones en Observer mode](report-transactions-observer-mode-react-native)
Envía esto a tu LLM:
```
Read these Adapty docs before writing code:
...
- **Problema frecuente:** Llama a `identify` después de la activación pero antes de obtener los paywalls para evitar la atribución anónima del perfil.
:::
### Prepararse para el lanzamiento \{#prepare-for-release\}

Una vez que tu integración funcione en sandbox, repasa la checklist de lanzamiento para asegurarte de que todo está listo para producción.
...
- [`llms.txt`](https://adapty.io/docs/es/llms.txt): Lista todas las páginas con enlaces `.md`. Es un [estándar emergente](https://llmstxt.org/) para hacer que los sitios web sean accesibles a los LLMs. Ten en cuenta que para algunos agentes de IA (p. ej., ChatGPT) necesitarás descargar `llms.txt` y subirlo al chat como archivo.
- [`llms-full.txt`](https://adapty.io/docs/es/llms-full.txt): Toda la documentación de Adapty combinada en un único archivo. Es muy grande — úsalo solo cuando necesites el panorama completo.
- [`react-native-llms.txt`](https://adapty.io/docs/es/react-native-llms.txt) y [`react-native-llms-full.txt`](https://adapty.io/docs/es/react-native-llms-full.txt) específicos de React Native: Subconjuntos por plataforma que ahorran tokens en comparación con el sitio completo.