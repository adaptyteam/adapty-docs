...
Si necesitas **desbloquear manualmente las funciones premium** para usuarios o grupos de usuarios específicos, puedes hacerlo usando la API de Adapty. Esto es útil para campañas promocionales, acceso de inversores o casos especiales de soporte al cliente.

En esta guía aprenderás a identificar usuarios y otorgarles niveles de acceso de forma programática.

#### Casos de uso de ejemplo \{#sample-use-cases\}

- **Códigos promocionales**: cuando los usuarios introducen un código promocional válido en tu app, otórgales acceso automáticamente a las funciones premium.
...
Adapty usa `customer_user_id` para identificar usuarios en todas las plataformas y dispositivos. Esto es fundamental para garantizar que los usuarios conserven su acceso después de reinstalar la app o cambiar de dispositivo.

Solo tienes que crear este ID una vez. Cuando los usuarios se registren por primera vez desde la app, puedes pasar su customer user ID durante la activación del SDK, o usar el método `identify` si el SDK se activó antes del registro.
...
Este comportamiento es normal y no afectará la precisión de los análisis. Lee más [aquí](ios-quickstart-identify).
:::
...
Una vez que un usuario está identificado con un `customer_user_id`, puedes otorgarle niveles de acceso usando la API del lado del servidor. Esta llamada a la API otorgará el nivel de acceso al usuario para que pueda acceder a las funciones de pago sin haber realizado ningún pago.
...
Cuando tu app se inicie, [comprueba este atributo en el perfil del usuario](subscription-status) para otorgar acceso automáticamente.
...
## Paso 3. Verificar el acceso en la app \{#step-3-verify-access-in-the-app\}

Tras otorgar el acceso mediante la API, el perfil del usuario se actualizará automáticamente. Obtén su perfil para comprobar el estado de su suscripción y desbloquear las funciones premium.
