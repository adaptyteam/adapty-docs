...
keywords: ['instalar sdk', 'sdk instalar', 'instalar sdk kotlin multiplatform']
rank: 50
---
...
- **Core Adapty**: Este SDK es imprescindible para que Adapty funcione correctamente en tu app.
- **AdaptyUI** (`io.adapty:adapty-kmp-ui`): Este módulo es necesario si utilizas el [Adapty Paywall Builder](adapty-paywall-builder) con la capa de renderizado de Compose Multiplatform (`view.present()`). Si tu proyecto no usa Compose Multiplatform, puedes utilizar [`createNativePaywallView`](kmp-present-paywalls#without-compose-multiplatform) y [`createNativeOnboardingView`](kmp-present-onboardings#without-compose-multiplatform) del módulo principal en su lugar.
:::tip
...
- Si usas catálogos de versiones, añade las dependencias en tu archivo `libs.versions.toml` y luego referencíalas en `build.gradle.kts`
...
3. Sustituye `"YOUR_PUBLIC_SDK_KEY"` en el código.
:::info
- Asegúrate de usar la clave SDK pública para inicializar Adapty; la clave secreta solo debe usarse en la [API server-side](getting-started-with-server-side-api).
...
Si tienes previsto activar el módulo **AdaptyUI** para usar el [Paywall Builder de Adapty](kmp-present-paywalls), asegúrate de establecer `.withActivateUI(true)` en tu configuración.
...
#### Configurar el sistema de registros \{#set-up-the-logging-system\}
...
Usa este parámetro para cumplir con las políticas de App Store/Play Store, evitar que aparezca el mensaje de App Tracking Transparency, o si tu app no necesita atribución publicitaria ni análisis basados en identificadores de publicidad.
