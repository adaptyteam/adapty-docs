...
metadataTitle: "Instalar el SDK de Adapty en Kotlin Multiplatform | Documentación de Adapty"
keywords: ['instalar sdk', 'sdk install', 'instalar sdk kotlin multiplatform']
rank: 50
---
...
- Gradle estándar: añade las dependencias a tu `build.gradle` **de nivel de módulo**
- Si tu proyecto usa archivos `.gradle.kts`, añade las dependencias a tu `build.gradle.kts` **de nivel de módulo**
...
Añade la inicialización lo antes posible, normalmente en el código Kotlin compartido para ambas plataformas.
...
```
:::important
Espera a que `activate` se complete antes de llamar a cualquier otro método del SDK de Adapty. Consulta [Orden de llamadas en el SDK de Kotlin Multiplatform](kmp-sdk-call-order) para ver la secuencia completa.
:::
...
:::info
- Asegúrate de usar la clave SDK pública para inicializar Adapty. La clave secreta debe usarse únicamente para la [API del lado del servidor](getting-started-with-server-side-api).
...
- Si usas [Adapty Paywall Builder](adapty-paywall-builder), primero [activa el módulo AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) a continuación y luego sigue la [guía de inicio rápido de Paywall Builder](kmp-quickstart-paywalls).
...
Si tienes previsto activar el módulo **AdaptyUI** para usar el [Paywall Builder de Adapty](kmp-present-paywalls), asegúrate de incluir `.withActivateUI(true)` en tu configuración.
...
Puedes establecer el nivel de log en tu aplicación antes de configurar Adapty:
...
Usa este parámetro para reforzar la privacidad del usuario, cumplir con las normativas regionales de protección de datos (como GDPR o CCPA) o reducir la recopilación de datos innecesaria cuando las funciones basadas en IP no son requeridas por tu app.
```kotlin title="Kotlin" showLineNumbers
...
Usa este parámetro para cumplir con las políticas de App Store/Play Store, evitar que se muestre el aviso de App Tracking Transparency, o si tu app no necesita atribución publicitaria ni análisis basado en identificadores publicitarios.
...
De forma predeterminada, AdaptyUI almacena en caché los archivos multimedia (como imágenes y vídeos) para mejorar el rendimiento y reducir el uso de red. Puedes personalizar la configuración de caché proporcionando una configuración personalizada.
...
Si la Activity que inicia el flujo de compra usa un `launchMode` no predeterminado, Android puede recrearla o reutilizarla de forma incorrecta cuando el usuario vuelve desde Google Play, una aplicación bancaria o un navegador. Esto puede provocar que el resultado de la compra se pierda o se trate como cancelado.

Para garantizar que las compras funcionen correctamente, usa solo los modos de lanzamiento `standard` o `singleTop` en la Activity que inicia el flujo de compra, y evita cualquier otro modo.
