...
| **expires_at**                     | ISO 8601 date | <p>Fecha y hora en que expira el acceso. Adapty calcula `is_active` a partir de este campo.</p><p>Durante un período de gracia, contiene el fin del período de gracia; tras un reembolso o cancelación, contiene la fecha de cancelación. No existe un campo separado para el fin del período de gracia.</p> |
...
| **is_in_grace_period**             | Boolean       | Booleano que indica si el perfil está en el período de gracia. Mientras sea `true`, `expires_at` contiene el fin del período de gracia. |
...
| **original_purchase_date**         | ISO 8601 date | En las suscripciones recurrentes, la compra original es la primera transacción de la cadena; su ID, llamado ID de transacción original, vincula la cadena de renovaciones. Las transacciones posteriores son extensiones de ella. La fecha de compra original es la fecha y hora de esa primera transacción. |
| **original_transaction_id**        | String        | <p>En las suscripciones recurrentes, es el ID de transacción original que vincula la cadena de renovaciones. La transacción original es la primera de la cadena; las posteriores son extensiones de ella.</p><p>Si no hay extensiones, `original_transaction_id` coincide con store_transaction_id.</p>El identificador de la transacción de la compra original. |
| **paywall_name**                   | String        | Nombre del paywall donde se originó la transacción.          |
...
| **profile_ip_address**             | String        | IP del perfil (puede ser IPv4 o IPv6, con preferencia por IPv4 cuando esté disponible). Es `null` si **Collect users' IP addresses** está deshabilitado en los [ajustes de la app](https://app.adapty.io/settings/general). |
...
| **store**                          | String        | Store donde se compró el producto. Valores estándar: **app_store**, **play_store**, **stripe**, **paddle**. <br/>Si configuras [transacciones en un store personalizado](api-adapty/operations/setTransaction) mediante la API server-side, se usa el valor del parámetro **store**. |
...
| **vendor_product_id**              | String        | <p>ID del producto en el store (Apple/Google/Stripe).</p><p>Si se otorgó acceso sin una transacción real en el store, `vendor_product_id` será uno de los siguientes:</p><ul><li>`adapty_server_side_product`: otorgado mediante la [API server-side](api-adapty/operations/grantAccessLevel).</li><li>`adapty_dashboard_product`: [otorgado manualmente](give-access-level-to-specific-customer) en el Adapty Dashboard.</li><li>`adapty_promotion`: heredado.</li></ul> |
