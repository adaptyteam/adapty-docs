...
[El repositorio de specs de CocoaPods pasará a ser de solo lectura en diciembre de 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/), por lo que a partir de la v4 los SDKs nativos `Adapty`, `AdaptyUI` y `AdaptyPlugin` **ya no se incorporan como sub-dependencias de CocoaPods** — el podspec los obtiene a través de **Swift Package Manager** (mediante el helper `spm_dependency`). Esto requiere dos cosas:
...
- **Frameworks dinámicos** — las dependencias de SPM requieren enlazado dinámico. La forma de habilitarlo difiere entre Expo y React Native sin configuración adicional.
...
:::warning
Cambiar del enlace estático predeterminado a frameworks dinámicos puede entrar en conflicto con librerías que aún no admiten cabeceras modulares, y es incompatible con Flipper. Si encuentras problemas de compilación, consulta este [artículo sobre la integración de Swift Package Manager con librerías de React Native](https://www.callstack.com/blog/integrating-swift-package-manager-with-react-native-libraries).
:::
...
El tipo devuelto cambia de `AdaptyPaywall` a `AdaptyFlow`, y el parámetro `locale` se elimina — cuando renderizas un flow, el idioma se resuelve automáticamente; para paywalls personalizadas, todos los idiomas se devuelven en `flow.remoteConfigs`:
...
| Campo `AdaptyPaywall` v3 | Campo `AdaptyFlow` v4 | Acción |
|---|---|---|
...
Si utilizas el componente React, cámbiale el nombre y pasa la prop `flow`:
...
```
:::note
Una vista de flow creada con `createFlowView` es de un solo uso: tras llamar a `dismiss()`, la vista se destruye, por lo que debes llamar a `createFlowView` de nuevo para volver a mostrar el flow. Un `AdaptyFlowView` embebido se cierra desmontándolo — devolver `true` desde un handler no cierra una vista embebida, así que cambia tu propio estado en su lugar, por ejemplo en `onCloseButtonPress`.
:::
## Gestión de eventos \{#handling-events\}

La interfaz de gestión de eventos se ha renombrado de `EventHandlers` a `FlowEventHandlers`, y tres callbacks han cambiado de nombre. Los cuerpos de los handlers existentes no necesitan modificaciones de código — solo renómbralos:
...
+ onError: (error) => { /* … */ },
```
Todos los demás manejadores de eventos conservan sus nombres. Dos de ellos también reciben un segundo argumento: `onPurchaseCompleted` ahora es `(purchaseResult, product)` y `onPurchaseFailed` ahora es `(error, product)`, donde `product` es el `AdaptyPaywallProduct` implicado. Consulta [Gestionar eventos de flow y paywall](react-native-handling-events-1) para ver la lista completa.

:::note
`onDisappeared` solo se dispara para un flow presentado de forma modal con `createFlowView().present()`. El componente `AdaptyFlowView` no lo expone como prop — para descartar una vista embebida, desmóntala.
:::

La v4 también añade algunas funcionalidades opcionales:
...
Estos cambios no generan errores de compilación, así que compruébalos en tiempo de ejecución:
...
- **`onUrlPress`**: El comportamiento predeterminado ahora abre la URL a través de la capa nativa, respetando la configuración de navegador integrado o externo del dashboard. Sobreescribe el handler para gestionar la apertura de URLs tú mismo.
