...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Inicializa tu MMP o SDK de análisis (AppsFlyer, Adjust, PostHog, Branch)                                     | Al iniciar la app, primero                                                      | Espera el callback de UID del MMP, por ejemplo `getAppsFlyerId`.                                 |
| 2a   | `Adapty.Activate(builder.Build(), ...)` con `SetCustomerUserId` configurado en el builder                           | Al iniciar la app, después del paso 1, si tienes el ID de usuario del cliente             | Recomendado. No se crea ningún perfil anónimo.                                             |
| 2b   | `Adapty.Activate(builder.Build(), ...)` sin `SetCustomerUserId`                                           | Al iniciar la app, después del paso 1, si no tienes el ID de usuario del cliente (o nunca lo recopilas) | Adapty crea un perfil anónimo.                                                           |
| 3    | `Adapty.SetIntegrationIdentifier(key, value, callback)` para cada MMP                                          | Después del paso 2, antes de cualquier llamada de acción del usuario                                              | Necesario para que los IDs del MMP se asocien al perfil correcto.                                               |
| 4    | `Adapty.Identify("YOUR_USER_ID", callback)`                                                                   | Después del paso 3 (o paso 2 si no hay MMP), antes del paso 5 — solo en el flujo 2b con autenticación | Espera el callback de finalización. Las llamadas concurrentes durante `Identify` generan `#3006 profileWasChanged`. |
| 5    | `GetPaywall` (`GetFlow` en SDK v4), `GetPaywallProducts`, `RestorePurchases`, `MakePurchase`, `UpdateAttribution` (`UpdateExternalAttribution` en SDK 4.1), `UpdateProfile`  | Después del paso 4 si llamas a `Identify`; si no, después del paso 3 (o paso 2 si no hay MMP)      | Estas llamadas necesitan un perfil estable.                                                             |
