...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Inicializa tu MMP o SDK de analíticas (AppsFlyer, Adjust, PostHog, Branch)                                     | Al iniciar la app, primero                                                              | Espera el callback de UID del MMP, por ejemplo `getAppsFlyerId`.                                 |
| 2a   | `Adapty.Activate(builder.Build(), ...)` con `SetCustomerUserId` establecido en el builder                           | Al iniciar la app, después del paso 1, si tienes el customer user ID                             | Recomendado. No se crea ningún perfil anónimo.                                                             |
| 2b   | `Adapty.Activate(builder.Build(), ...)` sin `SetCustomerUserId`                                           | Al iniciar la app, después del paso 1, si no tienes el customer user ID (o nunca lo recopilas) | Adapty crea un perfil anónimo.                                                           |
| 3    | `Adapty.SetIntegrationIdentifier(key, value, callback)` para cada MMP                                          | Después del paso 2, antes de cualquier llamada de acción del usuario                              | Necesario para que los IDs del MMP lleguen al perfil correcto.                                               |
| 4    | `Adapty.Identify("YOUR_USER_ID", callback)`                                                                   | Después del paso 3 (o del paso 2 si no hay MMP), antes del paso 5 — solo en el camino 2b con autenticación | Espera el callback de finalización. Las llamadas concurrentes durante `Identify` producen `#3006 profileWasChanged`. |
