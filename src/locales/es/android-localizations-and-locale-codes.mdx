...
- **Flows creados en el builder**: el SDK no lee el idioma del dispositivo, así que resuélvelo en tu app y pásalo como argumento `locale` de `AdaptyUI.getFlowConfiguration`. El argumento es opcional — omítelo y el flow se renderizará en `en`, o en su [idioma predeterminado](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) si el flow no tiene localización en `en`. Si solicitas una localización que el flow no tiene, la vista usa el idioma predeterminado del flow sin mostrar ningún error, y las cadenas que falten en la localización elegida se tomarán de la predeterminada.

Rendering en `en` por defecto requiere Android SDK 4.0.1. En 4.0.0, omitir `locale` renderiza la localización predeterminada del flow.
- **Paywalls personalizados (Remote Config)**: `getFlow` devuelve todas las localizaciones configuradas en `flow.remoteConfigs`. Cada entrada tiene un código `locale` y el contenido de la configuración (`jsonString` o el `dataMap` parseado). Selecciona la entrada que corresponda al usuario, con tu propio fallback:
