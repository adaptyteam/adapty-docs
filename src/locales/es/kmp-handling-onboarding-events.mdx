---
title: "Gestionar eventos de onboarding en el SDK de Kotlin Multiplatform"
...
Los onboardings configurados con el builder generan eventos a los que tu aplicación puede responder. A continuación se explica cómo hacerlo.
...
Si usas [`createNativeOnboardingView`](kmp-present-onboardings#without-compose-multiplatform), `view.isStandaloneView` es `false` — la implementación por defecto no llama a `view.dismiss()`. En su lugar, elimina la vista de tu layout y llama a `dispose()` en este callback.
...
:::tip
Gestiona este evento para abrir un paywall dentro del onboarding. Si quieres abrirlo después de que el onboarding se cierre, hay una forma más directa: gestiona [`onboardingViewOnCloseAction`](#closing-onboarding) y abre el paywall sin depender de los datos del evento.
:::
