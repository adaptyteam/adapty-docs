---
title: "Migrar el SDK de Adapty Capacitor a v4.1.1"
description: "Migra al SDK de Adapty Capacitor v4.1.1: reemplaza las APIs de paywall por APIs de flow, adopta las APIs de atribución renombradas, habilita Adapty Attribution de forma explícita y vuelve a descargar tu archivo de respaldo."
metadataTitle: "Migración al SDK de Adapty Capacitor v4.1.1 | Documentación de Adapty"
---
...
La línea 4.x introduce flows y renombra las APIs de paywall en consecuencia. Las nuevas APIs funcionan tanto con el nuevo Flow Builder como con el Paywall Builder existente — no se requieren cambios de configuración en el Adapty Dashboard. Además, la versión 4.1.1 hace que la atribución de Adapty sea opcional (opt-in), renombra el método de atribución externa, cambia el formato del archivo de respaldo y añade compras in-app promocionadas en la App Store.
...
| v3 | v4.1.1 |
|---|---|
| Atribución de Adapty habilitada automáticamente | deshabilitada por defecto — actívala con `adaptyAttributionEnabled: true` |
...
:::warning
Si usas [Atribución de Adapty](user-acquisition) y actualizas al SDK 4.1.1 sin activarla, fallará silenciosamente: las instalaciones dejarán de registrarse y no recibirás ningún aviso.
:::

En versiones anteriores, el SDK registraba las instalaciones para [Atribución de Adapty](user-acquisition) automáticamente. A partir de la versión 4.1.1 del SDK, esto está desactivado por defecto: el SDK no registra instalaciones, los eventos `'onInstallationDetailsSuccess'` y `'onInstallationDetailsFail'` nunca se disparan, y `getCurrentInstallationStatus` devuelve el estado `not_available`.
...
El formato del [archivo de respaldo](fallback-flows) cambió en la línea 4.x y de nuevo en 4.1.1. Descarga el archivo otra vez desde **[Placements](https://app.adapty.io/placements)** > **Fallbacks** y agrégalo a tu app, aunque ya hayas descargado uno para una beta 4.0.

:::warning
Este paso no produce ningún error de compilación. Si lo omites, `setFallback` rechazará el archivo desactualizado y todos los placements perderán su paywall de respaldo.
:::
...
| `productIdentifiers` | `flow.paywalls[i].productIdentifiers` | Los identificadores de producto ahora están en cada variación del flow, no en el flow en sí. |
| `products` (obsoleto en v3) | eliminado | Usa `flow.paywalls[i].productIdentifiers`, o llama a `getPaywallProducts(flow)` para obtener los productos completos. `ProductReference` se ha eliminado como tipo público. |
| `webPurchaseUrl?` | `flow.paywalls[i].webPurchaseUrl` | Movido del flow a cada variación del paywall. |
| `version?: number` | `flowVersionId?: string` | Renombrado, y el tipo cambió de `number` a `string`. |
| `requestLocale` | eliminado | El idioma ya no forma parte del modelo. |
...
SDK 4.1.1 añade compatibilidad con [compras in-app promocionadas en tu página de producto de la App Store](capacitor-making-purchases#in-app-purchases-from-the-app-store). Es una nueva funcionalidad, no un paso de migración: el SDK gestiona las compras promocionadas por sí solo, por lo que obtienes este comportamiento sin escribir ningún código.
