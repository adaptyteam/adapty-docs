...
- `AdaptyUI.OpenUrl(url, openIn, ...)` y `AdaptyUI.RequestAppReview(...)` — el manejo nativo detrás de las acciones `open_url` y las solicitudes de reseña de la app. Llama a `OpenUrl` desde `FlowViewDidPerformAction` para mantener el comportamiento predeterminado de URLs; `RequestAppReview` respalda el prompt de reseña de la app por defecto, que los flows aún no activan.
