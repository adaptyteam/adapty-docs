...
| `AdaptyEventListener` | `IAdaptyEventListener`, con un nuevo método requerido `OnReceivePromotedPurchase` |
...
| `Adapty.UpdateAttribution(data, source, ...)` con un `string` source | `Adapty.UpdateExternalAttribution(jsonString, provider, ...)` con un `AdaptyExternalAttributionProvider` |
...
| `source` como `string` | `provider` como `AdaptyExternalAttributionProvider` |
| `AdaptyProfile.AppliedAttributionSources` como `IReadOnlyList<string>` | `AdaptyProfile.AppliedExternalAttributionProviders` como `IReadOnlyList<AdaptyExternalAttributionProvider>` |
