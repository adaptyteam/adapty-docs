---
title: "Migrar el SDK de Adapty Unity a v. 4.0"
description: "Migra al SDK de Adapty Unity v4.0 (beta) reemplazando las APIs de paywall por APIs de flow, compatibles tanto con el Flow Builder como con el Paywall Builder."
metadataTitle: "Migración al SDK de Adapty Unity v4.0 (beta) | Documentación de Adapty"
---
...
La versión 4.0 es una versión preliminar, así que indica la etiqueta beta exacta al instalarla. Mediante Unity Package Manager, usa la URL de Git con la etiqueta en lugar de `#upm`:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Si instalas mediante el paquete de Unity, descarga `adapty-unity-plugin-4.0.0-beta.1.unitypackage` desde la carpeta [Releases](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Releases). Consulta [Instalar el SDK de Adapty](sdk-installation-unity#install-adapty-sdk) para ver la configuración completa.

La versión 4 incluye dos cambios en la configuración de compilación:
...
| Propiedad v3 de `AdaptyPaywall` | Propiedad v4 de `AdaptyFlow` | Acción |
|---|---|---|
| `RemoteConfig` (única, nullable) | `RemoteConfigs` (lista) | Un flow lleva un Remote Config por idioma configurado. Lee el que corresponda al usuario desde `flow.RemoteConfigs`. El acceso directo `flow.RemoteConfig` devuelve la primera entrada. |
| _(nuevo)_ | `Paywalls` (lista de `AdaptyFlowPaywall`) | Cada entrada es una variación de paywall del flow, con su propio `Name`, `VariationId` y `ProductIdentifiers`. Los métodos de paywall web reciben un `AdaptyFlowPaywall` — consulta [Métodos de paywall web](#web-paywall-methods). |
| `ProductIdentifiers`, `VendorProductIds` | se mantienen | En `AdaptyFlow`, estos agregan los productos de todas las variaciones de paywall. Cada variación también expone sus propios `ProductIdentifiers` y `VendorProductIds`. Para obtener productos, sigue llamando a `GetPaywallProducts(flow)`. |
| `HasViewConfiguration` | eliminada | Elimina cualquier comprobación de `HasViewConfiguration` en tu código — `CreateFlowView` devuelve un error en su lugar (consulta [Mostrar flows](#displaying-flows)). |
...
- `Adapty.SetSystemRequestsHandler(...)` con un `IAdaptyUISystemRequestsHandler` — reservado para solicitudes del sistema desde un flow: avisos de permisos del SO (`FlowViewDidAskPermission`) y solicitudes de valoración de la app (`FlowViewDidRequestAppReview`). Los flows aún no activan estas solicitudes, por lo que no necesitas registrar un handler.
- `AdaptyUICreateFlowViewParameters.Locale` — renderiza un flow o paywall con una [localización del Builder](add-paywall-locale-in-adapty-paywall-builder) específica en lugar de la que Adapty resuelve desde el dispositivo. Un flow se localiza cuando se crea su vista, por lo que este es el único momento para elegir su localización, y la vista creada informa de la localización con la que fue construida en `view.Locale`. Consulta [Usar localizaciones y códigos de idioma](unity-localizations-and-locale-codes).
...
- `AdaptyUI.OpenUrl(url, openIn, ...)` y `AdaptyUI.RequestAppReview(...)` — la gestión nativa detrás de las acciones `open_url` y las solicitudes de valoración de la app. Llama a `OpenUrl` desde `FlowViewDidPerformAction` para mantener el comportamiento predeterminado de las URLs; `RequestAppReview` respalda el aviso de valoración predeterminado, que los flows aún no activan.
