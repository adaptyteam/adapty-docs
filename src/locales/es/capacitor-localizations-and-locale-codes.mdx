...
En el SDK v4, no se pasa un código de idioma al obtener un flow: `getFlow` devuelve el flow con todas sus localizaciones.

- **Flows creados en el builder**: `createFlowView` no acepta parámetro de idioma, y el SDK no lee el idioma del dispositivo. El flow se renderiza en su [idioma por defecto](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — en iOS, en `en` cuando el flow tiene una localización en inglés.
