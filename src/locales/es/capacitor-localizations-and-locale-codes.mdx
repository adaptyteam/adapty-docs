...
`view.locale` informa la localización con la que se construyó la vista. Tanto la opción `locale` como `view.locale` requieren Capacitor SDK 4.0.1-beta.1, y `view.locale` es `undefined` en versiones anteriores. El [manejador `onAppeared`](capacitor-handling-events#the-view-argument) informa el mismo valor.
- **Paywalls personalizados (Remote Config)**: `getFlow` devuelve todas las localizaciones configuradas en `flow.remoteConfigs`. Cada entrada tiene un código `lang` y un objeto `data`. Selecciona la entrada que coincida con el usuario, con tu propio fallback:
