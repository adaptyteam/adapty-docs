...
description: "Migra al SDK de Adapty iOS v4.0 reemplazando las APIs de paywall por APIs de flow, que renderizan flows además de los paywalls del builder anterior."
metadataTitle: "Migración al SDK de Adapty iOS v4.0 | Documentación de Adapty"
---

El SDK de Adapty iOS 4.0 introduce los flows y renombra las APIs de paywall en consecuencia. Las nuevas APIs funcionan con flows y siguen siendo compatibles con los paywalls del builder anterior — no se requieren cambios de configuración en el Adapty Dashboard.
...
Adapty iOS SDK 4.0 elimina el soporte para CocoaPods. Instala el SDK con [Swift Package Manager](sdk-installation-ios#install-adapty-sdk). La v4 también requiere **Xcode 26** o posterior, ya que su manifiesto de paquete declara Swift tools 6.2.

Si tu proyecto todavía usa CocoaPods, elimina los pods `Adapty` y `AdaptyUI` de tu `Podfile`, ejecuta `pod install` para limpiarlos y luego añade el paquete en Xcode a través de **File → Add Package Dependency** usando `https://github.com/adaptyteam/AdaptySDK-iOS.git`.
...
## Gestión de compras promocionadas en App Store eliminada temporalmente \{#app-store-promoted-in-app-purchases-temporarily-removed\}

Como parte de la migración a StoreKit 2, el SDK de Adapty para iOS 4.0 elimina la API para gestionar las compras in-app promocionadas en App Store: el método delegado `shouldAddStorePayment(for:)` y el tipo `AdaptyDeferredProduct` que recibe no están disponibles en la versión 4.0. Estas compras siguen completándose y Adapty registra la transacción, pero la versión 4.0 no ofrece ninguna forma de interceptarlas ni diferirlas.

:::warning
Esta eliminación solo aplica a la versión 4.0 — el manejo regresa en el SDK 4.1 con una nueva API. Si tu app intercepta compras in-app promocionadas, migra de 3.x directamente a 4.1. Consulta [Compras in-app desde el App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
...
Los tipos devueltos cambian de `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` a `AdaptyFlow` / `AdaptyUI.FlowConfiguration`. El parámetro `locale` deja de estar en la llamada de obtención y pasa a `getFlowConfiguration`: `AdaptyFlow` también expone `paywalls` — las variaciones de paywall del flow, como valores `AdaptyFlowPaywall`. Los métodos del paywall web reciben uno de estos valores.
...
El formato del archivo de respaldo [cambió en SDK v4](fallback-flows). Descarga el nuevo archivo desde **[Placements](https://app.adapty.io/placements)** > **Fallbacks** y agrégalo a tu app.

## Seguimiento de vistas de paywall \{#tracking-paywall-views\}

### logShowPaywall(_:) → logShowFlow(_:)

`logShowPaywall` ha sido renombrado a `logShowFlow` y ahora recibe un `AdaptyFlow` en lugar de un `AdaptyPaywall`. El evento sigue registrándose contra la misma variación, por lo que las métricas de embudo y pruebas A/B existentes siguen funcionando sin cambios en el dashboard.
...
:::note
SDK 4.1 cambia el nombre de este método a `updateExternalAttribution(_:provider:)` y el tipo a `AdaptyExternalAttributionProvider`. Los nombres antiguos se eliminan en la versión 4.1 sin deprecación previa. Si migras directamente a la versión 4.1, consulta [Migrar a v4.1](migration-to-ios-sdk-41).
:::
