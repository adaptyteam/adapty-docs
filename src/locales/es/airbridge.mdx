---
title: "Airbridge"
description: "Conecta Adapty con Airbridge para rastrear insights de marketing y atribución."
...
[Airbridge](https://www.airbridge.io/) ofrece un análisis integrado del rendimiento de marketing para sitios web y aplicaciones móviles, consolidando datos recopilados de múltiples dispositivos, plataformas y canales. Con el Identity Resolution Engine de Airbridge, puedes combinar datos de identidad de clientes dispersos procedentes de interacciones web y de aplicaciones en una identidad unificada basada en personas, lo que se traduce en una atribución más precisa.

Adapty ofrece un conjunto completo de datos que te permite rastrear [eventos de suscripción](events) desde los stores en un solo lugar. Con Adapty, puedes ver fácilmente cómo se comportan tus suscriptores, conocer sus preferencias y usar esa información para comunicarte con ellos de forma dirigida y efectiva.

La integración entre Adapty y Airbridge funciona de dos maneras principales.

1. **Recibir datos de atribución de Airbridge**
   Una vez que hayas configurado la integración con Airbridge, Adapty comenzará a recibir datos de atribución de Airbridge. Puedes acceder y consultar fácilmente estos datos en la página del usuario.
2. **Enviar eventos de suscripción a Airbridge**
   Adapty puede enviar a Airbridge todos los eventos de suscripción configurados en tu integración. De este modo, podrás hacer seguimiento de estos eventos desde el dashboard de Airbridge. Esta integración es muy útil para evaluar la efectividad de tus campañas publicitarias.
...
| `device.deviceUUID`                          | String  | El UUID de dispositivo de Airbridge.                           |
...
| `device.ifa`                                 | String  | **Solo iOS**. ID for Advertisers.                              |
| `device.ifv`                                 | String  | **Solo iOS**. ID for Vendors.                                  |
| `device.gaid`                                | String  | **Solo Android**. Google Advertising ID.                       |
| `app`                                        | Object  | Información de la app.                                         |
| `app.packageName`                            | String  | El nombre de paquete / bundle ID de la aplicación.             |
| `app.version`                                | String  | La versión de la aplicación.                                   |
| `eventUUID`                                  | String  | ID único del evento en Adapty.                                 |
| `eventTimestamp`                             | Long    | Timestamp del evento en milisegundos.                          |
| `eventData`                                  | Object  | Detalles del evento.                                           |
| `eventData.goal.category`                    | String  | La categoría de evento de Airbridge (mapeada desde el evento de Adapty). |
...
| `...semanticAttributes.currency`             | String  | Código de moneda (p. ej., "USD").                              |
| `...semanticAttributes.period`               | String  | Período de suscripción en formato de duración ISO 8601 (p. ej., "P1M"). |
| `...semanticAttributes.isRenewal`            | Boolean | `true` si es una transacción de renovación.                    |
