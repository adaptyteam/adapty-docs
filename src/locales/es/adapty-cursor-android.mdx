...
La skill de Adapty CLI permite que tu LLM configure tu app, productos, niveles de acceso, paywalls y placements directamente, sin tener que abrir el Dashboard en cada paso. Solo necesitas [conectar tu store](integrate-payments) en el Dashboard.

```
npx skills add adaptyteam/adapty-cli --skill adapty-cli
```

Una vez añadido la skill, ejecuta `/adapty-cli` en tu agente. Te guiará paso a paso, incluyendo cuándo abrir el Dashboard para conectar tu store.
...
4. **Crea un flow o paywall y un placement**: En el Adapty Dashboard, crea un flow (o un paywall si vas a construir la UI tú mismo), luego asígnalo a un placement en la página **Placements**. En el código, el ID del placement es la cadena que pasas a `Adapty.getFlow("YOUR_PLACEMENT_ID")`.
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Creas flows en el editor visual sin código de Adapty, y el SDK los renderiza automáticamente.
- [**Paywalls creados manualmente**](android-making-purchases): Construyes tu propia UI de paywall en código, pero sigues usando Adapty para obtener productos y gestionar compras.
...
**Guías:**
- [Habilitar compras con Flow & Paywall Builder (inicio rápido)](android-quickstart-paywalls)
- [Obtener flows y su configuración](android-get-pb-paywalls)
- [Mostrar flows](android-present-paywalls)
- [Gestionar eventos de flow](android-handling-events)
...
:::tip[Checkpoint]
- **Expected:** Aparece el paywall con tus productos configurados. Al pulsar un producto se activa el diálogo de compra en sandbox.
