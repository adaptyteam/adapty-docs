...
**Motivo**: El evento no es compatible con esta integración, o lo desactivaste al configurarla.
...
1. Comprueba si la integración admite este tipo de evento. Los siguientes eventos no llegan a todas las integraciones:
    ...
    - [Eventos sintéticos](events#synthetic-events) (`trial_active`, `subscription_active`): Adjust, Amplitude, AppMetrica, AppsFlyer, Branch, Facebook Analytics, Firebase, Mixpanel, OneSignal, PostHog, Singular, SplitMetrics y el webhook.

2. Si la integración es compatible, asegúrate de que el evento está activado en la configuración de la integración.
...
- La prueba o suscripción terminó antes de que transcurriera el retraso. Una compra convertida, expirada o reembolsada no genera nada, y Adapty no vuelve a comprobarlo. Busca `trial_converted`, `trial_expired`, `subscription_expired` o `subscription_refunded` en su lugar.
- El usuario desactivó la renovación automática antes de que transcurriera el retraso. Su acceso se mantiene hasta el final del período pagado, pero Adapty omite el envío. Busca `trial_renewal_cancelled` o `subscription_renewal_cancelled` en su lugar.
- Activaste el evento después de que comenzara la compra. Adapty inicia el temporizador cuando llega `trial_started` o `subscription_started`, así que solo las compras realizadas después de que actives el evento generarán uno.
