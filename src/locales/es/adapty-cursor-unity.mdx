...
Una vez añadido la skill, ejecuta `/adapty-cli` en tu agente. Te guiará paso a paso, incluyendo el momento en que debas abrir el Dashboard para conectar tus stores.
...
4. **Crea un flow o paywall y un placement**: En el Adapty Dashboard, crea un flow (o un paywall si vas a construir la interfaz tú mismo), luego asígnalo a un placement en la página **Placements**. En el código, el ID del placement es la cadena que pasas a `Adapty.GetFlow("YOUR_PLACEMENT_ID")`.
...
- **Flows y placements adicionales**: Añade más llamadas `GetFlow` con diferentes IDs de placement.
...
- [**Paywalls creados manualmente**](unity-making-purchases): Construyes tu propia UI de paywall en código, pero sigues usando Adapty para obtener productos y gestionar compras.
...
Prueba cada compra en el sandbox a medida que avanzas — no esperes hasta el final. Consulta [Probar compras en sandbox](test-purchases-in-sandbox) para obtener instrucciones de configuración.
...
**Guías:**
- [Activar compras con Flow y Paywall Builder (inicio rápido)](unity-quickstart-paywalls)
- [Obtener flows y su configuración](unity-get-pb-paywalls)
- [Mostrar flows](unity-present-paywalls)
- [Gestionar eventos de flow](unity-handling-events)
...
:::tip[Checkpoint]
- **Expected:** El paywall aparece con los productos configurados. Al pulsar un producto se activa el diálogo de compra sandbox.
- **Gotcha:** Flow vacío o error `GetFlow` → verifica que el ID del placement coincida exactamente con el del dashboard y que el placement tenga una audiencia asignada.
:::
