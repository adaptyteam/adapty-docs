...
En la lista de perfiles, puedes buscar un usuario específico por:

- **Profile ID**: el identificador interno de Adapty para el usuario (también llamado Adapty ID).
- **Customer user ID**: el identificador de tu app para el usuario, si has configurado uno.
- **Email**: el correo electrónico del usuario, si se ha enviado como atributo personalizado.
- **Transaction ID**: el ID de transacción del store correspondiente a una compra.

La búsqueda coincide exactamente con el valor completo: las coincidencias parciales no funcionan, y los espacios en blanco alrededor del identificador hacen que la búsqueda no devuelva nada — pega el identificador completo sin espacios adicionales. El ID de usuario personalizado, el correo electrónico y el ID de transacción también distinguen entre mayúsculas y minúsculas.
...
| ---------------- | ------------------------------------------------------------ |
| Customer user ID | Identificador de tu usuario final en tu sistema.             |
| Adapty ID        | Identificador interno de Adapty para tu usuario final, llamado Profile ID. |
| IDFA             | El Identificador para Anunciantes, asignado por Apple al dispositivo del usuario. Requiere el permiso de App Tracking Transparency (ATT) en iOS 14+. No disponible en Android. |
| IP country       | País del usuario final, determinado por su dirección IP más reciente. |
| Store country    | País de la cuenta de App Store o Google Play del usuario final, según lo indicado por el store. Adapty lo recibe con las transacciones del store, por lo que estará vacío para usuarios sin compras. |
...
En la sección **Attributes** de un perfil, puedes ver los atributos personalizados definidos mediante el SDK o la API. También puedes asignar atributos manualmente con el botón **Add attribute**.
...
Para modificar o eliminar un atributo, haz clic en los tres puntos que aparecen junto a él y selecciona **Edit** o **Delete**. Eliminar un atributo solo afecta a ese perfil; los demás perfiles que tengan el mismo atributo lo conservarán.
