...
keywords: ['instalar sdk', 'sdk install', 'instalar sdk android', 'google play billing library', 'gpbl', 'billing library', 'adaptyui']
rank: 70
---
...
- Si usas catálogos de versiones, añade las dependencias a tu archivo `libs.versions.toml` y luego referencialas en `build.gradle.kts`
...
:::important
Espera a que `Adapty.activate` se complete antes de llamar a cualquier otro método del SDK de Adapty. Consulta [Orden de llamadas en el SDK de Android](android-sdk-call-order) para ver la secuencia completa.
:::
...
- Si construyes tu propia UI de paywall, consulta la [guía de inicio rápido para paywalls personalizados](android-quickstart-manual).
...
#### Configura el sistema de logging \{#set-up-the-logging-system\}
...
#### Desactivar la recopilación y el uso compartido del ID de publicidad (Ad ID) \{#disable-advertising-id-ad-id-collection-and-sharing\}

Al activar el módulo de Adapty, establece `adIdCollectionDisabled` en `true` para desactivar la recopilación del [ID de publicidad](https://support.google.com/googleplay/android-developer/answer/6048248) del usuario. El valor predeterminado es `false`.
...
Si tu app usa múltiples procesos, inicializa Adapty una sola vez; de lo contrario, pueden producirse comportamientos inesperados.
...
#### 3. Crear archivos de reglas de copia de seguridad combinadas \{#3-create-merged-backup-rules-files\}
...
#### Las compras fallan al volver de otra app \{#purchases-fail-after-returning-from-another-app\}
