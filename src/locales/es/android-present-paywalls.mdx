...
Si has creado un flow o un paywall en el antiguo Paywall Builder, no necesitas preocuparte por renderizarlo en el código de tu app para mostrárselo al usuario. Ese flow o paywall ya contiene tanto lo que debe mostrarse como la forma en que debe mostrarse.

:::warning

Esta guía cubre flows y paywalls renderizados por Adapty. El proceso es distinto para los paywalls de Remote Config y el [modo Observer](observer-vs-full-mode).

- Para mostrar **paywalls de Remote Config**, consulta [Renderizar paywall diseñado por Remote Config](present-remote-config-paywalls).
- Para mostrar **paywalls en modo Observer**, consulta [Android - Presentar flows en modo Observer](android-present-paywall-builder-paywalls-in-observer-mode)

:::

Para obtener el objeto `flowConfiguration` que se usa a continuación, consulta [Obtener flows y paywalls](android-get-pb-paywalls).
...
| **insets**                    | opcional | <p>Los insets son los espacios alrededor del flow que evitan que los elementos interactivos queden ocultos detrás de las barras del sistema.</p><p>Valor predeterminado: `Unspecified`, lo que significa que Adapty ajustará los insets automáticamente, algo que funciona muy bien para flows de borde a borde. </p><p>Si tu flow no es de borde a borde, puede que quieras definir insets personalizados. Para saber cómo hacerlo, consulta la sección [Change flow insets](android-present-paywalls#change-flow-insets) más abajo.</p> |
...
| **tagResolver**               | opcional | Usa `AdaptyUiTagResolver` para resolver etiquetas personalizadas dentro del texto del flow. Este resolver recibe un parámetro de etiqueta y lo resuelve en la cadena de texto correspondiente.                                                                                                                                                                                                                                                                                                                                                                      |
...
Esta guía cubre únicamente los paywalls creados con el antiguo Paywall Builder para **Adapty SDK 3.x**. Requieren SDK v3.0. Presentar un paywall creado para SDK 2.x o anterior, un paywall con Remote Config, o un paywall en [modo Observer](observer-vs-full-mode) funciona de manera diferente.
