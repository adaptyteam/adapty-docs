...
2. `openWebPaywall(paywall)` que genera URLs por paywall sin añadir los datos del producto a las URLs. En la versión SDK v4, esto toma un `AdaptyFlowPaywall` — una variación de paywall del flow obtenido. Comprueba que `flow.paywalls` no esté vacío antes de indexarlo, por ejemplo `flow.paywalls[0]`.
:::
