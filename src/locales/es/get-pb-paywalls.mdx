...
| **locale**               | <p>opcional</p><p>valor predeterminado: `nil`</p> | El identificador de la [localización del paywall](add-paywall-locale-in-adapty-paywall-builder). Se espera como un código de idioma con una o dos subetiquetas separadas por `-` (p. ej., `en`, `pt-br`). Consulta [Localizaciones y códigos de idioma](localizations-and-locale-codes). |
| **customLayoutId**       | <p>opcional</p><p>valor predeterminado: `nil`</p><p>SDK 4.1+</p> | El ID personalizado de un layout definido en el Flow Builder. Pásalo para renderizar ese layout específico en lugar del que el SDK selecciona automáticamente según el tipo de dispositivo y el tamaño de pantalla. Si ningún layout coincide con el ID, el flow se carga sin configuración de vista. |
| **loadTimeout**          | valor predeterminado: 5 seg | Este valor limita el tiempo de espera de este método. Si se alcanza el tiempo de espera, se devolverán los datos en caché o el fallback local. Ten en cuenta que, en casos excepcionales, este método puede superar ligeramente el tiempo especificado en `loadTimeout`, ya que la operación puede consistir en varias solicitudes internas. |
...
| **systemRequestsHandler** | opcional      | Un objeto que cumple con `AdaptySystemRequestsHandler` y gestiona las solicitudes de permisos del sistema y de valoración activadas por las acciones del flow. Solo es necesario si tu flow incluye este tipo de acciones. |
| **assetsResolver**       | opcional       | Un diccionario `[String: AdaptyCustomAsset]` que reemplaza imágenes y vídeos en el flow/paywall. Consulta [Personalizar assets](#customize-assets). |
| **timerResolver**        | opcional       | Un objeto que cumple con `AdaptyTimerResolver` y proporciona fechas de finalización para los temporizadores definidos por el desarrollador. Consulta [Configurar temporizadores definidos por el desarrollador](#set-up-developer-defined-timers). |
