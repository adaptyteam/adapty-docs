...
description: "Muestra un paywall de inmediato y actualízalo para usuarios de Apple Ads una vez que se aplique la atribución en React Native, usando AdaptyProfile.appliedAttributionSources."
metadataTitle: "Mostrar paywall dirigido por AA en el primer lanzamiento | React Native SDK | Documentación de Adapty"
---

Este artículo se aplica a la build de iOS de tu aplicación. La atribución de Apple Ads solo existe en iOS.
...
1. Llama a `getPaywall` de inmediato. Sin atribución aplicada aún, Adapty resuelve la solicitud con la audiencia predeterminada, por lo que el usuario ve un paywall enseguida.
2. Cuando aparezca `'apple_search_ads'`, vuelve a llamar a `getPaywall`. Adapty resuelve ahora la solicitud con la audiencia de Apple Ads y devuelve el paywall segmentado, que reemplaza al primero.

`appliedAttributionSources` puede estar vacío o ausente. Eso significa una de las siguientes cosas:
...
:::important
En cada inicio posterior, el perfil en caché ya lleva `'apple_search_ads'` en `appliedAttributionSources`, por lo que el primer `getPaywall` ya devuelve el paywall segmentado por Apple Ads — no hay una segunda solicitud ni ningún cambio visible. El flow de dos pasos solo importa en el primer inicio, mientras la atribución aún está en curso.
:::
