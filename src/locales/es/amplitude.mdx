...
[Amplitude](https://amplitude.com/) es un potente servicio de análisis móvil. Con Adapty, puedes enviar eventos a Amplitude fácilmente, ver cómo se comportan tus usuarios y tomar decisiones más inteligentes.

Adapty proporciona un conjunto completo de datos que te permite hacer seguimiento de [eventos de suscripción](events) desde los stores en un solo lugar y los envía a tu cuenta de Amplitude. Esto te permite relacionar el comportamiento de tus usuarios con su historial de pagos en Amplitude, y así tomar mejores decisiones de producto.

### Cómo configurar la integración con Amplitude

En Adapty, puedes configurar flows separados para **eventos de producción** y **eventos de prueba** del entorno sandbox de Apple, Stripe o Paddle, o de la cuenta de prueba de Google.

- Para eventos de producción, introduce las claves API de **Production** del dashboard de Amplitude, con una clave API única para cada plataforma: iOS, Android, Stripe y Paddle.
- Para eventos de prueba, usa los campos **Sandbox** según sea necesario.
...
| Campo | Descripción |
   | ---------------------------------------------------- | ------------------------------------------------------------ |
   | **Amplitude iOS/ Android/ Stripe/ Paddle API key** | Introduce la **API Key** de Amplitude para iOS/ Android/ Stripe/ Paddle en Adapty. La encontrarás en **Project settings** de Amplitude. Si necesitas ayuda, consulta la [documentación de Amplitude](https://amplitude.com/docs/apis/authentication). Empieza con las claves de **Sandbox** para hacer pruebas y luego cambia a las claves de **Production** una vez que las pruebas sean correctas. Rellena solo las plataformas en las que vendes: si una plataforma no tiene clave, no enviará eventos. |
...
**Configuración de amplitudeUserId**
