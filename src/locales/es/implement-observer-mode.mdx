...
2. [Reportar las transacciones](report-transactions-observer-mode) de tu infraestructura de compras existente a Adapty.

Si también necesitas flows, paywalls y pruebas A/B, se requiere configuración adicional, como se describe a continuación.
...
Si también quieres usar los flows, paywalls y funciones de pruebas A/B de Adapty, puedes hacerlo, pero requiere algo de configuración adicional en Observer Mode. Esto es lo que necesitarás hacer además de los pasos anteriores:

1. Muestra los [paywalls con Remote Config](present-remote-config-paywalls) como de costumbre. Para los flows y paywalls creados con el Flow & Paywall Builder, consulta [Presentar flows en modo Observer](ios-present-paywall-builder-paywalls-in-observer-mode).
2. [Asocia los flows y paywalls](report-transactions-observer-mode) con las transacciones de compra.
