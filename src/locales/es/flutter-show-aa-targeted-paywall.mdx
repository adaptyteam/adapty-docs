---
title: "Mostrar un paywall orientado a AA en el primer lanzamiento con Flutter SDK"
description: "Espera brevemente a la atribución de Apple Ads antes de mostrar el paywall en el primer lanzamiento en Flutter, con retroceso a la audiencia predeterminada en caso de tiempo de espera. Usa AdaptyProfile.appliedAttributionSources."
metadataTitle: "Mostrar un paywall orientado a AA en el primer lanzamiento | Flutter SDK | Documentación de Adapty"
---

Este artículo aplica a la compilación para iOS de tu app. La atribución de Apple Ads solo existe en iOS.
...
:::important
`appliedAttributionSources` solo muestra datos de Apple Ads. La atribución de otros proveedores es visible en los perfiles de usuario y está disponible en los filtros de segmento, pero aún no aquí.
:::
...
:::note
Los ejemplos usan los nombres de la API 3.x. En el SDK 4.0 y versiones posteriores, los paywalls se obtienen con `getFlow`/`getFlowForDefaultAudience` y el tipo devuelto es `AdaptyFlow`. Consulta [Migrar Adapty Flutter SDK a v4.0](migration-to-flutter-sdk-v4). La propiedad del perfil mantiene el nombre `appliedAttributionSources` en todas las versiones de Flutter.
:::
...
1. **La atribución llega antes de que se agote el tiempo de espera.** Llama a `getPaywall` — Adapty resuelve la solicitud contra la audiencia de Apple Ads y devuelve el paywall segmentado.
2. **El tiempo de espera se agota primero.** Muestra el paywall de la audiencia predeterminada, para que los usuarios sin atribución de Apple Ads no tengan que esperar. `getPaywallForDefaultAudience` lo devuelve sin esperar la segmentación.

`appliedAttributionSources` puede estar vacío. Eso significa una de las siguientes cosas:
...
En los tres casos, `getPaywallForDefaultAudience` es seguro de llamar — devuelve el paywall de la audiencia predeterminada independientemente del estado del perfil.

:::important
La espera solo se aplica al primer lanzamiento. Una vez que se ha registrado la atribución de Apple Ads, queda almacenada en el perfil de forma permanente. En cada lanzamiento posterior, el perfil en caché ya incluye `AdaptyAttributionSource.appleAds` en `appliedAttributionSources`, por lo que la ruta de atribución se resuelve de inmediato y `getPaywall` devuelve el paywall segmentado por Apple Ads sin ningún retraso.
:::
