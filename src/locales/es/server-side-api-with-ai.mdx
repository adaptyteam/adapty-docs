...
- **Límite de solicitudes**: Se aplica por clave API y se asigna por app, por lo que no hay una cifra única válida para todas — consulta [Límites de solicitudes](https://adapty.io/docs/es/ss-authorization.md#rate-limits). Es una tasa sostenida, no un presupuesto por minuto, y algunos endpoints tienen un límite inferior: [createVirtualCurrencyTransaction](https://adapty.io/docs/es/api-adapty/operations/createVirtualCurrencyTransaction.md) permite 600 solicitudes por minuto por app por defecto. Indica a tu agente que reintente con backoff al recibir un `429` y que respete el encabezado `Retry-After` cuando esté presente.
- **Claves específicas por app**: Cada clave funciona para una sola app; usa la clave correspondiente en cada caso.
