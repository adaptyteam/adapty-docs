---
title: "Facebook Ads"
description: "Integra Facebook Ads con Adapty para una marketing de suscripciones eficaz."
...
Con la integración de Facebook Ads, puedes consultar fácilmente las estadísticas de tu app en Meta Analytics. Adapty envía eventos a Meta Ads Manager, lo que te permite crear audiencias similares basadas en suscripciones para obtener mejores resultados. Así puedes ver con precisión cuánto dinero generan tus anuncios a partir de las suscripciones.

La integración entre Adapty y Facebook Ads funciona de la siguiente manera: Adapty envía a Facebook Ads todos los eventos de suscripción que hayas configurado en tu integración. Esta integración es útil para evaluar la efectividad de tus campañas publicitarias.
...
## Coincidencia avanzada (Conversions API) \{#advanced-matching-conversions-api\}

Facebook CAPI es una integración independiente de la integración de Facebook Ads descrita anteriormente, con sus propias credenciales, y se configura de forma independiente en el Adapty Dashboard. Si la activas, Adapty reenvía un payload de identidad hasheado a la API de Conversiones de Meta para un matching avanzado. Desde el perfil de Adapty del cliente, Adapty aplica SHA-256 y envía: email, número de teléfono, nombre y apellidos, fecha de nacimiento, género y país. Los identificadores de clic y navegador de Facebook (`fbc`, `fbp`) y la dirección IP/user agent se envían sin hashear, igual que en cualquier integración de Meta CAPI.
