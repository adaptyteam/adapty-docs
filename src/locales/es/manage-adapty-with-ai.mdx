...
## Crea y edita flows con una herramienta de codificación IA \{#build-and-edit-flows-with-an-ai-coding-tool\}

La [skill flow-generator](flow-generator-skill) edita tus flows de Adapty desde el terminal: añade un idioma, reescribe textos, agrega o elimina pantallas, configura bifurcaciones o crea un paywall a partir de una descripción. Previsualiza y valida cada cambio en local, y luego guarda un borrador listo para que lo publiques.

Antes de publicar, la [skill flow-audit](flow-audit-skill) lee el mismo flow y comprueba si está listo para producción: triggers, cumplimiento con la store, productos y localización, y termina con un veredicto de aprobado o rechazado.
