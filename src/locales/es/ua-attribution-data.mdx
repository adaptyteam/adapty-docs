...
---
Cuando Adapty relaciona una instalación con una campaña, devuelve los datos de atribución a tu app en el callback `onInstallationDetailsSuccess`. Usa estos datos para personalizar la experiencia del usuario según el canal o campaña que generó la instalación.

:::important
A partir de la versión 4.1 del SDK de Adapty, la atribución de Adapty está desactivada por defecto. El callback `onInstallationDetailsSuccess` solo se ejecuta después de que [habilites la atribución de Adapty al activar el SDK](user-acquisition#step-1-enable-adapty-attribution-in-the-sdk).
:::
...
| `ad_id` | Identificador del anuncio / creativo |
| `ad_name` | Nombre del anuncio / creativo |

Todos los campos son opcionales. Para las instalaciones orgánicas o cuando no se pudo determinar la atribución, el campo `payload` no incluye el objeto `attribution`.
