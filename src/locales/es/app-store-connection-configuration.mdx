---
title: "Configurar la integración de App Store"
...
Esta sección describe cómo establecer la conexión entre App Store y Adapty para tu app de iOS. Esto es necesario para que podamos mostrar análisis de suscripciones y validar compras. Puedes completar la integración durante el onboarding inicial o más tarde en **App Settings** dentro del Adapty Dashboard.

Cada credencial de esta página habilita una parte diferente del intercambio. [Cómo Adapty sincroniza datos con los stores](store-sync#what-adapty-reads-from-the-stores) muestra qué datos aporta cada una.

Aunque es posible que hayas configurado inicialmente la integración de tu app con Adapty durante el onboarding, puedes modificar estos ajustes más adelante en **App settings**.

:::danger
Los cambios de configuración se pueden realizar de forma segura durante la fase de Sandbox, hasta que tu app esté en producción con el SDK de Adapty instalado. Los cambios realizados después del lanzamiento pueden romper el flow de compras en tu app.

Si una clave es revocada, eliminada o rotada en App Store Connect, sube el reemplazo en cuanto lo detectes: todo lo que depende de ella permanecerá roto hasta que lo hagas. Consulta [Si las actualizaciones dejan de llegar](store-sync#if-updates-stop-arriving).
:::
...
:::note
App Store Connect tiene dos tipos de claves cuyos campos se etiquetan como **Issuer ID** y **Key ID**. Los pasos 2–3 usan la clave de **In-App Purchase** de la pestaña **Integrations** → **In-App Purchase**. La clave de App Store Connect API de la pestaña **Team keys** es una clave diferente con sus propios IDs — la añadirás en el [Paso 6](#step-6-add-app-store-connect-api-key).
:::
...
## Paso 6. Añade la clave de la API de App Store Connect \{#step-6-add-app-store-connect-api-key\}
...
:::note
Esta es un par de claves diferente a la clave de compras in-app que añadiste en los [pasos 2–3](#step-2-provide-issuer-id-and-key-id). Genera la clave en la pestaña **Team keys** y copia su **Issuer ID** y **Key ID** desde ahí — los valores de la clave de compras in-app no funcionarán aquí.
:::
