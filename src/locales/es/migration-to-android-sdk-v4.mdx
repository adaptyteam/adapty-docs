---
title: "Migrar Adapty Android SDK a v4.0"
description: "Migra al Adapty Android SDK v4.0 reemplazando las APIs de paywall con APIs de flow, que renderizan flows además de los paywalls del antiguo builder."
metadataTitle: "Migración al Adapty Android SDK v4.0 | Documentación de Adapty"
---

Adapty Android SDK 4.0 introduce flows y renombra las APIs de paywall en consecuencia. Las nuevas APIs funcionan con flows y siguen siendo compatibles con los paywalls del antiguo builder — no se requieren cambios de configuración en el Adapty Dashboard.
...
Establece la versión de `adapty-bom` en `4.0.1` (o posterior) y sincroniza el proyecto. El BOM resuelve automáticamente las versiones correspondientes de `android-sdk` y `android-ui`. Consulta [Instalar el SDK de Adapty](sdk-installation-android) para ver las declaraciones de dependencias.
...
- **Los onboardings están obsoletos.** `AdaptyUI.getOnboardingView` y `AdaptyUI.getOnboardingConfiguration` están marcados como `@Deprecated` en la versión 4.0 — migra los onboardings a flows creados en el [Flow & Paywall Builder](adapty-flow-builder).

## Obtener flows \{#fetching-flows\}

### getPaywall + getViewConfiguration → getFlow + getFlowConfiguration

El tipo de retorno del fetch cambia de `AdaptyPaywall` a `AdaptyFlow`, y el cargador de configuración pasa de llamarse `AdaptyUI.getViewConfiguration` a `AdaptyUI.getFlowConfiguration` (devolviendo `AdaptyUI.FlowConfiguration` en lugar de `AdaptyUI.LocalizedViewConfiguration`). El parámetro `locale` sale de la llamada al fetch y pasa a `getFlowConfiguration`: `AdaptyFlow` también expone `paywalls` — las variaciones de paywall del flow, como valores `AdaptyFlowPaywall`. Los métodos de paywall web reciben uno de estos valores.
...
`locale` es opcional en `getFlowConfiguration`: si lo omites, la vista se renderiza en `en`, o en la localización predeterminada del flow cuando este no tiene `en`. Consulta [Localizaciones y códigos de idioma](android-localizations-and-locale-codes).
...
`logShowPaywall` pasa a llamarse `logShowFlow` y ahora recibe un `AdaptyFlow` en lugar de un `AdaptyPaywall`. El evento se sigue registrando en la misma variación, por lo que las métricas de funnel y pruebas A/B existentes seguirán funcionando sin cambios en el dashboard.
...
Fuentes predefinidas: `AdaptyAttributionSource.APPLE_ADS`, `.ADJUST`, `.APPSFLYER`, `.BRANCH`, `.TENJIN`. Para cualquier otra fuente, constrúyela a partir de una cadena: `AdaptyAttributionSource("your_source")`.

:::note
SDK 4.1 renombra este método a `updateExternalAttribution` y el tipo a `AdaptyExternalAttributionProvider`. Los nombres anteriores se eliminan en la versión 4.1 sin haber sido marcados como obsoletos. Si migras directamente a la versión 4.1, consulta [Migrar a v4.1](migration-to-android-sdk-41).
:::
