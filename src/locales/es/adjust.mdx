---
title: "Adjust"
description: "Conecta Adjust con Adapty para un mejor seguimiento y análisis de suscripciones."
...
[Adjust](https://www.adjust.com/) es una de las principales plataformas de Mobile Measurement Partner (MMP) que recopila y presenta datos de campañas de marketing. Esto ayuda a las empresas a hacer seguimiento del rendimiento de sus campañas.
...
:::warning
Adapty envía las [compras de Stripe y Paddle](#web-purchases-stripe-and-paddle) a Adjust con el app token de la plataforma en la que el comprador instaló tu app. Si tienes una app de Android y vendes en la web, rellena el **App Token** de Android. Sin él, la compra web de un comprador de Android fallará con [Missing integration credentials](handle-integration-errors#missing-integration-credentials).
:::
...
### Conecta tu app con Adjust \{#connect-your-app-to-adjust\}

Una vez completados los pasos anteriores, añade los dos métodos siguientes a tu app. Estos establecerán la comunicación entre tu app y Adjust:
...
2. **Para recibir datos de atribución desde Adjust**: Actualiza los datos de atribución con el método `updateExternalAttribution()` del SDK (llamado `updateAttribution()` en versiones anteriores a la 4.1 del SDK)
...
Adjust recibe compras de [Stripe](stripe) y [Paddle](paddle) junto con las compras de App Store y Google Play. Una compra web ocurre fuera de la app, por lo que no tiene una plataforma móvil propia. Adapty utiliza en su lugar la plataforma del comprador. Cuando el perfil de Adapty del usuario se creó en App Store o Google Play, la compra web se envía a Adjust como si procediera de esa store:

| Store del perfil del comprador | Qué envía Adapty con la compra |
|:-------------------------------|:-------------------------------|
| App Store                      | Tu **App Token** y **OAuth Token** de iOS, más el `idfa` y el `idfv` del comprador. |
| Google Play                    | Tu **App Token** y **OAuth Token** de Android, más el `android_id` y el `gps_adid` del comprador. |

La compra llega a Adjust asociada al mismo dispositivo que la instalación, de modo que Adjust puede atribuirla a la campaña que captó al usuario.

La página de integración de Adjust no tiene campos separados para Stripe ni Paddle. Las compras web reutilizan las credenciales de iOS y Android de [Conectar Adapty con Adjust](#connect-adapty-to-adjust).

Un comprador que nunca abrió tu app móvil no tiene un ID de dispositivo de Adjust, y su perfil se creó en Stripe o Paddle. Adapty envía su compra a Adjust con tu **App Token** de iOS y sin ID de publicidad. Adjust no tiene entonces ningún identificador que vincule la compra con una instalación.
