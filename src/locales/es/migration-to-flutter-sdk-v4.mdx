---
title: "Migrar el SDK de Flutter de Adapty a v4.0"
description: "Migra al SDK de Flutter de Adapty v4.0 reemplazando las APIs de paywall con APIs de flow, que renderizan flows además de los antiguos paywalls del builder."
metadataTitle: "Migración al SDK de Flutter de Adapty v4.0 | Documentación de Adapty"
---

El SDK de Flutter de Adapty 4.0 introduce los flows y renombra las APIs de paywall en consecuencia. Las nuevas APIs funcionan con flows y siguen siendo compatibles con los paywalls del antiguo builder — no se requieren cambios de configuración en el Adapty Dashboard.

:::note
Esta guía cubre la migración de 3.x → 4.0. El SDK 4.1 introduce algunos cambios adicionales en la API: la atribución de Adapty pasa a ser opcional (opt-in), los métodos de atribución externa se renombran y el formato del archivo de respaldo cambia de nuevo. Cuando termines aquí, consulta [Migrar a v4.1](migration-to-flutter-sdk-41).
:::
...
El formato del archivo de respaldo [cambió en el SDK v4](fallback-flows). Descarga el nuevo archivo desde **[Placements](https://app.adapty.io/placements)** > **Fallbacks** y añádelo a tu app.
...
`logShowPaywall` ha sido renombrado a `logShowFlow` y ahora recibe un `AdaptyFlow`. El evento se sigue registrando contra la misma variación, por lo que las métricas de embudo y de prueba A/B existentes siguen funcionando sin cambios en el dashboard.
...
Cambia el nombre del método y pasa el `AdaptyFlow` mediante el parámetro `flow`. El resto de parámetros (`loadTimeout`, `preloadProducts`, `customTags`, `customTimers`, `customAssets`, `productPurchaseParams`) no cambian, ni tampoco los métodos de la vista `present`, `dismiss` y `showDialog`:

:::note
`customTimers` sigue existiendo, pero solo afecta a los paywalls del Paywall Builder heredado. El [temporizador de cuenta atrás](flow-timer) de un flow se rige por el comportamiento configurado en el Flow & Paywall Builder, por lo que un flow ignorará cualquier valor que pases aquí.
:::
...
- `setFlowsEventsObserver` (y `setOnboardingsEventsObserver`) ahora aceptan `null` para desconectar un observer previamente establecido, de modo que el SDK ya no lo retiene.
- El nuevo callback opcional `flowViewDidReceiveAnalyticEvent` reporta eventos de análisis de un flow, comenzando con una vista de pantalla por cada pantalla que abre el usuario. Consulta [Seguimiento de vistas de pantalla en flows](flutter-flow-screen-views).

La v4 también añade funcionalidades que puedes activar de forma opcional:
...
La API de onboarding heredada está obsoleta a partir de la v4.0 en favor del [Flow & Paywall Builder](adapty-flow-builder). Sigue funcionando, y tu IDE marca los símbolos obsoletos a través de sus anotaciones `@Deprecated` — no hay advertencias en tiempo de ejecución. Estos símbolos se eliminarán en una versión futura, así que planifica la migración de tus onboardings al Flow & Paywall Builder.
