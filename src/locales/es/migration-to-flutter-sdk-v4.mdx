---
title: "Migrar Adapty Flutter SDK a v4.0"
description: "Migra al Adapty Flutter SDK v4.0 reemplazando las APIs de paywall por APIs de flow, compatibles tanto con Flow Builder como con Paywall Builder."
metadataTitle: "Migración al Adapty Flutter SDK v4.0 | Documentación de Adapty"
---
...
Renombra el método y pasa el `AdaptyFlow` mediante el parámetro `flow`. Los demás parámetros (`loadTimeout`, `preloadProducts`, `customTags`, `customTimers`, `customAssets`, `productPurchaseParams`) no cambian, ni tampoco los métodos de la vista `present`, `dismiss` y `showDialog`:

:::note
`customTimers` sigue existiendo, pero solo afecta a los paywalls del Paywall Builder heredado. El [temporizador de cuenta atrás](flow-timer) de un flow se rige por el comportamiento configurado en el Flow Builder, por lo que el flow ignora lo que pases aquí.
:::
