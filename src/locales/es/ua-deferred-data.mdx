---
title: "Deeplinks diferidos en Adapty Attribution"
description: "Configura deeplinks diferidos en Adapty Attribution."
metadataTitle: "Deeplinks diferidos | Adapty Attribution | Documentación de Adapty"
keywords: ['adapty attribution', 'adapty ua', 'adquisición de usuarios', 'deeplink', 'deeplink diferido']
---
Los deeplinks diferidos te permiten pasar datos personalizados a tu app cuando los usuarios la instalan después de hacer clic en tus anuncios. Por ejemplo, puedes llevarlos a una sección específica de tu app justo después de que la instalen y la abran.

:::important
A partir de la versión 4.1 del SDK de Adapty, Adapty Attribution está desactivado por defecto. El callback `onInstallationDetailsSuccess` solo se ejecuta después de que [habilites Adapty Attribution al activar el SDK](user-acquisition#step-1-enable-adapty-attribution-in-the-sdk).
:::
...
3. Después de que el usuario instala tu app y la abre por primera vez, Adapty recupera los datos almacenados y tu app recibe los parámetros personalizados, lo que te permite reaccionar a distintos valores en el código de la app.
...
2. En tu plataforma publicitaria (Meta, TikTok, Google Ads, etc.), pega el enlace en el campo de URL de destino del anuncio y añade los parámetros de datos diferidos como parámetros de consulta adicionales, cada uno precedido de `&`. Por ejemplo, para enviar a los usuarios de iOS a una pantalla de "Bienvenida" tras la instalación, añade `&ios_deferred_data=welcome`. La URL de destino final tendrá este aspecto:
...
3. Responde a los parámetros en el código de tu app. Ten en cuenta que los parámetros de datos diferidos están en el parámetro `payload`, y que `payload` es un JSON escapado, por lo que necesitas parsearlo en el código de tu app.

Por ejemplo, así puedes gestionar las instalaciones donde `ios_deferred_data` es `welcome`:
