...
En el SDK v4, no es necesario pasar un código de idioma al obtener un flow: `getFlow` devuelve el flow con todas sus localizaciones.

- **Flows creados en el builder**: `createFlowView` no acepta parámetro de idioma, y el SDK no lee el idioma del dispositivo. El flow se renderiza en su [idioma predeterminado](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — en iOS, en `en` cuando el flow tiene una localización en inglés. Para confirmar qué localización aplicó Adapty, lee `view.locale` en el controlador que devuelve `createFlowView`. Requiere React Native SDK 4.0.2 o posterior y es `undefined` en versiones anteriores.
