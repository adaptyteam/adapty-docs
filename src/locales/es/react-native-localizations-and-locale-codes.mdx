...
4. Si tampoco se encuentra ninguna coincidencia, Adapty devuelve el contenido para el idioma predeterminado del flow

Esta forma hace que `'pt_BR'`, `pt-BR` y `pt-br` se resuelvan en la misma localización.
...
- **Flows creados en el builder**: el SDK no lee el idioma del dispositivo, así que resuélvelo en tu app y pásalo como el parámetro `locale` de `createFlowView`, o en la prop `params` del componente embebido `AdaptyFlowView`. Es opcional — omítelo y el flow se renderiza en `en`, o en su [idioma predeterminado](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) cuando el flow no tiene localización en `en`. Si solicitas una localización que el flow no tiene, la vista vuelve al idioma predeterminado del flow sin error, y cualquier cadena que falte en la localización elegida se obtiene de la predeterminada.
