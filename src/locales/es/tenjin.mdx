...
### Conectar tu app a Tenjin \{#connect-your-app-to-tenjin\}

Usa el método del SDK `Adapty.updateExternalAttribution()` (llamado `updateAttribution()` antes de la versión 4.1 del SDK) para obtener los datos de atribución de Tenjin y pasárselos a Adapty.
...
| ----------------------------- | ---------------- | ------------------------------------------------------------ |
| **price**                     | Float            | El precio unitario del artículo comprado en la unidad estándar de la moneda (p. ej., USD se reporta en dólares). |
| **locale**                    | String           | El idioma y región del dispositivo. Para Android: `Locale.getDefault().toString()`. Para iOS: `[[NSLocale currentLocale] localeIdentifier]`. |
| **country**                   | String           | El código de país estándar ISO (p. ej., US para Estados Unidos). |
| **postcut**                   | String (Boolean) | Indica si la compra se envió tras el recorte de la plataforma. 1 para verdadero, 0 para falso. |
| **currency**                  | String           | El código de moneda ISO (p. ej., USD para dólares estadounidenses). |
...
| **bundle_id**                 | String           | El identificador del paquete de la app (p. ej., `com.example.app`). |
| **ip_address**                | String (IPv4)    | La dirección IP del usuario. Se usa para determinar el país. |
| **os_version**                | String           | La versión del SO del dispositivo. Para Android: `String.valueOf(Build.VERSION.SDK_INT)`. Para iOS: `[[UIDevice currentDevice] systemVersion]`. |
...
| **advertising_id**            | UUID             | El ID publicitario del dispositivo. Obligatorio para Android. En iOS puede estar vacío o con todos ceros. |
| **os_version_release**        | String           | La versión de lanzamiento del SO. Para Android: `String.valueOf(Build.VERSION.RELEASE)`. Para iOS: `[[UIDevice currentDevice] systemVersion]`. |
...
### Algunos eventos aparecen como fallidos en el Event Feed \{#some-events-show-as-failed-in-the-event-feed\}

Tenjin solo acepta los eventos enumerados en [Configurar eventos y etiquetas](#configure-events-and-tags). El resto de eventos sigue generando una fila en el Event Feed, marcada como entrega fallida con el tooltip "The event type is not supported for integration." Esto incluye `access_level_updated`, que no encontrarás en la sección **Events names**, ya que es exclusivo de la [integración con webhook](webhook).