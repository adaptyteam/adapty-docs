...
## Gestiona los eventos de webhook \{#handle-webhook-events\}

Los webhooks suelen entregarse entre 5 y 60 segundos después de que ocurre el evento. Sin embargo, los eventos de cancelación pueden tardar hasta 2 horas en entregarse tras la cancelación de una suscripción por parte del usuario.

La entrega de webhooks de Adapty es de tipo "al menos una vez": cada evento recibe al menos un intento de entrega, y Adapty reintenta en caso de fallo en lugar de descartarlo. Si el código de estado de respuesta de tu servidor está fuera del rango 200-404, Adapty reintenta la entrega con retroceso exponencial. El primer reintento ocurre aproximadamente **1 minuto** después del fallo inicial, duplicándose en cada intento posterior — hasta 9 reintentos distribuidos a lo largo de 24 horas. Te recomendamos configurar tu webhook para realizar solo una validación básica del cuerpo del evento recibido de Adapty antes de responder. Si tu servidor no puede procesar el evento y no quieres que Adapty reintente, usa un código de estado dentro del rango 200-404. Además, gestiona las tareas que consumen tiempo de forma asíncrona y responde a Adapty rápidamente. Si Adapty no recibe una respuesta en 10 segundos, considerará el intento como fallido y volverá a intentarlo.

Los eventos no tienen garantía de llegar en orden — consulta [Campos de evento](webhook-event-types-and-fields#event-fields) para saber cómo ordenarlos y deduplicarlos por tu cuenta.

:::note
Se aplican dos límites adicionales a los reintentos, además del calendario anterior. Si tu endpoint no ha recibido ninguna entrega correcta durante 24 horas, Adapty deja de reintentar los eventos fallidos hasta que se complete una entrega con éxito. Además, un evento que no se haya entregado en las 24 horas siguientes a su creación deja de reintentarse. Los eventos que fallaron durante una interrupción prolongada no se reenvían automáticamente; contacta con el [soporte de Adapty](mailto:support@adapty.io) para reenviarlos.
:::
