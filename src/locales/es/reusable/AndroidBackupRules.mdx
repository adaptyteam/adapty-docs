...
<Callout type="note">
Estos cambios deben realizarse en el directorio de la plataforma Android (que normalmente se encuentra en la carpeta `android/` de tu proyecto).
</Callout>
...
#### 1. Añade el espacio de nombres `tools` a tu manifiesto \{#1-add-the-tools-namespace-to-your-manifest\}
...
En el mismo archivo `AndroidManifest.xml`, actualiza la etiqueta `<application>` para que tu app proporcione los valores finales e indique al fusionador de manifiestos que reemplace los valores de las bibliotecas:
...
Crea archivos XML en el directorio `res/xml/` de tu proyecto Android que combinen las reglas de Adapty con las de otros SDKs. Android usa diferentes formatos de reglas de copia de seguridad según la versión del sistema operativo, por lo que crear ambos archivos garantiza la compatibilidad con todas las versiones de Android que soporta tu app.

<Callout type="note">
Los ejemplos a continuación muestran AppsFlyer como SDK de terceros de ejemplo. Reemplaza o añade reglas para cualquier otro SDK que estés usando en tu app.
</Callout>
...
**Para Android 11 e inferior** (usa el formato heredado de contenido de copia de seguridad completa):
