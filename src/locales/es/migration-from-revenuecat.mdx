...
Tu plan de migración tiene 5 pasos lógicos y tarda una media de 2 horas. El 90% de las migraciones se completan en menos de un día laborable.

1. Conoce las diferencias principales; crea y prepara una cuenta de Adapty _(5 minutos)_;
...
3. Configura las [notificaciones del servidor de Apple App Store](enable-app-store-server-notifications) para Adapty y (opcionalmente) el [reenvío de eventos sin procesar](enable-app-store-server-notifications#raw-events-forwarding) _(5 minutos)_;
4. Prueba y publica las actualizaciones de tu app _(30 minutos);_
5. (Opcional) Exporta tus datos históricos de RevenueCat en formato CSV usando las [exportaciones de datos programadas de RevenueCat](https://www.revenuecat.com/docs/integrations/scheduled-data-exports) _(5 minutos);_
...
Todos los usuarios que alguna vez hayan activado una suscripción se migrarán a Adapty en cuanto abran la nueva versión de tu app con el SDK de Adapty. La validación del estado de la suscripción y el acceso premium se restaurarán automáticamente.
:::

Antes de publicar una nueva versión de tu app con el SDK de Adapty, asegúrate de revisar nuestra [lista de verificación de lanzamiento](release-checklist).
...
:::warning
No te apresures con la importación de datos históricos

Debes esperar al menos una semana después del lanzamiento con el SDK antes de importar los datos históricos. Durante ese tiempo recibiremos toda la información sobre los precios de compra desde el SDK, por lo que los datos que importes serán más relevantes.
:::
...
### Lo que no se transmite \{#what-doesnt-come-across\}

- Cada transacción se revalida con el store durante la importación, por lo que las filas que el store ya no reconoce se descartan.
- Las filas sin un ID de transacción real del store — por ejemplo, las entitlements promocionales o concedidas manualmente de RevenueCat — se importan como perfiles sin transacciones. Para restaurar el acceso de estos usuarios, concédeles un nivel de acceso con la [API del servidor](api-adapty/operations/grantAccessLevel).
- El historial de reembolsos y problemas de facturación no se transfiere tal cual: un reembolso conserva únicamente su fecha de cancelación, y el estado de problema de facturación de una suscripción se registra en el momento de la importación, no con las fechas históricas.
