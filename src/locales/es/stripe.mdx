...
4. Copia la clave generada del pop-up y ve a [App Settings → Stripe](https://app.adapty.io/settings/stripe) en Adapty. Pega la clave en la sección **Stripe App Restricted API Key** según tu modo. Ten en cuenta que debes generar claves distintas para los modos de prueba y producción. Usa una clave del **Test mode** de Stripe, no de un **Sandbox** de Stripe — Adapty no es compatible con los Sandboxes de Stripe.
...
3. A continuación, copia la URL del webhook que aparece al final de la misma página en Adapty. Ve a [**Developers** → **Webhooks**](https://dashboard.stripe.com/webhooks) en Stripe y haz clic en el botón **Add endpoint**:
...
4. Pega la URL del webhook de Adapty en el campo **Endpoint URL**. Luego elige la **Latest API version** en el campo **Version** del webhook. A continuación, selecciona los siguientes eventos:
...
5. Pulsa "Add endpoint" y luego pulsa "Reveal" bajo "Signing secret". Esta es la clave que se utiliza para decodificar los datos del webhook en el lado de Adapty; cópiala después de revelarla:
...
6. Por último, pega esta clave en **App Settings** → **Stripe** de Adapty, en el campo "Stripe Webhook Secret":
...
:::warning
Adapty relaciona los eventos entrantes de Stripe mediante el par exacto `product_id` + `price_id`. Cambiar un precio en Stripe crea un nuevo `price_id`, y los eventos de compras con el nuevo precio dejan de coincidir silenciosamente: Stripe muestra el webhook como entregado, pero Adapty no registra las transacciones. Después de cualquier cambio de precio, asegúrate de que cada `price_id` que vendas esté añadido a un producto en Adapty.
:::
...
### Stripe muestra una entrega del 100%, pero ningún evento llega a Adapty \{#stripe-shows-100-delivery-but-no-events-reach-adapty\}

Adapty solo captura un secreto de firma de webhook cuando crea el endpoint por sí mismo. Si ya existe un endpoint en la URL de Adapty dentro de tu cuenta de Stripe —por ejemplo, de un intento de conexión anterior o de haber seguido la configuración manual obsoleta indicada arriba—, al volver a conectar se omite la creación de uno nuevo y Adapty nunca recibe (ni vuelve a recibir) el secreto de firma. Todos los eventos siguen llegando al endpoint existente y devuelven un `200`, pero fallan la verificación de firma en el lado de Adapty y se descartan.

Para solucionarlo, en [Developers → Webhooks](https://dashboard.stripe.com/webhooks) elimina el endpoint que apunta a tu URL de webhook de Adapty y, a continuación, vuelve a conectar la integración de Stripe desde [App Settings → Stripe](https://app.adapty.io/settings/stripe) para que Adapty pueda crear uno nuevo y capturar su secreto.
...
### Ingresos netos e impuestos \{#net-revenue-and-tax\}

Para calcular `net_revenue_usd` y `net_revenue_local` de una transacción de Stripe, Adapty aplica una deducción fija del 4,5%. Se trata de una estimación de modelado, no del impuesto real recaudado por Stripe — Adapty no lee los importes de impuestos desde Stripe, incluido Stripe Tax, por lo que `tax_amount_usd` y `tax_amount_local` siempre son `0` para las transacciones de Stripe.
...
### Una cuenta de Stripe conectada a varias aplicaciones de Adapty \{#one-stripe-account-connected-to-several-adapty-apps\}

El endpoint del webhook que recibe un evento determina qué aplicación de Adapty lo procesa — cada aplicación de Adapty tiene su propia URL de endpoint —, no la clave de API restringida. Cada endpoint que registres en Stripe recibe todos los eventos, y cada aplicación procesa su copia de forma independiente. Una aplicación ignora los eventos de productos que no están añadidos a ella, por lo que las aplicaciones con conjuntos de productos separados conviven sin problemas. Sin embargo, si el mismo producto y precio de Stripe están añadidos a más de una aplicación, cada una de esas aplicaciones registra la transacción.
...
Ten en cuenta que `variation_id` solo se lee desde los metadatos en los objetos Stripe Subscription (`sub_...`) y Checkout Session (`ses_...`):
...
Adjust no tiene campos de Stripe: Adapty envía una compra de Stripe a Adjust con el token de la aplicación de la plataforma en la que el comprador instaló tu app. Consulta [Compras web (Stripe y Paddle)](adjust#web-purchases-stripe-and-paddle).
