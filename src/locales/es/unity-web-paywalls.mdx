...
2. `OpenWebPaywall(paywall)`, que genera URLs por paywall sin añadir los datos del producto a las URLs. Úsalo cuando los productos de tu paywall en Adapty difieran de los del paywall web.

En SDK v4, el argumento `paywall` recibe un `AdaptyFlowPaywall`, que es una de las variantes del paywall en `flow.Paywalls`. Consulta la [guía de migración](migration-to-unity-sdk-v4).
:::
