...
1. `openWebPaywall(product)` que genera URLs por paywall y añade los datos del producto a las URLs.
2. `openWebPaywall(paywall)` que genera URLs por paywall sin añadir los datos del producto a las URLs. En SDK v4 este método recibe un `AdaptyFlowPaywall`, una variación de paywall del flow obtenido. Comprueba que `flow.paywalls` no esté vacío antes de acceder a sus elementos por índice, por ejemplo `flow.paywalls[0]`.
:::
