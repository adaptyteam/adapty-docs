...
La forma en que creas productos en Adapty depende de si ya los tienes en los stores:
- **[Si los productos no existen todavía en App Store y/o Google Play, créalos en Adapty y publícalos en los stores directamente](#create-product-and-push-to-store)**.
- **[Si los productos ya existen en App Store y/o Google Play, créalos en Adapty y conecta los productos existentes de los stores.](#create-product-and-connect-existing-store-products)**

El precio que introduces aquí no se puede cambiar después de la creación, y un cambio de precio en la consola del store [no se refleja en Adapty](store-sync#product-and-price-discrepancies).

:::tip
También puedes crear productos de forma programática usando la [CLI para desarrolladores](developer-cli-reference#adapty-products-create).
:::

## Crear un producto y publicarlo en el store \{#create-product-and-push-to-store\}
...
Si configuraste la integración con App Store hace tiempo, comprueba también que hayas [añadido la clave API de App Store Connect](app-store-connection-configuration#step-6-add-app-store-connect-api-key).
:::
...
2. Haz clic en **Create product** en la esquina superior derecha. Adapty admite todos los tipos de productos: suscripciones, no consumibles \(incluido el acceso de por vida\) y consumibles.
...
4. Introduce los siguientes datos:
    - **Product name**: escribe el nombre del producto que se usará en el Adapty Dashboard. El nombre es principalmente para tu referencia, así que elige el que te resulte más cómodo usar en el Adapty Dashboard.
    - **Access Level**: selecciona el [nivel de acceso](access-level) al que pertenece el producto. El nivel de acceso determina las funciones que se desbloquean al comprar el producto. Ten en cuenta que esta lista solo contiene los niveles de acceso creados previamente. El nivel de acceso `premium` se crea en Adapty por defecto, pero también puedes [añadir más niveles de acceso](access-level).
    - **Subscription duration**: selecciona la duración de la suscripción en la lista. No podrás cambiarla una vez creado el producto.
        ...
        - **Non-Subscriptions**: para productos únicos sin duración. Una compra de este tipo no otorga un nivel de acceso — aparece en el array `nonSubscriptions` del perfil de usuario, y tu app decide qué desbloquear.
        - **Consumables**: los artículos consumibles pueden comprarse varias veces y consumirse durante la vida útil de la aplicación. Ejemplos: moneda del juego y extras. Una compra consumible tampoco otorga un nivel de acceso. Para otorgar un nivel de acceso desde una compra única, usa **Lifetime**.
...
## Crear un producto y conectar productos existentes del store \{#create-product-and-connect-existing-store-products\}
...
**Si todavía no tienes ningún producto creado**, considera seguir la guía [Push to stores](#create-product-and-push-to-store) para crearlos tanto en Adapty como en los stores al mismo tiempo.
:::
...
4. Introduce los siguientes datos:
    ...
    - **Product name**: introduce el nombre del producto que se usará en el Adapty Dashboard. El nombre es principalmente para tu referencia, así que elige el que te resulte más cómodo para utilizar en el Adapty Dashboard.
...
- **Duración de la suscripción**: selecciona la duración de la suscripción de la lista. No puedes cambiarla una vez creado el producto.
      - **Weekly/Monthly/2 Months/3 Months/6 Months/Annual**: La duración de la suscripción.
      - **Lifetime**: Usa un período de por vida para los productos que desbloquean las funciones premium de la app para siempre.
      - **Non-Subscriptions**: Para productos de una sola vez sin duración. Una compra de no suscripción no otorga un nivel de acceso — aparece en el array `nonSubscriptions` del perfil del usuario, y tu app decide qué desbloquear.
      - **Consumables**: Los artículos consumibles se pueden comprar varias veces y se pueden utilizar durante la vida de la aplicación. Ejemplos son la moneda del juego y los extras. Una compra consumible tampoco otorga un nivel de acceso. Para otorgar un nivel de acceso desde una compra única, usa **Lifetime**.
   - **Price (USD)**: El precio del producto en USD. Si tu producto ya está en el store, este valor no afectará su precio real en el store; puedes seleccionar cualquier valor de la lista. Más adelante, puedes [personalizar precios para diferentes regiones](edit-product#set-country-specific-prices) directamente en el Adapty Dashboard.
...
:::warning
Cada ID de producto de la store debe coincidir exactamente con la store. Si no coincide, no se generará ningún error: la compra se procesará, pero el cliente no recibirá ningún nivel de acceso.
:::
