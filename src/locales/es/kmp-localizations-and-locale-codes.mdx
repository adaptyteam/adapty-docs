...
3. Si no encuentra ninguna coincidencia, Adapty toma la subcadena antes del primer guion (`pt` de `pt-br`) y busca la localización correspondiente
4. Si tampoco encuentra ninguna coincidencia, Adapty devuelve el contenido para el idioma predeterminado del flow

Esta forma permite que `'pt_BR'`, `pt-BR` y `pt-br` se resuelvan en la misma localización.

## Implementación de localizaciones \{#implementing-localizations\}

En el SDK v4, no pasas un código de localización al obtener un flow — `getFlow` devuelve el flow con todas sus localizaciones, y Adapty aplica una cuando se construye la vista del flow.

- **Flows creados en el builder**: el SDK no lee el idioma del dispositivo, así que resuélvelo en tu app y pásalo como parámetro `locale` de `createFlowView`. Es opcional: si lo omites, el flow se renderiza en `en`, o en su [idioma predeterminado](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) cuando el flow no tiene localización en `en`. Si solicitas una localización que el flow no tiene, la vista recurre al idioma predeterminado sin mostrar ningún error, y cualquier cadena que falte en la localización elegida se toma de la predeterminada.
