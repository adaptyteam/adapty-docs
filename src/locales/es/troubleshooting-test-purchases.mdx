...
## La compra en sandbox se rechaza como no autorizada \{#the-sandbox-purchase-is-refused-as-unauthorized\}

La store muestra un aviso del tipo `You are not authorized to make purchases of this in-app in sandbox at this time`. Apple rechaza la compra, no Adapty. Comprueba lo siguiente en el dispositivo de prueba y en App Store Connect:

- **Las compras in-app están permitidas en el dispositivo**: En **Settings** > **Screen Time** > **Apps & Websites** > **App Store**, **In-app Purchases** debe estar en **Allowed**. Apple describe esta configuración en [Turn off Apple In-App Purchases from Screen Time](https://support.apple.com/en-us/102470).
- **La cuenta sandbox está iniciada sesión en el lugar correcto**: Inicia sesión en [**Settings** > **Developer** > **Sandbox Apple Account**](test-purchases-in-sandbox#step-4-switch-to-sandbox-test-account), no desde la configuración principal de tu cuenta de Apple. La instrucción de Apple es "usar la opción de inicio de sesión Sandbox en Configuración, no la configuración principal de tu cuenta de Apple."
- **La dirección de correo electrónico del tester no es una cuenta de Apple existente**: Apple indica que reutilizar una "causará errores durante las pruebas." No puedes editar la dirección de correo de un tester una vez creado, así que [crea un nuevo tester](test-purchases-in-sandbox#step-1-create-sandbox-test-account-in-app-store-connect) con una dirección diferente.
