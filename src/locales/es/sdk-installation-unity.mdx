...
El SDK de Adapty es compatible con iOS 13.0 en adelante, pero requiere iOS 15.0+ para trabajar con paywalls creados en el Paywall Builder. Adapty SDK 4.0 (beta) —que añade soporte para [Flow Builder](adapty-flow-builder)— requiere **iOS 15.0+** para toda la app: un validador de compilación en el Unity Editor detiene la compilación de iOS si el deployment target es inferior.

:::info
Adapty es compatible con Google Play Billing Library hasta la versión 8.x. De forma predeterminada, Adapty utiliza Google Play Billing Library v7.0.0. Para usar una versión más reciente, [sobreescribe la dependencia de Billing](https://developer.android.com/google/play/billing/integrate#dependency) en tu build de Android.
...
SDK 4.0 — que añade soporte para [Flow Builder](adapty-flow-builder) — es una versión preliminar. Para instalarlo mediante Unity Package Manager, añade la etiqueta beta a la URL de Git:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Si lo instalas mediante el paquete de Unity, descarga `adapty-unity-plugin-4.0.0-beta.1.unitypackage` desde la [versión 4.0.0-beta.1](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.0.0-beta.1).

Dos cambios en la configuración de compilación vienen con la v4: el SDK nativo de Adapty para iOS se declara como un paquete Swift remoto y ya no se instala mediante CocoaPods:

- Actualiza el External Dependency Manager a **1.2.188 o posterior** — las versiones anteriores no admiten dependencias de Swift Package Manager. Esta es la versión que el SDK 4.0 declara como dependencia de pares, por lo que Unity te avisará si tu proyecto tiene una versión más antigua.
- Los pasos de CocoaPods mencionados anteriormente (`iOS Resolver -> Install Cocoapods`, abrir `Unity-iPhone.xcworkspace`) aplican solo al SDK 3.x. En el SDK 4.0, EDM añade el paquete Swift al proyecto Xcode generado de forma automática.
- Establece el deployment target de iOS en **15.0 o posterior**. Un validador de compilación en el Unity Editor detiene la compilación para iOS si no se cumple este requisito.

Consulta la [guía de migración](migration-to-unity-sdk-v4) para ver la lista completa de cambios del SDK 4.0.
...
:::note
En la versión 4.0 del SDK, las interfaces de listener siguen la convención de prefijo I de C#: implementa `IAdaptyEventListener` en lugar de `AdaptyEventListener`. Los métodos no cambian. Consulta la [guía de migración](migration-to-unity-sdk-v4).
:::

:::important
Espera el callback de finalización de `Activate` antes de llamar a cualquier otro método del SDK de Adapty. Consulta [Orden de llamadas en Unity SDK](unity-sdk-call-order) para ver la secuencia completa.
:::
