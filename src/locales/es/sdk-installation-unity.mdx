...
metadataTitle: "Instalar el SDK de Adapty en Unity | Documentación de Adapty"
keywords: ['instalar sdk', 'instalación sdk', 'instalar sdk unity', 'google play billing library', 'gpbl', 'biblioteca de facturación']
rank: 30
---
...
- **AdaptyUI**: Este módulo renderiza flows, además de los paywalls del constructor antiguo.
...
El SDK de Adapty es compatible con iOS 13.0+, pero requiere iOS 15.0+ para trabajar con paywalls creados en el Paywall Builder. Adapty SDK 4.1 — que añade soporte para [flows](adapty-flow-builder) — requiere **iOS 15.0+** para toda la app: un validador de compilación en el Unity Editor detiene la compilación de iOS si el deployment target es inferior. Compilar para iOS con SDK 4.x también requiere **Xcode 26** o posterior, ya que el SDK nativo de iOS que añade como paquete Swift está compilado con Swift tools 6.2.

:::info
El SDK de Adapty incluye su propia copia de la Google Play Billing Library: no necesitas añadirla a tu app.

Desde la versión 3.17.0, el SDK de Adapty para Unity utiliza Billing Library v8 por defecto. Las versiones 3.15.0 a 3.15.2 usan v7, y también funcionan cuando otra dependencia de tu compilación Android eleva Billing Library a v8.

La compatibilidad con una versión de Billing Library no significa que Adapty sea compatible con todas las funcionalidades que Google introdujo en ella. Antes de adoptar una nueva capacidad de facturación de Google Play, consulta [Producto en Play Store](android-products).
:::
...
Instala el SDK de Adapty mediante Unity Package Manager usando una Git URL:
...
Descarga el [`adapty-unity-plugin-*.unitypackage`](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Releases) de GitHub e impórtalo en tu proyecto.
...
1. Instala el [plugin External Dependency Manager (EDM)](https://github.com/googlesamples/unity-jar-resolver#getting-started). El SDK de Adapty lo utiliza para gestionar las dependencias de iOS y las dependencias de gradle de Android.
...
3. Al compilar tu proyecto de Unity para iOS, obtendrás el archivo `Unity-iPhone.xcworkspace`, que debes abrir en lugar de `Unity-iPhone.xcodeproj`; de lo contrario, no se usarán las dependencias de Cocoapods.

### Adapty SDK 4.1

SDK 4.1 — que añade soporte para [flows](adapty-flow-builder) — es la primera versión estable de la línea 4.x. Para instalarlo a través del Unity Package Manager, añade la etiqueta de versión a la URL de Git:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

Si instalas mediante el paquete de Unity, descarga `adapty-unity-plugin-4.1.0.unitypackage` desde la [versión 4.1.0](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0).

Dos cambios en la configuración de compilación vienen con la versión 4.x: el SDK nativo de Adapty para iOS se declara como un paquete Swift remoto y ya no se instala a través de CocoaPods:

- Actualiza el External Dependency Manager a **1.2.188 o posterior** — las versiones anteriores no admiten dependencias de Swift Package Manager. Esta es la versión que declara el SDK 4.1 como dependencia entre pares, por lo que Unity te avisará si tu proyecto tiene una versión más antigua.
- Los pasos de CocoaPods mencionados anteriormente (`iOS Resolver -> Install Cocoapods`, abrir `Unity-iPhone.xcworkspace`) son exclusivos del SDK 3.x. Con el SDK 4.1, EDM añade el paquete Swift al proyecto Xcode generado de forma automática.
- Establece el deployment target de iOS en **15.0 o posterior**. Un validador de compilación en el Unity Editor detiene el build de iOS si no se cumple este requisito.

Consulta la [guía de migración](migration-to-unity-sdk-v4) para ver la lista completa de cambios en la línea 4.x.

## Activar el módulo Adapty del SDK \{#activate-adapty-module-of-adapty-sdk\}

Activa el SDK en el código de tu aplicación.

:::note
El SDK solo necesita activarse una vez en tu aplicación.
:::
...
:::note
En el SDK 4.1, las interfaces de listener siguen la convención de prefijo I de C# — implementa `IAdaptyEventListener` en lugar de `AdaptyEventListener` — y la interfaz requiere un método adicional, `OnReceivePromotedPurchase`. Consulta la [guía de migración](migration-to-unity-sdk-v4).
:::
...
Crea un script para escuchar los eventos de Adapty. Nómbralo `AdaptyListener` en tu escena. Te recomendamos usar el método `DontDestroyOnLoad` en este objeto para asegurarte de que persista durante toda la vida útil de la aplicación.
...
- Si usas el [Flow & Paywall Builder](adapty-flow-builder), primero [activa el módulo AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) a continuación y luego sigue la [guía de inicio rápido](unity-quickstart-paywalls).
- Si creas tu propia interfaz de paywall, consulta la [guía de inicio rápido para paywalls personalizados](unity-quickstart-manual).

## Activar el módulo AdaptyUI del SDK \{#activate-adaptyui-module-of-adapty-sdk\}

Si tienes previsto usar el [Flow & Paywall Builder](adapty-flow-builder) y has instalado el módulo AdaptyUI, necesitas que AdaptyUI esté activo. Puedes activarlo durante la configuración:
...
#### La app se bloquea al mostrar un paywall en Android \{#app-crashes-when-a-paywall-is-displayed-on-android\}

Si tu app se bloquea en Android al mostrar un paywall, es posible que falte el plugin de Kotlin en la configuración de Gradle. Para añadirlo:
