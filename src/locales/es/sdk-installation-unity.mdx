...
El SDK de Adapty es compatible con iOS 13.0+, pero requiere iOS 15.0+ para trabajar con paywalls creados en el paywall builder. Adapty SDK 4.1 — que añade compatibilidad con [flows](adapty-flow-builder) — requiere **iOS 15.0+** para toda la aplicación: un validador de compilación en Unity Editor detiene la compilación de iOS si el deployment target es inferior. Compilar para iOS con SDK 4.x también requiere **Xcode 26** o posterior, ya que el SDK nativo de iOS que añade como paquete Swift está compilado con Swift tools 6.2.

:::info
Adapty Unity SDK 4.0 y versiones posteriores funcionan con Google Play Billing Library v8.

La compatibilidad con una versión de la Billing Library no significa que Adapty sea compatible con todas las funcionalidades que Google introdujo en ella. Antes de adoptar una nueva capacidad de facturación de Google Play, consulta [Producto en Play Store](android-products).
:::
