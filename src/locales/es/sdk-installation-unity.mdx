...
El SDK de Adapty es compatible con iOS 13.0+, pero requiere iOS 15.0+ para trabajar con paywalls creados en el Paywall Builder. Adapty SDK 4.1 — que añade compatibilidad con [flows](adapty-flow-builder) — requiere **iOS 15.0+** para toda la aplicación: un validador de compilación en el Unity Editor detiene la compilación de iOS si el deployment target es inferior. Compilar para iOS con SDK 4.x también requiere **Xcode 26** o posterior, ya que el SDK nativo de iOS que se añade como paquete Swift se compila con Swift tools 6.2.

:::info
A partir de la versión 3.17.0, el SDK de Adapty para Unity utiliza Billing Library v8 por defecto. Las versiones 3.15.0 a 3.15.2 usan v7 y también funcionan cuando otra dependencia en tu build de Android eleva la Billing Library a v8.
