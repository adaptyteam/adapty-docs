...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Inicializa tu SDK de MMP o analíticas (AppsFlyer, Adjust, PostHog, Branch)                                     | Al arrancar la app, lo primero                                                      | Espera el callback de UID del MMP, por ejemplo `getAppsFlyerUID`.                                |
| 2a   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                       | Al arrancar la app, después del paso 1, si tienes el ID de usuario                             | Recomendado. No se crea ningún perfil anónimo.                                             |
| 2b   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").build())` sin `customerUserId`                      | Al arrancar la app, después del paso 1, si no tienes el ID de usuario (o nunca lo recoges) | Adapty crea un perfil anónimo.                                                           |
| 3    | `Adapty.setIntegrationIdentifier(AdaptyIntegrationIdentifier.appsflyerId(uid)) { error -> ... }`               | Después del paso 2, antes de cualquier llamada de acción del usuario                                              | Necesario para que los IDs del MMP se asocien al perfil correcto. Pasa una lista de identificadores para configurar varios MMPs en una sola llamada. |
| 4    | `Adapty.identify("YOUR_USER_ID") { error -> ... }`                                                            | Después del paso 3 (o del paso 2 si no hay MMP), antes del paso 5 — solo en el flujo 2b con autenticación | Usa el callback de finalización. Las llamadas concurrentes durante `identify` pueden caer en el perfil anónimo. |
| 5    | `getPaywall`, `getPaywallProducts`, `restorePurchases`, `makePurchase`, `updateExternalAttribution`, `updateProfile`  | Después del paso 4 si llamas a `identify`; si no, después del paso 3 (o del paso 2 si no hay MMP)      | Estas llamadas necesitan un perfil estable.                                                             |
...
Si llamas a `updateProfile` para establecer atributos personalizados y luego obtienes un placement (`getPaywall`) en la misma sesión, espera a que `updateProfile` termine antes de realizar la obtención. Si se lanzan juntos, el SDK puede calcular el hash del segmento del placement a partir de atributos que aún no han terminado de propagarse, lo que añade un reintento antes de que la obtención se resuelva. Consulta [Gestionar errores en el SDK de Android](android-sdk-error-handling) si el propio reintento falla.
