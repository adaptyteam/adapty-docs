...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Inicializa tu MMP o SDK de analítica (AppsFlyer, Adjust, PostHog, Branch)                                     | Al arrancar la app, lo primero                                                      | Espera al callback del UID del MMP, por ejemplo `getAppsFlyerUID`.                                |
| 2a   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                       | Al arrancar la app, después del paso 1, si ya tienes el customer user ID                             | Recomendado. No se crea ningún perfil anónimo.                                             |
| 2b   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").build())` sin `customerUserId`                      | Al arrancar la app, después del paso 1, si no tienes el customer user ID (o nunca lo recopilas) | Adapty crea un perfil anónimo.                                                           |
| 3    | `Adapty.setIntegrationIdentifier("appsflyer_id", uid)` para cada MMP                                           | Después del paso 2, antes de cualquier llamada iniciada por el usuario                                              | Necesario para que los IDs del MMP queden vinculados al perfil correcto.                                               |
| 4    | `Adapty.identify("YOUR_USER_ID") { error -> ... }`                                                            | Después del paso 3 (o del paso 2 si no hay MMP), antes del paso 5 — solo en la ruta 2b con autenticación | Usa el callback de finalización. Las llamadas concurrentes durante `identify` pueden caer en el perfil anónimo. |
| 5    | `getPaywall`, `getPaywallProducts`, `restorePurchases`, `makePurchase`, `updateExternalAttribution`, `updateProfile`  | Después del paso 4 si llamas a `identify`; si no, después del paso 3 (o del paso 2 si no hay MMP)      | Estas llamadas requieren un perfil estable.                                                             |
