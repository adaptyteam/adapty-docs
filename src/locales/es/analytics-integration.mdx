...
description: "Integra herramientas de análisis con Adapty para rastrear y optimizar las suscripciones de usuarios."
metadataTitle: "Guía de integración de análisis | Documentación de Adapty"
---

Adapty envía todos los [eventos de suscripción](events) a servicios de análisis como [Amplitude](amplitude), [Mixpanel](mixpanel) y [AppMetrica](appmetrica). También podemos enviar eventos a tu servidor mediante la integración con [webhook](webhook). Lo mejor de todo es que no tienes que encargarte de enviar ningún evento: nosotros lo hacemos por ti. Solo asegúrate de configurar la integración en el Adapty Dashboard.

Adapty admite la integración con los siguientes servicios de análisis de terceros:
...
:::note
¿No ves tu proveedor de análisis?

¡Cuéntanos! [Crea una solicitud de función](https://adapty.featurebase.app/en?b=6979f233ebd3cffd4f425ba0) y lo consideraremos.
:::
...
| **vendor_product_id**         | str           | ID del producto en Apple App Store, Google Play Store o Stripe. |
...
| **consecutive_payments**      | int           | Número de períodos consecutivos sin interrupciones durante los que un usuario está suscrito. Incluye el período actual. |
| **rate_after_first_year**     | bool          | Booleano que indica que la suscripción cumple los requisitos para una comisión reducida (normalmente el 15%) después de un año de renovaciones continuas. Las comisiones varían según la elegibilidad del programa y el país. Consulta [Comisiones e impuestos del store](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue) para más detalles. |
...
| **paywall_name**              | str           | Nombre del paywall desde el que se originó la transacción.   |
| **paywall_revision**          | int           | Revisión del paywall desde el que se originó la transacción. El valor se establece en 1. |
| **developer_id**              | str           | ID de desarrollador (SDK) del placement desde el que se originó la transacción. |
| **ab_test_name**              | str           | Nombre de la prueba A/B desde la que se originó la transacción. |
| **ab_test_revision**          | int           | Revisión de la prueba A/B desde la que se originó la transacción. El valor se establece en 1. |
...
| **profile_ip_address**        | str           | IP del perfil (puede ser IPv4 o IPv6, con preferencia por IPv4 cuando está disponible). Se actualiza cada vez que cambia la IP del dispositivo. |
...
| **renewed_at**                | ISO 8601 date | Fecha y hora en que se inició más recientemente el período actual del acceso de pago. Hasta la primera renovación, coincide con `activated_at`. |
...
:::warning
Bloquear el análisis externo detiene la entrega de eventos a **todas** las integraciones para este usuario, incluidos los [webhooks](webhook) personalizados. No lo uses si tu backend depende de eventos de webhook para gestionar el acceso de este usuario.

La configuración no es retroactiva: los eventos creados antes de activarla se siguen entregando. Tampoco afecta al análisis propio de Adapty: las transacciones del usuario siguen contando en los gráficos del dashboard.
:::
