...
| Las compras in-app promocionadas se completaban automáticamente, sin posibilidad de interceptarlas | El método delegado `didReceivePromotedPurchase(_:)` y `AdaptyPromotedProduct` delegan la finalización a tu app |

## ⚠️ La atribución de Adapty está desactivada por defecto \{#adapty-attribution-is-disabled-by-default\}

:::warning
Si actualizas al SDK 4.1 y no lo activas explícitamente, la [atribución de Adapty](user-acquisition) deja de funcionar sin avisar: las instalaciones dejan de registrarse y no recibirás ninguna advertencia.
:::

En la versión 4.0 y anteriores, el SDK registraba instalaciones para [Adapty Attribution](user-acquisition) automáticamente. A partir de la versión 4.1, esta función está desactivada por defecto: el SDK no registra instalaciones, y los callbacks delegados `onInstallationDetailsSuccess` y `onInstallationDetailsFail` nunca se ejecutan. `getCurrentInstallationStatus()` devuelve `.notAvailable`.
...
## Retorno de las compras in-app promocionadas en el App Store \{#app-store-promoted-in-app-purchases-return\}

SDK 4.1 recupera el hook para las compras in-app promocionadas en el App Store, que [4.0 eliminó](migration-to-ios-sdk-v4#app-store-promoted-in-app-purchases-temporarily-removed). Con 4.0, la compra se completaba igualmente y Adapty registraba la transacción; simplemente, tu app no podía interceptarla. 4.1 mantiene ese comportamiento por defecto, así que esto es una nueva funcionalidad y no un paso de migración.
...
:::warning
El nuevo mecanismo está basado en StoreKit 2 y requiere **iOS 16.4** o posterior. El método `shouldAddStorePayment` de la versión 3.x funcionaba en versiones anteriores de iOS. Por debajo de iOS 16.4, `didReceivePromotedPurchase` no se ejecuta nunca y las compras promocionadas se completan sin pasar por tu app.
:::
