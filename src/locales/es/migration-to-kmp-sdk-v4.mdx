...
El módulo `adapty-kmp-ui` solo es necesario si renderizas flows y paywalls con la capa Compose Multiplatform (`view.present()`). Consulta [Instalar el SDK de Adapty](sdk-installation-kotlin-multiplatform) para la configuración completa.

Los SDKs nativos subyacentes de Adapty se actualizan a la versión 4.0.0 en ambas plataformas y se resuelven automáticamente — no es necesario realizar cambios en la configuración de compilación. El deployment target de iOS se mantiene en **15.0**, sin cambios en esta versión.
