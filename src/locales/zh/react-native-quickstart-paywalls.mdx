---
title: "在 React Native SDK 中使用流程和付费墙编辑工具启用购买功能"
description: "使用 Adapty 流程和付费墙编辑工具启用应用内购买的快速入门指南。"
metadataTitle: "Adapty 流程与付费墙编辑工具快速入门 | Adapty 文档"
...
- [**流程**](adapty-flow-builder) – 向用户展示产品的页面序列，在无代码 Flow & Paywall Builder 中构建，SDK 通过 `getFlow` 获取。如果您更倾向于用自己的代码构建 UI，请改用付费墙——详见[手动实现付费墙](react-native-quickstart-manual)。
- [**版位**](placements) – 应用中展示流程的位置和时机（如 `main`、`onboarding`、`settings`）。您在看板中将流程绑定到版位，然后在代码中通过版位 ID 请求它们。这样可以轻松运行 A/B 测试，并向不同用户展示不同流程。
...
| 手动创建付费墙 | 🟡 中等 | 您在应用代码中实现付费墙 UI，但仍从 Adapty 获取流程对象，以保持产品方案的灵活性。请参阅[指南](react-native-quickstart-manual)。 |
| 观察者模式 | 🔴 复杂 | 您已有自己的购买处理基础设施并希望继续使用。请注意，观察者模式在 Adapty 中存在一定限制。请参阅[文章](observer-vs-full-mode)。 |

:::important
**以下步骤展示了如何在应用中实现通过 Adapty Flow & Paywall Builder 创建的流程。**

如果您希望自行构建付费墙 UI，请参阅[手动实现付费墙](react-native-quickstart-manual)。
:::

要在应用代码中展示通过 Adapty Flow & Paywall Builder 创建的流程，您只需：

1. **获取流程**：从 Adapty 获取。
2. **展示流程，Adapty 会自动处理购买**：在应用中显示该视图。
3. **处理按钮操作**：将用户交互与应用的响应逻辑关联起来。例如，当用户点击按钮时打开链接或关闭流程。
...
4. [创建版位并将你的流程添加到其中](create-placement)。
5. 在应用代码中[安装并激活 Adapty SDK](sdk-installation-reactnative)。本指南使用 Adapty React Native SDK v4 API。

## 1. 获取流程 \{#1-get-the-flow\}

你的流程与在看板中配置的版位相关联。通过版位，你可以为不同的目标受众展示不同的流程，或者运行 [A/B 测试](ab-tests)。

要获取在 Adapty Flow & Paywall Builder 中创建的流程，请使用 `getFlow` 方法，通过[版位](placements) ID 获取 `flow` 对象。该流程包含展示所需的 UI 元素和样式。
