...
Adapty 渲染的付费墙在 SDK v4 中以流程形式提供，因此以下规则同时适用于两者。

匹配是精确的。SDK 会将你传入的代码与流程的本地化代码逐字符比较：不会改变大小写，不会将下划线（`_`）替换为连字符（`-`），也不会回退到语言子标签。对于带有 `pt-br` 本地化的流程，只有 `pt-br` 能匹配：`pt-BR`、`pt_BR` 和 `pt-PT` 均无法匹配。

当代码未匹配到任何本地化语言时，流程会静默地以其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——SDK 不会返回错误，也不会记录警告。

当代码匹配到某个本地化语言时，Adapty 会将该本地化语言与默认本地化语言合并：匹配到的本地化语言中未定义的字符串和资源会从默认本地化语言中获取。

省略语言区域代码与请求流程的默认本地化并不相同：SDK 会固定替换为 `en`。如果一个流程的默认语言区域是 `de`，但它包含 `en` 本地化，则仍会以 `en` 渲染；只有在没有 `en` 本地化时，才会回退到 `de`。

:::warning
请严格按照看板中配置的格式传入语言区域代码——小写子标签之间用连字符分隔。不要直接传入系统语言区域标识符：`CultureInfo.CurrentCulture.Name` 返回的是 `pt-BR`，这会导致回退到默认本地化。请在应用中先转换该值再传入。
:::
...
- **由 Adapty 渲染的流程和付费墙**：SDK 不会读取设备语言，请在应用中自行解析语言设置，并在[创建视图时传入](#choose-the-localization-of-a-flow)。语言区域代码为可选项——省略时流程以 `en` 渲染；若流程未配置 `en` 本地化，则以其默认语言区域渲染。
- **自定义（远程配置）付费墙**：`GetFlow` 会在 `flow.RemoteConfigs` 中返回所有已配置的本地化内容。每个条目均为 `AdaptyRemoteConfig`，包含 `Locale` 代码和 `Dictionary` 键值对。请自行选择与用户匹配的条目，并设置相应的回退逻辑：
