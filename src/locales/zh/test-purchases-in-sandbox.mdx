...
:::info
我们建议使用真实设备测试应用内购买。虽然沙盒购买可以在模拟器上运行，但要完整测试所有流程（包括支付对话框和生物识别提示），仍需要真实设备。
:::

测试应用内购买主要有两种方式：

- **通过 Xcode 构建并在测试设备上运行**：适合开发人员和 QA 工程师。
...
6. [打开应用并完成测试购买](#step-7-make-test-purchase)。
...
如果 TestFlight 中的应用已更新：

1. 从测试设备上删除该应用。
2. [从 TestFlight 下载应用](#step-3-download-the-app-from-testflight)。
3. [切换到沙盒测试账户](#step-4-switch-to-sandbox-test-account)。
4. [清除购买历史记录](#step-5-clear-purchase-history)。
5. [打开应用并进行测试购买](#step-7-make-test-purchase)。
