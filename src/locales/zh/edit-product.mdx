...
:::warning
在 App Store Connect 或 Google Play Console 中所做的更改不会同步回 Adapty。Adapty 中显示的价格在您创建产品时设定，如果您在商店中修改了价格，该价格不会自动更新。这不会影响您的收入分析，因为[收入报告使用客户实际支付的金额](store-sync#product-and-price-discrepancies)。
:::

:::note
如果您更改了访问等级，该更改仅对新订阅生效。对于现有订阅者，当前访问等级保持不变，并将在下次订阅续期时自动更新。
:::
