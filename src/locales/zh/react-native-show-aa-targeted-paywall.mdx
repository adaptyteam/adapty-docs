...
description: "在 React Native 中使用 AdaptyProfile.appliedAttributionSources，立即显示付费墙，并在应用 Apple Ads 归因后为 Apple Ads 用户升级付费墙。"
...
1. 立即调用 `getPaywall`。由于尚未应用任何归因，Adapty 会根据默认目标受众解析请求，用户将立即看到付费墙。
2. 当 `'apple_search_ads'` 出现时，再次调用 `getPaywall`。此时 Adapty 会根据 Apple Ads 目标受众解析请求，并返回定向付费墙，替换之前的那个。

`appliedAttributionSources` 可以为空或不存在。这意味着以下情况之一：
...
:::important
在每次后续启动时，缓存的用户画像中已经包含 `'apple_search_ads'`（在 `appliedAttributionSources` 中），因此第一次调用 `getPaywall` 就会直接返回基于 Apple Ads 细分的付费墙——不会有第二次请求，也不会出现可见的变化。两步流程只在首次启动时有意义，此时归因数据仍在传输中。
:::
