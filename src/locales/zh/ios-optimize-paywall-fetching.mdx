...
后续的 `getFlow` 仅在传入 `fetchPolicy: .returnCacheDataElseLoad` 或 `.returnCacheDataIfNotExpiredElseLoad(maxAge:)` 时才会读取该缓存。默认策略 `.reloadRevalidatingCacheData` 每次调用都会请求网络，因此预加载配合默认策略会多消耗一次请求，毫无收益——预热的缓存副本仅作为离线备用。

适合在以下场景使用：当你已知当前会话需要哪些版位，但暂时不想展示它们时——例如，在 `activate` 和 `identify` 完成后，为用户尚未点击的按钮背后的流程提前预热。
