...
- 这些方法只有在尝试所有版位之后才会抛出错误，错误信息会汇总各版位的失败情况。某个版位失败不会中断其他版位的处理。
- 如果某个版位超时或出现网络错误，SDK 会回退到该版位的默认目标受众变体。其他类型的失败则按原样上报。
- 如果超时在面向目标受众的请求完成之前触发，SDK 仍会在剩余时间内尝试获取默认目标受众变体。
- 预加载只是提前填充缓存，不返回内容——你仍需调用 `getFlow` 或 `getOnboarding` 来展示内容。
- `preloadFlows` 和 `preloadOnboardings` 不涉及媒体资源。请求流程的 UI 配置（`getFlowConfiguration`）会在后台开始缓存图片和视频，但该缓存过程独立于调用本身运行——没有任何回调、代理方法或错误来通知缓存完成时机。
