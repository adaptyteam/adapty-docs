...
:::important
Flow Builder 目前支持 iOS 和 Android SDK v4 及更高版本。其他平台的支持即将推出。
:::

流程（flow）是一个或多个直接嵌入产品的屏幕。你可以在 [Flow Builder](adapty-flow-builder) 中进行设计，无需编写代码。

Adapty SDK 会在每个平台上原生渲染流程。你的应用只需调用 `getFlow`，SDK 就会自动展示屏幕、处理购买并上报事件，无需单独维护 UI 代码或付费墙。
