---
title: "启用购买"
description: "在 Adapty 中添加流程或付费墙以展示您的产品，然后将其附加到版位。"
...
- **流程与付费墙编辑工具（推荐）**：无需编写代码，可视化编辑完整的购买流程。Adapty SDK 会在设备端原生渲染结果，无需编写任何 UI 代码。
- **手动付费墙**：你自己创建付费墙，绑定产品，并在应用代码中自行渲染 UI。
- **Adapty 付费墙编辑工具（旧版）**：无代码付费墙编辑器。

两种方式最终殊途同归：你需要将构建好的内容绑定到一个[版位](placements)。版位是你的应用在运行时调用的入口，用于为不同用户获取对应的内容。

<Tabs groupId="purchase-setup" queryString>
<TabItem value="flow-builder" label="使用流程与付费墙编辑工具" default>

:::important
流程需要 Adapty SDK v4.0 或更高版本。
:::

流程由一个或多个内嵌产品的页面组成，你可以在[流程与付费墙编辑工具](adapty-flow-builder)中进行设计，无需编写任何代码。

Adapty SDK 会在各平台上原生渲染流程。你的应用只需调用 `getFlow`，SDK 便会自动呈现页面、处理购买并上报事件，无需维护单独的 UI 代码或付费墙。
...
了解更多关于 [Adapty Flow & 付费墙编辑工具](adapty-flow-builder) 的内容。
...
在应用代码中，你只需硬编码版位 ID。其他所有内容——运行哪个流程、销售哪些产品、界面如何呈现——都在 Adapty 看板中配置，随时可以修改，无需更新应用。

:::tip
Adapty 支持向不同用户群体展示不同流程，并分析其效果。了解更多关于[目标受众](audience)和 [A/B 测试](ab-tests)的内容。
:::
...
您将在应用代码中渲染此付费墙。 <InlineTooltip tooltip="手动实现付费墙">请参阅适用于您平台的指南：[iOS](ios-implement-paywalls-manually)、[Android](android-implement-paywalls-manually)、[React Native](react-native-implement-paywalls-manually)、[Flutter](flutter-implement-paywalls-manually)、[Unity](unity-implement-paywalls-manually)、[Kotlin Multiplatform](kmp-implement-paywalls-manually)、[Capacitor](capacitor-implement-paywalls-manually)。</InlineTooltip>
...
:::tip
Adapty 支持向不同用户群体展示不同付费墙并分析其效果。了解更多关于[目标受众](audience)和 [A/B 测试](ab-tests)的内容。
:::

</TabItem>
<TabItem value="paywall-builder" label="Adapty Paywall Builder (Legacy)">

在[付费墙编辑工具](adapty-paywall-builder)中构建的付费墙是一个无代码页面，产品直接嵌入其中。Adapty SDK 会原生渲染该页面，无需编写任何 UI 代码。
