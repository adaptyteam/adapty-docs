...
:::tip
**从 SDK v4（测试版）开始**，你可以构建[流程](android-get-pb-paywalls)作为用户引导的更强大替代方案。与在 WebView 中运行的用户引导不同，流程直接在设备上原生渲染——带来更流畅的动画、一致的 Android 外观与体验、更快的加载速度，以及无需依赖 WebView 运行时。请参阅[获取流程和付费墙](android-get-pb-paywalls)和[展示流程和付费墙](android-present-paywalls)以开始使用。
:::
