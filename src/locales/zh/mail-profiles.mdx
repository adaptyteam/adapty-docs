...
- **Adapty SDK**：SDK 从您的应用中收集邮件地址和购买数据。请参阅[将 Adapty 连接到 Adapty Mail](mail-collect-emails)。
- **Adapty Mail API**：您的后端通过服务器对服务器的方式发送用户画像和交易数据。请参阅[通过 API 发送数据](mail-send-data-via-api)。

Adapty Mail 将每封邮件、每次点击和每次购买都关联到一个用户画像。系统优先通过发送系统自身的 ID 来识别用户画像，其次是 `customer_user_id`，最后是邮件地址，因此同一用户从不同来源进入时，最终都会落在同一个用户画像上。Profiles 页面为只读模式，你可以查看用户画像并取消订阅，但无法创建、编辑或删除它们。这些数据由源应用或 API 管理。
