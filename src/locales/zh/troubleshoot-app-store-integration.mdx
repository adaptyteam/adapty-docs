...
## 购买成功但用户失去访问权限 \{#a-purchase-succeeds-but-the-customer-loses-access\}

用户完成付款后短暂获得了访问权限，但下次应用从 Adapty 刷新用户画像时访问权限消失。Apple 已扣款，但交易从未到达 Adapty——在此之前，该用户画像没有任何访问等级，且这笔销售也不会出现在你的报告中。

原因通常是以下几种之一：

- **凭据失效**：Adapty 无法通过 Apple 验证交易。请参考[更新停止推送时的处理方法](store-sync#if-updates-stop-arriving)逐一排查凭据，并替换负责购买验证的凭据。
- **产品未在 Adapty 中关联**：客户购买的 Store 产品 ID 未关联到任何 Adapty 产品，因此没有可授予的访问等级。请确认该产品以正确的 **App Store Product ID** 存在，并已分配[访问等级](access-level)。

Adapty SDK 会持续重试该交易，因此一旦 Adapty 能够记录，访问等级便会恢复。
