---
title: "修复 Kotlin Multiplatform SDK 中的 Code-1000 noProductIDsFound 错误"
...
1000 代码错误 `noProductIDsFound` 表示您在付费墙上请求的产品在 App Store 中均无法购买，尽管它们已在其中列出。此错误有时会附带 `InvalidProductIdentifiers` 警告。如果该警告在没有错误的情况下出现，可以安全忽略。
...
4. 将表格中的产品 ID 与 Adapty 看板中 [**Products**](https://app.adapty.io/products) 选项卡中的产品 ID 进行比较。如果 ID 不匹配，请从表格中复制产品 ID，并在 Adapty 看板中[创建产品](create-product)。
...
4. 滚动到 **Availability** 部分，检查所有必要的国家和地区是否已列出。
...
5. 确保所有必要的价格均已列出。
    ...
    ## 步骤 5. 检查应用付费状态、银行账户和税务表格是否有效 \{#step-5-check-app-paid-status-bank-account-and-tax-forms-are-active\}
    ...
    3. 向下滚动，检查您的 **Paid Apps Agreement**、**Bank Account** 和 **Tax forms** 是否均显示为 **Active**。
    ...
    按照以上步骤操作，您应该能够解决 `InvalidProductIdentifiers` 警告，并让您的产品在商店中正常上架。