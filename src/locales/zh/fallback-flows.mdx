...
1. 在 Flow & Paywall Builder 中创建一个 [流程](adapty-flow-builder)。
...
将文件添加到你的应用代码中，然后参照对应平台的配置指南。加载付费墙备用资源的 API 同样适用于流程备用资源，前提是你的应用已升级到支持流程功能的 SDK 版本：
...
- **每个版位只有一个实验变体。** 如果一个版位有多个流程（对应不同目标受众或 A/B 测试变体），备用文件将使用权重最高或受众范围最广的那个实验变体。
- **不支持 A/B 测试。** 线上流程的 A/B 测试由服务器解析；备用文件始终只提供一个选定的实验变体。
- **不支持远程更新。** 更新备用文件需要发布新版本应用。如需在运行时更新，请通过线上流程推送，而非使用远程配置。
