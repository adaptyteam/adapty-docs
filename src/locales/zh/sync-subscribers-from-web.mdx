---
title: "同步网页与移动端之间的购买记录"
description: "在网页和移动端同步订阅用户。"
...
如果您的用户可以在**网站**上购买产品，您可以将他们的访问等级自动与**移动应用**保持同步。

本指南将介绍如何使用 Adapty API 和 SDK 实现这一目标。

#### 示例使用场景 \{#sample-use-case\}
...
这种行为完全正常，不会影响数据分析的准确性。了解更多信息请点击[此处](ios-quickstart-identify)。
:::
...
## 步骤 4：在 Adapty 中更新订阅状态 \{#step-4-update-subscription-status-in-adapty\}

在网站上完成支付后，调用 Adapty API 根据用户购买的产品更新其访问等级。

请参阅[此处](api-adapty/operations/grantAccessLevel)的方法参考文档。
...
## 步骤 5：在应用中同步状态 \{#step-5-sync-status-in-the-app\}

当用户打开您的移动应用时，拉取更新后的用户画像并解锁付费功能。

您需要手动获取用户画像，或者让其自动同步，然后从中获取访问等级。

下面展示了如何获取用户画像并检查其状态。更多详情请访问[此处](ios-check-subscription-status)。
