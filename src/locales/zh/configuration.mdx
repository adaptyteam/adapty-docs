...
metadataTitle: "Adapty 配置设置 | Adapty 文档"
---

借助 Adapty 集成，您可以将订阅事件和购买数据无缝传输到您首选的平台或工作流程。无论您是在寻找用户行为洞察、客户互动策略，还是为营销团队提供更强大的产品分析，Adapty 都能轻松将应用内购买事件转发到您选择的集成平台。
...
<ZoomImage id="20bf659-CleanShot_2023-08-22_at_13.26.562x.webp" width="700px" alt="适用于通过此集成发送的所有事件的集成设置" />

集成提供以下配置选项，这些选项会影响通过此集成发送的所有事件：
...
<ZoomImage id="c79f5cd-screencapture-app-adapty-io-integrations-pushwoosh-2023-08-22-13_31_07.webp" width="700px" alt="每个事件包含一个开关和一个事件名称字段的事件分组" />

虽然我们建议使用 Adapty 的默认事件名称，但您也可以根据实际需求自由修改事件名称。

### 为合成事件设置计时器 \{#set-the-timer-for-synthetic-events\}

:::warning
请将 **Trial still active** 的延迟时间设置为短于试用期本身。例如，7 天试用期配合 7 天延迟，当 Adapty 检查时，试用期已经转换或过期，该事件将永远不会触发。
:::

[合成事件](events#synthetic-events)按计时器触发，而非响应状态变化。在事件开关旁边的字段中输入天数，Adapty 会等待相应天数后，若试用或订阅仍在进行中，则发送该事件。

该字段接受 1 到 90 天的范围。**Trial still active** 从第 1 天起生效，**Subscription still active** 从第 7 天起生效。每个集成单独维护自己的延迟设置，因此同一事件可能会在不同时间分别发送到两个目标系统。
