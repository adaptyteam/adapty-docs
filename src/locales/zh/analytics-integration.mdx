---
title: "分析集成"
description: "将分析工具与 Adapty 集成，以跟踪和优化用户订阅。"
metadataTitle: "分析集成指南 | Adapty 文档"
---
...
Adapty 将所有[订阅事件](events)发送到分析服务，例如 [Amplitude](amplitude)、[Mixpanel](mixpanel) 和 [AppMetrica](appmetrica)。我们也可以通过 [webhook](webhook) 集成将事件发送到您的服务器。最棒的是，您无需手动发送任何事件，我们会为您完成。只需在 Adapty 看板中配置集成即可。
...
请告知我们！[提交功能请求](https://adapty.featurebase.app/en?b=6979f233ebd3cffd4f425ba0)，我们将考虑添加。
:::
...
| **environment**               | str           | 可以是 _Sandbox_ 或 _Production_。                          |
...
| **store**                     | str           | 可以是 _app_store_ 或 _play_store_。                        |
| **trial_duration**            | str           | 试用期时长（天）。以"{} days"格式发送，例如"7 days"。       |
| **cancellation_reason**       | str           | <p>用户取消订阅的原因。</p><p></p><p>可以是</p><p>iOS 和 Android</p><p>_voluntarily_cancelled_、_billing_error_、_refund_</p><p>iOS</p><p>_price_increase_、_product_was_not_available_、_unknown_</p><p>Android</p><p>_new_subscription_replace_、_cancelled_by_developer_</p> |
| **subscription_expires_at**   | ISO 8601 date | 订阅到期日期。通常为将来的日期。                             |
| **consecutive_payments**      | int           | 用户连续订阅的周期数，不含中断。包含当前周期。               |
| **rate_after_first_year**     | bool          | 布尔值，表示该订阅在持续续订一年后是否符合降低佣金率（通常为 15%）的条件。佣金率因计划资格和国家/地区而异。详见[商店佣金和税费](controls-filters-grouping-compare-proceeds#store-commission-and-taxes)。 |
| **promotional_offer_id**      | str           | 在 Adapty 看板产品部分中指定的促销活动 ID。                  |
| **store_offer_category**      | str           | 可以是 _introductory_ 或 _promotional_。                    |
| **store_offer_discount_type** | str           | 可以是 _free_trial_、_pay_as_you_go_ 或 _pay_up_front_。    |
...
### 禁用广告标识符的收集 \{#disable-collection-of-advertising-identifiers\}

<Tabs groupId="current-os" queryString>
<TabItem value="swift" label="iOS" default>
您可以使用 `idfaCollectionDisabled` 属性禁用 IDFA 收集。请确保在调用 `.activate()` 方法之前进行设置。
...
您可以在激活 Adapty SDK 时使用 `withAdIdCollectionDisabled` 属性禁用 AAID/GAID 收集：
...
<TabItem value="flutter" label="Flutter" default>
您可以使用 `withAppleIdfaCollectionDisabled` 属性禁用 IDFA 收集，使用 `withGoogleAdvertisingIdCollectionDisabled` 属性禁用 Google/Android 广告 ID 收集。在激活 Adapty SDK 时将其设置为 `true`：
...
<TabItem value="unity" label="Unity" default>
您可以在激活 Adapty SDK 时使用 `SetIDFACollectionDisabled` 属性禁用 IDFA 收集。目前暂不支持禁用 AAID/GAID 收集。
...
您也可以在激活 Adapty SDK 时使用 `idfaCollectionDisabled` 属性禁用 IDFA 收集，或使用 `adIdCollectionDisabled` 属性禁用 AAID/GAID 收集。
