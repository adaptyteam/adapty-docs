...
metadataTitle: "从 Superwall 迁移到 Adapty | Adapty 文档"
keywords: ['迁移', 'superwall']
---

从 Superwall 迁移到 Adapty 通常只需约两小时。你只需替换 SDK、将应用商店的服务端通知指向 Adapty，然后发布新版本即可。付费订阅用户无需任何操作——Adapty 会在首次启动时自动从 App Store 和 Google Play 的收据中恢复他们的访问权限。

:::info
你的订阅用户将自动完成迁移
所有曾经激活过订阅的用户，只要打开集成了 Adapty SDK 的新版本应用，就会自动同步到 Adapty。订阅状态验证和高级访问权限会自动恢复。
:::
...
大多数 Superwall 概念在 Adapty 中都有直接对应的概念：
| Superwall            | Adapty                                            | 变更说明                                                                 |
| :------------------- | :------------------------------------------------ | :--------------------------------------------------------------------------- |
| Campaign             | [版位](placements) + [目标受众](audience)    | Campaign 逻辑拆分为版位（触发位置）和目标受众（匹配规则）。 |
| Placement            | [版位](placements)                           | 概念相同，名称相同。                                                     |
| Audience filter      | [目标受众](audience)                              | 规则集存在于版位内部。                                           |
| Entitlement          | [访问等级](access-level)                      | 具名标识符（例如 `premium`）。                                   |
| WebView paywall      | [付费墙编辑工具付费墙](adapty-paywall-builder) | 由 Adapty SDK 原生渲染，而非通过 `WKWebView`。                         |
| `PurchaseController` | 内置                                          | 无需实现任何协议——Adapty 自动处理购买流程。                         |
| Feature gating       | [访问等级](access-level) 检查                | 检查 `profile.accessLevels["premium"]?.isActive`。                           |
在接触代码之前，有两个思维转变值得关注：
- **获取与展示是两个独立步骤**：Superwall 的 `register` 方法在一次调用中完成付费墙的获取、活动评估和 UI 展示。Adapty 将这些步骤拆分开来——你需要先获取付费墙，拿到其配置，再进行展示。虽然多写了几行代码，但这样可以预加载配置、显示自定义加载状态，或根据自己的逻辑取消展示。
- **订阅状态基于访问等级**：Superwall 提供单一的 `subscriptionStatus` 发布属性。Adapty 返回一个 [`AdaptyProfile`](https://swift.adapty.io/documentation/adapty/adaptyprofile)，其中包含命名的访问等级，因此同一用户可以同时拥有 `sports` 和 `science` 两个访问等级。如需同步读取，建议从 `AdaptyDelegate` 缓存用户画像，而不是每次视图加载时都调用 `getProfile()`。
...
### 识别与登出用户 \{#identify-and-log-out-users\}

将 `Superwall.shared.identify` 替换为 `Adapty.identify`，将 `Superwall.shared.reset` 替换为 `Adapty.logout`。两个 SDK 都会在首次启动时自动生成匿名用户画像，因此这些调用仅在用户登录或登出时才需要执行。识别用户后请重新获取付费墙——新用户可能会匹配到不同的目标受众。
...
将 `Superwall.shared.register` 替换为两步流程：先用 `Adapty.getPaywall` 获取付费墙，再用 `AdaptyUI.getPaywallConfiguration` 加载其视图配置，然后将其展示出来。

有两点区别需要说明：
- **功能门控替代了 `feature:` 闭包**：付费墙关闭后，检查返回的用户画像（或通过 `Adapty.getProfile` 获取）上的活跃访问等级，并以此进行分支处理。
- **付费墙由 SDK 渲染**：Superwall 在 `WKWebView` 中渲染付费墙，而 Adapty 通过付费墙编辑工具原生渲染付费墙——字体、产品信息和按钮均由 SDK 绘制。
请参阅适合您平台的付费墙快速入门指南 — [iOS](ios-quickstart-paywalls)、[Android](android-quickstart-paywalls)、[React Native](react-native-quickstart-paywalls)、[Flutter](flutter-quickstart-paywalls)、[Kotlin Multiplatform](kmp-quickstart-paywalls)、[Unity](unity-quickstart-paywalls) 或 [Capacitor](capacitor-quickstart-paywalls)。
### 检查订阅状态 \{#check-subscription-status\}

将 `Superwall.shared.subscriptionStatus` 替换为对用户画像中指定访问等级的检查：`profile.accessLevels["premium"]?.isActive`。通过 `AdaptyDelegate.didLoadLatestProfile(_:)` 监听变化，而非使用 `@Published` 属性模式，并在本地缓存用户画像以便同步读取。
请参阅您所使用平台的订阅状态指南 — [iOS](ios-check-subscription-status)、[Android](android-check-subscription-status)、[React Native](react-native-check-subscription-status)、[Flutter](flutter-check-subscription-status)、[Kotlin Multiplatform](kmp-check-subscription-status)、[Unity](unity-check-subscription-status) 或 [Capacitor](capacitor-check-subscription-status)。
### 处理购买与恢复 \{#handle-purchases-and-restores\}

使用付费墙编辑工具时，两款 SDK 都会在付费墙界面内自动处理购买流程——**可以跳过此步骤**。
...
将应用商店的服务器通知指向 Adapty。Adapty 在没有这些通知的情况下也能运行，但数据分析、第三方集成以及 A/B 测试数据图表都依赖于它们：

- **App Store**：参照[启用 App Store 服务器通知](enable-app-store-server-notifications)。
- **Google Play**：参照[启用实时开发者通知](enable-real-time-developer-notifications-rtdn)。
如果你想在推出过程中同时运行 Superwall 和 Adapty，可以使用[原始事件转发](enable-app-store-server-notifications#raw-events-forwarding)——Adapty 会将商店事件代理回 Superwall，同时你可以验证新的集成是否正常工作。
