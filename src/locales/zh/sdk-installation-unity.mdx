...
Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.0（beta）——新增了 [Flow Builder](adapty-flow-builder) 支持——要求整个应用的最低部署目标为 **iOS 15.0+**：Unity Editor 中的构建验证器会在部署目标低于此版本时阻止 iOS 构建。

:::info
Adapty 支持 Google Play Billing Library 最高至 8.x 版本。默认情况下，Adapty 使用 Google Play Billing Library v7.0.0。如需使用更新版本，请在 Android 构建中[覆盖 Billing 依赖项](https://developer.android.com/google/play/billing/integrate#dependency)。

支持某个 Billing Library 版本并不意味着 Adapty 支持该版本中 Google 引入的所有功能。在使用新的 Google Play 计费功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
...
1. 安装 [External Dependency Manager (EDM) 插件](https://github.com/googlesamples/unity-jar-resolver#getting-started)。Adapty SDK 使用它来处理 iOS 依赖项和 Android gradle 依赖项。
...
SDK 4.0 — 新增了 [Flow Builder](adapty-flow-builder) 支持 — 目前处于预发布阶段。如需通过 Unity Package Manager 安装，请在 Git URL 末尾添加测试版标签：

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

如果通过 Unity 软件包安装，请从 [4.0.0-beta.1 release](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.0.0-beta.1) 下载 `adapty-unity-plugin-4.0.0-beta.1.unitypackage`。

两项构建配置变更随 v4 一同推出——原生 iOS Adapty SDK 现在以远程 Swift Package 形式声明，不再通过 CocoaPods 安装：

- 将 External Dependency Manager 更新至 **1.2.188 或更高版本** —— 旧版本不支持 Swift Package Manager 依赖项。这是 SDK 4.0 声明的对等依赖版本，如果你的项目使用了更旧的版本，Unity 会发出警告。
- 上述 CocoaPods 步骤（`iOS Resolver -> Install Cocoapods`，打开 `Unity-iPhone.xcworkspace`）仅适用于 SDK 3.x。在 SDK 4.0 中，EDM 会自动将 Swift Package 添加到生成的 Xcode 项目中。
- 将 iOS 部署目标设置为 **15.0 或更高版本**。Unity Editor 中的构建验证器会在不满足此要求时阻止 iOS 构建。

请参阅[迁移指南](migration-to-unity-sdk-v4)，了解 SDK 4.0 的完整变更列表。
...
:::note
在 SDK 4.0 中，监听器接口遵循 C# 的 I 前缀约定：请实现 `IAdaptyEventListener` 而非 `AdaptyEventListener`。方法保持不变。详见[迁移指南](migration-to-unity-sdk-v4)。
:::

:::important
在调用任何其他 Adapty SDK 方法之前，请先等待 `Activate` 的完成回调。完整调用顺序详见 [Unity SDK 中的调用顺序](unity-sdk-call-order)。
:::
