...
Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.1（新增 [Flow Builder](adapty-flow-builder) 支持）要求整个应用使用 **iOS 15.0+**：如果部署目标低于该版本，Unity Editor 中的构建校验器将阻止 iOS 构建。

:::info
Adapty Unity SDK 4.0 及更高版本适配 Google Play Billing Library v8。
...
SDK 4.1（新增 [Flow Builder](adapty-flow-builder) 支持）是 4.x 系列的第一个稳定版本。如需通过 Unity Package Manager 安装，请在 Git URL 末尾附加版本标签：

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

如需通过 Unity 包安装，请从 [4.1.0 release](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0) 下载 `adapty-unity-plugin-4.1.0.unitypackage`。
