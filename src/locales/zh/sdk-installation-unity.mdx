...
Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.1——新增了对[流程](adapty-flow-builder)的支持——要求整个应用的部署目标为 **iOS 15.0+**：Unity Editor 中的构建验证器会在部署目标低于该版本时阻止 iOS 构建。使用 SDK 4.x 为 iOS 构建还需要 **Xcode 26** 或更高版本——其添加为 Swift 包的原生 iOS SDK 使用 Swift tools 6.2 构建。

:::info
Adapty Unity SDK 4.0 及更高版本支持 Google Play Billing Library v8。

与 Billing Library 某版本兼容，并不代表 Adapty 支持该版本中 Google 引入的所有功能。在使用新的 Google Play 计费功能之前，请参阅 [Play Store 中的产品](android-products)。
:::
