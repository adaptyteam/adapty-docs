...
Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.1（新增对[流程](adapty-flow-builder)的支持）要求整个应用的最低部署目标为 **iOS 15.0+**：若部署目标低于此版本，Unity Editor 中的构建验证器将阻止 iOS 构建。使用 SDK 4.x 构建 iOS 应用还需要 **Xcode 26** 或更高版本——其作为 Swift Package 引入的原生 iOS SDK 使用 Swift tools 6.2 构建。
...
|---|---|
| 3.17.0 及更高版本 | v8 |
| 3.15.0–3.15.2 | 默认为 v7，若其他依赖项将其升级则使用 v8 |

:::note
兼容某个 Billing Library 版本并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 计费功能之前，请参阅 [Play Store 中的产品](android-products)。
:::
