...
Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.1（新增对[流程](adapty-flow-builder)的支持）要求整个应用的部署目标为 **iOS 15.0+**：Unity Editor 中内置的构建校验器会在部署目标低于该版本时阻止 iOS 构建。使用 SDK 4.x 为 iOS 构建时，还需要 **Xcode 26** 或更高版本——其添加的原生 iOS SDK Swift Package 是使用 Swift tools 6.2 构建的。

:::info
从 3.17.0 版本起，Adapty Unity SDK 默认使用 Billing Library v8。3.15.0 至 3.15.2 版本使用 v7，同时也兼容 Android 构建中其他依赖将 Billing Library 升级至 v8 的情况。

兼容某个 Billing Library 版本并不代表 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 计费功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
