...
:::tip
想看看 Adapty SDK 如何在移动应用中实际集成？查看我们的[示例应用](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Assets)，其中展示了完整的配置过程，包括展示付费墙、发起购买及其他基本功能。
:::

## 系统要求 \{#requirements\}

Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+。Adapty SDK 4.1（新增对[流程](adapty-flow-builder)的支持）要求整个应用的最低部署目标为 **iOS 15.0+**：Unity 编辑器内置的构建验证器会在部署目标低于该版本时阻止 iOS 构建。使用 SDK 4.x 为 iOS 构建还需要 **Xcode 26** 或更高版本——它以 Swift Package 方式引入的原生 iOS SDK 是用 Swift tools 6.2 构建的。

:::info
Adapty SDK 已内置 Google Play Billing Library，无需在应用中单独添加。

从 3.17.0 版本起，Adapty Unity SDK 默认使用 Billing Library v8。3.15.0 至 3.15.2 版本使用 v7，同时也兼容 Android 构建中其他依赖项将 Billing Library 升级至 v8 的情况。

与某版本 Billing Library 兼容，并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 计费功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
...
SDK 4.1 — 新增了对[流程](adapty-flow-builder)的支持 — 是 4.x 系列的首个稳定版本。要通过 Unity Package Manager 安装，请在 Git URL 后附加版本标签：

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

如果通过 Unity 包安装，请从 [4.1.0 release](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0) 下载 `adapty-unity-plugin-4.1.0.unitypackage`。
...
有关 4.x 版本的完整变更列表，请参阅[迁移指南](migration-to-unity-sdk-v4)。
...
:::note
在 SDK 4.1 中，监听器接口遵循 C# 的 I 前缀命名规范——请实现 `IAdaptyEventListener` 而非 `AdaptyEventListener`——该接口还多了一个方法 `OnReceivePromotedPurchase`。详情请参阅[迁移指南](migration-to-unity-sdk-v4)。
:::

:::important
在调用任何其他 Adapty SDK 方法之前，请等待 `Activate` 的完成回调。完整调用顺序请参阅 [Unity SDK 中的调用顺序](unity-sdk-call-order)。
:::
...
- 如果您使用[流程与付费墙编辑工具](adapty-flow-builder)，请先[激活 AdaptyUI 模块](#activate-adaptyui-module-of-adapty-sdk)，然后按照[快速入门指南](unity-quickstart-paywalls)操作。
- 如果您自行构建付费墙 UI，请参阅[自定义付费墙快速入门指南](unity-quickstart-manual)。

## 激活 Adapty SDK 的 AdaptyUI 模块 \{#activate-adaptyui-module-of-adapty-sdk\}

如果你计划使用 [Flow & 付费墙编辑工具](adapty-flow-builder) 并已安装 AdaptyUI 模块，则需要激活 AdaptyUI。你可以在配置时进行激活：
...
如果您使用 [Adapty 归因](user-acquisition)，请在激活 SDK 时将 `SetAdaptyAttributionEnabled` 设置为 `true`。默认值为 `false`：不设置此参数时，SDK 不会注册安装或向您的应用传递安装详情。在低于 4.1 的 SDK 版本中，Adapty 归因会自动启用。
