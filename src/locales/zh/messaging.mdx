...
在移动市场竞争日益激烈的背景下，获客既不容易也代价不菲。因此，合理运营已吸引的用户能够有效改善单位经济效益，在竞争激烈的细分领域尤为如此。
...
Webhook 事件以 JSON 格式发送。所有事件遵循相同的结构，但其字段会根据事件类型、应用商店以及您的具体配置而有所不同。
...
| **net_revenue_usd**           | float         | 扣除 Apple/Google 佣金及税费后的净收入（USD）。可能为空。 |
| **price_local**               | float         | 扣除 Apple/Google 佣金前的本地货币产品价格，即收入。 |
| **proceeds_local**            | float         | 扣除 Apple/Google 佣金后的本地货币产品价格，即净收入。 |
...
| **trial_duration**            | str           | 试用期天数，格式为 "{} days"，例如 "7 days"。 |
...
| **rate_after_first_year**     | bool          | 布尔值，表示该订阅是否在连续续订满一年后符合降低佣金率（通常为 15%）的条件。佣金率因计划资格和国家/地区而异。详情请参阅[商店佣金与税费](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue)。 |
...
| **paywall_revision**          | int           | 交易来源付费墙的版本号，固定为 1。 |
| **developer_id**              | str           | 交易来源版位的开发者（SDK）ID。 |
| **ab_test_name**              | str           | 交易来源 A/B 测试的名称。       |
| **ab_test_revision**          | int           | 交易来源 A/B 测试的版本号，固定为 1。 |
...
| **profile_country**           | str           | 由 Adapty 根据用户画像 IP 判断。                   |
...
| **renewed_at**                | ISO 8601 date | 当前付费访问周期最近一次开始的日期和时间。在首次续订前，与 `activated_at` 相同。 |
