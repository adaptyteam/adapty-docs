...
Adapty Attribution 与 Google Cloud Storage 的集成允许您将用户获取活动数据安全地存储在一个中心位置。您可以将活动效果数据、归因数据和用户获取事件以 .csv 文件格式保存到 Google Cloud Storage 存储桶中。  
要设置此集成，您需要在 Google Cloud Console 和 Adapty Attribution 看板中完成几个简单步骤。

:::note
计划

Adapty Attribution 每个日历日向 Google Cloud Storage 导出一组文件。导出调度器每天运行三次——分别在 UTC **02:00**、**04:00** 和 **06:00**。每次运行要么创建前一天的导出任务（如果尚不存在），要么在之前的运行失败时重试。因此，您的数据通常在 02:00 运行后即可获取，后续运行的目的是从失败中恢复，而非新增数据。

每个文件将包含前一个完整日历日（UTC）内产生的事件数据。例如，3 月 8 日导出的数据将包含 3 月 7 日 00:00:00 至 23:59:59（UTC）之间的所有事件。
:::

:::warning
定时导出仅适用于付费套餐或试用期内的公司。若您的公司不满足上述条件，调度器将跳过导出，不会写入任何文件，即使集成在看板中仍处于启用状态。如果您的存储桶为空且配置看起来正确，请先检查您的套餐状态。
:::
...
| `bundle_id`              | 应用包标识符（例如 `com.example.app`）    |
| `device_brand`           | 设备品牌（例如 `Apple`、`Samsung`）      |
| `device_model`           | 设备型号（例如 `iPhone15,2`）            |
| `os_version`             | 主要 OS 版本                             |
...
| `asa_creative_set_name`  | ASA 广告创意组名称                        |
...
| `click_time`             | 归因点击的时间戳，以 Unix 秒为单位。若该安装没有关联的点击，则为空。 |