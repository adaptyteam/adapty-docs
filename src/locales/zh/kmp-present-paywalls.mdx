...
如果你已经创建了流程，或者在旧版付费墙编辑工具中创建了付费墙，则无需在移动应用代码中手动编写渲染逻辑来向用户展示它。此类流程或付费墙已包含展示内容及展示方式的完整定义。

:::warning

本指南适用于由 Adapty 渲染的流程和付费墙。远程配置付费墙与 [Observer 模式](observer-vs-full-mode) 的处理方式有所不同。
...
如果您已使用付费墙编辑工具自定义了付费墙，则无需在移动应用代码中手动渲染即可向用户展示。此类付费墙同时包含展示内容与展示方式。

:::warning

本指南仅适用于**在旧版编辑工具中构建的付费墙**。远程配置付费墙和 [Observer 模式](observer-vs-full-mode) 的付费墙展示流程有所不同。
