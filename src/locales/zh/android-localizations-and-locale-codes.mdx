...
匹配是精确的。SDK 会逐字符比较你传入的代码与流程本地化代码：不会更改大小写，不会将下划线（`_`）替换为连字符（`-`），也不会回退到语言子标签。对于包含 `pt-br` 本地化的流程，只有 `pt-br` 能匹配：`pt-BR`、`pt_BR` 和 `pt-PT` 均无法匹配。

当代码未匹配到任何本地化时，流程会静默地以其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——SDK 不会返回错误，也不会记录任何警告。

当代码匹配成功时，Adapty 会将匹配到的本地化与默认本地化合并：在匹配的本地化中未定义的字符串和资源将从默认本地化中获取。

省略语言代码并不等同于请求流程的默认本地化：SDK 会替换为固定的 `en`。如果一个流程的默认语言是 `de`，但该流程存在 `en` 本地化，则仍会以 `en` 渲染；只有当该流程没有 `en` 本地化时，才会回退到 `de`。此行为适用于 Android SDK 4.0.1 及更高版本——在 4.0.0 中，省略语言代码会渲染流程的默认本地化。

:::warning
请严格按照看板中配置的格式传入语言区域代码——子标签全部小写，以连字符分隔。不要直接传入系统的语言区域标识符：`Locale.getDefault().toLanguageTag()` 返回 `pt-BR`，`Locale.getDefault().toString()` 返回 `pt_BR`，两者都会回退到默认本地化。请在应用中转换该值后再传入。
:::
...
请注意，我们不建议使用这种方式，因为很难预测 Adapty 服务器最终会收到什么数据。

如果你仍决定采用此方式，请确保已覆盖所有相关的使用场景。
