...
- **在编辑工具中构建的 Flow**：SDK 不会自动读取设备语言，请在应用中自行解析后，通过 `AdaptyUI.getFlowConfiguration` 的 `locale` 参数传入。该参数为可选参数——如果省略，Flow 将以其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染。
- **自定义（远程配置）付费墙**：`getFlow` 会在 `flow.remoteConfigs` 中返回所有已配置的本地化内容。每条记录包含一个 `locale` 代码和配置内容（`jsonString` 或已解析的 `dataMap`）。请自行实现回退逻辑，选取匹配当前用户的记录：
