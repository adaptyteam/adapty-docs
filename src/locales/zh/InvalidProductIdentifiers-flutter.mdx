---
title: "修复 Flutter SDK 中的 Code-1000 noProductIDsFound 错误"
description: "解决在 Adapty 中管理订阅时出现的无效产品标识符错误。"
metadataTitle: "修复 Flutter 中的 Code-1000 noProductIDsFound | Adapty 文档"
...
1000 代码错误 `noProductIDsFound` 表示您在付费墙上请求的产品均无法在 App Store 中购买，即使这些产品已在 App Store 中列出。此错误有时可能伴随 `InvalidProductIdentifiers` 警告出现。如果该警告单独出现而不伴随错误，可安全忽略。

如果您遇到 `noProductIDsFound` 错误，请按照以下步骤进行解决：
...
4. 将表中的产品 ID 与 Adapty 看板中 [**Products**](https://app.adapty.io/products) 标签页中的产品 ID 进行比对。如果 ID 不匹配，请从表中复制产品 ID，并在 Adapty 看板中[创建产品](create-product)。
...
5. 确保所有所需价格均已列出。
    ...
    按照以上步骤操作后，您应该能够解决 `InvalidProductIdentifiers` 警告，并使您的产品在应用商店中正常上架。