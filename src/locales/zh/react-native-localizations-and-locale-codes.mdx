...
- **在编辑工具中构建的流程**：`createFlowView` 不接受语言区域参数，SDK 也不读取设备语言区域。流程将以其[默认语言区域](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——在 iOS 上，若流程包含英语本地化，则以 `en` 显示。
- **自定义（远程配置）付费墙**：`getFlow` 会在 `flow.remoteConfigs` 中返回所有已配置的本地化内容。每个条目包含 `lang` 语言代码和 `data` 对象。请根据用户情况选择对应条目，并自行实现回退逻辑：
