...
2. Adapty 查找完全匹配该语言代码的本地化内容
3. 如果未找到匹配项，Adapty 会截取第一个连字符之前的子字符串（例如 `pt-br` 取 `pt`），再次查找匹配的本地化内容
4. 如果仍未找到匹配项，Adapty 将返回该流程默认语言区域的内容

这样一来，`'pt_BR'`、`pt-BR` 和 `pt-br` 都会解析到同一个本地化。

## 实现本地化 \{#implementing-localizations\}

在 SDK v4 中，获取流程时无需传入语言区域代码——`getFlow` 会返回包含所有本地化内容的流程，Adapty 在构建流程视图时会自动应用其中一个。

- **在编辑工具中构建的流程**：SDK 不会读取设备语言，因此需要在应用中自行解析并通过 `createFlowView` 的 `locale` 参数传入，或通过嵌入式 `AdaptyFlowView` 组件的 `params` 属性传入。该参数为可选项——若省略，流程将以 `en` 渲染；若流程没有 `en` 本地化版本，则使用其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)。当请求的本地化版本在流程中不存在时，视图会静默回退到流程默认语言，所选本地化版本中缺失的字符串也会从默认语言中获取。
