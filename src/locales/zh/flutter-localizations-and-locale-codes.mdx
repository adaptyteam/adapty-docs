...
3. 如果未找到匹配项，Adapty 截取第一个连字符之前的子字符串（例如 `pt-br` 取 `pt`），并查找匹配的本地化内容
...
- **在编辑工具中构建的流程**：SDK 不会读取设备语言，请在您的应用中自行获取并通过 `createFlowView` 或 `AdaptyUIFlowPlatformView` 的 `locale` 参数传入。该参数为可选项——省略时流程将以 `en` 渲染，如果流程没有 `en` 本地化，则使用其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)。当您请求的语言在流程中不存在时，视图会静默回退到流程默认语言，所选语言缺少的字符串也会从默认语言中获取。
