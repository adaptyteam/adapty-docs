...
description: "使用 Adapty 在 React Native 中处理流程和付费墙的按钮操作，实现更好的应用变现。"
...
1. 在[编辑工具中添加按钮](paywall-buttons)，并为其分配已有动作或创建自定义动作 ID。
2. 在应用代码中编写处理逻辑，响应每个已分配的动作。

本指南介绍如何在代码中处理自定义动作和已有动作。

:::warning
**购买、恢复、关闭流程/付费墙以及打开 URL 均会自动处理。** 你可以配置这些默认行为，也可以为自定义动作实现响应逻辑。
:::
...
如果你使用 Adapty 付费墙编辑工具构建付费墙，必须正确设置按钮：
...
:::warning
**只有购买、恢复购买、关闭付费墙和打开 URL 这几类操作会自动处理。** 其他所有按钮操作都需要在应用代码中实现相应的响应逻辑。
:::
