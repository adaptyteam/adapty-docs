...
5. 无论是 iOS 还是 Android，请打开 [AppsFlyer 网站](https://www.appsflyer.com/home) 并登录。
...
9. 在 **New token** 窗口中，输入令牌名称。该名称仅供您自己参考。
...
### 将应用连接至 AppsFlyer \{#connect-your-app-to-appsflyer\}

完成上述步骤后，调用 `updateExternalAttribution` 方法（在 SDK 4.1 版本之前名为 `updateAttribution`）以保存归因数据，并使用 `Adapty.setIntegrationIdentifier()` 设置集成标识符。

在 Adapty 中调用用户识别之前，请先初始化 AppsFlyer SDK 并等待其 UID 回调。否则，`appsflyer_id` 会落到 Adapty 在激活时创建的一个短暂匿名用户画像上，且不一定能转移到已识别的用户画像。一旦出现这种情况，AppsFlyer 收入转发将静默失败。
