...
description: "构建一个 FlutterFlow 动作流程，用于获取 Adapty 付费墙和产品数据，供您自定义付费墙页面使用。"
metadataTitle: "在 FlutterFlow 中创建展示付费墙数据的流程 | Adapty 文档"
---
...
如前所述，要在您的 FlutterFlow 付费墙中展示数据，我们首先需要从 Adapty 获取相关内容。第一步是获取 Adapty 付费墙本身。具体操作如下：

1. 打开您的付费墙页面，切换到右侧面板中的 **Actions** 部分，然后打开 **Action Flow Editor**。
...
3. 点击 **Add Action**，然后搜索 `getPaywall` 自定义动作并选择它。
...
4. 在 **Set Actions Arguments** 部分，输入您在 Adapty 看板中[已创建的版位](create-placement)的真实 ID，该版位包含付费墙。在此示例中为 `monthly`。请务必使用您的真实版位 ID！
...
6. 在 **Action Output Variable Name** 中，新建一个变量并命名为 `getPaywallResult`。我们将在下一步中使用该变量来引用 Adapty 付费墙并请求其产品。
