---
title: "Branch"
description: "将 Branch 与 Adapty 集成，以跟踪深度链接和应用转化。"
metadataTitle: "Branch 订阅跟踪集成 | Adapty 文档"
---
...
2. 使用 `.updateExternalAttribution()` 方法（在 SDK 4.1 版本之前名为 `.updateAttribution()`）来保存归因数据。如果您在上一步中未指定 Branch 用户 ID，请在此处将其传入 `networkUserId` 参数。
...
| `custom_data`                  | Object | 详细的事件属性（包含所有可用的[事件字段](webhook-event-types-and-fields#for-most-event-types)）。                        |
