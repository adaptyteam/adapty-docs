---
title: "迁移 Adapty Kotlin Multiplatform SDK 至 v4.0"
description: "通过将付费墙 API 替换为 flow API，迁移至 Adapty Kotlin Multiplatform SDK v4.0（测试版），兼容 Flow Builder 和 Paywall Builder。"
...
`createFlowView` 在流程未配置视图时会返回 `AdaptyResult.Error`——这取代了 v3 中的 `hasViewConfiguration` 检查：
...
- `AdaptyUI.setObserverModeResolver(...)` 与 `AdaptyUIObserverModeResolver` — 在 SDK 以[观察者模式](implement-observer-mode-kmp)运行时，驱动从流程中发起的购买和恢复操作。此前该功能仅在原生 iOS 和 Android SDK 中提供。请参阅[在观察者模式下展示流程](kmp-present-flows-in-observer-mode)。
- `AdaptyUI.setSystemRequestsHandler(...)` 与 `AdaptyUISystemRequestsHandler` — 用于处理流程中的系统请求（操作系统权限提示和应用评价请求）。目前流程尚不触发此类请求，因此无需注册处理程序。
- 新增可选回调 `flowViewDidReceiveAnalyticEvent`，用于上报来自流程的分析事件，从用户打开每个页面的页面浏览事件开始。请参阅[追踪流程页面浏览](kmp-flow-screen-views)。
- `AdaptyUI.openWebUrl(url, openIn)` 和 `AdaptyUI.requestAppReview()` — 这两者支持默认的 `OpenUrlAction` 处理和默认的 `handleAppReviewRequest`，因此 URL 跳转和应用评价提示均可开箱即用地以原生方式处理。仅当您覆盖了这些默认行为时，才需要直接调用它们。
- `AdaptyUIFlowView.locale` — 上报构建视图时所使用的本地化语言，方便您了解用户实际看到的是哪种语言版本。需要 SDK 4.0.1-beta.1 或更高版本。
