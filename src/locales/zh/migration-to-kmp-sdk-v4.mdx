...
`AdaptyPaywallProduct` 保持原名不变——产品仍属于流程，`getPaywallProducts` 也保持原名，现在接受 `AdaptyFlow` 作为参数。`getFlow` 和 `getFlowForDefaultAudience` 方法不再接受 `locale` 参数。购买与用户画像相关的 API（`makePurchase`、`restorePurchases`、`getProfile`、`identify`、`updateProfile`）以及 `setFallback` 保持相同的签名，但备用付费墙文件本身必须重新下载——详见[备用文件](#fallback-files)。用户引导方法仍然可用，但已被标记为弃用——详见[用户引导 API 弃用说明](#onboarding-api-deprecation)。部分默认行为有所变更——详见[默认行为变更](#default-behavior-changes)。
