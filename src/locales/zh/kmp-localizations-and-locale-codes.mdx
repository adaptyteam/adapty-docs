...
当 Adapty 查找与用户语言区域匹配的本地化内容时，会依次执行以下步骤：
...
这样，`'pt_BR'`、`pt-BR` 和 `pt-br` 都会解析到同一个本地化版本。

## 实现本地化 \{#implementing-localizations\}

在 SDK v4 中，获取流程时无需传入语言区域代码——`getFlow` 会返回包含所有本地化内容的流程，Adapty 会在构建流程视图时自动应用对应的本地化。

- **在编辑工具中构建的流程**：SDK 不会读取设备语言环境，因此请在应用中自行解析并通过 `createFlowView` 的 `locale` 参数传入。该参数为可选项——省略时流程将以 `en` 渲染，若流程没有 `en` 本地化，则以其[默认语言环境](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染。若请求的本地化在流程中不存在，视图会静默回退至流程默认语言环境，而所选本地化中缺失的字符串也会从默认语言环境中补充。
