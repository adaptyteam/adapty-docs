...
- **在编辑工具中构建的流程**：`createFlowView` 不接受语言区域参数，SDK 也不会读取设备语言区域。流程将以其[默认语言区域](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——在 iOS 上，当流程包含英文本地化时，默认以 `en` 渲染。
- **自定义（远程配置）付费墙**：`getFlow` 在 `flow.remoteConfigs` 中返回所有已配置的本地化内容。每个条目都是一个 `AdaptyRemoteConfig`，包含 `locale` 代码和 `dataMap`。请按需选择与用户匹配的条目，并自行实现回退逻辑：
