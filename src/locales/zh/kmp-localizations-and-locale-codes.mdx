...
匹配规则为精确匹配。SDK 会将你传入的代码与流程本地化代码逐字符进行比较：不会更改大小写，不会将下划线（`_`）替换为连字符（`-`），也不会回退到语言子标签。对于包含 `pt-br` 本地化的流程，只有 `pt-br` 能匹配：`pt-BR`、`pt_BR` 和 `pt-PT` 均无法匹配。

当代码未匹配任何本地化时，流程会静默地以其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——SDK 不会返回错误，也不会记录警告。

当代码匹配成功时，Adapty 会将匹配的本地化与默认本地化合并：匹配本地化中未定义的字符串和资源将从默认本地化中获取。

省略语言代码并不等同于请求流程的默认本地化：SDK 会固定替换为 `en`。如果一个流程的默认语言是 `de`，只要该流程包含 `en` 本地化，它仍会以 `en` 渲染；只有当流程不包含 `en` 本地化时，才会回退到 `de`。

:::warning
请严格按照看板中配置的格式传入语言区域代码——子标签全部小写，并以连字符分隔。不要直接传入平台的语言区域标识符：在 Android 上，`Locale.getDefault().toLanguageTag()` 返回 `pt-BR`；在 iOS 上，`NSLocale.currentLocale.localeIdentifier` 返回 `pt_BR`。两者都会回退到默认本地化。请在应用中完成转换后再传入该值。
:::
...
- **由 Adapty 渲染的流程和付费墙**：SDK 不会自动读取设备语言，因此请在应用中自行获取语言信息，并通过 `createFlowView` 的 `locale` 参数传入。该参数为可选项——若省略，流程将以 `en` 渲染；若流程未包含 `en` 本地化内容，则以其[默认语言](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染。
