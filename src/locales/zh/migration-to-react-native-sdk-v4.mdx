---
title: "将 Adapty React Native SDK 迁移至 v4.0"
description: "通过将付费墙 API 替换为流程 API，迁移至 Adapty React Native SDK v4.0，新 API 支持渲染流程以及旧版编辑工具的付费墙。"
metadataTitle: "迁移至 Adapty React Native SDK v4.0 | Adapty 文档"
---

Adapty React Native SDK 4.0 引入了流程功能，并对付费墙 API 进行了相应重命名。新 API 支持流程，同时仍兼容旧版编辑工具中的付费墙——Adapty 看板侧无需进行任何配置变更。

:::note
本指南介绍从 3.x 迁移到 4.0 的步骤。SDK 4.1 在此基础上又对部分 API 进行了调整——Adapty 归因变为按需启用、外部归因方法已重命名，备用文件格式也再次发生变化。完成本指南后，请参阅[迁移至 v4.1](migration-to-react-native-sdk-41)。
:::
...
`AdaptyPaywallProduct` 保持原名不变——产品仍属于某个流程，`getPaywallProducts` 现在接受 `AdaptyFlow` 参数。`getFlow` 和 `getFlowForDefaultAudience` 方法不再接受 `locale` 参数——请改为将其传入 `createFlowView`。视图方法 `present`、`dismiss`、`setEventHandlers`、`showDialog`，以及事件处理器 `onCloseButtonPress`、`onUrlPress`、`onCustomAction`、`onProductSelected`、`onPurchaseStarted`、`onPurchaseCompleted`、`onPurchaseFailed`、`onRestoreStarted`、`onRestoreCompleted`、`onRestoreFailed`、`onLoadingProductsFailed`、`onWebPaymentNavigationFinished` 和 `onAndroidSystemBack` 与 v3 中的名称保持一致。部分默认行为有所变更——详见[默认行为变更](#default-behavior-changes)。
...
### iOS：原生 SDK 现已通过 Swift Package Manager 分发 \{#ios-native-sdks-now-come-through-swift-package-manager\}

[CocoaPods 的 spec 仓库将于 2026 年 12 月变为只读](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)，因此从 v4 开始，原生的 `Adapty`、`AdaptyUI` 和 `AdaptyPlugin` SDK **不再作为 CocoaPods 子依赖项引入** —— podspec 改为通过 **Swift Package Manager**（使用 `spm_dependency` 辅助方法）拉取它们。这需要满足以下三个条件：

- **Xcode 26 或更高版本** — 原生 iOS SDK 的包清单声明了 Swift tools 6.2，旧版 Xcode 无法读取。
- **React Native 0.75 或更高版本** — 需要 `spm_dependency` podspec 辅助工具。在旧版本中，`pod install` 会报错失败；请先升级 React Native，或继续使用 `react-native-adapty` 3.x。
- **动态框架** — SPM 依赖项需要动态链接。启用方式因 Expo 和裸 React Native 项目而有所不同。

#### Expo

在 `app.json`（或 `app.config.js`）中添加 [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) 配置插件，并将 iOS 框架设置为动态加载：
...
如果 iOS 构建随后出现 `'React/RCTBridge.h' file not found` 错误，请将 `expo-modules-autolinking` 升级至 57.0.10 或更高版本（Expo SDK 57）或 56.0.22 或更高版本（Expo SDK 56）——详情及临时解决方案请参阅[故障排查说明](sdk-installation-react-native-expo#react-header-not-found)。

#### Bare React Native

为你的 iOS target 添加动态框架，然后重新安装 pods：
...
如果您之前通过 CocoaPods 将 `Adapty`、`AdaptyUI` 或 `AdaptyPlugin` 作为子依赖项引入，请先从 `Podfile` 中移除所有显式的 `pod 'Adapty'`、`pod 'AdaptyUI'` 或 `pod 'AdaptyPlugin'` 行。

:::warning
将默认的静态链接切换为动态框架，可能会与尚不支持模块化头文件的库产生冲突，且与 Flipper 不兼容。如果遇到构建问题，请参阅这篇[将 Swift Package Manager 与 React Native 库集成的说明](https://www.callstack.com/blog/integrating-swift-package-manager-with-react-native-libraries)。
:::

请参阅[安装 Adapty SDK](sdk-installation-reactnative)了解完整的安装设置。

## 获取流程 \{#fetching-flows\}

### getPaywall → getFlow

返回类型从 `AdaptyPaywall` 变更为 `AdaptyFlow`，`locale` 参数从获取调用移至 `createFlowView`；对于自定义付费墙，所有语言环境均通过 `flow.remoteConfigs` 返回：
...
`locale` 在 `createFlowView` 中保持可选：省略时，视图将以 `en` 渲染，或在流程没有 `en` 本地化时使用流程的默认本地化。此功能需要 SDK 4.0.2 或更高版本——请参阅[本地化与语言代码](react-native-localizations-and-locale-codes)。
...
备用文件格式[在 SDK v4 中发生了变更](fallback-flows)。请从 **[Placements](https://app.adapty.io/placements)** > **Fallbacks** 下载新文件并将其打包到应用中。
...
|---|---|---|
| `remoteConfig?`（单个） | `remoteConfigs?: AdaptyRemoteConfig[]`（数组） | 一个流程针对每种已配置的语言携带一份远程配置。读取与用户匹配的那份：`flow.remoteConfigs?.find((c) => c.lang === 'en')`。 |
...
| `hasViewConfiguration` | 在 4.0 中移除，4.1 中恢复 | 使用 4.0 时，请从代码中移除所有 `hasViewConfiguration` 检查。该标志将在 4.1 的 `AdaptyFlow` 中恢复——参见[迁移至 v4.1](migration-to-react-native-sdk-41#hasviewconfiguration-returns-to-adaptyflow)。 |
...
`logShowPaywall` 已重命名为 `logShowFlow`，现在接受一个 `AdaptyFlow` 参数。该事件仍然记录在相同的实验变体下，因此现有的漏斗和 A/B 测试数据图表无需在看板中做任何修改即可继续正常使用。
...
与 v3 相同，当展示由 Adapty 渲染的流程或付费墙时，无需手动调用此方法——Adapty 会自动追踪这些浏览事件。
...
所有其他事件处理程序保持原有名称不变。其中两个新增了第二个参数：`onPurchaseCompleted` 变为 `(purchaseResult, product)`，`onPurchaseFailed` 变为 `(error, product)`，其中 `product` 是参与操作的 `AdaptyPaywallProduct`。完整列表请参阅[处理 flow 与付费墙事件](react-native-handling-events-1)。
...
- `adapty.openWebUrl(url, openIn?)` 和 `adapty.requestAppReview()` 方法 —— 这两个方法支持默认的 `onUrlPress` 和 `onRequestAppReview` 处理器，因此 URL 跳转和应用评价弹窗均可开箱即用地原生处理。只有在你覆盖这些处理器时，才需要直接调用它们。
- 通过新的 `onObserverPurchaseInitiated` / `onObserverRestoreInitiated` 处理器，在流程中支持观察者模式下的购买处理。详见[在观察者模式下处理购买](react-native-handling-events-1#handle-purchases-in-observer-mode)。
...
旧版用户引导 API 在 v4.0 中已弃用，请改用 [Flow & 付费墙编辑工具](adapty-flow-builder)。该 API 目前仍可使用，但将在未来版本中移除，请尽快将您的用户引导迁移到 Flow & 付费墙编辑工具。
