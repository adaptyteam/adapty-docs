...
`buildReactNativeFromSource` 在 **Expo SDK 57 及更高版本**中是必需的。Expo SDK 57 附带了预构建的 React Native 框架，一旦框架变为动态模式，其他包就无法访问其头文件，导致 iOS 构建失败，报错信息类似 `expo-updates` 或 `@expo/ui` 中的 `'React/RCTBridge.h' file not found`。从源码构建 React Native 可以避免此冲突，代价是 iOS 构建时间更长。在 Expo SDK 56 及更早版本中，可以省略此选项。
