...
description: "关于在 Android 上安装 Adapty SDK 以实现订阅功能的分步指南。"
...
- **AdaptyUI**：此模块用于渲染流程，以及旧版编辑工具付费墙。AdaptyUI 会随核心模块自动激活。

:::tip
想看看 Adapty SDK 如何集成到真实移动应用中？查看我们的[示例应用](https://github.com/adaptyteam/AdaptySDK-Android/tree/master/app)，其中展示了完整的配置流程，包括展示付费墙、进行购买以及其他基本功能。
:::
...
Adapty SDK 支持以下 Google Play Billing Library 版本：

| Adapty SDK 版本 | Billing Library 版本 |
|---|---|
| 4.0.2 及更高版本 | v8 |
| 4.0.0–4.0.1 | 默认使用 v7，若其他依赖项将其升级则使用 v8 |
| 3.x 版本线中的 3.17.3 及更高版本 | v8 |
| 3.15.0–3.17.2 | 默认使用 v7，若其他依赖项将其升级则使用 v8 |
...
- 如果项目使用 `.gradle.kts` 文件，则在模块级 `build.gradle.kts` 中添加依赖
- 如果使用版本目录，则在 `libs.versions.toml` 文件中添加依赖，然后在 `build.gradle.kts` 中引用
...
:::important
在调用其他 Adapty SDK 方法之前，请等待 `Adapty.activate` 完成。完整调用顺序请参阅 [Android SDK 中的调用顺序](android-sdk-call-order)。
:::

接下来在你的应用中配置付费墙：

- 如果你使用 [Flow & 付费墙编辑工具](adapty-flow-builder)，请参阅[快速入门](android-quickstart-paywalls)。
- 如果你自行构建付费墙 UI，请参阅[自定义付费墙快速入门](android-quickstart-manual)。

## 激活 Adapty SDK 的 AdaptyUI 模块 \{#activate-adaptyui-module-of-adapty-sdk\}

如果你计划使用 [Flow & 付费墙编辑工具](adapty-flow-builder)，则需要 AdaptyUI 模块。激活核心模块时，该模块会自动激活，无需额外操作。
...
从 Adapty SDK 4.1 版本开始，[Adapty 归因](user-acquisition) 默认处于禁用状态。在激活 Adapty 模块时，请将 `adaptyAttributionEnabled` 设置为 `true`，以允许 SDK 记录安装事件并将安装详情传递给您的应用。
...
使用 `AdaptyUI.configureMediaCache` 覆盖默认缓存大小和有效期。此步骤为可选项——如果不调用该方法，将使用默认值（磁盘大小 100MB，有效期 7 天）。
