---
title: "安装并配置 Android SDK"
description: "在 Android 上安装 Adapty SDK 的分步指南，适用于订阅类应用。"
...
- **AdaptyUI**：如果您使用 [Adapty 付费墙编辑工具](adapty-paywall-builder)（一款无需编写代码即可轻松创建跨平台付费墙的可视化工具），则需要此模块。AdaptyUI 会随核心模块一并自动激活。
:::tip
想看看 Adapty SDK 集成到移动应用的真实案例？欢迎查看我们的[示例应用](https://github.com/adaptyteam/AdaptySDK-Android/tree/master/app)，其中涵盖了完整的集成配置，包括展示付费墙、发起购买以及其他基础功能。
:::
## 系统要求 \{#requirements\}
...
- 如果项目使用 `.gradle.kts` 文件，将依赖项添加到模块级 `build.gradle.kts`
...
:::important
在调用任何其他 Adapty SDK 方法之前，请等待 `Adapty.activate` 执行完毕。完整调用顺序请参阅 [Android SDK 中的调用顺序](android-sdk-call-order)。
:::

接下来在你的应用中配置付费墙：

- 如果你使用 [Adapty 付费墙编辑工具](adapty-paywall-builder)，请参考[付费墙编辑工具快速入门](android-quickstart-paywalls)。
- 如果你自行构建付费墙 UI，请参考[自定义付费墙快速入门](android-quickstart-manual)。
...
如果你需要将 Adapty 的日志消息发送到自己的系统或保存到文件，可以覆盖默认行为：
<Tabs>
<TabItem value="kotlin" label="Kotlin" default>
```kotlin showLineNumbers
...
除非你明确发送用户的个人数据，否则 Adapty 不会存储这些数据。你也可以根据应用商店或所在国家/地区的规定，实施额外的数据安全策略。
...
激活 Adapty 模块时，将 `adIdCollectionDisabled` 设为 `true` 可禁止收集用户的[广告 ID](https://support.google.com/googleplay/android-developer/answer/6048248)。默认值为 `false`。
使用此参数可遵守 Play Store 政策、避免触发广告 ID 权限提示，或者当您的应用不需要基于广告 ID 的广告归因或分析时使用。
...
使用 `AdaptyUI.configureMediaCache` 可以覆盖默认的缓存大小和有效期。这是可选的——如果不调用此方法，将使用默认值（磁盘大小 100MB，有效期 7 天）。
...
| diskCacheValidityTime   | 可选     | 缓存文件的有效时长。默认为 7 天。                     |
:::tip
...
Google Play 在某些场景下要求使用混淆账户 ID，以提升用户隐私和安全性。这些 ID 可帮助 Google Play 识别购买行为，同时保持用户信息匿名，对于防欺诈和数据分析尤为重要。

如果你的应用涉及敏感用户数据，或需要遵守特定隐私法规，则可能需要设置这些 ID。混淆 ID 让 Google Play 能够追踪购买记录，而无需暴露真实的用户标识符。
<Tabs groupId="current-os" queryString>
<TabItem value="kotlin" label="Kotlin" default>
...
如果你的应用使用了多个进程，请只初始化一次 Adapty，否则可能会出现意外行为。
...
如果你在未设置此值的情况下尝试在另一个进程中激活 Adapty，SDK 将记录警告并跳过激活。
### 启用本地访问等级 \{#enable-local-access-levels\}

默认情况下，[本地访问等级](local-access-levels) 在 Android 上是禁用的。要启用此功能，请将 `withLocalAccessLevelAllowed` 设置为 `true`：
...
部分 SDK（包括 Adapty）会自带 Android Auto Backup 配置。如果你同时使用多个定义了备份规则的 SDK，Android 清单合并器可能会报错，提示 `android:fullBackupContent`、`android:dataExtractionRules` 或 `android:allowBackup` 冲突。
...
- 告知 manifest 合并器使用你的应用中与备份相关属性的值。

- 将 Adapty 和其他 SDK 的备份规则合并到一个 XML 文件中（Android 12+ 则需要一对文件）。

#### 1. 在 manifest 中添加 `tools` 命名空间 \{#add-the-tools-namespace-to-your-manifest\}
...
#### 2. 在 `<application>` 中覆盖备份属性 \{#override-backup-attributes-in-application\}
在您应用的 `AndroidManifest.xml` 中，更新 `<application>` 标签，使应用提供最终值，并告知 manifest 合并工具替换库中的值：
...
#### 3. 创建合并的备份规则文件 \{#3-create-merged-backup-rules-files\}

在 `app/src/main/res/xml/` 目录下创建 XML 文件，将 Adapty 的规则与其他 SDK 的规则合并。由于 Android 在不同 OS 版本中使用不同的备份规则格式，同时创建两个文件可确保与你的应用所支持的所有 Android 版本兼容。

:::note
以下示例以 AppsFlyer 作为第三方 SDK 的示例。请根据你在应用中实际使用的其他 SDK，替换或添加相应规则。
:::
...
为确保购买功能正常运行，启动购买流程的 Activity 只能使用 `standard` 或 `singleTop` 启动模式，避免使用其他模式。
