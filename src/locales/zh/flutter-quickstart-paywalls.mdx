---
title: "在 Flutter SDK 中通过 Flow 和付费墙编辑工具启用购买功能"
...
- [**流程**](adapty-flow-builder) – 向用户展示产品的屏幕序列，在无代码 Flow & Paywall Builder 中构建。SDK 通过 `getFlow` 获取流程。如果你更倾向于在自己的代码中构建 UI，请使用付费墙——参见[手动实现付费墙](flutter-quickstart-manual)。
- [**版位**](placements) – 在应用中展示流程的位置和时机（如 `main`、`onboarding`、`settings`）。在看板中将流程绑定到版位，然后在代码中通过版位 ID 请求。这样便于进行 A/B 测试，并向不同用户展示不同的流程。
...
| 手动创建付费墙 | 🟡 中等 | 您在应用代码中自行实现付费墙 UI，但仍从 Adapty 获取流程对象，以保持产品组合的灵活性。请参阅[指南](flutter-quickstart-manual)。 |
| 观察者模式 | 🔴 复杂 | 您已有自己的购买处理基础设施并希望继续使用。请注意，观察者模式在 Adapty 中存在一定限制。请参阅[文章](observer-vs-full-mode)。 |

:::important
**以下步骤介绍如何实现在 Adapty 流程与付费墙编辑工具中创建的流程。**

如果您希望自行构建付费墙 UI，请参阅[手动实现付费墙](flutter-quickstart-manual)。
:::

要在应用代码中展示在 Adapty 流程与付费墙编辑工具中创建的流程，您只需：

1. **获取流程**：从 Adapty 获取流程。
2. **展示流程，由 Adapty 处理购买**：在应用中显示该视图。
...
1. 在 Adapty 看板中，将你的应用连接至 [App Store](initial_ios) 和/或 [Google Play](initial-android)。
...
您的流程与在看板中配置的版位相关联。通过版位，您可以针对不同的目标受众运行不同的流程，也可以运行 [A/B 测试](ab-tests)。

要获取在 Adapty Flow & Paywall Builder 中创建的流程，您需要：

1. 使用 `getFlow` 方法通过[版位](placements) ID 获取 `flow` 对象，并通过 `hasViewConfiguration` 属性检查该流程是否在编辑工具中创建。

2. 使用 `createFlowView` 方法创建流程视图。该视图包含显示流程所需的 UI 元素和样式。

:::important
要获取视图配置，请先[发布流程](builder-save-publish)。含有未发布编辑内容的流程状态为 **Dirty**，其版位将继续使用上次发布的版本。
:::
