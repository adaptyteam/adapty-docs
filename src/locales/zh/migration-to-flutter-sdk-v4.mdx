...
description: "通过将付费墙 API 替换为流程 API，迁移至 Adapty Flutter SDK v4.0，同时兼容 Flow Builder 和 Paywall Builder。"
metadataTitle: "迁移至 Adapty Flutter SDK v4.0 | Adapty 文档"
---

Adapty Flutter SDK 4.0 引入了流程功能，并对付费墙 API 进行了相应重命名。新 API 同时兼容全新的 Flow Builder 和现有的 Paywall Builder，无需在 Adapty 看板侧做任何配置变更。
...
:::note
`customTimers` 依然存在，但它只对旧版付费墙编辑工具的付费墙生效。流程的[倒计时器](flow-timer)按照流程编辑工具中设置的行为运行，因此流程会忽略你在此处传入的值。
:::
