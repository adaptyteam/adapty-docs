---
title: "将 Adapty Flutter SDK 迁移至 v4.0"
description: "通过将付费墙 API 替换为流程 API，将 SDK 迁移至 Adapty Flutter SDK v4.0，流程 API 可渲染流程及旧版编辑工具中的付费墙。"
metadataTitle: "迁移至 Adapty Flutter SDK v4.0 | Adapty 文档"
---

Adapty Flutter SDK 4.0 引入了流程功能，并相应地对付费墙 API 进行了重命名。新 API 可用于流程，同时也兼容旧版编辑工具中的付费墙——无需在 Adapty 看板侧进行任何配置更改。

:::note
本指南介绍 3.x → 4.0 的迁移。SDK 4.1 在此基础上还调整了部分 API——Adapty 归因变为按需启用，外部归因方法已重命名，备用付费墙文件格式也再次变更。完成本指南后，请参阅[迁移至 v4.1](migration-to-flutter-sdk-41)。
:::
...
备用文件格式[在 SDK v4 中已更改](fallback-flows)。请从 **[Placements](https://app.adapty.io/placements)** > **Fallbacks** 下载新文件并将其打包到您的应用中。
...
`logShowPaywall` 已重命名为 `logShowFlow`，现在接收一个 `AdaptyFlow` 参数。事件仍会记录到相同的实验变体中，因此现有漏斗和 A/B 测试的数据图表无需更改看板配置即可继续正常使用。
...
与 v3 一样，当显示由 Adapty 渲染的流程或付费墙时，无需手动调用此方法——Adapty 会自动追踪这些浏览行为。

## 显示流程 \{#displaying-flows\}

### createPaywallView → createFlowView \{#createpaywallview--createflowview\}

重命名该方法，并通过 `flow` 参数传入 `AdaptyFlow`。其他参数（`loadTimeout`、`preloadProducts`、`customTags`、`customTimers`、`customAssets`、`productPurchaseParams`）保持不变，视图方法 `present`、`dismiss` 和 `showDialog` 同样不变：

:::note
`customTimers` 仍然存在，但它只影响旧版付费墙编辑工具的付费墙。流程中的[倒计时](flow-timer)按照 Flow & Paywall Builder 中设置的行为运行，因此流程会忽略此处传入的任何值。
:::
...
- `setFlowsEventsObserver`（和 `setOnboardingsEventsObserver`）现在接受 `null` 来解绑之前设置的观察者，因此 SDK 不再持有对它的引用。
- 新增可选回调 `flowViewDidReceiveAnalyticEvent`，用于上报流程中的分析事件，从用户打开的每个屏幕的页面浏览事件开始记录。详见 [追踪流程屏幕浏览](flutter-flow-screen-views)。
...
旧版用户引导 API 已在 v4.0 中弃用，请改用 [Flow & 付费墙编辑工具](adapty-flow-builder)。该 API 目前仍可正常使用，IDE 会通过 `@Deprecated` 注解标记已弃用的符号——运行时不会产生任何警告。这些符号将在未来版本中移除，请尽快将您的用户引导迁移至 Flow & 付费墙编辑工具。
