...
description: "在 React Native 应用中使用 Adapty 向用户展示流程和付费墙。"
...
如果您已创建流程或在旧版付费墙编辑工具中创建了付费墙，则无需在移动应用代码中手动处理其渲染逻辑来向用户展示它。此类流程已包含展示内容和展示方式的完整配置。

在开始之前，请确保：

1. 您已创建[流程](paywall-builder-templates)或[付费墙](create-paywall)。
2. 您已将其添加到[版位](placements)中。
3. 您已[获取流程并准备好视图](react-native-get-pb-paywalls)。

:::warning

本指南**仅适用于由 Adapty 渲染的流程和付费墙**，需要 SDK v4.0 或更高版本。展示流程的方式与远程配置付费墙有所不同。
...
本指南仅适用于使用旧版付费墙编辑工具为 **Adapty SDK 3.x** 构建的付费墙，需要 SDK v3.0 或更高版本。展示为 SDK 2.x 或更早版本构建的付费墙，或展示远程配置付费墙，操作方式有所不同。
