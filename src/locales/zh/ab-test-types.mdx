...
* 用户遇到的第一个版位决定了 Adapty 展示哪个付费墙。你无法更改用户的分组，也无法将同一用户加入多个跨版位 A/B 测试。

* 设备只有在应用切换到活跃状态时，才会同步在其他地方（例如用户的其他设备上）完成的跨版位分组信息，且最多每 60 秒同步一次。这一同步无法手动触发，因此分组信息最多需要一分钟，或等到下次应用进入前台时，才能同步到长时间运行的会话中。

:::warning
一旦用户看到跨版位付费墙，他们将在 90 天内持续看到该付费墙，即使你已停止测试。如需更改此时长，请在 **General** 设置中调整 **[Cross-placement variation stickiness](general#9-cross-placement-variation-stickiness)**。
:::
