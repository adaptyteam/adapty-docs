...
- **在编辑工具中构建的流程**：`createFlowView` 不接受语言区域参数，SDK 也不读取设备的语言区域设置。流程将以其[默认语言区域](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)渲染——在 iOS 上，如果流程包含英语本地化，则以 `en` 渲染。
- **自定义（远程配置）付费墙**：`getFlow` 在 `flow.remoteConfigs` 中返回所有已配置的本地化内容。每个条目包含一个 `lang` 代码和一个 `data` 对象。请使用自己的回退逻辑选择与用户匹配的条目：
