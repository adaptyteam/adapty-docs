...
`view.locale` 报告的是视图构建时所使用的本地化语言。`locale` 选项和 `view.locale` 均需要 Capacitor SDK 4.0.1-beta.1，在更早版本中 `view.locale` 为 `undefined`。[`onAppeared` 处理程序](capacitor-handling-events#the-view-argument) 也会返回相同的值。
- **自定义（远程配置）付费墙**：`getFlow` 在 `flow.remoteConfigs` 中返回所有已配置的本地化语言。每个条目包含一个 `lang` 语言代码和一个 `data` 对象。按照自己的降级逻辑选择与用户匹配的条目：
