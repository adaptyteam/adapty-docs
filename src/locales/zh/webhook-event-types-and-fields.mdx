...
| **active_introductory_offer_type** | String        | 已应用的新用户优惠类型。可能的值为 `free_trial`、`pay_as_you_go` 和 `pay_up_front`。 |
| **active_promotional_offer_id**    | String        | 促销活动的 ID，如 Adapty 看板产品部分所示。                  |
| **active_promotional_offer_type**  | String        | 已应用的促销活动类型。可能的值为 `free_trial`、`pay_as_you_go` 和 `pay_up_front`。 |
...
| **billing_issue_detected_at**      | ISO 8601 date | 发生账单问题的日期和时间。                                   |
...
| **expires_at**                     | ISO 8601 date | <p>访问过期的日期和时间。Adapty 根据此字段计算 `is_active`。</p><p>在宽限期内，此字段保存宽限期的结束时间；退款或取消后，此字段保存取消日期。没有单独的宽限期结束字段。</p> |
| **is_active**                      | Boolean       | 表示访问等级是否有效的布尔值。在事件生成时，根据 `expires_at`（结合 `starts_at` 和 `is_lifetime`）计算得出。 |
| **is_in_grace_period**             | Boolean       | 表示用户画像是否处于宽限期的布尔值。为 `true` 时，`expires_at` 保存宽限期的结束时间。 |
| **is_lifetime**                    | Boolean       | 表示访问等级是否为永久授权的布尔值。                         |
| **is_refund**                      | Boolean       | 表示交易是否为退款的布尔值。                                 |
| **original_purchase_date**         | ISO 8601 date | 对于自动续订订阅，原始购买是该续订链中的第一笔交易，其 ID 称为原始交易 ID，用于关联续订链；后续交易均为其延续。原始购买日期即该第一笔交易的日期和时间。 |
| **original_transaction_id**        | String        | <p>对于自动续订订阅，这是关联续订链的原始交易 ID。原始交易是链中的第一笔；后续交易均为其延续。</p><p>如果没有续订，`original_transaction_id` 与 store_transaction_id 相同。</p>原始购买的交易标识符。 |
...
| **purchase_date**                  | ISO 8601 date | 产品购买的日期和时间。                                       |
| **renewed_at**                     | ISO 8601 date | 访问将续订的日期和时间。                                     |
| **starts_at**                      | ISO 8601 date | 访问等级开始的日期和时间。                                   |
| **store**                          | String        | 购买产品的商店。标准值：**app_store**、**play_store**、**stripe**、**paddle**。<br/>如果使用服务端 API 设置了[自定义商店交易](api-adapty/operations/setTransaction)，则使用 **store** 参数中的值。 |
