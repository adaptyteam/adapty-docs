...
Adapty 会在订阅事件发生时发送 Webhook。本节定义了这些事件类型及每个 Webhook 所包含的数据。
...
:::note
在 Google Play 上，Adapty 可能会发送 `subscription_started` 事件，且所有金额字段均为零——例如，当 Google 用一个订阅替换另一个订阅，或订阅以免费阶段开始时。请勿将此类事件计入收入。当 Google 替换订阅时，会生成新的购买令牌，因此 Adapty 会以不同的 `store_transaction_id` 上报付费交易。
:::
...
| **base_plan_id** | String | Google Play Store 中的[基础方案 ID](https://support.google.com/googleplay/android-developer/answer/12154973)，或 Stripe 中的[价格 ID](https://docs.stripe.com/products-prices/how-products-and-prices-work#use-products-and-prices)。 |
...
| **consecutive_payments** | Integer | 用户连续订阅的周期数（不中断），包含当前周期。 |
| **currency** | String | 本地货币。 |
| **developer_id** | String | 交易来源的[版位](placements) ID。 |
| **discount_amount_local** | Float | 本次交易的折扣金额：标准价格减去实际收取金额（扣除 Apple/Google 分成前），以本地货币计。全价购买时为 `0`。免费试用时等于完整标准价格（`original_price_local`），因为未收取任何费用。当优惠已应用但标准价格未知时为 `null`（参见 `original_price_local`）。App Store 预付费优惠始终为 `null`：因为单次预付费涵盖多个计费周期，无法与每周期标准价格进行比较。 |
| **discount_amount_usd** | Float | `discount_amount_local` 的美元换算值。 |
| **environment** | String | 可能的值为 `Sandbox` 或 `Production`。 |
| **event_datetime** | ISO 8601 date | 事件的日期和时间，与事件根层级的值相同。 |
| **original_price_local** | Float | 产品的标准未折扣价格（扣除 Apple/Google 分成前），以本地货币计。对于订阅，这是续订价格。全价购买时等于 `price_local`，一次性购买时也始终等于 `price_local`，因为商店不会为其单独报告标准价格。当商店无法提供可靠标准价格时（例如自动续订已关闭、续订仍带有优惠，或存在待处理的产品变更），折扣购买时为 `null`。 |
| **original_price_usd** | Float | `original_price_local` 的美元换算值。 |
| **original_purchase_date** | ISO 8601 date | 对于周期性订阅，原始购买是该链中的第一笔交易，其 ID 称为原始交易 ID，用于关联续订链；后续交易均为其延伸。原始购买日期即该第一笔交易的日期和时间。 |
| **original_transaction_id** | String | <p>对于周期性订阅，这是关联续订链的原始交易 ID。原始交易是链中的第一笔；后续交易均为其延伸。</p><p>如果没有延伸交易，`original_transaction_id` 与 store_transaction_id 相同。</p> |
| **paywall_name** | String | 交易来源的付费墙名称。 |
| **paywall_revision** | String | 交易来源的付费墙版本号，默认值为 1。 |
| **price_local** | Float | 本次交易实际收取的金额（扣除 Apple/Google 分成前），以本地货币计。免费试用时为 `null`，因为未收取任何费用。 |
| **price_usd** | Float | 本次交易实际收取的金额（扣除 Apple/Google 分成前），以美元计。免费试用时为 `null`，因为未收取任何费用。 |
...
| **rate_after_first_year** | Boolean | 布尔值，表示该订阅在持续续订满一年后是否符合降低佣金比例（通常为 15%）的条件。佣金比例因计划资格和国家/地区而异。详情请参阅[商店佣金与税费](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue)。 |
| **store** | String | 购买产品的商店。标准值：**app_store**、**play_store**、**stripe**、**paddle**。<br/>若通过服务端 API 设置了[自定义商店交易](api-adapty/operations/setTransaction)，则使用 **store** 参数中的值。 |
| **store_country** | String | 应用商店向我们报告的国家/地区。 |
...
| **store_offer_number_of_periods** | Integer | 优惠折扣覆盖的基础计费周期数（1 个或更多）。仅在优惠适用时存在。App Store 预付费优惠以及商店未报告优惠时长时为 `null`。 |
| **subscription_expires_at** | ISO 8601 date | <p>订阅的到期日期。</p><p>在退款、账单问题、宽限期和访问等级事件中，该值为取消日期或宽限期结束日期（如适用）。在购买和续订事件中，该值为订阅的常规到期日期。</p> |
...
| **activated_at**                   | ISO 8601 date | 访问等级最近一次激活的日期和时间。                           |
| **active_introductory_offer_type** | String        | 所应用的新用户优惠类型。可选值为 `free_trial`、`pay_as_you_go` 和 `pay_up_front`。 |
| **active_promotional_offer_id**    | String        | 促销活动的 ID，如 Adapty 看板产品部分所示。                  |
| **active_promotional_offer_type**  | String        | 所应用的促销活动类型。可选值为 `free_trial`、`pay_as_you_go` 和 `pay_up_front`。 |
...
| **billing_issue_detected_at**      | ISO 8601 date | 发生结算问题的日期和时间。                                   |
| **cancellation_reason**            | String        | 取消原因，可选值为：`voluntarily_cancelled`、`billing_error`、`price_increase`、`product_was_not_available`、`refund`、`cancelled_by_developer`、`new_subscription_replace`、`upgraded`、`unknown`、`adapty_revoked`。 |
...
| **expires_at**                     | ISO 8601 date | <p>访问等级到期的日期和时间。Adapty 根据此字段计算 `is_active`。</p><p>在宽限期内，该字段保存宽限期的结束时间；退款或取消后，该字段保存取消日期。没有单独的宽限期结束字段。</p> |
| **is_active**                      | Boolean       | 表示访问等级是否有效的布尔值。在生成事件时，根据 `expires_at`（结合 `starts_at` 和 `is_lifetime`）计算得出。 |
| **is_in_grace_period**             | Boolean       | 表示用户画像是否处于宽限期的布尔值。为 `true` 时，`expires_at` 保存宽限期的结束时间。 |
| **is_lifetime**                    | Boolean       | 表示访问等级是否为永久授权的布尔值。                         |
| **is_refund**                      | Boolean       | 表示该交易是否为退款的布尔值。                               |
| **original_purchase_date**         | ISO 8601 date | 对于自动续订订阅，原始购买是该续订链中的第一笔交易，其 ID（称为原始交易 ID）将整个续订链关联在一起；后续交易均为其延续。原始购买日期即该第一笔交易的日期和时间。 |
| **original_transaction_id**        | String        | <p>对于自动续订订阅，这是将整个续订链关联在一起的原始交易 ID。原始交易是链中的第一笔；后续交易均为其延续。</p><p>如果没有后续续订，`original_transaction_id` 与 store_transaction_id 相同。</p>原始购买的交易标识符。 |
| **paywall_name**                   | String        | 交易来源的付费墙名称。                                       |
| **paywall_revision**               | String        | 交易来源的付费墙修订版本号，默认值为 1。                     |
| **profile_country**                | String        | 由 Adapty 根据用户画像的 IP 地址确定。                       |
| **profile_event_id**               | UUID          | 唯一事件 ID，可用于去重。                                    |
| **profile_has_access_level**       | Boolean       | 表示用户画像是否拥有有效访问等级的布尔值。                   |
| **profile_id**                     | UUID          | Adapty 内部用户画像 ID。                                     |
| **profile_ip_address**             | String        | 用户画像的 IP 地址（可以是 IPv4 或 IPv6，优先使用 IPv4）。若在[应用设置](https://app.adapty.io/settings/general)中禁用了 **Collect users' IP addresses**，则为 `null`。 |
| **profile_total_revenue_usd**      | Float         | 该用户画像的总收入（含退款）。                               |
| **purchase_date**                  | ISO 8601 date | 产品购买的日期和时间。                                       |
| **renewed_at**                     | ISO 8601 date | 当前付费访问周期最近一次开始的日期和时间。在首次续订之前，该值与 `activated_at` 相同。 |
| **starts_at**                      | ISO 8601 date | 访问等级开始的日期和时间。                                   |
| **store**                          | String        | 购买产品的商店。标准值：**app_store**、**play_store**、**stripe**、**paddle**。<br/>如果通过服务端 API 设置了[自定义商店交易](api-adapty/operations/setTransaction)，则使用 **store** 参数中的值。 |
| **store_country**                  | String        | 应用商店发送给 Adapty 的国家/地区信息。                      |
| **subscription_expires_at**        | ISO 8601 date | 订阅的到期日期。在此事件中，其值与 `expires_at` 相同：宽限期内为宽限期结束时间；退款或取消后为取消日期。 |
| **transaction_id**                 | String        | 交易的唯一标识符。                                           |
| **trial_duration**                 | String        | 试用期时长（以天为单位，例如"7 days"）。                     |
| **variation_id**                   | UUID          | 实验变体的标识符，用于将购买归因到对应的付费墙。             |
| **vendor_product_id**              | String        | <p>商店（Apple/Google/Stripe）中的产品 ID。</p><p>如果访问权限不是通过真实商店交易授予的，`vendor_product_id` 将为以下之一：</p><ul><li>`adapty_server_side_product` — 通过[服务端 API](api-adapty/operations/grantAccessLevel) 授予。</li><li>`adapty_dashboard_product` — 在 Adapty 看板中[手动授予](give-access-level-to-specific-customer)。</li><li>`adapty_promotion` — 旧版。</li></ul> |
