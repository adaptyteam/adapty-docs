---
title: "将 Adapty Capacitor SDK 迁移至 v4.1.1"
description: "迁移至 Adapty Capacitor SDK v4.1.1：将付费墙 API 替换为流程 API、采用重命名后的归因 API、显式启用 Adapty Attribution，并重新下载备用付费墙文件。"
metadataTitle: "迁移至 Adapty Capacitor SDK v4.1.1 | Adapty 文档"
---
...
4.x 版本引入了流程功能，并相应地对付费墙 API 进行了重命名。新 API 同时适用于新版 Flow Builder 和现有的付费墙编辑工具，Adapty 看板侧无需任何配置变更。此外，4.1.1 版本将 Adapty Attribution 改为按需启用，重命名了外部归因方法，更改了备用文件格式，并新增了 App Store 应用内促销购买支持。
...
| v3 | v4.1.1 |
|---|---|
| 自动启用 Adapty Attribution | 默认禁用 — 通过 `adaptyAttributionEnabled: true` 启用 |
...
| 已为 3.x 下载备用文件 | 新备用文件格式 — 请重新下载文件 |
| 不支持应用内促销购买 | `'onPromotedPurchaseReceived'` 事件及 `adapty.makePromotedPurchase({ product })` |
...
:::warning
如果你使用了 [Adapty 归因](user-acquisition)，但在未开启选项的情况下升级到 SDK 4.1.1，它会静默失效——安装事件将停止记录，且不会有任何警告提示。
:::

在早期版本中，SDK 会自动为 [Adapty Attribution](user-acquisition) 注册安装信息。从 SDK 4.1.1 版本起，此功能默认关闭：SDK 不再注册安装信息，`'onInstallationDetailsSuccess'` 和 `'onInstallationDetailsFail'` 事件不会触发，`getCurrentInstallationStatus` 返回 `not_available` 状态。
...
[备用文件](fallback-flows)的格式在 4.x 版本线中发生了变化，4.1.1 版本又再次更新。请从 **[Placements](https://app.adapty.io/placements)** > **Fallbacks** 重新下载该文件并将其打包到应用中，即使你已经为 4.0 测试版下载过一份也需要重新下载。

:::warning
跳过此步骤不会产生任何构建错误。但如果你跳过了，`setFallback` 会拒绝旧版文件，导致所有版位失去其备用付费墙。
:::
...
| _（新增）_ | `paywalls: AdaptyFlowPaywall[]` | 每个条目对应流程中的一个付费墙变体。 |
...
另外，4.1.1 还新增了一个 SDK 级别的事件（而非流程处理器）：`'onPromotedPurchaseReceived'`，通过 `adapty.addListener` 传递。若未注册监听器，SDK 将自行完成促销购买；注册监听器后，则由你的应用负责完成购买流程。详见 [App Store 应用内推广购买](#app-store-promoted-in-app-purchases)。
...
SDK 4.1.1 新增了对 [App Store 产品页面推广应用内购买](capacitor-making-purchases#in-app-purchases-from-the-app-store) 的支持。这是一项新功能，无需迁移操作：SDK 会自动完成推广购买，无需编写任何代码即可获得该行为。
