...
[备用文件](fallback-flows) 的格式在 4.0 版本中发生了变化，在 4.1.1 版本中再次更新。请从 **[Placements](https://app.adapty.io/placements)** > **Fallbacks** 重新下载该文件并将其打包到应用中，即使你已经为 4.0 测试版下载过。
...
:::note
`customTimers` 仍然存在，但它只影响旧版付费墙编辑工具的付费墙。流程的[倒计时器](flow-timer)按照 Flow & Paywall Builder 中设置的行为运行，因此流程会忽略你在此处传入的值。
:::

| 参数 | 描述 |
|---|---|
| `locale` | 渲染流程时所使用的本地化语言。该参数已从 `getPaywall` 移至此处——详见 [getPaywall → getFlow](#getpaywall--getflow)。 |
...
旧版用户引导 API 在 v4 中已弃用，请改用 [Flow & 付费墙编辑工具](adapty-flow-builder)。目前该 API 仍可正常使用，但将在未来版本中移除，请尽快将您的用户引导迁移至 Flow & 付费墙编辑工具。
