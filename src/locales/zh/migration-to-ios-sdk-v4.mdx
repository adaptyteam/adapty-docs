...
description: "通过将付费墙 API 替换为流程 API，将项目迁移至 Adapty iOS SDK v4.0，新 API 可渲染流程以及旧版编辑工具中创建的付费墙。"
metadataTitle: "迁移至 Adapty iOS SDK v4.0 | Adapty 文档"
---

Adapty iOS SDK 4.0 引入了流程功能，并对付费墙相关 API 进行了重命名。新 API 同时支持流程和旧版编辑工具创建的付费墙——Adapty 看板侧无需做任何配置变更。
...
Adapty iOS SDK 4.0 已放弃对 CocoaPods 的支持。请使用 [Swift Package Manager](sdk-installation-ios#install-adapty-sdk) 安装 SDK。v4 还要求使用 **Xcode 26** 或更高版本，因为其 package manifest 声明了 Swift tools 6.2。

如果你的项目仍在使用 CocoaPods，请从 `Podfile` 中移除 `Adapty` 和 `AdaptyUI` pod，运行 `pod install` 清理它们，然后在 Xcode 中通过 **File → Add Package Dependency** 使用 `https://github.com/adaptyteam/AdaptySDK-iOS.git` 添加该 package。
...
## App Store 促销购买处理功能暂时移除 \{#app-store-promoted-in-app-purchases-temporarily-removed\}

作为 StoreKit 2 迁移工作的一部分，Adapty iOS SDK 4.0 移除了处理 App Store 促销内购的 API：`shouldAddStorePayment(for:)` 代理方法及其接收的 `AdaptyDeferredProduct` 类型在 4.0 中均不可用。此类购买仍会正常完成，Adapty 也会记录相应的交易——但 4.0 不再为你的应用提供任何拦截或延迟这些购买的途径。

:::warning
此移除仅适用于 4.0 版本——该功能将在 SDK 4.1 中以新 API 的形式恢复。如果您的应用需要拦截应用内推广购买，请直接从 3.x 迁移至 4.1。请参阅[来自 App Store 的应用内购买](making-purchases#in-app-purchases-from-the-app-store)。
:::
...
返回类型从 `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` 变更为 `AdaptyFlow` / `AdaptyUI.FlowConfiguration`。`locale` 参数从获取调用中移出，转移到 `getFlowConfiguration`：`AdaptyFlow` 同时暴露 `paywalls` —— 流程的付费墙变体，类型为 `AdaptyFlowPaywall`。Web 付费墙方法接受其中之一作为参数。
...
`logShowPaywall` 已重命名为 `logShowFlow`，现在接受 `AdaptyFlow` 而非 `AdaptyPaywall`。该事件仍会记录到同一实验变体，因此现有的漏斗和 A/B 测试数据图表无需修改看板即可继续正常使用。
...
与 v3 一样，当展示由 Adapty 渲染的流程或付费墙时，无需手动调用此方法——Adapty 会自动追踪这些浏览事件。
...
:::note
SDK 4.1 将此方法重命名为 `updateExternalAttribution(_:provider:)`，将类型重命名为 `AdaptyExternalAttributionProvider`。旧名称在 4.1 版本中直接移除，未经弃用警告。如果你直接迁移到 4.1，请参阅[迁移至 v4.1](migration-to-ios-sdk-41)。
:::
