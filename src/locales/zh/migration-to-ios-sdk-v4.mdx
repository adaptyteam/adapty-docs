...
作为 StoreKit 2 迁移的一部分，Adapty iOS SDK 4.0 移除了对 App Store 促销应用内购买的支持。`shouldAddStorePayment(for:)` 代理方法及其接收的 `AdaptyDeferredProduct` 类型在 4.0 中不可用。

:::warning
此移除仅适用于 4.0——促销应用内购买支持将在 SDK 4.1 中以全新 API 的形式回归。如果你的应用依赖于促销应用内购买，请直接从 3.x 迁移至 4.1。请参阅[来自 App Store 的应用内购买](making-purchases#in-app-purchases-from-the-app-store)。
:::
