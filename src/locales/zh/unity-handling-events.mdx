...
在[流程与付费墙编辑工具](adapty-flow-builder)中配置的流程和付费墙无需额外代码即可完成购买和恢复购买操作。但它们会生成一些你的应用可以响应的事件，包括按钮点击（关闭按钮、URL、产品选择等）以及购买相关操作的通知。请参阅以下内容了解如何响应这些事件。
...
使用[付费墙编辑工具](adapty-paywall-builder)配置的付费墙无需额外代码即可完成购买和恢复购买操作。但它们会生成一些事件，供您的应用响应。这些事件包括按钮点击（关闭按钮、URL、产品选择等）以及付费墙上与购买相关操作的通知。请参阅以下内容，了解如何响应这些事件。

:::warning
本指南仅适用于**旧版编辑工具构建的付费墙**，需要 Adapty SDK v3.3.0 或更高版本。
:::
