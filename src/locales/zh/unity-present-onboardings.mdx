...
description: "了解如何有效展示用户引导以提高转化率。"
...
如果你已经使用编辑工具自定义了用户引导，就无需在 Unity 应用代码中单独处理其渲染逻辑来向用户展示它。这类用户引导已经包含了应展示的内容以及展示方式的完整配置。
...
要显示用户引导，请对 `CreateOnboardingView` 方法创建的 `view` 调用 `view.Present()` 方法。每个 `view` 只能使用一次。如需再次显示付费墙，请再次调用 `CreateOnboardingView` 创建新的 `view` 实例。

:::warning
重复使用同一个 `view` 而不重新创建它，可能会导致 `AdaptyUIError.viewAlreadyPresented` 错误。
:::
