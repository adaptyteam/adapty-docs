...
2. 在该方法被调用时保存最新的用户画像数据，这样就可以在应用各处直接使用，无需额外发起网络请求。

:::note
在 SDK 4.1 中，监听器接口遵循 C# I-前缀命名规范——请实现 `IAdaptyEventListener` 而非 `AdaptyEventListener`——同时该接口还新增了一个方法 `OnReceivePromotedPurchase`。详情请参阅[迁移指南](migration-to-unity-sdk-v4)。
:::
