...
:::important
本指南涵盖购买、恢复、产品选择和流程渲染的事件处理。你还必须实现按钮处理（关闭流程、打开链接等）。详情请参阅[按钮操作处理指南](android-handle-paywall-actions)。
:::

在 [Flow & Paywall Builder](adapty-flow-builder) 中配置的流程和付费墙无需额外代码即可完成购买和恢复购买操作。但它们会生成一些事件，供你的应用响应。这些事件包括按钮点击（关闭按钮、URL、产品选择等）以及购买相关操作的通知。请参阅以下内容了解如何响应这些事件。

<SampleApp />

如果你需要控制或监控购买界面上发生的流程，请实现 `AdaptyFlowEventListener` 的各个方法。
...
该方法在 Observer 模式下不会被调用。详情请参阅 [Android - 在 Observer 模式下展示流程](android-present-paywall-builder-paywalls-in-observer-mode)。
...
在 Observer 模式下，该方法不会被调用。详情请参阅 [Android - 在 Observer 模式下展示流程](android-present-paywall-builder-paywalls-in-observer-mode)。
...
该方法不会在 Observer 模式下被调用。详情请参阅 [Android - 在 Observer 模式下展示流程](android-present-paywall-builder-paywalls-in-observer-mode) 主题。

#### 完成网页支付导航 \{#finished-web-payment-navigation\}

此方法在尝试为特定产品打开[网页付费墙](web-paywall)后被调用。包括导航成功和失败两种情况：
...
请参阅[追踪流程页面浏览](android-flow-screen-views)，了解如何处理这些事件。

流程还会上报用户输入和选择的值。请参阅[处理流程中的数据](android-flow-input)以读取这些值。
...
| 方法 | 描述 |
|:-------|:------------|
| **onShowAppRate** | 为流程中的应用评分请求预留。当前流程尚不触发应用评分请求，因此无需实现。 |
| **onShowRequestPermission** | 为流程中的系统权限请求（如推送通知或相机访问）预留。当前流程尚不触发权限请求，因此无需实现。 |
...
使用[付费墙编辑工具](adapty-paywall-builder)配置的付费墙无需额外代码即可完成购买和恢复购买操作。但它们会生成一些事件，供您的应用响应。这些事件包括按钮点击（关闭按钮、URL、产品选择等）以及付费墙上购买相关操作的通知。请参阅以下内容了解如何响应这些事件。

:::warning
本指南仅适用于**旧版编辑工具构建的付费墙**，需要 Adapty SDK v3.0 或更高版本。
:::
