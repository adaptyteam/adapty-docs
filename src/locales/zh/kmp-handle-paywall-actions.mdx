...
description: "在 Kotlin Multiplatform 应用中处理流程和付费墙的按钮操作。"
...
如果你正在使用 Adapty 流程与付费墙编辑工具构建流程，正确设置按钮至关重要：
...
:::warning
**只有购买和恢复购买会被自动处理。** 其他所有按钮操作（如关闭付费墙或打开链接）都需要在应用代码中实现相应的处理逻辑。
:::

如果你正在使用 Adapty 付费墙编辑工具构建付费墙，务必正确设置按钮：

1. 在旧版付费墙编辑工具中，添加一个按钮，并为其分配一个预置操作或自定义操作 ID。
