...
在 4.0 及更早版本中，SDK 会自动为 [Adapty 归因](user-acquisition) 注册安装事件。从 4.1 版本开始，该功能默认关闭：SDK 不再注册安装事件，`onInstallationDetailsSuccess` 和 `onInstallationDetailsFail` 委托回调也不会触发。`getCurrentInstallationStatus()` 返回 `.notAvailable`。
