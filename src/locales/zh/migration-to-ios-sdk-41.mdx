...
| `Adapty.updateAttribution(_ attributionJson: String, source:)` | 已从公共 API 中移除；请改用字典传参 |
...
| 为 4.0 下载的备用付费墙文件 | 新备用付费墙文件格式；请重新下载该文件 |
| 推广型应用内购买自动完成，无法拦截 | `didReceivePromotedPurchase(_:)` 代理方法和 `AdaptyPromotedProduct` 将完成处理权交给您的应用 |
...
## App Store 推广内购回归 \{#app-store-promoted-in-app-purchases-return\}

SDK 4.1 恢复了 App Store 推广内购的钩子，该功能在 [4.0 中被移除](migration-to-ios-sdk-v4#app-store-promoted-in-app-purchases-temporarily-removed)。在 4.0 中，此类购买仍会完成，Adapty 也会记录该交易，只是你的应用无法拦截它。4.1 保留了这一默认行为，因此这是一项新增能力，而非迁移步骤。
...
:::warning
新机制基于 StoreKit 2，需要 **iOS 16.4** 或更高版本。3.x 的 `shouldAddStorePayment` 方法支持更早的 iOS 版本。在 iOS 16.4 以下，`didReceivePromotedPurchase` 不会触发，促销购买将在不经过应用的情况下直接完成。
:::
