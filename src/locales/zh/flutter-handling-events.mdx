...
:::important
本指南涵盖购买、恢复、产品选择和渲染的事件处理。关闭视图和打开链接由默认的 `flowViewDidPerformAction` 实现处理——如需覆盖这些行为或处理自定义按钮操作，请参阅[处理按钮操作指南](flutter-handle-paywall-actions)。
:::
...
有关如何处理这些事件，请参阅[跟踪流程屏幕浏览量](flutter-flow-screen-views)。

流程还会上报用户输入和选择的值。请参阅[处理流程中的数据](flutter-flow-input)以了解如何读取这些值。
...
使用[付费墙编辑工具](adapty-paywall-builder)配置的付费墙无需额外代码即可完成购买和恢复购买操作。不过，它们会生成一些你的应用可以响应的事件，包括按钮点击（关闭按钮、URL、产品选择等）以及付费墙上购买相关操作的通知。请参阅以下内容，了解如何响应这些事件。

:::warning

本指南仅适用于**使用旧版编辑工具构建的付费墙**，需要 Adapty SDK v3.0 或更高版本。
