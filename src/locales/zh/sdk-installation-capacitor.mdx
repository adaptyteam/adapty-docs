...
使用 Adapty SDK v4（测试版）构建 iOS 应用需要 **Xcode 26** 或更高版本——其底层 iOS SDK 使用 Swift tools 6.2 构建。iOS 15.0+、Capacitor 8 以及 Android minSdk 24 的要求与 SDK 3.16+ 相同。

:::info
从 SDK v3.17 起，Adapty SDK 默认使用 Google Play Billing Library v8.0.0。

兼容某个 Billing Library 版本并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 计费能力之前，请参阅 [Play Store 中的产品](android-products)。
:::
