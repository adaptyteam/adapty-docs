...
使用 Adapty SDK v4（测试版）为 iOS 构建需要 **Xcode 26** 或更高版本——其底层 iOS 原生 SDK 使用 Swift tools 6.2 构建。iOS 15.0+、Capacitor 8 以及 Android minSdk 24 的要求与 SDK 3.16+ 相同。

:::info
Adapty Capacitor SDK 4.0.2 及更高版本支持 Google Play Billing Library v8。

与某个 Billing Library 版本兼容，并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 结算功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
