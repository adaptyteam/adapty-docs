...
构建 iOS 应用需要 **Xcode 26** 或更高版本——原生 iOS SDK 使用 Swift tools 6.2 构建。Android 最低支持的 SDK 版本为 24。

:::info
从 3.17.0 起，Adapty Capacitor SDK 默认使用 Billing Library v8。3.15.0 至 3.16.3 版本使用 v7，但如果 Android 构建中的其他依赖项将 Billing Library 升级到 v8，这些版本同样可以正常运行。

与某个 Billing Library 版本兼容并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在采用新的 Google Play 计费功能之前，请参阅 [Google Play 中的产品](android-products)。
:::
