...
1. 了解核心差异；创建并配置 Adapty 账号 _（5 分钟）_；
...
3. 为 Adapty 配置 [Apple App Store 服务器通知](enable-app-store-server-notifications)，以及（可选）[原始事件转发](enable-app-store-server-notifications#raw-events-forwarding) _（5 分钟）_；
4. 测试并发布应用更新 _（30 分钟）_；
5. （可选）使用 [RevenueCat 的定时数据导出](https://www.revenuecat.com/docs/integrations/scheduled-data-exports)将 RevenueCat 历史数据导出为 CSV 格式 _（5 分钟）_；
...
在集成 SDK 的版本发布后，请至少等待一周再进行历史数据导入。在此期间，我们将通过 SDK 收集所有购买价格信息，从而确保导入数据更加准确。
:::

按照 [RevenueCat 官方文档](https://www.revenuecat.com/docs/integrations/scheduled-data-exports) 中的说明，以 CSV 格式从 RevenueCat 导出历史数据。
...
### 哪些内容无法传达 \{#what-doesnt-come-across\}

- 导入时，每笔交易都会向应用商店重新验证，应用商店无法识别的记录将被丢弃。
- 没有真实应用商店交易 ID 的记录——例如 RevenueCat 的促销活动或手动授予的权益——将作为无交易的用户画像导入。如需恢复此类用户的访问权限，请通过[服务端 API](api-adapty/operations/grantAccessLevel) 为其授予访问等级。
- 退款和账单问题历史不会原样迁移：退款仅保留取消日期，订阅的账单问题状态以导入时的状态为准，而非历史日期。
