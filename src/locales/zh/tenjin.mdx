...
使用 `Adapty.updateExternalAttribution()` SDK 方法（在 SDK 4.1 版本之前名为 `updateAttribution()`）从 Tenjin 获取归因数据，并将其传递给 Adapty。
...
Adapty 会根据 [**Tenjin 集成页面**](https://app.adapty.io/integrations/tenjin) 上 **Events names** 部分的配置，将所选事件发送至 Tenjin。每个事件的结构如下：
...
| **参数**                      | **类型**         | **描述**                                                      |
| ----------------------------- | ---------------- | ------------------------------------------------------------ |
| **price**                     | Float            | 以标准货币单位计算的商品单价（例如，美元以美元报告）。 |
...
| **postcut**                   | String (Boolean) | 表示购买是否在平台抽成后发送。1 为 true，0 为 false。 |
...
| **ip_address**                | String (IPv4)    | 用户的 IP 地址，用于查询所在国家。          |
...
### 某些事件在事件流中显示为失败 \{#some-events-show-as-failed-in-the-event-feed\}

Tenjin 只接受[配置事件和标签](#configure-events-and-tags)中列出的事件。其他所有事件仍会在事件流中生成一条记录，但会标记为投递失败，并显示提示信息"The event type is not supported for integration."。其中包括 `access_level_updated`——你在 **Events names** 部分完全找不到它，因为它是[webhook 集成](webhook)专用的事件。