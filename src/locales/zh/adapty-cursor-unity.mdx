---
title: "借助 AI 工具将 Adapty 集成到 Unity 应用"
description: "使用 Cursor、Context7、ChatGPT、Claude 或其他 AI 工具将 Adapty 集成到 Unity 应用的分步指南。"
metadataTitle: "借助 AI 工具将 Adapty 集成到 Unity | Adapty 文档"
...
本指南帮助您借助 LLM 将 Adapty 集成到您的 Unity 应用中。您将从准备 Adapty 看板开始，然后通过向 LLM 发送相关文档链接，逐步完成各个实施阶段。最后，您还将了解如何为 AI 工具配置 Adapty 文档的最佳实践。
:::tip
复制整个页面的 Markdown 内容并粘贴到您的 LLM 中即可开始——点击页面顶部的 **Copy for LLM**，或打开 [.md 版本](https://adapty.io/docs/zh/adapty-cursor-unity.md)。LLM 将根据指南链接和检查点引导您完成每个阶段。
:::
...
如果您希望手动配置所有内容，以下是编写代码之前需要准备的内容。您的 LLM 无法自动查找看板中的值——您需要自行提供这些信息。
...
3. **至少创建一个产品**：在 Adapty 看板中，前往 **Products** 页面。您无需在代码中直接引用产品——Adapty 通过付费墙来提供产品。
   [添加产品](quickstart-products)
4. **创建付费墙和版位**：在 Adapty 看板中，在 **Paywalls** 页面创建付费墙，然后在 **Placements** 页面将其分配给一个版位。在代码中，版位 ID 是传递给 `Adapty.GetPaywall("YOUR_PLACEMENT_ID")` 的字符串。
   [创建付费墙](quickstart-paywalls)
5. **设置访问等级**：在 Adapty 看板中，在 **Products** 页面对每个产品进行配置。在代码中，检查的字符串为 `profile.AccessLevels["premium"]?.IsActive`。默认的 `premium` 访问等级适用于大多数应用。如果付费用户根据所购产品获得不同功能的访问权限（例如 `basic` 方案与 `pro` 方案），请在开始编码之前[创建额外的访问等级](assigning-access-level-to-a-product)。
:::tip
一旦您拥有所有这五项信息，就可以开始编写代码了。告诉您的 LLM："我的公共 SDK 密钥是 X，我的版位 ID 是 Y"，这样它就能生成正确的初始化和付费墙获取代码。
:::
...
:::warning
尽管 Context7 无需手动粘贴文档链接，但实现顺序至关重要。请按照以下[实现指南](#implementation-walkthrough)逐步操作，以确保一切正常运行。
:::
...
### 规划您的集成方案 \{#plan-your-integration\}

在开始编写代码之前，请让您的 LLM 分析您的项目并制定实施计划。如果您的 AI 工具支持规划模式（例如 Cursor 或 Claude Code 的 plan 模式），请使用该模式，以便 LLM 在编写任何代码之前，能够同时读取您的项目结构和 Adapty 文档。
...
通过 Unity Package Manager 添加 Adapty SDK 包，并使用您的 Public SDK key 激活它。这是基础步骤——没有它，其他一切都无法运行。
...
- **常见问题：** "Public API key is missing" → 请检查是否已将占位符替换为从 **App settings** 获取的真实密钥。
:::
### 展示付费墙并处理购买 \{#show-paywalls-and-handle-purchases\}

通过版位 ID 获取付费墙，展示付费墙，并处理购买事件。所需指南取决于您处理购买的方式。

在操作过程中，请随时在沙盒环境中测试每次购买——不要等到最后再测试。设置说明请参阅[在沙盒中测试购买](test-purchases-in-sandbox)。
...
将您的应用用户账户与 Adapty 用户画像关联，以便购买记录在不同设备间保持同步。
...
一旦您的集成在沙盒中正常工作，请参照发布检查清单，确保一切都已准备好用于生产环境。
...
```
:::tip[Checkpoint]
- **预期结果：** 所有清单项均已确认：商店连接、服务器通知、购买流程、访问等级检查以及隐私要求。
...
- [`llms.txt`](https://adapty.io/docs/zh/llms.txt)：列出所有页面的 `.md` 链接。这是一项[新兴标准](https://llmstxt.org/)，旨在让 LLM 能够访问网站内容。请注意，对于某些 AI 代理（例如 ChatGPT），您需要先下载 `llms.txt`，然后将其作为文件上传到对话中。
- [`llms-full.txt`](https://adapty.io/docs/zh/llms-full.txt)：将整个 Adapty 文档网站合并为单个文件。文件体积非常大——仅在需要完整内容时使用。
- Unity 专属文件 [`unity-llms.txt`](https://adapty.io/docs/zh/unity-llms.txt) 和 [`unity-llms-full.txt`](https://adapty.io/docs/zh/unity-llms-full.txt)：平台专属子集，与完整站点相比可节省 token 用量。