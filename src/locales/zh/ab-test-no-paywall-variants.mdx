...
:::important
如果你自行渲染付费墙（不使用 Adapty 渲染），请在展示流程/付费墙 A 时调用 [`logShowFlow` (iOS SDK v4+) / `logShowPaywall`](present-remote-config-paywalls#track-paywall-view-events)。否则，Adapty 无法统计测试中的展示次数。流程/付费墙 B 从不展示，因此无需记录其展示事件。
:::
