---
title: "迁移 Adapty Android SDK 至 v4.0"
description: "通过将付费墙 API 替换为流程 API 来迁移至 Adapty Android SDK v4.0，流程 API 不仅支持渲染流程，同时也兼容旧版编辑工具创建的付费墙。"
metadataTitle: "迁移至 Adapty Android SDK v4.0 | Adapty 文档"
---

Adapty Android SDK 4.0 引入了流程功能，并对付费墙 API 进行了相应的重命名。新 API 支持流程，同时也兼容旧版编辑工具创建的付费墙——无需在 Adapty 看板侧进行任何配置变更。
...
将 `adapty-bom` 版本设置为 `4.0.1`（或更高版本）并同步项目。BOM 会自动为你解析匹配的 `android-sdk` 和 `android-ui` 版本。依赖项声明请参阅[安装 Adapty SDK](sdk-installation-android)。
...
- **用户引导功能已弃用。** `AdaptyUI.getOnboardingView` 和 `AdaptyUI.getOnboardingConfiguration` 在 4.0 版本中已标记为 `@Deprecated`——请将用户引导迁移至在[流程与付费墙编辑工具](adapty-flow-builder)中构建的流程。
...
获取返回类型从 `AdaptyPaywall` 变更为 `AdaptyFlow`，配置加载器从 `AdaptyUI.getViewConfiguration` 更名为 `AdaptyUI.getFlowConfiguration`（返回 `AdaptyUI.FlowConfiguration` 而非 `AdaptyUI.LocalizedViewConfiguration`）。`locale` 参数从获取调用中移出，改为放在 `getFlowConfiguration` 上：`AdaptyFlow` 同时也暴露了 `paywalls`——即该流程的付费墙变体，以 `AdaptyFlowPaywall` 值的形式呈现。Web 付费墙方法接受其中一个值。
...
`locale` 在 `getFlowConfiguration` 中保持可选：省略它时，视图将以 `en` 渲染，或者当流程没有 `en` 时，以流程的默认本地化渲染。详见[本地化与语言区域代码](android-localizations-and-locale-codes)。
...
备用文件格式[在 SDK v4 中已更改](fallback-flows)。请从 **[Placements](https://app.adapty.io/placements)** > **Fallbacks** 下载新文件并将其打包到应用中。

## 跟踪流程浏览数据 \{#tracking-flow-views\}

### logShowPaywall → logShowFlow

`logShowPaywall` 已重命名为 `logShowFlow`，现在接受 `AdaptyFlow` 而非 `AdaptyPaywall`。事件仍会记录到相同的实验变体中，因此现有的漏斗和 A/B 测试数据图表无需更改看板配置即可继续正常使用。
...
:::note
SDK 4.1 将此方法重命名为 `updateExternalAttribution`，类型重命名为 `AdaptyExternalAttributionProvider`。旧名称在 4.1 中直接移除，不会提前标记为废弃。如果你直接迁移到 4.1，请参阅[迁移至 v4.1](migration-to-android-sdk-41)。
:::
