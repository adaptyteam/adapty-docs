...
description: "在 Flutter 上安装 Adapty SDK 的分步指南，适用于订阅类应用。"
...
:::tip
想看一个真实的 Adapty SDK 集成示例？查看我们的[示例应用](https://github.com/adaptyteam/AdaptySDK-Flutter/tree/master/example)，其中演示了完整的配置流程，包括展示付费墙、完成购买及其他基本功能。
:::

## 系统要求 \{#requirements\}

Adapty SDK 支持 iOS 13.0+，但使用付费墙编辑工具创建的付费墙需要 iOS 15.0+ 才能正常运行。

Adapty Flutter SDK 4.0——新增了对[流程](adapty-flow-builder)的支持——将最低版本要求提升至 **iOS 15.0+**、**Xcode 26+** 以及 **Flutter 3.32.0+**（Dart 3.8.0+）。具体安装方式请参阅下方的 [Adapty SDK 4.0](#adapty-sdk-40-swift-package-manager)。

:::info
从 3.17.0 版本起，Adapty Flutter SDK 默认使用 Billing Library v8。3.15.0 至 3.15.5 版本使用 v7，也支持在 Android 构建中其他依赖项将 Billing Library 升级至 v8 的情况。

与某个 Billing Library 版本兼容并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在使用新的 Google Play 计费功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
...
:::important
以下步骤安装的是最新稳定版 SDK（3.x）。如果你需要 v4 版本——[流程与付费墙编辑工具](adapty-flow-builder)所必需，也是[快速入门](flutter-quickstart-paywalls)所使用的版本——请参考下方的 [Adapty SDK 4.0: Swift Package Manager](#adapty-sdk-40-swift-package-manager)。
:::
...
将 Adapty Flutter SDK 4.0（新增对[流程](adapty-flow-builder)的支持）添加到你的 `pubspec.yaml`：
...
从 v4 开始，原生 iOS SDK 不再通过 CocoaPods 分发——插件仅通过 **Swift Package Manager** 拉取（[CocoaPods 的 spec 仓库将于 2026 年 12 月起设为只读](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。如果你使用的是 Flutter 3.32–3.43，请先一次性启用 Swift Package Manager 支持：
...
:::important
在调用任何其他 Adapty SDK 方法之前，请等待 `activate` 完成。完整调用顺序请参见 [Flutter SDK 中的调用顺序](flutter-sdk-call-order)。
:::

现在在应用中配置付费墙：

- 如果您使用 [Flow & Paywall Builder](adapty-flow-builder)，请先[激活 AdaptyUI 模块](#activate-adaptyui-module-of-adapty-sdk)（见下文），然后按照[快速入门指南](flutter-quickstart-paywalls)操作。
- 如果您自行构建付费墙 UI，请参见[自定义付费墙快速入门](flutter-quickstart-manual)。

## 激活 Adapty SDK 的 AdaptyUI 模块 \{#activate-adaptyui-module-of-adapty-sdk\}

如果你计划使用 [Flow & 付费墙编辑工具](adapty-flow-builder)，并已[安装 AdaptyUI 模块](sdk-installation-flutter#install-adapty-sdk)，还需要激活 AdaptyUI：
...
在你的 `AndroidManifest.xml` 中，确保启动购买流程的 Activity 的 `launchMode` 设置为 `standard` 或 `singleTop`：
...
:::note
此问题适用于 **SDK 3.x**，该版本的原生 iOS SDK 以 CocoaPods 方式安装。从 SDK 4.0 起，它们以 Swift 包的形式安装，因此在 `post_install` 中设置的 `SWIFT_VERSION` 覆盖不再对其生效。
:::
