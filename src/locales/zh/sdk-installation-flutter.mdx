...
Adapty Flutter SDK 4.0——新增对[流程](adapty-flow-builder)的支持——将最低要求提升至 **iOS 15.0+**、**Xcode 26+** 以及 **Flutter 3.32.0+**（Dart 3.8.0+）。安装细节请参阅下方的 [Adapty SDK 4.0](#adapty-sdk-40-swift-package-manager)。

:::info
Adapty SDK 内置了 Google Play Billing Library，无需在应用中单独添加。

从 3.17.0 版本起，Adapty Flutter SDK 默认使用 Billing Library v8。3.15.0 至 3.15.5 版本使用 v7，同时也兼容 Android 构建中其他依赖将 Billing Library 升级至 v8 的情况。

兼容某个 Billing Library 版本，并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在使用新的 Google Play 计费功能之前，请参阅[在 Play Store 中配置产品](android-products)。
:::
