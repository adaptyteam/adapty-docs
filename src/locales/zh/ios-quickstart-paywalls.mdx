---
title: "在 iOS SDK 中通过 Flow 与付费墙编辑工具启用购买功能"
description: "通过 Adapty Flow 与付费墙编辑工具启用应用内购买的快速入门指南。"
...
- [**产品**](product) – 用户可以购买的所有内容（订阅、消耗型商品、永久授权）
- [**流程**](adapty-flow-builder) – 通过无代码 Flow & Paywall 编辑工具构建的屏幕序列，用于向用户展示产品，SDK 通过 `getFlow` 获取。如果你更倾向于用自己的代码构建 UI，请改用付费墙 — 详见[手动实现付费墙](ios-quickstart-manual)。
- [**版位**](placements) – 在应用中展示流程的位置和时机（如 `main`、`onboarding`、`settings`）。在看板中将流程绑定到版位后，即可在代码中通过版位 ID 请求。这样可以轻松运行 A/B 测试，向不同用户展示不同流程。

Adapty 为您提供三种在应用中启用购买的方式，请根据应用需求选择其中一种：

| 实现方式 | 复杂度 | 适用场景 |
|---|---|---|
| Adapty Flow 与付费墙编辑工具 | ✅ 简单 | 你[在无代码编辑器中创建完整的、可直接购买的流程](quickstart-paywalls)。Adapty 自动渲染界面，并在后台处理所有复杂的购买流程、收据验证和订阅管理。 |
| 手动创建付费墙 | 🟡 中等 | 你在应用代码中自行实现付费墙 UI，但仍从 Adapty 获取流程对象，以保持产品配置的灵活性。请参阅[指南](ios-quickstart-manual)。 |
| 观察者模式 | 🔴 复杂 | 你已有自己的购买处理基础设施，并希望继续沿用。请注意，观察者模式在 Adapty 中存在一定限制。请参阅[文章](observer-vs-full-mode)。 |

:::important
**以下步骤展示了如何实现在 Adapty Flow & Paywall Builder 中创建的流程。**

如果您希望自行构建付费墙 UI，请参阅[手动实现付费墙](ios-quickstart-manual)。
:::

要在应用代码中展示在 Adapty Flow & Paywall Builder 中创建的流程，您只需：

1. **获取流程**：从 Adapty 获取流程。
2. **展示流程，让 Adapty 处理购买**：在应用中显示对应的视图。
3. **处理按钮事件**：将用户交互与应用的响应逻辑关联起来。例如，当用户点击按钮时，打开链接或关闭流程。
...
3. [创建流程](paywall-builder-templates)并[为其添加产品](paywall-product-block)。
4. [创建版位并将流程添加到其中](create-placement)。
5. 在你的应用代码中[安装并激活 Adapty SDK](sdk-installation-ios)。本指南使用 Adapty iOS SDK v4 API。

## 1. 获取流程 \{#1-get-the-flow\}

你的流程与在看板中配置的版位相关联。版位让你可以针对不同的目标受众展示不同的流程，或运行 [A/B 测试](ab-tests)。

要获取在 Adapty Flow & Paywall Builder 中创建的流程，你需要：

1. 通过 [版位](placements) ID 使用 `getFlow` 方法获取 `flow` 对象，并检查它是否包含视图配置。
2. 使用 `getFlowConfiguration` 方法获取视图配置。它包含展示流程所需的 UI 元素和样式。
