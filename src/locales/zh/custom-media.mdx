...
:::tip
若要让图片或视频铺满整个屏幕（包括刘海和 Home 指示器后面的区域），请将其设为固定定位，所有偏移值设为 0，并勾选 **Ignore safe area**。详见[元素定位](manage-paywall-ui-elements#ignore-safe-area)。
:::
## 图片 \{#image\}
...
上传不超过 50 MB、时长不超过 30 秒的 `.MP4` 或 `.WEBM` 文件。

- **Aspect** — Fit、Fill 或 Cover，默认为 Fill。
- **Loop** — 循环播放视频，默认开启。
- **Use custom media ID** — 详见下方[自定义媒体 ID](#custom-media-id)。

视频在编辑器预览中不会播放，画布仅显示静止帧。在设备上运行时，视频默认静音播放。开启 Loop 后，视频将无限循环。
