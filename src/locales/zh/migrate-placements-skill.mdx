...
然后运行 `/reload-plugins` 激活技能。如果使用其他 AI 编程工具，请参考 [flow-generator 的安装步骤](flow-generator-skill#install-the-skill) —— 同一个仓库，同样的命令。Codex 的插件安装方式与 Claude Code 相同；其他工具则是将技能文件复制到指定位置，因此如需获取上次安装后新增的技能，请重新执行相应命令。

然后请求它——在将斜线命令映射到技能的工具中输入 `/migrate-placements`，或者在不支持的工具中输入 "Use the migrate-placements skill"。
