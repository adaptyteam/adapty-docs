---
title: "在流程与付费墙编辑工具中添加文字和列表并设置样式"
description: "在 Adapty 的流程与付费墙编辑工具中添加并设置标题、副标题、段落和列表的样式，为用户打造符合品牌调性的体验。"
metadataTitle: "文字 | 流程与付费墙 | Adapty 文档"
---

只需单击即可添加标题、段落或列表，设置样式以匹配品牌风格，并使用[动态变量](onboarding-variables)为每位用户个性化内容。

<AIPromo title="一键重写标题文案。">
  让 AI 编辑器帮你重写文案、突出免费试用，或一次性精简页面上的所有文本元素。
</AIPromo>
...
3. 为 [**Open URL**](onboarding-actions#open-url) 操作设置目标地址。
    ...
    :::important
    空的 Open URL 操作会[阻止预览和发布](flow-common-issues#a-flow-wont-preview-or-publish)。
    :::
...
:::tip
要编辑其他语言的条件文本，请在编辑器底部切换当前语言环境。请先在 **Localizations** 面板中添加语言——详见[在 Flow 与付费墙编辑工具中添加语言环境](add-paywall-locale-in-adapty-paywall-builder)。
:::
