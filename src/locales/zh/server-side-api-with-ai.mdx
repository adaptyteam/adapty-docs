...
- **频率限制**：按 API 密钥执行，且按应用分配，因此没有统一的数值适用于所有应用——详见[频率限制](https://adapty.io/docs/zh/ss-authorization.md#rate-limits)。这是一个持续速率限制，而非每分钟预算，某些端点的上限更低：[createVirtualCurrencyTransaction](https://adapty.io/docs/zh/api-adapty/operations/createVirtualCurrencyTransaction.md) 默认每个应用每分钟允许 600 次请求。请告知您的 agent 在收到 `429` 时进行退避重试，并在响应中包含 `Retry-After` 时遵守该值。
- **应用专属密钥**：每个密钥仅适用于一个应用，请为每个应用使用对应的密钥。
- **必须提供一个标识符**：每个请求都需要提供 `adapty-customer-user-id` 或 `adapty-profile-id`。
