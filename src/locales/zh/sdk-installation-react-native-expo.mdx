...
构建 iOS 需要 **Swift 6.0** 或更高版本。[Kids Mode](kids-mode-react-native) 需要 **Swift 6.1** 或更高版本。

:::info
Adapty React Native SDK 4.0.3 及更高版本支持 Google Play Billing Library v8。

兼容某个 Billing Library 版本并不意味着 Adapty 支持 Google 在该版本中引入的所有功能。在使用新的 Google Play 结算功能之前，请先查阅 [Play Store 中的产品](android-products)。
:::
...
:::important
从 v4 版本开始，Adapty React Native SDK 不再支持通过 CocoaPods 安装其原生依赖项。如果你需要 v4 或更高版本（用于 [流程与付费墙编辑工具](adapty-flow-builder)），请参阅下方的 [Adapty SDK 4.0：启用 Swift Package Manager](#adapty-sdk-40-enable-swift-package-manager)。
:::
...
v4 通过 Swift Package Manager 而非 CocoaPods 子依赖来拉取原生 iOS SDK（`Adapty`、`AdaptyUI`、`AdaptyPlugin`）（[CocoaPods 的 spec 仓库将于 2026 年 12 月变为只读](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。SPM 需要动态框架，在 Expo 中可通过 [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) 插件来启用。将其添加到 `app.json`（或 `app.config.js`）：
...
如果 iOS 构建失败并提示 `'React/RCTBridge.h' file not found`，请参阅[故障排除条目](#react-header-not-found)。

有关完整迁移步骤，请参阅[将 Adapty React Native SDK 迁移至 v4](migration-to-react-native-sdk-v4)。
...
:::important
此设置仅满足 Adapty、AppsFlyer 和 expo-secure-store 的备份需求。
如果项目中的其他库定义了自定义备份规则，则需要手动配置。
:::

#### iOS 构建失败，报错 `'React/RCTBridge.h' file not found` \{#react-header-not-found\}

在[为 Adapty SDK 4.0 启用动态框架](#adapty-sdk-40-enable-swift-package-manager)后，iOS 构建可能在 `expo-updates`、`@expo/ui` 或其他包含 Objective-C 源码的包中失败，报错如下：
