...
description: "在 Expo 项目中安装 Adapty React Native SDK 的分步指南，适用于基于订阅的应用。"
...
如果你需要一个完整的教程，了解如何在 React Native 应用中实现应用内购买，请查看[这篇文章](https://adapty.io/blog/react-native-in-app-purchases-tutorial/)。
...
- [Expo Go & Web 示例](https://github.com/adaptyteam/AdaptySDK-React-Native/tree/master/examples/ExpoGoWebMock)，用于模拟模式下的测试
:::
...
| 要求 | 版本 |
|---|---|
| React Native | 0.75 或更高版本 |
| iOS | 15.0 或更高版本 |
| Swift | 6.2 或更高版本，随 Xcode 26 捆绑提供 |

:::info
Adapty SDK 内置了 Google Play 结算库，无需在应用中单独添加。

从 3.17.0 版本起，Adapty React Native SDK 默认使用结算库 v8。3.15.0 至 3.15.6 版本使用 v7，但如果 Android 构建中的其他依赖将结算库升级到 v8，这些版本同样可以正常使用。

与某个 Billing Library 版本兼容，并不意味着 Adapty 支持该版本引入的所有 Google 新功能。在使用新的 Google Play 计费功能之前，请参阅 [Play Store 中的产品](android-products)。
:::
...
:::important
Adapty React Native SDK v4 及更高版本通过 Swift Package 分发其原生 iOS SDK（`Adapty`、`AdaptyUI`、`AdaptyPlugin`）。CocoaPods 仍可在 Expo 项目中安装这些包，但 Swift Package 采用动态链接，因此您的构建需要启用动态框架——详见[为 Adapty SDK v4 启用动态框架](#enable-dynamic-frameworks)。
:::
...
:::important
[Expo Dev Client](https://docs.expo.dev/versions/latest/sdk/dev-client/)（自定义开发构建版本）是在 Expo 项目中使用 Adapty 的必要条件。
...
### 为 Adapty SDK v4 启用动态框架 \{#enable-dynamic-frameworks\}

React Native SDK 4.0（新增对[流程](adapty-flow-builder)的支持）需要 **React Native 0.75 或更高版本**。安装 SDK：

```sh
npx expo install react-native-adapty@^4.0.0
```

v4 将原生 iOS SDK（`Adapty`、`AdaptyUI`、`AdaptyPlugin`）以 Swift 包的形式引入，而非 CocoaPods 子依赖（[CocoaPods 的 spec 仓库将于 2026 年 12 月进入只读模式](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。Swift 包采用动态链接，在 Expo 中可通过 [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) 插件启用。将其添加到 `app.json`（或 `app.config.js`）中：
...
如果 iOS 构建时出现 `'React/RCTBridge.h' file not found` 错误，请参阅[故障排查条目](#react-header-not-found)。

完整迁移步骤请参阅 [Adapty React Native SDK 迁移至 v4](migration-to-react-native-sdk-v4)。
...
- 如果您使用[流程与付费墙编辑工具](adapty-flow-builder)，请参阅[快速入门指南](react-native-quickstart-paywalls)。
- 如果您自行构建付费墙 UI，请参阅[自定义付费墙快速入门](react-native-quickstart-manual)。

:::tip
为避免开发环境中出现激活错误，请参阅[相关技巧](#development-environment-tips)。
:::

## 激活 Adapty SDK 的 AdaptyUI 模块 \{#activate-adaptyui-module-of-adapty-sdk\}

如果你计划使用 [Flow 与付费墙编辑工具](adapty-flow-builder)，则需要 AdaptyUI 模块。当你激活核心模块时，它会自动激活，无需额外操作。
...
- Adapty 的原生模块**完全被绕过**——即使 Xcode/Android 构建中缺少原生 SDK 文件或 API key 无效，也不会触发错误。
...
您可以在激活时通过 `mockConfig` 自定义模拟数据。配置格式和支持的参数请参阅[此处](https://react-native.adapty.io/interfaces/adaptymockconfig)。
...
:::important
此设置仅涵盖 Adapty、AppsFlyer 和 expo-secure-store 的备份要求。
如果您的项目中有其他库定义了自定义备份规则，则需要手动配置这些规则。
:::

#### iOS 构建失败，报错 `'React/RCTBridge.h' file not found` \{#react-header-not-found\}

在[为 Adapty SDK v4 启用动态框架](#enable-dynamic-frameworks)后，iOS 构建可能在 `expo-updates`、`@expo/ui` 或其他包含 Objective-C 源文件的包中失败，报错如下：
