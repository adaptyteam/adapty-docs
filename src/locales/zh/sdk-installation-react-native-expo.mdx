...
`buildReactNativeFromSource` 在 **Expo SDK 57 及更高版本**上是必需的。Expo SDK 57 附带了一个预构建的 React Native 框架，一旦框架为动态链接，其他包便无法访问其头文件，导致 iOS 构建失败，报错如 `expo-updates` 或 `@expo/ui` 中的 `'React/RCTBridge.h' file not found`。从源码构建 React Native 可以避免此冲突，但代价是 iOS 构建时间更长。在 Expo SDK 56 及更早版本中，可以省略此选项。
...
有关完整迁移说明，请参阅[将 Adapty React Native SDK 迁移至 v4](migration-to-react-native-sdk-v4)。
