...
:::important
从 v4 开始，Adapty React Native SDK 不再支持通过 CocoaPods 安装其原生依赖项。如果你需要 v4 或更高版本（用于 [Flow Builder](adapty-flow-builder)），请参阅下方的 [Adapty SDK 4.0 (beta)：启用 Swift Package Manager](#adapty-sdk-40-beta-enable-swift-package-manager)。
:::
...
React Native SDK 4.0 — 新增 [Flow Builder](adapty-flow-builder) 支持 — 目前处于预发布阶段，需要 **React Native 0.75 或更高版本**。请安装精确版本号（npm 无法通过 `^`/`~` 范围解析预发布版本）：

```sh
npx expo install react-native-adapty@4.0.0-beta.1
```
v4 通过 Swift Package Manager 而非 CocoaPods 子依赖来引入原生 iOS SDK（`Adapty`、`AdaptyUI`、`AdaptyPlugin`）（[CocoaPods 的 spec 仓库将于 2026 年 12 月变为只读](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。SPM 需要动态框架，在 Expo 中可通过 [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) 插件启用。将其添加到 `app.json`（或 `app.config.js`）中：
