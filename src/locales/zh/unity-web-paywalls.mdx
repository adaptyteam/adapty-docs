...
1. `OpenWebPaywall(product)` 根据付费墙生成 URL，并将产品数据一并添加到 URL 中。
2. `OpenWebPaywall(paywall)` 根据付费墙生成 URL，但不添加产品数据到 URL 中。当 Adapty 付费墙中的产品与 Web 付费墙中的产品不同时，请使用此版本。

在 SDK v4 中，`paywall` 参数接受 `AdaptyFlowPaywall` 类型——即 `flow.Paywalls` 中的某个付费墙变体。详见[迁移指南](migration-to-unity-sdk-v4)。
:::
