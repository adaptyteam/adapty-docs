...
本指南将带你一步步将 Adapty 集成到 Android 应用中，借助 AI 编程工具完成整个流程——只需按正确顺序将相关 Adapty 文档喂给它即可。
...
3. **至少创建一个产品**：在 Adapty 看板中，进入 **Products** 页面。你无需在代码中直接引用产品——Adapty 会通过付费墙将其推送给用户。
   [添加产品](quickstart-products)

4. **创建流程或付费墙，并配置版位**：在 Adapty 看板中，创建一个流程（或者如果你打算自己构建 UI，则创建付费墙），然后在 **Placements** 页面将其分配到某个版位。在代码中，版位 ID 就是你传给 `Adapty.getFlow("YOUR_PLACEMENT_ID")` 的字符串。
...
这些内容不是开始编写代码的必要条件，但随着集成的深入，你会需要用到它们：
...
- [**手动创建付费墙**](android-making-purchases)：自行用代码构建付费墙 UI，仍通过 Adapty 获取产品并处理购买。
- [**观察者模式**](observer-vs-full-mode)：保留现有的购买基础设施，仅使用 Adapty 进行数据分析和集成。

不确定选哪个？请参阅[快速入门中的对比表](android-quickstart-paywalls)。
...
通过版位 ID 获取付费墙，将其展示给用户，并处理购买事件。具体需要参考哪些指南，取决于你的购买处理方式。

建议边开发边在沙盒中测试每笔购买，不要等到最后再测。设置方法请参阅[在沙盒中测试购买](test-purchases-in-sandbox)。
...
**指南：**
- [使用 Flow 与付费墙编辑工具启用购买（快速入门）](android-quickstart-paywalls)
- [获取流程及其配置](android-get-pb-paywalls)
- [展示流程](android-present-paywalls)
- [处理流程事件](android-handling-events)
...
:::tip[Checkpoint]
- **预期结果：** 付费墙显示已配置的产品，点击产品触发沙盒购买对话框。
- **注意事项：** 流程为空或出现 `getFlow` 错误 → 检查版位 ID 是否与看板完全一致，以及该版位是否已分配目标受众。
:::
