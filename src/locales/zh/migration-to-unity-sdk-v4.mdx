...
4.x 版本引入了流程功能，并相应地重命名了付费墙相关 API。新 API 同时兼容新版 Flow Builder 和现有的付费墙编辑工具，Adapty 看板端无需做任何配置变更。此外，4.1 版本还对外部归因 API 进行了重命名，将 Adapty Attribution 改为按需启用，新增了一个必需的监听器方法，并更改了备用文件格式。
...
| Adapty 归因自动启用 | 默认禁用——通过 `Builder.SetAdaptyAttributionEnabled(true)` 启用 |
| 备用文件适用于 3.x 版本 | 新备用文件格式——请重新下载文件 |
| `Adapty.SetFallbackPaywalls(...)`（v3 中已弃用） | 已移除——请使用 `Adapty.SetFallback(fileName, ...)` |
| `Builder.SetIDFACollectionDisabled(...)`（v3 中已弃用） | 已移除——请使用 `Builder.SetAppleIDFACollectionDisabled(...)` |
...
:::note
`CustomTimers` 仍然存在，但它只影响旧版付费墙编辑工具的付费墙。流程的[倒计时器](flow-timer)按照流程编辑工具中设置的行为运行，因此流程会忽略你在此传入的任何内容。
:::
...
从 SDK 4.1 版本开始，用于传入外部归因数据提供商（Adjust、AppsFlyer、Branch、Tenjin 或自定义提供商）归因数据的 API 已重命名，以与原生 SDK 保持一致。此次重命名没有提供弃用别名，因此所有现有调用处在重命名前将无法通过编译：
...
只有名称发生了变化。provider 仍然是和之前相同的字符串（`"adjust"`、`"appsflyer"`、`"branch"`、`"tenjin"`、`"apple_search_ads"`、`"custom"`），归因数据仍然以字典或 JSON 字符串的形式传入，profile 属性仍然是 `IReadOnlyList<string>`：
...
旧版用户引导 API 在 v4 中已弃用，请改用 [Flow Builder](adapty-flow-builder)。该 API 目前仍可使用，但将在未来版本中移除，请提前将你的用户引导迁移至 Flow Builder。

已弃用的符号：`GetOnboarding`、`GetOnboardingForDefaultAudience`、`AdaptyUI.CreateOnboardingView`、`AdaptyUI.PresentOnboardingView`、`AdaptyUI.DismissOnboardingView` 和 `Adapty.SetOnboardingsEventsListener`。
