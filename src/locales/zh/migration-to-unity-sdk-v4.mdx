...
| `Adapty.SetFallbackPaywalls(...)` （v3 中已废弃） | 已移除——请改用 `Adapty.SetFallback(fileName, ...)` |
| `Builder.SetIDFACollectionDisabled(...)` （v3 中已废弃） | 已移除——请改用 `Builder.SetAppleIDFACollectionDisabled(...)` |
...
v4.0 是预发布版本，安装时请固定确切的 beta 标签。通过 Unity Package Manager 安装时，请使用带有标签的 Git URL，而非 `#upm`：

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

如果通过 Unity 包安装，请从 [Releases](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Releases) 文件夹下载 `adapty-unity-plugin-4.0.0-beta.1.unitypackage`。完整配置步骤请参阅 [安装 Adapty SDK](sdk-installation-unity#install-adapty-sdk)。
...
|---|---|---|
| `RemoteConfig`（单个，可空） | `RemoteConfigs`（列表） | 一个流程为每种已配置的语言携带一个远程配置。从 `flow.RemoteConfigs` 中读取与当前用户匹配的那一项。`flow.RemoteConfig` 快捷方式返回第一条记录。 |
| _（新增）_ | `Paywalls`（`AdaptyFlowPaywall` 列表） | 每一项代表流程中的一个付费墙变体，包含各自的 `Name`、`VariationId` 和 `ProductIdentifiers`。Web 付费墙方法接受 `AdaptyFlowPaywall` 参数——详见[Web 付费墙方法](#web-paywall-methods)。 |
| `ProductIdentifiers`、`VendorProductIds` | 保留 | 在 `AdaptyFlow` 上，这些属性聚合了所有付费墙变体的产品。每个变体也会暴露自己的 `ProductIdentifiers` 和 `VendorProductIds`。获取产品时，继续调用 `GetPaywallProducts(flow)` 即可。 |
| `HasViewConfiguration` | 已移除 | 请从代码中移除所有 `HasViewConfiguration` 检查——`CreateFlowView` 会直接返回错误（详见[展示流程](#displaying-flows)）。 |
...
- `Adapty.SetSystemRequestsHandler(...)` 配合 `IAdaptyUISystemRequestsHandler` — 用于处理流程中的系统请求：操作系统权限提示（`FlowViewDidAskPermission`）和应用评价请求（`FlowViewDidRequestAppReview`）。目前流程尚未触发此类请求，因此无需注册处理器。
- `AdaptyUICreateFlowViewParameters.Locale` — 使用指定的[编辑工具本地化](add-paywall-locale-in-adapty-paywall-builder)语言渲染流程或付费墙，而非 Adapty 根据设备自动解析的语言。流程在视图创建时完成本地化，因此这是选择本地化语言的唯一时机，创建的视图会通过 `view.Locale` 上报其构建时所用的本地化语言。详见[使用本地化语言和区域代码](unity-localizations-and-locale-codes)。
- `IAdaptyFlowsEventsListener` 上新增的 `FlowViewDidReceiveAnalyticEvent` 回调，用于接收流程中的自定义分析事件。目前流程尚未向代码发送此类事件，因此实现时保持方法体为空即可。
- `AdaptyUI.OpenUrl(url, openIn, ...)` 和 `AdaptyUI.RequestAppReview(...)` — 分别负责处理 `open_url` 操作和应用评价请求的原生逻辑。在 `FlowViewDidPerformAction` 中调用 `OpenUrl` 可保留默认的 URL 跳转行为；`RequestAppReview` 负责弹出默认的应用评价提示，但流程目前尚未触发该功能。
