...
:::tip
以下规则要求 `adapty.activate()` 和 `adapty.identify()` 均已完成。详见 [Capacitor SDK 调用顺序](capacitor-sdk-call-order)。
:::

以下建议使用 v3 方法名。在 SDK v4 中，`getPaywall` 已重命名为 `getFlow`（参见[迁移指南](migration-to-capacitor-sdk-v4)）——所有规则同样适用。
