---
title: "Flow 与付费墙编辑工具中的输入框与表单"
description: "添加文本框、复选框等交互式表单元素。"
metadataTitle: "输入框与表单 | 流程与付费墙 | Adapty 文档"
---

使用输入框收集用户填写的数据，例如姓名、电子邮件地址或出生日期。保存用户的回答，并在流程的其他位置引用，例如在后续页面上以姓名称呼用户。
...
:::tip
你的应用也可以接收输入值。要收集这些值，请[处理 SDK 的分析回调](ios-flow-input)。每当用户离开一个输入框时，回调会传递该输入框的值，你的应用可以在内部进行处理。
:::

每个输入框都会自动作为变量使用——无需任何设置，也不需要配置 **On Submit** 动作。该值通过输入框的 **Element ID** 引用，你可以在 **Input Settings** 中设置该 ID。

要在流程中的其他地方使用输入值（例如，个性化文案、填充其他字段或驱动条件导航），请插入一个变量并选择：

**Element > Screen > `<elementId>.value`**

:::link
参阅以下相关指南，了解如何使用已保存的输入值：
