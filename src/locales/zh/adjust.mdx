---
title: "Adjust"
description: "将 Adjust 与 Adapty 集成，实现更好的订阅跟踪与分析。"
...
[Adjust](https://www.adjust.com/) 是领先的移动归因平台（MMP）之一，用于收集和呈现营销活动数据，帮助公司追踪其广告投放效果。
...
:::warning
Adapty 会将 [Stripe 和 Paddle 购买](#web-purchases-stripe-and-paddle) 通过买家安装您应用所在平台的 App Token 发送给 Adjust。如果您发布了 Android 应用并在网页端销售，请填写 Android **App Token**。若未填写，Android 买家的网页购买将失败，并提示 [Missing integration credentials](handle-integration-errors#missing-integration-credentials)。
:::
...
### 将您的应用连接到 Adjust \{#connect-your-app-to-adjust\}

完成上述步骤后，将以下两个方法添加到您的应用中。它们将建立您的应用与 Adjust 之间的通信：

1. **向 Adjust 发送订阅数据**：将 Adjust 设备 ID 传递给 `setIntegrationIdentifier()` SDK 方法
2. **从 Adjust 接收归因数据**：使用 `updateExternalAttribution()` SDK 方法更新归因数据（在 SDK 4.1 版本之前该方法名为 `updateAttribution()`）

如果使用 Adjust 5.0 或更高版本，请参考以下示例：
...
Adjust 会接收 [Stripe](stripe) 和 [Paddle](paddle) 的购买记录，与 App Store 和 Google Play 的购买记录一同处理。Web 购买发生在应用之外，因此本身没有移动平台属性。Adapty 会使用买家的平台信息代替。当其 Adapty 用户画像是通过 App Store 或 Google Play 创建时，Web 购买会以该商店的来源发送给 Adjust：

| 买家的用户画像存储 | Adapty 随购买发送的内容 |
|:----------------------|:------------------------------------|
| App Store             | 您的 iOS **App Token** 和 **OAuth Token**，以及买家的 `idfa` 和 `idfv`。 |
| Google Play           | 您的 Android **App Token** 和 **OAuth Token**，以及买家的 `android_id` 和 `gps_adid`。 |

购买事件会与安装时的同一设备关联发送至 Adjust，因此 Adjust 可以将其归因到引入该用户的广告系列。

Adjust 集成页面没有单独的 Stripe 或 Paddle 字段。Web 端购买会复用 [Connect Adapty to Adjust](#connect-adapty-to-adjust) 中配置的 iOS 和 Android 凭据。

从未打开过移动端应用的买家没有 Adjust 设备 ID，其用户画像是在 Stripe 或 Paddle 上创建的。Adapty 会使用您的 iOS **App Token** 将其购买事件发送给 Adjust，但不携带任何广告 ID。因此，Adjust 无法通过任何标识符将该购买与某次安装关联起来。
