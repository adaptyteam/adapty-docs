---
title: "借助 AI 将 Adapty 集成到你的 React Native 应用"
...
本指南将引导你逐步将 Adapty 集成到 React Native 应用中，借助 AI 编程工具完成——你只需按正确顺序提供相应的 Adapty 文档即可。如需全自动流程，请使用 [adapty-sdk-integration skill](adapty-sdk-integration-skill-react-native)。
...
如果你更倾向于手动配置，以下是编写代码前需要准备的内容。LLM 无法自动获取看板中的值，需要你手动提供。

1. **连接应用商店**：在 Adapty 看板中，前往 **App settings → General**，将 App Store 和 Google Play 都连接好（如果你的应用同时面向两个平台）。这是购买功能正常运行的必要条件。
   [连接应用商店](integrate-payments)
2. **复制您的公共 SDK 密钥**：在 Adapty 看板中，进入 **App settings → General**，找到 **API keys** 部分。在代码中，这就是您传入 `adapty.activate("YOUR_PUBLIC_SDK_KEY")` 的字符串。

3. **至少创建一个产品**：在 Adapty 看板中，进入 **Products** 页面。您无需在代码中直接引用产品——Adapty 会通过付费墙将产品下发给用户。
   [添加产品](quickstart-products)
4. **创建付费墙和版位**：在 Adapty 看板中，先在 **Paywalls** 页面创建付费墙，再在 **Placements** 页面将其分配到版位。在代码中，版位 ID 就是传入 `adapty.getPaywall("YOUR_PLACEMENT_ID")` 的字符串。
   [创建付费墙](quickstart-paywalls)
5. **设置访问等级**：在 Adapty 看板的 **Products** 页面中，为每个产品配置访问等级。在代码中，通过 `profile.accessLevels['premium']?.isActive` 检查对应字符串。默认的 `premium` 访问等级适用于大多数应用。如果付费用户根据所购产品获得不同功能的访问权限（例如 `basic` 方案与 `pro` 方案），请在开始编写代码前[创建额外的访问等级](assigning-access-level-to-a-product)。
:::tip
一旦集齐这五项信息，就可以开始写代码了。告诉你的 LLM："我的 Public SDK key 是 X，我的版位 ID 是 Y"，它就能生成正确的初始化和付费墙获取代码。
:::
...
:::warning
尽管 Context7 无需手动粘贴文档链接，但实现顺序很重要。请按照下方的[实现步骤](#implementation-walkthrough)逐步操作，确保一切正常运行。
:::
...
在开始写代码之前，先让 LLM 分析你的项目并制定实施计划。如果你使用的 AI 工具支持规划模式（如 Cursor 或 Claude Code 的 plan mode），建议先启用该模式，让 LLM 在编写代码前同时读取你的项目结构和 Adapty 文档。

告诉 LLM 你采用哪种购买集成方式——这决定了它应该参考哪些指南：
- [**Adapty 付费墙编辑工具**](adapty-paywall-builder)：在 Adapty 的无代码编辑工具中创建付费墙，SDK 会自动渲染。
- [**手动创建付费墙**](react-native-making-purchases)：用代码构建自己的付费墙 UI，但仍使用 Adapty 获取产品并处理购买。
- [**观察者模式**](observer-vs-full-mode)：保留现有的购买基础设施，仅使用 Adapty 进行数据分析和集成。

不确定选哪种？请查看[快速入门中的对比表格](react-native-quickstart-paywalls)。
### 安装并配置 SDK \{#install-and-configure-the-sdk\}

使用 npm（或 yarn）添加 Adapty SDK 依赖，并用你的 Public SDK key 激活它。这是一切功能的基础——没有它，其他任何功能都无法使用。

我们为 Expo 和纯 React Native 项目分别提供了独立的安装指南——请选择适合你项目的那一份。

**指南：**
- [使用 Expo 安装](sdk-installation-react-native-expo)
- [使用纯 React Native 安装](sdk-installation-react-native-pure)
:::tip[Checkpoint]
- **预期结果：** 应用能在 iOS 和 Android 上成功构建并运行，Metro bundler 日志中显示 Adapty 激活日志。
...
每完成一步都建议在沙盒中测试购买，不要等到最后再测。设置说明请参阅[在沙盒中测试购买](test-purchases-in-sandbox)。

<Tabs groupId="paywall-approach">

<TabItem value="builder" label="付费墙编辑工具" default>
**指南：**
- [使用付费墙开启购买（快速入门）](react-native-quickstart-paywalls)
- [获取付费墙编辑工具的付费墙及其配置](react-native-get-pb-paywalls)
...
```
:::tip[检查点]
- **预期效果：** 付费墙正常显示，且包含你配置的产品。点击某个产品后会触发沙盒购买弹窗。
...
```
:::tip[检查点]
- **预期效果：** 您的自定义付费墙已显示从 Adapty 获取的产品。点击某个产品会触发沙盒购买弹窗。
- **常见问题：** 产品数组为空 → 请确认该付费墙在看板中已分配产品，且版位已设置目标受众。
:::
...
- [在 Observer 模式下上报交易](report-transactions-observer-mode-react-native)
将此内容发送给你的 LLM：
```
Read these Adapty docs before writing code:
...
- **注意事项：** 没有事件 → 请确认你已向 Adapty 上报交易，并已为两个应用商店配置服务器通知。
:::
...
将应用的用户账号与 Adapty 用户画像关联，确保购买记录在多设备间保持同步。
...
沙盒环境中的集成测试通过后，请按照发布检查清单逐项确认，确保一切就绪可投入生产。
...
```
:::tip[Checkpoint]
- **预期：** 所有检查项均已确认：商店连接、服务器通知、购买流程、访问等级检查，以及隐私要求。
...
- [`llms.txt`](https://adapty.io/docs/zh/llms.txt)：列出所有页面的 `.md` 链接。这是一项[新兴标准](https://llmstxt.org/)，旨在让网站对大语言模型（LLM）更易访问。请注意，某些 AI 助手（如 ChatGPT）需要先下载 `llms.txt` 文件，再上传到对话中才能使用。
- [`llms-full.txt`](https://adapty.io/docs/zh/llms-full.txt)：将整个 Adapty 文档站合并为单个文件。内容较大，仅在需要完整文档时使用。
- React Native 专用版本 [`react-native-llms.txt`](https://adapty.io/docs/zh/react-native-llms.txt) 和 [`react-native-llms-full.txt`](https://adapty.io/docs/zh/react-native-llms-full.txt)：平台专属子集，相比完整站点可节省 token 用量。