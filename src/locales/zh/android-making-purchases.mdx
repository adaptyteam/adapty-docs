...
在移动应用中展示付费墙是向用户提供高级内容或服务访问权限的重要步骤。不过，只有在 Adapty 负责渲染界面时——即使用[流程](adapty-flow-builder)或[旧版付费墙编辑工具](adapty-paywall-builder)构建的付费墙——展示付费墙才能独立完成购买。

如果你在自己的代码中渲染界面，则需要调用一个名为 `.makePurchase()` 的独立方法来完成购买并解锁所需内容。该方法是用户与付费墙交互并完成目标交易的核心入口。

如果你的付费墙针对用户正在购买的产品设置了有效的促销活动，Adapty 会在购买时自动应用该优惠。

:::warning
请注意，只有在 Adapty 渲染页面时，新用户优惠才会被自动应用。
...
**需要分步骤的操作指引？** 请查阅[快速入门指南](android-implement-paywalls-manually)，其中提供了包含完整上下文的端到端实现说明。
:::
...
:::warning
仅在用户从同一 Google 账号下已激活的订阅进行切换时，才传入 `subscriptionUpdateParams`。若是全新购买，请在调用 `makePurchase` 时不要传该参数——传入此参数会导致购买失败，并抛出 `CURRENT_SUBSCRIPTION_TO_UPDATE_NOT_FOUND_IN_HISTORY` 错误，该错误信息具有误导性，会指向非活跃订阅或其他 Google 账号。
:::
