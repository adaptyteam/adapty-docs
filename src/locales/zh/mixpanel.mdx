---
title: "Mixpanel"
description: "将 Mixpanel 与 Adapty 连接，获得强大的订阅分析能力。"
...
[Mixpanel](https://mixpanel.com/home/) 是一款功能强大的产品分析服务。其基于事件驱动的追踪方案，帮助产品团队深入了解不同平台上最佳的用户获取、转化和留存策略，从而获得有价值的洞察。
...
2. 启用开关并输入你的 **Mixpanel Token**。Adapty 为每个平台分别维护 **Production** 和 **Sandbox** 令牌，支持的平台包括：iOS、Android、Stripe 和 Paddle。只需填写你希望接收数据的平台——未填写令牌的平台不会发送任何事件。
3. 将 **Mixpanel Data Residency** 设置为与你的 Mixpanel 项目匹配的值。此字段为必填项，默认值为 **US**。若使用 `api.mixpanel.com` 端点，请选择 **US**；若使用 `api-eu.mixpanel.com`，请选择 **Europe**。
