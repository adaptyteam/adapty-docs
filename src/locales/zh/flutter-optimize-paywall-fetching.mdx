...
:::tip
以下规则假设 `Adapty().activate()` 和 `Adapty().identify()` 已完成。请参阅 [Flutter SDK 的调用顺序](flutter-sdk-call-order)。
:::

以下建议使用 v3 方法名称。在 SDK v4 中，`getPaywall` 已重命名为 `getFlow`，获取策略类型更名为 `AdaptyFlowFetchPolicy`——所有规则均适用，不受影响。
