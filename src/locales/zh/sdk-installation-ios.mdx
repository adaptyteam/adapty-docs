---
title: "安装并配置 iOS SDK"
description: "在 iOS 上为订阅应用安装 Adapty SDK 的分步指南。"
...
- **AdaptyUI**：如果你使用 [Adapty 付费墙编辑工具](adapty-paywall-builder)（一款无需编写代码即可轻松创建跨平台付费墙的工具），则需要集成此可选模块。

:::tip
想看 Adapty SDK 集成到移动应用的真实案例？请查看我们的[示例应用](https://github.com/adaptyteam/AdaptySDK-iOS/tree/master/Examples)，其中涵盖完整的集成配置，包括展示付费墙、发起购买及其他基础功能。
:::
对于完整的实现演示，您也可以观看以下视频：
...
Adapty SDK 通过 Swift Package Manager 安装。在 Xcode 中，依次点击 **File** -> **Add Package Dependency...**。请注意，不同版本的 Xcode 添加包依赖的步骤可能略有差异，如有需要请参阅 Xcode 文档。
1. 输入仓库 URL：
   ```
   https://github.com/adaptyteam/AdaptySDK-iOS.git
   ```
2. 选择版本（建议选择最新稳定版），然后点击 **Add Package**。
3. 在 **Choose Package Products** 窗口中，选择所需模块：
   ...
   注意：
     ...
     - 不要从列表中选择其他软件包——您不会用到它们。
   :::
4. 点击 **Add Package** 完成安装。
5. **验证安装：** 在项目导航器中，您应该能在 **Package Dependencies** 下看到"Adapty"（以及"AdaptyUI"，如果已选择的话）。
:::important
Adapty iOS SDK 4.0 目前为预发布版本。Swift Package Manager 无法通过 **Up to Next Major Version**（`from:`）规则解析 beta 版本，因此你必须固定确切的版本号。在 Xcode 中，将 **Dependency Rule** 设置为 **Exact Version** 并输入 `4.0.0-beta.1`。在 `Package.swift` 中，使用 `.exact("4.0.0-beta.1")`。详见 [将 Adapty iOS SDK 迁移至 v4](migration-to-ios-sdk-v4)。
:::
...
- 如果你使用 [Adapty 付费墙编辑工具](adapty-paywall-builder)，请先[激活 AdaptyUI 模块](#activate-adaptyui-module-of-adapty-sdk)，再按照[付费墙编辑工具快速入门](ios-quickstart-paywalls)操作。
...
// 如果只使用 AppDelegate，请将以下代码放在
// application(_:didFinishLaunchingWithOptions:) 方法中。

// 如果使用 SceneDelegate，请将以下代码放在
...
Task {
   do {
      let configurationBuilder = AdaptyConfiguration
         ...
         .with(logLevel: .verbose) // 开发阶段推荐开启
...
如需将 Adapty 的日志消息发送到你自己的系统或保存到文件，请使用 `setLogHandler` 方法并在其中实现自定义日志逻辑。该处理器会接收包含消息内容和严重级别的日志记录。
```swift showLineNumbers title="Swift"
...
除非您明确发送，否则 Adapty 不会存储用户的个人数据。同时，您也可以实施额外的数据安全策略，以符合应用商店或所在国家/地区的相关规定。

#### 禁用 IDFA 收集与共享 \{#disable-idfa-collection-and-sharing\}

在激活 Adapty 模块时，将 `idfaCollectionDisabled` 设置为 `true` 即可禁用 IDFA 的收集与共享。
使用此参数可遵守 App Store 审核指南，或在应用不需要 IDFA 时避免触发 App Tracking Transparency 提示。默认值为 `false`。有关 IDFA 收集的更多详情，请参阅[分析集成](analytics-integration#disable-collection-of-advertising-identifiers)章节。
...
请注意，AdaptyUI 配置是可选的。你可以在不传入配置的情况下激活 AdaptyUI 模块。但如果你使用了配置，则所有参数均为必填项。
```swift showLineNumbers title="Swift"
...
| 参数 | 是否必填 | 描述 |
| :-------------------------- | :------- | :----------------------------------------------------------- |
| memoryStorageTotalCostLimit | 必填 | 存储空间的总费用上限（字节）。 |
| memoryStorageCountLimit | 必填 | 内存存储的条目数量上限。 |
| diskStorageSizeLimit | 必填 | 存储空间的磁盘文件大小上限（字节）。0 表示不限制。 |
...
默认情况下，Adapty 会在成功验证后自动完成交易。但如果你需要进行高级交易验证（例如服务端收据验证、欺诈检测或自定义业务逻辑），可以将 SDK 配置为手动完成交易模式。
```swift showLineNumbers title="Swift"
let configurationBuilder = AdaptyConfiguration
    .builder(withAPIKey: "YOUR_PUBLIC_SDK_KEY")
...
:::note
仅删除本地 SDK 缓存。与 Apple 的交易记录以及 Adapty 服务器上的用户数据不受影响。
:::
```swift showLineNumbers
let configurationBuilder = AdaptyConfiguration
    .builder(withAPIKey: "YOUR_PUBLIC_SDK_KEY")
...
使用 [Tuist](https://tuist.dev/) 构建时，可能会遇到 Swift 6 严格并发编译错误。常见表现包括 `AdaptyUIBuilderLogic` 中出现 `@Sendable` 属性不匹配，或类似的跨模块 Sendability 错误。
这是因为 Tuist 在从 SPM 包生成 Xcode 项目时，不会保留 `swift-tools-version: 6.0` 设置。因此，部分 Adapty 目标（`Adapty`、`AdaptyUI`、`AdaptyUIBuilder`）会以 Swift 5 规则编译，而其他目标则使用 Swift 6，导致跨模块的 `@Sendable` 不匹配。

**修复方案**：升级到 Adapty SDK **3.15.5** 或更高版本，无论 Swift 语言版本是否混用，该版本均可解决此问题。

**临时方案**：如果暂时无法升级，请在 Tuist 配置中为所有三个 Adapty 目标显式指定 Swift 6：
```swift showLineNumbers
targetSettings: [
