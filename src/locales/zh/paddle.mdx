...
:::important
要使您的 Paddle 事件与集成正常配合，您的用户必须至少通过 App Store/Google Play 账户登录过一次应用。
:::

完成 Paddle 集成后，Adapty 即可立即提供数据洞察。为了充分利用您的 Paddle 数据，您可以设置额外的 Adapty 集成来转发 Paddle 事件，将所有订阅分析数据汇总到统一的 Adapty 看板中。
...
Amplitude、Firebase / Google Analytics 和 Mixpanel 会为每个平台单独保存凭证。请在集成页面填写 **Paddle** 字段——Paddle 事件不使用您的 iOS 或 Android 凭证。
