...
完成 [AdServices 框架设置](#connect-adapty-to-the-adservices-framework)后，Adapty 会自动开始收集 Apple Search Ad 归因数据，无需添加任何 SDK 代码。

对于 iOS 应用，此归因数据**始终**优先于其他来源的数据。如果不需要此行为，请按照以下说明*禁用* ASA 归因。
