...
2. 粘贴带有占位符（作为查询参数）的托管付费墙 URL。Adapty Mail 会为每位收件人替换对应的值，以便结账页面识别访客。示例：

   ```
   https://example.com/paywall?email={email}&adapty_profile_id={adapty.external_profile_id}
   ```

   创建页面列出了所有可用的占位符。请参阅[设置结账](mail-checkout#use-your-own-hosted-paywall)，了解每个占位符的具体含义。
3. 保存并发布。
...
- **[测试你的流程](mail-testing)**：在正式上线前，让测试用户完整跑一遍整个链路。
- **[收集用户邮箱](mail-collect-emails)**：如果你的应用尚未收集用户邮箱，这里提供一些获取邮箱覆盖率的策略。
- **[设置发件域名](mail-sending-domain)**：DNS 记录详情、预热层级及故障排查。
