...
description: "在 React Native 中为订阅类应用安装 Adapty SDK 的分步指南。"
rank: 60
---
...
:::important
从 v4 开始，Adapty React Native SDK 不再支持通过 CocoaPods 安装其原生依赖项。如果您需要 v4 或更高版本（用于 [Flow Builder](adapty-flow-builder)），请参阅下方的 [Adapty SDK 4.0（测试版）：启用 Swift Package Manager](#adapty-sdk-40-beta-enable-swift-package-manager)。
:::
...
### Adapty SDK 4.0（测试版）：启用 Swift Package Manager \{#adapty-sdk-40-beta-enable-swift-package-manager\}

React Native SDK 4.0 — 新增了 [Flow Builder](adapty-flow-builder) 支持 — 目前为预发布版本，需要 **React Native 0.75 或更高版本**。请安装指定版本（npm 无法通过 caret/tilde 范围解析预发布版本）：
...
```
v4 通过 Swift Package Manager 而非 CocoaPods 子依赖来拉取原生 iOS SDK（`Adapty`、`AdaptyUI`、`AdaptyPlugin`）（[CocoaPods 的 spec 仓库将于 2026 年 12 月停止写入](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。SPM 需要动态框架——在 `ios/Podfile` 的 target 中添加以下内容，然后重新安装 pods：
...
:::warning
从默认的静态链接切换到动态 framework 可能会与尚不支持模块化头文件的库产生冲突，且与 Flipper 不兼容。详情请参阅 [将 Adapty React Native SDK 迁移至 v4](migration-to-react-native-sdk-v4)。
:::
