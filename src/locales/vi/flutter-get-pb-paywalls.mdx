...
| **locale** | tùy chọn | Mã định danh của [bản dịch flow](add-paywall-locale-in-adapty-paywall-builder) để render view — ví dụ: `en` hoặc `pt-br`. Nếu bỏ qua, view sẽ render bằng `en`, hoặc bằng ngôn ngữ mặc định của flow khi flow không có `en`. Xem [Bản dịch và mã ngôn ngữ](flutter-localizations-and-locale-codes). |
| **customTags** | tùy chọn | Định nghĩa một map các custom tag và giá trị tương ứng. Custom tag đóng vai trò là placeholder trong nội dung, được thay thế động bằng các chuỗi cụ thể để cá nhân hóa nội dung trong flow/paywall. Tham khảo chủ đề [Custom tag trong paywall builder](custom-tags-in-paywall-builder) để biết thêm chi tiết. |
