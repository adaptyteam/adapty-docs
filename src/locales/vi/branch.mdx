---
title: "Branch"
description: "Tích hợp Branch với Adapty để theo dõi deep link và chuyển đổi ứng dụng."
...
[Branch](https://www.branch.io/) giúp khách hàng tiếp cận, tương tác và đánh giá kết quả trên nhiều thiết bị, kênh và nền tảng khác nhau. Đây là một nền tảng thân thiện với người dùng, được thiết kế để tăng doanh thu di động thông qua các liên kết chuyên biệt hoạt động liền mạch trên mọi thiết bị, kênh và nền tảng.

Adapty cung cấp đầy đủ dữ liệu giúp bạn theo dõi [các sự kiện gói đăng ký](events) từ các cửa hàng tại một nơi duy nhất. Với Adapty, bạn có thể dễ dàng nắm bắt hành vi của người dùng, hiểu sở thích của họ, và sử dụng thông tin đó để giao tiếp theo cách có mục tiêu và hiệu quả.

Sự tích hợp giữa Adapty và Branch hoạt động theo hai hướng chính.
...
2. **Gửi sự kiện gói đăng ký đến Branch**  
   Adapty có thể gửi tất cả các sự kiện gói đăng ký đã được cấu hình trong phần tích hợp của bạn đến Branch. Nhờ đó, bạn có thể theo dõi các sự kiện này trong Branch dashboard.
...
2. Sử dụng phương thức `.updateExternalAttribution()` (tên cũ là `.updateAttribution()` trước phiên bản SDK 4.1) để lưu dữ liệu attribution. Nếu bạn chưa chỉ định Branch user ID ở bước trước, hãy truyền nó vào tham số `networkUserId` tại đây.
...
| `user_data.idfa`               | String | **Chỉ iOS**. ID for Advertisers.                                                                                                |
| `user_data.idfv`               | String | **Chỉ iOS**. ID for Vendors.                                                                                                    |
| `user_data.aaid`               | String | **Chỉ Android**. Google Advertising ID.                                                                                         |
| `event_data`                   | Object | Chỉ số sự kiện tiêu chuẩn (chỉ có với PURCHASE và các sự kiện tương tự).                                                       |
| `event_data.transaction_id`    | String | ID giao dịch của cửa hàng.                                                                                                      |
| `event_data.revenue`           | Float  | Số tiền doanh thu.                                                                                                              |
| `event_data.currency`          | String | Mã tiền tệ (ví dụ: "USD").                                                                                                      |
| `custom_data`                  | Object | Thuộc tính chi tiết của sự kiện (chứa tất cả các [trường sự kiện](webhook-event-types-and-fields#for-most-event-types) có sẵn). |
