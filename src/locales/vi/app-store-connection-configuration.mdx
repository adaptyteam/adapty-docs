...
description: "Cấu hình kết nối App Store của bạn để theo dõi gói đăng ký một cách liền mạch."
...
Phần này mô tả cách thiết lập kết nối giữa App Store và Adapty cho ứng dụng iOS của bạn. Đây là yêu cầu bắt buộc để chúng tôi có thể hiển thị analytics về gói đăng ký và xác thực các giao dịch mua. Bạn có thể hoàn tất tích hợp trong quá trình onboarding ban đầu hoặc sau đó trong **App Settings** trên Adapty Dashboard.

Mỗi thông tin xác thực trên trang này mở khóa một phần khác nhau của quá trình trao đổi dữ liệu. [Cách Adapty đồng bộ dữ liệu với các cửa hàng](store-sync#what-adapty-reads-from-the-stores) cho thấy mỗi thông tin xác thực mang dữ liệu gì.

Mặc dù bạn có thể đã cấu hình tích hợp giữa ứng dụng di động và Adapty trong quá trình onboarding, bạn vẫn có thể chỉnh sửa các cài đặt này sau đó trong **App settings**.

:::danger
Các thay đổi cấu hình có thể được thực hiện an toàn trong giai đoạn Sandbox, cho đến khi ứng dụng di động của bạn ra mắt với Adapty SDK được cài đặt. Các thay đổi sau khi phát hành có thể làm hỏng luồng mua hàng trong ứng dụng của bạn.

Nếu một key bị thu hồi, xóa, hoặc thay thế trong App Store Connect, hãy tải key mới lên ngay khi bạn phát hiện: mọi thứ phụ thuộc vào key đó sẽ bị gián đoạn cho đến khi bạn thực hiện. Xem [Nếu cập nhật ngừng hoạt động](store-sync#if-updates-stop-arriving).
:::
...
:::note
App Store Connect có hai loại key mà cả hai đều có trường được gán nhãn **Issuer ID** và **Key ID**. Các bước 2–3 sử dụng key **In-App Purchase** từ tab **Integrations** → **In-App Purchase**. Key App Store Connect API từ tab **Team keys** là một key khác với các ID riêng — bạn sẽ thêm nó ở [Bước 6](#step-6-add-app-store-connect-api-key).
:::
...
:::note
Đây là cặp key khác với In-App Purchase key bạn đã thêm ở [Bước 2–3](#step-2-provide-issuer-id-and-key-id). Hãy tạo key trong tab **Team keys** và sao chép **Issuer ID** cũng như **Key ID** từ đó — các giá trị của In-App Purchase key sẽ không dùng được ở đây.
:::

1. Trong App Store Connect, truy cập [**Users and Access > Integrations > Team keys**](https://appstoreconnect.apple.com/access/integrations/api) và nhấn **+**.
...
2. Trong cửa sổ **Generate API key**, nhập tên cho key và cấp quyền truy cập **Admin**.
