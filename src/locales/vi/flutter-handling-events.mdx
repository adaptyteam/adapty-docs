...
:::important
Hướng dẫn này đề cập đến việc xử lý các sự kiện mua hàng, khôi phục, chọn sản phẩm và hiển thị. Việc đóng màn hình và mở liên kết được xử lý bởi triển khai mặc định của `flowViewDidPerformAction` — xem [hướng dẫn xử lý hành động nút](flutter-handle-paywall-actions) để ghi đè chúng hoặc xử lý các hành động nút tùy chỉnh.
:::
...
Xem [Theo dõi lượt xem màn hình flow](flutter-flow-screen-views) để biết cách xử lý chúng.

Flow cũng báo cáo các giá trị mà người dùng nhập và chọn. Xem [Xử lý dữ liệu từ flow](flutter-flow-input) để đọc chúng.
...
Hướng dẫn này chỉ áp dụng cho **các paywall được xây dựng bằng builder cũ**, yêu cầu Adapty SDK v3.0 trở lên.
