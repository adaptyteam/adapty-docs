...
Việc xây dựng ứng dụng iOS với Adapty SDK v4 (beta) yêu cầu **Xcode 26** trở lên — SDK iOS gốc mà nó sử dụng được xây dựng bằng Swift tools 6.2. Các yêu cầu về iOS 15.0+, Capacitor 8 và Android minSdk 24 giống như SDK 3.16+.

:::info
Adapty Capacitor SDK 4.0.2 trở lên hoạt động với Google Play Billing Library v8.

Khả năng tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google giới thiệu trong đó. Trước khi áp dụng một tính năng thanh toán mới của Google Play, hãy xem [Sản phẩm trên Play Store](android-products).
:::
