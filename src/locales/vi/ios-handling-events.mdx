...
| **flowConfiguration**  | bắt buộc | Một đối tượng `AdaptyUI.FlowConfiguration` chứa thông tin hiển thị của flow hoặc paywall. Tham khảo [Lấy flow và paywall](get-pb-paywalls) để biết thêm chi tiết.                                                                                       |
| **didFinishPurchase**  | bắt buộc | Được gọi khi `Adapty.makePurchase()` hoàn tất thành công. Flow không tự động đóng — đặt binding hiển thị của bạn thành `false` tại đây, hoặc không làm gì để flow tiếp tục sau khi mua hàng.                                                                                     |
...
| **didReceiveError**    | bắt buộc | Được gọi khi flow gặp lỗi hiển thị hoặc lỗi runtime từ script của flow (ví dụ: ngoại lệ JavaScript, mã `AdaptyUIError` `4105`). Trong trường hợp lỗi hiển thị, hãy [liên hệ Adapty Support](mailto:support@adapty.io).            |
...
| **didPerformAction**   | tùy chọn | Được gọi khi người dùng nhấn vào một nút. Hai action ID được định nghĩa sẵn là `close` và `openURL`; các ID khác là tùy chỉnh và có thể được thiết lập trong builder.                                                                                                        |
...
Giao thức khai báo 13 phương thức. Bốn phương thức không có cài đặt mặc định và bắt buộc phải triển khai khi conform: `didFinishPurchase`, `didFailPurchase`, `didFinishRestoreWith`, và `didFailRestoreWith`. Các phương thức còn lại có cài đặt mặc định no-op và có thể override khi bạn muốn tùy chỉnh hành vi. Các phương thức được nhóm theo mục đích dưới đây.
...
`didFinishPurchase` và `didFailPurchase` không có cài đặt mặc định và bắt buộc phải được triển khai. Controller sẽ không tự đóng sau khi mua hàng thành công — hãy gọi `controller.dismiss(animated:)` khi phù hợp, hoặc không làm gì để cho phép flow nhiều màn hình tiếp tục sau khi mua hàng.
...
Được gọi sau khi điều hướng thanh toán web hoàn tất, dù thành công hay thất bại.
...
Xem [Theo dõi lượt xem màn hình flow](ios-flow-screen-views) để biết cách xử lý chúng.

Flow cũng báo cáo các giá trị mà người dùng nhập và chọn. Xem [Xử lý dữ liệu từ flow](ios-flow-input) để đọc chúng.
...
Paywall được cấu hình bằng [Paywall Builder](adapty-paywall-builder) không cần thêm code để thực hiện và khôi phục giao dịch mua hàng. Tuy nhiên, chúng tạo ra một số sự kiện mà ứng dụng của bạn có thể phản hồi. Các sự kiện đó bao gồm thao tác nhấn nút (nút đóng, URL, lựa chọn sản phẩm, v.v.) cũng như thông báo về các hành động liên quan đến giao dịch mua hàng được thực hiện trên paywall. Tìm hiểu cách phản hồi các sự kiện này bên dưới.

Hướng dẫn này chỉ dành cho **paywall Paywall Builder mới** yêu cầu Adapty SDK v3.0 trở lên.

:::tip

Bạn muốn xem ví dụ thực tế về cách tích hợp Adapty SDK vào ứng dụng di động? Hãy tham khảo [ứng dụng mẫu](sample-apps) của chúng tôi, nơi minh họa toàn bộ quá trình thiết lập, bao gồm hiển thị paywall, thực hiện mua hàng và các chức năng cơ bản khác.
...
Để kiểm soát hoặc theo dõi các tiến trình diễn ra trên màn hình paywall trong ứng dụng của bạn, hãy triển khai các phương thức `AdaptyPaywallControllerDelegate`.
...
Điều này sẽ không được gọi trong chế độ Observer. Tham khảo chủ đề [iOS - Hiển thị paywall Paywall Builder trong chế độ Observer](ios-present-paywall-builder-paywalls-in-observer-mode) để biết thêm chi tiết.
