...
:::important
Nếu bạn tự render paywall (không dùng Adapty render), hãy gọi [`logShowFlow` (iOS SDK v4+) / `logShowPaywall`](present-remote-config-paywalls#track-paywall-view-events) khi hiển thị Flow/Paywall A. Nếu không, Adapty sẽ không thể đếm lượt xem trong A/B test. Không cần log lượt xem cho Flow/Paywall B vì nó không bao giờ được hiển thị.
:::
