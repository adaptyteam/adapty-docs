...
description: "Hướng dẫn từng bước để tích hợp Adapty vào ứng dụng Unity của bạn sử dụng Cursor, Context7, ChatGPT, Claude, hoặc các công cụ AI khác."
metadataTitle: "Tích hợp Adapty với sự hỗ trợ AI cho Unity | Tài liệu Adapty"
customSlug: /adapty-cursor-unity
displayed_sidebar: sdkunity
---

Hướng dẫn này sẽ dẫn bạn từng bước tích hợp Adapty vào ứng dụng Unity của mình bằng công cụ AI — bạn chỉ cần cung cấp đúng tài liệu Adapty theo đúng thứ tự.
...
4. **Tạo flow hoặc paywall và một placement**: Trong Adapty Dashboard, tạo một flow (hoặc paywall nếu bạn tự xây dựng giao diện), sau đó gán nó vào một placement trên trang **Placements**. Trong code, placement ID là chuỗi bạn truyền vào `Adapty.GetFlow("YOUR_PLACEMENT_ID")`.
...
Những mục này không bắt buộc để bắt đầu code, nhưng bạn sẽ cần chúng khi integration trưởng thành hơn:
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Bạn tạo flow trong builder no-code của Adapty, và SDK tự động render chúng.
- [**Paywall tự tạo**](unity-making-purchases): Bạn tự xây dựng giao diện paywall bằng code nhưng vẫn dùng Adapty để lấy sản phẩm và xử lý giao dịch mua.
- [**Observer mode**](observer-vs-full-mode): Bạn giữ nguyên hệ thống mua hàng hiện có và chỉ dùng Adapty cho analytics và tích hợp.
...
Hãy kiểm tra từng giao dịch mua trong sandbox ngay khi thực hiện — đừng chờ đến cuối. Xem [Kiểm tra giao dịch mua trong sandbox](test-purchases-in-sandbox) để biết hướng dẫn thiết lập.
...
**Hướng dẫn:**
- [Kích hoạt mua hàng với Flow & Paywall Builder (quickstart)](unity-quickstart-paywalls)
- [Lấy flow và cấu hình của chúng](unity-get-pb-paywalls)
- [Hiển thị flow](unity-present-paywalls)
- [Xử lý sự kiện flow](unity-handling-events)
- [Phản hồi các hành động nút](unity-handle-paywall-actions)
...
- **Gotcha:** Flow trống hoặc lỗi `GetFlow` → kiểm tra xem placement ID có khớp chính xác với dashboard không và placement đã được gán đối tượng chưa.
:::

</TabItem>

<TabItem value="manual" label="Paywall thủ công">
