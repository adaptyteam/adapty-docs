...
description: "Tải flows và paywalls từ Adapty trong ứng dụng React Native của bạn."
...
Lưu ý rằng chủ đề này đề cập đến các flow và paywall được thiết kế trong các builder của Adapty. Nếu bạn đang triển khai paywall theo cách thủ công, vui lòng tham khảo chủ đề [Lấy paywalls và sản phẩm cho remote config paywalls trong ứng dụng di động](fetch-paywalls-and-products-react-native).
...
- **SDK đã được cài đặt**: Xem [hướng dẫn cài đặt React Native SDK](sdk-installation-reactnative).
...
| **fetchPolicy**   | mặc định: `'reload_revalidating_cache_data'` | <p>`fetchPolicy` xác định SDK đọc từ lớp nào trước, chứ không phải liệu nó có thể sử dụng cache hay không. Theo mặc định, SDK sẽ truy vấn máy chủ trước và trả về dữ liệu đã cache nếu yêu cầu đó thất bại. Chúng tôi khuyến nghị tùy chọn này vì nó đảm bảo người dùng của bạn luôn nhận được dữ liệu mới nhất.</p><p></p><p>Tuy nhiên, nếu bạn cho rằng người dùng của mình thường xuyên gặp kết nối internet không ổn định, hãy cân nhắc sử dụng `'return_cache_data_else_load'`, đảo ngược thứ tự đó — đọc cache trước và chỉ truy vấn máy chủ khi không có gì trong cache. Người dùng có thể không nhận được dữ liệu mới nhất tuyệt đối, nhưng thời gian tải sẽ nhanh hơn, bất kể kết nối internet của họ có chập chờn đến đâu. Cache được cập nhật thường xuyên, vì vậy hoàn toàn an toàn khi sử dụng trong phiên để tránh các yêu cầu mạng.</p><p></p><p>Chính sách thứ ba, `'return_cache_data_if_not_expired_else_load'`, nằm giữa hai chính sách trên: đọc cache trước khi bản sao trong cache còn trẻ hơn giá trị `maxAgeSeconds` bạn truyền vào cùng với nó, và truy vấn máy chủ khi cache đã cũ hơn.</p><p></p><p>Lưu ý rằng cache vẫn còn nguyên sau khi khởi động lại ứng dụng và chỉ bị xóa khi ứng dụng được cài đặt lại hoặc thông qua việc dọn dẹp thủ công.</p><p></p><p>Adapty SDK lưu trữ paywall cục bộ theo hai lớp: cache được cập nhật thường xuyên như mô tả ở trên và [paywall dự phòng](fallback-paywalls). Chúng tôi cũng sử dụng CDN để tải paywall nhanh hơn và một máy chủ dự phòng độc lập trong trường hợp CDN không thể truy cập được. Hệ thống này được thiết kế để đảm bảo bạn luôn nhận được phiên bản mới nhất của paywall trong khi vẫn đảm bảo độ tin cậy ngay cả khi kết nối internet hạn chế.</p> |
| **loadTimeoutMs** | mặc định: 5 giây | <p>Giá trị này giới hạn thời gian chờ cho phương thức này. Nếu hết thời gian chờ, dữ liệu đã cache hoặc fallback cục bộ sẽ được trả về.</p><p>Lưu ý rằng trong một số trường hợp hiếm gặp, phương thức này có thể hết thời gian chờ muộn hơn một chút so với giá trị được chỉ định trong `loadTimeout`, vì thao tác có thể bao gồm nhiều yêu cầu khác nhau bên dưới.</p><p>Đối với Android: Bạn có thể tạo `TimeInterval` bằng các extension function (như `5.seconds`, trong đó `.seconds` đến từ `import com.adapty.utils.seconds`), hoặc `TimeInterval.seconds(5)`. Để không giới hạn thời gian chờ, hãy dùng `TimeInterval.INFINITE`.</p> |

## Tham số phản hồi \{#response-parameters\}

| Tham số | Mô tả |
| :-------- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Flow   | Một đối tượng `AdaptyFlow` chứa các định danh của flow (`id`, `variationId`), tên, placement, các biến thể paywall (`paywalls`), Remote Config (`remoteConfigs`) và — từ SDK 4.1 trở lên — cờ `hasViewConfiguration`. |

## Lấy cấu hình view \{#fetch-the-view-configuration\}

:::important
Hãy đảm bảo [xuất bản flow](builder-save-publish). Một flow có chỉnh sửa chưa được xuất bản sẽ có trạng thái **Dirty**, và placement của nó vẫn tiếp tục phục vụ phiên bản đã xuất bản gần nhất.
:::

Nếu placement được thiết kế trong **Flow & Paywall Builder** hoặc **old Paywall Builder**, Adapty sẽ tự render giao diện cho bạn. Tạo view bằng `createFlowView`, sau đó [hiển thị flow hoặc paywall](react-native-present-paywalls). Nếu placement là paywall tùy chỉnh không có Builder UI, hãy [xử lý nó như một Remote Config paywall](present-remote-config-paywalls-react-native).

Từ SDK 4.1 trở đi, flag `hasViewConfiguration` trên flow giúp phân biệt hai loại trước khi bạn tạo view:
...
Trên phiên bản 4.0, flag này không còn nữa, và `createFlowView` sẽ ném ra `AdaptyError` nếu flow không có cấu hình view.

Trong React Native SDK, hãy gọi `createFlowView` trực tiếp — bạn không cần fetch cấu hình view trước.

:::warning
Kết quả của phương thức `createFlowView` chỉ có thể được sử dụng một lần. Nếu bạn cần sử dụng lại, hãy gọi lại phương thức `createFlowView`. Gọi hai lần mà không tạo lại có thể dẫn đến lỗi `AdaptyUIError.viewAlreadyPresented`.
:::
...
| **locale**           | tùy chọn | Mã định danh của [bản địa hóa flow](add-paywall-locale-in-adapty-paywall-builder) dùng để hiển thị giao diện — ví dụ: `en` hoặc `pt-br`. Nếu bỏ qua, giao diện sẽ hiển thị bằng `en`, hoặc theo bản địa hóa mặc định của flow nếu flow không có `en`. Yêu cầu SDK 4.0.2 trở lên. Xem [Bản địa hóa và mã locale](react-native-localizations-and-locale-codes). |
| **customLayoutId**   | <p>tùy chọn</p><p>mặc định: `undefined`</p><p>SDK 4.1+</p> | Dành riêng cho tính năng của Flow & Paywall Builder chưa được phát hành. Cho đến khi builder có thể định nghĩa layout ID, mọi giá trị bạn truyền vào đều không khớp với layout nào và `createFlowView` sẽ ném ra lỗi `AdaptyError`. Hãy để trống. |
| **customTags**       | tùy chọn | Từ điển các custom tag và giá trị tương ứng, dùng làm placeholder trong nội dung. Custom tag chỉ áp dụng cho [paywall dùng builder cũ](custom-tags-in-paywall-builder) — flow dùng [biến](onboarding-variables) thay thế. |
| **prefetchProducts** | tùy chọn | Bật để tối ưu thời điểm hiển thị sản phẩm trên màn hình. Khi là `true`, AdaptyUI sẽ tự động tải các sản phẩm cần thiết. Mặc định: `false`. |
| **android.enableSafeArea** | tùy chọn | Chỉ dành cho Android (bị bỏ qua trên iOS). Truyền dưới dạng object lồng nhau: `android: { enableSafeArea: true }`. Khi là `true`, flow view sẽ áp dụng padding safe-area. Mặc định là `true` cho kiểu hiển thị modal (`createFlowView` + `present()`) và `false` cho component `AdaptyFlowView` nhúng trực tiếp. Giá trị mặc định phù hợp với hầu hết các trường hợp. |
...
| **fetchPolicy** | mặc định: `'reload_revalidating_cache_data'` | <p>`fetchPolicy` xác định lớp nào SDK đọc trước, chứ không phải việc có thể dùng cache hay không. Theo mặc định, SDK sẽ gọi lên server trước và trả về dữ liệu đã cache nếu yêu cầu đó thất bại. Chúng tôi khuyên dùng tùy chọn này vì nó đảm bảo người dùng luôn nhận được dữ liệu mới nhất.</p><p></p><p>Tuy nhiên, nếu bạn cho rằng người dùng của mình thường gặp vấn đề với kết nối internet không ổn định, hãy cân nhắc dùng `'return_cache_data_else_load'` — tùy chọn này đảo ngược thứ tự đó: đọc cache trước và chỉ gọi server khi không có gì trong cache. Người dùng có thể không nhận được dữ liệu mới tuyệt đối nhất, nhưng thời gian tải sẽ nhanh hơn, bất kể kết nối internet của họ có chập chờn đến đâu. Cache được cập nhật thường xuyên nên hoàn toàn an toàn khi dùng trong phiên để tránh các yêu cầu mạng không cần thiết.</p><p></p><p>Chính sách thứ ba, `'return_cache_data_if_not_expired_else_load'`, nằm giữa hai tùy chọn trên: nó đọc cache trước khi bản cache còn trẻ hơn giá trị `maxAgeSeconds` bạn truyền vào, và gọi server khi cache đã cũ hơn giá trị đó.</p><p></p><p>Lưu ý rằng cache vẫn còn nguyên sau khi khởi động lại ứng dụng và chỉ bị xóa khi gỡ cài đặt ứng dụng hoặc thực hiện dọn dẹp thủ công.</p> |
...
Sau khi [bạn đã thiết kế phần giao diện cho paywall của mình](adapty-paywall-builder) bằng Paywall Builder cũ trong Adapty Dashboard, bạn có thể hiển thị nó trong ứng dụng di động. Bước đầu tiên trong quá trình này là lấy paywall được liên kết với placement và cấu hình view của nó như mô tả bên dưới.

:::warning
Các paywall được xây dựng trong Paywall Builder của SDK 3.x yêu cầu React Native SDK phiên bản 3.0 trở lên.
:::
...
| **locale**        | <p>tùy chọn</p><p>mặc định: `en`</p> | <p>Định danh của bản địa hóa paywall. Tham số này phải là mã ngôn ngữ gồm một hoặc hai subtag được phân tách bằng ký tự dấu trừ (**-**). Subtag đầu tiên là mã ngôn ngữ, subtag thứ hai là mã vùng.</p><p></p><p>Ví dụ: `en` là tiếng Anh, `pt-br` là tiếng Bồ Đào Nha Brazil.</p><p>Xem [Localizations and locale codes](localizations-and-locale-codes) để biết thêm thông tin về mã locale và cách chúng tôi khuyến nghị sử dụng.</p> |
| **fetchPolicy**   | mặc định: `'reload_revalidating_cache_data'` | <p>`fetchPolicy` xác định SDK đọc từ lớp nào trước, chứ không phải liệu SDK có thể dùng cache hay không. Theo mặc định, SDK sẽ gọi lên server trước và trả về dữ liệu đã cache nếu yêu cầu đó thất bại. Chúng tôi khuyến nghị tùy chọn này vì nó đảm bảo người dùng luôn nhận được dữ liệu mới nhất.</p><p></p><p>Tuy nhiên, nếu bạn cho rằng người dùng của mình thường xuyên gặp tình trạng kết nối mạng không ổn định, hãy cân nhắc dùng `'return_cache_data_else_load'` — tùy chọn này đảo ngược thứ tự: đọc cache trước, chỉ gọi server khi không có gì trong cache. Người dùng có thể không nhận được dữ liệu mới nhất tuyệt đối, nhưng thời gian tải sẽ nhanh hơn bất kể kết nối mạng có bị gián đoạn. Cache được cập nhật thường xuyên nên việc dùng cache trong phiên để tránh các yêu cầu mạng là an toàn.</p><p></p><p>Chính sách thứ ba, `'return_cache_data_if_not_expired_else_load'`, nằm giữa hai tùy chọn trên: đọc cache trước khi bản cache còn trẻ hơn giá trị `maxAgeSeconds` bạn truyền vào, và gọi server khi cache đã cũ hơn giá trị đó.</p><p></p><p>Lưu ý rằng cache vẫn được giữ nguyên khi khởi động lại ứng dụng và chỉ bị xóa khi cài đặt lại ứng dụng hoặc thông qua việc dọn dẹp thủ công.</p><p></p><p>Adapty SDK lưu trữ paywall cục bộ theo hai lớp: cache được cập nhật thường xuyên như mô tả ở trên và [paywall dự phòng](fallback-paywalls). Chúng tôi cũng sử dụng CDN để tải paywall nhanh hơn và một server dự phòng độc lập trong trường hợp CDN không khả dụng. Hệ thống này được thiết kế để đảm bảo bạn luôn nhận được phiên bản mới nhất của paywall, đồng thời duy trì độ tin cậy ngay cả khi kết nối mạng kém.</p> |
| **loadTimeoutMs** | mặc định: 5 giây | <p>Giá trị này giới hạn thời gian chờ cho phương thức này. Nếu hết thời gian chờ, dữ liệu đã cache hoặc fallback cục bộ sẽ được trả về.</p><p>Lưu ý rằng trong một số trường hợp hiếm gặp, phương thức này có thể hết thời gian chờ muộn hơn một chút so với giá trị được chỉ định trong `loadTimeout`, vì thao tác có thể bao gồm nhiều yêu cầu khác nhau bên dưới.</p><p>Đối với Android: Bạn có thể tạo `TimeInterval` bằng các hàm mở rộng (như `5.seconds`, trong đó `.seconds` đến từ `import com.adapty.utils.seconds`), hoặc `TimeInterval.seconds(5)`. Để không giới hạn thời gian chờ, hãy dùng `TimeInterval.INFINITE`.</p> |
...
:::note
Nếu bạn hỗ trợ nhiều ngôn ngữ, hãy thêm bản địa hóa cho paywall của mình. Để biết các mã cần sử dụng, xem [Bản địa hóa và mã locale](react-native-localizations-and-locale-codes).
:::
...
| **locale** | <p>tùy chọn</p><p>mặc định: `en`</p> | <p>Định danh của [bản địa hóa paywall](add-remote-config-locale). Tham số này là mã ngôn ngữ gồm một hoặc nhiều thẻ con được phân tách bằng dấu trừ (**-**). Thẻ con đầu tiên là ngôn ngữ, thẻ con thứ hai là khu vực.</p><p></p><p>Ví dụ: `en` là tiếng Anh, `pt-br` là tiếng Bồ Đào Nha của Brazil.</p><p></p><p>Xem [Bản địa hóa và mã locale](react-native-localizations-and-locale-codes) để biết thêm thông tin về mã locale và cách chúng tôi khuyến nghị sử dụng chúng.</p>                                                                                                                                                                                                                                       |
| **fetchPolicy** | mặc định: `'reload_revalidating_cache_data'` | <p>`fetchPolicy` xác định SDK đọc từ tầng nào trước, chứ không phải là có được dùng cache hay không. Mặc định, SDK sẽ gọi lên server trước và trả về dữ liệu đã cache nếu yêu cầu đó thất bại. Chúng tôi khuyến nghị cách này vì nó đảm bảo người dùng luôn nhận được dữ liệu mới nhất.</p><p></p><p>Tuy nhiên, nếu bạn cho rằng người dùng của mình thường xuyên gặp tình trạng mạng không ổn định, hãy cân nhắc dùng `'return_cache_data_else_load'` — cách này đảo ngược thứ tự ưu tiên: đọc cache trước và chỉ gọi server khi không có gì trong cache. Người dùng có thể không nhận được dữ liệu mới nhất tuyệt đối, nhưng thời gian tải sẽ nhanh hơn dù kết nối mạng có kém đến đâu. Cache được cập nhật thường xuyên nên hoàn toàn an toàn khi dùng trong phiên làm việc để tránh các yêu cầu mạng không cần thiết.</p><p></p><p>Chính sách thứ ba, `'return_cache_data_if_not_expired_else_load'`, nằm giữa hai cách trên: đọc cache trước khi bản cache còn trẻ hơn giá trị `maxAgeSeconds` bạn truyền vào, và gọi server khi cache đã cũ hơn giá trị đó.</p><p></p><p>Lưu ý rằng cache vẫn được giữ nguyên khi khởi động lại ứng dụng và chỉ bị xóa khi cài đặt lại ứng dụng hoặc thực hiện dọn dẹp thủ công.</p> |
