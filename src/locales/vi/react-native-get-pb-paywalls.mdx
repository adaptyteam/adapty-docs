...
Nếu placement được thiết kế trong **Flow & Paywall Builder** hoặc **old Paywall Builder**, Adapty sẽ tự động render giao diện cho bạn. Tạo view bằng `createFlowView`, sau đó [hiển thị flow hoặc paywall](react-native-present-paywalls). Nếu placement là custom paywall không có Builder UI, hãy [xử lý như Remote Config paywall](present-remote-config-paywalls-react-native).

Trong React Native SDK, gọi trực tiếp `createFlowView` — bạn không cần fetch cấu hình view trước.
...
| **locale**           | tùy chọn | Mã định danh của [bản địa hóa flow](add-paywall-locale-in-adapty-paywall-builder) để render view — ví dụ: `en` hoặc `pt-br`. Nếu bỏ qua, view sẽ render bằng `en`, hoặc theo ngôn ngữ mặc định của flow nếu flow không có `en`. Yêu cầu SDK 4.0.2 trở lên. Xem [Bản địa hóa và mã ngôn ngữ](react-native-localizations-and-locale-codes). |
| **customTags**       | tùy chọn | Một dictionary các custom tag và giá trị tương ứng, dùng làm placeholder trong nội dung. Custom tag chỉ áp dụng cho [paywall dùng builder cũ](custom-tags-in-paywall-builder) — các flow dùng [variables](onboarding-variables) thay thế. |
...
| **android.enableSafeArea** | tùy chọn | Chỉ dành cho Android (bị bỏ qua trên iOS). Truyền dưới dạng object lồng nhau: `android: { enableSafeArea: true }`. Khi là `true`, flow view sẽ áp dụng padding safe-area. Mặc định là `true` cho kiểu trình bày modal (`createFlowView` + `present()`) và `false` cho component `AdaptyFlowView` nhúng vào. Giá trị mặc định phù hợp với hầu hết các trường hợp. |
