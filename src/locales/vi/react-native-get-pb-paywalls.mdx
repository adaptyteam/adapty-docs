...
| **locale**           | tùy chọn | Mã định danh của [localization flow](add-paywall-locale-in-adapty-paywall-builder) để render view — ví dụ: `en` hoặc `pt-br`. Nếu bỏ qua, view sẽ render bằng `en`, hoặc theo localization mặc định của flow khi flow không có `en`. Yêu cầu SDK 4.0.2 trở lên. Xem [Localizations và mã locale](react-native-localizations-and-locale-codes). |
...
| **android.enableSafeArea** | tùy chọn | Chỉ dành cho Android (bị bỏ qua trên iOS). Truyền dưới dạng đối tượng lồng nhau: `android: { enableSafeArea: true }`. Khi là `true`, flow view sẽ áp dụng padding safe-area. Mặc định là `true` với dạng hiển thị modal (`createFlowView` + `present()`) và `false` với component `AdaptyFlowView` nhúng. Giá trị mặc định phù hợp với hầu hết các trường hợp. |
