...
1. `openWebPaywall(product)` tạo URL theo paywall và cũng thêm dữ liệu sản phẩm vào URL.
2. `openWebPaywall(paywall)` tạo URL theo paywall mà không thêm dữ liệu sản phẩm vào URL. Trong SDK v4, phương thức này nhận một `AdaptyFlowPaywall` — biến thể paywall của flow đã được tải. Hãy kiểm tra `flow.paywalls` không rỗng trước khi truy cập theo chỉ mục, ví dụ `flow.paywalls[0]`.
:::
