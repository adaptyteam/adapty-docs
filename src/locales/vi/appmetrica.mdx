...
[AppMetrica](https://appmetrica.yandex.com/about) là một công cụ phân tích miễn phí giúp bạn theo dõi hành vi người dùng và phân tích hiệu suất ứng dụng di động theo thời gian thực. Bằng cách tích hợp AppMetrica với Adapty, bạn có thể hiểu sâu hơn về các chỉ số gói đăng ký và mức độ tương tác của người dùng.
...
| Tham số            | Kiểu    | Mô tả                                                                                                                                                                   |
|:---------------------|:--------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `post_api_key`       | String  | Post API Key của AppMetrica.                                                                                                                                                 |
| `application_id`     | String  | Application ID của AppMetrica.                                                                                                                                               |
| `revenue_event_type` | String  | Loại sự kiện doanh thu (ví dụ: "subscription_renewed", "refund", "intro_started"). Xem [ánh xạ sự kiện AppMetrica](#events-and-tags).                             |
| `price`              | Float   | Số tiền doanh thu (dựa trên cài đặt tính toán doanh thu của bạn).                                                                                                                  |
| `currency`           | String  | Mã tiền tệ (ví dụ: "USD").                                                                                                                                                  |
| `product_id`         | String  | Product ID từ cửa hàng.                                                                                                                                                |
| `quantity`           | Integer | Luôn là 1.                                                                                                                                                                     |
| `transaction_id`     | String  | Transaction ID của cửa hàng.                                                                                                                                                         |
| `payload`            | String  | Chuỗi JSON được mã hóa URL chứa thông tin chi tiết sự kiện. Tự động cắt bớt nếu vượt quá 30KB bằng cách xóa các trường tùy chọn theo thứ tự ưu tiên để giữ lại dữ liệu quan trọng nhất. |
| `os_name`            | String  | "ios" hoặc "android".                                                                                                                                                           |
| `profile_id`         | String  | AppMetrica Profile ID (nếu đã đặt), nếu không thì là Customer User ID (nếu có).                                                                                                    |
| `appmetrica_device_id` | String  | AppMetrica Device ID Hash. Chỉ được gửi nếu `profile_id` không có sẵn.                                                                                                        |
| `session_type`       | String  | Luôn là "foreground".                                                                                                                                                          |
| `ios_ifa`            | String  | **Chỉ iOS**. ID for Advertisers.                                                                                                                                             |
| `ios_ifv`            | String  | **Chỉ iOS**. ID for Vendors.                                                                                                                                                 |
