...
[Mixpanel](https://mixpanel.com/home/) là một dịch vụ phân tích sản phẩm mạnh mẽ. Giải pháp theo dõi dựa trên sự kiện của nó giúp các nhóm sản phẩm có được những thông tin chi tiết có giá trị về chiến lược thu hút, chuyển đổi và giữ chân người dùng tối ưu trên nhiều nền tảng khác nhau.

Tích hợp này cho phép bạn đưa tất cả các sự kiện Adapty vào Mixpanel. Nhờ đó, bạn sẽ có cái nhìn toàn diện hơn về hoạt động kinh doanh gói đăng ký và hành động của khách hàng. Adapty cung cấp đầy đủ dữ liệu để bạn theo dõi [sự kiện gói đăng ký](events) từ các cửa hàng tại một nơi. Với Adapty, bạn có thể dễ dàng nắm bắt hành vi của người dùng, hiểu họ thích gì, và từ đó giao tiếp với họ theo cách có mục tiêu và hiệu quả.
...
2. Bật toggle và nhập **Mixpanel Token** của bạn. Adapty lưu riêng biệt token **Production** và **Sandbox** cho từng nền tảng: iOS, Android, Stripe và Paddle. Chỉ điền vào các nền tảng bạn muốn nhận dữ liệu — nền tảng nào không có token sẽ không gửi sự kiện nào.
3. Đặt **Mixpanel Data Residency** khớp với dự án Mixpanel của bạn. Trường này bắt buộc và mặc định là **US**. Chọn **US** cho endpoint `api.mixpanel.com` hoặc **Europe** cho `api-eu.mixpanel.com`.
