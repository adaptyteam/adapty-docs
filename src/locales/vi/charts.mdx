...
Để xem mô tả và so sánh từng biểu đồ, hãy xem [Bảng so sánh chỉ số](metric-comparison-table). Để biết về bộ lọc, nhóm và điều chỉnh khoảng thời gian, hãy xem [Bộ điều khiển Analytics](controls-filters-grouping-compare-proceeds).

Paywall, placement và flow có trang chỉ số riêng: [Chỉ số Paywall](paywall-metrics), [Chỉ số Placement](placement-metrics) và [Chỉ số Flow](flow-metrics). Hầu hết các biểu đồ trên trang này cũng có thể được [lọc hoặc nhóm theo flow](filter-flows#filter-analytics-by-flow).
