---
title: "Tích hợp Adapty vào ứng dụng iOS của bạn với sự hỗ trợ của AI"
description: "Hướng dẫn từng bước tích hợp Adapty vào ứng dụng iOS của bạn bằng Cursor, Context7, ChatGPT, Claude hoặc các công cụ AI khác."
metadataTitle: "Tích hợp Adapty với sự hỗ trợ AI cho iOS | Tài liệu Adapty"
---

Hướng dẫn này sẽ đưa bạn qua từng bước tích hợp Adapty vào ứng dụng iOS của mình bằng công cụ lập trình AI — bạn chỉ cần cung cấp đúng tài liệu Adapty theo đúng thứ tự.

<SkillPromo />

## Trước khi bắt đầu: thiết lập dashboard \{#before-you-start-dashboard-setup\}

Adapty yêu cầu một số cấu hình trên dashboard trước khi bạn viết bất kỳ code SDK nào. Bạn có thể thực hiện điều này với skill LLM tương tác, hoặc thủ công thông qua Dashboard.
...
Sau khi thêm skill, chạy `/adapty-cli` trong agent của bạn. Nó sẽ hướng dẫn bạn từng bước — bao gồm cả khi nào cần mở Dashboard để kết nối cửa hàng.
...
- [**Paywalls tạo thủ công**](ios-quickstart-manual): Bạn tự xây dựng giao diện paywall bằng code nhưng vẫn dùng Adapty để lấy sản phẩm và xử lý giao dịch mua.
- [**Observer mode**](observer-vs-full-mode): Bạn giữ nguyên hạ tầng mua hàng hiện có và chỉ dùng Adapty cho phân tích và tích hợp.
...
### Hiển thị flow hoặc paywall và xử lý giao dịch mua \{#show-flows-or-paywalls-and-handle-purchases\}

Lấy một flow hoặc paywall theo placement ID, hiển thị nó, và xử lý các sự kiện mua hàng. Các hướng dẫn bạn cần phụ thuộc vào cách bạn xử lý giao dịch mua.

Hãy kiểm tra từng giao dịch mua trong sandbox ngay khi bạn thực hiện — đừng chờ đến cuối. Xem [Kiểm tra giao dịch mua trong sandbox](test-purchases-in-sandbox) để biết hướng dẫn cài đặt.
...
**Hướng dẫn:**
- [Kích hoạt mua hàng với Flow & Paywall Builder (quickstart)](ios-quickstart-paywalls)
- [Lấy flow và cấu hình của chúng](get-pb-paywalls)
- [Hiển thị flow](ios-present-paywalls)
- [Xử lý sự kiện flow](ios-handling-events)
- [Phản hồi các hành động nút](handle-paywall-actions)
