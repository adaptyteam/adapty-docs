...
Nếu bạn đã tạo một flow, hoặc một paywall trong Paywall Builder cũ, bạn không cần lo lắng về việc render nó trong code ứng dụng di động để hiển thị cho người dùng. Flow hoặc paywall như vậy đã chứa cả nội dung cần hiển thị lẫn cách hiển thị nó.

:::warning

Hướng dẫn này đề cập đến các flow và paywall được render bởi Adapty. Quy trình sẽ khác đối với paywall dùng Remote Config và [chế độ Observer](observer-vs-full-mode).
...
Theo mặc định, embedded view không áp dụng safe-area paddings — layout của bạn cần tự xử lý insets. Nếu muốn view tự áp dụng chúng, hãy truyền `androidEnableSafeArea = true` vào `createNativeFlowView`. Tham số này chỉ dành cho Android.
</TabItem>
<TabItem value="ios" label="iOS">
Vì các phương thức mặc định của interface KMP trở thành `@required` trong Swift, bạn không thể triển khai `AdaptyUIFlowsEventsObserver` trực tiếp từ Swift. Hãy khai báo một base class open trong `iosMain` trước:
...
Hướng dẫn này chỉ dành cho **các paywall được xây dựng bằng builder cũ**. Quy trình hiển thị paywall khác nhau đối với paywall remote config và [Observer mode](observer-vs-full-mode).
