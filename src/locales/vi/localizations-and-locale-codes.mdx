...
Paywall mà Adapty hiển thị được phân phối dưới dạng flow trong SDK v4, vì vậy quy tắc dưới đây áp dụng cho cả hai.

Khớp là tuyệt đối chính xác. SDK so sánh mã bạn truyền vào với mã bản địa hóa của flow theo từng ký tự: không thay đổi chữ hoa/thường, không thay thế dấu gạch dưới (`_`) bằng dấu gạch ngang (`-`), và không tự động lùi về language subtag. Với một flow có bản địa hóa `pt-br`, chỉ `pt-br` mới khớp: `pt-BR`, `pt_BR`, và `pt-PT` đều không khớp.

Khi mã không khớp với bất kỳ bản địa hóa nào, flow sẽ hiển thị theo [ngôn ngữ mặc định](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — SDK không trả về lỗi và không ghi log cảnh báo.

Khi mã khớp, Adapty sẽ hợp nhất bản địa hóa đó với bản địa hóa mặc định: các chuỗi và tài nguyên mà bản địa hóa được khớp không định nghĩa sẽ được lấy từ bản địa hóa mặc định.

Việc bỏ qua mã ngôn ngữ không đồng nghĩa với việc yêu cầu bản địa hóa mặc định của flow: SDK sẽ thay thế bằng `en` cố định. Một flow có ngôn ngữ mặc định là `de` vẫn hiển thị bằng `en` nếu nó có bản địa hóa `en`, và chỉ dự phòng về `de` khi không có.

:::warning
Truyền mã locale chính xác như đã cấu hình trong dashboard — các subtag viết thường, phân cách bằng dấu gạch ngang. Đừng truyền trực tiếp identifier locale của hệ thống: `Locale.current.identifier` trả về `pt_BR` và `Locale.current.identifier(.bcp47)` trả về `pt-BR`, cả hai đều sẽ rơi về localization mặc định. Hãy chuyển đổi giá trị trong ứng dụng của bạn trước khi truyền vào.
:::
