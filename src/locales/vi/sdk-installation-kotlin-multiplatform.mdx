...
- **AdaptyUI** (`io.adapty:adapty-kmp-ui`): Module này hiển thị các flow và paywall dạng old builder thông qua lớp render Compose Multiplatform (`view.present()`). Nếu dự án của bạn không sử dụng Compose Multiplatform, bạn có thể dùng [`createNativePaywallView`](kmp-present-paywalls#without-compose-multiplatform) và [`createNativeOnboardingView`](kmp-present-onboardings#without-compose-multiplatform) từ module core thay thế.

:::tip
Muốn xem ví dụ thực tế về cách tích hợp Adapty SDK vào ứng dụng di động? Hãy xem [ứng dụng mẫu](https://github.com/adaptyteam/AdaptySDK-KMP/tree/main/example) của chúng tôi, minh họa toàn bộ quá trình thiết lập, bao gồm hiển thị paywall, thực hiện mua hàng và các chức năng cơ bản khác.
:::
...
Từ phiên bản 3.17.0, Adapty Kotlin Multiplatform SDK sử dụng Billing Library v8 theo mặc định. Các phiên bản từ 3.15.0 đến 3.16.2 sử dụng v7, và cũng hoạt động được khi một dependency khác trong bản build Android của bạn nâng Billing Library lên v8.

Khả năng tương thích với phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng Google giới thiệu trong đó. Trước khi áp dụng một tính năng mới của Google Play billing, hãy xem [Sản phẩm trong Play Store](android-products).
:::
...
- Gradle thông thường: Thêm dependency vào **module-level** `build.gradle`
- Nếu dự án của bạn dùng file `.gradle.kts`, thêm dependency vào **module-level** `build.gradle.kts`
- Nếu bạn dùng version catalogs, thêm dependency vào file `libs.versions.toml` rồi tham chiếu nó trong `build.gradle.kts`
...
- Nếu bạn dùng [Flow & Paywall Builder](adapty-flow-builder), trước tiên hãy [kích hoạt module AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) bên dưới, sau đó làm theo [hướng dẫn quickstart](kmp-quickstart-paywalls).
- Nếu bạn tự xây dựng giao diện paywall, xem [hướng dẫn quickstart cho paywall tùy chỉnh](kmp-quickstart-manual).

## Kích hoạt module AdaptyUI của Adapty SDK \{#activate-adaptyui-module-of-adapty-sdk\}

Nếu bạn có kế hoạch kích hoạt module **AdaptyUI** để sử dụng [Flow & Paywall Builder](adapty-flow-builder), hãy đảm bảo đặt `.withActivateUI(true)` trong cấu hình của bạn.

:::info
important
Trong code của bạn, bạn phải kích hoạt module Adapty cốt lõi trước khi kích hoạt AdaptyUI.
:::
...
Nếu bạn sử dụng [Adapty Attribution](user-acquisition), hãy đặt `withAdaptyAttributionEnabled` thành `true` khi kích hoạt SDK. Giá trị mặc định là `false`: nếu không có tham số này, SDK sẽ không ghi nhận lượt cài đặt hay truyền thông tin cài đặt về ứng dụng của bạn. Ở các phiên bản SDK thấp hơn 4.1, Adapty Attribution được bật tự động.
