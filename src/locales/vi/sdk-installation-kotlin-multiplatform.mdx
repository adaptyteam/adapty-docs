...
:::important
Adapty Kotlin Multiplatform SDK 4.0 là bản pre-release. Gradle không tự động chọn phiên bản pre-release thông qua các dải phiên bản động (như `+` hoặc `latest.release`), vì vậy bạn phải chỉ định chính xác phiên bản — ví dụ `io.adapty:adapty-kmp:4.0.1-beta.1`, hoặc `adapty-kmp = "4.0.1-beta.1"` trong `libs.versions.toml`. Xem [Migrate Adapty Kotlin Multiplatform SDK sang v4](migration-to-kmp-sdk-v4).
:::
