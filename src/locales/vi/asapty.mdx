...
Sử dụng tích hợp [Asapty](https://asapty.com/), bạn có thể tối ưu hóa các chiến dịch Search Ads. Adapty gửi các sự kiện gói đăng ký đến Asapty, giúp bạn xây dựng các dashboard tùy chỉnh dựa trên attribution từ Apple Search Ads.

Tích hợp cụ thể này không thêm bất kỳ dữ liệu attribution nào vào Adapty, vì chúng tôi đã có đầy đủ thông tin cần thiết trực tiếp từ [ASA](apple-search-ads).
...
### Sự kiện không đến được Adapty \{#events-dont-reach-asapty\}

- Đảm bảo bạn đã cấu hình [Apple Search Ads](apple-search-ads) trong Adapty và [tải lên thông tin xác thực](https://app.adapty.io/settings/apple-search-ads), nếu thiếu, Asapty sẽ không hoạt động.
- Chỉ những hồ sơ người dùng có attribution ASA chi tiết (không phải organic) mới gửi được sự kiện đến Asapty. Bạn sẽ thấy thông báo "The user profile is missing the required integration data." nếu attribution không đủ điều kiện.
- Các hồ sơ người dùng được tạo trước khi cấu hình tích hợp sẽ không thể gửi sự kiện đến Asapty.
- Nếu tích hợp với Adapty vẫn không hoạt động dù đã cấu hình đúng, hãy đảm bảo toggle **Receive Apple Search Ads attribution in Adapty** đã được bật trong tab [**App Settings** -> **Apple Search Ads**](https://app.adapty.io/settings/apple-search-ads).
