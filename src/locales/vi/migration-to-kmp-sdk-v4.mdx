...
Module `adapty-kmp-ui` chỉ cần thiết nếu bạn render flow và paywall bằng layer Compose Multiplatform (`view.present()`). Xem [Cài đặt Adapty SDK](sdk-installation-kotlin-multiplatform) để biết hướng dẫn cài đặt đầy đủ.

Các native Adapty SDK bên dưới đã được nâng lên 4.0.0 trên cả hai nền tảng và được resolve tự động — không cần thay đổi gì trong build. Deployment target iOS vẫn giữ nguyên là **15.0**, không thay đổi trong bản phát hành này.
