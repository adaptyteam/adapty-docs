...
- `AdaptyUI.openWebUrl(url, openIn)` và `AdaptyUI.requestAppReview()` — hỗ trợ xử lý `OpenUrlAction` mặc định và `handleAppReviewRequest` mặc định, vì vậy URL và yêu cầu đánh giá ứng dụng được xử lý natively ngay từ đầu. Chỉ gọi trực tiếp khi bạn ghi đè các giá trị mặc định đó.
