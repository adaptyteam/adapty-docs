...
Phiên bản 4.x giới thiệu tính năng flow và đổi tên các API paywall cho phù hợp. Các API mới hoạt động với cả Flow Builder mới lẫn Paywall Builder hiện có — không cần thay đổi cài đặt nào trên Adapty Dashboard. Ngoài ra, phiên bản 4.1 chuyển Adapty Attribution sang dạng opt-in, đổi tên các API attribution bên ngoài và kiểu gói đăng ký của sản phẩm, đồng thời bổ sung tính năng in-app purchase được quảng bá trên App Store.
...
| Callback `paywallViewDidPerformAction`, `paywallViewDidAppear` và các callback `paywallView...` khác | Callback `flowViewDidPerformAction`, `flowViewDidAppear` và các callback `flowView...` khác |
| `paywallViewDidFailRendering` | `flowViewDidReceiveError` |
| `Adapty.updateAttribution(attribution, source)` với nguồn kiểu `String` | `Adapty.updateExternalAttribution(attribution, provider)` với `AdaptyExternalAttributionProvider` |
...
| Không hỗ trợ in-app purchase được quảng bá | `OnPromotedPurchaseListener` và `Adapty.makePromotedPurchase(product)` |
...
:::note
`customTimers` vẫn còn tồn tại, nhưng nó chỉ ảnh hưởng đến các paywall dùng Paywall Builder cũ. [Đồng hồ đếm ngược](flow-timer) của một flow chạy theo hành vi được thiết lập trong Flow Builder, vì vậy flow sẽ bỏ qua bất kỳ giá trị nào bạn truyền vào đây.
:::
...
Nếu bạn không dùng Compose Multiplatform, phương thức factory gốc cũng được đổi tên theo cách tương tự:
...
SDK 4.1 bổ sung hỗ trợ cho [in-app purchase được quảng bá trên trang sản phẩm App Store của bạn](kmp-making-purchases#in-app-purchases-from-the-app-store) trên iOS. Các phiên bản Kotlin Multiplatform trước đây không hỗ trợ tính năng này, vì vậy những gì bạn đang có hiện tại sẽ không bị ảnh hưởng — nhưng tính năng này cần có code trước khi hoạt động được.
...
`createFlowView`, `createNativeFlowView`, và `AdaptyUIFlowPlatformView` có thêm tham số tùy chọn mới là `customLayoutId`. Truyền tham số này vào để hiển thị một layout cụ thể đã được định nghĩa trong Flow Builder thay vì layout mà SDK tự động chọn dựa trên loại thiết bị và kích thước màn hình:

```kotlin showLineNumbers
AdaptyUI.createFlowView(flow, customLayoutId = "tablet_landscape")
```

Nếu không có layout nào khớp với ID, flow sẽ tải mà không có cấu hình view. Tham số này là tùy chọn và mặc định là `null`, nên các lệnh gọi hiện có sẽ không bị ảnh hưởng.
...
## API onboarding đã bị deprecated \{#onboarding-api-deprecation\}

API onboarding cũ đã bị deprecated trong v4, thay thế bằng [Flow Builder](adapty-flow-builder). API này vẫn hoạt động, nhưng sẽ bị xóa trong phiên bản tương lai, vì vậy hãy lên kế hoạch migration các onboarding của bạn sang Flow Builder.

Các ký hiệu đã deprecated: `getOnboarding`, `getOnboardingForDefaultAudience`, `AdaptyUI.createOnboardingView`, `AdaptyUI.createNativeOnboardingView`, và `AdaptyUIOnboardingsEventsObserver`.
