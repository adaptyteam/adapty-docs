---
title: "Migrate Adapty Kotlin Multiplatform SDK sang v4.0"
description: "Migrate sang Adapty Kotlin Multiplatform SDK v4.0 (beta) bằng cách thay thế API paywall bằng API flow, tương thích với cả Flow Builder và Paywall Builder."
metadataTitle: "Hướng dẫn migration sang Adapty Kotlin Multiplatform SDK v4.0 (beta) | Tài liệu Adapty"
---
...
`createFlowView` trả về `AdaptyResult.Error` nếu flow không có view được cấu hình — thay thế cho việc kiểm tra `hasViewConfiguration` ở v3:
...
:::note
Một flow view chỉ dùng được một lần: sau khi bạn gọi `dismiss()`, view sẽ bị hủy, vì vậy hãy gọi lại `createFlowView` để hiển thị flow thêm một lần nữa.
:::
...
- `AdaptyUI.setObserverModeResolver(...)` với `AdaptyUIObserverModeResolver` — điều khiển các lượt mua và khôi phục được khởi tạo từ flow khi SDK chạy ở chế độ [Observer mode](implement-observer-mode-kmp). Trước đây tính năng này chỉ có trên SDK iOS và Android native. Xem [Hiển thị flow trong Observer mode](kmp-present-flows-in-observer-mode).
- `AdaptyUI.setSystemRequestsHandler(...)` với `AdaptyUISystemRequestsHandler` — dành cho các yêu cầu hệ thống từ flow (lời nhắc cấp quyền của OS và yêu cầu đánh giá ứng dụng). Flow hiện chưa kích hoạt các yêu cầu này, nên bạn không cần đăng ký handler.
- Callback tùy chọn mới `flowViewDidReceiveAnalyticEvent` báo cáo các sự kiện analytics từ flow, bắt đầu với lượt xem màn hình cho mỗi màn hình người dùng mở. Xem [Theo dõi lượt xem màn hình flow](kmp-flow-screen-views).
- `AdaptyUI.openWebUrl(url, openIn)` và `AdaptyUI.requestAppReview()` — hỗ trợ xử lý mặc định `OpenUrlAction` và `handleAppReviewRequest` mặc định, giúp URL và lời nhắc đánh giá ứng dụng được xử lý natively ngay từ đầu. Chỉ gọi trực tiếp khi bạn ghi đè những mặc định đó.
- `AdaptyUIFlowView.locale` — báo cáo bản địa hóa mà view được xây dựng, giúp bạn biết người dùng thực sự đang thấy phiên bản nào. Yêu cầu SDK 4.0.1-beta.1 trở lên.
- `AdaptyConfig.ServerCluster.CN` — tùy chọn cụm máy chủ mới bên cạnh `DEFAULT` và `EU`, dùng để kết nối ứng dụng với [máy chủ Trung Quốc của Adapty](china-cluster).
