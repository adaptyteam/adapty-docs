...
Adapty gửi tất cả [các sự kiện gói đăng ký](events) đến các dịch vụ phân tích như [Amplitude](amplitude), [Mixpanel](mixpanel) và [AppMetrica](appmetrica). Chúng tôi cũng có thể gửi sự kiện đến máy chủ của bạn thông qua tích hợp [webhook](webhook). Điều tuyệt vời là bạn không cần phải tự gửi bất kỳ sự kiện nào — chúng tôi sẽ làm điều đó cho bạn. Chỉ cần đảm bảo bạn đã cấu hình tích hợp trong Adapty Dashboard.

Adapty hỗ trợ tích hợp với các dịch vụ phân tích bên thứ ba sau:
...
Các sự kiện webhook được gửi theo định dạng JSON. Tất cả sự kiện đều có cùng cấu trúc, nhưng các trường dữ liệu có thể khác nhau tùy theo loại sự kiện, cửa hàng và cấu hình cụ thể của bạn.
...
| **price_local**               | float         | Giá sản phẩm trước khi Apple/Google khấu trừ theo đơn vị tiền tệ địa phương. Doanh thu. |
| **proceeds_local**            | float         | Giá sản phẩm sau khi Apple/Google khấu trừ theo đơn vị tiền tệ địa phương. Doanh thu thuần. |
| **transaction_id**            | str           | Mã định danh duy nhất cho một giao dịch, chẳng hạn như mua hàng hoặc gia hạn. |
...
| **trial_duration**            | str           | Thời lượng dùng thử tính bằng ngày. Được gửi theo định dạng "{} days", ví dụ: "7 days". |
...
| **consecutive_payments**      | int           | Số kỳ thanh toán liên tiếp mà người dùng đã đăng ký mà không bị gián đoạn. Bao gồm kỳ hiện tại. |
| **rate_after_first_year**     | bool          | Boolean cho biết gói đăng ký có đủ điều kiện để được giảm tỷ lệ hoa hồng (thường là 15%) sau một năm gia hạn liên tục hay không. Tỷ lệ hoa hồng thay đổi tùy theo điều kiện chương trình và quốc gia. Xem [Hoa hồng cửa hàng và thuế](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue) để biết thêm chi tiết. |
...
| **paywall_name**              | str           | Tên paywall nơi giao dịch được thực hiện.                    |
| **paywall_revision**          | int           | Phiên bản của paywall nơi giao dịch được thực hiện. Giá trị được đặt là 1. |
| **developer_id**              | str           | ID nhà phát triển (SDK) của placement nơi giao dịch được thực hiện. |
| **ab_test_name**              | str           | Tên A/B test nơi giao dịch được thực hiện.                   |
...
| **profile_event_id**          | uuid          | ID sự kiện duy nhất có thể dùng để loại trừ trùng lặp.       |
| **store_country**             | str           | Quốc gia do cửa hàng gửi cho chúng tôi.                      |
...
| **renewed_at**                | ISO 8601 date | Ngày và giờ kỳ hiện tại của mức độ truy cập có trả phí gần nhất bắt đầu. Cho đến lần gia hạn đầu tiên, giá trị này trùng với `activated_at`. |
| **expires_at**                | ISO 8601 date | Ngày và giờ mức độ truy cập có trả phí hết hạn.              |
...
:::warning
Chặn phân tích bên ngoài sẽ dừng việc gửi sự kiện đến **tất cả** các tích hợp cho người dùng này, bao gồm cả [webhooks](webhook) tùy chỉnh. Đừng sử dụng tính năng này nếu backend của bạn phụ thuộc vào các sự kiện webhook để quản lý quyền truy cập của người dùng.

Cài đặt này không có hiệu lực hồi tố: các sự kiện được tạo trước khi cài đặt này được bật vẫn sẽ được gửi đi. Ngoài ra, nó cũng không ảnh hưởng đến phân tích của chính Adapty — các giao dịch của người dùng vẫn được tính trong các biểu đồ trên dashboard.
:::
