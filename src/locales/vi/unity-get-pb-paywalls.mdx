...
| **Locale**                   | tùy chọn       | Mã định danh của [bản dịch Builder](add-paywall-locale-in-adapty-paywall-builder) dùng để hiển thị flow hoặc paywall, ví dụ `en` hoặc `pt-br`. Flow được bản địa hóa khi view của nó được tạo, vì vậy đây là nơi duy nhất để chọn bản địa hóa. Nếu bỏ qua, Adapty sẽ tự xác định bản địa hóa từ thiết bị. Xem [Sử dụng bản địa hóa và mã locale](unity-localizations-and-locale-codes). |
...
| **PreloadProducts**          | tùy chọn       | Đặt thành `true` để tải trước sản phẩm nhằm cải thiện hiệu suất. Khi bật, sản phẩm được tải trước, giảm thời gian cần thiết để hiển thị flow hoặc paywall. |
| **ProductPurchaseParameters** | tùy chọn      | Chỉ dành cho Android (bị bỏ qua trên iOS). Một dictionary ánh xạ `AdaptyProductIdentifier` sang `AdaptyPurchaseParameters`. Dùng để cấu hình các tham số mua hàng cụ thể như ưu đãi cá nhân hóa hoặc tham số cập nhật gói đăng ký cho từng sản phẩm trong flow hoặc paywall. |
| **EnableSafeAreaPaddings**   | tùy chọn       | Chỉ dành cho Android (bị bỏ qua trên iOS). Khi đặt thành `true`, view của flow áp dụng padding vùng an toàn. Mặc định: `true`. Giá trị mặc định phù hợp với hầu hết các trường hợp. |
