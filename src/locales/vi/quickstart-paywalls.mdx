---
title: "Bật tính năng mua hàng"
description: "Thêm flow hoặc paywall trong Adapty để hiển thị sản phẩm của bạn, sau đó gắn nó vào một placement."
metadataTitle: "Bật tính năng mua hàng | Tài liệu Adapty"
---
...
- **Flow & Paywall Builder (được khuyến nghị)**: Trình soạn thảo trực quan không cần code cho toàn bộ hành trình mua hàng. Adapty SDK render kết quả trực tiếp trên thiết bị, nên bạn không cần viết code UI.
- **Paywall thủ công**: Bạn tạo một paywall, gắn sản phẩm vào đó và tự render UI trong code ứng dụng.
- **Adapty Paywall Builder (Legacy)**: Trình soạn thảo paywall không cần code.

Cả hai lựa chọn đều kết thúc theo cùng một cách: bạn gắn những gì đã xây dựng vào một [placement](placements). Placement là thứ ứng dụng của bạn gọi tại runtime để lấy đúng nội dung cho đúng người dùng.
...
Một flow bao gồm một hoặc nhiều màn hình với các sản phẩm được nhúng trực tiếp vào. Bạn thiết kế flow trong [Flow & Paywall Builder](adapty-flow-builder) — không cần code.

SDK sẽ hiển thị flow trực tiếp trên từng nền tảng. App của bạn chỉ cần gọi `getFlow`, và SDK sẽ tự trình bày các màn hình, xử lý giao dịch mua và báo cáo sự kiện. Không cần code UI riêng, không phải duy trì paywall song song.
...
Tìm hiểu thêm về [Adapty Flow & Paywall Builder](adapty-flow-builder).

<ZoomImage id="new-flow.webp" />

Các hướng dẫn theo template dưới đây sẽ đi qua từng bước theo những pattern phổ biến nhất:

<CustomDocCardList ids={['basic-paywall-screen', 'paywall-with-tabs', 'paywall-features-per-product', 'onboarding-flow-tutorial']} />

Sau khi flow đã được lưu và xuất bản, hãy tiếp tục gắn nó vào một placement.
...
Trong code ứng dụng, bạn chỉ cần hardcode các placement ID. Mọi thứ còn lại — flow nào chạy, sản phẩm nào được bán, giao diện trông như thế nào — đều được cấu hình trong Adapty Dashboard và có thể thay đổi bất cứ lúc nào mà không cần cập nhật ứng dụng.

:::tip
Adapty cho phép bạn hiển thị các flow khác nhau cho từng nhóm người dùng và phân tích hiệu suất. Tìm hiểu thêm về [đối tượng](audience) và [A/B test](ab-tests).
:::
...
Bạn sẽ render paywall này trong code ứng dụng của mình. <InlineTooltip tooltip="triển khai paywall thủ công">Xem hướng dẫn cho nền tảng của bạn: [iOS](ios-implement-paywalls-manually), [Android](android-implement-paywalls-manually), [React Native](react-native-implement-paywalls-manually), [Flutter](flutter-implement-paywalls-manually), [Unity](unity-implement-paywalls-manually), [Kotlin Multiplatform](kmp-implement-paywalls-manually), [Capacitor](capacitor-implement-paywalls-manually).</InlineTooltip>
...
Một paywall được xây dựng trong [Paywall Builder](adapty-paywall-builder) là màn hình không cần code với các sản phẩm được nhúng trực tiếp. SDK của Adapty render nó theo kiểu native, nên bạn không cần viết code UI.
