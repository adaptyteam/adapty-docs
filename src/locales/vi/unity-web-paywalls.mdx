...
2. `OpenWebPaywall(paywall)` tạo URL theo paywall mà không thêm dữ liệu sản phẩm vào URL. Dùng phiên bản này khi các sản phẩm trong Adapty paywall của bạn khác với những sản phẩm trong web paywall.

Trong SDK v4, đối số `paywall` nhận một `AdaptyFlowPaywall` — một trong các biến thể paywall trong `flow.Paywalls`. Xem [hướng dẫn migration](migration-to-unity-sdk-v4).
:::
