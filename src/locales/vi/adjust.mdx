...
[Adjust](https://www.adjust.com/) là một trong những nền tảng Mobile Measurement Partner (MMP) hàng đầu, thu thập và trình bày dữ liệu từ các chiến dịch marketing. Điều này giúp các công ty theo dõi hiệu quả chiến dịch của mình.

Adapty cung cấp đầy đủ dữ liệu giúp bạn theo dõi [các sự kiện gói đăng ký](events) từ các cửa hàng tại một nơi. Với Adapty, bạn có thể dễ dàng hiểu hành vi của người dùng, biết họ thích gì, và dùng thông tin đó để tiếp cận họ một cách có mục tiêu và hiệu quả. Nhờ đó, tích hợp này cho phép bạn theo dõi các sự kiện gói đăng ký trong Adjust và phân tích chính xác doanh thu mà các chiến dịch của bạn tạo ra.

Tích hợp giữa Adapty và Adjust hoạt động theo hai hướng chính.

1. **Adapty nhận dữ liệu attribution từ Adjust**
   Sau khi thiết lập tích hợp Adjust, Adapty sẽ bắt đầu nhận dữ liệu attribution từ Adjust. Bạn có thể dễ dàng xem dữ liệu này trên trang hồ sơ người dùng.
...
2. **Adapty gửi các sự kiện gói đăng ký đến Adjust**
   Adapty có thể gửi tất cả các sự kiện gói đăng ký đã được cấu hình trong tích hợp của bạn đến Adjust. Nhờ đó, bạn có thể theo dõi các sự kiện này trong Adjust dashboard. Tích hợp này rất hữu ích để đánh giá hiệu quả của các chiến dịch quảng cáo.
...
:::warning
Adapty gửi [giao dịch mua qua Stripe và Paddle](#web-purchases-stripe-and-paddle) đến Adjust kèm theo app token của nền tảng mà người mua đã cài đặt ứng dụng của bạn. Nếu bạn phát hành ứng dụng Android và bán hàng trên web, hãy điền **App Token** cho Android. Nếu không có, giao dịch mua trên web của người dùng Android sẽ thất bại với lỗi [Missing integration credentials](handle-integration-errors#missing-integration-credentials).
:::
...
### Kết nối ứng dụng của bạn với Adjust \{#connect-your-app-to-adjust\}

Sau khi hoàn thành các bước mô tả ở trên, thêm hai phương thức sau vào ứng dụng của bạn. Chúng sẽ thiết lập kết nối giữa ứng dụng và Adjust:

1. **Để gửi dữ liệu gói đăng ký đến Adjust**: Truyền Adjust device ID vào phương thức SDK `setIntegrationIdentifier()`
2. **Để nhận dữ liệu attribution từ Adjust**: Cập nhật dữ liệu attribution bằng phương thức SDK `updateExternalAttribution()` (đặt tên là `updateAttribution()` trước SDK phiên bản 4.1)

Đối với Adjust phiên bản 5.0 trở lên, hãy dùng ví dụ sau:
...
## Mua hàng trên Web (Stripe và Paddle) \{#web-purchases-stripe-and-paddle\}

Adjust nhận dữ liệu mua hàng từ [Stripe](stripe) và [Paddle](paddle) cùng với các giao dịch mua hàng từ App Store và Google Play. Giao dịch mua hàng trên web diễn ra bên ngoài ứng dụng, nên không gắn với nền tảng di động nào. Adapty sẽ dùng nền tảng của người mua thay thế. Khi hồ sơ người dùng Adapty của họ được tạo trên App Store hoặc Google Play, giao dịch mua hàng trên web sẽ được gửi đến Adjust như thể nó đến từ cửa hàng đó:

| Cửa hàng hồ sơ người mua | Những gì Adapty gửi kèm giao dịch mua |
|:-------------------------|:---------------------------------------|
| App Store                | **App Token** và **OAuth Token** iOS của bạn, cùng với `idfa` và `idfv` của người mua. |
| Google Play              | **App Token** và **OAuth Token** Android của bạn, cùng với `android_id` và `gps_adid` của người mua. |

Giao dịch mua được gửi đến Adjust gắn với cùng thiết bị đã cài đặt ứng dụng, nhờ đó Adjust có thể ghi nhận giao dịch đó cho chiến dịch đã thu hút người dùng.

Trang tích hợp Adjust không có trường riêng cho Stripe hay Paddle. Các giao dịch mua trên web sử dụng lại thông tin xác thực iOS và Android từ phần [Kết nối Adapty với Adjust](#connect-adapty-to-adjust).

Người mua chưa từng mở ứng dụng di động của bạn sẽ không có Adjust device ID, và hồ sơ người dùng của họ được tạo trên Stripe hoặc Paddle. Adapty gửi giao dịch mua của họ đến Adjust cùng với **App Token** iOS của bạn nhưng không có advertising ID. Khi đó, Adjust không có định danh nào để liên kết giao dịch mua với một lượt cài đặt.
