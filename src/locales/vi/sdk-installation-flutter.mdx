...
Adapty Flutter SDK yêu cầu **iOS 15.0+**, **Xcode 26+** và **Flutter 3.32.0+** (Dart 3.8.0+). Xem [Swift Package Manager (iOS)](#swift-package-manager-ios) bên dưới để biết chi tiết cài đặt.
...
:::important
Các bước này yêu cầu **Flutter 3.32.0+** (Dart 3.8.0+). Plugin kéo native iOS SDK thông qua Swift Package Manager — xem [Swift Package Manager (iOS)](#swift-package-manager-ios) để biết cách thiết lập một lần.
:::
...
Nếu bạn sử dụng [Adapty Attribution](user-acquisition), hãy gọi `withAdaptyAttributionEnabled(true)` khi kích hoạt SDK. Giá trị mặc định là `false`: nếu không có tham số này, SDK sẽ không ghi nhận lượt cài đặt hoặc gửi thông tin cài đặt về ứng dụng của bạn. Trong các phiên bản SDK thấp hơn 4.1, Adapty Attribution được bật tự động.
