...
:::tip
Bạn muốn xem ví dụ thực tế về cách tích hợp Adapty SDK vào ứng dụng di động? Hãy xem [ứng dụng mẫu](https://github.com/adaptyteam/AdaptySDK-Flutter/tree/master/example) của chúng tôi, nơi minh họa toàn bộ quá trình thiết lập, bao gồm hiển thị paywall, thực hiện mua hàng và các chức năng cơ bản khác.
:::
...
Adapty Flutter SDK 4.0 — bổ sung hỗ trợ cho [flow](adapty-flow-builder) — nâng tối thiểu lên **iOS 15.0+**, **Xcode 26+**, và **Flutter 3.32.0+** (Dart 3.8.0+). Xem [Adapty SDK 4.0](#adapty-sdk-40-swift-package-manager) bên dưới để biết chi tiết cài đặt.

:::info
Adapty SDK đi kèm bản sao riêng của Google Play Billing Library — bạn không cần thêm nó vào ứng dụng.

Từ phiên bản 3.17.0, Adapty Flutter SDK sử dụng Billing Library v8 theo mặc định. Các phiên bản từ 3.15.0 đến 3.15.5 sử dụng v7, và cũng hoạt động khi một dependency khác trong Android build của bạn nâng Billing Library lên v8.

Khả năng tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google giới thiệu trong đó. Trước khi áp dụng một tính năng thanh toán Google Play mới, hãy xem [Sản phẩm trên Play Store](android-products).
:::
...
:::important
Các bước dưới đây cài đặt SDK ổn định mới nhất (3.x). Nếu bạn cần v4 — bắt buộc để dùng [Flow & Paywall Builder](adapty-flow-builder) và được sử dụng trong [quickstart](flutter-quickstart-paywalls) — hãy làm theo hướng dẫn [Adapty SDK 4.0: Swift Package Manager](#adapty-sdk-40-swift-package-manager) bên dưới.
:::
...
Thêm Adapty Flutter SDK 4.0 — hỗ trợ [flow](adapty-flow-builder) — vào `pubspec.yaml` của bạn:
...
Từ v4 trở đi, iOS SDK gốc không còn được phân phối qua CocoaPods nữa — plugin sẽ kéo nó qua **Swift Package Manager** ([kho spec của CocoaPods sẽ chỉ đọc vào tháng 12 năm 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Nếu bạn đang dùng Flutter 3.32–3.43, hãy bật hỗ trợ Swift Package Manager một lần:
...
- Nếu bạn dùng [Flow & Paywall Builder](adapty-flow-builder), trước tiên hãy [kích hoạt module AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) bên dưới, rồi làm theo [hướng dẫn nhanh](flutter-quickstart-paywalls).
...
Nếu bạn có kế hoạch sử dụng [Flow & Paywall Builder](adapty-flow-builder) và đã [cài đặt module AdaptyUI](sdk-installation-flutter#install-adapty-sdk), bạn cũng cần kích hoạt AdaptyUI:
...
:::note
Điều này áp dụng cho **SDK 3.x**, nơi các iOS SDK gốc được cài đặt dưới dạng CocoaPods. Từ SDK 4.0 trở đi, chúng được cài đặt dưới dạng Swift package, do đó việc ghi đè `SWIFT_VERSION` trong `post_install` sẽ không còn ảnh hưởng đến chúng nữa.
:::
