...
- **Giới hạn tốc độ**: Áp dụng theo API key và được chỉ định theo từng app, nên không có con số cố định nào áp dụng cho mọi app — xem [Giới hạn tốc độ](https://adapty.io/docs/vi/ss-authorization.md#rate-limits). Đây là giới hạn liên tục chứ không phải ngân sách theo phút, và một số endpoint bị giới hạn thấp hơn: [createVirtualCurrencyTransaction](https://adapty.io/docs/vi/api-adapty/operations/createVirtualCurrencyTransaction.md) cho phép tối đa 600 request mỗi phút trên mỗi app theo mặc định. Hãy cấu hình agent của bạn để thử lại với backoff khi gặp lỗi `429` và tuân theo `Retry-After` nếu có.
- **Key theo từng app**: Mỗi key chỉ hoạt động cho một app; hãy dùng đúng key tương ứng với từng app.
- **Bắt buộc có một định danh**: Mỗi request cần có `adapty-customer-user-id` hoặc `adapty-profile-id`.
