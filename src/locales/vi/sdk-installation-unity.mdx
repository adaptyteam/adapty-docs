...
Adapty SDK hỗ trợ iOS 13.0+, nhưng yêu cầu iOS 15.0+ để hoạt động với các paywall được tạo trong paywall builder. Adapty SDK 4.1 — bổ sung hỗ trợ cho [flow](adapty-flow-builder) — yêu cầu **iOS 15.0+** cho toàn bộ ứng dụng: một trình kiểm tra build trong Unity Editor sẽ dừng quá trình build iOS nếu deployment target thấp hơn mức này. Khi build cho iOS với SDK 4.x, bạn cũng cần **Xcode 26** trở lên — native iOS SDK được thêm vào dưới dạng Swift package và được build bằng Swift tools 6.2.

:::info
Từ phiên bản 3.17.0, Adapty Unity SDK sử dụng Billing Library v8 theo mặc định. Các phiên bản từ 3.15.0 đến 3.15.2 sử dụng v7, và cũng hoạt động khi một dependency khác trong Android build của bạn nâng Billing Library lên v8.

Việc tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google giới thiệu trong đó. Trước khi áp dụng một tính năng billing mới của Google Play, hãy xem [Sản phẩm trong Play Store](android-products).
:::
