...
Adapty SDK hỗ trợ iOS 13.0+, nhưng yêu cầu iOS 15.0+ để hoạt động với các paywall được tạo trong paywall builder. Adapty SDK 4.0 (beta) — bổ sung hỗ trợ [Flow Builder](adapty-flow-builder) — yêu cầu **iOS 15.0+** cho toàn bộ ứng dụng: một trình kiểm tra build trong Unity Editor sẽ dừng quá trình build iOS nếu deployment target thấp hơn mức này.

:::info
Adapty tương thích với Google Play Billing Library lên đến phiên bản 8.x. Theo mặc định, Adapty sử dụng Google Play Billing Library v7.0.0. Để dùng phiên bản mới hơn, hãy [ghi đè Billing dependency](https://developer.android.com/google/play/billing/integrate#dependency) trong bản build Android của bạn.

Tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google giới thiệu trong đó. Trước khi áp dụng tính năng thanh toán Google Play mới, hãy xem [Sản phẩm trên Play Store](android-products).
:::
...
SDK 4.0 — bổ sung hỗ trợ [Flow Builder](adapty-flow-builder) — là phiên bản pre-release. Để cài đặt qua Unity Package Manager, hãy thêm thẻ beta vào URL Git:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Nếu bạn cài đặt qua Unity package, hãy tải `adapty-unity-plugin-4.0.0-beta.1.unitypackage` từ [bản phát hành 4.0.0-beta.1](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.0.0-beta.1).

Hai thay đổi về cài đặt build đi kèm với v4 — iOS Adapty SDK gốc được khai báo là một remote Swift package và không còn cài đặt qua CocoaPods nữa:

- Cập nhật External Dependency Manager lên **1.2.188 trở lên** — các phiên bản cũ hơn không hỗ trợ Swift Package Manager dependencies. Đây là phiên bản SDK 4.0 khai báo là peer dependency, vì vậy Unity sẽ cảnh báo nếu dự án của bạn đang dùng phiên bản cũ hơn.
- Các bước CocoaPods ở trên (`iOS Resolver -> Install Cocoapods`, mở `Unity-iPhone.xcworkspace`) chỉ áp dụng cho SDK 3.x. Với SDK 4.0, EDM sẽ tự động thêm Swift package vào dự án Xcode được tạo ra.
- Đặt iOS deployment target là **15.0 trở lên**. Nếu không, build validator trong Unity Editor sẽ dừng quá trình build iOS.

Xem [hướng dẫn migration](migration-to-unity-sdk-v4) để biết danh sách đầy đủ các thay đổi trong SDK 4.0.
...
:::note
Trong SDK 4.0, các giao diện listener tuân theo quy ước tiền tố I- của C#: implement `IAdaptyEventListener` thay vì `AdaptyEventListener`. Các phương thức không thay đổi. Xem [hướng dẫn migration](migration-to-unity-sdk-v4) để biết thêm chi tiết.
:::
