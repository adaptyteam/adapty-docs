...
Adapty SDK hỗ trợ iOS 13.0+, nhưng yêu cầu iOS 15.0+ để hoạt động với các paywall được tạo trong paywall builder. Adapty SDK 4.1 — bổ sung hỗ trợ cho [flows](adapty-flow-builder) — yêu cầu **iOS 15.0+** cho toàn bộ ứng dụng: một trình kiểm tra build trong Unity Editor sẽ dừng quá trình build iOS nếu deployment target thấp hơn mức này. Để build cho iOS với SDK 4.x, bạn cũng cần **Xcode 26** trở lên — native iOS SDK được thêm vào dưới dạng Swift package được build với Swift tools 6.2.
