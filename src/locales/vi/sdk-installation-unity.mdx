...
- **Core Adapty**: Đây là SDK thiết yếu, bắt buộc phải có để Adapty hoạt động đúng trong ứng dụng của bạn.
- **AdaptyUI**: Module này cần thiết nếu bạn sử dụng [Adapty Paywall Builder](adapty-paywall-builder), công cụ no-code thân thiện giúp tạo paywall đa nền tảng một cách dễ dàng.

:::tip
Muốn xem ví dụ thực tế về cách tích hợp Adapty SDK vào ứng dụng di động? Hãy khám phá [ứng dụng mẫu](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Assets) của chúng tôi, minh họa toàn bộ quá trình cài đặt, bao gồm hiển thị paywall, thực hiện mua hàng và các chức năng cơ bản khác.
:::

## Yêu cầu \{#requirements\}

Adapty SDK hỗ trợ iOS 13.0+, nhưng yêu cầu iOS 15.0+ để hoạt động với các paywall được tạo trong paywall builder. Adapty SDK 4.1 — bổ sung hỗ trợ [Flow Builder](adapty-flow-builder) — yêu cầu **iOS 15.0+** cho toàn bộ ứng dụng: một trình kiểm tra build trong Unity Editor sẽ dừng quá trình build iOS nếu deployment target thấp hơn mức này.

:::info
Adapty Unity SDK 4.0 trở lên hoạt động với Google Play Billing Library v8.

Khả năng tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google đã giới thiệu trong phiên bản đó. Trước khi áp dụng một tính năng thanh toán mới của Google Play, hãy xem [Sản phẩm trên Play Store](android-products).
:::
...
SDK 4.1 — bổ sung hỗ trợ [Flow Builder](adapty-flow-builder) — là bản phát hành ổn định đầu tiên của dòng 4.x. Để cài đặt qua Unity Package Manager, thêm tag phiên bản vào Git URL:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

Nếu bạn cài đặt qua Unity package, hãy tải `adapty-unity-plugin-4.1.0.unitypackage` từ [bản phát hành 4.1.0](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0).
