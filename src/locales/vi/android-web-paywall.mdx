...
metadataTitle: "Chấp nhận thanh toán qua web cho ứng dụng Android | Tài liệu Adapty"
...
1. `openWebPaywall(product)` tạo URL theo paywall và thêm thông tin sản phẩm vào URL.
2. `openWebPaywall(paywall)` tạo URL theo paywall mà không thêm thông tin sản phẩm vào URL. Trong SDK v4, phương thức này nhận `AdaptyFlowPaywall` — một biến thể paywall của flow đã fetch. Hãy kiểm tra `flow.paywalls` không rỗng trước khi truy cập vào nó, ví dụ `flow.paywalls[0]`.
:::
