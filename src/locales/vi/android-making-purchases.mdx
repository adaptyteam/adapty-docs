---
title: "Thực hiện mua hàng trong ứng dụng di động với Android SDK"
...
Hiển thị paywall trong ứng dụng di động của bạn là bước thiết yếu để cung cấp cho người dùng quyền truy cập vào nội dung hoặc dịch vụ cao cấp. Tuy nhiên, chỉ riêng việc hiển thị paywall mới tự động hỗ trợ thanh toán khi Adapty tự render màn hình — tức là, một [flow](adapty-flow-builder), hoặc paywall từ [Paywall Builder cũ](adapty-paywall-builder).

Nếu bạn tự render màn hình bằng code của mình, bạn phải sử dụng một phương thức riêng gọi là `.makePurchase()` để hoàn tất giao dịch mua và mở khóa nội dung mong muốn. Phương thức này là cổng kết nối để người dùng tương tác với các paywall và thực hiện giao dịch theo ý muốn.
...
:::note
**Adapty có render màn hình của bạn không?** Đối với paywall dạng [flow](adapty-flow-builder) hoặc [Paywall Builder](adapty-paywall-builder), các giao dịch mua hàng sẽ được xử lý tự động — bạn có thể bỏ qua bước này.

**Cần hướng dẫn từng bước?** Xem [hướng dẫn quickstart](android-implement-paywalls-manually) để có hướng dẫn triển khai đầy đủ từ đầu đến cuối.
:::
...
:::warning
Chỉ truyền `subscriptionUpdateParams` khi người dùng chuyển từ một gói đăng ký đang hoạt động được mua trên cùng tài khoản Google. Đối với giao dịch mua mới, hãy gọi `makePurchase` mà không có tham số này — việc truyền vào sẽ khiến giao dịch mua thất bại với lỗi `CURRENT_SUBSCRIPTION_TO_UPDATE_NOT_FOUND_IN_HISTORY`, trong đó thông báo lỗi lại trỏ đến một gói đăng ký không còn hoạt động hoặc một tài khoản Google khác.
:::
