...
- **Flows được xây dựng trong builder**: `createFlowView` không nhận tham số locale, và SDK không đọc locale của thiết bị. Flow sẽ hiển thị theo [locale mặc định](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — trên iOS, là `en` khi flow có bản nội địa hóa tiếng Anh.
- **Paywall tùy chỉnh (remote config)**: `getFlow` trả về tất cả các bản nội địa hóa đã cấu hình trong `flow.remoteConfigs`. Mỗi mục có một mã `locale` và nội dung config (`data` dạng chuỗi, hoặc `dictionary` đã được parse). Hãy chọn mục phù hợp với người dùng, với logic fallback do bạn tự định nghĩa:
