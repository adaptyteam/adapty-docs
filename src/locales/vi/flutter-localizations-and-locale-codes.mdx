...
Việc khớp là chính xác tuyệt đối. SDK so sánh mã bạn truyền vào với mã bản địa hóa của flow theo từng ký tự: không thay đổi chữ hoa/thường, không thay thế dấu gạch dưới (`_`) bằng dấu gạch ngang (`-`), và không tự động dùng thẻ ngôn ngữ con. Với flow có bản địa hóa `pt-br`, chỉ `pt-br` mới khớp: `pt-BR`, `pt_BR`, và `pt-PT` đều không khớp.

Khi mã không khớp với bất kỳ localization nào, flow sẽ tự động hiển thị theo [ngôn ngữ mặc định](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — SDK không trả về lỗi và không ghi cảnh báo.

Khi mã khớp, Adapty gộp localization đó với localization mặc định: các chuỗi và assets mà localization được khớp không định nghĩa sẽ được lấy từ localization mặc định.

Bỏ qua mã locale không giống với việc yêu cầu localization mặc định của flow: SDK sẽ thay thế bằng `en` cố định. Một flow có locale mặc định là `de` vẫn hiển thị bằng `en` nếu nó có bản localization `en`, và chỉ fallback về `de` khi không có.

:::warning
Truyền mã ngôn ngữ chính xác như đã cấu hình trong dashboard — các thẻ con viết thường, phân cách bằng dấu gạch ngang. Đừng truyền trực tiếp identifier ngôn ngữ của hệ thống: `Platform.localeName` trả về `pt_BR` và `PlatformDispatcher.instance.locale.toLanguageTag()` trả về `pt-BR`, cả hai đều sẽ fallback về bản địa hóa mặc định. Hãy chuyển đổi giá trị trong ứng dụng của bạn trước khi truyền vào.
:::
