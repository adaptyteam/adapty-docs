...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Khởi tạo MMP hoặc analytics SDK (AppsFlyer, Adjust, PostHog, Branch)                                          | Khởi động app, đầu tiên                                                                 | Chờ callback UID của MMP, ví dụ `getAppsFlyerUID`.                                             |
| 2a   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                       | Khởi động app, sau bước 1, nếu bạn có customer user ID                                  | Khuyến nghị. Không bao giờ tạo hồ sơ người dùng ẩn danh.                                       |
| 2b   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").build())` không có `customerUserId`                     | Khởi động app, sau bước 1, nếu bạn chưa có customer user ID (hoặc không thu thập)       | Adapty sẽ tạo hồ sơ người dùng ẩn danh.                                                        |
| 3    | `Adapty.setIntegrationIdentifier("appsflyer_id", uid)` cho từng MMP                                           | Sau bước 2, trước mọi lệnh gọi hành động của người dùng                                 | Bắt buộc để MMP ID được gắn vào đúng hồ sơ người dùng.                                         |
| 4    | `Adapty.identify("YOUR_USER_ID") { error -> ... }`                                                            | Sau bước 3 (hoặc bước 2 nếu không có MMP), trước bước 5 — chỉ trên nhánh 2b có xác thực | Dùng completion callback. Các lệnh gọi đồng thời trong `identify` có thể rơi vào hồ sơ người dùng ẩn danh. |
| 5    | `getPaywall`, `getPaywallProducts`, `restorePurchases`, `makePurchase`, `updateExternalAttribution`, `updateProfile`  | Sau bước 4 nếu bạn gọi `identify`; ngược lại sau bước 3 (hoặc bước 2 nếu không có MMP) | Các lệnh gọi này cần một hồ sơ người dùng ổn định.                                              |
