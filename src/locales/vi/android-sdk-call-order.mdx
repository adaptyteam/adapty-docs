...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | Khởi tạo MMP hoặc analytics SDK của bạn (AppsFlyer, Adjust, PostHog, Branch)                                     | Khi khởi động ứng dụng, thực hiện đầu tiên                                                      | Chờ callback UID của MMP, ví dụ `getAppsFlyerUID`.                                |
| 2a   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                       | Khi khởi động ứng dụng, sau bước 1, nếu bạn đã có customer user ID                             | Được khuyến nghị. Sẽ không có hồ sơ người dùng ẩn danh nào được tạo.                                             |
| 2b   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").build())` không có `customerUserId`                      | Khi khởi động ứng dụng, sau bước 1, nếu bạn chưa có customer user ID (hoặc không thu thập) | Adapty sẽ tạo một hồ sơ người dùng ẩn danh.                                                           |
| 3    | `Adapty.setIntegrationIdentifier(AdaptyIntegrationIdentifier.appsflyerId(uid)) { error -> ... }`               | Sau bước 2, trước bất kỳ lệnh gọi nào do người dùng thực hiện                                              | Bắt buộc để MMP ID được gán vào đúng hồ sơ người dùng. Truyền danh sách identifier để thiết lập nhiều MMP trong một lần gọi. |
| 4    | `Adapty.identify("YOUR_USER_ID") { error -> ... }`                                                            | Sau bước 3 (hoặc bước 2 nếu không dùng MMP), trước bước 5 — chỉ trên đường dẫn 2b có xác thực | Sử dụng completion callback. Các lệnh gọi đồng thời trong quá trình `identify` có thể bị gán vào hồ sơ người dùng ẩn danh. |
| 5    | `getPaywall`, `getPaywallProducts`, `restorePurchases`, `makePurchase`, `updateExternalAttribution`, `updateProfile`  | Sau bước 4 nếu bạn gọi `identify`; ngược lại sau bước 3 (hoặc bước 2 nếu không dùng MMP)      | Các lệnh gọi này cần một hồ sơ người dùng ổn định.                                                             |
