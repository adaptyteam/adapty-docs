...
Nếu bạn đã tạo một flow, hoặc một paywall trong Paywall Builder cũ, bạn không cần lo lắng về việc render nó trong code ứng dụng di động để hiển thị cho người dùng. Flow hoặc paywall như vậy đã bao gồm cả nội dung cần hiển thị lẫn cách thức hiển thị.

Để lấy đối tượng `AdaptyUI.FlowConfiguration` được sử dụng bên dưới, hãy xem [Lấy flow và paywall](get-pb-paywalls).
