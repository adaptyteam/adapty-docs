...
metadataTitle: "Migrate Adapty Flutter SDK lên v4.0 | Tài liệu Adapty"
---

Adapty Flutter SDK 4.0 giới thiệu flow và đổi tên các API paywall cho phù hợp. Các API mới hoạt động với flow, đồng thời vẫn tương thích với paywall từ builder cũ — không cần thay đổi cấu hình trên Adapty Dashboard.

:::note
Hướng dẫn này đề cập đến việc migration từ 3.x → 4.0. SDK 4.1 thay đổi thêm một số API — Adapty Attribution trở thành opt-in, các phương thức attribution bên ngoài được đổi tên, và định dạng file dự phòng thay đổi một lần nữa. Sau khi hoàn thành ở đây, hãy xem [Migrate sang v4.1](migration-to-flutter-sdk-41).
:::
...
`logShowPaywall` đã được đổi tên thành `logShowFlow` và bây giờ nhận một `AdaptyFlow`. Sự kiện vẫn được ghi lại theo cùng một biến thể, vì vậy các chỉ số funnel và A/B test hiện có vẫn hoạt động bình thường mà không cần thay đổi trên dashboard.
...
Đổi tên phương thức và truyền `AdaptyFlow` qua tham số `flow`. Các tham số còn lại (`loadTimeout`, `preloadProducts`, `customTags`, `customTimers`, `customAssets`, `productPurchaseParams`) không thay đổi, tương tự với các phương thức của view như `present`, `dismiss`, và `showDialog`:

:::note
`customTimers` vẫn tồn tại, nhưng chỉ ảnh hưởng đến các paywall Paywall Builder cũ. Bộ đếm ngược [countdown timer](flow-timer) của một flow chạy theo hành vi được thiết lập trong Flow & Paywall Builder, vì vậy flow sẽ bỏ qua bất kỳ giá trị nào bạn truyền vào đây.
:::
...
- `setFlowsEventsObserver` (và `setOnboardingsEventsObserver`) hiện chấp nhận `null` để hủy đăng ký observer đã thiết lập trước đó, giúp SDK không còn giữ tham chiếu đến nó nữa.
- Callback tùy chọn mới `flowViewDidReceiveAnalyticEvent` báo cáo các sự kiện analytics từ một flow, bắt đầu bằng sự kiện xem màn hình cho mỗi màn hình người dùng mở. Xem [Theo dõi lượt xem màn hình flow](flutter-flow-screen-views).

v4 cũng bổ sung các tính năng bạn có thể kích hoạt theo nhu cầu:
...
API onboarding cũ đã bị deprecated trong v4.0 và được thay thế bằng [Flow & Paywall Builder](adapty-flow-builder). API này vẫn hoạt động bình thường, và IDE của bạn sẽ đánh dấu các symbol đã deprecated thông qua annotation `@Deprecated` — không có cảnh báo nào xuất hiện ở runtime. Các symbol này sẽ bị xóa trong phiên bản tương lai, vì vậy hãy lên kế hoạch migrate các onboarding của bạn sang Flow & Paywall Builder.
