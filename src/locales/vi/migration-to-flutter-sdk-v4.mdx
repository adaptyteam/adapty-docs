---
title: "Migrate Adapty Flutter SDK sang v4.0"
description: "Migrate sang Adapty Flutter SDK v4.0 bằng cách thay thế paywall API bằng flow API, tương thích với cả Flow Builder và Paywall Builder."
metadataTitle: "Migration sang Adapty Flutter SDK v4.0 | Tài liệu Adapty"
---

Adapty Flutter SDK 4.0 giới thiệu flows và đổi tên các paywall API tương ứng. Các API mới hoạt động với cả Flow Builder mới lẫn Paywall Builder hiện có — không cần thay đổi cấu hình nào trên Adapty Dashboard.
...
Đổi tên phương thức và truyền `AdaptyFlow` qua tham số `flow`. Các tham số khác (`loadTimeout`, `preloadProducts`, `customTags`, `customTimers`, `customAssets`, `productPurchaseParams`) không thay đổi, các phương thức view `present`, `dismiss`, và `showDialog` cũng vậy:

:::note
`customTimers` vẫn tồn tại, nhưng chỉ có tác dụng với paywall thuộc Paywall Builder cũ. [Bộ đếm ngược](flow-timer) của một flow chạy theo hành vi được thiết lập trong Flow Builder, nên flow sẽ bỏ qua bất kỳ giá trị nào bạn truyền vào đây.
:::
