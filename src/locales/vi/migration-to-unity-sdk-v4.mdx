...
| `Adapty.SetFallbackPaywalls(...)` (đã deprecated trong v3) | đã bị xóa — dùng `Adapty.SetFallback(fileName, ...)` |
| `Builder.SetIDFACollectionDisabled(...)` (đã deprecated trong v3) | đã bị xóa — dùng `Builder.SetAppleIDFACollectionDisabled(...)` |
...
|---|---|---|
| `RemoteConfig` (đơn, nullable) | `RemoteConfigs` (danh sách) | Một flow mang một remote config cho mỗi ngôn ngữ được cấu hình. Đọc mục phù hợp với người dùng từ `flow.RemoteConfigs`. Shortcut `flow.RemoteConfig` trả về mục đầu tiên. |
...
| `ProductIdentifiers`, `VendorProductIds` | giữ nguyên | Trên `AdaptyFlow`, các thuộc tính này tổng hợp sản phẩm từ tất cả các biến thể paywall. Mỗi biến thể cũng hiển thị `ProductIdentifiers` và `VendorProductIds` riêng. Để lấy sản phẩm, tiếp tục gọi `GetPaywallProducts(flow)`. |
...
- `Adapty.SetObserverModeResolver(...)` với `IAdaptyUIObserverModeResolver` — xử lý các giao dịch mua và khôi phục được khởi tạo từ flow khi SDK chạy ở [Observer mode](implement-observer-mode-unity). Trước đây tính năng này chỉ có trên SDK iOS và Android gốc. Xem [Hiển thị flow trong Observer mode](unity-present-flows-in-observer-mode).
- `Adapty.SetSystemRequestsHandler(...)` với `IAdaptyUISystemRequestsHandler` — dành cho các yêu cầu hệ thống từ flow: yêu cầu quyền hệ điều hành (`FlowViewDidAskPermission`) và yêu cầu đánh giá ứng dụng (`FlowViewDidRequestAppReview`). Hiện tại flow chưa kích hoạt các yêu cầu này, nên bạn chưa cần đăng ký handler.
- `AdaptyUICreateFlowViewParameters.Locale` — hiển thị flow hoặc paywall với một [bản địa hóa Builder](add-paywall-locale-in-adapty-paywall-builder) cụ thể thay vì bản địa hóa mà Adapty tự chọn từ thiết bị. Flow được bản địa hóa khi view của nó được tạo, vì vậy đây là nơi duy nhất để chọn bản địa hóa, và view đã tạo sẽ báo cáo bản địa hóa mà nó được xây dựng với trong `view.Locale`. Xem [Sử dụng bản địa hóa và mã ngôn ngữ](unity-localizations-and-locale-codes).
- Callback `FlowViewDidReceiveAnalyticEvent` mới trên `IAdaptyFlowsEventsListener` được dành riêng cho các sự kiện phân tích tùy chỉnh từ flow. Hiện tại flow chưa gửi các sự kiện này đến code của bạn, vì vậy hãy triển khai nó với phần thân rỗng.
- `AdaptyUI.OpenUrl(url, openIn, ...)` và `AdaptyUI.RequestAppReview(...)` — xử lý gốc phía sau các hành động `open_url` và yêu cầu đánh giá ứng dụng. Gọi `OpenUrl` từ `FlowViewDidPerformAction` để giữ nguyên hành vi URL mặc định; `RequestAppReview` hỗ trợ lời nhắc đánh giá ứng dụng mặc định, mà flow chưa kích hoạt.
