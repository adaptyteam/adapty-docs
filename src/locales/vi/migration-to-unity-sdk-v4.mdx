---
title: "Migrate Adapty Unity SDK sang v4.0"
description: "Migrate sang Adapty Unity SDK v4.0 (beta) bằng cách thay thế API paywall bằng API flow, tương thích với cả Flow Builder và Paywall Builder."
metadataTitle: "Migrate sang Adapty Unity SDK v4.0 (beta) | Tài liệu Adapty"
---

Adapty Unity SDK 4.0 (beta) giới thiệu các flow và đổi tên API paywall tương ứng. Các API mới hoạt động với cả Flow Builder mới và Paywall Builder hiện có — không cần thay đổi cài đặt nào trên Adapty Dashboard.
...
- `Adapty.SetObserverModeResolver(...)` với `IAdaptyUIObserverModeResolver` — xử lý các giao dịch mua và khôi phục được khởi tạo từ flow khi SDK chạy ở [Observer mode](implement-observer-mode-unity). Trước đây tính năng này chỉ có trong SDK iOS và Android gốc. Xem [Present flows in Observer mode](unity-present-flows-in-observer-mode).
- `Adapty.SetSystemRequestsHandler(...)` với `IAdaptyUISystemRequestsHandler` — dành cho các yêu cầu hệ thống từ flow: lời nhắc quyền hệ điều hành (`FlowViewDidAskPermission`) và yêu cầu đánh giá ứng dụng (`FlowViewDidRequestAppReview`). Flow chưa kích hoạt các yêu cầu này, vì vậy bạn không cần đăng ký handler.
- `AdaptyUICreateFlowViewParameters.Locale` (đặt bằng `SetLocale`) — hiển thị flow hoặc paywall với một [bản dịch Builder](add-paywall-locale-in-adapty-paywall-builder) cụ thể thay vì bản dịch mặc định của flow. Flow được dịch khi view của nó được tạo, do đó đây là nơi duy nhất để chọn bản dịch, và view đã tạo sẽ báo cáo bản dịch mà nó được xây dựng trong `view.Locale`. Xem [Sử dụng bản dịch và mã ngôn ngữ](unity-localizations-and-locale-codes).
- Callback `FlowViewDidReceiveAnalyticEvent` mới trên `IAdaptyFlowsEventsListener` được dành riêng cho các sự kiện phân tích tùy chỉnh từ flow. Flow chưa phát các sự kiện này đến code của bạn, vì vậy hãy triển khai nó với phần thân rỗng.
- `AdaptyUI.OpenUrl(url, openIn, ...)` và `AdaptyUI.RequestAppReview(...)` — xử lý gốc đằng sau các hành động `open_url` và yêu cầu đánh giá ứng dụng. Gọi `OpenUrl` từ `FlowViewDidPerformAction` để giữ hành vi URL mặc định; `RequestAppReview` hỗ trợ lời nhắc đánh giá ứng dụng mặc định, mà flow chưa kích hoạt.
