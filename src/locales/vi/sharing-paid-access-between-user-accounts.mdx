---
title: "Chia sẻ quyền truy cập có trả phí giữa các tài khoản người dùng"
description: "Chia sẻ quyền truy cập có trả phí giữa các tài khoản người dùng khác nhau để hỗ trợ người dùng có nhiều thiết bị hoặc nhiều hồ sơ ứng dụng"
metadataTitle: "Chia sẻ quyền truy cập có trả phí giữa các tài khoản người dùng khác nhau để đảm bảo người dùng không mất quyền truy cập vào các giao dịch mua | Tài liệu Adapty"
---

Khi người dùng thực hiện mua hàng, Adapty sẽ gán một [mức độ truy cập](access-level) mới cho [hồ sơ người dùng](identifying-users) đang hoạt động của họ. Mức độ truy cập này cho phép người mua truy cập vào nội dung có trả phí.

Hồ sơ người dùng của người mua có thể bị thay đổi ngoài ý muốn nếu họ cài lại ứng dụng hoặc đăng nhập vào một tài khoản trong ứng dụng mới. Để đảm bảo quyền truy cập không bị gián đoạn, Adapty tự động chia sẻ mức độ truy cập của người dùng giữa hồ sơ ban đầu và các hồ sơ tiếp theo.

Cách tiếp cận này phù hợp với hầu hết các ứng dụng. Nhưng nếu logic nghiệp vụ của bạn yêu cầu, bạn có thể chọn chính sách chia sẻ quyền truy cập có trả phí hạn chế hơn.

Mở trang [Cài đặt chung](https://app.adapty.io/settings/general) để thiết lập chính sách chia sẻ mức độ truy cập. Để thuận tiện cho việc kiểm tra, bạn có thể thay đổi cài đặt này chỉ cho [môi trường sandbox](#sharing-paid-access-on-sandbox).

<Details>

:::important
Nếu ứng dụng của bạn không xác thực người dùng, bạn có thể bỏ qua cài đặt này. Các hồ sơ người dùng ẩn danh được liên kết với cùng một tài khoản cửa hàng *luôn luôn* chia sẻ mức độ truy cập của họ.
:::

    <summary>Tôi nên chọn chính sách chia sẻ truy cập nào? (Nhấn để mở rộng)</summary>

| Ứng dụng của tôi...                                          | Lựa chọn tốt nhất                                           |
    | ------------------------------------------------------------ | ------------------------------------------------------------ |
    | Không có tính năng xác thực, chỉ sử dụng ID hồ sơ ẩn danh của Adapty. | Sử dụng cài đặt **Enabled (default)**. |
    | Có thể xác thực người dùng, nhưng cho phép họ mua hàng mà không cần tài khoản. | Bật cài đặt **Transfer access to new user**. Người dùng sẽ có thể đăng ký và nhận các giao dịch mua ẩn danh. |
    ...
    | Yêu cầu khách hàng tạo tài khoản trước khi mua, với quy tắc nghiêm ngặt ràng buộc giao dịch mua với một Customer User ID duy nhất. | **Tắt** tính năng chia sẻ mức độ truy cập. |
</Details>
...
:::warning
Việc chuyển giao sẽ di chuyển chính mức độ truy cập, không chỉ giao dịch mua trên cửa hàng. Khi một lần khôi phục chuyển quyền truy cập sang hồ sơ người dùng mới, Adapty sẽ thu hồi mức độ truy cập đó từ hồ sơ người dùng cũ — ngay cả khi một cửa hàng khác đã cấp quyền đó — ví dụ như một giao dịch mua qua web (Stripe) được ánh xạ tới cùng một mức độ truy cập. Nếu các giao dịch mua qua web và in-app cần độc lập với nhau, hãy ánh xạ các sản phẩm của chúng tới các mức độ truy cập riêng biệt.
:::
