...
:::warning
**Onboarding đã bị deprecated trong SDK v4 và sẽ bị xóa trong các phiên bản tới.** Chúng không còn nhận được bản sửa lỗi hay cải tiến nào nữa. Hãy sử dụng [flows](unity-get-pb-paywalls) thay thế: không giống onboarding chạy bên trong WebView, flows render trực tiếp trên thiết bị — mang lại animation mượt mà hơn, giao diện native nhất quán, thời gian tải nhanh hơn và không phụ thuộc vào WebView runtime. Xem [Lấy flows & paywalls](unity-get-pb-paywalls) và [Hiển thị flows & paywalls](unity-present-paywalls) để bắt đầu.
:::
...
Các onboarding được cấu hình bằng builder sẽ tạo ra các sự kiện mà ứng dụng của bạn có thể xử lý. Tìm hiểu cách xử lý các sự kiện này bên dưới.
