...
## Triển khai nội địa hóa \{#implementing-localizations\}

Trong SDK v4, bạn không cần truyền mã locale khi lấy flow — `getFlow` trả về flow cùng với tất cả các bản nội địa hóa của nó.

- **Flow được xây dựng trong builder**: `createFlowView` không nhận tham số locale, và SDK không đọc locale của thiết bị. Flow hiển thị theo [locale mặc định](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — trên iOS, hiển thị bằng `en` khi flow có bản dịch tiếng Anh. Để xác nhận Adapty đã áp dụng bản dịch nào, hãy đọc `view.locale` trên controller mà `createFlowView` trả về. Tính năng này yêu cầu React Native SDK 4.0.2 trở lên và trả về `undefined` trên các phiên bản cũ hơn.
- **Paywall tùy chỉnh (remote config)**: `getFlow` trả về tất cả các bản dịch đã được cấu hình trong `flow.remoteConfigs`. Mỗi mục có một mã `lang` và một đối tượng `data`. Chọn mục phù hợp với người dùng, với cơ chế dự phòng do bạn tự định nghĩa:
