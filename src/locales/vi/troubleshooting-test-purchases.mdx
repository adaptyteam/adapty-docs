...
## Lỗi mua hàng sandbox bị từ chối do không được ủy quyền \{#the-sandbox-purchase-is-refused-as-unauthorized\}

Cửa hàng hiển thị thông báo như `You are not authorized to make purchases of this in-app in sandbox at this time`. Apple từ chối giao dịch, không phải Adapty. Hãy kiểm tra các điều sau trên thiết bị test và trong App Store Connect:

- **In-app purchase được cho phép trên thiết bị**: Trong **Settings** > **Screen Time** > **Apps & Websites** > **App Store**, mục **In-app Purchases** phải được đặt thành **Allowed**. Apple mô tả cài đặt này tại [Turn off Apple In-App Purchases from Screen Time](https://support.apple.com/en-us/102470).
- **Tài khoản sandbox được đăng nhập đúng vị trí**: Đăng nhập tại [**Settings** > **Developer** > **Sandbox Apple Account**](test-purchases-in-sandbox#step-4-switch-to-sandbox-test-account), không phải tại phần cài đặt Apple Account chính. Hướng dẫn của Apple là "dùng tùy chọn đăng nhập Sandbox trong Settings, không phải trong cài đặt Apple Account chính của bạn."
- **Địa chỉ email của tester không phải là Apple Account đã tồn tại**: Apple cho biết việc dùng lại một địa chỉ như vậy "sẽ gây lỗi trong quá trình kiểm thử." Bạn không thể chỉnh sửa địa chỉ email của tester sau khi đã tạo, vì vậy hãy [tạo tester mới](test-purchases-in-sandbox#step-1-create-sandbox-test-account-in-app-store-connect) với địa chỉ khác.
