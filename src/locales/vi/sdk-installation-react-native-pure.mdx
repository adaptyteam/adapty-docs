...
description: "Hướng dẫn từng bước cài đặt Adapty SDK trên React Native cho ứng dụng dựa trên gói đăng ký."
rank: 60
---
...
:::important
Hướng dẫn này chỉ áp dụng cho **các dự án React Native thuần (không dùng Expo)**.
Nếu bạn đang dùng **Expo**, hãy làm theo [hướng dẫn cài đặt Expo](sdk-installation-react-native-expo).
:::
...
| Yêu cầu | Phiên bản |
|---|---|
| React Native | 0.75 trở lên. [Tích hợp SPM của React Native](https://reactnative.dev/blog/2026/08/11/react-native-0.87#experimental-swift-package-manager-support-for-ios) cần 0.87 trở lên. |
| iOS | 15.0 trở lên |
| Swift | 6.2 trở lên, đi kèm với Xcode 26 |

:::info
Adapty SDK đã tích hợp sẵn Google Play Billing Library — bạn không cần thêm thư viện này vào ứng dụng của mình.

Từ phiên bản 3.17.0, Adapty React Native SDK sử dụng Billing Library v8 theo mặc định. Các phiên bản từ 3.15.0 đến 3.15.6 sử dụng v7, và cũng hoạt động khi một dependency khác trong bản build Android của bạn nâng Billing Library lên v8.

Tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google đã giới thiệu trong đó. Trước khi áp dụng một tính năng billing mới của Google Play, hãy xem [Sản phẩm trên Play Store](android-products).
:::
...
### Thiết lập dự án iOS của bạn \{#set-up-your-ios-project\}

Adapty phân phối các iOS SDK native (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) dưới dạng Swift package — v4 đã ngừng hỗ trợ CocoaPods ([kho spec của CocoaPods sẽ chuyển sang chế độ chỉ đọc vào tháng 12 năm 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Cả hai tùy chọn đều cài đặt các Swift package đó; điểm khác biệt nằm ở cách thức thực hiện việc cài đặt.
...
- **Tích hợp SPM của React Native (SDK 4.1 trở lên)**: Dự án iOS của bạn bỏ CocoaPods, và React Native tự giải quyết các Swift package từ manifest `Package.swift` mà Adapty cung cấp. [React Native đã thêm tích hợp này trong phiên bản 0.87](https://reactnative.dev/blog/2026/08/11/react-native-0.87#experimental-swift-package-manager-support-for-ios) dưới dạng bản xem trước và chưa khuyến nghị dùng cho production.

<Tabs groupId="ios-package-manager" queryString>

<TabItem value="cocoapods" label="CocoaPods" default>

Khai báo dynamic frameworks trong target `ios/Podfile` của bạn:
...
:::warning
Chuyển từ liên kết tĩnh mặc định sang dynamic frameworks có thể xung đột với các thư viện chưa hỗ trợ modular headers, và không tương thích với Flipper. Xem [Migrate Adapty React Native SDK to v4](migration-to-react-native-sdk-v4) để biết thêm chi tiết.
:::
...
React Native đọc file manifest `Package.swift` của Adapty và tự động xử lý các iOS SDK native. Bạn không cần thêm gì vào dự án Xcode.

Giữ nguyên `ios/Podfile` dù file này không cài đặt pod nào. React Native CLI xác định vị trí dự án iOS bằng cách tìm Podfile, nếu không có nó, autolinking sẽ bỏ qua iOS hoàn toàn.

Nếu app của bạn dùng các pod khác ngoài React Native, hãy giữ chúng trong Podfile, chạy `pod install`, và mở `.xcworkspace` thay vì `.xcodeproj`. Workspace bao gồm cả hai trình quản lý gói.

:::note
Trên một bản clone mới hoặc CI checkout, hãy chạy `npx react-native spm` một lần trước khi build. Xcode project sẽ đồng bộ lại các dependency trong quá trình build, sau khi Xcode đã giải quyết xong package graph, vì vậy một lần build bắt đầu ngay sau khi cài đặt có thể thành công mà không có Adapty.
:::

Để quay lại CocoaPods, hãy chạy `npx react-native spm deinit`, sau đó chạy `pod install`.
...
- Nếu bạn sử dụng [Flow & Paywall Builder](adapty-flow-builder), hãy xem [quickstart](react-native-quickstart-paywalls).
- Nếu bạn tự xây dựng giao diện paywall, hãy xem [quickstart cho paywall tùy chỉnh](react-native-quickstart-manual).

:::tip
Để tránh lỗi kích hoạt trong môi trường phát triển, hãy xem [các mẹo](#development-environment-tips).
:::

## Kích hoạt module AdaptyUI của Adapty SDK \{#activate-adaptui-module-of-adapty-sdk\}

Nếu bạn có kế hoạch sử dụng [Flow & Paywall Builder](adapty-flow-builder), bạn cần module AdaptyUI. Module này sẽ được kích hoạt tự động khi bạn kích hoạt module cốt lõi; bạn không cần thực hiện thêm bất kỳ thao tác nào khác.
...
- Nó **không render** flow hoặc paywall cũ được tạo bằng builder (AdaptyUI).
...
#### Lỗi phiên bản Kotlin Gradle plugin trên React Native dưới 0.73 \{#kotlin-gradle-plugin-version-error-on-react-native-below-073\}
