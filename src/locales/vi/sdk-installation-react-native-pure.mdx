...
:::important
Kể từ phiên bản v4, Adapty React Native SDK không còn hỗ trợ cài đặt các dependency native thông qua CocoaPods nữa. Nếu bạn cần v4 trở lên (để sử dụng [Flow Builder](adapty-flow-builder)), hãy làm theo hướng dẫn [Adapty SDK 4.0 (beta): enable Swift Package Manager](#adapty-sdk-40-beta-enable-swift-package-manager) bên dưới.
:::
...
### Adapty SDK 4.0 (beta): bật Swift Package Manager

React Native SDK 4.0 — bổ sung hỗ trợ [Flow Builder](adapty-flow-builder) — là phiên bản pre-release yêu cầu **React Native 0.75 trở lên**. Cài đặt đúng phiên bản (npm không tự phân giải pre-release qua cú pháp caret/tilde):
...
```
v4 kéo các native iOS SDK (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) thông qua Swift Package Manager thay vì các sub-dependency của CocoaPods ([kho spec của CocoaPods sẽ chuyển sang chế độ chỉ đọc vào tháng 12 năm 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). SPM yêu cầu dynamic framework — thêm đoạn sau vào target trong `ios/Podfile`, rồi cài lại pods:
...
```
Nếu trước đây bạn đã kéo `Adapty`, `AdaptyUI` hoặc `AdaptyPlugin` dưới dạng sub-dependencies CocoaPods, hãy xóa bất kỳ dòng `pod 'Adapty'`, `pod 'AdaptyUI'` hoặc `pod 'AdaptyPlugin'` nào khỏi `Podfile` của bạn trước.

:::warning
Việc chuyển từ liên kết tĩnh (static linkage) mặc định sang dynamic frameworks có thể xung đột với các thư viện chưa hỗ trợ modular headers, và không tương thích với Flipper. Xem [Migrate Adapty React Native SDK sang v4](migration-to-react-native-sdk-v4) để biết thêm chi tiết.
:::
