...
:::info
Mặc định, Adapty SDK 4.0.2 trở lên hoạt động với Google Play Billing Library v8. Các phiên bản SDK cũ hơn hoạt động với v7.0.0 theo mặc định và tương thích với Billing Library lên đến 8.x — để buộc dùng phiên bản mới hơn, hãy thêm thủ công [dependency](https://developer.android.com/google/play/billing/integrate#dependency).

Khả năng tương thích với một phiên bản Billing Library không có nghĩa là Adapty hỗ trợ mọi tính năng mà Google giới thiệu trong đó. Trước khi áp dụng một tính năng Google Play billing mới, hãy xem [Sản phẩm trong Play Store](android-products).
:::
