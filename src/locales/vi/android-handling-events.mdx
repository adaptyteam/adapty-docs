...
:::important
Hướng dẫn này đề cập đến việc xử lý sự kiện cho các giao dịch mua, khôi phục, chọn sản phẩm và hiển thị flow. Bạn cũng cần triển khai xử lý nút bấm (đóng flow, mở liên kết, v.v.). Xem [hướng dẫn xử lý hành động nút bấm](android-handle-paywall-actions) của chúng tôi để biết thêm chi tiết.
:::
...
Nếu bạn cần kiểm soát hoặc theo dõi các tiến trình diễn ra trên màn hình mua hàng, hãy triển khai các phương thức `AdaptyFlowEventListener`.
...
Phương thức này sẽ không được gọi trong chế độ Observer. Tham khảo chủ đề [Android - Hiển thị flow trong chế độ Observer](android-present-paywall-builder-paywalls-in-observer-mode) để biết thêm chi tiết.

#### Mua hàng thành công, bị hủy hoặc đang chờ xử lý \{#successful-canceled-or-pending-purchase\}

Nếu giao dịch mua hàng thành công, phương thức này sẽ được gọi:
...
Chúng tôi khuyên bạn nên đóng màn hình trong trường hợp đó.

Phương thức này sẽ không được gọi trong Observer mode. Tham khảo chủ đề [Android - Hiển thị flow trong Observer mode](android-present-paywall-builder-paywalls-in-observer-mode) để biết thêm chi tiết.
...
Phương thức này sẽ không được gọi trong chế độ Observer. Tham khảo chủ đề [Android - Present flows in Observer mode](android-present-paywall-builder-paywalls-in-observer-mode) để biết thêm chi tiết.

#### Hoàn tất điều hướng thanh toán web \{#finished-web-payment-navigation\}

Phương thức này được gọi sau khi thực hiện thử mở [web paywall](web-paywall) cho một sản phẩm cụ thể. Điều này bao gồm cả các lần điều hướng thành công và thất bại:
...
| Tham số | Mô tả |
|:------------|:---------------------------------------------------------------------------------------------------|
| **product** | Một `AdaptyPaywallProduct` cho paywall web được mở. Có thể là `null`. |
| **error** | Một đối tượng `AdaptyError` nếu điều hướng paywall web thất bại; `null` nếu điều hướng thành công. |
...
Xem [Theo dõi lượt xem màn hình flow](android-flow-screen-views) để biết cách xử lý chúng.

Flow cũng báo cáo các giá trị người dùng nhập và chọn. Xem [Xử lý dữ liệu từ flow](android-flow-input) để đọc chúng.
...
| Phương thức | Mô tả |
|:-------|:------------|
| **onShowAppRate** | Dành riêng cho yêu cầu đánh giá ứng dụng từ một flow. Hiện tại các flow chưa kích hoạt yêu cầu đánh giá ứng dụng, vì vậy bạn không cần triển khai phương thức này. |
| **onShowRequestPermission** | Dành riêng cho yêu cầu cấp quyền hệ thống (chẳng hạn như thông báo đẩy hoặc quyền truy cập camera) từ một flow. Hiện tại các flow chưa kích hoạt yêu cầu cấp quyền, vì vậy bạn không cần triển khai phương thức này. |
...
:::warning
Hướng dẫn này chỉ đề cập đến **các paywall được xây dựng trong builder cũ**, yêu cầu Adapty SDK v3.0 trở lên.
:::
