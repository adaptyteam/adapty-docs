...
2. `openWebPaywall(paywall)` tạo URL theo paywall mà không thêm dữ liệu sản phẩm vào URL. Trong SDK v4, phương thức này nhận một `AdaptyFlowPaywall` — một biến thể paywall của flow đã fetch. Hãy kiểm tra rằng `flow.paywalls` không rỗng trước khi truy cập vào nó, ví dụ `flow.paywalls[0]`.
:::
