...
Kế hoạch migration của bạn gồm 5 bước logic và mất trung bình 2 tiếng. 90% các migration đều hoàn thành trong chưa đầy một ngày làm việc.

1. Tìm hiểu những khác biệt cốt lõi; tạo và chuẩn bị tài khoản Adapty _(5 phút)_;
2. Cài đặt Adapty SDK cho nền tảng của bạn ([iOS](sdk-installation-ios), [Android](sdk-installation-android), [React Native](sdk-installation-reactnative), [Flutter](sdk-installation-flutter), [Kotlin Multiplatform](sdk-installation-kotlin-multiplatform), [Unity](sdk-installation-unity)) thay thế cho RevenueCat SDK _(1 giờ)_;
3. Thiết lập [thông báo máy chủ Apple App Store](enable-app-store-server-notifications) tới Adapty và (tùy chọn) [chuyển tiếp sự kiện thô](enable-app-store-server-notifications#raw-events-forwarding) _(5 phút)_;
4. Kiểm tra và phát hành bản cập nhật ứng dụng của bạn _(30 phút);_
5. (Tùy chọn) Xuất dữ liệu lịch sử RevenueCat của bạn ở định dạng CSV bằng [tính năng xuất dữ liệu theo lịch của RevenueCat](https://www.revenuecat.com/docs/integrations/scheduled-data-exports) _(5 phút);_
6. (Tùy chọn) Import dữ liệu lịch sử qua bộ phận hỗ trợ của Adapty _(30 phút)_.

:::info
Người dùng đăng ký của bạn sẽ được migrate tự động

Tất cả người dùng đã từng kích hoạt gói đăng ký sẽ ngay lập tức chuyển sang Adapty ngay khi họ mở phiên bản mới của ứng dụng có tích hợp Adapty SDK. Trạng thái gói đăng ký sẽ được xác thực và quyền truy cập premium sẽ được khôi phục tự động.
:::
...
## (Tùy chọn) Xuất dữ liệu lịch sử của bạn từ RevenueCat sang định dạng CSV \{#optional-export-your-revenuecat-historical-data-in-csv-format\}

:::warning
Đừng vội vàng nhập dữ liệu lịch sử

Bạn nên đợi ít nhất một tuần sau khi phát hành với SDK trước khi thực hiện nhập dữ liệu lịch sử. Trong thời gian đó, chúng tôi sẽ thu thập được toàn bộ thông tin về giá mua hàng từ SDK, giúp dữ liệu bạn nhập trở nên chính xác hơn.
:::

Xuất dữ liệu lịch sử của bạn từ RevenueCat sang định dạng CSV bằng cách làm theo hướng dẫn trong [tài liệu chính thức của RevenueCat](https://www.revenuecat.com/docs/integrations/scheduled-data-exports).
...
### Những gì không được truyền tải \{#what-doesnt-come-across\}

- Mỗi giao dịch đều được xác minh lại với cửa hàng trong quá trình import, vì vậy các hàng mà cửa hàng không còn nhận ra sẽ bị bỏ qua.
- Các hàng không có ID giao dịch thực từ cửa hàng — ví dụ như entitlement được cấp thủ công hoặc ưu đãi của RevenueCat — sẽ được import dưới dạng hồ sơ người dùng không có giao dịch. Để khôi phục quyền truy cập cho những người dùng này, hãy cấp cho họ mức độ truy cập thông qua [server-side API](api-adapty/operations/grantAccessLevel).
- Lịch sử hoàn tiền và sự cố thanh toán không được chuyển nguyên vẹn: hoàn tiền chỉ giữ lại ngày hủy, còn trạng thái sự cố thanh toán của gói đăng ký được ghi nhận tại thời điểm import, không kèm theo ngày lịch sử.
