...
Adapty CLI cho phép bạn thiết lập cấu hình ứng dụng hoàn toàn từ dòng lệnh. Hãy dùng nó như một lựa chọn thay thế cho [dashboard quickstart](integrate-payments) nếu bạn thích làm việc trên terminal hoặc dùng [MCP clients](https://github.com/adaptyteam/adapty-cli/tree/main/skills/adapty-cli).

:::note
Kết nối Adapty với App Store Connect và Google Play vẫn cần thiết lập một lần trên dashboard — được hướng dẫn ở bước 3.
:::

Sau khi hoàn thành, ứng dụng, mức độ truy cập, sản phẩm, paywall và placement của bạn đều sẽ hiển thị trên [Adapty dashboard](https://app.adapty.io).

Nếu bạn chạy CLI trong Claude Cowork, hãy cho phép các domain của Adapty trước khi bắt đầu — xem [Chạy CLI trong Cowork](#run-the-cli-in-cowork).
...
## Chạy CLI trong Cowork \{#run-the-cli-in-cowork\}

Claude Cowork chạy các lệnh trong một sandbox chỉ kết nối được với những domain được phép. Cho đến khi bạn cho phép Adapty, CLI không thể đăng nhập hay đọc tài khoản của bạn. Bỏ qua phần này nếu bạn chạy CLI từ terminal của chính mình.

Cho phép cả `adapty.io` và `*.adapty.io`. Wildcard không bao gồm apex domain trong hầu hết các cách triển khai allowlist. Bản thân CLI kết nối tới `api.adapty.io`, còn một agent đọc tài liệu Adapty trong khi hoạt động sẽ kết nối tới apex domain.

Cowork áp dụng các cài đặt này khi một task bắt đầu. Nếu bạn thay đổi chế độ truy cập hoặc thêm domain trong lúc đang hội thoại, hội thoại đó vẫn giữ nguyên cài đặt ban đầu — hãy bắt đầu một task mới.

Mỗi task cũng khởi động trên một máy sạch: không có CLI, không có token, và không có cấu hình nào được giữ lại từ phiên trước, vì vậy các bước cài đặt và đăng nhập phải lặp lại ở đầu mỗi task.
...
5. Trong **Additional allowed domains**, thêm `adapty.io`, rồi thêm `*.adapty.io`.

<ZoomImage id="cowork-network-egress.png" width="700px" alt="Cài đặt Allow network egress trong Claude, với domain Adapty trong danh sách additional allowed domains" />

Chế độ truy cập còn lại, **All domains**, cho phép sandbox truy cập bất kỳ trang web nào. Khi chọn tùy chọn này, bạn không cần thêm các domain của Adapty.

### Không gian làm việc doanh nghiệp \{#corporate-workspaces\}

Quản trị viên kiểm soát danh sách cho phép. Hãy nhờ họ thêm cả hai domain vào không gian làm việc.

## Tiếp theo là gì \{#whats-next\}

Tất cả các thực thể giờ đã hiển thị trên [Adapty Dashboard](https://app.adapty.io). Tiếp theo:

- [Thiết kế paywall của bạn](adapty-paywall-builder) — dùng Paywall Builder không cần code để thêm hình ảnh, bố cục và nội dung cho paywall vừa tạo.
- [Tích hợp SDK Adapty](quickstart-sdk) — thêm SDK vào ứng dụng để tải và hiển thị placement.
- Định tuyến các [phân khúc](segments) người dùng khác nhau đến các paywall khác nhau — xem [`placements update`](developer-cli-reference#adapty-placements-update) và [`segments list`](developer-cli-reference#adapty-segments-list) trong tài liệu tham chiếu lệnh.
