...
description: "Migrate sang Adapty React Native SDK v4.0 bằng cách thay thế các paywall API bằng flow API, tương thích với cả Flow Builder và Paywall Builder."
metadataTitle: "Migrate lên Adapty React Native SDK v4.0 | Tài liệu Adapty"
---

Adapty React Native SDK 4.0 giới thiệu các flow và đổi tên các paywall API tương ứng. Các API mới hoạt động được với cả Flow Builder mới lẫn Paywall Builder hiện có — không cần thay đổi cấu hình nào trên Adapty Dashboard.
...
`buildReactNativeFromSource` là bắt buộc trên **Expo SDK 57 trở lên**. Expo SDK 57 đi kèm với React Native framework được build sẵn mà các package khác không thể truy cập header khi frameworks là dynamic, khiến iOS build thất bại với các lỗi như `'React/RCTBridge.h' file not found` trong `expo-updates` hoặc `@expo/ui`. Build React Native từ source sẽ tránh được xung đột này, nhưng đổi lại thời gian build iOS sẽ lâu hơn. Trên Expo SDK 56 trở xuống, bạn có thể bỏ qua tùy chọn này.
...
Nếu trước đây bạn đã kéo `Adapty`, `AdaptyUI`, hoặc `AdaptyPlugin` dưới dạng sub-dependencies của CocoaPods, hãy xóa các dòng `pod 'Adapty'`, `pod 'AdaptyUI'`, hoặc `pod 'AdaptyPlugin'` trong `Podfile` trước.
