...
```
### iOS: SDK gốc hiện được phân phối qua Swift Package Manager \{#ios-native-sdks-now-come-through-swift-package-manager\}

[Kho spec của CocoaPods sẽ chuyển sang chế độ chỉ đọc vào tháng 12 năm 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/), vì vậy bắt đầu từ v4, các SDK gốc `Adapty`, `AdaptyUI`, và `AdaptyPlugin` **không còn được kéo vào như sub-dependencies của CocoaPods nữa** — podspec sẽ kéo chúng qua **Swift Package Manager** (thông qua helper `spm_dependency`). Điều này yêu cầu hai thứ:
- **React Native 0.75 trở lên** — cần thiết cho helper podspec `spm_dependency`. Nếu dùng phiên bản cũ hơn, `pod install` sẽ thất bại với lỗi rõ ràng; hãy nâng cấp React Native trước, hoặc tiếp tục dùng `react-native-adapty` 3.x.
...
Kiểu trả về thay đổi từ `AdaptyPaywall` sang `AdaptyFlow`, và tham số `locale` bị loại bỏ — khi bạn render một flow, locale sẽ được tự động xác định; với các paywall tùy chỉnh, tất cả các locale được trả về trong `flow.remoteConfigs`:
...
`getFlow` trả về `AdaptyFlow` thay vì `AdaptyPaywall`, và cấu trúc object đã thay đổi:
| Trường `AdaptyPaywall` v3 | Trường `AdaptyFlow` v4 | Hành động |
|---|---|---|
| `remoteConfig?` (đơn lẻ) | `remoteConfigs?: AdaptyRemoteConfig[]` (mảng) | Một flow chứa một remote config cho mỗi ngôn ngữ đã cấu hình. Đọc config khớp với người dùng: `flow.remoteConfigs?.find((c) => c.lang === 'en')`. |
| `products` | `flow.paywalls[i].productIdentifiers` | Mã định danh sản phẩm giờ nằm trên từng biến thể của flow, không còn trên flow nữa. |
...
Nếu bạn dùng React component để render, hãy đổi tên component và truyền prop `flow`:
...
```
:::note
Một flow view được tạo bằng `createFlowView` chỉ dùng được một lần: sau khi gọi `dismiss()`, view sẽ bị hủy, vì vậy hãy gọi lại `createFlowView` để hiển thị flow lần nữa. Một `AdaptyFlowView` được nhúng vào sẽ bị đóng khi unmount — việc trả về `true` từ một handler không đóng view được nhúng, vì vậy hãy tự thay đổi state của bạn, ví dụ trong `onCloseButtonPress`.
:::
## Xử lý sự kiện \{#handling-events\}

Giao diện event-handler được đổi tên từ `EventHandlers` thành `FlowEventHandlers`, và ba callback cũng được đổi tên. Phần thân handler hiện có không cần thay đổi code — chỉ cần đổi tên:
...
+ onError: (error) => { /* … */ },
```
Tất cả các event handler khác giữ nguyên tên. Hai handler cũng có thêm tham số thứ hai: `onPurchaseCompleted` nay là `(purchaseResult, product)` và `onPurchaseFailed` nay là `(error, product)`, trong đó `product` là `AdaptyPaywallProduct` liên quan. Xem [Xử lý sự kiện flow & paywall](react-native-handling-events-1) để biết danh sách đầy đủ.

:::note
`onDisappeared` chỉ được kích hoạt khi flow được hiển thị dạng modal thông qua `createFlowView().present()`. Component `AdaptyFlowView` không hỗ trợ nó như một prop — để đóng một embedded view, hãy unmount nó.
:::

v4 cũng bổ sung một số tính năng bạn có thể chọn sử dụng:
- Phương thức `adapty.openWebUrl(url, openIn?)` và `adapty.requestAppReview()` — hỗ trợ các handler mặc định `onUrlPress` và `onRequestAppReview`, giúp URL và yêu cầu đánh giá ứng dụng được xử lý tự động. Chỉ gọi trực tiếp khi bạn muốn ghi đè các handler đó.
- Xử lý mua hàng ở chế độ observer bên trong flow thông qua các handler mới `onObserverPurchaseInitiated` / `onObserverRestoreInitiated`. Xem [Xử lý mua hàng ở chế độ observer](react-native-handling-events-1#handle-purchases-in-observer-mode).
...
- **`onAndroidSystemBack`**: Giá trị mặc định đã thay đổi từ đóng view sang giữ nguyên. Để khôi phục hành vi cũ, hãy trả về `true` từ handler.
- **`onPurchaseCompleted`**: Giá trị mặc định đã thay đổi từ đóng view (trừ khi người dùng hủy mua) sang luôn giữ nguyên view. Để khôi phục hành vi cũ, hãy trả về `purchaseResult.type !== 'user_cancelled'` từ handler.
- **`onRestoreCompleted`**: Giá trị mặc định đã thay đổi từ đóng view sau khi khôi phục thành công sang giữ nguyên view. Để khôi phục hành vi cũ, hãy trả về `true` từ handler.
- **`onUrlPress`**: Giá trị mặc định bây giờ sẽ mở URL qua native layer, tuân theo cài đặt trình duyệt in-app hoặc external từ dashboard. Ghi đè handler để tự xử lý việc mở URL.
