...
Nếu bạn đã tạo một flow hoặc một paywall trong Paywall Builder cũ, bạn không cần lo lắng về việc render nó trong code ứng dụng di động để hiển thị cho người dùng. Flow đó đã bao gồm cả nội dung cần hiển thị lẫn cách hiển thị.
...
3. Bạn đã [fetch flow và chuẩn bị view](react-native-get-pb-paywalls).

:::warning

Hướng dẫn này chỉ dành cho **các flow và paywall được render bởi Adapty**, yêu cầu SDK v4.0 trở lên. Quy trình trình bày flow khác với paywall remote config.

- Để trình bày **paywall remote config**, xem [Render paywall thiết kế bằng remote config](present-remote-config-paywalls).

:::

Adapty React Native SDK cung cấp hai cách để trình bày flow và paywall:

- **React component**: Component nhúng cho phép bạn tích hợp vào kiến trúc và hệ thống điều hướng của ứng dụng.
...
Nếu bạn đã tùy chỉnh paywall bằng Paywall Builder, bạn không cần lo lắng về việc render nó trong code ứng dụng di động để hiển thị cho người dùng. Paywall như vậy đã bao gồm cả nội dung cần hiển thị lẫn cách hiển thị.
...
Hướng dẫn này chỉ áp dụng cho các paywall được xây dựng bằng Paywall Builder cũ dành cho **Adapty SDK 3.x**. Chúng yêu cầu SDK v3.0 trở lên. Việc hiển thị paywall được xây dựng cho SDK 2.x hoặc cũ hơn, hoặc remote config paywall, hoạt động theo cách khác.

- Để hiển thị **remote config paywalls**, xem [Hiển thị paywall được thiết kế bằng remote config](present-remote-config-paywalls).

:::

Adapty React Native SDK cung cấp hai cách để hiển thị paywall:

- **React component**: Component nhúng cho phép bạn tích hợp nó vào kiến trúc và hệ thống điều hướng của ứng dụng.
