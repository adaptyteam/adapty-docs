...
| Tham số                  | Bắt buộc       | Mô tả |
| :----------------------- | :------------- | :---------- |
| **forFlow**              | bắt buộc       | Đối tượng `AdaptyFlow` lấy từ `Adapty.getFlow`. |
| **locale**               | <p>tùy chọn</p><p>mặc định: `nil`</p> | Mã định danh của [bản địa hóa paywall](add-paywall-locale-in-adapty-paywall-builder). Nhập dưới dạng mã ngôn ngữ với một hoặc hai thẻ con phân tách bằng dấu `-` (ví dụ: `en`, `pt-br`). Xem [Bản địa hóa và mã ngôn ngữ](localizations-and-locale-codes). |
| **customLayoutId**       | <p>tùy chọn</p><p>mặc định: `nil`</p><p>SDK 4.1+</p> | ID tùy chỉnh của một layout được định nghĩa trong Flow Builder. Truyền vào để render layout cụ thể đó thay vì layout mà SDK tự động chọn dựa trên loại thiết bị và kích thước màn hình. Nếu không có layout nào khớp với ID, flow sẽ tải mà không có cấu hình view. |
| **loadTimeout**          | mặc định: 5 giây | Giá trị này giới hạn thời gian chờ cho phương thức này. Nếu hết thời gian, dữ liệu đã cache hoặc fallback cục bộ sẽ được trả về. Lưu ý rằng trong một số trường hợp hiếm gặp, phương thức này có thể hết thời gian chờ muộn hơn một chút so với giá trị chỉ định trong `loadTimeout`, do hoạt động có thể bao gồm nhiều yêu cầu khác nhau bên dưới. |
...
| **systemRequestsHandler** | tùy chọn      | Đối tượng tuân theo `AdaptySystemRequestsHandler` xử lý các yêu cầu quyền hệ thống và yêu cầu đánh giá được kích hoạt bởi các hành động trong flow. Chỉ cần thiết nếu flow của bạn bao gồm các hành động như vậy. |
| **assetsResolver**       | tùy chọn       | Từ điển `[String: AdaptyCustomAsset]` ghi đè hình ảnh và video trong flow/paywall. Xem [Tùy chỉnh tài nguyên](#customize-assets). |
| **timerResolver**        | tùy chọn       | Đối tượng tuân theo `AdaptyTimerResolver` cung cấp ngày kết thúc cho các bộ đếm thời gian do nhà phát triển định nghĩa. Xem [Thiết lập bộ đếm thời gian do nhà phát triển định nghĩa](#set-up-developer-defined-timers). |
