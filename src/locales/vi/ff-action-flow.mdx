...
description: "Xây dựng một action flow trong FlutterFlow để lấy dữ liệu paywall và sản phẩm từ Adapty cho trang paywall của bạn."
metadataTitle: "Tạo flow để hiển thị dữ liệu paywall trong FlutterFlow | Tài liệu Adapty"
---

:::important
Khi sử dụng plugin FlutterFlow, bạn không thể dùng các paywall được tạo trong Adapty Paywall Builder. Bạn phải tự tạo trang paywall trong FlutterFlow và kết nối nó với Adapty.
:::

Sau khi thêm thư viện Adapty làm dependency cho dự án FlutterFlow, đã đến lúc xây dựng flow để **lấy dữ liệu paywall và sản phẩm từ Adapty và hiển thị chúng trên paywall bạn đã thiết kế trong FlutterFlow**.

Trước tiên, chúng ta cần nhận dữ liệu paywall từ Adapty. Chúng ta sẽ bắt đầu bằng cách yêu cầu paywall từ Adapty, sau đó lấy các sản phẩm liên quan, và cuối cùng kiểm tra xem dữ liệu có được nhận thành công hay không. Nếu thành công, chúng ta sẽ hiển thị tiêu đề và giá sản phẩm trên trang paywall. Nếu không, chúng ta sẽ hiển thị thông báo lỗi.

Trước khi tiếp tục, hãy đảm bảo bạn đã hoàn thành những bước sau:

1. [Đã tạo ít nhất một paywall và thêm ít nhất một sản phẩm vào đó](create-paywall) trong Adapty Dashboard.
2. [Đã tạo ít nhất một placement](create-placement) và [thêm paywall của bạn vào đó](add-audience-paywall-ab-test) trong Adapty Dashboard.

Bắt đầu thôi!

## Bước 1.1. Lấy paywall từ Adapty \{#step-11-request-adapty-paywall\}

Như đã đề cập, để hiển thị dữ liệu trên paywall FlutterFlow của bạn, trước tiên chúng ta cần lấy dữ liệu đó từ Adapty. Bước đầu tiên là lấy chính paywall từ Adapty. Cách thực hiện như sau:

1. Mở màn hình paywall của bạn và chuyển sang phần **Actions** ở khung bên phải. Tại đó, mở **Action Flow Editor**.
...
3. Nhấp vào **Add Action**. Sau đó, tìm kiếm custom action `getPaywall` và chọn nó.
...
4. Trong phần **Set Actions Arguments**, nhập ID thực của [placement bạn đã tạo](create-placement) trong Adapty Dashboard có chứa paywall. Trong ví dụ này là `monthly`. Hãy đảm bảo sử dụng ID placement thực của bạn!
...
5. Nếu bạn đã [bản địa hóa](localizations-and-locale-codes) paywall của mình trong Adapty Dashboard, bạn cũng có thể thiết lập đối số **locale**.
