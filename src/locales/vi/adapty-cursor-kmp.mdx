---
title: "Tích hợp Adapty vào ứng dụng Kotlin Multiplatform của bạn với sự hỗ trợ của AI"
...
metadataTitle: "Tích hợp Adapty với sự hỗ trợ của AI cho Kotlin Multiplatform | Tài liệu Adapty"
displayed_sidebar: sdkkmp
---

Hướng dẫn này sẽ đưa bạn qua từng bước tích hợp Adapty vào ứng dụng Kotlin Multiplatform bằng công cụ lập trình AI — bạn chỉ cần cung cấp đúng tài liệu Adapty theo đúng thứ tự.
...
4. **Tạo một flow hoặc paywall và một placement**: Trong Adapty Dashboard, tạo một flow (hoặc một paywall nếu bạn tự xây dựng giao diện), sau đó gán nó vào một placement trên trang **Placements**. Trong code, placement ID là chuỗi bạn truyền vào `Adapty.getFlow("YOUR_PLACEMENT_ID")`.
...
Những mục này không bắt buộc để bắt đầu code, nhưng bạn sẽ cần đến chúng khi tích hợp trưởng thành hơn:

- **A/B test**: Cấu hình trên trang **Placements**. Không cần thay đổi code.
  [A/B test](ab-tests)
- **Flow và placement bổ sung**: Thêm nhiều lệnh gọi `getFlow` với các placement ID khác nhau.
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Bạn tạo flow trong trình xây dựng no-code của Adapty, và SDK tự động hiển thị chúng.
- [**Paywall tự tạo thủ công**](kmp-making-purchases): Bạn tự xây dựng giao diện paywall trong code nhưng vẫn dùng Adapty để lấy sản phẩm và xử lý giao dịch mua.
...
Chưa biết chọn cái nào? Đọc [bảng so sánh trong hướng dẫn quickstart](kmp-quickstart-paywalls).
...
**Hướng dẫn:**
- [Kích hoạt mua hàng với Flow & Paywall Builder (quickstart)](kmp-quickstart-paywalls)
- [Lấy flow và cấu hình của chúng](kmp-get-pb-paywalls)
- [Hiển thị flow](kmp-present-paywalls)
- [Xử lý sự kiện trong flow](kmp-handling-events)
