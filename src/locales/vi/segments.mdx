...
### Cách các toán tử xử lý thuộc tính bị thiếu \{#how-operators-match-missing-attributes\}

Toán tử **≠** chỉ khớp với các hồ sơ người dùng có giá trị thuộc tính được đặt: hồ sơ người dùng không có giá trị cho thuộc tính đó sẽ không khớp với `attribute ≠ value`. Điều tương tự áp dụng cho các toán tử so sánh (`>`, `<`, `≥`, `≤`, `between`). Để khớp các hồ sơ người dùng không có thuộc tính, hãy dùng toán tử **is not set**. Để nhắm mục tiêu tất cả hồ sơ người dùng ngoại trừ những hồ sơ có giá trị cụ thể, kết hợp `≠` với **is not set** — hoặc đặt giá trị rõ ràng cho mọi hồ sơ người dùng và lọc chỉ bằng `≠`.

So sánh cũng được kiểm tra theo kiểu dữ liệu: nếu hồ sơ người dùng lưu trữ thuộc tính với kiểu dữ liệu khác với kiểu mà phân khúc so sánh — ví dụ, một giá trị chuỗi trong khi phân khúc yêu cầu một số — thì hồ sơ đó được tính là không có thuộc tính đó.
