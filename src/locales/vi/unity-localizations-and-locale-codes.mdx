...
Kết quả khớp là chính xác tuyệt đối. SDK so sánh mã bạn truyền vào với mã bản địa hóa của flow theo từng ký tự: không thay đổi chữ hoa/thường, không thay thế dấu gạch dưới (`_`) bằng dấu gạch ngang (`-`), và không fallback về language subtag. Với flow có bản địa hóa `pt-br`, chỉ `pt-br` mới khớp: `pt-BR`, `pt_BR`, và `pt-PT` đều không khớp.

Khi mã ngôn ngữ không khớp với bất kỳ bản dịch nào, flow sẽ hiển thị theo [ngôn ngữ mặc định](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — SDK không trả về lỗi và không ghi cảnh báo.

Khi mã ngôn ngữ khớp, Adapty sẽ kết hợp bản dịch đó với bản dịch mặc định: những chuỗi văn bản và tài nguyên mà bản dịch khớp không định nghĩa sẽ được lấy từ bản dịch mặc định.

Bỏ qua mã locale không có nghĩa là yêu cầu localization mặc định của flow: SDK sẽ thay thế bằng `en` cố định. Một flow có locale mặc định là `de` vẫn sẽ hiển thị bằng `en` nếu nó có localization `en`, và chỉ fallback về `de` khi không có.

:::warning
Truyền mã locale chính xác như đã cấu hình trong dashboard — các subtag viết thường, phân tách bằng dấu gạch ngang. Đừng truyền trực tiếp system locale identifier: `CultureInfo.CurrentCulture.Name` trả về `pt-BR`, và sẽ fallback về localization mặc định. Hãy chuyển đổi giá trị trong app trước khi truyền vào.
:::
...
- **Flows và paywall được render bởi Adapty**: SDK không đọc locale của thiết bị, vì vậy hãy tự xử lý trong app của bạn và [truyền locale khi tạo view](#choose-the-localization-of-a-flow). Mã locale là tùy chọn — bỏ qua nó và flow sẽ render bằng `en`, hoặc bằng locale mặc định nếu flow không có bản dịch `en`.
- **Paywall tùy chỉnh (remote config)**: `GetFlow` trả về tất cả các bản dịch đã cấu hình trong `flow.RemoteConfigs`. Mỗi mục là một `AdaptyRemoteConfig` có mã `Locale` và một `Dictionary` chứa các giá trị. Hãy chọn mục phù hợp với người dùng, với logic fallback do bạn tự định nghĩa:
