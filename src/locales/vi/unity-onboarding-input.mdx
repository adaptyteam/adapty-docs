---
title: "Xử lý dữ liệu từ onboardings trong Unity SDK"
description: "Lưu và sử dụng dữ liệu từ onboardings trong ứng dụng Unity của bạn với Adapty SDK."
metadataTitle: "Onboardings | Unity SDK | Tài liệu Adapty"
displayed_sidebar: sdkunity
---

:::warning
**Onboarding đã bị deprecated trong SDK v4 và sẽ bị xóa trong phiên bản tương lai.** Chúng không còn nhận được các bản sửa lỗi hay cải tiến nữa. Hãy sử dụng [flows](unity-get-pb-paywalls) thay thế: khác với onboarding chạy bên trong WebView, flows render trực tiếp trên thiết bị — mang lại animation mượt mà hơn, giao diện native nhất quán, tốc độ tải nhanh hơn và không phụ thuộc vào WebView runtime. Xem [Lấy flows & paywalls](unity-get-pb-paywalls) và [Hiển thị flows & paywalls](unity-present-paywalls) để bắt đầu.
:::

Khi người dùng trả lời câu hỏi trong quiz hoặc nhập dữ liệu vào trường input, phương thức `OnboardingViewOnStateUpdatedAction` sẽ được gọi. Bạn có thể lưu hoặc xử lý loại trường đó trong code của mình.

Implement phương thức `OnboardingViewOnStateUpdatedAction` trong class của bạn:

:::note
Trong SDK 4.0, các interface listener tuân theo quy ước tiền tố I- của C#: hãy implement `IAdaptyOnboardingsEventsListener` thay vì `AdaptyOnboardingsEventsListener`. Các phương thức không thay đổi. Xem [hướng dẫn migration](migration-to-unity-sdk-v4).
:::
