...
Khi người dùng trả lời câu hỏi trong bài kiểm tra hoặc nhập dữ liệu vào trường nhập liệu, phương thức `OnboardingViewOnStateUpdatedAction` sẽ được gọi. Bạn có thể lưu hoặc xử lý loại trường đó trong code của mình.

Triển khai phương thức `OnboardingViewOnStateUpdatedAction` trong class của bạn:

:::note
Trong SDK 4.1, các interface listener tuân theo quy ước tiền tố I- của C#: hãy implement `IAdaptyOnboardingsEventsListener` thay vì `AdaptyOnboardingsEventsListener`. Các phương thức không thay đổi. Xem [hướng dẫn migration](migration-to-unity-sdk-v4).
:::
