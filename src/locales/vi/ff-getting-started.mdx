---
title: "Bắt đầu"
description: "Tìm hiểu cách hoạt động của placement, đối tượng và A/B test trước khi tích hợp Adapty vào ứng dụng FlutterFlow của bạn."
metadataTitle: "Bắt đầu với plugin Adapty FlutterFlow | Tài liệu Adapty"

---

Với Adapty, bạn có thể tạo và chạy các paywall cũng như A/B test tại nhiều điểm khác nhau trong hành trình của người dùng ứng dụng, chẳng hạn như Onboarding, Settings, v.v. Các điểm này được gọi là [Placements](placements). Một placement trong ứng dụng có thể quản lý nhiều paywall hoặc [A/B test](ab-tests) cùng lúc, mỗi cái được thiết kế cho một nhóm người dùng nhất định — chúng tôi gọi đó là [Audiences](audience). Ngoài ra, bạn có thể thử nghiệm với các paywall, thay thế cái này bằng cái khác theo thời gian mà không cần phát hành phiên bản ứng dụng mới. Thứ duy nhất bạn cần hardcode trong ứng dụng là placement ID.
...
Thư viện Adapty giữ cho paywall của bạn luôn được cập nhật với các sản phẩm mới nhất từ Adapty Dashboard. Nó [lấy dữ liệu sản phẩm](ff-action-flow) và [hiển thị lên paywall của bạn](ff-add-variables-to-paywalls), [xử lý các giao dịch mua](ff-make-purchase), và [kiểm tra mức độ truy cập của người dùng](ff-check-subscription-status) để xác định xem họ có được truy cập nội dung trả phí hay không.
...
- Thư viện Adapty dành cho FlutterFlow không hỗ trợ ứng dụng web. Tránh biên dịch ứng dụng web với thư viện này.
- Thư viện Adapty dành cho FlutterFlow không hỗ trợ paywall được tạo bằng Adapty Paywall Builder. Bạn cần tự thiết kế paywall trong FlutterFlow trước khi kích hoạt tính năng mua hàng với Adapty.
