...
[Amplitude](https://amplitude.com/) là một dịch vụ phân tích di động mạnh mẽ. Với Adapty, bạn có thể dễ dàng gửi sự kiện đến Amplitude, theo dõi hành vi người dùng và đưa ra các quyết định thông minh.

Adapty cung cấp đầy đủ dữ liệu giúp bạn theo dõi [sự kiện gói đăng ký](events) từ các cửa hàng ở một nơi duy nhất và gửi chúng đến tài khoản Amplitude của bạn. Điều này cho phép bạn kết hợp hành vi người dùng với lịch sử thanh toán của họ trong Amplitude, từ đó đưa ra các quyết định sản phẩm phù hợp hơn.

### Cách thiết lập tích hợp Amplitude \{#how-to-set-up-amplitude-integration\}

Trong Adapty, bạn có thể thiết lập các flow riêng biệt cho **sự kiện production** và **sự kiện test** từ môi trường sandbox của Apple, Stripe, Paddle hoặc tài khoản test của Google.

- Với sự kiện production, nhập API key **Production** từ Amplitude dashboard, với một API key riêng cho từng nền tảng: iOS, Android, Stripe và Paddle.
- Với sự kiện test, sử dụng các trường **Sandbox** tùy theo nhu cầu.
...
| Field                                                | Description                                                  |
    | ---------------------------------------------------- | ------------------------------------------------------------ |
    | **Amplitude iOS/ Android/ Stripe/ Paddle API key**   | Nhập **API Key** Amplitude cho iOS/ Android/ Stripe/ Paddle vào Adapty. Bạn có thể tìm thấy key này trong **Project settings** của Amplitude. Để được hỗ trợ, hãy xem [tài liệu Amplitude](https://amplitude.com/docs/apis/authentication). Hãy bắt đầu với key **Sandbox** để kiểm thử, sau đó chuyển sang key **Production** sau khi kiểm thử thành công. Chỉ cần điền vào các nền tảng bạn bán hàng — nền tảng nào không có key sẽ không gửi sự kiện nào. |
