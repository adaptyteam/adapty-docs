...
| File fallback được tải xuống cho 4.0 | Định dạng file fallback mới; tải lại file |
| Promoted in-app purchase được hoàn tất tự động, không có cách nào để chặn lại | Phương thức delegate `didReceivePromotedPurchase(_:)` và `AdaptyPromotedProduct` chuyển giao việc hoàn tất cho ứng dụng của bạn |
...
## Khôi phục in-app purchase được quảng bá trên App Store \{#app-store-promoted-in-app-purchases-return\}

SDK 4.1 khôi phục hook cho in-app purchase được quảng bá trên App Store, vốn đã bị [4.0 loại bỏ](migration-to-ios-sdk-v4#app-store-promoted-in-app-purchases-temporarily-removed). Với 4.0, giao dịch mua vẫn hoàn tất và Adapty vẫn ghi lại transaction đó; chỉ là ứng dụng của bạn không thể can thiệp vào quá trình đó. 4.1 giữ nguyên hành vi mặc định này, vì vậy đây là một tính năng mới thay vì một bước migration.
...
:::warning
Cơ chế mới được xây dựng trên StoreKit 2 và yêu cầu **iOS 16.4** trở lên. Phương thức `shouldAddStorePayment` của phiên bản 3.x hoạt động trên các phiên bản iOS cũ hơn. Dưới iOS 16.4, `didReceivePromotedPurchase` sẽ không bao giờ được kích hoạt và các promoted purchase sẽ hoàn tất mà không đến được ứng dụng của bạn.
:::
