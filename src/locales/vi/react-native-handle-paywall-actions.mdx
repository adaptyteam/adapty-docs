...
description: "Xử lý các hành động nút từ flow và paywall trong React Native sử dụng Adapty để tối ưu hóa kiếm tiền từ ứng dụng."
...
Nếu bạn đang xây dựng flow trong Adapty Flow & Paywall Builder, điều quan trọng là phải thiết lập các nút đúng cách:

1. Thêm [nút trong builder](paywall-buttons) và gán cho nó một action có sẵn hoặc tạo một action ID tùy chỉnh.
2. Viết code trong ứng dụng của bạn để xử lý từng action mà bạn đã gán.

Hướng dẫn này trình bày cách xử lý các action tùy chỉnh và action có sẵn trong code của bạn.

:::warning
**Việc mua hàng, khôi phục, đóng flow/paywall và mở URL được xử lý tự động.** Bạn có thể cấu hình hành vi mặc định của chúng hoặc triển khai phản hồi cho các action tùy chỉnh.
:::
...
2. Viết code trong ứng dụng của bạn để xử lý từng action đã gán.

Hướng dẫn này giải thích cách xử lý các hành động tùy chỉnh và các hành động có sẵn trong code của bạn.

:::warning
**Chỉ có các hành động mua hàng, khôi phục, đóng paywall và mở URL được xử lý tự động.** Tất cả các hành động nút khác đều yêu cầu bạn tự triển khai phần xử lý tương ứng trong code của ứng dụng.
:::
