---
title: "Hiển thị flow & paywall - Android"
description: "Hiển thị flow và paywall cho người dùng trong ứng dụng Android của bạn."
metadataTitle: "Hiển thị Flow & Paywall - Android | Tài liệu Adapty"
---

<SDKv4>

<MethodPromo method="getFlow" label="Hiển thị flow và paywall" />

Nếu bạn đã tạo một flow hoặc paywall bằng Paywall Builder cũ, bạn không cần phải lo lắng về việc render nó trong code ứng dụng di động để hiển thị cho người dùng. Một flow hoặc paywall như vậy chứa cả nội dung cần hiển thị lẫn cách hiển thị nó.

:::warning

Hướng dẫn này đề cập đến các flow và paywall được render bởi Adapty. Quy trình sẽ khác đối với remote config paywall và [Observer mode](observer-vs-full-mode).

- Để hiển thị **paywall Remote Config**, xem [Hiển thị paywall được thiết kế bằng Remote Config](present-remote-config-paywalls).
- Để hiển thị **paywall ở chế độ Observer**, xem [Android - Hiển thị flow ở chế độ Observer](android-present-paywall-builder-paywalls-in-observer-mode)

:::

Để lấy đối tượng `flowConfiguration` dùng bên dưới, xem [Lấy flow & paywall](android-get-pb-paywalls).
...
| :---------------------------- | :------- |:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **flowConfiguration**         | bắt buộc | Cung cấp một đối tượng `AdaptyUI.FlowConfiguration` chứa thông tin hiển thị của flow. Sử dụng phương thức `AdaptyUI.getFlowConfiguration(flow)` để tải nó. Tham khảo phần [Fetch the view configuration](android-get-pb-paywalls#fetch-the-view-configuration) để biết thêm chi tiết.                                                                                                 |
...
| **eventListener**             | tùy chọn | Cung cấp một `AdaptyFlowEventListener` để theo dõi các sự kiện của flow. Nên kế thừa `AdaptyFlowDefaultEventListener` để dễ sử dụng hơn. Tham khảo phần [Handle flow & paywall events](android-handling-events) để biết thêm chi tiết.                                                                                                                                                                                                                                       |
| **insets**                    | tùy chọn | <p>Insets là khoảng cách xung quanh flow để ngăn các thành phần có thể nhấn bị ẩn sau thanh hệ thống.</p><p>Mặc định: `Unspecified`, tức là Adapty sẽ tự động điều chỉnh insets, phù hợp cho các flow edge-to-edge.</p><p>Nếu flow của bạn không phải edge-to-edge, bạn có thể muốn đặt insets tùy chỉnh. Xem hướng dẫn trong phần [Change flow insets](android-present-paywalls#change-flow-insets) bên dưới.</p> |
| **customAssets**              | tùy chọn | Truyền một đối tượng `AdaptyCustomAssets` để thay thế hình ảnh và video trong flow hoặc paywall của bạn lúc runtime. Tham khảo phần [Customize assets](android-get-pb-paywalls#customize-assets) để biết thêm chi tiết.                                                                                                                                                      |
| **tagResolver**               | tùy chọn | Sử dụng `AdaptyUiTagResolver` để xử lý các thẻ tùy chỉnh trong văn bản của flow. Resolver này nhận một tham số thẻ và chuyển đổi nó thành chuỗi tương ứng.                                                                                                                                                                                                                                                                                                      |
...
Hướng dẫn này chỉ áp dụng cho các paywall được xây dựng trong Paywall Builder cũ dành cho **Adapty SDK 3.x**. Chúng yêu cầu SDK v3.0. Việc hiển thị một paywall được xây dựng cho SDK 2.x hoặc cũ hơn, một Remote Config paywall, hoặc một paywall ở [chế độ Observer](observer-vs-full-mode) sẽ hoạt động khác.

- Để hiển thị **Remote config paywalls**, xem [Render paywall designed by remote config](present-remote-config-paywalls).
- Để hiển thị **Observer mode paywalls**, xem [Android - Present Paywall Builder paywalls in Observer mode](android-present-paywall-builder-paywalls-in-observer-mode)
