...
keywords: ['установка sdk', 'установить sdk', 'установка sdk react native', 'react native', 'google play billing library', 'gpbl', 'billing library']
...
:::important
Дождитесь завершения `activate` перед вызовом любых других методов Adapty SDK. Подробнее о порядке вызовов — в статье [Порядок вызовов в React Native SDK](react-native-sdk-call-order).
:::
...
- Если вы создаёте собственный интерфейс пейвола, смотрите [быстрый старт для пользовательских пейволов](react-native-quickstart-manual).
...
Используйте этот параметр для защиты конфиденциальности пользователей, соответствия региональным требованиям законодательства о защите данных (GDPR, CCPA и др.) или сокращения сбора данных, когда функции на основе IP-адреса не нужны вашему приложению.
...
По умолчанию AdaptyUI кэширует медиафайлы (например, изображения и видео), чтобы повысить производительность и снизить нагрузку на сеть. Вы можете настроить параметры кэша, передав собственную конфигурацию.
...
| memoryStorageCountLimit | нет | Максимальное количество элементов в памяти. По умолчанию используется значение, зависящее от платформы. |
| diskStorageSizeLimit | нет | Максимальный размер файла на диске в байтах. По умолчанию используется значение, зависящее от платформы. |
...
#### Откладывание активации SDK в целях разработки \{#delay-sdk-activation-for-development-purposes\}

При активации SDK Adapty заранее запрашивает все необходимые пользовательские данные, что обеспечивает более быстрый доступ к актуальной информации.

Однако в симуляторе iOS это может создавать неудобства: во время разработки симулятор часто запрашивает аутентификацию. Adapty не может управлять процессом аутентификации StoreKit, но может откладывать запросы, которые SDK делает для получения свежих данных.
Включив свойство `__debugDeferActivation`, вызов активации откладывается до следующего вызова Adapty SDK. Это предотвращает лишние запросы аутентификационных данных, если они не нужны.

Важно учитывать, что **эта функция предназначена только для разработки**, так как не охватывает все возможные пользовательские сценарии. В продакшене не следует откладывать активацию: на реальных устройствах аутентификационные данные, как правило, сохраняются и повторных запросов не возникает.

Вот рекомендуемый подход к использованию:
...
Для локальной разработки и тестирования можно включить mock-режим — это позволит обойтись без аккаунтов в песочнице App Store/Google Play и ускорить итерации. Mock-режим полностью обходит нативные модули Adapty и возвращает симулированные данные.
...
- **Не отрисовывает** пейволы/онбординги, созданные с помощью **Adapty Paywall Builder (AdaptyUI)**.
...
Чтобы тестировать реальные покупки и пейволы Paywall Builder, отключите mock-режим и используйте sandbox-аккаунты.
:::
...
Если Activity, запускающая флоу покупки, использует нестандартный `launchMode`, Android может некорректно пересоздать или повторно использовать её, когда пользователь возвращается из Google Play, банковского приложения или браузера. Это может привести к потере результата покупки или к тому, что покупка будет считаться отменённой.
...
В файле `AndroidManifest.xml` убедитесь, что для Activity, запускающей флоу покупки, установлен режим `standard` или `singleTop`:
```xml
<activity
...
Поды Adapty объявляют `s.swift_version = '6.0'` и требуют Swift 6 для сборки. Ваш собственный код приложения может остаться на Swift 5 — только поды Adapty (`Adapty`, `AdaptyUI`, `AdaptyUIBuilder`, `AdaptyLogger`, `AdaptyPlugin`) должны собираться с Swift 6.

Самая распространённая причина — хук `post_install` в `ios/Podfile`, который переписывает `SWIFT_VERSION` для каждого пода:
