...
[Airbridge](https://www.airbridge.io/) — это платформа для комплексного анализа маркетинговой эффективности сайтов и мобильных приложений, которая консолидирует данные, собранные с нескольких устройств, платформ и каналов. С помощью движка Identity Resolution Engine от Airbridge можно объединять разрозненные данные об идентичности пользователей из веб- и приложений в единый профиль на основе реального человека, что обеспечивает более точную атрибуцию.

Adapty предоставляет полный набор данных, позволяющий отслеживать [события подписок](events) из сторов в одном месте. С Adapty вы легко увидите, как ведут себя ваши подписчики, узнаете их предпочтения и сможете использовать эту информацию для точечной и эффективной коммуникации с ними.

Интеграция между Adapty и Airbridge работает двумя основными способами.

1. **Получение данных атрибуции из Airbridge**
   После настройки интеграции с Airbridge Adapty начнёт получать данные атрибуции от Airbridge. Вы можете легко просматривать эти данные на странице пользователя.
2. **Отправка событий подписки в Airbridge**
   Adapty может отправлять в Airbridge все события подписки, настроенные в вашей интеграции. В результате вы сможете отслеживать эти события в дашборде Airbridge. Эта интеграция полезна для оценки эффективности рекламных кампаний.
...
| `user.attributes`                            | Object  | Кастомные атрибуты пользователя.                                                  |
| `device`                                     | Object  | Информация об устройстве.                                                         |
| `device.deviceUUID`                          | String  | UUID устройства в Airbridge.                                                      |
...
| `device.ifa`                                 | String  | **Только iOS**. ID для рекламодателей.                                            |
| `device.ifv`                                 | String  | **Только iOS**. ID для вендоров.                                                  |
| `device.gaid`                                | String  | **Только Android**. Google Advertising ID.                                        |
| `app`                                        | Object  | Информация о приложении.                                                          |
| `app.packageName`                            | String  | Имя пакета / bundle ID приложения.                                                |
| `app.version`                                | String  | Версия приложения.                                                                |
| `eventUUID`                                  | String  | Уникальный ID события в Adapty.                                                   |
...
| `...semanticAttributes.period`               | String  | Период подписки в формате ISO 8601 (например, "P1M").                             |
...
| `...semanticAttributes.products[].name`      | String  | То же, что и `productID`.                                                         |
| `...semanticAttributes.products[].position`  | Integer | Позиция продукта в списке (всегда 1).                                             |

## Устранение неполадок \{#troubleshooting\}

### `access_level_updated` отображается как сбой в ленте событий \{#access_level_updated-shows-as-failed-in-the-event-feed\}
