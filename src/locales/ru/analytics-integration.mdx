---
title: "Интеграции с аналитическими системами"
...
Webhook-события отправляются в формате JSON. Все события имеют одинаковую структуру, но набор полей зависит от типа события, стора и вашей конфигурации.

<UsdConversion />

| Property                      | Type          | Description                                                  |
| ----------------------------- | ------------- | ------------------------------------------------------------ |
| **profile_id**                | uuid          | Adapty ID пользователя.                                      |
...
| **net_revenue_usd**           | float         | Чистая выручка (доход после вычета комиссии Apple/Google и налогов) в USD. Может быть пустым. |
...
| **cancellation_reason**       | str           | <p>Причина, по которой пользователь отменил подписку.</p><p></p><p>Возможные значения:</p><p>iOS и Android</p><p>_voluntarily_cancelled_, _billing_error_, _refund_</p><p>iOS</p><p>_price_increase_, _product_was_not_available_, _unknown_</p><p>Android</p><p>_new_subscription_replace_, _cancelled_by_developer_</p> |
| **subscription_expires_at**   | ISO 8601 date | Дата истечения срока действия подписки. Обычно в будущем.    |
| **consecutive_payments**      | int           | Количество периодов, в течение которых пользователь оформлял подписку без перерывов. Включает текущий период. |
...
| **paywall_name**              | str           | Название пейвола, из которого была совершена транзакция.     |
| **paywall_revision**          | int           | Ревизия пейвола, из которого была совершена транзакция. Значение равно 1. |
| **developer_id**              | str           | Developer (SDK) ID плейсмента, из которого была совершена транзакция. |
| **ab_test_name**              | str           | Название A/B-теста, из которого была совершена транзакция.   |
| **ab_test_revision**          | int           | Ревизия A/B-теста, из которого была совершена транзакция. Значение равно 1. |
...
| **profile_ip_address**        | str           | IP профиля (может быть IPv4 или IPv6, при наличии предпочтение отдаётся IPv4). Обновляется каждый раз при смене IP устройства. |
...
| **will_renew**                | bool          | Булево значение, указывающее, будет ли продлён платный уровень доступа. |
...
| **renewed_at**                | ISO 8601 date | Дата и время последнего начала текущего периода платного доступа. До первого продления совпадает с `activated_at`. |
...
:::warning
Блокировка внешней аналитики останавливает доставку событий во **все** интеграции для этого пользователя, включая пользовательские [вебхуки](webhook). Не используйте её, если ваш бэкенд опирается на события вебхуков для управления доступом этого пользователя.

Настройка не имеет обратной силы: события, созданные до её применения, всё равно будут доставлены. Она также не влияет на собственную аналитику Adapty — транзакции пользователя по-прежнему учитываются в графиках дашборда.
:::
