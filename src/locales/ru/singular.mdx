...
Adapty предоставляет полный набор данных, позволяющий отслеживать [события подписки](events) из сторов в одном месте. С Adapty вы легко увидите, как ведут себя ваши подписчики, узнаете, что им нравится, и сможете использовать эту информацию для целевой и эффективной коммуникации с ними. Таким образом, данная интеграция позволяет отслеживать события подписки в Singular и точно анализировать, сколько дохода приносят ваши кампании.

Adapty может отправлять все события подписки, настроенные в вашей интеграции, в Singular. Это позволит отслеживать эти события в дашборде Singular. Интеграция полезна для оценки эффективности рекламных кампаний.
...
| Параметр                  | Тип     | Описание                                      |
|:--------------------------|:--------|:----------------------------------------------|
| `is_revenue_event`        | Boolean | `true`, если событие содержит выручку.        |
| `amt`                     | Float   | Сумма выручки.                                |
...
### `access_level_updated` отображается как «Не выполнено» в Event Feed \{#access_level_updated-shows-as-failed-in-the-event-feed\}
