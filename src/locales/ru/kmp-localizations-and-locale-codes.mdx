...
Когда Adapty подбирает локализацию под локаль пользователя, происходит следующее:
...
Таким образом `'pt_BR'`, `pt-BR` и `pt-br` указывают на одну и ту же локализацию.

## Реализация локализаций \{#implementing-localizations\}

В SDK v4 код локали не передаётся при получении флоу.

- **Пейволы из Flow Builder и Paywall Builder**: Adapty автоматически определяет локализацию по настройкам устройства и локализациям, которые вы задали в билдере. Рендерьте флоу с помощью `createFlowView` — код локали не нужен.
- **Кастомные пейволы (на базе Remote Config)**: `getFlow` возвращает все настроенные локализации в `flow.remoteConfigs`. Каждый элемент — это `AdaptyRemoteConfig` с кодом `locale` и `dataMap`. Выберите подходящий элемент для пользователя, реализовав собственный фолбэк:
