...
description: "Управляйте локализациями приложения и кодами локали для охвата глобальной аудитории в вашем Kotlin Multiplatform приложении."
...
Коды локалей используются, когда Adapty подбирает локализацию для флоу и когда вы читаете Remote Config для пользовательского пейвола.

Коды локалей устроены непросто и могут различаться от платформы к платформе, поэтому Adapty опирается на единый внутренний стандарт для всех поддерживаемых платформ. Понимание этого стандарта поможет вам предсказать, какую локализацию получит пользователь.
...
4. Если совпадение снова не найдено, Adapty возвращает контент для локали по умолчанию флоу

Таким образом, `'pt_BR'`, `pt-BR` и `pt-br` — все три варианта разрешаются в одну и ту же локализацию.

## Реализация локализаций \{#implementing-localizations\}

В SDK v4 при получении флоу не нужно передавать код локали — `getFlow` возвращает флоу со всеми его локализациями, а Adapty применяет нужную при построении представления флоу.

- **Флоу, созданные в билдере**: SDK не читает локаль устройства, поэтому определите её в приложении и передайте как параметр `locale` в `createFlowView`. Это необязательно — если параметр не указан, флоу отрисовывается на `en` или в своей [локали по умолчанию](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale), если локализация `en` для флоу не задана. Если запросить локализацию, которой у флоу нет, представление автоматически переключится на локаль по умолчанию без ошибки, а строки, отсутствующие в выбранной локализации, подтянутся из локали по умолчанию.
...
Для кодов локалей Adapty использует слегка изменённый [стандарт BCP 47](https://en.wikipedia.org/wiki/IETF_language_tag): каждый код состоит из подтегов в нижнем регистре, разделённых дефисами. Несколько примеров: `en` (английский), `pt-br` (португальский (Бразилия)), `zh` (упрощённый китайский), `zh-hant` (традиционный китайский).
