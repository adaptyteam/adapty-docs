...
description: "Отслеживайте статус подписки пользователя в Adapty и управляйте им для повышения удержания клиентов."
metadataTitle: "Понимание статуса подписки | Документация Adapty"
keywords: ['getProfile', 'cache']
---

С Adapty отслеживать статус подписки очень просто. Не нужно вручную прописывать идентификаторы продуктов в коде — достаточно проверить наличие активного [уровня доступа](access-level), чтобы убедиться, что пользователь подписан.

Прежде чем приступить к проверке статуса подписки, настройте [уведомления сервера App Store](enable-app-store-server-notifications).
