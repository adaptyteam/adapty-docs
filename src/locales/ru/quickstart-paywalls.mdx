...
description: "Добавьте флоу или пейвол в Adapty, чтобы показать продукты пользователям, и прикрепите их к плейсменту."
...
- **Flow & Paywall Builder (рекомендуется)**: Визуальный редактор без кода для всего пути покупки. SDK Adapty рендерит результат нативно, поэтому UI-код писать не нужно.
- **Ручной пейвол**: Вы создаёте пейвол, привязываете к нему продукты и сами рендерите UI в коде приложения.
- **Adapty Paywall Builder (Legacy)**: Редактор пейволов без кода.

Оба варианта заканчиваются одинаково: вы привязываете созданное к [плейсменту](placements). Плейсмент — это то, что ваше приложение вызывает в рантайме, чтобы получить нужный контент для нужного пользователя.

<Tabs groupId="purchase-setup" queryString>
<TabItem value="flow-builder" label="Использовать Flow & Paywall Builder" default>

:::important
Флоу требуют Adapty SDK версии 4.0 или выше.
:::

Флоу — это один или несколько экранов с встроенными продуктами. Вы создаёте его в [Flow & Paywall Builder](adapty-flow-builder) — без написания кода.

SDK Adapty отображает флоу нативно на каждой платформе. Ваше приложение вызывает `getFlow`, SDK показывает экраны, обрабатывает покупки и передаёт события. Никакого отдельного UI-кода, никакого пейвола, который нужно поддерживать отдельно.

## 1. Создайте флоу \{#1-build-the-flow\}

1. Перейдите в раздел [**Flows**](https://app.adapty.io/flows) в главном меню Adapty.
2. Нажмите **Create flow** и создайте свой флоу.

Подробнее о [Adapty Flow & Paywall Builder](adapty-flow-builder).

<ZoomImage id="new-flow.webp" />

Гайды по шаблонам ниже разбирают наиболее распространённые сценарии шаг за шагом:

<CustomDocCardList ids={['basic-paywall-screen', 'paywall-with-tabs', 'paywall-features-per-product', 'onboarding-flow-tutorial']} />

После того как флоу сохранён и опубликован, переходите к его подключению к плейсменту.
...
В коде приложения вы указываете только идентификаторы плейсментов. Всё остальное — какой флоу запускается, какие продукты продаёт, как выглядит — настраивается в дашборде Adapty и может быть изменено в любой момент без обновления приложения.
...
Этот пейвол вы будете отрисовывать в коде своего приложения. <InlineTooltip tooltip="реализация пейволов вручную">Следуйте гайду для вашей платформы: [iOS](ios-implement-paywalls-manually), [Android](android-implement-paywalls-manually), [React Native](react-native-implement-paywalls-manually), [Flutter](flutter-implement-paywalls-manually), [Unity](unity-implement-paywalls-manually), [Kotlin Multiplatform](kmp-implement-paywalls-manually), [Capacitor](capacitor-implement-paywalls-manually).</InlineTooltip>
...
Пейвол, созданный в [Paywall Builder](adapty-paywall-builder), — это no-code экран с встроенными продуктами. SDK Adapty отображает его нативно, поэтому писать UI-код не нужно.
