...
Если вы создали флоу или пейвол в старом Paywall Builder, вам не нужно беспокоиться о его отображении в коде мобильного приложения — всё, что нужно показать пользователю и как именно это сделать, уже задано внутри самого флоу или пейвола.

:::warning

Этот гайд посвящён флоу и пейволам, которые отображает Adapty. Для пейволов на Remote Config и [режима Observer](observer-vs-full-mode) процесс отличается.
...
Если вы настроили пейвол с помощью Paywall Builder, вам не нужно беспокоиться о его отображении в коде мобильного приложения — всё необходимое уже заложено в самом пейволе: и что показывать, и как показывать.

:::warning

Этот гайд предназначен **только для пейволов, созданных в старом билдере**. Процесс отображения пейволов отличается для пейволов на Remote Config и для [режима Observer](observer-vs-full-mode).

Для отображения **пейволов на Remote Config** см. [Отображение пейвола, созданного с помощью Remote Config](present-remote-config-paywalls-kmp).
