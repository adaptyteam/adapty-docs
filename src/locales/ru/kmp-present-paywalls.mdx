...
Поскольку методы по умолчанию KMP-интерфейсов становятся `@required` в Swift, реализовать `AdaptyUIFlowsEventsObserver` напрямую из Swift не получится. Сначала объявите открытый базовый класс в `iosMain`:
