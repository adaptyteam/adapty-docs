...
4. **Создайте флоу или пейвол и плейсмент**: В дашборде Adapty создайте флоу (или пейвол, если вы будете строить UI самостоятельно), затем назначьте его на плейсмент на странице **Placements**. В коде идентификатор плейсмента — это строка, которую вы передаёте в `Adapty().getFlow()`.
...
Без этого можно начать писать код, но по мере развития интеграции всё это понадобится:
...
- **Интеграции с аналитикой**: Настраиваются на странице **Integrations**. Настройка зависит от конкретной интеграции. См. [интеграции с аналитикой](analytics-integration) и [интеграции атрибуции](attribution-integration).
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Вы создаёте флоу в визуальном редакторе Adapty, а SDK рендерит их автоматически.
- [**Паywalls, созданные вручную**](flutter-making-purchases): Вы строите собственный интерфейс пейвола в коде, но используете Adapty для получения продуктов и обработки покупок.
...
Не уверены, что выбрать? Прочитайте [таблицу сравнения в quickstart](flutter-quickstart-paywalls).
...
Тестируйте каждую покупку в песочнице по мере разработки — не откладывайте на конец. Инструкции по настройке см. в разделе [Тестирование покупок в песочнице](test-purchases-in-sandbox).
...
**Гайды:**
- [Включить покупки с Flow и Paywall Builder (быстрый старт)](flutter-quickstart-paywalls)
- [Получить флоу и их конфигурацию](flutter-get-pb-paywalls)
- [Отобразить флоу](flutter-present-paywalls)
- [Обработать события флоу](flutter-handling-events)
...
- **Возможная проблема:** Пустой флоу или ошибка `getFlow` → убедитесь, что ID плейсмента точно совпадает с указанным в дашборде и у плейсмента назначена аудитория.
:::
