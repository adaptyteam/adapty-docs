...
4. Скопируйте сгенерированный ключ из всплывающего окна и перейдите в [App Settings → Stripe](https://app.adapty.io/settings/stripe) в Adapty. Вставьте ключ в раздел **Stripe App Restricted API Key** в зависимости от вашего режима. Обратите внимание, что для тестового и боевого режимов нужно генерировать разные ключи. Используйте ключ из **Test mode** Stripe, а не из **Sandbox** — Adapty не поддерживает Stripe Sandboxes.
...
<summary>Устаревший процесс установки</summary>
...
2. Нажмите кнопку **Reveal live (test) key** рядом с заголовком **Secret key**, скопируйте ключ и перейдите в [App Settings → Stripe](https://app.adapty.io/settings/stripe) в Adapty. Вставьте ключ сюда:
...
4. Вставьте URL-адрес вебхука из Adapty в поле **Endpoint URL**. Затем выберите **Latest API version** в поле **Version** вебхука. После этого выберите следующие события:
...
5. Нажмите "Add endpoint", затем нажмите "Reveal" под полем "Signing secret". Это ключ, который используется для декодирования данных вебхука на стороне Adapty — скопируйте его после того, как он станет виден:
...
6. Наконец, вставьте этот ключ в настройки приложения Adapty **App Settings** → **Stripe** в поле «Stripe Webhook Secret»:
...
:::warning
Adapty сопоставляет входящие события Stripe по точной паре `product_id` + `price_id`. Изменение цены в Stripe создаёт новый `price_id`, и события для покупок по новой цене перестают совпадать без каких-либо предупреждений: Stripe показывает вебхук как доставленный, но Adapty не записывает транзакции. После любого изменения цены убедитесь, что каждый `price_id`, по которому вы продаёте, добавлен в продукт в Adapty.
:::
...
### Stripe показывает 100% доставку, но события не доходят до Adapty \{#stripe-shows-100-delivery-but-no-events-reach-adapty\}

Adapty сохраняет секрет подписи вебхука только в момент создания эндпоинта. Если в вашем Stripe-аккаунте уже существует вебхук-эндпоинт на URL Adapty — например, от предыдущей попытки подключения или после ручной настройки по устаревшей инструкции выше — повторное подключение не создаёт новый эндпоинт, и Adapty никогда не получает (или не получает повторно) секрет подписи. Все события по-прежнему доходят до существующего эндпоинта и получают ответ `200`, однако проверка подписи на стороне Adapty завершается ошибкой, и события отбрасываются.

Чтобы исправить это, в [Developers → Webhooks](https://dashboard.stripe.com/webhooks) удалите эндпоинт, указывающий на URL вашего вебхука Adapty, затем переподключите интеграцию со Stripe в [App Settings → Stripe](https://app.adapty.io/settings/stripe), чтобы Adapty создал новый эндпоинт и получил его секрет.
...
### Чистая выручка и налоги \{#net-revenue-and-tax\}

Для расчёта `net_revenue_usd` и `net_revenue_local` по транзакциям Stripe Adapty применяет фиксированный вычет 4,5%. Это расчётная оценка, а не фактически собранный Stripe налог — Adapty не считывает суммы налогов из Stripe, в том числе из Stripe Tax, поэтому `tax_amount_usd` и `tax_amount_local` для транзакций Stripe всегда равны `0`.
...
### Один аккаунт Stripe, подключённый к нескольким приложениям Adapty \{#one-stripe-account-connected-to-several-adapty-apps\}

Какое приложение Adapty обрабатывает событие, определяет URL эндпоинта вебхука, получившего это событие, — у каждого приложения Adapty свой URL эндпоинта, а не restricted API key. Каждый зарегистрированный в Stripe эндпоинт получает все события, и каждое приложение обрабатывает свою копию независимо. Приложение игнорирует события для продуктов, которые в него не добавлены, поэтому приложения с разными наборами продуктов мирно сосуществуют. Однако если один и тот же продукт и цена Stripe добавлены в несколько приложений, каждое из них зафиксирует транзакцию.
...
Обратите внимание, что `variation_id` читается только из метаданных объектов Stripe Subscription (`sub_...`) и Checkout Session (`ses_...`):
...
Adjust не имеет полей Stripe: Adapty отправляет покупку Stripe в Adjust с токеном приложения той платформы, на которой пользователь установил ваше приложение. См. [Веб-покупки (Stripe и Paddle)](adjust#web-purchases-stripe-and-paddle).
