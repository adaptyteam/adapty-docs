---
title: "Включение покупок с помощью Flow и Paywall Builder в React Native SDK"
description: "Quickstart-гайд по включению встроенных покупок с помощью Adapty Flow и Paywall Builder."
...
- [**Flows**](adapty-flow-builder) – последовательности экранов, которые показывают продукты пользователям; создаются в no-code Flow & Paywall Builder. SDK получает их через `getFlow`. Если вы предпочитаете строить UI в собственном коде, используйте пейвол — см. [Реализация пейволов вручную](react-native-quickstart-manual).
...
|---|---|---|
| Adapty Flow & Paywall Builder | ✅ Легко | Вы [создаёте готовый к покупке флоу в конструкторе без кода](quickstart-paywalls). Adapty автоматически отображает его и берёт на себя весь процесс покупки, валидацию чеков и управление подписками. |
| Пейволы, созданные вручную | 🟡 Средне | Вы реализуете UI пейвола в коде приложения, но получаете объект флоу из Adapty, сохраняя гибкость в управлении продуктами. См. [гайд](react-native-quickstart-manual). |
| Observer mode | 🔴 Сложно | У вас уже есть собственная инфраструктура для обработки покупок, и вы хотите продолжать её использовать. Обратите внимание, что observer mode имеет ограничения в Adapty. См. [статью](observer-vs-full-mode). |
...
5. [Установите и активируйте SDK](sdk-installation-reactnative) в коде приложения. В этом гайде используются API Adapty React Native SDK v4.

## 1. Получите флоу \{#1-get-the-flow\}

Ваши флоу связаны с плейсментами, настроенными в дашборде. Плейсменты позволяют показывать разные флоу для разных аудиторий или запускать [A/B-тесты](ab-tests).

Чтобы получить флоу, созданный в Adapty Flow & Paywall Builder, получите объект `flow` по ID [плейсмента](placements) с помощью метода `getFlow`. Флоу содержит элементы UI и стили, необходимые для его отображения.
