...
|---|---|
| `internal_id` | UUID, который принимает `--org` в [`campaigns create`](#adapty-asa-campaigns-create), а также `--campaign-group` в качестве [фильтра списка](#list-filters) |
| `org_id` | Числовой идентификатор организации Apple, принимаемый `--org-id` в [`campaigns bulk-create`](#adapty-asa-campaigns-bulk-create). `--org` и `--campaign-group` его не принимают |

`--org` существует только у `campaigns create`. Ни одна из команд list его не принимает.

Каждая строка также содержит поле `payment_model`. Если его значение равно `LOC`, организация выставляет счёт по кредитной линии, и каждая кампания в ней требует флагов [`--invoice-*`](#adapty-asa-campaigns-create).
...
Организация с `payment_model` равным `LOC` выставляет счета по кредитной линии, и Apple требует указания параметров выставления счёта для каждой из её кампаний. Передавайте все пять флагов `--invoice-*` в одном вызове — неполный набор отклоняется до отправки запроса. См. [Настройка параметров выставления счёта для кредитной линии](developer-cli-ads-manager#invoicing-options).

Ответ содержит `serving_status` и `serving_state_reasons`. Кампания может быть создана, но не запущена: кампания с типом `MAX_CONVERSIONS` ждёт [автоматической группы объявлений](#adapty-asa-ad-groups-create), а кампания с кредитной линией ждёт настройки параметров выставления счетов. В обоих случаях команда выводит причину и команду, которая её устраняет.
...
| `--from-file` | — | Bulk-шаблон Apple Ads для конвертации на сервере: [`Campaign_And_Adgroup_Template.xlsx`](https://ads.apple.com/app-store/help/campaigns/0089-make-edits-in-bulk) или [ключевые слова `.csv`](https://ads.apple.com/app-store/help/keywords/0014-add-and-manage-keywords) |
| `--org-id` | — | Числовой идентификатор организации Apple (`org_id`). См. [`orgs list`](#adapty-asa-orgs-list). Обязателен при использовании `--from-file` |
...
Если операция не завершилась за `--timeout` секунд, команда возвращает управление и выводит вызов [`bulk-status`](#adapty-asa-campaigns-bulk-status) для последующего запуска.
...
| Аргумент | Описание |
|---|---|
| `operation-id` | ID операции (UUID), выведенный командой [`campaigns bulk-create`](#adapty-asa-campaigns-bulk-create) |
...
Получите готовый пул ключевых слов для одного из ваших приложений. Это те же пулы, которые использует Ads Manager при настройке кампании в дашборде.
...
| Флаг | Обязательный | Описание |
|---|---|---|
| `--adam-id` | Да | Apple App Store ID (`adam_id`) вашего приложения, из [`asa apps list`](#adapty-asa-apps-list) |
| `--type` | Да | `brand`, `generic` или `competitor` |
| `--country` | Нет | Двухбуквенный код страны ISO. Повторите для нескольких стран. По умолчанию: все страны в пуле |

Флаг `--type` выбирает пул:

| Тип | Ключевые слова |
|---|---|
| `brand` | Брендовые запросы вашего приложения, их варианты написания и органические запросы, содержащие бренд. В результатах также перечислены сами брендовые запросы |
| `generic` | Небрендовые запросы, по которым ранжируется ваше приложение и его главные органические конкуренты, — с удалёнными брендовыми запросами. `relevance_tier: top_organic` отмечает запросы, по которым ваше приложение уже занимает позиции в органическом топ-10 |
| `competitor` | Брендовые запросы каждого конкурента, выбранного для приложения в [AI Growth Advisor](autopilot-analysis#select-competitors), по одному пулу на конкурента. Остаётся пустым до выбора конкурентов |

Каждое ключевое слово содержит страну, популярность, оценку и медианный органический ранг. Ставки и типы соответствия не заданы — выберите их самостоятельно, затем добавьте ключевые слова с помощью [`asa keywords add`](#adapty-asa-keywords-add).

Проверьте `status` пула перед использованием его ключевых слов:

| Статус | Значение |
|---|---|
| `ready` | Пул сформирован |
| `building` | Пул ещё формируется. Повторите попытку примерно через минуту |
| `empty` | Пул сформирован, но не содержит ключевых слов |
| `failed` | Формирование завершилось ошибкой. Повторите попытку позже |

Первый вызов `brand` или `generic` для приложения формирует пул во время запроса и может занять несколько десятков секунд. Команда имеет лимит 10 вызовов в минуту на компанию, и одновременно выполняется только одна сборка `brand` или `generic` на компанию. Второй вызов в это время вернёт `429 cli_analytics_busy` с заголовком `Retry-After: 5`.
...
CLI сохраняет JSON-правило, которое вы передаёте, с одним исключением: настройки действия **Add as keyword** задаются через флаги. Смотрите [Автоматизации](ads-manager-automations) для описания каждого типа правил и [Запуск правил автоматизации](developer-cli-ads-manager#run-automation-rules) — для создания файла правила.

### Флаги действия «добавить как ключевое слово» \{#add-keyword-flags\}

Команды `automations create` и `automations update` имеют общий набор флагов, задающих `actions[0].params` для действия `add-as-keyword-to` — **Add as keyword** в [правиле поисковых запросов](ads-manager-automations-search-terms), **Add as keyword to…** в [правиле для ключевых слов](ads-manager-automations-keyword-rules). Передача любого из этих флагов заменяет весь объект `params`, поэтому вводить JSON вручную для этого действия не нужно.

| Флаг | JSON-поле | Описание |
|---|---|---|
| `--target-ad-group` | `targets.internal_ids` | Группа объявлений, получающая ключевое слово (UUID), может повторяться. Правило без целевой группы объявлений не делает ничего |
| `--match-type` | `match_type` | `BROAD` или `EXACT` |
| `--cpt-bid-type` | `cpt_bid.type` | Источник ставки для нового ключевого слова: `ad_group_default_bid`, `set_to`, `search_term_current_cpt` или `keyword_current_bid` |
| `--cpt-bid` | `cpt_bid.value` | При `set_to` — сама ставка. При `search_term_current_cpt` или `keyword_current_bid` — процентная наценка к копируемой ставке. Не принимается при `ad_group_default_bid` |
| `--negate` | `negate` | Также добавляет поисковый запрос как минус-слово — в своей `ad-group` или своей `campaign`. `--no-negate` не оставляет минус-слов. Только для правил поисковых запросов |
| `--skip-enable-duplicates` | `skip_enable_duplicate_keywords` | Пропускать ключевое слово, которое уже существует в целевой группе объявлений, вместо его активации. Только для правил поисковых запросов |
| `--pause-original` | `pause_in_original_ad_group` | Приостановить исходное ключевое слово в его группе объявлений. Только для правил ключевых слов |

`--cpt-bid-type` и `--match-type` не имеют значений по умолчанию, поэтому оба обязательны, если файл правила не содержит собственных `params`.

Четыре флага зависят от поля `operate_with` правила: `--negate`, `--no-negate` и `--skip-enable-duplicates` относятся к правилу `search-term`, `--pause-original` — к правилу `targeting-keyword`. Флаг из другого набора завершается с кодом выхода `2` и выводит список применимых флагов.

:::warning
Берите `params` для этого действия из флагов, а не из правила с другим действием. API определяет `params` по форме, а не по названию варианта — поэтому ключ, принадлежащий другому действию, заставит его выбрать то действие и проигнорировать остальное: вернёт `200` и сохранит правило, которое не добавит ключевые слова ни в одну группу объявлений.
:::
...
Создаёт правило автоматизации из JSON-файла с описанием правила.
...
| `--run-now` | Ставит в очередь первый запуск сразу после сохранения правила |

`--file` обязателен, и файл должен содержать ровно одно действие: API хранит одно действие и одно условие на правило.

Также принимает [флаги действия add-as-keyword](#add-keyword-flags), которые заполняют или переопределяют `actions[0].params`:
...
`--start` и `--stop` взаимоисключают друг друга. Файл, переданный здесь, не должен содержать `internal_id`.

Также принимает [флаги действия add-as-keyword](#add-keyword-flags). Передача одного из них превращает вызов в операцию чтение-изменение-запись: CLI читает правило, перестраивает `actions[0].params` и отправляет весь список `actions` обратно, поскольку API заменяет `actions` целиком, а не объединяет с существующим. Правки, сделанные в дашборде между чтением и записью, будут перезаписаны.
...
| Флаг | Обязательный | Описание |
|---|---|---|
| `--entity` | Да | На что составлять отчёт: `campaign`, `ad-group`, `keyword` или `ad` |
| `--date-from` | Да | Начало периода (`YYYY-MM-DD`) |
| `--date-to` | Да | Конец периода (`YYYY-MM-DD`) |
| `--metric` | Да | Название метрики, можно указывать несколько раз |
| `--app` | Нет | Фильтровать только по этим приложениям. Можно указывать несколько раз |
| `--campaign` | Нет | Фильтровать только по этим кампаниям. Можно указывать несколько раз |
| `--ad-group` | Нет | Фильтровать только по этим группам объявлений. Можно указывать несколько раз |
| `--group-by` | Нет | Группировка строк по `country`, `day`, `week`, `month`, `quarter` или `year`. Можно указывать несколько раз |
| `--by-days` | Нет | Окно продления в днях для метрик когорт, можно указывать несколько раз. Максимум 16 за вызов. Если не указать — используются значения по умолчанию из дашборда |
| `--order-by` | Нет | Метрика или поле для сортировки |
| `--order-by-day` | Нет | Ранжировать по метрике когорты при данном окне продления. Должно быть одним из значений `--by-days` |
| `--order` | Нет | `asc` или `desc`. По умолчанию: `desc` |

Принимает [флаги пагинации](#global-flags). Из [фильтров списка](#list-filters) поддерживает только `--app`, `--campaign` и `--ad-group`; параметров `--search` и `--status` нет. Чтобы сузить отчёт по другим критериям, сопоставьте строки с идентификаторами из команды `list` с нужной областью.

Каждая строка — одна сущность, агрегированная на сервере и отсортированная по `--order-by`. Запрос «топ-N» — это один вызов: задайте `--order-by` и `--page-size N`, не перебирая страницы и не суммируя результаты вручную:

```bash
adapty asa metrics --entity campaign --date-from 2026-07-01 --date-to 2026-07-31 --order-by spend --page-size 5
```

`--metric` принимает названия метрик, которые отслеживает Ads Manager, в номенклатуре дашборда — например `spend`, `taps` или `gross_roas`. Полный список и описание расчёта каждой метрики см. в разделе [Метрики](adapty-ads-manager-metrics). Если указать несуществующее название, команда завершится с ошибкой и выведет список допустимых значений.

Каждая метрика вычисляется по всему уровню сущности до разбивки на страницы, поэтому запрашивайте только нужные столбцы, а не весь каталог. `subscribers` и `paid_subscribers` считают уникальные профили на сущность — это примерно в семнадцать раз дороже простого запроса расходов и установок; то же касается `arppu` и `arpas`, которые производятся от них. На любом `--entity` эти четыре имени требуют `--campaign` или `--ad-group` для сужения запроса:

```bash
adapty asa metrics --entity keyword --date-from 2026-07-01 --date-to 2026-07-31 --metric arpas --campaign <campaign-id>
```

Без одного из этих двух фильтров запрос завершается ошибкой `422 cli_metric_scope_too_wide`. Сужение области — самый дешёвый способ ускорить любой запрос метрик, поскольку стоимость определяется количеством агрегируемых сущностей, а не размером страницы.

Длина периода отчётности ограничена наименьшим значением `--group-by`. Чтобы охватить более длинный период, используйте более крупную группировку, а не разбивайте запрос на несколько частей:

| Наименьшее значение `--group-by` | Максимальный период |
|---|---|
| `day` | 28 дней |
| Без группировки по периоду | 90 дней |
| `week` | 180 дней |
| `month` и крупнее | 365 дней |

Гранулярность по дням охватывает не более четырёх недель. Для более длительного периода запрашивайте данные с гранулярностью `week` — один запрос вмещает до полугода.

Каждая страница также ограничена 5000 строк разбивки, где строка — это одна сущность × страна × период. Если страница превышает лимит, запрос завершается ошибкой `422 cli_response_too_large`. Уменьшите детализацию группировки, сузьте временной диапазон или снизьте значение `--page-size`. При группировке по `day` ошибка сообщает подходящий размер страницы — используйте именно его, а не вычисляйте самостоятельно. Команда `metrics overview` не возвращает разбивку по сущностям, поэтому это ограничение к ней не применяется.

Метрики `ltv` не существует. Lifetime value — это когортная метрика, считываемая в окне продления, поэтому `--by-days` — это способ запросить значение за день 7 или день 90:

```bash
adapty asa metrics --entity campaign --date-from 2026-07-01 --date-to 2026-07-31 --metric roas --by-days 7 --by-days 90
```

`--order-by-day` сортирует строки по одному из этих окон, что возвращает топ кампаний по ROAS за день 90 в одном вызове.
...
| Статус | Код | Описание |
|---|---|---|
| `402` | `ads_manager_subscription_required` | У компании нет активной подписки на Ads Manager |
| `404` | — | Сущность не существует или принадлежит другой компании |
| `409` | `cli_idempotency_in_progress` | Запись с тем же ключом идемпотентности ещё выполняется |
| `422` | `cli_idempotency_key_reuse` | Тот же ключ идемпотентности использован с другим телом запроса |
| `422` | `cli_metric_scope_too_wide` | Запрошена метрика подсчёта профилей без `--campaign` или `--ad-group` |
| `422` | `cli_response_too_large` | Страница `metrics` превысит 5000 строк разбивки |
| `429` | `cli_analytics_busy` | Пул аналитики занят или ещё выполняется другой сбор пула ключевых слов для компании. Время ожидания указано в заголовке `Retry-After` |
| `429` | `cli_cooldown_active` | Слишком много отклонённых запросов перевело токен в режим ожидания |
