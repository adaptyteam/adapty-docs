...
- **Ограничение частоты запросов**: применяется на уровне API-ключа и назначается для каждого приложения отдельно, поэтому единого значения нет — см. [Ограничения частоты запросов](https://adapty.io/docs/ru/ss-authorization.md#rate-limits). Это устойчивая частота, а не бюджет в минуту; некоторые эндпоинты имеют более низкий лимит: [createVirtualCurrencyTransaction](https://adapty.io/docs/ru/api-adapty/operations/createVirtualCurrencyTransaction.md) по умолчанию допускает 600 запросов в минуту на приложение. Настройте агента на повторные попытки с экспоненциальной задержкой при получении `429` и на соблюдение заголовка `Retry-After`, если он присутствует.
- **Ключи для конкретных приложений**: каждый ключ работает только для одного приложения; используйте соответствующий ключ для каждого из них.
