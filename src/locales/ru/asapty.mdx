---
title: "Asapty"
description: "Узнайте об Asapty и его роли в экосистеме подписок Adapty."
...
С помощью интеграции [Asapty](https://asapty.com/) вы можете оптимизировать свои кампании в Search Ads. Adapty отправляет события подписок в Asapty, где вы можете создавать собственные дашборды на основе атрибуции Apple Search Ads.

Эта конкретная интеграция не добавляет никаких данных атрибуции в Adapty, поскольку у нас уже есть всё необходимое напрямую из [ASA](apple-search-ads).
...
### События не доходят до Adapty \{#events-dont-reach-asapty\}

- Убедитесь, что вы настроили [Apple Search Ads](apple-search-ads) в Adapty и [загрузили учётные данные](https://app.adapty.io/settings/apple-search-ads) — без них Asapty не будет работать.
- Только профили с детальной, неорганической атрибуцией ASA будут передавать свои события в Asapty. Если атрибуция недостаточна, вы увидите сообщение «The user profile is missing the required integration data.»
- Профили, созданные до настройки интеграций, не смогут передавать события в Asapty.
- Если интеграция с Adapty не работает, несмотря на правильную настройку, убедитесь, что переключатель **Receive Apple Search Ads attribution in Adapty** включён на вкладке [**App Settings** -> **Apple Search Ads**](https://app.adapty.io/settings/apple-search-ads).

### `access_level_updated` отображается как сбой в Event Feed \{#access_level_updated-shows-as-failed-in-the-event-feed\}
