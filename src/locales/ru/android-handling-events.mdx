...
:::important
Этот гайд охватывает обработку событий для покупок, восстановлений, выбора продукта и рендеринга флоу. Вам также необходимо реализовать обработку кнопок (закрытие флоу, открытие ссылок и т. д.). Подробнее см. в нашем [гайде по обработке действий с кнопками](android-handle-paywall-actions).
:::

Флоу и пейволы, настроенные в [Flow & Paywall Builder](adapty-flow-builder), не требуют дополнительного кода для совершения и восстановления покупок. Однако они генерируют ряд событий, на которые ваше приложение может реагировать. К таким событиям относятся нажатия кнопок (кнопки закрытия, URL-ссылки, выбор продуктов и т. д.), а также уведомления о действиях, связанных с покупками. Ниже описано, как обрабатывать эти события.

<SampleApp />

Если вам нужно контролировать или отслеживать процессы, происходящие на экране покупки, реализуйте методы `AdaptyFlowEventListener`.
...
Метод не будет вызван в режиме Observer. Подробнее см. в разделе [Android — отображение флоу в режиме Observer](android-present-paywall-builder-paywalls-in-observer-mode).
...
Мы рекомендуем закрывать экран в этом случае.

Метод не вызывается в режиме Observer. Подробнее см. в разделе [Android — Отображение флоу в режиме Observer](android-present-paywall-builder-paywalls-in-observer-mode).
...
Этот метод не вызывается в режиме Observer. Подробнее см. в разделе [Android — отображение флоу в режиме Observer](android-present-paywall-builder-paywalls-in-observer-mode).

#### Завершение навигации к веб-оплате \{#finished-web-payment-navigation\}

Этот метод вызывается после попытки открыть [веб-пейвол](web-paywall) для конкретного продукта. Это касается как успешных, так и неуспешных попыток навигации:
...
Подробнее о том, что с ними делать, см. в разделе [Отслеживание просмотров экранов флоу](android-flow-screen-views).

Флоу также передаёт значения, которые пользователи вводят и выбирают. Подробнее см. в разделе [Обработка данных из флоу](android-flow-input).
...
| Метод | Описание |
|:-------|:------------|
| **onShowAppRate** | Зарезервирован для запросов оценки приложения из флоу. Флоу пока не инициируют запросы оценки, поэтому реализовывать этот метод не нужно. |
| **onShowRequestPermission** | Зарезервирован для запросов системных разрешений (например, push-уведомлений или доступа к камере) из флоу. Флоу пока не инициируют запросы разрешений, поэтому реализовывать этот метод не нужно. |
...
Пейволы, настроенные с помощью [Paywall Builder](adapty-paywall-builder), не требуют дополнительного кода для совершения и восстановления покупок. Однако они генерируют ряд событий, на которые ваше приложение может реагировать. К ним относятся нажатия кнопок (кнопки закрытия, URL-адреса, выбор продуктов и т. д.), а также уведомления о действиях, связанных с покупками, выполненных на пейволе. Ниже описано, как реагировать на эти события.

:::warning
Этот гайд охватывает только **пейволы, созданные в старом билдере**, для которых требуется Adapty SDK v3.0 или выше.
:::
...
<TabItem value="new" label="SDK версии 3.10.0 и выше" default>

Когда пользователь пытается оформить новую подписку, пока активна другая, вы можете управлять тем, как обрабатывать новую покупку, переопределив этот метод. Доступны два варианта:
