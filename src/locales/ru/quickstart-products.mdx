...
description: "Добавляйте встроенные покупки и подписки в Adapty и связывайте их с листингами в App Store, Google Play, Stripe, Paddle или кастомном сторе."
...
Прежде чем использовать основные функции Adapty, нужно добавить каждый продукт, который вы продаёте, и привязать его к каждому стору или платёжной платформе. Это позволит доставлять продукты на устройства пользователей и отслеживать их в аналитике.

В Adapty всё, что продаёт ваше приложение, — это **продукт**. Если один и тот же товар есть в App Store, Google Play или Stripe, вы можете объединить их в один продукт в Adapty. Настройте один раз и управляйте им на всех платформах из одного места.
...
<Tabs groupId="products" queryString>
    ...
    <TabItem value="no-products" label="Продуктов в сторах ещё нет" default>
        ...
        6. Для iOS настройте introductory offer — бесплатный пробный период — выбрав его **Free duration** из выпадающего списка. На этом этапе можно добавить бесплатный пробный период. После того как основной продукт пройдёт ревью в сторе, вы сможете [добавить другие офферы](offers) (например, promotional или win-back), привязав их существующие ID из консоли стора.
