...
:::important
Этот гайд охватывает обработку событий для покупок, восстановлений, выбора продукта и рендеринга флоу. Вам также необходимо реализовать обработку нажатий кнопок (закрытие флоу, открытие ссылок и т.д.). Подробнее см. в нашем [гайде по обработке действий флоу](kmp-handle-paywall-actions).
:::

Флоу и пейволы, которые отображает Adapty, не требуют дополнительного кода для совершения и восстановления покупок. Однако они генерируют события, на которые ваше приложение может реагировать. К таким событиям относятся нажатия кнопок (кнопки закрытия, URL-ссылки, выбор продуктов и т. д.), а также уведомления о действиях, связанных с покупками. Ниже описано, как обрабатывать эти события.
...
В нормальных условиях такие ошибки не должны возникать, поэтому если вы с ними столкнулись — сообщите нам.
...
Kotlin Multiplatform передаёт параметры события в виде сырой JSON-строки, поэтому перед использованием декодируйте `paramsJsonString`.

<FlowScreenShowedParams />

О том, что делать с этими данными, читайте в разделе [Отслеживание просмотров экранов флоу](kmp-flow-screen-views).

Флоу также передаёт значения, которые пользователи вводят и выбирают. Подробнее об их чтении — в разделе [Обработка данных из флоу](kmp-flow-input).
...
Смотрите [гайд по обработке действий флоу](kmp-handle-paywall-actions) для полного списка действий.
...
:::warning
Этот гайд предназначен **только для пейволов, созданных в старом билдере**.
:::
