...
2. `openWebPaywall(paywall)` — генерирует URL по пейволу без добавления данных продукта в URL. В SDK v4 принимает `AdaptyFlowPaywall` — вариант пейвола из полученного флоу. Перед обращением по индексу убедитесь, что `flow.paywalls` не пустой, например `flow.paywalls[0]`.
:::
