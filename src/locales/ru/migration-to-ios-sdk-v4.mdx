...
## Временное удаление поддержки продвигаемых встроенных покупок App Store \{#app-store-promoted-in-app-purchases-temporarily-removed\}

В рамках миграции на StoreKit 2 Adapty iOS SDK 4.0 убирает поддержку продвигаемых встроенных покупок App Store. Метод делегата `shouldAddStorePayment(for:)` и тип `AdaptyDeferredProduct`, который он принимает, недоступны в версии 4.0.

:::warning
Это ограничение касается только версии 4.0 — поддержка продвигаемых встроенных покупок вернётся в SDK 4.1 с новым API. Если ваше приложение использует продвигаемые встроенные покупки, выполните миграцию с 3.x напрямую на 4.1. См. [Встроенные покупки из App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
