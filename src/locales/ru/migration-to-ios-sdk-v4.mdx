---
title: "Миграция iOS SDK Adapty до версии v4.0"
description: "Мигрируйте на Adapty iOS SDK v4.0 (beta): замените paywall API на flow API, совместимые как с Flow Builder, так и с Paywall Builder."
...
## Режим Kids Mode: отдельные продукты заменены на параметр пакета \{#kids-mode-separate-products-replaced-by-a-package-trait\}

В v3 вы включали [Kids Mode](kids-mode), выбирая отдельные продукты пакета **Adapty_KidsMode** и **AdaptyUI_KidsMode** и переименовывая импорты. Начиная с `4.0.0-beta.2`, эти продукты удалены. Kids Mode теперь является параметром Swift-пакета (Swift package trait) под названием `KidsMode` для обычного пакета Adapty — его включение исключает IDFA и AdSupport из всего SDK на уровне компиляции.
...
2. Включите трейт `KidsMode`. В Xcode 26.4 и новее включите его для зависимости AdaptySDK-iOS в разделе **Package Dependencies** вашего проекта. Если вы добавляете Adapty как зависимость в `Package.swift` (требуется `swift-tools-version` 6.1 или новее), включите его там:
...
3. Измените импорты обратно на обычные модули:
   ...
   ```
:::note
Версии Xcode ранее 26.4 не позволяют включить трейты для проекта Xcode через интерфейс. В таком случае создайте небольшой локальный Swift-пакет, зависящий от Adapty с включённым трейтом `KidsMode`, и добавьте зависимость от этого пакета в целевой объект вашего приложения.
:::
...
В v3 `didFinishPurchase` был необязательным: если вы его не реализовывали, пейвол закрывался автоматически после успешной покупки. Начиная с `4.0.0-beta.2`, эта реализация авто-закрытия по умолчанию удалена, чтобы флоу мог продолжить работу после успешной покупки — например, показать оставшиеся экраны флоу. Теперь вы сами решаете, что происходит после покупки: закрыть экран или ничего не делать, чтобы флоу продолжился.
