...
description: "Мигрируйте на Adapty iOS SDK v4.0, заменив paywall API на flow API, которые отображают флоу наряду со старыми пейволами из билдера."
metadataTitle: "Миграция на Adapty iOS SDK v4.0 | Документация Adapty"
---

Adapty iOS SDK 4.0 вводит флоу и соответствующим образом переименовывает paywall API. Новые API работают с флоу и по-прежнему совместимы с пейволами из старого билдера — никаких изменений на стороне дашборда Adapty не требуется.
...
Adapty iOS SDK 4.0 отказывается от поддержки CocoaPods. Устанавливайте SDK через [Swift Package Manager](sdk-installation-ios#install-adapty-sdk). Версия v4 также требует **Xcode 26** или новее, так как манифест пакета объявляет Swift tools 6.2.

Если ваш проект всё ещё использует CocoaPods, удалите поды `Adapty` и `AdaptyUI` из `Podfile`, выполните `pod install` для их удаления, затем добавьте пакет в Xcode через **File → Add Package Dependency**, указав `https://github.com/adaptyteam/AdaptySDK-iOS.git`.
...
## Обработка promoted-покупок из App Store временно удалена \{#app-store-promoted-in-app-purchases-temporarily-removed\}

В рамках миграции на StoreKit 2 в Adapty iOS SDK 4.0 удалён API для обработки promoted-покупок из App Store: метод делегата `shouldAddStorePayment(for:)` и тип `AdaptyDeferredProduct`, который он получает, недоступны в версии 4.0. Такие покупки по-прежнему завершаются, и Adapty фиксирует транзакцию — однако 4.0 не даёт приложению возможности перехватить или отложить их.

:::warning
Это удаление касается только версии 4.0 — поддержка возвращается в SDK 4.1 с новым API. Если ваше приложение перехватывает продвигаемые встроенные покупки, мигрируйте с версии 3.x напрямую на 4.1. См. [Встроенные покупки из App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
...
Возвращаемые типы меняются с `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` на `AdaptyFlow` / `AdaptyUI.FlowConfiguration`. Параметр `locale` переходит из вызова получения данных в `getFlowConfiguration`: `AdaptyFlow` также раскрывает `paywalls` — варианты пейвола флоу, как значения `AdaptyFlowPaywall`. Веб-методы пейвола принимают одно из этих значений.
...
Формат резервного файла [изменился в SDK v4](fallback-flows). Скачайте новый файл в разделе **[Placements](https://app.adapty.io/placements)** > **Fallbacks** и добавьте его в ваше приложение.

## Отслеживание просмотров пейвола \{#tracking-paywall-views\}

### logShowPaywall(_:) → logShowFlow(_:)

`logShowPaywall` переименован в `logShowFlow` и теперь принимает `AdaptyFlow` вместо `AdaptyPaywall`. Событие по-прежнему логируется для той же вариации, поэтому существующие метрики воронки и A/B-тестов продолжают работать без изменений в дашборде.
...
Как и в v3, вам не нужно вызывать этот метод при отображении флоу или пейволов, рендеримых Adapty — Adapty отслеживает эти просмотры автоматически.
...
:::note
В SDK 4.1 этот метод переименован в `updateExternalAttribution(_:provider:)`, а тип — в `AdaptyExternalAttributionProvider`. Старые названия удалены в версии 4.1 без предварительного устаревания. Если вы переходите сразу на версию 4.1, см. [Миграция на v4.1](migration-to-ios-sdk-41).
:::
