...
Инпуты и группы выбора автоматически передают свои значения. Чтобы различать их в коде, задайте каждому инпуту понятный **Element ID**, а каждой группе выбора — **Group ID** в билдере.
