...
description: "Обрабатывайте события флоу и пейвола в iOS-приложении."
...
| **didFinishPurchase**  | обязательный | Вызывается при успешном завершении `Adapty.makePurchase()`. Флоу не закрывается автоматически — здесь можно установить привязку презентации в `false` или ничего не делать, позволив флоу продолжить работу после покупки.                                          |
...
| **didReceiveError**    | обязательный | Вызывается при возникновении ошибки рендеринга или ошибки времени выполнения в скрипте флоу (например, исключение JavaScript, код `AdaptyUIError` `4105`). В случае ошибки рендеринга [обратитесь в поддержку Adapty](mailto:support@adapty.io).                   |
| **placeholderBuilder** | необязательный | Функция для отображения плейсхолдера во время загрузки флоу или пейвола. По умолчанию используется `ProgressView`.                                                                                                                                                |
| **fullScreen**         | необязательный | Определяет, отображается ли флоу или пейвол в полноэкранном режиме или в виде шторки. По умолчанию `true`.                                                                                                                                                        |
...
| **didPerformAction**   | необязательный | Вызывается при нажатии пользователем на кнопку. Предопределены два идентификатора действий: `close` и `openURL`; остальные — пользовательские и задаются в билдере.                                                                                               |
...
| **didFinishWebPaymentNavigation** | необязательный | Вызывается при завершении навигации в рамках веб-оплаты.                                                                                                                                                                                       |
| **didStartRestore**    | необязательный | Вызывается, когда пользователь начинает процесс восстановления покупок.                                                                                                                                                                                           |
| **didFailLoadingProducts** | необязательный | Вызывается при ошибках во время загрузки продуктов. Верните `true`, чтобы повторить загрузку.                                                                                                                                                                 |
| **didPartiallyLoadProducts** | необязательный | Вызывается, когда продукты загружены частично.                                                                                                                                                                                                              |
| **showAlertItem**      | необязательный | Привязка, управляющая отображением алертов поверх флоу или пейвола.                                                                                                                                                                                               |
| **showAlertBuilder**   | необязательный | Функция для отображения алерта.                                                                                                                                                                                                                                   |

## Обработка событий в UIKit \{#handling-events-in-uikit\}

Для приложений на UIKit события обрабатываются через протокол `AdaptyFlowControllerDelegate`. Подробнее о настройке `AdaptyFlowController` с `AdaptyFlowControllerDelegate` смотрите в разделе [Отображение флоу и пейволов — iOS](ios-present-paywalls).
...
`didFinishPurchase` и `didFailPurchase` не имеют реализаций по умолчанию и должны быть реализованы. Контроллер не закрывается автоматически после успешной покупки — вызовите `controller.dismiss(animated:)` в нужный момент или оставьте всё как есть, чтобы многоэкранный флоу продолжился после покупки.
...
Вызывается после завершения навигации при веб-оплате — успешного или нет.
...
Смотрите [Отслеживание просмотров экранов флоу](ios-flow-screen-views), чтобы узнать, как с ними работать.

Флоу также передаёт значения, которые пользователи вводят и выбирают. Смотрите [Обработка данных из флоу](ios-flow-input), чтобы их считать.
...
:::important
Этот гайд охватывает обработку событий для покупок, восстановлений, выбора продукта и отрисовки пейвола. Вам также необходимо реализовать обработку кнопок (закрытие пейвола, открытие ссылок и т. д.). Подробнее см. в [гайде по обработке действий кнопок](handle-paywall-actions).
:::

Пейволы, настроенные с помощью [Paywall Builder](adapty-paywall-builder), не требуют дополнительного кода для совершения и восстановления покупок. Однако они генерируют ряд событий, на которые ваше приложение может реагировать. Среди этих событий — нажатия кнопок (кнопки закрытия, URL, выбор продуктов и т. д.), а также уведомления о действиях, связанных с покупками на пейволе. Ниже описано, как реагировать на эти события.

Этот гайд охватывает только **пейволы, созданные в старом билдере**, для работы с которыми требуется Adapty SDK версии 3.0 или выше.

:::tip

Хотите посмотреть на реальный пример интеграции Adapty SDK в мобильное приложение? Ознакомьтесь с нашими [демо-приложениями](sample-apps) — они демонстрируют полную настройку: отображение пейволов, совершение покупок и другие базовые функции.
...
Для управления процессами, происходящими на экране пейвола в вашем мобильном приложении, или их отслеживания реализуйте методы `AdaptyPaywallControllerDelegate`.
...
В режиме Observer mode этот метод не вызывается. Подробнее см. в разделе [iOS — Показ пейволов Paywall Builder в режиме Observer mode](ios-present-paywall-builder-paywalls-in-observer-mode).
...
В обычной ситуации такие ошибки возникать не должны, поэтому если вы столкнулись с одной из них, пожалуйста, сообщите нам.
