...
Интеграция с [SplitMetrics Acquire](https://splitmetrics.com/acquire/) позволяет точно отслеживать доход от подписок, полученный через Apple Search Ads, и видеть, сколько денег приносит реклама на протяжении нескольких месяцев.

Кроме того, Adapty отправляет [события подписок](events) в SplitMetrics Acquire, чтобы вы могли строить кастомные дашборды и автоматизацию на основе атрибуции Apple Search Ads.
Данные об атрибуции в Adapty при этом не добавляются — всё необходимое мы уже получаем напрямую из ASA.
