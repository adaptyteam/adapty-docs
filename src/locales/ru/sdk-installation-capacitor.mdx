...
На iOS Adapty SDK подключает нативные SDK только через **Swift Package Manager** — podspec для CocoaPods был удалён ([репозиторий спеков CocoaPods переходит в режим только для чтения в декабре 2026 года](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). iOS-проект вашего приложения должен использовать интеграцию Capacitor через SPM:
