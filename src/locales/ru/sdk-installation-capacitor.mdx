...
Для сборки под iOS с Adapty SDK v4 (beta) требуется **Xcode 26** или новее — нативный iOS SDK использует Swift tools 6.2. Требования к iOS 15.0+, Capacitor 8 и Android minSdk 24 остаются такими же, как для SDK 3.16+.

:::info
Adapty Capacitor SDK 4.0.2 и новее работает с Google Play Billing Library v8.

Совместимость с версией Billing Library не означает, что Adapty поддерживает все возможности, которые Google в ней представила. Прежде чем использовать новые функции Google Play Billing, ознакомьтесь с разделом [Продукты в Play Store](android-products).
:::
