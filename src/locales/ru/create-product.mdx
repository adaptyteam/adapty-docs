...
:::
## Создание продукта и его публикация в сторе \{#create-product-and-push-to-store\}

:::warning
Перед началом убедитесь, что вы настроили интеграцию с нужными сторами:
- [App Store](initial_ios)
- [Google Play](initial-android)

Если интеграция с App Store была настроена давно, убедитесь, что вы [добавили ключ App Store Connect API](app-store-connection-configuration#step-6-add-app-store-connect-api-key).
:::

<YouTube id="qUpC2XG-r5E" />

Чтобы добавить новый продукт в ваше приложение:
...
2. Нажмите **Create product** в правом верхнем углу. Adapty поддерживает все типы продуктов: подписки, нерасходуемые \(включая пожизненный доступ\) и расходуемые покупки.
...
## Создайте продукт и подключите существующие продукты стора \{#create-product-and-connect-existing-store-products\}

:::warning
Прежде чем начать, убедитесь, что вы:
- Настроили интеграцию с нужными сторами:
  - [App Store](initial_ios)
  - [Google Play](initial-android)
...
**Если у вас ещё нет созданных продуктов**, рекомендуем воспользоваться гайдом [Опубликовать в сторы](#create-product-and-push-to-store) — так вы создадите продукты одновременно в Adapty и в сторах.
:::
...
4. Введите следующие данные:
    ...
    - **Product name**: введите название продукта, которое будет использоваться в дашборде Adapty. Это название служит прежде всего для вашего удобства, поэтому выбирайте то, с которым вам комфортно работать в дашборде Adapty.
