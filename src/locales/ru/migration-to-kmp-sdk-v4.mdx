---
title: "Миграция Adapty Kotlin Multiplatform SDK на v4.0"
description: "Перейдите на Adapty Kotlin Multiplatform SDK v4.0 (beta): замените paywall API на flow API, совместимые как с Flow Builder, так и с Paywall Builder."
...
:::note
Флоу-вью предназначено для однократного использования: после вызова `dismiss()` вью уничтожается, поэтому для повторного показа флоу вызовите `createFlowView` снова.
:::
...
- `AdaptyUI.setObserverModeResolver(...)` с `AdaptyUIObserverModeResolver` — управляет покупками и восстановлениями, инициированными из флоу, пока SDK работает в [режиме наблюдателя](implement-observer-mode-kmp). Ранее это было доступно только в нативных SDK для iOS и Android. См. [Отображение флоу в режиме наблюдателя](kmp-present-flows-in-observer-mode).
- `AdaptyUI.setSystemRequestsHandler(...)` с `AdaptyUISystemRequestsHandler` — зарезервировано для системных запросов из флоу (запросы разрешений ОС и запросы на оценку приложения). Флоу пока не инициируют такие запросы, поэтому регистрировать обработчик не обязательно.
- Новый необязательный колбэк `flowViewDidReceiveAnalyticEvent` передаёт аналитические события из флоу, начиная с просмотра экрана при каждом открытии пользователем нового экрана. См. [Отслеживание просмотров экранов флоу](kmp-flow-screen-views).
- `AdaptyUI.openWebUrl(url, openIn)` и `AdaptyUI.requestAppReview()` — обеспечивают стандартную обработку `OpenUrlAction` и `handleAppReviewRequest`, поэтому URL и запросы на оценку приложения обрабатываются нативно из коробки. Вызывайте их напрямую только если переопределяете поведение по умолчанию.
- `AdaptyUIFlowView.locale` — возвращает локализацию, с которой было построено представление, чтобы вы могли определить, какая именно локализация отображается пользователю. Требуется SDK 4.0.1-beta.1 или более поздней версии.
