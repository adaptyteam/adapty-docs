...
| :-------------- | :------------- | :----------------------------------------------------------- |
| **flow**        | обязательный   | Объект `AdaptyFlow`, полученный через `Adapty.getFlow`. |
| **locale**      | необязательный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder) для отображения, задаётся как код языка с одним или двумя подтегами через `-` (например, `en`, `pt-br`). Если не указан, отображение происходит на `en` или в локализации флоу по умолчанию, если `en` отсутствует. См. [Локализации и коды языков](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>необязательный</p><p>по умолчанию: `null`</p><p>SDK 4.1+</p> | Пользовательский ID макета в конфигурации макетов флоу. Передайте его, чтобы отобразить конкретный макет вместо того, который SDK выбирает автоматически исходя из типа устройства и размера экрана. Если макет с указанным ID не найден, вызов завершится ошибкой no-view-configuration. Flow & Paywall Builder пока не присваивает пользовательские ID макетам, поэтому оставьте это поле пустым. |
| **loadTimeout** | по умолчанию: 5 сек | Ограничивает таймаут для этого метода. При его истечении будут возвращены кешированные данные или локальный резервный вариант. Обратите внимание, что в редких случаях метод может завершиться чуть позже указанного значения `loadTimeout`, поскольку операция может состоять из нескольких запросов. |
