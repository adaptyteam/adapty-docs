...
| **locale**      | необязательный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder) для отображения экрана. Ожидается в формате языкового кода с одним или двумя подтегами, разделёнными `-` (например, `en`, `pt-br`). Если не указан, экран отображается на `en` или в локализации флоу по умолчанию, если `en` отсутствует. См. [Локализации и коды языков](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>необязательный</p><p>по умолчанию: `null`</p><p>SDK 4.1+</p> | Пользовательский ID макета, определённого во Flow Builder. Передайте его, чтобы отобразить конкретный макет вместо того, который SDK выбирает автоматически исходя из типа устройства и размера экрана. Если макет с указанным ID не найден, флоу загружается без конфигурации представления. |
| **loadTimeout** | по умолчанию: 5 сек | Ограничивает тайм-аут для этого метода. При достижении тайм-аута будут возвращены кэшированные данные или локальный резервный вариант. Обратите внимание, что в редких случаях метод может завершиться с небольшой задержкой относительно значения `loadTimeout`, поскольку операция может включать несколько запросов. |
...
| :-------------- | :------------- | :----------------------------------------------------------- |
| **flow**        | обязательный   | Объект `AdaptyFlow`, полученный через `Adapty.getFlow`. |
| **locale**      | опциональный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder) для отображения, указывается как код языка с одним или двумя подтегами через `-` (например, `en`, `pt-br`). Если не указан, отображение будет на `en`, или на локализации флоу по умолчанию, если `en` отсутствует. См. [Локализации и коды локалей](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>опциональный</p><p>по умолчанию: `null`</p><p>SDK 4.1+</p> | Пользовательский ID макета, определённого во Flow Builder. Передайте его, чтобы отобразить конкретный макет вместо того, который SDK выбирает автоматически исходя из типа устройства и размера экрана. Если макет с указанным ID не найден, флоу загружается без конфигурации отображения. |
| **loadTimeout** | по умолчанию: 5 сек | Ограничивает таймаут выполнения метода. При истечении таймаута возвращаются кешированные данные или локальный резервный вариант. Обратите внимание, что в редких случаях метод может завершиться чуть позже указанного в `loadTimeout` значения, поскольку операция может включать несколько запросов. |
