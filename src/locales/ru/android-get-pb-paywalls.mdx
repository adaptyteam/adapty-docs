...
| **locale**      | необязательный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder) для отображения, задаётся как языковой код с одним или двумя подтегами, разделёнными `-` (например, `en`, `pt-br`). Если не указан, отображение выполняется на `en` или в локализации флоу по умолчанию, если `en` отсутствует. См. [Локализации и коды локалей](android-localizations-and-locale-codes). |
| **loadTimeout** | по умолчанию: 5 сек | Ограничивает тайм-аут для этого метода. При достижении тайм-аута будут возвращены кешированные данные или локальный резервный вариант. Обратите внимание: в редких случаях метод может завершиться чуть позже указанного в `loadTimeout` значения, поскольку операция может включать несколько запросов. |
...
| :-------------- | :------------- | :----------------------------------------------------------- |
| **flow**        | обязательный   | Объект `AdaptyFlow`, полученный через `Adapty.getFlow`. |
| **locale**      | необязательный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder), на котором будет отображаться экран. Задаётся в виде языкового кода с одним или двумя подтегами, разделёнными `-` (например, `en`, `pt-br`). Если не указан, отображение происходит на языке `en` или на языке локализации флоу по умолчанию, если в нём нет варианта `en`. См. [Локализации и коды локалей](android-localizations-and-locale-codes). |
| **loadTimeout** | по умолчанию: 5 сек | Ограничивает время ожидания для этого метода. По истечении таймаута будут возвращены кешированные данные или локальный резервный вариант. Обратите внимание, что в редких случаях метод может завершиться чуть позже указанного в `loadTimeout` значения, поскольку операция может включать несколько запросов. |
