...
4. **Создайте флоу или пейвол и плейсмент**: В дашборде Adapty создайте флоу (или пейвол, если вы будете строить UI самостоятельно), затем назначьте его плейсменту на странице **Placements**. В коде идентификатор плейсмента — это строка, которую вы передаёте в `adapty.getFlow()`.
...
- **Интеграции аналитики**: Настройте на странице **Integrations**. Процесс настройки зависит от интеграции. Смотрите [интеграции аналитики](analytics-integration) и [интеграции атрибуции](attribution-integration).
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Вы создаёте флоу в конструкторе Adapty без кода, а SDK автоматически их отображает.
- [**Паywalls, созданные вручную**](capacitor-making-purchases): Вы создаёте собственный интерфейс пейвола в коде, но используете Adapty для получения продуктов и обработки покупок.
...
Тестируйте каждую покупку в песочнице по мере работы — не откладывайте до конца. Инструкции по настройке см. в разделе [Тестирование покупок в песочнице](test-purchases-in-sandbox).
