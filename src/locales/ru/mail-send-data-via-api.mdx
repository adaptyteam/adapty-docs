...
:::note
**API или SDK?** Большинство приложений передают данные в Adapty Mail через SDK Adapty, который автоматически собирает email-адреса и информацию о покупках — см. [Подключение Adapty к Adapty Mail](mail-collect-emails). Используйте API, если в вашем приложении нет SDK Adapty, если данные уже хранятся на вашем сервере или если вы импортируете подписчиков из другого источника. API можно также использовать совместно с другой точкой входа: Adapty Mail сопоставляет входящие данные по `customer_user_id` и email-адресу и хранит один профиль на каждого пользователя.
:::
...
:::
Если тот же пользователь также подключается к Adapty Mail через SDK, добавьте `customer_user_id` — ваш собственный идентификатор пользователя, который вы передаёте в `Adapty.identify()`. Adapty Mail использует его для сопоставления и объединяет оба источника в одном профиле, вместо того чтобы создавать дубликат и отправлять письма дважды.
...
:::note
`country` и `store_country` принимают двухбуквенный код [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) — `US`, а не `USA` или `United States`. Любое другое значение будет отклонено.
:::

Полный список доступных полей см. в справочнике [Save profile](api-mail/operations/saveProfile).
...
Идентифицируйте профиль по `external_profile_id`, `customer_user_id` или `email` — укажите хотя бы один из них. Удаление необратимо: повторное сохранение с теми же идентификаторами не пересоздаёт профиль.

Чтобы повторный запрос был безопасным, идентифицируйте профиль по `external_profile_id` или `customer_user_id`. Повторное удаление, в котором указан только `email`, вернёт `404`, так как удаление очищает сохранённый адрес. Подробнее см. в справочнике [Удаление профиля](api-mail/operations/deleteProfile).
