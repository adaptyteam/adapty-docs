...
:::important
Это руководство охватывает обработку событий для покупок, восстановлений, выбора продукта и рендеринга. Закрытие экрана и открытие ссылок обрабатываются реализацией `flowViewDidPerformAction` по умолчанию — см. наш [гайд по обработке действий кнопок](flutter-handle-paywall-actions), чтобы переопределить их или обработать действия кастомных кнопок.
:::
...
Смотрите [Отслеживание просмотров экранов флоу](flutter-flow-screen-views), чтобы узнать, что с ними делать.

Флоу также передаёт значения, которые пользователи вводят и выбирают. Смотрите [Обработка данных из флоу](flutter-flow-input), чтобы их считать.
...
Пейволы, настроенные с помощью [Paywall Builder](adapty-paywall-builder), не требуют дополнительного кода для совершения и восстановления покупок. Однако они генерируют определённые события, на которые ваше приложение может реагировать. К таким событиям относятся нажатия кнопок (кнопки закрытия, URL, выбор продуктов и т. д.), а также уведомления о действиях, связанных с покупками, выполненных на пейволе. Ниже описано, как реагировать на эти события.

:::warning

Этот гайд охватывает только **пейволы, созданные в старом билдере**, для работы которых требуется Adapty SDK v3.0 или более поздней версии.
