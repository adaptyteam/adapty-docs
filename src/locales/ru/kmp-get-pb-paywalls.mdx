...
| **locale**                   | необязательный | Идентификатор [локализации флоу](add-paywall-locale-in-adapty-paywall-builder) для отображения — например, `en` или `pt-br`. Если не указан, отображение происходит на `en` или в локализации флоу по умолчанию, если `en` недоступен. См. [Локализации и коды языков](kmp-localizations-and-locale-codes). |
| **loadTimeout**              | необязательный | Ограничивает таймаут для этого метода. При его истечении будут возвращены кэшированные данные или локальный резервный вариант. Обратите внимание, что в редких случаях метод может завершиться чуть позже указанного в `loadTimeout` значения, поскольку операция может включать несколько запросов. Можно использовать функции-расширения вроде `5.seconds` из `kotlin.time.Duration.Companion`. |
| **preloadProducts**          | необязательный | Установите `true` для предварительной загрузки продуктов с целью повышения производительности. При включении продукты загружаются заранее, сокращая время отображения флоу или пейвола. |
| **productPurchaseParams**    | необязательный | Словарь, отображающий [`AdaptyProductIdentifier`](https://kmp.adapty.io/adapty/com.adapty.kmp.models/-adapty-product-identifier/) на [`AdaptyPurchaseParameters`](https://kmp.adapty.io/adapty/com.adapty.kmp.models/-adapty-purchase-parameters/). Используйте его для настройки параметров покупки — например, персонализированных предложений или параметров обновления подписки для отдельных продуктов во флоу или пейволе. |
