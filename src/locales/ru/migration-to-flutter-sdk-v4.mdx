...
description: "Перейдите на Adapty Flutter SDK v4.0, заменив paywall API на flow API, совместимые с Flow Builder и Paywall Builder."
metadataTitle: "Миграция на Adapty Flutter SDK v4.0 | Документация Adapty"
---

Adapty Flutter SDK 4.0 вводит флоу и соответствующим образом переименовывает paywall API. Новые API работают как с новым Flow Builder, так и с существующим Paywall Builder — никаких изменений в настройках дашборда Adapty не требуется.
...
:::note
`customTimers` по-прежнему существует, но влияет только на пейволы, созданные в Legacy Paywall Builder. [Таймер обратного отсчёта](flow-timer) в флоу работает по настройкам, заданным в Flow Builder, поэтому флоу игнорирует значение, переданное в этом параметре.
:::
