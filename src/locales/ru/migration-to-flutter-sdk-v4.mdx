---
title: "Миграция Adapty Flutter SDK на v4.0"
description: "Мигрируйте на Adapty Flutter SDK v4.0, заменив API пейволов на API флоу, которые отображают флоу вместе с пейволами старого билдера."
metadataTitle: "Миграция на Adapty Flutter SDK v4.0 | Документация Adapty"
---

Adapty Flutter SDK 4.0 вводит флоу и переименовывает API пейволов соответствующим образом. Новые API работают с флоу и по-прежнему совместимы с пейволами старого билдера — никаких изменений в настройках дашборда Adapty не требуется.

:::note
Этот гайд охватывает миграцию с 3.x на 4.0. SDK 4.1 вносит ещё ряд изменений поверх этого — атрибуция Adapty становится опциональной, методы внешней атрибуции переименованы, а формат файла резервного пейвола снова изменился. Завершив миграцию здесь, см. [Миграция на v4.1](migration-to-flutter-sdk-41).
:::
...
Формат резервного файла [изменился в SDK v4](fallback-flows). Скачайте новый файл в разделе **[Placements](https://app.adapty.io/placements)** > **Fallbacks** и добавьте его в своё приложение.
...
`logShowPaywall` переименован в `logShowFlow` и теперь принимает `AdaptyFlow`. Событие по-прежнему фиксируется для той же вариации, поэтому существующие метрики воронки и A/B-тестов продолжат работать без изменений в дашборде.
...
Как и в v3, этот метод не нужно вызывать при отображении флоу или пейволов, отрисованных самим Adapty — такие просмотры отслеживаются автоматически.
...
:::note
`customTimers` по-прежнему существует, но влияет только на пейволы, созданные в Legacy Paywall Builder. [Таймер обратного отсчёта](flow-timer) в флоу работает согласно настройкам, заданным в Flow & Paywall Builder, поэтому флоу игнорирует значение, переданное в этом параметре.
:::
...
- `setFlowsEventsObserver` (и `setOnboardingsEventsObserver`) теперь принимают `null` для отключения ранее установленного наблюдателя, так что SDK больше не удерживает его.
- Новый необязательный коллбэк `flowViewDidReceiveAnalyticEvent` сообщает об аналитических событиях флоу, начиная с просмотра экрана для каждого экрана, который открывает пользователь. См. [Отслеживание просмотров экранов флоу](flutter-flow-screen-views).

v4 также добавляет возможности, которые можно подключить по желанию:
...
Устаревший API онбординга помечен как deprecated в v4.0 — вместо него рекомендуется использовать [Flow & Paywall Builder](adapty-flow-builder). Он продолжает работать, а IDE помечает устаревшие символы через аннотации `@Deprecated` — предупреждений во время выполнения нет. В одном из будущих релизов эти символы будут удалены, поэтому заранее спланируйте перенос ваших онбордингов во Flow & Paywall Builder.
