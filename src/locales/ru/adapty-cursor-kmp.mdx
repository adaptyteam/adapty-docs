...
- **Интеграции с аналитикой**: Настройте на странице **Integrations**. Процесс настройки зависит от конкретной интеграции. См. [интеграции с аналитикой](analytics-integration) и [интеграции с атрибуцией](attribution-integration).
...
- [**Паywalls, созданные вручную**](kmp-making-purchases): Вы строите собственный интерфейс пейвола в коде, но по-прежнему используете Adapty для получения продуктов и обработки покупок.
...
Не знаете, что выбрать? Прочитайте [сравнительную таблицу в quickstart](kmp-quickstart-paywalls).
...
### Покажите пейволы и обрабатывайте покупки \{#show-paywalls-and-handle-purchases\}

Получите пейвол по идентификатору плейсмента, отобразите его и обрабатывайте события покупок. Набор нужных вам гайдов зависит от того, как вы обрабатываете покупки.
...
**Гайды:**
- [Включить покупки с Flow и Paywall Builder (быстрый старт)](kmp-quickstart-paywalls)
- [Получить флоу и их конфигурацию](kmp-get-pb-paywalls)
- [Отобразить флоу](kmp-present-paywalls)
- [Обработать события флоу](kmp-handling-events)
- [Реагировать на действия кнопок](kmp-handle-paywall-actions)
...
:::tip[Checkpoint]
- **Ожидаемое поведение:** Пейвол отображается с настроенными продуктами. При нажатии на продукт появляется диалог покупки в песочнице.
- **Частая ошибка:** Пустой флоу или ошибка `getFlow` → убедитесь, что ID плейсмента точно совпадает с тем, что указан в дашборде, и что у плейсмента назначена аудитория.
:::
