...
v4.0 — это предрелизная версия, поэтому при установке укажите точный бета-тег. Через Unity Package Manager используйте Git URL с тегом вместо `#upm`:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Если вы устанавливаете через Unity-пакет, скачайте `adapty-unity-plugin-4.0.0-beta.1.unitypackage` из папки [Releases](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Releases). Полные инструкции по настройке см. в разделе [Установка Adapty SDK](sdk-installation-unity#install-adapty-sdk).
...
|---|---|---|
| `RemoteConfig` (единственный, nullable) | `RemoteConfigs` (список) | Флоу содержит один Remote Config на каждый настроенный язык. Читайте тот, который соответствует пользователю, через `flow.RemoteConfigs`. Свойство `flow.RemoteConfig` возвращает первый элемент. |
| _(новое)_ | `Paywalls` (список `AdaptyFlowPaywall`) | Каждый элемент — один вариант пейвола во флоу со своими `Name`, `VariationId` и `ProductIdentifiers`. Методы веб-пейвола принимают `AdaptyFlowPaywall` — см. [Методы веб-пейвола](#web-paywall-methods). |
| `ProductIdentifiers`, `VendorProductIds` | сохранены | В `AdaptyFlow` эти свойства агрегируют продукты по всем вариантам пейвола. Каждый вариант также содержит собственные `ProductIdentifiers` и `VendorProductIds`. Для получения продуктов продолжайте вызывать `GetPaywallProducts(flow)`. |
| `HasViewConfiguration` | удалено | Удалите все проверки `HasViewConfiguration` из кода — `CreateFlowView` вместо этого вернёт ошибку (см. [Отображение флоу](#displaying-flows)). |
...
- `Adapty.SetObserverModeResolver(...)` с `IAdaptyUIObserverModeResolver` — управление покупками и восстановлениями, инициированными из флоу, когда SDK работает в [режиме Observer](implement-observer-mode-unity). Ранее это было доступно только в нативных SDK для iOS и Android. См. [Флоу в режиме Observer](unity-present-flows-in-observer-mode).
- `Adapty.SetSystemRequestsHandler(...)` с `IAdaptyUISystemRequestsHandler` — зарезервировано для системных запросов из флоу: запросы разрешений ОС (`FlowViewDidAskPermission`) и запросы на оценку приложения (`FlowViewDidRequestAppReview`). Флоу пока не инициируют такие запросы, поэтому регистрировать обработчик не нужно.
- `AdaptyUICreateFlowViewParameters.Locale` — отображение флоу или пейвола с конкретной [локализацией Builder](add-paywall-locale-in-adapty-paywall-builder) вместо той, которую Adapty определяет по устройству. Флоу локализуется при создании представления, поэтому здесь можно задать локализацию единственный раз; созданное представление сообщает использованную локализацию через `view.Locale`. См. [Использование локализаций и кодов локалей](unity-localizations-and-locale-codes).
