---
title: "Миграция Adapty Unity SDK на v4.0"
description: "Мигрируйте на Adapty Unity SDK v4.0 (beta): замените paywall API на flow API, совместимые с Flow Builder и Paywall Builder."
metadataTitle: "Миграция на Adapty Unity SDK v4.0 (beta) | Документация Adapty"
---

Adapty Unity SDK 4.0 (beta) вводит флоу и переименовывает paywall API соответствующим образом. Новые API работают как с новым Flow Builder, так и с существующим Paywall Builder — никаких изменений в настройках дашборда Adapty не требуется.
...
- `Adapty.SetObserverModeResolver(...)` с `IAdaptyUIObserverModeResolver` — управляет покупками и восстановлениями, инициированными из флоу, когда SDK работает в [режиме Observer](implement-observer-mode-unity). Ранее это было доступно только в нативных SDK для iOS и Android. См. [Отображение флоу в режиме Observer](unity-present-flows-in-observer-mode).
- `Adapty.SetSystemRequestsHandler(...)` с `IAdaptyUISystemRequestsHandler` — зарезервировано для системных запросов из флоу: системных запросов разрешений (`FlowViewDidAskPermission`) и запросов на оценку приложения (`FlowViewDidRequestAppReview`). Флоу пока не инициируют эти запросы, поэтому регистрировать обработчик не обязательно.
- `AdaptyUICreateFlowViewParameters.Locale` (задаётся через `SetLocale`) — отображает флоу или пейвол с конкретной [локализацией Builder](add-paywall-locale-in-adapty-paywall-builder) вместо локализации флоу по умолчанию. Флоу локализуется при создании представления, поэтому это единственное место для выбора локализации; созданное представление сообщает о применённой локализации в `view.Locale`. См. [Использование локализаций и кодов языков](unity-localizations-and-locale-codes).
- Новый коллбэк `FlowViewDidReceiveAnalyticEvent` в `IAdaptyFlowsEventsListener` зарезервирован для пользовательских аналитических событий из флоу. Флоу пока не отправляют их в ваш код, поэтому реализуйте метод с пустым телом.
- `AdaptyUI.OpenUrl(url, openIn, ...)` и `AdaptyUI.RequestAppReview(...)` — нативная обработка действий `open_url` и запросов на оценку приложения. Вызывайте `OpenUrl` из `FlowViewDidPerformAction`, чтобы сохранить стандартное поведение при открытии URL; `RequestAppReview` обеспечивает стандартный запрос на оценку приложения, который флоу пока не инициируют.
