...
:::info
В Adapty Android SDK 4.0.2 и выше `restorePurchases()` завершается успешно, даже если восстанавливать нечего: метод возвращает текущий профиль, поэтому проверяйте в нём статус уровня доступа. В более ранних версиях SDK метод в таком случае завершается с ошибкой `NO_PURCHASES_TO_RESTORE`.
:::

:::note
Google Play сообщает только о покупках, которыми пользователь владеет в данный момент, поэтому `restorePurchases()` не может найти подписки, истёкшие до того, как вы подключили Adapty. Чтобы добавить их в Adapty, [импортируйте исторические данные](importing-historical-data-to-adapty).
:::
