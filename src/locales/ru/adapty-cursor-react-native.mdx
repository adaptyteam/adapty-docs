...
4. **Создайте флоу или пейвол и плейсмент**: в дашборде Adapty создайте флоу (или пейвол, если будете строить UI самостоятельно), затем привяжите его к плейсменту на странице **Placements**. В коде ID плейсмента — это строка, которую вы передаёте в `adapty.getFlow("YOUR_PLACEMENT_ID")`.
...
- [**Adapty Flow и Paywall Builder**](adapty-flow-builder): Вы создаёте флоу в конструкторе Adapty без кода, а SDK отображает их автоматически.
...
Не знаете, что выбрать? Прочитайте [сравнительную таблицу в быстром старте](react-native-quickstart-paywalls).
...
Получите пейвол по ID плейсмента, отобразите его и обработайте события покупок. Необходимые гайды зависят от того, как вы обрабатываете покупки.
...
**Гайды:**
- [Включение покупок с Flow & Paywall Builder (быстрый старт)](react-native-quickstart-paywalls)
- [Получение флоу и их конфигурации](react-native-get-pb-paywalls)
- [Отображение флоу](react-native-present-paywalls)
- [Обработка событий флоу](react-native-handling-events-1)
- [Реагирование на действия кнопок](react-native-handle-paywall-actions)
...
:::tip[Checkpoint]
- **Ожидаемый результат:** Пейвол появляется с настроенными продуктами. Нажатие на продукт открывает диалог покупки в песочнице.
- **Возможная проблема:** Пустой флоу или ошибка `getFlow` → убедитесь, что ID плейсмента точно совпадает с тем, что указан в дашборде, и плейсменту назначена аудитория.
:::
