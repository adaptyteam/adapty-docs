...
| v4.0 | v4.1 |
|---|---|
| Adapty Attribution включена автоматически | Adapty Attribution отключена по умолчанию; включите с помощью `.with(adaptyAttributionEnabled: true)` |
...
| Перечисление `AdaptySubscriptionOfferType` | Структура `AdaptySubscriptionOfferType`; `.code` удалено |
| Файл резервного пейвола загружен для 4.0 | Новый формат файла резервного пейвола; загрузите файл заново |
| Promoted in-app purchases завершаются автоматически, без возможности перехвата | Метод делегата `didReceivePromotedPurchase(_:)` и `AdaptyPromotedProduct` передают завершение вашему приложению |

## ⚠️ Атрибуция Adapty отключена по умолчанию \{#adapty-attribution-is-disabled-by-default\}

:::warning
Если вы обновитесь до SDK 4.1 и не подключите эту функцию явно, [атрибуция Adapty](user-acquisition) перестанет работать без каких-либо предупреждений — установки перестанут регистрироваться.
:::

В версии 4.0 и более ранних SDK автоматически регистрировал установки для [Атрибуции Adapty](user-acquisition). Начиная с версии 4.1, это отключено по умолчанию: SDK не регистрирует установки, а callback-делегаты `onInstallationDetailsSuccess` и `onInstallationDetailsFail` никогда не вызываются. `getCurrentInstallationStatus()` возвращает `.notAvailable`.
...
SDK 4.1 восстанавливает хук для продвигаемых встроенных покупок App Store, который [был удалён в версии 4.0](migration-to-ios-sdk-v4#app-store-promoted-in-app-purchases-temporarily-removed). В версии 4.0 такая покупка всё равно завершалась, и Adapty фиксировал транзакцию — просто приложение не могло её перехватить. В 4.1 это поведение сохраняется по умолчанию, поэтому данный пункт является новой возможностью, а не шагом миграции.
...
:::warning
Новый механизм основан на StoreKit 2 и требует **iOS 16.4** или выше. Метод `shouldAddStorePayment` в версии 3.x работал на более ранних версиях iOS. На устройствах ниже iOS 16.4 метод `didReceivePromotedPurchase` не вызывается, и продвигаемые покупки завершаются без участия вашего приложения.
:::
