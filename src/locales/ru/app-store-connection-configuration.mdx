...
description: "Настройте подключение к App Store для отслеживания подписок без сбоев."
...
В этом разделе описано, как настроить связь между App Store и Adapty для вашего iOS-приложения. Это необходимо для отображения аналитики подписок и валидации покупок. Интеграцию можно завершить во время первоначального онбординга или позже в **App Settings** дашборда Adapty.

Каждый ключ на этой странице открывает доступ к определённой части обмена данными. В разделе [Как Adapty синхронизирует данные со сторами](store-sync#what-adapty-reads-from-the-stores) описано, какие данные передаёт каждый из них.
...
:::danger
Изменения конфигурации безопасно вносить в период Sandbox, пока мобильное приложение ещё не вышло в продакшн с установленным Adapty SDK. Изменения после релиза могут нарушить флоу покупок в приложении.

Если ключ был отозван, удалён или ротирован в App Store Connect, загрузите замену как можно скорее: всё, что от него зависит, будет неработоспособным до тех пор, пока вы этого не сделаете. См. [Если обновления перестали поступать](store-sync#if-updates-stop-arriving).
:::
...
:::note
App Store Connect имеет два типа ключей, у каждого из которых есть поля **Issuer ID** и **Key ID**. На шагах 2–3 используется ключ **In-App Purchase** с вкладки **Integrations** → **In-App Purchase**. Ключ App Store Connect API с вкладки **Team keys** — это другой ключ с собственными идентификаторами: вы добавите его на [шаге 6](#step-6-add-app-store-connect-api-key).
:::
...
Сгенерируйте API-ключ App Store Connect и добавьте его в Adapty, чтобы [управлять продуктами в App Store из дашборда Adapty](create-product#create-product-and-push-to-store).

:::note
Это другая пара ключей — не та, что вы добавили на [шагах 2–3](#step-2-provide-issuer-id-and-key-id) для встроенных покупок. Сгенерируйте ключ на вкладке **Team keys** и скопируйте оттуда **Issuer ID** и **Key ID** — значения ключа для встроенных покупок здесь не подойдут.
:::
...
2. В окне **Generate API key window** введите имя ключа и предоставьте ему доступ **Admin**.
