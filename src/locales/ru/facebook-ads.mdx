---
title: "Facebook Ads"
description: "Интеграция Facebook Ads с Adapty для эффективного маркетинга подписок."
...
Благодаря интеграции с Facebook Ads вы можете удобно отслеживать статистику приложения в Meta Analytics. Adapty отправляет события в Meta Ads Manager, помогая формировать похожие аудитории на основе данных о подписках для повышения отдачи от рекламы. Так вы сможете точно видеть, какой доход приносят ваши рекламные кампании за счёт подписок.

Интеграция Adapty с Facebook Ads работает следующим образом: Adapty отправляет в Facebook Ads все события подписок, настроенные в вашей интеграции. Это позволяет оценивать эффективность рекламных кампаний.
...
## Расширенное сопоставление (Conversions API) \{#advanced-matching-conversions-api\}

Facebook CAPI — это отдельная интеграция, никак не связанная с интеграцией Facebook Ads, описанной выше. У неё собственные учётные данные, и настраивается она независимо в дашборде Adapty. При включении Adapty передаёт захешированный идентификационный payload в Meta Conversions API для расширенного сопоставления. Из профиля пользователя Adapty хеширует через SHA-256 и отправляет: email, номер телефона, имя и фамилию, дату рождения, пол и страну. Идентификаторы кликов и браузера Facebook (`fbc`, `fbp`), а также IP-адрес и user agent передаются без хеширования — как и в любой другой интеграции с Meta CAPI.
