...
3. **Создайте хотя бы один продукт**: В дашборде Adapty перейдите на страницу **Products**. Продукты не используются в коде напрямую — Adapty передаёт их через пейволы.
   [Добавить продукты](quickstart-products)

4. **Создайте флоу или пейвол и плейсмент**: В дашборде Adapty создайте флоу (или пейвол, если вы будете строить UI самостоятельно), затем назначьте его плейсменту на странице **Placements**. В коде идентификатор плейсмента — это строка, которую вы передаёте в `Adapty.getFlow("YOUR_PLACEMENT_ID")`.
...
- **A/B-тесты**: Настройте на странице **Placements**. Изменения в коде не требуются.
  [A/B-тесты](ab-tests)
- **Дополнительные флоу и плейсменты**: Добавьте больше вызовов `getFlow` с разными идентификаторами плейсментов.
- **Интеграции с аналитикой**: Настройте на странице **Integrations**. Процесс настройки зависит от интеграции. См. [интеграции с аналитикой](analytics-integration) и [интеграции атрибуции](attribution-integration).
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Вы создаёте флоу в no-code конструкторе Adapty, а SDK отображает их автоматически.
- [**Вручную созданные пейволы**](android-making-purchases): Вы строите собственный UI пейвола в коде, но всё равно используете Adapty для получения продуктов и обработки покупок.
...
Не знаете, что выбрать? Прочитайте [таблицу сравнения в quickstart](android-quickstart-paywalls).
...
**Гайды:**
- [Включить покупки с помощью Flow и Paywall Builder (быстрый старт)](android-quickstart-paywalls)
- [Получить флоу и их конфигурацию](android-get-pb-paywalls)
- [Отобразить флоу](android-present-paywalls)
- [Обрабатывать события флоу](android-handling-events)
...
:::tip[Checkpoint]
- **Ожидаемый результат:** Пейвол появляется с настроенными вами продуктами. Нажатие на продукт открывает диалог покупки в песочнице.
- **Возможная проблема:** Пустой флоу или ошибка `getFlow` → убедитесь, что ID плейсмента точно совпадает с указанным на дашборде и что для плейсмента назначена аудитория.
:::
