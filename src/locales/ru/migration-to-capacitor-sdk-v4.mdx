---
title: "Миграция Adapty Capacitor SDK на v4.1.1"
description: "Миграция на Adapty Capacitor SDK v4.1.1: замена paywall API на flow API, переименованные attribution API, явное включение Adapty Attribution и повторная загрузка резервного файла."
metadataTitle: "Миграция на Adapty Capacitor SDK v4.1.1 | Документация Adapty"
---
...
| v3 | v4.1.1 |
|---|---|
| Атрибуция Adapty включена автоматически | отключена по умолчанию — включите через `adaptyAttributionEnabled: true` |
...
| Файл резервного пейвола загружен для версии 3.x | новый формат файла резервного пейвола — загрузите файл повторно |
| Продвигаемые встроенные покупки не поддерживаются | событие `'onPromotedPurchaseReceived'` и `adapty.makePromotedPurchase({ product })` |
...
:::warning
Если вы используете [атрибуцию Adapty](user-acquisition) и обновляетесь до SDK 4.1.1, не включив её явно, всё сломается без каких-либо предупреждений — установки перестанут регистрироваться, и ничто об этом не сообщит.
:::

В более ранних версиях SDK автоматически регистрировал установки для [Атрибуции Adapty](user-acquisition). Начиная с версии SDK 4.1.1, это отключено по умолчанию: SDK не регистрирует установки, события `'onInstallationDetailsSuccess'` и `'onInstallationDetailsFail'` не срабатывают, а `getCurrentInstallationStatus` возвращает статус `not_available`.
...
Формат [резервного файла](fallback-flows) изменился в ветке 4.x и снова в версии 4.1.1. Скачайте файл заново в **[Placements](https://app.adapty.io/placements)** > **Fallbacks** и добавьте его в приложение, даже если вы уже скачивали его для бета-версии 4.0.

:::warning
Этот шаг не вызывает ошибок сборки. Если его пропустить, `setFallback` отклонит устаревший файл и все плейсменты лишатся резервного пейвола.
:::
...
SDK 4.1.1 добавляет поддержку [встроенных покупок, продвигаемых на странице продукта в App Store](capacitor-making-purchases#in-app-purchases-from-the-app-store). Это новая функция, а не шаг миграции: SDK самостоятельно завершает продвигаемые покупки, поэтому вы получаете нужное поведение без написания какого-либо кода.
