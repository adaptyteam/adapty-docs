...
Формат [резервного файла](fallback-flows) изменился в версии 4.0, а затем снова в 4.1.1. Скачайте файл заново в **[Placements](https://app.adapty.io/placements)** > **Fallbacks** и добавьте его в приложение, даже если вы уже скачивали его для бета-версии 4.0.
...
| Параметр | Описание |
|---|---|
| `locale` | Локализация, с которой будет отображаться флоу. Параметр перенесён сюда из `getPaywall` — см. [getPaywall → getFlow](#getpaywall--getflow). |
...
Устаревший API онбординга объявлен устаревшим в v4 в пользу [Flow & Paywall Builder](adapty-flow-builder). Он по-прежнему работает, но будет удалён в одном из следующих релизов, поэтому запланируйте миграцию ваших онбордингов во Flow & Paywall Builder.
