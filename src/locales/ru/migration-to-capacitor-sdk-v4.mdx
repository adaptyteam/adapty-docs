...
`locale` остаётся необязательным в `createFlowView`: если его не указать, вид отрисовывается на `en` или на языке по умолчанию флоу, если `en` в нём отсутствует. См. [Локализации и коды локалей](capacitor-localizations-and-locale-codes).
