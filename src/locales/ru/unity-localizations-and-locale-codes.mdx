...
Совпадение точное. SDK сравнивает переданный вами код с кодами локализации флоу символ за символом: регистр не меняется, подчёркивания (`_`) не заменяются дефисами (`-`), и откат к языковому субтегу не выполняется. Для флоу с локализацией `pt-br` подходит только `pt-br`: `pt-BR`, `pt_BR` и `pt-PT` — не совпадают.

Если код не совпадает ни с одной локализацией, флоу молча отображается в [локали по умолчанию](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — SDK не возвращает ошибку и не записывает предупреждение в лог.

Если код совпадает, Adapty объединяет эту локализацию с локализацией по умолчанию: строки и ресурсы, не определённые в найденной локализации, берутся из локализации по умолчанию.

Отсутствие кода локали — это не то же самое, что запрос дефолтной локализации флоу: SDK подставляет фиксированный `en`. Флоу, у которого дефолтная локаль — `de`, всё равно отображается на `en`, если у него есть локализация `en`, и только при её отсутствии возвращается к `de`.

:::warning
Передавайте код локали точно так, как он настроен в дашборде, — подтеги в нижнем регистре, разделённые дефисами. Не передавайте системный идентификатор локали напрямую: `CultureInfo.CurrentCulture.Name` возвращает `pt-BR`, и это приведёт к откату на дефолтную локализацию. Преобразуйте значение в приложении перед передачей.
:::
...
- **Флоу и пейволы, которые отрисовывает Adapty**: SDK не считывает локаль устройства, поэтому определите её в своём приложении и [передайте при создании вью](#choose-the-localization-of-a-flow). Код локали необязателен — если его не указать, флоу отобразится на `en` или в локали по умолчанию, если локализация `en` в флоу не настроена.
- **Кастомные (Remote Config) пейволы**: `GetFlow` возвращает все настроенные локализации в `flow.RemoteConfigs`. Каждый элемент — это `AdaptyRemoteConfig` с кодом `Locale` и словарём `Dictionary` значений. Выберите элемент, соответствующий пользователю, с собственным фолбэком:
