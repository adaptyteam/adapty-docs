...
```
**iOS: Включение режима Kids Mode в SDK v4 (бета)**

:::important
В SDK v4 нативный iOS SDK устанавливается через Swift Package Manager, а режим Kids Mode включается с помощью трейта `KidsMode` Swift-пакета, который исключает из компиляции весь код IDFA, AdSupport и AppTrackingTransparency. Для этого требуется **Xcode 26** или более поздняя версия.
:::

В SDK v4 используйте пакет `adapty_flutter_kids` вместо `adapty_flutter` в файле `pubspec.yaml`. Это вариант плагина с режимом Kids Mode, имеющий тот же публичный API и ту же версию — единственное отличие в том, что его нативный iOS SDK собирается с трейтом `KidsMode`:
```yaml showLineNumbers title="pubspec.yaml"
dependencies:
  adapty_flutter_kids: 4.0.0-beta.1
```

Ваш код на Dart остаётся прежним — нужно лишь обновить импорт на новое имя пакета:

```dart showLineNumbers title="Dart"
```

**iOS: включение Kids Mode через CocoaPods (SDK v3)**
