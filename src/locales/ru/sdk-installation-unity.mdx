...
Adapty SDK поддерживает iOS 13.0+, но для работы с пейволами, созданными в Paywall Builder, требуется iOS 15.0+. Adapty SDK 4.1 — который добавляет поддержку [флоу](adapty-flow-builder) — требует **iOS 15.0+** для всего приложения: валидатор сборки в Unity Editor остановит iOS-сборку, если deployment target ниже этого значения. Сборка для iOS с SDK 4.x также требует **Xcode 26** или новее — нативный iOS SDK, добавляемый как Swift package, собран с использованием Swift tools 6.2.

:::info
Adapty Unity SDK 4.0 и выше работает с Google Play Billing Library v8.

Совместимость с версией Billing Library не означает, что Adapty поддерживает каждую функцию, которую Google в ней представила. Прежде чем использовать новую возможность Google Play Billing, ознакомьтесь с разделом [Продукты в Play Store](android-products).
:::
