...
Adapty SDK поддерживает iOS 13.0+, однако для работы с пейволами, созданными в Paywall Builder, требуется iOS 15.0+. Adapty SDK 4.0 (бета) — который добавляет поддержку [Flow Builder](adapty-flow-builder) — требует **iOS 15.0+** для всего приложения: встроенный валидатор в Unity Editor блокирует сборку под iOS, если целевая версия ниже указанной.

:::info
Adapty совместим с Google Play Billing Library версий до 8.x включительно. По умолчанию Adapty использует Google Play Billing Library v7.0.0. Чтобы использовать более новую версию, [переопределите зависимость Billing](https://developer.android.com/google/play/billing/integrate#dependency) в сборке Android.

Совместимость с версией Billing Library не означает, что Adapty поддерживает все возможности, которые Google добавила в неё. Прежде чем использовать новые возможности Google Play billing, ознакомьтесь с разделом [Продукт в Play Store](android-products).
:::
...
SDK 4.0 — который добавляет поддержку [Flow Builder](adapty-flow-builder) — является предрелизным. Чтобы установить его через Unity Package Manager, добавьте бета-тег к Git URL:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Если вы устанавливаете через пакет Unity, скачайте `adapty-unity-plugin-4.0.0-beta.1.unitypackage` из [релиза 4.0.0-beta.1](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.0.0-beta.1).

Два изменения в настройке сборки появились в v4 — нативный iOS SDK Adapty объявлен как удалённый Swift-пакет и больше не устанавливается через CocoaPods:

- Обновите External Dependency Manager до версии **1.2.188 или выше** — более ранние версии не поддерживают зависимости Swift Package Manager. Именно эту версию SDK 4.0 объявляет как peer dependency, поэтому Unity предупредит вас, если в проекте установлена более старая.
- Шаги с CocoaPods выше (`iOS Resolver -> Install Cocoapods`, открытие `Unity-iPhone.xcworkspace`) относятся только к SDK 3.x. В SDK 4.0 EDM добавляет Swift-пакет в сгенерированный Xcode-проект автоматически.
- Установите iOS deployment target **15.0 или выше**. Если этого не сделать, валидатор сборки в Unity Editor заблокирует iOS-билд.

Смотрите [гайд по миграции](migration-to-unity-sdk-v4) для полного списка изменений в SDK 4.0.
...
:::note
В SDK 4.0 интерфейсы слушателей следуют соглашению об именовании C# с префиксом I: реализуйте `IAdaptyEventListener` вместо `AdaptyEventListener`. Методы остались без изменений. См. [руководство по миграции](migration-to-unity-sdk-v4).
:::

:::important
Дождитесь завершения колбэка `Activate` перед вызовом любых других методов SDK. Подробную последовательность см. в разделе [Порядок вызовов в Unity SDK](unity-sdk-call-order).
:::
