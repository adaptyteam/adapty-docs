...
Adapty SDK поддерживает iOS 13.0+, но для работы с пейволами, созданными в Paywall Builder, требует iOS 15.0+. Adapty SDK 4.1 — добавляющий поддержку [флоу](adapty-flow-builder) — требует **iOS 15.0+** для всего приложения: валидатор сборки в Unity Editor прерывает iOS-сборку, если deployment target ниже этой версии. Сборка для iOS с SDK 4.x также требует **Xcode 26** или новее — нативный iOS SDK добавляется как Swift package и собирается с помощью Swift tools 6.2.

:::info
Начиная с версии 3.17.0, Adapty Unity SDK по умолчанию использует Billing Library v8. Версии с 3.15.0 по 3.15.2 используют v7 и также работают, если другая зависимость в вашей сборке Android повышает Billing Library до v8.

Совместимость с определённой версией Billing Library не означает, что Adapty поддерживает все возможности, введённые Google в этой версии. Прежде чем использовать новую функцию биллинга Google Play, ознакомьтесь с разделом [Продукт в Play Store](android-products).
:::
