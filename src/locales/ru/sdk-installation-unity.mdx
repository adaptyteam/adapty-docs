...
- **AdaptyUI**: Этот модуль отвечает за отображение флоу, а также пейволов, созданных в старом билдере.

:::tip
Хотите увидеть реальный пример интеграции Adapty SDK в мобильное приложение? Посмотрите наше [демо-приложение](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Assets), в котором показана полная настройка: отображение пейволов, совершение покупок и другие базовые функции.
:::

## Требования \{#requirements\}

Adapty SDK поддерживает iOS 13.0+, но для работы с пейволами, созданными в Paywall Builder, требуется iOS 15.0+. Adapty SDK 4.1 — который добавляет поддержку [флоу](adapty-flow-builder) — требует **iOS 15.0+** для всего приложения: валидатор сборки в Unity Editor остановит iOS-сборку, если целевая версия ниже. Сборка для iOS с SDK 4.x также требует **Xcode 26** или более новой версии — нативный iOS SDK, добавляемый как Swift-пакет, собирается с инструментами Swift 6.2.

:::info
Adapty SDK поставляется со своей копией Google Play Billing Library — добавлять её в приложение не нужно.

Начиная с версии 3.17.0, Adapty Unity SDK использует Billing Library v8 по умолчанию. Версии с 3.15.0 по 3.15.2 используют v7 и также работают, если другая зависимость в вашем Android-билде повышает версию Billing Library до v8.

Совместимость с определённой версией Billing Library не означает, что Adapty поддерживает все функции, которые Google представила в ней. Прежде чем использовать новые возможности Google Play Billing, ознакомьтесь с разделом [Продукты в Play Store](android-products).
:::
...
1. Установите [плагин External Dependency Manager (EDM)](https://github.com/googlesamples/unity-jar-resolver#getting-started). SDK Adapty использует его для управления зависимостями iOS и зависимостями Android gradle.
...
SDK 4.1 — добавляющий поддержку [флоу](adapty-flow-builder) — это первый стабильный релиз ветки 4.x. Чтобы установить его через Unity Package Manager, добавьте тег версии к Git URL:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

Если вы устанавливаете через пакет Unity, скачайте `adapty-unity-plugin-4.1.0.unitypackage` из [релиза 4.1.0](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0).
...
Полный список изменений в ветке 4.x смотрите в [руководстве по миграции](migration-to-unity-sdk-v4).
...
:::note
В SDK 4.1 интерфейсы слушателей следуют соглашению об именовании C# с префиксом I — реализуйте `IAdaptyEventListener` вместо `AdaptyEventListener` — и интерфейс требует ещё одного метода, `OnReceivePromotedPurchase`. Смотрите [руководство по миграции](migration-to-unity-sdk-v4).
:::

:::important
Дождитесь колбэка завершения `Activate`, прежде чем вызывать любые другие методы Adapty SDK. Полная последовательность описана в разделе [Порядок вызовов в Unity SDK](unity-sdk-call-order).
:::
...
- Если вы используете [Flow & Paywall Builder](adapty-flow-builder), сначала [активируйте модуль AdaptyUI](#activate-adaptyui-module-of-adapty-sdk) ниже, а затем следуйте [быстрому старту](unity-quickstart-paywalls).
- Если вы создаёте собственный интерфейс пейвола, см. [быстрый старт для пользовательских пейволов](unity-quickstart-manual).

## Активация модуля AdaptyUI в SDK Adapty \{#activate-adaptyui-module-of-adapty-sdk\}

Если вы планируете использовать [Flow & Paywall Builder](adapty-flow-builder) и установили модуль AdaptyUI, его необходимо активировать. Это можно сделать при конфигурации:
...
### Включите Adapty Attribution \{#enable-adapty-attribution\}

:::info
Этот параметр доступен начиная с версии SDK 4.1.
:::

Если вы используете [Adapty Attribution](user-acquisition), установите `SetAdaptyAttributionEnabled` в `true` при активации SDK. Значение по умолчанию — `false`: без этого параметра SDK не регистрирует установки и не передаёт данные об установке в ваше приложение. В версиях SDK ниже 4.1 Adapty Attribution включается автоматически.
