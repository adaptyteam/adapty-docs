...
SDK 4.0 — с поддержкой [Flow Builder](adapty-flow-builder) — находится в статусе предрелиза. Чтобы установить его через Unity Package Manager, укажите в Git URL тег бета-версии вместо `#upm`:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Если вы устанавливаете через Unity package, скачайте `adapty-unity-plugin-4.0.0-beta.1.unitypackage` из папки [Releases](https://github.com/adaptyteam/AdaptySDK-Unity/tree/main/Releases).
