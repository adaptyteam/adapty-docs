...
Adapty SDK поддерживает iOS 13.0+, однако для работы с пейволами, созданными в Paywall Builder, требуется iOS 15.0+. Adapty SDK 4.1 — который добавляет поддержку [флоу](adapty-flow-builder) — требует **iOS 15.0+** для всего приложения: валидатор сборки в Unity Editor блокирует iOS-сборку, если deployment target ниже этой версии. Сборка для iOS с SDK 4.x также требует **Xcode 26** или новее — нативный iOS SDK, добавляемый в качестве Swift-пакета, собирается с использованием Swift tools 6.2.
...
|---|---|
| 3.17.0 и выше | v8 |
| 3.15.0–3.15.2 | v7 по умолчанию, v8 если другая зависимость её повышает |

:::note
Совместимость с версией Billing Library не означает, что Adapty поддерживает все функции, введённые Google в этой версии. Прежде чем использовать новую возможность биллинга Google Play, ознакомьтесь с разделом [Продукт в Play Store](android-products).
:::
