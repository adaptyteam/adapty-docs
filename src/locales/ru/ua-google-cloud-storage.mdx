...
Adapty Attribution интегрируется с Google Cloud Storage, позволяя безопасно хранить данные о кампаниях по привлечению пользователей в одном централизованном месте. Вы сможете сохранять данные о производительности кампаний, данные атрибуции и события привлечения пользователей в ваш бакет Google Cloud Storage в виде .csv файлов.
Чтобы настроить эту интеграцию, нужно выполнить несколько простых шагов в Google Cloud Console и дашборде Adapty Attribution.

:::note
Расписание

Adapty Attribution экспортирует один набор файлов за каждый календарный день в Google Cloud Storage. Планировщик экспорта запускается трижды в день — в **02:00**, **04:00** и **06:00 UTC**. Каждый запуск либо создаёт задание экспорта за предыдущий день (если оно ещё не существует), либо повторяет его, если предыдущий запуск завершился с ошибкой. Таким образом, данные обычно появляются после запуска в 02:00, а последующие запуски нужны для восстановления после сбоев, а не для добавления новых данных.

Каждый файл будет содержать данные о событиях, созданных за весь предыдущий календарный день по UTC. Например, данные, экспортированные 8 марта, будут содержать все события, созданные 7 марта с 00:00:00 до 23:59:59 по UTC.
:::

:::warning
Запланированный экспорт выполняется только для компаний с платным тарифом или в пробном периоде. Если ваша компания не относится ни к одной из этих категорий, планировщик пропустит задачу и не запишет ни одного файла, даже если интеграция остаётся включённой в дашборде. Если ваш бакет пуст и конфигурация выглядит корректно, сначала проверьте статус вашего тарифного плана.
:::
...
| `click_time`             | Временная метка атрибутированного клика в секундах Unix. Пусто, если установка не связана ни с одним кликом. |