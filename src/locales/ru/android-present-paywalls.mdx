...
| **eventListener**             | опциональный | Передайте `AdaptyUiEventListener` для отслеживания событий пейвола. Для удобства рекомендуется наследоваться от `AdaptyUiDefaultEventListener`. Подробнее см. в разделе [Обработка событий пейвола](android-handling-events).                                                                                                                                                                                                                                                |
| **insets**                    | опциональный | <p>Insets — это отступы вокруг пейвола, которые не дают интерактивным элементам скрываться за системными панелями.</p><p>По умолчанию: `UNSPECIFIED` — Adapty автоматически подбирает отступы, что хорошо работает для edge-to-edge пейволов.</p><p>Если ваш пейвол не является edge-to-edge, возможно, потребуется задать собственные отступы. Как это сделать — читайте в разделе [Изменение отступов пейвола](android-present-paywalls#change-paywall-insets) ниже.</p> |
...
| **tagResolver**               | опциональный | Используйте `AdaptyUiTagResolver` для обработки пользовательских тегов в тексте пейвола. Этот резолвер принимает тег в качестве параметра и возвращает соответствующую строку. Подробнее см. в разделе «Пользовательские теги в Paywall Builder».                                                                                                                                                                                                                             |
| **timerResolver**             | опциональный | Передайте резолвер сюда, если планируете использовать функциональность пользовательского таймера.                                                                                                                                                                                                                                                                                                                                                                           |
<SampleApp />
