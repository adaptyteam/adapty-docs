...
2. `openWebPaywall(paywall)` — генерирует URL по пейволу без добавления данных продукта в URL. В SDK v4 принимает `AdaptyFlowPaywall` — вариант пейвола из полученного флоу. Убедитесь, что `flow.paywalls` не пустой, прежде чем обращаться к элементу по индексу, например `flow.paywalls[0]`.
:::
