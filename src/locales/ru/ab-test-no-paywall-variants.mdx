...
1. Откройте раздел [**Flows**](https://app.adapty.io/flows)/[**Paywalls**](https://app.adapty.io/paywalls) в главном меню Adapty и выберите флоу/пейвол A.
...
Повторите те же шаги для флоу/пейвола B, но установите значение `show_paywall` равным `false`.
...
:::important
Если вы отображаете пейвол самостоятельно (без рендеринга Adapty), вызовите [`logShowFlow` (iOS SDK v4+) / `logShowPaywall`](present-remote-config-paywalls#track-paywall-view-events) при показе флоу/пейвола A. Без этого Adapty не сможет считать просмотры в тесте. Не логируйте просмотр для флоу/пейвола B, так как он никогда не показывается.
:::
