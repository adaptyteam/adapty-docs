...
Facebook Ads entegrasyonu sayesinde uygulama istatistiklerinizi Meta Analytics'te kolayca takip edebilirsiniz. Adapty, Meta Ads Manager'a etkinlikler göndererek aboneliklere dayalı benzer kitleler oluşturmanıza ve daha iyi dönüşümler elde etmenize yardımcı olur. Bu sayede reklamlarınızın aboneliklerden ne kadar gelir sağladığını doğru bir şekilde görebilirsiniz.

Adapty ile Facebook Ads arasındaki entegrasyon şu şekilde çalışır: Adapty, entegrasyonunuzda yapılandırılmış tüm abonelik olaylarını Facebook Ads'e gönderir. Bu entegrasyon, reklam kampanyalarınızın etkinliğini değerlendirmek için faydalıdır.
...
## Gelişmiş eşleştirme (Conversions API) \{#advanced-matching-conversions-api\}

Facebook CAPI, yukarıda açıklanan Facebook Ads entegrasyonundan bağımsız bir entegrasyondur; kendi kimlik bilgilerine sahiptir ve Adapty Kontrol Paneli'nde ayrı olarak yapılandırılır. Etkinleştirirseniz, Adapty gelişmiş eşleştirme için Meta'nın Conversions API'sine karma kimlik verisi iletir. Adapty, kullanıcının Adapty profilinden e-posta, telefon numarası, ad ve soyadı, doğum tarihi, cinsiyet ve ülke bilgilerini SHA-256 ile şifreleyerek gönderir. Facebook tıklama ve tarayıcı tanımlayıcıları (`fbc`, `fbp`) ile IP adresi/kullanıcı aracısı ise diğer Meta CAPI entegrasyonlarında olduğu gibi şifrelenmeden iletilir.
