...
description: "Adapty kullanarak uygulama içi satın almalar ve abonelikleri yönetme rehberi."
...
Mobil uygulamanızda paywall'ları görüntülemek, kullanıcılara premium içerik veya hizmetlere erişim sunmanın önemli bir adımıdır. Ancak bir paywall'ın satın alma işlemlerini kendi başına desteklemesi yalnızca Adapty'nin ekranı render ettiği durumlarda geçerlidir; yani bir [flow](adapty-flow-builder) veya [eski Paywall Builder](adapty-paywall-builder) ile oluşturulan bir paywall söz konusu olduğunda.

Ekranı kendi kodunuzda render ediyorsanız, satın alma işlemini tamamlamak ve istenen içeriğin kilidini açmak için `.makePurchase()` adlı ayrı bir yöntem kullanmanız gerekir. Bu yöntem, kullanıcıların paywall'larla etkileşime girmesi ve istedikleri işlemleri gerçekleştirmesi için bir köprü görevi görür.
...
:::warning
Unutmayın ki başlangıç teklifi yalnızca ekranı Adapty render ettiğinde otomatik olarak uygulanır.
...
:::note
**Ekranı Adapty mı oluşturuyor?** Bir [flow](adapty-flow-builder) veya [Paywall Builder](adapty-paywall-builder) paywalli için satın almalar otomatik olarak işlenir; bu adımı atlayabilirsiniz.

**Adım adım rehber mi arıyorsunuz?** Tam bağlam ile uçtan uca uygulama talimatları için [hızlı başlangıç kılavuzuna](android-implement-paywalls-manually) göz atın.
:::
...
:::warning
**Not:** Apple'ın StoreKit sürümü v2.0'dan düşük ve Adapty SDK sürümü v2.9.0'dan düşükse, bunun yerine [Apple App Store paylaşımlı sırrı](app-store-connection-configuration#step-5-enter-app-store-shared-secret) sağlamanız gerekir. Bu yöntem Apple tarafından artık kullanım dışı bırakılmıştır.
:::
...
:::warning
`subscriptionUpdateParams` parametresini yalnızca kullanıcı aynı Google hesabında satın alınmış aktif bir abonelikten geçiş yaptığında kullanın. Yeni bir satın alma için `makePurchase`'ı bu parametre olmadan çağırın — parametreyi geçirmek, mesajı yanıltıcı biçimde etkin olmayan bir aboneliğe veya farklı bir Google hesabına işaret eden `CURRENT_SUBSCRIPTION_TO_UPDATE_NOT_FOUND_IN_HISTORY` hatasıyla satın almanın başarısız olmasına neden olur.
:::
