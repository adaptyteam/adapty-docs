...
* Kullanıcının karşılaştığı ilk placement, Adapty'nin hangi paywall'ı göstereceğini belirler. Kullanıcının atamasını değiştiremez veya aynı kullanıcıyı birden fazla Crossplacement A/B testine ekleyemezsiniz.

* Bir cihaz, başka bir yerde (örneğin kullanıcının diğer cihazında) yapılan cross-placement atamalarını yalnızca uygulama aktif hale geldiğinde ve en fazla 60 saniyede bir öğrenir. Bu senkronizasyon zorlanamaz; bu nedenle bir atama, uzun süre çalışan bir oturuma ulaşmak için bir dakikaya veya bir sonraki ön plana geçişe kadar sürebilir.

:::warning
Bir kullanıcı Crossplacement paywall aldığında, testi durdursanız bile bunu 90 gün boyunca görür. Bu süreyi değiştirmek için **General** ayarlarında **[Cross-placement variation stickiness](general#9-cross-placement-variation-stickiness)** seçeneğini düzenleyin.
:::
