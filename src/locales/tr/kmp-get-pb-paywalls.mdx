...
| Parametre                    | Zorunluluk     | Açıklama                                                     |
| :--------------------------- | :------------- | :----------------------------------------------------------- |
| **flow**                     | zorunlu        | `Adapty.getFlow` ile elde edilen bir `AdaptyFlow` nesnesi. |
| **locale**                   | isteğe bağlı   | Görünümün hangi [flow yerelleştirmesiyle](add-paywall-locale-in-adapty-paywall-builder) render edileceğini belirten tanımlayıcı — örneğin `en` veya `pt-br`. Belirtilmezse görünüm `en` dilinde ya da flow'un `en` yerelleştirmesi yoksa varsayılan yerelleştirmede render edilir. Bkz. [Yerelleştirmeler ve yerel ayar kodları](kmp-localizations-and-locale-codes). |
| **loadTimeout**              | isteğe bağlı   | Bu değer, metodun zaman aşımını sınırlar. Zaman aşımına ulaşılırsa önbelleğe alınmış veri veya yerel yedek döndürülür. Arka planda farklı isteklerden oluşabileceğinden bu metod zaman zaman `loadTimeout` içinde belirtilenden biraz daha geç zaman aşımına uğrayabilir. `kotlin.time.Duration.Companion` paketinden `5.seconds` gibi uzantı fonksiyonlarını kullanabilirsiniz. |
| **preloadProducts**          | isteğe bağlı   | Daha iyi performans için ürünleri önceden yüklemek amacıyla `true` olarak ayarlayın. Etkinleştirildiğinde ürünler önceden yüklenir, bu da flow veya paywall'ın gösterilmesi için gereken süreyi azaltır. |
| **productPurchaseParameters**| isteğe bağlı   | [`AdaptyProductIdentifier`](https://kmp.adapty.io/adapty/com.adapty.kmp.models/-adapty-product-identifier/) ile [`AdaptyPurchaseParameters`](https://kmp.adapty.io/adapty/com.adapty.kmp.models/-adapty-purchase-parameters/) arasında bir eşleme. Flow veya paywall'daki bireysel ürünler için kişiselleştirilmiş teklifler veya abonelik güncelleme parametreleri gibi satın alma özgü parametreleri yapılandırmak için kullanın. |
