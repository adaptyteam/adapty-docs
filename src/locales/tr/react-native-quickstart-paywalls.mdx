---
title: "React Native SDK'da Flow & Paywall Builder ile satın almaları etkinleştirme"
...
| Manuel oluşturulan paywalllar | 🟡 Orta | Paywall arayüzünü uygulama kodunuzda siz uygularsınız; ancak ürün tekliflerinde esnekliği korumak için flow nesnesini Adapty'den alırsınız. [Kılavuza](react-native-quickstart-manual) bakın. |
| Observer mode | 🔴 Zor | Kendi satın alma altyapınız zaten var ve bunu kullanmaya devam etmek istiyorsunuz. Observer mode'un Adapty'de bazı sınırlamaları olduğunu unutmayın. [Makaleye](observer-vs-full-mode) bakın. |

:::important
**Aşağıdaki adımlar, Adapty Flow & Paywall Builder'da oluşturulmuş bir flow'u nasıl uygulayacağınızı göstermektedir.**

Paywall arayüzünü kendiniz oluşturmayı tercih ediyorsanız, bkz. [Paywall'ları manuel olarak uygulama](react-native-quickstart-manual).
:::

Adapty Flow & Paywall Builder'da oluşturulmuş bir flow'u uygulamanızda görüntülemek için kodunuzda yalnızca şunları yapmanız gerekir:

1. **Flow'u alın**: Adapty'den alın.
2. **Görüntüleyin; satın almaları Adapty halleder**: View'u uygulamanızda gösterin.
3. **Buton aksiyonlarını yönetin**: Kullanıcı etkileşimlerini uygulamanızın yanıtlarıyla ilişkilendirin. Örneğin, kullanıcılar butonlara tıkladığında bağlantı açın veya flow'u kapatın.
...
3. [Bir flow oluşturun](paywall-builder-templates) ve [ürünleri flow'a ekleyin](paywall-product-block).
4. [Bir placement oluşturun ve flow'unuzu buna ekleyin](create-placement).
5. Uygulama kodunuza [Adapty SDK'yı yükleyip etkinleştirin](sdk-installation-reactnative). Bu kılavuz Adapty React Native SDK v4 API'lerini kullanmaktadır.
...
Adapty Flow & Paywall Builder'da oluşturulan bir flow'u edinmek için `getFlow` metodunu kullanarak [placement](placements) ID'sine göre `flow` nesnesini alın. Flow, görüntülemek için gereken UI öğelerini ve stillendirme bilgilerini içerir.
