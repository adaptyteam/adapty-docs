...
---
:::tip
**SDK v4'ten itibaren**, onboarding'lere daha güçlü bir alternatif olarak [flow'lar](android-get-pb-paywalls) oluşturabilirsiniz. WebView içinde çalışan onboarding'lerin aksine, flow'lar cihazda native olarak render edilir — bu sayede daha akıcı animasyonlar, tutarlı bir Android görünümü ve hissi, daha hızlı yükleme süreleri ve WebView çalışma zamanı bağımlılığı ortadan kalkar. Başlamak için [Flow'ları ve paywallları al](android-get-pb-paywalls) ve [Flow'ları ve paywallları göster](android-present-paywalls) bölümlerine bakın.
:::
...
:::tip
Eğer paywall'ı onboarding içinde açmak istiyorsanız bu olayı ele alın. Paywall kapandıktan sonra açmak istiyorsanız daha doğrudan bir yöntem var: [`AdaptyOnboardingCloseAction`](#closing-onboarding) olayını ele alın ve olay verisine gerek kalmadan paywall'ı açın.
:::

Onboarding'lerde paywall'larla çalışmanın en sorunsuz yolu, action ID'yi bir paywall placement ID'siyle eşit tutmaktır. Bu şekilde `AdaptyOnboardingOpenPaywallAction` sonrasında placement ID'yi kullanarak paywall'ı hemen alıp açabilirsiniz:
```kotlin
