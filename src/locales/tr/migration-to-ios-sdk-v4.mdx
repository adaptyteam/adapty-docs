...
## App Store'da tanıtılan uygulama içi satın almalar geçici olarak kaldırıldı \{#app-store-promoted-in-app-purchases-temporarily-removed\}

StoreKit 2 geçişinin bir parçası olarak, Adapty iOS SDK 4.0, App Store'da tanıtılan uygulama içi satın alma desteğini kaldırmaktadır. `shouldAddStorePayment(for:)` delegate metodu ve bu metodun aldığı `AdaptyDeferredProduct` tipi 4.0'da kullanılamamaktadır.

:::warning
Bu kaldırma işlemi yalnızca 4.0 için geçerlidir — tanıtılan uygulama içi satın alma desteği, SDK 4.1'de yeni bir API ile geri dönmektedir. Uygulamanız tanıtılan uygulama içi satın almalara bağımlıysa, 3.x'ten doğrudan 4.1'e geçiş yapın. Bkz. [App Store'dan uygulama içi satın almalar](making-purchases#in-app-purchases-from-the-app-store).
:::
