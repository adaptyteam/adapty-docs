...
Yukarıda açıklanan adımları tamamladıktan sonra, attribution verilerini kaydetmek için `updateExternalAttribution` metodunu (SDK 4.1 sürümünden önce `updateAttribution` olarak adlandırılıyordu) çağırın ve entegrasyon tanımlayıcısını ayarlamak için `Adapty.setIntegrationIdentifier()` metodunu kullanın.

AppsFlyer SDK'yı başlatın ve Adapty'de kullanıcıları tanımlamadan önce UID callback'ini bekleyin. Aksi takdirde `appsflyer_id`, aktivasyon sırasında oluşturulan kısa ömürlü anonim bir Adapty profiline atanır ve tanımlanan profile her zaman aktarılamayabilir. Bu durumda, AppsFlyer gelir iletimi sessizce başarısız olur.
