...
Bu dezavantajlara karşın daha hızlı veri çekme kullanım durumunuz için daha önemliyse, aşağıda gösterildiği gibi `GetPaywallForDefaultAudience` kullanın. Aksi takdirde, [yukarıda](#fetch-paywall-designed-with-paywall-builder) açıklandığı şekilde `GetPaywall` kullanın.
:::
```csharp showLineNumbers
