...
[Yedek dosya](fallback-flows) formatı 4.0'da ve yeniden 4.1.1'de değişti. Daha önce bir 4.0 beta sürümü için indirmiş olsanız bile, dosyayı **[Placements](https://app.adapty.io/placements)** > **Fallbacks** bölümünden tekrar indirip uygulamanıza ekleyin.
...
:::note
`customTimers` hâlâ mevcuttur, ancak yalnızca eski Paywall Builder paywalllarını etkiler. Bir flow'un [geri sayım sayacı](flow-timer), Flow & Paywall Builder'da belirlenen davranışa göre çalışır; dolayısıyla bir flow, buraya ilettiğiniz değeri yok sayar.
:::

| Parametre | Açıklama |
|---|---|
| `locale` | Flow'un hangi dille görüntüleneceğini belirtir. Bu parametre `getPaywall`'dan buraya taşındı — bkz. [getPaywall → getFlow](#getpaywall--getflow). |
...
## Onboarding API'sının kullanımdan kaldırılması \{#onboarding-api-deprecation\}

Eski onboarding API'sı, v4'te [Flow & Paywall Builder](adapty-flow-builder) lehine kullanımdan kaldırılmıştır. Hâlâ çalışmaktadır ancak gelecekteki bir sürümde kaldırılacaktır; bu nedenle onboardinglerınızı Flow & Paywall Builder'a taşımayı planlayın.
