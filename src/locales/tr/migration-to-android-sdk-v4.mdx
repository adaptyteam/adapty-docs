---
title: "Adapty Android SDK'yı v4.0'a Taşıyın"
description: "Adapty Android SDK v4.0'a geçiş yapın: paywall API'larını flow API'larıyla değiştirin; yeni API'lar hem flow'ları hem de eski builder paywalllarını destekler."
metadataTitle: "Adapty Android SDK v4.0'a Geçiş | Adapty Dokümanları"
---

Adapty Android SDK 4.0, flow'ları tanıtır ve paywall API'larını buna göre yeniden adlandırır. Yeni API'lar flow'larla çalışır; eski builder'dan gelen paywalllarla da sorunsuz çalışmaya devam eder — Adapty Kontrol Paneli tarafında herhangi bir kurulum değişikliği gerekmez.
...
`adapty-bom` sürümünü `4.0.1` (veya üzeri) olarak ayarlayın ve projeyi senkronize edin. BOM, sizin için uygun `android-sdk` ve `android-ui` sürümlerini otomatik olarak belirler. Bağımlılık tanımlamaları için [Adapty SDK'yı Yükle](sdk-installation-android) sayfasına bakın.
...
- **Onboardinglar kullanımdan kaldırıldı.** `AdaptyUI.getOnboardingView` ve `AdaptyUI.getOnboardingConfiguration`, 4.0 sürümünde `@Deprecated` olarak işaretlendi — onboardingları [Flow & Paywall Builder](adapty-flow-builder) ile oluşturulan flow'lara taşıyın.

## Flow'ları getirme \{#fetching-flows\}

### getPaywall + getViewConfiguration → getFlow + getFlowConfiguration

Fetch dönüş türü `AdaptyPaywall`'dan `AdaptyFlow`'a değişiyor ve yapılandırma yükleyici `AdaptyUI.getViewConfiguration`'dan `AdaptyUI.getFlowConfiguration`'a yeniden adlandırılıyor (`AdaptyUI.LocalizedViewConfiguration` yerine `AdaptyUI.FlowConfiguration` döndürüyor). `locale` parametresi fetch çağrısından çıkarılarak `getFlowConfiguration`'a taşınıyor: `AdaptyFlow` aynı zamanda `paywalls`'ı da sunuyor — flow'un paywall varyasyonları, `AdaptyFlowPaywall` değerleri olarak. Web paywall metotları bunlardan birini alıyor.
...
`locale` parametresi `getFlowConfiguration` üzerinde isteğe bağlı olmaya devam eder: belirtmezseniz görünüm `en` dilinde render edilir ya da flow'un `en` yerelleştirmesi yoksa flow'un varsayılan yerelleştirmesinde render edilir. Bkz. [Yerelleştirmeler ve yerel ayar kodları](android-localizations-and-locale-codes).
...
`logShowPaywall`, `logShowFlow` olarak yeniden adlandırıldı ve artık `AdaptyPaywall` yerine `AdaptyFlow` alıyor. Olay hâlâ aynı varyasyon üzerinden kaydedildiğinden, mevcut huni ve A/B testi metrikleri kontrol panelinde herhangi bir değişiklik yapılmadan çalışmaya devam eder.
...
Önceden tanımlı kaynaklar: `AdaptyAttributionSource.APPLE_ADS`, `.ADJUST`, `.APPSFLYER`, `.BRANCH`, `.TENJIN`. Başka bir kaynak için bir string'den oluşturun: `AdaptyAttributionSource("your_source")`.

:::note
SDK 4.1, bu metodu `updateExternalAttribution` olarak ve türü `AdaptyExternalAttributionProvider` olarak yeniden adlandırır. Eski adlar, 4.1'de deprecation uyarısı verilmeden kaldırılmıştır. Doğrudan 4.1'e geçiş yapıyorsanız, [v4.1'e Geçiş](migration-to-android-sdk-41) bölümüne bakın.
:::
