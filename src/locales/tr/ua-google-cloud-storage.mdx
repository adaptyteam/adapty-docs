...
Adapty Attribution'ın Google Cloud Storage ile entegrasyonu, kullanıcı edinme kampanya verilerinizi güvenli bir şekilde tek bir merkezi konumda saklamanıza olanak tanır. Kampanya performans verilerinizi, attribution verilerinizi ve kullanıcı edinme etkinliklerinizi Google Cloud Storage bucket'ınıza .csv dosyaları olarak kaydedebileceksiniz.
Bu entegrasyonu kurmak için Google Cloud Console ve Adapty Attribution Kontrol Paneli'nde birkaç basit adımı takip etmeniz gerekecek.

:::note
Zamanlama

Adapty Attribution, Google Cloud Storage'a her takvim günü için bir set dosya aktarır. Dışa aktarma zamanlayıcısı günde üç kez çalışır — **02:00**, **04:00** ve **06:00 UTC**. Her çalışma, bir önceki günün dışa aktarma işini henüz oluşturulmadıysa oluşturur ya da daha önceki bir çalışma başarısız olduysa yeniden dener. Verileriniz genellikle 02:00 çalışmasından sonra gelir; sonraki çalışmalar veri eklemek için değil, başarısız işleri kurtarmak için mevcuttur.

Her dosya, önceki takvim gününün tamamında (UTC) oluşturulan etkinliklere ait verileri içerir. Örneğin, 8 Mart'ta dışa aktarılan veriler, 7 Mart'ta UTC 00:00:00 ile 23:59:59 arasında oluşturulan tüm etkinlikleri kapsar.
:::

:::warning
Zamanlanmış dışa aktarma yalnızca ücretli planda olan veya deneme sürecindeki şirketler için çalışır. Şirketiniz bu iki kategoriden birine girmiyorsa, zamanlayıcı bu adımı atlar ve entegrasyon kontrol panelinde etkin görünse bile herhangi bir dosya oluşturmaz. Bucket'ınız boşsa ve yapılandırma doğru görünüyorsa, önce plan durumunuzu kontrol edin.
:::
...
| `bundle_id`              | Uygulama bundle tanımlayıcısı (örn. `com.example.app`) |
| `device_brand`           | Cihaz markası (örn. `Apple`, `Samsung`)   |
| `device_model`           | Cihaz modeli (örn. `iPhone15,2`)          |
| `os_version`             | Ana OS sürümü                             |
...
| `utm_content`            | UTM content parametresi                   |
| `click_time`             | Attribution edilen tıklamanın Unix saniye cinsinden zaman damgası. Kuruluma atfedilen tıklama yoksa boş kalır. |