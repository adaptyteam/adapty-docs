...
:::note
**Google Play promosyon kodları**: Google Play promosyon kodu kullanılarak yapılan bir satın alma, `orderId` olmadan gelebilir. Adapty'nin tek seferlik (abonelik dışı) satın alma doğrulaması bir `orderId` gerektirdiğinden, bu kodla yapılan satın almalar otomatik olarak doğrulanmaz veya erişim verilmez. Aşağıdaki adımlarla erişimi manuel olarak verin — Sunucu Tarafı API'si `orderId`'ye bağlı değildir.
:::
