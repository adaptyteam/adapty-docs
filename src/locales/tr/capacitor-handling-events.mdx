...
[Flow & Paywall Builder](adapty-flow-builder)'da oluşturulan flow'lar ve paywalllar, satın alma yapmak ve geri yüklemek için ekstra kod gerektirmez. Ancak, uygulamanızın yanıt verebileceği bazı olaylar üretirler. Bu olaylar arasında düğme basışları (kapat düğmeleri, URL'ler, ürün seçimleri vb.) ile flow üzerinde gerçekleştirilen satın alma işlemlerine ilişkin bildirimler yer alır. Bu olaylara nasıl yanıt vereceğinizi aşağıda öğrenebilirsiniz.

Mobil uygulamanızdaki flow ekranında gerçekleşen süreçleri kontrol etmek veya izlemek için `view.setEventHandlers` metodunu uygulayın:
...
Ne yapacağınızı öğrenmek için [Flow ekran görüntülemelerini izleme](capacitor-flow-screen-views) konusuna bakın.

Bir flow, kullanıcıların yazdığı ve seçtiği değerleri de raporlar. Bunları okumak için [Flow'lardan gelen verileri işleme](capacitor-flow-input) konusuna bakın.
