...
keywords: ['sdk yükle', 'sdk kurulumu', 'android sdk yükle', 'google play billing library', 'gpbl', 'billing library', 'adaptyui']
rank: 70
---
...
- **AdaptyUI**: [Adapty Paywall Builder](adapty-paywall-builder) kullanıyorsanız bu modüle ihtiyacınız vardır. Adapty Paywall Builder, kolayca çapraz platform paywall'lar oluşturmanızı sağlayan, kod gerektirmeyen kullanıcı dostu bir araçtır. AdaptyUI, core modülüyle birlikte otomatik olarak etkinleştirilir.
:::tip
Adapty SDK'nın bir mobil uygulamaya nasıl entegre edildiğine dair gerçek bir örnek görmek ister misiniz? Paywall gösterme, satın alma yapma ve diğer temel işlevler dahil tam kurulumu sergileyen [örnek uygulamamıza](https://github.com/adaptyteam/AdaptySDK-Android/tree/master/app) göz atın.
:::
...
dependencies {
    ...
    // Paywall Builder kullanmayı planlıyorsanız bu satırı ekleyin:
    implementation("io.adapty:android-ui")
}
```
...
Bağımlılık çözümlenmiyorsa, Gradle script'lerinizde `mavenCentral()` olduğundan emin olun.

<details>
   <summary>Nasıl ekleneceğine dair talimat</summary>

   Projenizde `settings.gradle` dosyasında `dependencyResolutionManagement` yoksa, en üst seviye `build.gradle` dosyanızın repositories bölümünün sonuna şunu ekleyin:
...
```
Aksi takdirde, `settings.gradle` dosyanızın `dependencyResolutionManagement` bölümündeki `repositories` kısmına şunları ekleyin:
...
:::important
Diğer Adapty SDK metodlarını çağırmadan önce `Adapty.activate` işleminin tamamlanmasını bekleyin. Tam sıralama için [Android SDK'da çağrı sırası](android-sdk-call-order) bölümüne bakın.
:::

Şimdi uygulamanızda paywallları kurun:

- [Adapty Paywall Builder](adapty-paywall-builder) kullanıyorsanız [Paywall Builder hızlı başlangıç kılavuzu](android-quickstart-paywalls)'nu takip edin.
- Kendi paywall arayüzünüzü oluşturuyorsanız [özel paywalllar için hızlı başlangıç kılavuzu](android-quickstart-manual)'na bakın.
...
#### Günlük kayıt sistemini kurun \{#set-up-the-logging-system\}
...
#### Kayıt sistemi mesajlarını yönlendirme \{#redirect-the-logging-system-messages\}
...
Bu parametreyi; kullanıcı gizliliğini artırmak, bölgesel veri koruma düzenlemelerine (GDPR veya CCPA gibi) uymak ya da IP tabanlı özelliklere uygulamanızda ihtiyaç duyulmadığında gereksiz veri toplamayı azaltmak için kullanabilirsiniz.
...
#### Reklam kimliği (Ad ID) toplama ve paylaşımını devre dışı bırakma \{#disable-advertising-id-ad-id-collection-and-sharing\}
...
Bu parametreyi Play Store politikalarına uymak, reklam kimliği izin istemini tetiklemekten kaçınmak veya uygulamanız Reklam Kimliği tabanlı attribution ya da analitik gerektirmiyorsa kullanın.
...
`AdaptyUI.configureMediaCache` yöntemini kullanarak varsayılan önbellek boyutunu ve geçerlilik süresini değiştirebilirsiniz. Bu adım isteğe bağlıdır; bu yöntemi çağırmazsanız varsayılan değerler kullanılır (100 MB disk boyutu, 7 günlük geçerlilik süresi).
...
Uygulamanız birden fazla process kullanıyorsa, Adapty'yi yalnızca bir kez başlatın; aksi takdirde beklenmedik davranışlar yaşanabilir.
...
Bazı SDK'lar (Adapty dahil), kendi Android Auto Backup yapılandırmalarıyla birlikte gelir. Yedekleme kuralları tanımlayan birden fazla SDK kullanıyorsanız, Android manifest birleştirici `android:fullBackupContent`, `android:dataExtractionRules` veya `android:allowBackup` içeren bir hatayla başarısız olabilir.
...
#### 2. `<application>` içindeki yedekleme özelliklerini geçersiz kılın \{#2-override-backup-attributes-in-application\}
Uygulamanızın `AndroidManifest.xml` dosyasında, uygulamanızın nihai değerleri sağladığını ve manifest birleştiricisine kütüphane değerlerini değiştirmesini söylediğini belirtmek için `<application>` etiketini güncelleyin:
...
- Manifest merger, uygulamanızın yapılandırmasını kullanır ve çakışan yedekleme özelliklerinde artık başarısız olmaz.
