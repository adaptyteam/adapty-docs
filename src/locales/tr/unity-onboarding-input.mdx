...
description: "Adapty SDK ile Unity uygulamanızda onboarding verilerini kaydedin ve kullanın."
metadataTitle: "Onboardings | Unity SDK | Adapty Dokümanları"
displayed_sidebar: sdkunity
---

:::warning
**Onboardings, SDK v4'te kullanımdan kaldırılmıştır ve gelecekteki bir sürümde kaldırılacaktır.** Artık hata düzeltmesi veya iyileştirme almamaktadır. Bunun yerine [flow'ları](unity-get-pb-paywalls) kullanın: onboarding'ler WebView içinde çalışırken, flow'lar cihazda yerel olarak render edilir — bu sayede daha akıcı animasyonlar, tutarlı bir yerel görünüm, daha hızlı yükleme süreleri ve WebView bağımlılığı olmadan çalışma imkânı sunar. Başlamak için [Flow'ları ve paywall'ları edinin](unity-get-pb-paywalls) ve [Flow'ları ve paywall'ları görüntüleyin](unity-present-paywalls) sayfalarına bakın.
:::

Kullanıcılarınız bir sınav sorusunu yanıtladığında veya bir giriş alanına veri girdiğinde, `OnboardingViewOnStateUpdatedAction` metodu çağrılır. Alan türünü kodunuzda kaydedebilir veya işleyebilirsiniz.

`OnboardingViewOnStateUpdatedAction` metodunu sınıfınızda uygulayın:

:::note
SDK 4.0'da listener arayüzleri C# I-ön eki kuralını takip eder: `AdaptyOnboardingsEventsListener` yerine `IAdaptyOnboardingsEventsListener` uygulayın. Metotlar değişmemiştir. [Geçiş kılavuzuna](migration-to-unity-sdk-v4) bakın.
:::
