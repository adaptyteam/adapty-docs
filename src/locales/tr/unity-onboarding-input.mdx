...
Kullanıcılarınız bir test sorusunu yanıtladığında veya bir giriş alanına veri girdiğinde `OnboardingViewOnStateUpdatedAction` metodu çağrılır. Alan türünü kodunuzda kaydedebilir veya işleyebilirsiniz.

Sınıfınızda `OnboardingViewOnStateUpdatedAction` metodunu uygulayın:

:::note
SDK 4.1'de listener arayüzleri C# I-öneki kuralına uymaktadır: `AdaptyOnboardingsEventsListener` yerine `IAdaptyOnboardingsEventsListener` uygulayın. Metotlar değişmemiştir. [Geçiş rehberine](migration-to-unity-sdk-v4) bakın.
:::
