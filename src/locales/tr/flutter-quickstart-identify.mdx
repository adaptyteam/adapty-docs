...
:::important
Müşteri kullanıcı kimlikleri her kullanıcı için benzersiz olmalıdır. Parametre değerini sabit kodlarsanız, tüm kullanıcılar tek bir kullanıcı olarak kabul edilir.
:::

Diğer SDK metodlarını çağırmadan önce her zaman `identify` işlemini `await` ile bekleyin. Eş zamanlı çağrılar `#3006 profileWasChanged` hatasına yol açar veya anonim profile düşer. Bkz. [Flutter SDK'da çağrı sırası](flutter-sdk-call-order).

```dart showLineNumbers
try {
  await Adapty().identify(customerUserId); // Her kullanıcı için benzersiz
