...
## Yeni abonelikler metriği ile `subscription_started` olayı karşılaştırması \{#new-subscriptions-metric-vs-the-subscription_started-event\}
[Yeni abonelikler](reactivated-subscriptions) metriği ve `subscription_started` [entegrasyon eventi](events) farklı şeyleri sayar, bu nedenle toplamları eşleşmez. Metrik, hem deneme süresi olmadan yapılan ilk satın almaları hem de deneme süresi-ücretli dönüşümlerini sayar. `subscription_started` eventi yalnızca deneme süresi olmadan yapılan ilk satın almalar için tetiklenir; bir deneme süresi ücretliye dönüştüğünde Adapty bunun yerine `trial_converted` gönderir. Sonuç olarak, uygulamanızda deneme süresi dönüşümleri olduğunda Yeni abonelikler sayısı, `subscription_started` eventi sayısından daha yüksek olur.
