...
[Yeni abonelikler](reactivated-subscriptions) metriği ile `subscription_started` [entegrasyon olayı](events) farklı şeyleri sayar, bu nedenle toplamları eşleşmez. Metrik, hem deneme süresi olmadan yapılan ilk satın almalarını hem de deneme-ücretli dönüşümlerini sayar. `subscription_started` olayı yalnızca deneme süresi olmadan yapılan ilk satın almalar için tetiklenir; bir deneme ücretli aboneliğe dönüştüğünde Adapty bunun yerine `trial_converted` olayını gönderir. Sonuç olarak, uygulamanızda deneme dönüşümleri olduğunda Yeni abonelikler sayısı, `subscription_started` olay sayısından daha yüksek olur.