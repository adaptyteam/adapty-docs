...
- **Builder'da oluşturulan flow'lar**: `createFlowView`, locale parametresi almaz ve SDK cihaz locale'ini okumaz. Flow, [varsayılan locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)'inde render edilir — iOS'ta, flow'un İngilizce yerelleştirmesi varsa `en` ile.
- **Özel (remote config) paywall'lar**: `getFlow`, yapılandırılan tüm yerelleştirmeleri `flow.remoteConfigs` içinde döndürür. Her girdinin bir `lang` kodu ve bir `data` nesnesi vardır. Kullanıcıya uyan girdiyi kendi fallback mantığınızla seçin:
