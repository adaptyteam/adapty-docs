...
### Flow'lar ve paywaller \{#flows-and-paywalls\}
...
Kod hiçbir yerelleştirmeyle eşleşmediğinde, flow sessizce [varsayılan dil](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) ayarıyla gösterilir — SDK bir hata döndürmez ve uyarı kaydetmez.

Kod eşleştiğinde ise Adapty, o yerelleştirmeyi varsayılan olanla birleştirir: eşleşen yerelleştirmede tanımlanmamış string'ler ve asset'ler varsayılan yerelleştirmeden alınır.

Yerel ayar kodunu atlamak, flow'un varsayılan yerelleştirmesini istemekle aynı şey değildir: SDK sabit olarak `en` değerini kullanır. Varsayılan yerel ayarı `de` olan bir flow, `en` yerelleştirmesi varsa `en` dilinde görüntülenir; yalnızca `en` yerelleştirmesi yoksa `de` diline geri döner.

:::warning
Locale kodunu kontrol panelinde yapılandırıldığı şekliyle iletin — küçük harfli alt etiketler kısa çizgiyle ayrılmış olmalıdır. Cihaz locale tanımlayıcısını olduğu gibi geçirmeyin: [`react-native-localize`](https://github.com/zoontek/react-native-localize) paketindeki `getLocales()[0].languageTag` değeri `pt-BR` döndürür ve bu varsayılan yerelleştirmeye geri düşer. Değeri uygulamanızda iletmeden önce dönüştürün.
:::
...
`view.locale`, görünümün gerçekte hangi yerelleştirmeyle oluşturulduğunu bildirir — istediğiniz locale mevcutsa onu, yoksa flow'un varsayılanını döndürür. `locale` parametresi ve `view.locale` her ikisi de React Native SDK 4.0.2 veya sonrasını gerektirir; daha eski sürümlerde `view.locale` `undefined` döner.
  ...
  Yerleşik `AdaptyFlowView` bileşeni kendi görünümünü oluşturduğundan, kodunuzun `locale` okuyacağı bir nesne yoktur. Yerelleştirmeyi bunun yerine [`onAppeared` handler'ının](react-native-handling-events-1#the-view-argument) aldığı nesneden alın:
