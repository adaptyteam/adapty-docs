---
title: "Branch"
description: "Abonelik takibi için Branch'i Adapty ile entegre edin."
...
[Branch](https://www.branch.io/), müşterilerin farklı cihazlar, kanallar ve platformlar arasında kullanıcılara ulaşmasını, onlarla etkileşim kurmasını ve sonuçları değerlendirmesini sağlar. Tüm cihazlar, kanallar ve platformlarda sorunsuz çalışan özel bağlantılar aracılığıyla mobil geliri artırmak için tasarlanmış kullanıcı dostu bir platformdur.

Adapty, mağazalardan gelen [abonelik etkinliklerini](events) tek bir yerde takip etmenizi sağlayan eksiksiz bir veri seti sunar. Adapty ile abonelerinizin davranışlarını kolayca gözlemleyebilir, tercihlerini öğrenebilir ve bu bilgileri hedefli ve etkili bir şekilde iletişim kurmak için kullanabilirsiniz.

Adapty ile Branch arasındaki entegrasyon iki ana şekilde çalışır.

1. **Branch'ten attribution verisi alma**
   Entegrasyonu kurduktan sonra Adapty, Branch'ten attribution verisi almaya başlar. Bu verilere kullanıcının profil sayfasından kolayca erişip görüntüleyebilirsiniz.
...
2. **Branch'e abonelik olayları gönderme**  
   Adapty, entegrasyonunuzda yapılandırılan tüm abonelik olaylarını Branch'e gönderebilir. Böylece bu olayları Branch kontrol panelinde takip edebilirsiniz.
...
### Uygulamanızı Branch'e Bağlayın \{#connect-your-app-to-branch\}

1. Bağlantıyı başlatmak için `.setIntegrationIdentifier()` SDK metodunu çağırın. Branch Identity ID'nizi `customerUserId` parametresine iletebilirsiniz.
...
2. Attribution verilerini kaydetmek için `.updateExternalAttribution()` metodunu (SDK sürüm 4.1 öncesinde `.updateAttribution()` olarak adlandırılıyordu) kullanın. Önceki adımda Branch kullanıcı kimliğini belirtmediyseniz, burada `networkUserId` parametresine iletin.
...
```swift showLineNumbers
  class YourBranchImplementation {
      func initializeBranch() {
          // Branch iOS SDK'nın başlatma metodundan aldığınız attribution'ı Adapty'e aktarın.
...
| `name`                         | String | Branch etkinlik adı (Adapty etkinliğinden eşlenir, örn. "PURCHASE").                                                         |
...
| `user_data.country`            | String | Kullanıcının IP adresine göre ülke kodu.                                                                                      |
| `user_data.ip`                 | String | Kullanıcının IP adresi.                                                                                                       |
| `user_data.idfa`               | String | **Yalnızca iOS**. Reklamverenler için ID.                                                                                     |
| `user_data.idfv`               | String | **Yalnızca iOS**. Satıcılar için ID.                                                                                         |
| `user_data.aaid`               | String | **Yalnızca Android**. Google Advertising ID.                                                                                  |
...
| `event_data.revenue`           | Float  | Gelir miktarı.                                                                                                                |
