---
title: "Capacitor SDK'da onboarding'leri al"
description: "Adapty'de Capacitor için onboarding'lerin nasıl alınacağını öğrenin."
...
:::warning
**Onboardinglar SDK v4'te kullanımdan kaldırılmıştır ve gelecekteki bir sürümde kaldırılacaktır.** Artık düzeltme veya iyileştirme almamaktadır. Bunun yerine [flow'ları](capacitor-get-pb-paywalls) kullanın: WebView içinde çalışan onboardingların aksine, flow'lar cihazda yerel olarak render edilir — bu sayede daha akıcı animasyonlar, tutarlı bir native görünüm ve his, daha hızlı yükleme süreleri ve WebView çalışma zamanı bağımlılığı olmaksızın çalışma imkânı sunar. Başlamak için [Flow'ları ve paywallları edinin](capacitor-get-pb-paywalls) ve [Flow'ları ve paywallları görüntüleyin](capacitor-present-paywalls) sayfalarına bakın.
:::
Adapty Kontrol Paneli'ndeki builder ile [onboarding'inizin görsel kısmını tasarladıktan](design-onboarding) sonra, bunu Capacitor uygulamanızda gösterebilirsiniz. Bu süreçteki ilk adım, aşağıda açıklandığı gibi placement ile ilişkilendirilmiş onboarding'i ve onun görünüm yapılandırmasını almaktır.
