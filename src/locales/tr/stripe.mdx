...
1. Stripe'ta [Developers → API Keys](https://dashboard.stripe.com/apikeys) sayfasına gidin:
...
2. **Secret key** başlığının yanındaki **Reveal live (test) key button** düğmesine tıklayın, ardından anahtarı kopyalayıp Adapty'nin [App Settings → Stripe](https://app.adapty.io/settings/stripe) sayfasına gidin. Anahtarı buraya yapıştırın:
...
4. Adapty'deki webhook URL'sini **Endpoint URL** alanına yapıştırın. Ardından webhook **Version** alanında **Latest API version** seçeneğini belirleyin. Ardından aşağıdaki olayları seçin:
...
5. "Add endpoint" düğmesine basın ve ardından "Signing secret" altındaki "Reveal" düğmesine basın. Bu, webhook verilerini Adapty tarafında çözmek için kullanılan anahtardır; ortaya çıkardıktan sonra kopyalayın:
...
### Stripe %100 teslimat gösteriyor ama hiçbir olay Adapty'e ulaşmıyor \{#stripe-shows-100-delivery-but-no-events-reach-adapty\}

Adapty, webhook imzalama sırrını yalnızca endpoint'i kendisi oluştururken kaydeder. Stripe hesabınızda Adapty'nin URL'sinde zaten bir webhook endpoint'i varsa — örneğin önceki bir bağlantı girişiminden ya da yukarıdaki kullanımdan kaldırılmış manuel kurulum adımlarından kalmışsa — yeniden bağlanma işlemi yeni bir endpoint oluşturmayı atlar ve Adapty imzalama sırrını hiç almaz (ya da yeniden almaz). Her olay mevcut endpoint'e ulaşmaya ve `200` dönmeye devam eder; ancak Adapty tarafında imza doğrulaması başarısız olur ve olay atılır.

Bunu düzeltmek için [Developers → Webhooks](https://dashboard.stripe.com/webhooks) sayfasında Adapty webhook URL'nize işaret eden endpoint'i silin, ardından Adapty'nin yeni bir tane oluşturabilmesi ve secret'ını yakalayabilmesi için [App Settings → Stripe](https://app.adapty.io/settings/stripe) sayfasından Stripe entegrasyonunu yeniden bağlayın.
...
### Net gelir ve vergi \{#net-revenue-and-tax\}

Bir Stripe işlemi için `net_revenue_usd` ve `net_revenue_local` hesaplamak amacıyla Adapty sabit %4,5 oranında bir kesinti uygular. Bu bir modelleme tahminidir, Stripe'ın gerçekte tahsil ettiği vergi değildir — Adapty, Stripe Tax dahil olmak üzere Stripe'tan vergi tutarlarını okumaz; bu nedenle Stripe işlemlerinde `tax_amount_usd` ve `tax_amount_local` her zaman `0` değerini alır.
...
### Birden fazla Adapty uygulamasına bağlı tek bir Stripe hesabı \{#one-stripe-account-connected-to-several-adapty-apps\}
...
`variation_id`'nin yalnızca Stripe Subscription (`sub_...`) ve Checkout Session (`ses_...`) nesnelerinin metadata'sından okunduğunu unutmayın:
...
Stripe etkinliklerinizi iletmek ve analiz etmek için kullanabileceğiniz entegrasyonlar:
- [Adjust](adjust)
- [Amplitude](amplitude/)
- [AppsFlyer](appsflyer)
- [Webhook](webhook)
...
Adjust'ta Stripe alanları yoktur: Adapty, bir Stripe satın almasını Adjust'a, alıcının uygulamanızı yüklediği platformun uygulama token'ı ile gönderir. Bkz. [Web satın almaları (Stripe ve Paddle)](adjust#web-purchases-stripe-and-paddle).
