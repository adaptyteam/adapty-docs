...
## Olay işleme yöntemlerinde view sınıfı adını güncelleme \{#update-view-class-name-in-event-handling-methods\}

Tüm olay işleme yöntemleri artık `AdaptyUIView` yerine yeni `AdaptyUIPaywallView` sınıfını kullanıyor:
```diff showLineNumbers
- void paywallViewDidPerformAction(AdaptyUIView view, AdaptyUIAction action)
