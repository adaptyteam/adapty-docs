...
[Mixpanel](https://mixpanel.com/home/), güçlü bir ürün analitik hizmetidir. Olay tabanlı takip çözümü, ürün ekiplerine farklı platformlarda en iyi kullanıcı edinimi, dönüşüm ve elde tutma stratejilerine ilişkin değerli bilgiler sunar.

Bu entegrasyon, tüm Adapty etkinliklerini Mixpanel'e aktarmanızı sağlar. Böylece abonelik işiniz ve müşteri davranışları hakkında çok daha kapsamlı bilgiler edinirsiniz. Adapty, mağazalardan gelen [abonelik etkinliklerini](events) tek bir yerden takip etmenizi sağlayan eksiksiz bir veri seti sunar. Adapty sayesinde abonelerinizin davranışlarını kolayca görebilir, ne istediklerini öğrenebilir ve bu bilgileri kullanarak onlarla hedefli ve etkili bir şekilde iletişim kurabilirsiniz.
## Mixpanel entegrasyonunu nasıl kurarsınız \{#how-to-set-up-mixpanel-integration\}

1. Adapty Kontrol Paneli'nde [Integrations -> Mixpanel](https://app.adapty.io/integrations/mixpanel) sayfasını açın.
2. Geçişi etkinleştirin ve **Mixpanel Token** bilginizi girin. Adapty, her platform için ayrı **Production** ve **Sandbox** token'ları tutar: iOS, Android, Stripe ve Paddle. Yalnızca veri almak istediğiniz platformları doldurun — token girilmemiş bir platform hiçbir event göndermez.
3. **Mixpanel Data Residency** değerini Mixpanel projenizle eşleşecek şekilde ayarlayın. Bu alan zorunludur ve varsayılan olarak **US** seçilidir. `api.mixpanel.com` endpoint'i için **US**, `api-eu.mixpanel.com` için **Europe** seçin.
...
### `access_level_updated` Olay Akışında başarısız olarak görünüyor \{#access_level_updated-shows-as-failed-in-the-event-feed\}
