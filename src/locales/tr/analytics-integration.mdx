...
metadataTitle: "Analytics Entegrasyon Rehberi | Adapty Dokümanları"
---

Adapty, tüm [abonelik etkinliklerini](events) [Amplitude](amplitude), [Mixpanel](mixpanel) ve [AppMetrica](appmetrica) gibi analitik servislere gönderir. Ayrıca [webhook](webhook) entegrasyonu kullanarak etkinlikleri sunucunuza da iletebiliriz. Bunun en güzel yanı, etkinlikleri kendiniz göndermenize gerek olmamasıdır; biz hallederiz. Tek yapmanız gereken entegrasyonu Adapty Kontrol Paneli'nde yapılandırmak.
...
Bize bildirin! [Bir özellik talebi oluşturun](https://adapty.featurebase.app/en?b=6979f233ebd3cffd4f425ba0), değerlendirip ekleyebiliriz.
:::
## Etkinlik özellikleri \{#event-properties\}

Webhook etkinlikleri JSON formatında gönderilir. Tüm etkinlikler aynı yapıyı takip eder, ancak alanları etkinlik türüne, mağazaya ve özel yapılandırmanıza göre farklılık gösterir.
...
| **price_local**               | float         | Apple/Google kesintisi öncesi yerel para birimindeki ürün fiyatı. Gelir. |
| **proceeds_local**            | float         | Apple/Google kesintisi sonrası yerel para birimindeki ürün fiyatı. Net gelir. |
...
| **subscription_expires_at**   | ISO 8601 date | Aboneliğin sona erme tarihi. Genellikle gelecekteki bir tarihtir. |
| **consecutive_payments**      | int           | Kullanıcının kesintisiz abonelikte geçirdiği dönem sayısı. Mevcut dönem dahildir. |
| **rate_after_first_year**     | bool          | Bir yıllık kesintisiz yenilemeden sonra aboneliğin indirimli komisyon oranına (%15) hak kazanıp kazanmadığını belirten boolean. Komisyon oranları programa uygunluk ve ülkeye göre değişir. Ayrıntılar için bkz. [Mağaza komisyonu ve vergiler](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue). |
| **promotional_offer_id**      | str           | Adapty Kontrol Paneli'ndeki Ürün bölümünde belirtilen promosyon teklifi ID'si. |
...
| **profile_total_revenue_usd** | float         | Profil için iade işlemleri dahil toplam gelir.               |
...
| **is_lifetime**               | bool          | Ücretli access level'ın süresiz erişim olup olmadığını belirten boolean. |
...
| **renewed_at**                | ISO 8601 date | Ücretli erişimin mevcut döneminin en son başladığı tarih ve saat. İlk yenilemeden önce `activated_at` ile aynı değeri alır. |
...
:::warning
Harici analizleri engellemek, bu kullanıcı için özel [webhook'lar](webhook) dahil **tüm** entegrasyonlara yapılan etkinlik teslimini durdurur. Backend'iniz bu kullanıcının erişimini yönetmek için webhook etkinliklerine güveniyorsa bunu kullanmayın.

Bu ayar geriye dönük olarak uygulanmaz: ayarlanmadan önce oluşturulan etkinlikler yine de iletilir. Ayrıca Adapty'nin kendi analizlerini etkilemez — kullanıcının işlemleri kontrol paneli grafiklerinde görünmeye devam eder.
:::
