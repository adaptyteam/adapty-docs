...
**iOS: SDK v4'te Kids Mode'u Etkinleştirme (beta)**

:::important
SDK v4'te, native iOS SDK'sı Swift Package Manager aracılığıyla kurulur ve Kids Mode, `KidsMode` Swift paket trait'i ile etkinleştirilir. Bu trait, tüm IDFA, AdSupport ve AppTrackingTransparency kodlarını derleme aşamasında devre dışı bırakır. Bu özellik **Xcode 26** veya üzerini gerektirir.
:::

SDK v4'te, `pubspec.yaml` dosyanızda `adapty_flutter` yerine `adapty_flutter_kids` paketini kullanın. Bu paket, aynı genel API'ye ve aynı sürüme sahip Kids Mode varyantıdır; tek farkı, native iOS SDK'sının `KidsMode` trait'iyle derlenmesidir:
