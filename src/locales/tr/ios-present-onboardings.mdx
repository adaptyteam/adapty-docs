...
:::tip
**SDK v4 (beta) itibarıyla**, onboarding'lere daha güçlü bir alternatif olarak [flow'lar](get-pb-paywalls) oluşturabilirsiniz. Onboarding'ler WebView içinde çalışırken, flow'lar cihazda doğrudan native olarak render edilir — bu sayede daha akıcı animasyonlar, tutarlı bir iOS görünümü, daha hızlı yükleme süreleri ve WebView runtime bağımlılığından kurtulma gibi avantajlar elde edersiniz. Başlamak için [Flow'ları ve paywall'ları getir](get-pb-paywalls) ve [Flow'ları ve paywall'ları göster](ios-present-paywalls) sayfalarına göz atın.
:::
