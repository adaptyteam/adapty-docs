---
title: "Flow'ları ve paywall'ları görüntüleme - Capacitor"
description: "Capacitor uygulamanızda Adapty ile kullanıcılara flow'ları ve paywall'ları sunun."
metadataTitle: "Capacitor'da Flow'ları ve Paywall'ları Görüntüleme | Adapty Dokümanları"
---
...
1. Bir [flow](paywall-builder-templates) veya [paywall](create-paywall) oluşturdunuz.
2. Bunu bir [placement](placements)'a eklediniz.
3. [Flow'u getirdiniz ve view'i hazırladınız](capacitor-get-pb-paywalls).
...
- **Remote config paywall'ları** sunmak için bkz. [Remote config ile tasarlanan paywall'ı render etme](present-remote-config-paywalls-capacitor).
...
:::warning
Bu kılavuz yalnızca eski Paywall Builder ile oluşturulan paywall'lar içindir. Remote config paywall'ların sunulması farklı bir süreç gerektirir. **Remote config paywall'ları** sunmak için bkz. [Remote config ile tasarlanan paywall'ı render etme](present-remote-config-paywalls-capacitor).
:::
