...
3. `show_paywall` adıyla ve `true` değeriyle bir alan oluşturun. **JSON** görünümünde bu giriş şöyle görünür:
...
Flow/Paywall B için de aynı adımları tekrarlayın; ancak `show_paywall` değerini `false` olarak ayarlayın.
...
:::important
Eğer paywall'ı kendiniz render ediyorsanız (Adapty onu render etmeden), Flow/Paywall A'yı görüntülediğinizde [`logShowFlow` (iOS SDK v4+) / `logShowPaywall`](present-remote-config-paywalls#track-paywall-view-events) çağrısı yapın. Bu olmadan Adapty, testteki görüntülemeleri sayamaz. Flow/Paywall B için görüntüleme kaydı yapmayın; zira hiçbir zaman gösterilmez.
:::
