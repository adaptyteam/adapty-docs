...
Adapty, bir kullanıcının yerel ayarına uyan lokalizasyonu ararken şu adımlar izlenir:
...
## Lokalizasyonları uygulamak \{#implementing-localizations\}

SDK v4'te flow çekerken bir yerel ayar kodu iletmezsiniz; `getFlow` tüm lokalizasyonlarıyla birlikte flow'u döndürür ve Adapty, flow görünümü oluşturulurken uygun olanı uygular.

- **Builder'da oluşturulan flow'lar**: SDK cihaz dilini okumaz, bu yüzden uygulamanızda çözümleyip `createFlowView` fonksiyonunun `locale` parametresi olarak geçin. Bu parametre isteğe bağlıdır — atarsanız flow `en` dilinde gösterilir ya da flow'da `en` yerelleştirmesi yoksa [varsayılan dil](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) kullanılır. Flow'un sahip olmadığı bir yerelleştirme istediğinizde, görünüm hata vermeden flow varsayılanına geri döner; seçilen yerelleştirmede eksik olan dizeler de varsayılandan gelir.
