...
Kod hiçbir lokalizasyonla eşleşmediğinde, flow sessizce [varsayılan locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) ile görüntülenir — SDK hata döndürmez ve uyarı kaydetmez.

Kod eşleştiğinde, Adapty eşleşen lokalizasyonu varsayılan olanla birleştirir: eşleşen lokalizasyonda tanımlanmayan string'ler ve asset'ler varsayılan lokalizasyondan gelir.

Locale kodunu atlamak, flow'un varsayılan yerelleştirmesini istemekle aynı şey değildir: SDK sabit olarak `en` kullanır. Varsayılan dili `de` olan bir flow, `en` yerelleştirmesi varsa yine `en` olarak görüntülenir; yalnızca `en` yerelleştirmesi yoksa `de`'ye geri döner.

:::warning
Yerel ayar kodunu tam olarak kontrol panelinde yapılandırıldığı şekilde iletin — küçük harfli alt etiketler kısa çizgiyle ayrılmış olmalıdır. Platform yerel ayar tanımlayıcısını olduğu gibi iletmeyin: Android'de `Locale.getDefault().toLanguageTag()` `pt-BR` döndürür; iOS'ta `NSLocale.currentLocale.localeIdentifier` `pt_BR` döndürür. Her ikisi de varsayılan yerelleştirmeye geri döner. Değeri uygulamanızda iletmeden önce dönüştürün.
:::
...
SDK v4'te bir flow getirirken locale kodu geçirmezsiniz — `getFlow`, flow'u tüm lokalizasyonlarıyla birlikte döndürür ve Adapty, flow görünümü oluşturulurken bunlardan birini uygular.

- **Adapty tarafından render edilen flow'lar ve paywall'lar**: SDK cihaz locale'ini okumaz, bu nedenle uygulamanızda bunu çözümleyip `createFlowView` fonksiyonunun `locale` parametresi olarak geçirin. Bu parametre isteğe bağlıdır — atlanırsa flow `en` dilinde render edilir ya da flow'un `en` lokalizasyonu yoksa [varsayılan locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)'iyle render edilir.
