...
description: "Müşteri elde tutmayı iyileştirmek için Adapty'de kullanıcı abonelik durumunu takip edin ve yönetin."
metadataTitle: "Abonelik Durumunu Anlama | Adapty Dokümanları"
keywords: ['getProfile', 'cache']
---

Adapty ile abonelik durumunu takip etmek son derece kolaydır. Ürün ID'lerini kodunuza manuel olarak eklemeniz gerekmez. Bunun yerine, aktif bir [access level](access-level) olup olmadığını kontrol ederek kullanıcının abonelik durumunu zahmetsizce doğrulayabilirsiniz.

Abonelik durumunu kontrol etmeye başlamadan önce [App Store Server Notifications](enable-app-store-server-notifications) kurulumunu yapın.
