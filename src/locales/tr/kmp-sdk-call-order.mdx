...
| 2a   | `Adapty.activate(configuration = AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                | Uygulama başlangıcında, 1. adımdan sonra, customer user ID'niz varsa                             | Önerilen. Anonim profil oluşturulmaz.                                                             |
| 2b   | `Adapty.activate(configuration = AdaptyConfig.Builder("KEY").build())` `withCustomerUserId` olmadan           | Uygulama başlangıcında, 1. adımdan sonra, customer user ID'niz yoksa (veya hiç toplamıyorsanız) | Adapty anonim bir profil oluşturur.                                                           |
| 3    | Her MMP için `Adapty.setIntegrationIdentifier("appsflyer_id", uid)`                                           | 2. adımdan sonra, herhangi bir kullanıcı işlemi çağrısından önce                              | MMP ID'lerinin doğru profile düşmesi için gereklidir.                                               |
| 4    | `Adapty.identify("YOUR_USER_ID").onSuccess { ... }.onError { ... }`                                           | 3. adımdan sonra (veya MMP yoksa 2. adımdan sonra), 5. adımdan önce — yalnızca kimlik doğrulamayla 2b yolunda | Herhangi bir kullanıcı işlemi çağrısından önce `onSuccess` bekleyin. `identify` sırasında yapılan eş zamanlı çağrılar anonim profile düşebilir. |
