...
description: "Cursor, Context7, ChatGPT, Claude veya diğer AI araçlarını kullanarak Adapty'yi Android uygulamanıza adım adım entegre etme kılavuzu."
metadataTitle: "Android için AI Yardımıyla Adapty Entegrasyonu | Adapty Dokümanları"
displayed_sidebar: sdkandroid
---

Bu kılavuz, bir yapay zeka kodlama aracıyla Adapty'yi Android uygulamanıza adım adım entegre etmenizi sağlar; doğru Adapty dokümanlarını doğru sırayla besleyerek ilerlersiniz.

<SkillPromo />

## Başlamadan önce: kontrol paneli kurulumu \{#before-you-start-dashboard-setup\}

Adapty, herhangi bir SDK kodu yazmadan önce kontrol panelinde bazı yapılandırmalar gerektirir. Bunu etkileşimli bir LLM skill'ini kullanarak ya da Kontrol Paneli üzerinden manuel olarak yapabilirsiniz.

### Skill yaklaşımı (önerilen) \{#skill-approach-recommended\}

Adapty CLI skill'i, LLM'inizin uygulamanızı, ürünlerinizi, access level'larınızı, paywall'larınızı ve placement'larınızı doğrudan Kontrol Panelini açmadan ayarlamasını sağlar. Yalnızca [mağazanızı bağlamanız](integrate-payments) Kontrol Paneli üzerinden yapılması gerekir.

```
npx skills add adaptyteam/adapty-cli --skill adapty-cli
```

Skill eklendikten sonra, agent'ınızda `/adapty-cli` komutunu çalıştırın. Her adımda — mağazanızı bağlamak için Kontrol Panelini açmanız gereken anlar dahil — sizi yönlendirecektir.
...
3. **En az bir ürün oluşturun**: Adapty Kontrol Paneli'nde **Products** sayfasına gidin. Ürünlere kod içinde doğrudan başvurmazsınız — Adapty bunları paywall'lar aracılığıyla iletir.
   [Ürün ekle](quickstart-products)

4. **Bir flow veya paywall ve placement oluşturun**: Adapty Kontrol Paneli'nde bir flow (ya da kullanıcı arayüzünü kendiniz oluşturacaksanız bir paywall) oluşturun, ardından bunu **Placements** sayfasından bir placement'a atayın. Kodda, placement ID'si `Adapty.getFlow("YOUR_PLACEMENT_ID")` fonksiyonuna geçirdiğiniz string'dir.
...
- **Analitik entegrasyonlar**: **Integrations** sayfasında yapılandırın. Kurulum entegrasyona göre değişir. Bkz. [analitik entegrasyonlar](analytics-integration) ve [attribution entegrasyonlar](attribution-integration).
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Flow'ları Adapty'nin no-code builder'ında oluşturursunuz; SDK bunları otomatik olarak render eder.
- [**Manuel olarak oluşturulan paywalllar**](android-making-purchases): Kendi paywall arayüzünüzü kodla oluşturursunuz ancak ürünleri çekmek ve satın almaları yönetmek için yine Adapty'yi kullanırsınız.
...
Hangisini seçeceğinizden emin değil misiniz? [Hızlı başlangıçtaki karşılaştırma tablosuna](android-quickstart-paywalls) göz atın.
...
Placement ID'ye göre bir paywall getir, görüntüle ve satın alma etkinliklerini yönet. İhtiyacınız olan kılavuzlar, satın alma işlemlerini nasıl yönettiğinize bağlıdır.

Her satın almayı ilerledikçe sandbox ortamında test edin — sona kadar beklemeyin. Kurulum talimatları için [Sandbox'ta satın alma testi](test-purchases-in-sandbox) bölümüne bakın.
...
**Kılavuzlar:**
- [Flow & Paywall Builder ile satın almaları etkinleştirme (hızlı başlangıç)](android-quickstart-paywalls)
- [Flow'ları ve yapılandırmalarını getirme](android-get-pb-paywalls)
- [Flow'ları görüntüleme](android-present-paywalls)
- [Flow olaylarını yönetme](android-handling-events)
- [Düğme eylemlerine yanıt verme](android-handle-paywall-actions)
...
- **Dikkat:** Boş flow veya `getFlow` hatası → placement ID'nin kontrol panelindeki ile tam olarak eşleştiğini ve placement'a bir kitle atandığını doğrulayın.
:::
