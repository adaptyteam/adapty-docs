...
| **locale**      | isteğe bağlı | Görünümün hangi [flow yerelleştirmesiyle](add-paywall-locale-in-adapty-paywall-builder) render edileceğini belirten tanımlayıcı; `-` ile ayrılmış bir veya iki alt etiket içeren dil kodu olarak beklenir (ör. `en`, `pt-br`). Belirtilmezse görünüm `en` dilinde; flow'un `en` yerelleştirmesi yoksa flow'un varsayılan yerelleştirmesinde render edilir. Bkz. [Yerelleştirmeler ve yerel ayar kodları](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>isteğe bağlı</p><p>varsayılan: `null`</p><p>SDK 4.1+</p> | Flow Builder'da tanımlanan bir layout'un özel kimliği. Bunu, SDK'nın cihaz türü ve ekran boyutuna göre otomatik seçtiği layout yerine belirtilen layout'u render etmek için kullanın. Kimlikle eşleşen bir layout bulunamazsa flow, görünüm yapılandırması olmadan yüklenir. |
| **loadTimeout** | varsayılan: 5 sn | Bu parametre, metodun zaman aşımı süresini sınırlar. Zaman aşımına ulaşılırsa önbelleğe alınmış veriler veya yerel yedek döndürülür. Nadir durumlarda bu metot, arka planda farklı isteklerden oluşabileceğinden `loadTimeout`'ta belirtilenden biraz daha geç zaman aşımına uğrayabilir. |
...
| :-------------- | :------------- | :----------------------------------------------------------- |
| **flow**        | zorunlu        | `Adapty.getFlow` aracılığıyla elde edilen bir `AdaptyFlow` nesnesi. |
| **locale**      | isteğe bağlı | Görünümün hangi [flow yerelleştirmesiyle](add-paywall-locale-in-adapty-paywall-builder) render edileceğini belirten tanımlayıcı; `-` ile ayrılmış bir veya iki alt etiket içeren dil kodu olarak beklenir (örn. `en`, `pt-br`). Belirtilmezse görünüm `en` dilinde ya da flow'un varsayılan yerelleştirmesinde (`en` yoksa) render edilir. Bkz. [Yerelleştirmeler ve locale kodları](android-localizations-and-locale-codes). |
| **customLayoutId** | <p>isteğe bağlı</p><p>varsayılan: `null`</p><p>SDK 4.1+</p> | Flow Builder'da tanımlanan bir düzene ait özel kimlik. Bunu geçerek SDK'nın cihaz türü ve ekran boyutuna göre otomatik seçmesi yerine belirli bir düzenin render edilmesini sağlayabilirsiniz. Kimlikle eşleşen düzen bulunamazsa flow, görünüm yapılandırması olmadan yüklenir. |
| **loadTimeout** | varsayılan: 5 sn | Bu değer, metodun zaman aşımı süresini sınırlar. Zaman aşımına ulaşılırsa önbelleğe alınmış veriler veya yerel yedek döndürülür. Nadir durumlarda bu metod, arka planda farklı isteklerden oluşabileceğinden `loadTimeout`'ta belirtilenden biraz daha geç zaman aşımına uğrayabilir. |
