...
| **locale**      | isteğe bağlı | Görünümü render etmek için kullanılacak [flow yerelleştirmesinin](add-paywall-locale-in-adapty-paywall-builder) tanımlayıcısı; `-` ile ayrılmış bir veya iki alt etiketten oluşan dil kodu biçiminde beklenir (örneğin `en`, `pt-br`). Belirtilmezse görünüm `en` dilinde ya da flow'un `en` yerelleştirmesi yoksa flow'un varsayılan yerelleştirmesinde render edilir. Bkz. [Yerelleştirmeler ve yerel ayar kodları](android-localizations-and-locale-codes). |
| **loadTimeout** | varsayılan: 5 sn | Bu değer, yöntem için zaman aşımı süresini sınırlar. Zaman aşımına ulaşılırsa önbelleğe alınmış veriler veya yerel yedek döndürülür. Arka planda farklı isteklerden oluşabileceğinden, bu yöntemin zaman aşımının nadir durumlarda `loadTimeout`'ta belirtilenden biraz daha geç gerçekleşebileceğini unutmayın. |
...
| :-------------- | :------------- | :----------------------------------------------------------- |
| **flow**        | zorunlu       | `Adapty.getFlow` aracılığıyla elde edilen bir `AdaptyFlow` nesnesi. |
| **locale**      | isteğe bağlı | Görünümün oluşturulacağı [flow yerelleştirmesinin](add-paywall-locale-in-adapty-paywall-builder) tanımlayıcısı; `-` ile ayrılmış bir veya iki alt etiket içeren dil kodu olarak beklenir (örn. `en`, `pt-br`). Belirtilmezse görünüm `en` dilinde oluşturulur ya da flow'un `en` yerelleştirmesi yoksa flow'un varsayılan yerelleştirmesinde oluşturulur. Bkz. [Yerelleştirmeler ve yerel ayar kodları](android-localizations-and-locale-codes). |
