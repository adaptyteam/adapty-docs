...
2. `openWebPaywall(paywall)` — URL'leri paywall'a göre oluşturur ancak ürün verilerini URL'lere eklemez. SDK v4'te bu, alınan flow'un bir paywall varyasyonu olan `AdaptyFlowPaywall` alır. Dizine erişmeden önce `flow.paywalls` dizisinin boş olmadığını kontrol edin; örneğin `flow.paywalls[0]`.
:::
