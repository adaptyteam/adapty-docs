...
Adapty, herhangi bir SDK kodu yazmadan önce biraz kontrol paneli yapılandırması gerektirir. Bunu etkileşimli bir LLM skill'ini kullanarak ya da Kontrol Paneli üzerinden manuel olarak yapabilirsiniz.

### Skill yaklaşımı (önerilir) \{#skill-approach-recommended\}

Adapty CLI skill'i, LLM'inizin uygulamanızı, ürünlerinizi, access level'larınızı, paywall'larınızı ve placement'larınızı doğrudan — her adım için Kontrol Paneli'ni açmadan — kurmasına olanak tanır. Yalnızca Kontrol Paneli'nde [mağazalarınızı bağlamanız](integrate-payments) gerekir.

```
npx skills add adaptyteam/adapty-cli --skill adapty-cli
```

Skill eklendikten sonra, ajanınızda `/adapty-cli` komutunu çalıştırın. Mağazalarınızı bağlamak için Kontrol Paneli'ni ne zaman açacağınız dahil her adımda sizi yönlendirecektir.
...
4. **Bir flow veya paywall oluşturun ve bir placement'a atayın**: Adapty Kontrol Paneli'nde bir flow (ya da UI'ı kendiniz oluşturacaksanız bir paywall) oluşturun, ardından **Placements** sayfasında bir placement'a atayın. Kodda, placement ID'si `Adapty().getFlow()` metoduna geçirdiğiniz string değeridir.
...
Kodlamaya başlamak için bunlar zorunlu değildir, ancak entegrasyonunuz olgunlaştıkça bunlara ihtiyaç duyacaksınız:
...
- [**Adapty Flow & Paywall Builder**](adapty-flow-builder): Flow'larınızı Adapty'nin no-code builder'ında oluşturursunuz; SDK bunları otomatik olarak render eder.
- [**Manuel oluşturulan paywalllar**](flutter-making-purchases): Kendi paywall arayüzünüzü kodla oluşturursunuz, ancak ürünleri almak ve satın almaları yönetmek için Adapty'yi kullanırsınız.
...
### Paywallları göster ve satın alma işlemlerini yönet \{#show-paywalls-and-handle-purchases\}

Paywall'ı placement ID'siyle çek, görüntüle ve satın alma olaylarını yönet. İhtiyaç duyduğun kılavuzlar, satın alma işlemlerini nasıl ele aldığına bağlıdır.

Her satın almayı ilerlerken sandbox'ta test et — sona kadar bekleme. Kurulum talimatları için bkz. [Sandbox'ta satın alma testi](test-purchases-in-sandbox).
...
**Kılavuzlar:**
- [Flow & Paywall Builder ile satın almaları etkinleştirin (hızlı başlangıç)](flutter-quickstart-paywalls)
- [Flow'ları ve yapılandırmalarını çekin](flutter-get-pb-paywalls)
- [Flow'ları görüntüleyin](flutter-present-paywalls)
- [Flow olaylarını yönetin](flutter-handling-events)
