...
Bu sayfa, Adapty'yi iOS uygulamanıza entegre etmenin iki yolunu kapsar. Otomatik uçtan uca bir akış için aşağıdaki SDK entegrasyon rehberini kullanın ya da daha aşağıdaki adım adım manuel açıklamayı takip edin.
## SDK entegrasyon becerisini kullanın (beta) \{#use-the-sdk-integration-skill-beta\}

[adapty-sdk-integration skill](https://github.com/adaptyteam/adapty-sdk-integration-skill), entegrasyonu uçtan uca otomatikleştirir: kontrol paneli kurulumu, SDK kurulumu, paywall ve aşama bazlı doğrulama. Aşağıdaki manuel adım adım anlatım, aracınız Claude Skills formatını desteklemiyorsa yedek olarak kullanılabilir.

**Desteklenen araçlar**: Claude Code, GitHub Copilot CLI, OpenAI Codex, Gemini CLI.
### Yükleme \{#install\}

Aracınıza uygun formu seçin. Tam liste [skill README](https://github.com/adaptyteam/adapty-sdk-integration-skill) içinde yer almaktadır.
- **Claude Code**: Shell üzerinden `claude plugin marketplace add adaptyteam/adapty-sdk-integration-skill` ardından `claude plugin install adapty-sdk-integration@adapty` komutunu çalıştırın.
...
Projenizde `/adapty-sdk-integration` komutunu çalıştırın. Skill, platformunuzu otomatik olarak algılar ve birkaç kurulum sorusu sorar. Ardından kontrol paneli kurulumu, SDK yükleme, paywall ve doğrulama aşamalarını adım adım yürütürken ilgili Adapty dokümanlarını getirir.

:::note
Skill beta aşamasındadır. Takılı kalır ya da beklenmedik davranışlar sergilerse, aşağıdaki manuel adım adım rehber her aşamayı kapsamlı biçimde ele almaktadır.
:::
