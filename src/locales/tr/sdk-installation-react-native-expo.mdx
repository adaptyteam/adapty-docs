...
iOS için derleme yapmak **Swift 6.0** veya daha yenisini gerektirir. [Kids Mode](kids-mode-react-native), **Swift 6.1** veya daha yenisini gerektirir.

:::info
Adapty React Native SDK 4.0.3 ve sonrası Google Play Billing Library v8 ile çalışır.

Bir Billing Library sürümüyle uyumluluk, Adapty'nin Google'ın o sürümde sunduğu her özelliği desteklediği anlamına gelmez. Yeni bir Google Play faturalandırma özelliğini kullanmadan önce [Play Store'daki Ürünler](android-products) sayfasına bakın.
:::
...
:::important
v4'ten itibaren, Adapty React Native SDK artık yerel bağımlılıklarının CocoaPods ile kurulumunu desteklememektedir. v4 veya sonrasına ihtiyacınız varsa ([Flow & Paywall Builder](adapty-flow-builder) için), bunun yerine aşağıdaki [Adapty SDK 4.0: Swift Package Manager'ı etkinleştirin](#adapty-sdk-40-enable-swift-package-manager) bölümünü takip edin.
:::
...
### Adapty SDK 4.0: Swift Package Manager'ı etkinleştirme \{#adapty-sdk-40-enable-swift-package-manager\}

React Native SDK 4.0 — [flow](adapty-flow-builder) desteği ekleyen bu sürüm — **React Native 0.75 veya üzeri** gerektirir. SDK'yı yükleyin:

```sh
npx expo install react-native-adapty@^4.0.0
```

v4, yerel iOS SDK'larını (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) CocoaPods alt bağımlılıkları yerine Swift Package Manager aracılığıyla çeker ([CocoaPods spec deposu Aralık 2026'da salt okunur hale geliyor](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). SPM dinamik framework'ler gerektirir; bunları Expo'da [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) eklentisiyle etkinleştirirsiniz. `app.json` (veya `app.config.js`) dosyasına ekleyin:
...
iOS derlemesi `'React/RCTBridge.h' file not found` hatasıyla başarısız olursa [sorun giderme kaydına](#react-header-not-found) bakın.

Tam migration için bkz. [Adapty React Native SDK'yı v4'e taşıma](migration-to-react-native-sdk-v4).
...
[Adapty SDK 4.0 için dinamik framework'ler etkinleştirildiğinde](#adapty-sdk-40-enable-swift-package-manager), iOS derlemesi `expo-updates`, `@expo/ui` veya Objective-C kaynaklarına sahip başka bir pakette başarısız olabilir:
