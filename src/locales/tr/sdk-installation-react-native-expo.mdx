...
`buildReactNativeFromSource`, **Expo SDK 57 ve sonrasında** zorunludur. Expo SDK 57, önceden derlenmiş bir React Native framework'ü ile birlikte gelir; frameworkler dinamik olduğunda diğer paketler bu framework'ün başlık dosyalarına erişemez ve iOS derlemesi `expo-updates` veya `@expo/ui` içinde `'React/RCTBridge.h' file not found` gibi hatalarla başarısız olur. React Native'i kaynaktan derlemek bu çakışmayı önler, ancak iOS derleme sürelerini uzatır. Expo SDK 56 ve öncesinde bu seçeneği atlayabilirsiniz.
...
Tam migration için [Adapty React Native SDK'yı v4'e Taşıma](migration-to-react-native-sdk-v4) bölümüne bakın.
