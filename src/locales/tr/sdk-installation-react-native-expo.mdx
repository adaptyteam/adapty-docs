...
:::important
Bir Expo projesinde Adapty'yi kullanmak için [Expo Dev Client](https://docs.expo.dev/versions/latest/sdk/dev-client/) (özel bir geliştirme derlemesi) gereklidir.

Expo Go özel yerel modülleri desteklemez, bu yüzden onu yalnızca UI/mantık geliştirmesi için [**mock modu**](#set-up-mock-mode-for-expo-go--expo-web) ile kullanabilirsiniz (gerçek satın almalar ve AdaptyUI/Paywall Builder render'ı olmadan).
:::
...
1. Adapty SDK'yı yükleyin (bu, `@adapty/core`'u da otomatik olarak yükler):
   ...
   npx expo prebuild
   ```
2. Uygulamanızı EAS veya yerel derleme kullanarak geliştirme için derleyin:
