...
description: "Abonelik tabanlı uygulamalar için Adapty React Native SDK'yı bir Expo projesine adım adım yükleme rehberi."
...
:::important
Bu kılavuz, Adapty React Native SDK'nın **bir Expo projesine** kurulumunu ve yapılandırılmasını ele almaktadır.
...
- **AdaptyUI**: Bu modül, flow'ları ve eski builder paywall'larını render eder. AdaptyUI, core modülüyle birlikte otomatik olarak etkinleştirilir.

React Native uygulamanızda IAP'yi nasıl uygulayacağınıza dair eksiksiz bir eğitim için [şuraya](https://adapty.io/blog/react-native-in-app-purchases-tutorial/) göz atabilirsiniz.

:::tip
Adapty SDK'nın bir Expo uygulamasına nasıl entegre edildiğine dair gerçek dünya örneği görmek ister misiniz? Örnek uygulamalarımıza göz atın:
...
- Mock mod ile test etmek için [Expo Go & Web örneği](https://github.com/adaptyteam/AdaptySDK-React-Native/tree/master/examples/ExpoGoWebMock)
:::
...
3.17.0 sürümünden itibaren Adapty React Native SDK, varsayılan olarak Billing Library v8'i kullanır. 3.15.0 ile 3.15.6 arasındaki sürümler v7'yi kullanır; ayrıca Android derlemenizde başka bir bağımlılık Billing Library'yi v8'e yükseltirse de bu sürümler sorunsuz çalışır.

Google Play Billing Library'nin belirli bir sürümüyle uyumlu olmak, Adapty'nin o sürümde Google'ın sunduğu her özelliği desteklediği anlamına gelmez. Yeni bir Google Play billing özelliğini benimsemeden önce [Play Store'da Ürün](android-products) sayfasına göz atın.
:::
...
:::important
Adapty React Native SDK v4 ve sonrası, yerel iOS SDK'larını (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) Swift paketi olarak sunar. CocoaPods, Expo projesine hâlâ yükleyebilir; ancak Swift paketleri dinamik olarak bağlandığından derlemenizin dinamik framework'lere ihtiyacı vardır — bkz. [Adapty SDK v4 için dinamik framework'leri etkinleştirin](#enable-dynamic-frameworks).
:::
...
:::important
[Expo Dev Client](https://docs.expo.dev/versions/latest/sdk/dev-client/) (özel bir geliştirme derlemesi), Adapty'yi bir Expo projesinde kullanmak için gereklidir.

Expo Go özel native modülleri desteklemediğinden, yalnızca UI/mantık geliştirme amacıyla [**mock modunda**](#set-up-mock-mode-for-expo-go--expo-web) kullanabilirsiniz (gerçek satın alma işlemi ve AdaptyUI/Paywall Builder render'ı yoktur).
:::
...
2. EAS veya yerel derleme kullanarak uygulamanızı geliştirme için derleyin:
   ...
   <Tabs>
   <TabItem value="eas" label="EAS derlemesi" default>
      ```sh
      # For iOS
...
### Adapty SDK v4 için dinamik framework'leri etkinleştirin \{#enable-dynamic-frameworks\}

Flow desteği ekleyen React Native SDK 4.0, **React Native 0.75 veya üzeri** gerektirir. SDK'yı yükleyin:

```sh
npx expo install react-native-adapty@^4.0.0
```

v4, yerel iOS SDK'larını (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) CocoaPods alt bağımlılıkları yerine Swift paketi olarak çeker ([CocoaPods'un spec deposu Aralık 2026'da salt okunur hale geliyor](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Swift paketleri dinamik olarak bağlanır; bunu Expo'da [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) eklentisiyle etkinleştirirsiniz. `app.json` (veya `app.config.js`) dosyanıza ekleyin:
...
iOS derlemesi `'React/RCTBridge.h' file not found` hatasıyla başarısız olursa, [sorun giderme bölümüne](#react-header-not-found) bakın.

Tam migration için [Adapty React Native SDK'yı v4'e geçirin](migration-to-react-native-sdk-v4) sayfasına bakın.
...
- Kendi paywall UI'ınızı oluşturuyorsanız, [özel paywall'lar için hızlı başlangıç kılavuzuna](react-native-quickstart-manual) bakın.
...
[Flow & Paywall Builder](adapty-flow-builder) kullanmayı planlıyorsanız AdaptyUI modülüne ihtiyacınız vardır. Bu modül, çekirdek modülü etkinleştirdiğinizde otomatik olarak etkinleşir; başka bir şey yapmanıza gerek yoktur.
...
[Adapty Attribution](user-acquisition) kullanıyorsanız, SDK'yı etkinleştirirken `adaptyAttributionEnabled` değerini `true` olarak ayarlayın. Varsayılan değer `false`'tur: bu parametre olmadan SDK, kurulumları kaydetmez veya kurulum ayrıntılarını uygulamanıza iletmez. SDK 4.1'in altındaki sürümlerde Adapty Attribution otomatik olarak etkinleştirilir.
...
- Flow'ları veya eski builder paywalllarını (AdaptyUI) **render etmez**.
...
Gerçek satın almaları ve Paywall Builder paywalllarını test etmek için, mock modun otomatik olarak devre dışı bırakıldığı bir Expo Dev Client / production build kullanın.
::::
...
:::note
Bu seçenek etkinleştirildiğinde, SDK zaten aktive edilmişse tüm aktivasyon çağrısını atlar; bu nedenle aktivasyon parametrelerindeki değişiklikler hızlı yenileme sırasında geçerli olmaz. Yeni aktivasyon parametrelerini uygulamak için uygulamayı tamamen kapatıp yeniden başlatın.
:::
...
iOS için derleme yaparken, **minimum iOS sürümü** veya dağıtım hedefiyle ilgili bir hata görebilirsiniz. Adapty **iOS 15.0+** gerektirir.

Expo, iOS projesini (`Podfile` dahil) `expo prebuild` sırasında oluşturduğundan, **`Podfile`'ı doğrudan düzenlememelisiniz**. Bunun yerine, dağıtım hedefini `expo-build-properties` yapılandırma eklentisi aracılığıyla ayarlayın.
...
1. Eklentiyi yükleyin:
   ...
   npx expo install expo-build-properties
   ```
2. iOS dağıtım hedefini ayarlamak için Expo yapılandırmanızı (`app.json` veya `app.config.js`) güncelleyin:
...
Android Auto Backup'ı yapılandıran birden fazla SDK (Adapty, AppsFlyer veya expo-secure-store gibi) ile Expo kullanırken, bir manifest birleştirme çakışmasıyla karşılaşabilirsiniz.
...
#### iOS derlemesi `'React/RCTBridge.h' file not found` hatasıyla başarısız oluyor \{#react-header-not-found\}

[Adapty SDK v4 için dinamik framework'ler etkinleştirildiğinde](#enable-dynamic-frameworks), iOS derlemesi `expo-updates`, `@expo/ui` veya Objective-C kaynaklarına sahip başka bir pakette başarısız olabilir:

```
error: 'React/RCTBridge.h' file not found
```

`expo-modules-autolinking` paketindeki (bir `expo` bağımlılığı) bir hata bu soruna yol açmaktadır. Etkilenen sürümler 57.0.5–57.0.9 (Expo SDK 57) ve 56.0.19–56.0.21 (Expo SDK 56)'dır; hata 57.0.10 ve 56.0.22'de giderilmiştir.

Derlemeyi düzeltmek için `expo-modules-autolinking`'i düzeltilmiş sürüme güncelleyin ve native projeyi yeniden oluşturun:
...
Expo SDK'nız için düzeltilmiş sürüm henüz mevcut değilse, bunun yerine React Native'i kaynaktan derleyin. Bu yöntem, iOS derlemelerinin daha uzun sürmesi pahasına hatayı aşmanızı sağlar:
