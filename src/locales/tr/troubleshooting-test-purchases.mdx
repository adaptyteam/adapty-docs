...
## Sandbox satın alma işlemi yetkisiz olarak reddedildi \{#the-sandbox-purchase-is-refused-as-unauthorized\}

Mağaza, `You are not authorized to make purchases of this in-app in sandbox at this time` gibi bir uyarı gösteriyor. Satın almayı reddeden Adapty değil, Apple'dır. Test cihazında ve App Store Connect'te aşağıdakileri doğrulayın:

- **Cihazda uygulama içi satın almalar etkin**: **Ayarlar** > **Ekran Süresi** > **Uygulamalar ve Web Siteleri** > **App Store** bölümünde **Uygulama İçi Satın Almalar** seçeneği **İzin Verildi** olarak ayarlı olmalıdır. Apple bu ayarı [Turn off Apple In-App Purchases from Screen Time](https://support.apple.com/en-us/102470) sayfasında açıklamaktadır.
- **Sandbox hesabı doğru alana giriş yapmış**: [**Ayarlar** > **Geliştirici** > **Sandbox Apple Hesabı**](test-purchases-in-sandbox#step-4-switch-to-sandbox-test-account) üzerinden giriş yapın; ana Apple hesabınızdan değil. Apple'ın talimatı, "Ayarlar'daki Sandbox giriş seçeneğini kullanın, ana Apple Hesabı ayarlarınızı değil" şeklindedir.
- **Test kullanıcısının e-posta adresi mevcut bir Apple Hesabı değil**: Apple, mevcut bir hesabın yeniden kullanılmasının "test sırasında hatalara yol açacağını" belirtmektedir. Bir test kullanıcısının e-posta adresini oluşturulduktan sonra düzenleyemezsiniz; bu nedenle farklı bir adresle [yeni bir test kullanıcısı oluşturun](test-purchases-in-sandbox#step-1-create-sandbox-test-account-in-app-store-connect).
