---
title: "Adapty Android SDK'yı v3.3'e Taşıma"
description: "Daha iyi performans ve yeni para kazanma özellikleri için Adapty Android SDK v3.3'e geçiş yapın."
...
1. Paywall Builder ile oluşturulmayan paywall'larda satın alma işlemlerini nasıl ele aldığınızı güncelleyin. `USER_CANCELED` ve `PENDING_PURCHASE` hata kodlarını işlemeyi bırakın. İptal edilen bir satın alma artık hata olarak kabul edilmez ve hata olmayan satın alma sonuçlarında görünecektir.
2. Paywall Builder ile oluşturulan paywall'lar için `onPurchaseCanceled` ve `onPurchaseSuccess` olaylarını yeni `onPurchaseFinished` olayıyla değiştirin. Bu değişiklik aynı nedenle yapılmaktadır: iptal edilen satın almalar artık hata olarak değerlendirilmez ve hata olmayan satın alma sonuçlarına dahil edilir.
3. Paywall Builder paywallları için `onAwaitingSubscriptionUpdateParams` metot imzasını değiştirin.
4. Dosya URI'sini doğrudan geçiriyorsanız yedek paywall sağlamak için kullanılan metodu güncelleyin.
...
Tam kod örneği için [Mobil uygulamada satın alma yapma](android-making-purchases#make-purchase) sayfasına göz atın.
## Paywall Builder satın alma olaylarını değiştirme \{#modify-paywall-builder-purchase-events\}

1. `onPurchaseFinished` olayı ekleyin:
...
Tam kod örneği için [Başarılı, iptal edilen veya beklemedeki satın alma](android-handling-events#successful-canceled-or-pending-purchase) bölümüne ve olay açıklamasına bakın.
...
3. `onPurchaseSuccess` ifadesini kaldırın:
   ```diff showLineNumbers
...
[Yükseltme aboneliği](android-handling-events#upgrade-subscription) doküman bölümüne bakarak son kod örneğini inceleyebilirsiniz.
## Yedek paywall'ları güncelleme \{#update-providing-fallback-paywalls\}
...
```diff showLineNumbers
Uri fileUri = // Yedek paywall'lar için dosyanın URI'sını al
- Adapty.setFallbackPaywalls(fileUri, callback);
+ Adapty.setFallbackPaywalls(FileLocation.fromFileUri(fileUri), callback);
```
...
Mobil uygulama kodunuzu aşağıda gösterildiği şekilde güncelleyin. Tam kod örneği için [Branch entegrasyonu için SDK yapılandırması](branch#connect-your-app-to-branch) bölümüne bakın.
```diff showLineNumbers
...
Mobil uygulama kodunuzu aşağıda gösterildiği gibi güncelleyin. Tam kod örneği için [OneSignal entegrasyonunda SDK yapılandırması](onesignal#sdk-configuration) bölümüne bakın.
...
Mobil uygulama kodunuzu aşağıda gösterildiği gibi güncelleyin. Tam kod örneği için [Pushwoosh entegrasyonu için SDK yapılandırmasına](pushwoosh#sdk-configuration) bakın.
