---
title: "AppMetrica"
description: "Derinlemesine abonelik analitiği için AppMetrica'yı Adapty ile entegre edin."
...
[AppMetrica](https://appmetrica.yandex.com/about), kullanıcı davranışını takip etmenizi ve mobil uygulamanızın performansını gerçek zamanlı olarak analiz etmenizi sağlayan ücretsiz bir analitik aracıdır. AppMetrica'yı Adapty ile entegre ederek abonelik metrikleriniz ve kullanıcı etkileşiminiz hakkında daha derin içgörüler elde edebilirsiniz.
...
|:---------------------|:--------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `post_api_key`       | String  | AppMetrica Post API Anahtarınız.                                                                                                                                                 |
| `application_id`     | String  | AppMetrica Uygulama ID'niz.                                                                                                                                               |
| `revenue_event_type` | String  | Gelir etkinliğinin türü (örn. "subscription_renewed", "refund", "intro_started"). Bkz. [AppMetrica etkinlik eşlemesi](#events-and-tags).                             |
| `price`              | Float   | Gelir tutarı (gelir hesaplama ayarlarınıza göre).                                                                                                                                  |
| `currency`           | String  | Para birimi kodu (örn. "USD").                                                                                                                                                  |
| `product_id`         | String  | Mağazadaki Ürün ID'si.                                                                                                                                                |
| `quantity`           | Integer | Her zaman 1.                                                                                                                                                                     |
| `transaction_id`     | String  | Mağaza İşlem ID'si.                                                                                                                                                         |
| `payload`            | String  | Etkinlik ayrıntılarını içeren URL-encode edilmiş JSON dizesi. En kritik verileri korumak amacıyla isteğe bağlı alanlar önem sırasıyla kaldırılarak 30KB'ı aşması durumunda otomatik olarak kırpılır. |
| `os_name`            | String  | "ios" veya "android".                                                                                                                                                           |
| `profile_id`         | String  | AppMetrica Profil ID'si (ayarlanmışsa), aksi takdirde Müşteri Kullanıcı ID'si (varsa).                                                                                                    |
| `appmetrica_device_id` | String  | AppMetrica Cihaz ID Hash'i. Yalnızca `profile_id` mevcut değilse gönderilir.                                                                                                        |
| `session_type`       | String  | Her zaman "foreground".                                                                                                                                                          |
| `ios_ifa`            | String  | **Yalnızca iOS**. Reklamverenler için ID.                                                                                                                                             |
| `ios_ifv`            | String  | **Yalnızca iOS**. Satıcılar için ID.                                                                                                                                                 |
| `google_aid`         | String  | **Yalnızca Android**. Google Reklam ID'si.                                                                                                                                      |
