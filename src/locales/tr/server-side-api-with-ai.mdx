...
- **Oran sınırı**: API anahtarı başına uygulanır ve uygulama başına atanır; bu nedenle her uygulama için geçerli tek bir değer yoktur — bkz. [Oran sınırları](https://adapty.io/docs/tr/ss-authorization.md#rate-limits). Bu, dakika başına bir bütçe değil, sürekli bir orandır ve bazı endpoint'ler daha düşük sınırlara sahiptir: [createVirtualCurrencyTransaction](https://adapty.io/docs/tr/api-adapty/operations/createVirtualCurrencyTransaction.md) varsayılan olarak uygulama başına dakikada 600 isteğe izin verir. Agentınıza `429` durumunda geri çekilme ile yeniden deneme yapmasını ve mevcut olduğunda `Retry-After` başlığını dikkate almasını söyleyin.
- **Uygulamaya özel anahtarlar**: Her anahtar yalnızca bir uygulama için geçerlidir; uygulama başına doğru anahtarı kullanın.
- **Bir tanımlayıcı zorunludur**: Her istek için `adapty-customer-user-id` veya `adapty-profile-id` gereklidir.
