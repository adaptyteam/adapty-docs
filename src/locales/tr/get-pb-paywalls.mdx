...
| **locale**               | <p>isteğe bağlı</p><p>varsayılan: `nil`</p> | [Paywall yerelleştirmesinin](add-paywall-locale-in-adapty-paywall-builder) tanımlayıcısı. `-` ile ayrılmış bir veya iki alt etiket içeren dil kodu olarak beklenir (ör. `en`, `pt-br`). Bkz. [Yerelleştirmeler ve yerel ayar kodları](localizations-and-locale-codes). |
| **customLayoutId**       | <p>isteğe bağlı</p><p>varsayılan: `nil`</p><p>SDK 4.1+</p> | Flow Builder'da tanımlanan bir düzenin özel kimliği. SDK'nın cihaz türü ve ekran boyutuna göre otomatik olarak seçtiği düzen yerine belirtilen düzeni göstermek için kullanılır. Kimlikle eşleşen bir düzen bulunamazsa flow, görünüm yapılandırması olmadan yüklenir. |
| **loadTimeout**          | varsayılan: 5 sn | Bu değer, yöntem için zaman aşımı süresini sınırlar. Zaman aşımına ulaşılırsa önbelleğe alınan veriler veya yerel yedek döndürülür. Arka planda farklı isteklerden oluşabileceğinden, bu yöntem nadir durumlarda `loadTimeout`'ta belirtilenden biraz daha geç zaman aşımına uğrayabilir. |
...
| **systemRequestsHandler** | isteğe bağlı  | Flow eylemleriyle tetiklenen sistem izni ve değerlendirme isteklerini yöneten `AdaptySystemRequestsHandler` ile uyumlu bir nesne. Yalnızca flow'unuz bu tür eylemler içeriyorsa gereklidir. |
| **assetsResolver**       | isteğe bağlı   | Flow/paywall'daki görselleri ve videoları geçersiz kılan bir `[String: AdaptyCustomAsset]` sözlüğü. Bkz. [Varlıkları özelleştirme](#customize-assets). |
| **timerResolver**        | isteğe bağlı   | Geliştirici tanımlı zamanlayıcılar için bitiş tarihlerini sağlayan `AdaptyTimerResolver` ile uyumlu bir nesne. Bkz. [Geliştirici tanımlı zamanlayıcıları ayarlama](#set-up-developer-defined-timers). |
