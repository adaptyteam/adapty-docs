...
:::tip
`Adapty.activate()` ve `Adapty.identify()` çağrılarının tamamlandığı varsayılmaktadır. Bkz. [Kotlin Multiplatform SDK'da çağrı sırası](kmp-sdk-call-order).
:::

Aşağıdaki tavsiyeler v3 metod adlarını kullanmaktadır. SDK v4'te `getPaywall`, `getFlow` olarak yeniden adlandırılmıştır ([migration kılavuzuna](migration-to-kmp-sdk-v4) bakın) — tüm kurallar değişmeksizin geçerliliğini korur.
