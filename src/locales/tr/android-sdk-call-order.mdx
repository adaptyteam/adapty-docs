...
|------|---------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| 1    | MMP veya analitik SDK'nızı başlatın (AppsFlyer, Adjust, PostHog, Branch)                                     | Uygulama başlatma, ilk                                                                      | MMP'nin UID callback'ini bekleyin, örneğin `getAppsFlyerUID`.                                |
| 2a   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").withCustomerUserId(...).build())`                       | Uygulama başlatma, 1. adımdan sonra, eğer customer user ID'niz varsa                             | Önerilir. Anonim profil oluşturulmaz.                                                             |
| 2b   | `Adapty.activate(context, AdaptyConfig.Builder("KEY").build())` `customerUserId` olmadan                      | Uygulama başlatma, 1. adımdan sonra, eğer customer user ID'niz yoksa (veya hiç toplamıyorsanız) | Adapty anonim bir profil oluşturur.                                                           |
| 3    | `Adapty.setIntegrationIdentifier(AdaptyIntegrationIdentifier.appsflyerId(uid)) { error -> ... }`               | 2. adımdan sonra, herhangi bir kullanıcı eylemi çağrısından önce                                              | MMP ID'lerinin doğru profile iletilmesi için gereklidir. Birden fazla MMP ayarlamak için tek çağrıda bir kimlik listesi geçin. |
| 4    | `Adapty.identify("YOUR_USER_ID") { error -> ... }`                                                            | 3. adımdan sonra (veya MMP yoksa 2. adımdan sonra), 5. adımdan önce — yalnızca kimlik doğrulamalı 2b yolunda | Tamamlanma callback'ini kullanın. `identify` sırasında eş zamanlı çağrılar anonim profile düşebilir. |
...
Aynı oturumda özel özellikler ayarlamak için `updateProfile` çağırıp ardından bir placement çekecekseniz (`getPaywall`), fetch işleminden önce `updateProfile`'ın tamamlanmasını bekleyin. İkisini aynı anda çalıştırmak, SDK'nın henüz yayılmamış özelliklerden placement'ın segment hash'ini hesaplamasına yol açabilir ve bu durum fetch tamamlanmadan önce ekstra bir yeniden deneme turu eklenmesine neden olur. Yeniden deneme başarısız olursa [Android SDK'da hataları yönetme](android-sdk-error-handling) bölümüne bakın.
