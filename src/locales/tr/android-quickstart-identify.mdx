...
:::important
Müşteri kullanıcı ID'leri her kullanıcı için benzersiz olmalıdır. Parametre değerini sabit kodlarsanız, tüm kullanıcılar tek bir kullanıcı olarak değerlendirilir.
:::

Diğer SDK metodlarını çağırmadan önce `identify` tamamlanma callback'inin tetiklenmesini bekleyin. Eş zamanlı çağrılar, tanımlanmış profil yerine anonim profile düşebilir. Bkz. [Android SDK'da çağrı sırası](android-sdk-call-order).

<Tabs groupId="current-os" queryString>
<TabItem value="kotlin" label="Kotlin" default>
```kotlin showLineNumbers
Adapty.identify("YOUR_USER_ID") { error -> // Her kullanıcı için benzersiz
    if (error == null) {
        // başarılı identify
    }
}
