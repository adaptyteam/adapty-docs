...
Bu kılavuz, flow'lara geçtiğinizde nelerin değiştiğini ve bu değişikliği eski uygulama sürümlerindeki kullanıcıları etkilemeden nasıl kullanıma sunacağınızı açıklar.

:::important
Flow'lar şu anda yalnızca iOS ve Android SDK v4 ve üzeri sürümlerde desteklenmektedir. Diğer platform ve framework'ler için destek yakında eklenecektir.
:::
...
4. **[SDK'yı güncelleyin](#update-the-sdk)**: iOS veya Android SDK v4'e yükseltin, flow'u placement'ından alın ve sandbox satın alımını doğrulayın. Bu, geliştirici tarafındaki ana görevdir.
...
Flow placement'ınız hazır olduğuna göre, uygulamayı buna bağlayın. Flow'lar yalnızca Adapty SDK v4 ve üzerinde çalışır. SDK'yı yükseltin ve `getFlow` ile yeni placement'ınızdan flow'u çekin. Platforma özel yükseltme adımları için v4 geçiş kılavuzuna bakın — [iOS](migration-to-ios-sdk-v4) veya [Android](migration-to-android-sdk-v4).

Flow bağlandıktan sonra, diğer satın alma flow'larında olduğu gibi doğrulayın: bir cihazda veya simülatörde çalıştırın ve ürünlerin, satın almanın ve access level'ın düzgün çalıştığını onaylamak için sandbox satın alması yapın ([iOS](ios-test) / [Android](testing-on-android)).
:::note
