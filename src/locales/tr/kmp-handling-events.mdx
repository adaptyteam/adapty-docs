...
:::important
Bu kılavuz; satın almalar, yenileme işlemleri, ürün seçimi ve flow render işlemleri için event yönetimini kapsar. Ayrıca buton işleme (flow'u kapatma, linkleri açma vb.) işlemlerini de uygulamanız gerekir. Ayrıntılar için [flow aksiyonlarını yönetme kılavuzumuza](kmp-handle-paywall-actions) bakın.
:::

Adapty'nin render ettiği flow'lar ve paywall'lar, satın alma yapmak ve geri yüklemek için ekstra kod gerektirmez. Ancak, uygulamanızın yanıt verebileceği bazı olaylar üretirler. Bu olaylar; buton tıklamaları (kapat butonları, URL'ler, ürün seçimleri vb.) ve satın alma ile ilgili eylemlere dair bildirimleri kapsar. Bu olaylara nasıl yanıt vereceğinizi aşağıda öğrenebilirsiniz.
...
Kotlin Multiplatform, olay parametrelerini ham bir JSON dizesi olarak iletir; bu nedenle herhangi birini okumadan önce `paramsJsonString` değerini çözümleyin.

<FlowScreenShowedParams />

Bunlarla ne yapacağınızı öğrenmek için [Flow ekran görüntülemelerini takip etme](kmp-flow-screen-views) sayfasına bakın.

Bir flow aynı zamanda kullanıcıların yazdığı ve seçtiği değerleri de raporlar. Bunları okumak için [Flow'lardan gelen verileri işleme](kmp-flow-input) bölümüne bakın.
...
[Flow aksiyonlarını yönetme kılavuzuna](kmp-handle-paywall-actions) bakarak tam aksiyon listesini inceleyebilirsiniz.

</SDKv4>

<SDKv3>

[Paywall Builder](adapty-paywall-builder) ile yapılandırılan paywalllar, satın alma yapmak ve geri yüklemek için ekstra kod gerektirmez. Ancak uygulamanızın yanıt verebileceği bazı olaylar üretirler. Bu olaylar; buton tıklamalarını (kapat butonları, URL'ler, ürün seçimleri vb.) ve paywall üzerinde gerçekleştirilen satın alma ile ilgili aksiyon bildirimlerini içerir. Bu olaylara nasıl yanıt vereceğinizi aşağıda öğrenin.

:::warning
Bu kılavuz yalnızca **eski builder ile oluşturulan paywalllar** içindir.
:::
