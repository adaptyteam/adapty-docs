...
:::info
Adapty Android SDK 4.0.2 ve sonrasında, geri yüklenecek hiçbir şey olmasa bile `restorePurchases()` başarıyla tamamlanır: mevcut profili döndürür, bu yüzden içindeki access level durumunu kontrol edin. Daha eski SDK sürümlerinde, bu durumda yöntem `NO_PURCHASES_TO_RESTORE` hatasıyla başarısız olur.
:::

:::note
Google Play yalnızca hâlâ sahip olunan satın almaları raporlar; bu nedenle `restorePurchases()`, Adapty'yi entegre etmeden önce süresi dolmuş abonelikleri bulamaz. Bunları Adapty'ye eklemek için [geçmiş verilerinizi içe aktarın](importing-historical-data-to-adapty).
:::
