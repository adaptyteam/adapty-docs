...
1. `OpenWebPaywall(product)` — paywall'a göre URL'ler oluşturur ve ürün verilerini de URL'lere ekler.
2. `OpenWebPaywall(paywall)` — paywall'a göre URL'ler oluşturur, ancak ürün verilerini URL'lere eklemez. Adapty paywall'ındaki ürünler web paywall'ındakilerden farklıysa bunu kullanın.

SDK v4'te `paywall` argümanı bir `AdaptyFlowPaywall` alır — `flow.Paywalls` içindeki paywall varyantlarından biri. [Geçiş rehberine](migration-to-unity-sdk-v4) bakın.
:::
