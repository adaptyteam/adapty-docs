...
:::tip
Aşağıdaki kurallar `adapty.activate()` ve `adapty.identify()` çağrılarının zaten tamamlanmış olduğunu varsayar. Bkz. [Capacitor SDK'da çağrı sırası](capacitor-sdk-call-order).
:::

Aşağıdaki öneriler v3 metod adlarını kullanmaktadır. SDK v4'te `getPaywall`, `getFlow` olarak yeniden adlandırılmıştır ([migration kılavuzuna bakın](migration-to-capacitor-sdk-v4)) — her kural değişmeden geçerlidir.
