...
Adapty Attribution'ın Amazon S3 ile entegrasyonu, kullanıcı edinme kampanya verilerini güvenli bir şekilde tek bir merkezi konumda saklamanıza olanak tanır. Kampanya performans verilerinizi, attribution verilerinizi ve kullanıcı edinme etkinliklerinizi Amazon S3 bucket'ınıza .csv dosyaları olarak kaydedebilirsiniz.
Bu entegrasyonu kurmak için AWS Console ve Adapty Attribution kontrol panelinde birkaç basit adım izlemeniz gerekecek.

:::note
Adapty Attribution exportu her takvim günü için bir dosya seti dışa aktarır. Export zamanlayıcısı günde üç kez çalışır — **02:00**, **04:00** ve **06:00 UTC**. Her çalışmada, önceki günün export işi henüz oluşturulmamışsa oluşturulur ya da daha önceki bir çalışma başarısız olduysa yeniden denenir. Bu nedenle verileriniz genellikle 02:00 çalışmasından sonra gelir; sonraki çalışmalar veri eklemek için değil, hatalardan kurtarmak amacıyla oradadır.

Her dosya, önceki tam takvim gününe ait UTC olarak oluşturulan olayların verilerini içerecektir. Örneğin, 8 Mart'ta dışa aktarılan veriler, 7 Mart'ta UTC ile 00:00:00 ile 23:59:59 arasında oluşturulan tüm olayları içerecektir.
:::

:::warning
Zamanlanmış dışa aktarma yalnızca ücretli planda olan veya deneme sürecindeki şirketler için çalışır. Şirketiniz bu iki kategoriden birine girmiyorsa, zamanlayıcı işlemi atlar ve herhangi bir dosya yazmaz; entegrasyon kontrol panelinde etkin görünse bile. Bucket'ınız boşsa ve yapılandırma doğru görünüyorsa, önce plan durumunuzu kontrol edin.
:::
...
| `bundle_id`              | Uygulama paket tanımlayıcısı (ör. `com.example.app`) |
...
| `asa_org_id`             | Apple Search Ads kuruluş kimliği          |
...
| `asa_creative_set_name`  | ASA kreatif seti adı                      |
| `fbclid`                 | Facebook Tıklama Kimliği                  |
| `ttclid`                 | TikTok Tıklama Kimliği                    |
| `utm_source`             | UTM kaynak parametresi                    |
| `utm_medium`             | UTM ortam parametresi                     |
| `utm_campaign`           | UTM kampanya parametresi                  |
| `utm_term`               | UTM terim parametresi                     |
| `utm_content`            | UTM içerik parametresi                    |
| `click_time`             | Attribution'a bağlı tıklamanın Unix saniye cinsinden zaman damgası. Kuruluma atfedilen tıklama yoksa boş bırakılır. |