...
:::important
このガイドでは、購入・復元・プロダクト選択・フロー表示に関するイベント処理を説明します。ボタンのハンドリング（フローを閉じる、リンクを開くなど）も別途実装する必要があります。詳細は[フローアクションの処理に関するガイド](kmp-handle-paywall-actions)を参照してください。
:::
...
通常の状況では、このようなエラーは発生しないはずです。もし発生した場合は、お知らせください。
...
Kotlin Multiplatform はイベントパラメータを生の JSON 文字列として渡すため、値を読み取る前に `paramsJsonString` をデコードしてください。

<FlowScreenShowedParams />

詳細は [フロー画面ビューのトラッキング](kmp-flow-screen-views) を参照してください。

フローはユーザーが入力・選択した値もレポートします。詳細は[フローからのデータ処理](kmp-flow-input)を参照してください。
