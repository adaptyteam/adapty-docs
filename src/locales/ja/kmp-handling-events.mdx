...
AdaptyがレンダリングするフローとペイウォールはAdaptyがレンダリングするフローとペイウォールは、購入の実行や復元のために追加のコードは必要ありません。ただし、アプリが対応できるイベントをいくつか生成します。これらのイベントには、ボタン操作（閉じるボタン、URL、プロダクト選択など）や、購入関連アクションに関する通知が含まれます。これらのイベントへの対応方法については、以下をご覧ください。
...
[フローアクションの処理に関するガイド](kmp-handle-paywall-actions)で、アクションの完全なリストを確認できます。

</SDKv4>

<SDKv3>

[ペイウォールビルダー](adapty-paywall-builder)で設定されたペイウォールは、購入や復元のために追加のコードを必要としません。ただし、アプリが応答できるイベントが発生します。これらのイベントには、ボタン操作（閉じるボタン、URL、プロダクト選択など）や、ペイウォール上で行われた購入関連アクションの通知が含まれます。これらのイベントへの対応方法については、以下を参照してください。

:::warning
このガイドは**旧ビルダーで作成されたペイウォール**専用です。
:::
