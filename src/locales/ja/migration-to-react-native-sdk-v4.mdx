...
`buildReactNativeFromSource` は **Expo SDK 57 以降で必須**です。Expo SDK 57 はビルド済みの React Native フレームワークを同梱しており、フレームワークがダイナミックになるとそのヘッダーに他のパッケージがアクセスできなくなります。そのため、`expo-updates` や `@expo/ui` で `'React/RCTBridge.h' file not found` のようなエラーが発生して iOS ビルドが失敗します。React Native をソースからビルドすることでこの競合を回避できますが、iOS のビルド時間が長くなります。Expo SDK 56 以前ではこのオプションを省略できます。
...
以前に`Adapty`、`AdaptyUI`、または`AdaptyPlugin`をCocoaPodsのサブ依存関係として追加していた場合は、`Podfile`から`pod 'Adapty'`、`pod 'AdaptyUI'`、`pod 'AdaptyPlugin'`の明示的な行をあらかじめ削除してください。
