---
title: "Adapty React Native SDK を v4.0 へ移行する"
description: "ペイウォール API をフロー API に置き換えることで、Adapty React Native SDK v4.0 へ移行します。フロー API は、旧ビルダーのペイウォールに加えてフローもレンダリングします。"
metadataTitle: "Adapty React Native SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty React Native SDK 4.0 ではフローが導入され、それに伴いペイウォール API の名称が変更されました。新しい API はフローと連携しますが、旧ビルダーのペイウォールとも引き続き動作します。Adapty ダッシュボード側での設定変更は不要です。
