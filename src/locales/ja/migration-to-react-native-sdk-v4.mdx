...
description: "ペイウォール API をフロー API に置き換えることで Adapty React Native SDK v4.0 へ移行します。Flow Builder と Paywall Builder の両方に対応しています。"
metadataTitle: "Adapty React Native SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty React Native SDK 4.0 ではフローが導入され、それに伴いペイウォール API の名称が変更されました。新しい API は新しい Flow Builder と既存のペイウォールビルダーの両方で動作します。Adapty ダッシュボード側の設定変更は不要です。
