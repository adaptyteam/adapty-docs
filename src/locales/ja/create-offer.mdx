...
[フロー & ペイウォールビルダー](adapty-flow-builder)では、オファーはProductsエレメント上のプロダクトに紐付けられます。まずプロダクトエレメントを追加してプロダクトを割り当ててください — [購入の設定](paywall-product-block)を参照してください。

オファーを紐付けるには：

1. キャンバス上で、オファーを表示するプロダクトカードを選択します。
2. 右パネルの **Product** の下で、プロダクトを選択し、オファードロップダウンからオファーを選びます。オファーを選ぶまでドロップダウンには **No offer** と表示され、オファーが設定されているプロダクトのオファーのみが一覧表示されます。
