...
|---|---|
| 3.17.0 以降 | v8 |
| 3.15.0〜3.16.2 | デフォルトで v7、別の依存関係によって引き上げられる場合は v8 |

:::note
特定の Billing Library バージョンとの互換性は、Google がそのバージョンで導入したすべての機能を Adapty がサポートすることを意味しません。新しい Google Play の課金機能を採用する前に、[Play Store のプロダクト](android-products)を参照してください。
:::

## Gradle を使って Adapty SDK をインストールする \{#install-adapty-sdk-via-gradle\}

Adapty SDK を Gradle でインストールする手順は、Android と iOS の両方のアプリに必要です。
