...
description: "ペイウォールをトライアルありのプロダクトとなしのプロダクトで切り替え、選択されたプロダクトをトグルに連動させます。"
metadataTitle: "トライアルトグルを使ったペイウォールの作成 | フローとペイウォール | Adapty ドキュメント"
---
...
## 2. トライアルトグルを追加する \{#add-the-trial-toggle\}

1. **+** > **Paywall & Commerce** > **Toggle** > **Trial toggle** をクリックします。
...
2. トグルが正しいタイプの選択グループに属していることを確認します。キャンバスの空白部分をクリックして **Screen settings** を開きます。**Selectable groups** の下で `trial_toggle` を開き、**Group Type** を **Toggle** に設定して **Save** をクリックします。ダイアログには、トグルがオンかどうかを検出するために使用する変数 `trial_toggle.isActive` が表示されます。
...
デフォルトではトグルをオフにしておいてください。事前に選択されたトライアルトグルはダークパターンとみなされ、App Storeの審査で却下されるリスクがあります。詳しくは[トグルとチェックボックス](builder-toggles)を参照してください。
