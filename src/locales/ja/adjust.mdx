---
title: "Adjust"
description: "サブスクリプションのトラッキングと分析を強化するために、AdjustをAdaptyと連携させましょう。"
...
[Adjust](https://www.adjust.com/)は、マーケティングキャンペーンのデータを収集・分析する主要なモバイル計測パートナー（MMP）プラットフォームの一つです。企業はこれを活用してキャンペーンのパフォーマンスを把握できます。

Adapty は、ストアからの[サブスクリプションイベント](events)を一か所で追跡できる完全なデータセットを提供します。Adapty を使えば、サブスクライバーの行動を簡単に把握し、好みを理解した上で、ターゲットを絞った効果的なコミュニケーションを取ることができます。このインテグレーションにより、Adjust でサブスクリプションイベントを追跡し、キャンペーンが生み出す収益を正確に分析できます。

Adapty と Adjust のインテグレーションは、主に 2 つの方法で機能します。

1. **Adapty が Adjust からアトリビューションデータを受信する**
   Adjust インテグレーションを設定すると、Adapty は Adjust からアトリビューションデータを受信し始めます。このデータはユーザーのプロファイルページで確認できます。
...
2. **AdaptyはサブスクリプションイベントをAdjustに送信します**
   Adaptyは、インテグレーションで設定されたすべてのサブスクリプションイベントをAdjustに送信できます。その結果、これらのイベントをAdjustのダッシュボード内で追跡できるようになります。このインテグレーションは、広告キャンペーンの効果を評価するのに役立ちます。
...
:::warning
Adapty は [Stripe および Paddle の購入](#web-purchases-stripe-and-paddle)を、購入者がアプリをインストールしたプラットフォームのアプリトークンを使用して Adjust に送信します。Android アプリを提供しており、Web で販売している場合は、Android の **App Token** を入力してください。入力がない場合、Android 購入者の Web 購入は [Missing integration credentials](handle-integration-errors#missing-integration-credentials) エラーで失敗します。
:::
...
上記の手順が完了したら、次の2つのメソッドをアプリに追加してください。これにより、アプリとAdjustの間で通信が確立されます。

1. **サブスクリプションデータをAdjustに送信する**: Adjustのデバイス IDを `setIntegrationIdentifier()` SDKメソッドに渡す
2. **Adjustからアトリビューションデータを受信する**: `updateExternalAttribution()` SDKメソッドでアトリビューションデータを更新する（SDKバージョン4.1以前では `updateAttribution()` という名前）

Adjustバージョン5.0以降では、次の例を使用してください。
...
## Web 購入（Stripe・Paddle）\{#web-purchases-stripe-and-paddle\}

Adjust は [Stripe](stripe) や [Paddle](paddle) による購入を、App Store・Google Play の購入と一緒に受け取ります。Web 購入はアプリの外で行われるため、モバイルプラットフォームが存在しません。Adapty は代わりに購入者のプラットフォームを使用します。購入者の Adapty プロファイルが App Store または Google Play で作成されていた場合、Web 購入はそのストアからの購入として Adjust に送信されます：

| 購入者のプロファイルストア | Adapty が購入と一緒に送信する情報 |
|:--------------------------|:----------------------------------|
| App Store                 | iOS の **App Token** と **OAuth Token**、および購入者の `idfa` と `idfv`。 |
| Google Play               | Android の **App Token** と **OAuth Token**、および購入者の `android_id` と `gps_adid`。 |

購入はインストールと同じデバイスに対して Adjust に送信されるため、Adjust はその購入をユーザーを獲得したキャンペーンに紐付けることができます。

Adjust連携ページには、StripeやPaddleに特化したフィールドはありません。Web購入は、[AdaptをAdjustに接続する](#connect-adapty-to-adjust)で設定したiOSおよびAndroidの認証情報を使用します。

モバイルアプリを一度も開いたことのない購入者にはAdjustのデバイスIDがなく、そのプロファイルはStripeまたはPaddleで作成されています。AdaptはiOSの**App Token**を使用し、広告IDなしでその購入をAdjustに送信します。このため、Adjustは購入をインストールと紐付ける識別子を持ちません。
