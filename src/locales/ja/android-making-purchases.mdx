---
title: "Android SDKでモバイルアプリ内購入を行う"
description: "Adaptyを使ったアプリ内課金とサブスクリプションの処理に関するガイド。"
metadataTitle: "AdaptyでのアプリUI内課金の処理 | Adapty ドキュメント"
keywords: ['makePurchase', 'pending']
rank: 95
displayed_sidebar: sdkandroid
---

モバイルアプリ内でペイウォールを表示することは、ユーザーにプレミアムコンテンツやサービスへのアクセスを提供するための重要なステップです。ただし、ペイウォールの表示だけで購入が完結するのは、Adapty が画面をレンダリングする場合、つまり[フロー](adapty-flow-builder)や[旧ペイウォールビルダー](adapty-paywall-builder)で作成したペイウォールを使う場合に限られます。

独自のコードで画面をレンダリングする場合は、購入を完了してコンテンツをアンロックするために、`.makePurchase()` という別のメソッドを使用する必要があります。このメソッドは、ユーザーがペイウォールを通じて購入手続きを進めるための入り口となります。

ペイウォールに、ユーザーが購入しようとしているプロダクトに対するアクティブなプロモーションオファーが設定されている場合、Adapty は購入時に自動的にそれを適用します。
...
:::note
**Adaptyがあなたのスクリーンをレンダリングしていますか？** [フロー](adapty-flow-builder)または[ペイウォールビルダー](adapty-paywall-builder)のペイウォールの場合、購入は自動的に処理されます。このステップはスキップできます。

**ステップバイステップのガイダンスをお探しですか？** 完全なコンテキストを含むエンドツーエンドの実装手順については、[クイックスタートガイド](android-implement-paywalls-manually)をご確認ください。
:::
...
:::warning
`subscriptionUpdateParams` を渡すのは、同じ Google アカウントで購入したアクティブなサブスクリプションからの切り替え時のみにしてください。新規購入の場合は、このパラメータを指定せずに `makePurchase` を呼び出してください。指定すると `CURRENT_SUBSCRIPTION_TO_UPDATE_NOT_FOUND_IN_HISTORY` エラーで失敗します。このエラーメッセージは、非アクティブなサブスクリプションまたは別の Google アカウントが原因であるかのように誤解させる内容になっています。
:::
