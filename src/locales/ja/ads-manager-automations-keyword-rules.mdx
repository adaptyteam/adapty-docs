...
8. **Schedule** セクションで：
   - 頻度を選択します：**Once**、**Every hour**、**Every 3 hours**、**Every 6 hours**、**Every 12 hours**、**Every day**、**Every 2 days**、**Every 5 days**、**Every 7 days**、**Every 14 days**、**Every 30 days**、または **Every 90 days**。すべてのルールタイプで同じ頻度が利用できます。
   - 実行時間を時間単位で選択します（すべての時間はUTCです）。
   - 保存と同時にルールをすぐに実行するには、スケジュールに加えて **Run this rule right now** を選択します。**Once** に設定されたルールには繰り返しスケジュールも実行時間もなく、保存後に一度だけ実行されます。

ルールは、スケジュールされた時刻（UTC）に実行されます。通常、数分以内に完了し、その後はログやメインダッシュボードで変更を確認できます。
