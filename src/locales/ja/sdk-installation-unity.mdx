...
Adapty SDK は iOS 13.0+ をサポートしていますが、ペイウォールビルダーで作成したペイウォールを使用するには iOS 15.0+ が必要です。Adapty SDK 4.0 (beta) — [フロービルダー](adapty-flow-builder)サポートを追加 — は、アプリ全体で **iOS 15.0+** が必要です。デプロイメントターゲットがそれより低い場合、Unity Editor のビルドバリデーターが iOS ビルドを停止します。

:::info
AdaptyはGoogle Play Billing Library 8.x以下に対応しています。デフォルトでは、AdaptyはGoogle Play Billing Library v7.0.0を使用します。新しいバージョンを使用するには、AndroidビルドでBillingの依存関係を[上書き](https://developer.android.com/google/play/billing/integrate#dependency)してください。

Billing Libraryのバージョンとの互換性は、AdaptyがGoogleが導入したすべての機能をサポートすることを意味するわけではありません。新しいGoogle Play課金機能を採用する前に、[Play StoreのProの商品](android-products)を確認してください。
:::
...
SDK 4.0 — [フロービルダー](adapty-flow-builder)のサポートを追加 — はプレリリース版です。Unity Package Manager経由でインストールするには、Git URLにベータタグを追加してください：

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.0.0-beta.1
```

Unityパッケージ経由でインストールする場合は、[4.0.0-beta.1リリース](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.0.0-beta.1)から`adapty-unity-plugin-4.0.0-beta.1.unitypackage`をダウンロードしてください。

v4には2つのビルド設定の変更が伴います。ネイティブiOS Adapty SDKはリモートのSwiftパッケージとして宣言されるようになり、CocoaPodsを介したインストールは行われなくなります。

- External Dependency Manager を **1.2.188 以降**にアップデートしてください。それより古いバージョンは Swift Package Manager の依存関係をサポートしていません。これは SDK 4.0 がピア依存関係として宣言しているバージョンであるため、プロジェクトに古いバージョンがある場合は Unity が警告を表示します。
- 上記の CocoaPods の手順（`iOS Resolver -> Install Cocoapods`、`Unity-iPhone.xcworkspace` を開く）は SDK 3.x 専用です。SDK 4.0 では、EDM が生成された Xcode プロジェクトに Swift パッケージを自動的に追加します。
- iOS のデプロイメントターゲットを **15.0 以降**に設定してください。Unity Editor のビルドバリデーターが、設定されていない場合に iOS ビルドを停止します。

完全な変更内容については、[移行ガイド](migration-to-unity-sdk-v4)をご覧ください。
...
:::note
SDK 4.0では、リスナーインターフェースがC#のIプレフィックス規約に従っています。`AdaptyEventListener`の代わりに`IAdaptyEventListener`を実装してください。メソッドは変わりません。詳細は[移行ガイド](migration-to-unity-sdk-v4)を参照してください。
:::

:::important
他のAdapty SDKメソッドを呼び出す前に、`Activate`の完了コールバックを待ってください。完全なシーケンスについては、[Unity SDKの呼び出し順序](unity-sdk-call-order)を参照してください。
:::
