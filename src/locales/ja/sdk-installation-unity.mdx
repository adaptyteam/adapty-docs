...
Adapty SDK は iOS 13.0+ をサポートしていますが、ペイウォールビルダーで作成したペイウォールを使用するには iOS 15.0+ が必要です。[フロー](adapty-flow-builder)のサポートを追加する Adapty SDK 4.1 は、アプリ全体で **iOS 15.0+** が必要です。デプロイメントターゲットがこれより低い場合、Unity Editor のビルドバリデーターが iOS ビルドを停止します。また、SDK 4.x を使用して iOS 向けにビルドするには **Xcode 26** 以降が必要です。これは、Swift パッケージとして追加されるネイティブ iOS SDK が Swift ツール 6.2 でビルドされているためです。
...
|---|---|
| 3.17.0 以降 | v8 |
| 3.15.0〜3.15.2 | デフォルトは v7、別の依存関係によって引き上げられる場合は v8 |

:::note
Billing Library のバージョンとの互換性があるからといって、Google がそのバージョンで導入したすべての機能を Adapty がサポートしているわけではありません。新しい Google Play の課金機能を導入する前に、[Play Store のプロダクト](android-products) を確認してください。
:::
