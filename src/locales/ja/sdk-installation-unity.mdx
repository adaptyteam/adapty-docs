...
Adapty SDK は iOS 13.0+ をサポートしていますが、ペイウォールビルダーで作成したペイウォールを使用するには iOS 15.0+ が必要です。[フロー](adapty-flow-builder)のサポートを追加した Adapty SDK 4.1 では、アプリ全体に **iOS 15.0+** が必要です。デプロイターゲットがそれより低い場合、Unity Editor のビルドバリデーターが iOS ビルドを停止します。
