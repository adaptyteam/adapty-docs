...
Adapty SDK は iOS 13.0 以上をサポートしていますが、ペイウォールビルダーで作成したペイウォールを使用するには iOS 15.0 以上が必要です。[フロー](adapty-flow-builder)のサポートを追加した Adapty SDK 4.1 では、アプリ全体に **iOS 15.0 以上**が必要です。デプロイメントターゲットがそれより低い場合、Unity エディターのビルドバリデーターが iOS ビルドを停止します。SDK 4.x で iOS 向けにビルドするには、**Xcode 26** 以降も必要です。これは、Swift パッケージとして追加されるネイティブ iOS SDK が Swift tools 6.2 でビルドされているためです。
