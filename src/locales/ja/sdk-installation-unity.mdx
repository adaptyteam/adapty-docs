...
Adapty SDK は iOS 13.0 以降をサポートしていますが、ペイウォールビルダーで作成したペイウォールを利用するには iOS 15.0 以降が必要です。[フロー](adapty-flow-builder)のサポートを追加した Adapty SDK 4.1 では、アプリ全体で **iOS 15.0 以降**が必要です。デプロイターゲットがそれ以下の場合、Unity Editor のビルドバリデーターが iOS ビルドを停止します。また、SDK 4.x で iOS 向けにビルドするには **Xcode 26** 以降が必要です。これは、SDK が Swift パッケージとして追加するネイティブ iOS SDK が Swift ツール 6.2 でビルドされているためです。

:::info
3.17.0以降、Adapty Unity SDKはデフォルトでBilling Library v8を使用します。バージョン3.15.0〜3.15.2はv7を使用しており、Android ビルド内の別の依存関係がBilling Libraryをv8に引き上げた場合にも動作します。

Billing Libraryのバージョンとの互換性は、AdaptyがGoogleが導入したすべての機能をサポートすることを意味するわけではありません。新しいGoogle Playの課金機能を採用する前に、[Play StoreのProの製品](android-products)をご確認ください。
:::
