---
title: "AIアシスタンスを使用してAdaptyをiOSアプリに統合する"
description: "Cursor、Context7、ChatGPT、Claude、またはその他のAIツールを使用して、AdaptyをiOSアプリに統合するためのステップバイステップガイド。"
metadataTitle: "AIアシスタンスを使用してAdaptyをiOSに統合する | Adapty ドキュメント"
---

このガイドでは、AIコーディングツールを使ってAdaptyをiOSアプリにステップごとに統合する方法を説明します。適切なAdaptyドキュメントを正しい順序でツールに与えていきます。
...
- [**手動作成のペイウォール**](ios-quickstart-manual): 独自のペイウォール UI をコードで構築しつつ、プロダクトの取得と購入処理には Adapty を使用します。
...
プレースメント ID でフローまたはペイウォールを取得し、表示して、購入イベントを処理します。必要なガイドは購入の処理方法によって異なります。
...
**ガイド:**
- [フローとペイウォールビルダーで購入を有効にする（クイックスタート）](ios-quickstart-paywalls)
- [フローとその設定を取得する](get-pb-paywalls)
- [フローを表示する](ios-present-paywalls)
- [フローのイベントを処理する](ios-handling-events)
- [ボタンアクションに応答する](handle-paywall-actions)
