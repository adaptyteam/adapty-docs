...
Webペイウォールを使用するには、それをトリガーする必要があります。トリガーの方法はセットアップによって異なります。

- フロービルダーを使用している場合は、購入をトラッキングしてデータをAdaptyに送り返すために用意したリンクを使用する[新しいボタンを追加する](#step-2a-add-a-web-purchase-button)だけです。
