...
| **Locale**                   | 任意       | フローまたはペイウォールのレンダリングに使用する [ビルダーのローカライゼーション](add-paywall-locale-in-adapty-paywall-builder) の識別子（例：`en` や `pt-br`）。フローのローカライゼーションはビューの作成時に行われるため、ここが唯一の設定箇所です。省略した場合、Adapty はデバイスの設定からローカライゼーションを解決します。詳細は [ローカライゼーションとロケールコードの使用](unity-localizations-and-locale-codes) を参照してください。 |
| **LoadTimeout**              | 任意       | このメソッドのタイムアウト時間を制限します。タイムアウトに達した場合、キャッシュされたデータまたはローカルフォールバックが返されます。このメソッドは内部で複数のリクエストを行うことがあるため、まれに `LoadTimeout` で指定した時間より若干遅れてタイムアウトすることがあります。 |
| **PreloadProducts**          | 任意       | `true` に設定すると、パフォーマンス向上のためにプロダクトをプリロードします。有効にすると、プロダクトが事前に読み込まれ、フローまたはペイウォールの表示に必要な時間が短縮されます。 |
| **ProductPurchaseParameters** | 任意      | Android のみ（iOS では無視されます）。`AdaptyProductIdentifier` から `AdaptyPurchaseParameters` へのディクショナリ。フローまたはペイウォール内の個々のプロダクトに対して、パーソナライズされたオファーやサブスクリプション更新パラメーターなど、購入固有のパラメーターを設定するために使用します。 |
| **EnableSafeAreaPaddings**   | 任意       | Android のみ（iOS では無視されます）。`true` の場合、フロービューにセーフエリアのパディングが適用されます。デフォルト：`true`。ほとんどのケースではデフォルト設定で問題ありません。 |
