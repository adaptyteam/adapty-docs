...
これに対応するために、`GetPaywallForDefaultAudience` メソッドを使用できます。このメソッドは **All Users** オーディエンスの指定されたプレースメントのペイウォールを取得します。ただし、推奨されるアプローチは上記の[ペイウォールを取得する](#fetch-paywall-designed-with-paywall-builder)セクションで詳述されている `getPaywall` メソッドを使用することであることを理解しておくことが重要です。
...
ユースケースにおいて高速な取得がこれらのデメリットを上回る場合は、以下のように `GetPaywallForDefaultAudience` を使用してください。それ以外の場合は、[上記](#fetch-paywall-designed-with-paywall-builder)で説明した `GetPaywall` を使用してください。
:::
