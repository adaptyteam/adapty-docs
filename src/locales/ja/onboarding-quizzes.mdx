---
title: "フローのクイズ"
description: "Adapty フローにインタラクティブなクイズを追加して、ユーザーの好みを収集し、パーソナライズされたフローを実現しましょう。コードは不要です。"
metadataTitle: "フローのクイズ | Adapty ドキュメント"
---

クイズを使うと、事前に定義した選択肢をユーザーに提示できます。入力欄とは異なり、クイズにはテキスト入力フィールドがなく、ユーザーはあなたが定義した選択肢から選ぶだけです。ユーザーの好みを収集したり、ユーザーをセグメント分けしたり、回答に応じてフローを分岐させたりするのに活用できます。また、アプリ側で [SDK を通じて回答を受け取り](ios-flow-input)、内部処理やバックエンドでの処理に利用することも可能です。
