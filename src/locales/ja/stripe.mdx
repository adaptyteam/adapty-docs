...
4. ポップアップに表示された生成済みのキーをコピーし、Adaptyの [App Settings → Stripe](https://app.adapty.io/settings/stripe) に移動します。使用するモードに応じて、**Stripe App Restricted API Key** セクションにキーを貼り付けてください。テストモードとライブモードでは異なるキーを生成する必要があります。Stripeの **Test mode** のキーを使用し、Stripe **Sandbox** のキーは使用しないでください — AdaptyはStripe Sandboxをサポートしていません。

<Zoom>
      <img src="/assets/shared/img/Stripe3.png"/>
</Zoom>

準備完了です！次に、Stripe でプロダクトを作成し、Adapty に追加してください。

<Details>

<summary>非推奨のインストールフロー</summary>

1. Stripe の [Developers → API Keys](https://dashboard.stripe.com/apikeys) に移動してください：
...
2. **Secret key** タイトルの横にある **Reveal live (test) key button** をクリックしてキーをコピーし、Adapty の [App Settings → Stripe](https://app.adapty.io/settings/stripe) に移動します。ここにキーを貼り付けます：
...
3. 次に、Adapty の同じページの下部からウェブフック URL をコピーします。Stripe の [**Developers** → **Webhooks**](https://dashboard.stripe.com/webhooks) に移動し、**Add endpoint** ボタンをクリックします：
...
4. Adapty のウェブフック URL を **Endpoint URL** フィールドに貼り付けます。次に、ウェブフックの **Version** フィールドで **Latest API version** を選択します。続いて、以下のイベントを選択します：
...
5. "Add endpoint" をクリックし、次に "Signing secret" の下にある "Reveal" をクリックします。これは Adapty 側でウェブフックデータをデコードするために使用するキーです。表示されたらコピーしてください：
...
:::warning
Adapty は受信した Stripe イベントを `product_id` と `price_id` の組み合わせで照合します。Stripe で価格を変更すると新しい `price_id` が生成され、新しい価格での購入イベントが照合されなくなります。Stripe 側ではウェブフックが正常に配信されたように見えますが、Adapty はトランザクションを記録しません。価格を変更した後は、販売しているすべての `price_id` が Adapty のプロダクトに追加されていることを確認してください。
:::
...
### Stripeは100%の配信を示しているが、Adaptyにイベントが届かない \{#stripe-shows-100-delivery-but-no-events-reach-adapty\}

AdaptyはエンドポイントをAdaptyが自ら作成した際にのみ、Webhookの署名シークレットを取得します。Adaptyのお客様のStripeアカウントにAdaptyのURLのWebhookエンドポイントがすでに存在している場合（例：以前の接続試行時や、上記の非推奨の手動セットアップに従った場合など）、再接続しても新しいエンドポイントは作成されず、Adaptyは署名シークレットを受信（または再受信）しません。すべてのイベントは既存のエンドポイントに到達して`200`が返されますが、Adapty側で署名の検証に失敗し、破棄されます。

これを解決するには、[Developers → Webhooks](https://dashboard.stripe.com/webhooks) でAdaptyのウェブフックURLを指すエンドポイントを削除し、[App Settings → Stripe](https://app.adapty.io/settings/stripe) からStripeインテグレーションを再接続して、Adaptyが新しいエンドポイントを作成してシークレットを取得できるようにしてください。
...
### 正味収益と税金 \{#net-revenue-and-tax\}

Stripeトランザクションの`net_revenue_usd`と`net_revenue_local`を計算するにあたり、AdaptyはフラットRate4.5%の控除を適用します。これはモデリングによる推定値であり、Stripeが実際に徴収した税額ではありません。AdaptyはStripe Tax を含むStripeからの税額情報を読み取らないため、Stripeトランザクションの`tax_amount_usd`と`tax_amount_local`は常に`0`となります。
...
### 1つのStripeアカウントを複数のAdaptyアプリに接続する \{#one-stripe-account-connected-to-several-adapty-apps\}

イベントを受け取るWebhookエンドポイントが、そのイベントをどのAdaptyアプリで処理するかを決定します。各Adaptyアプリには固有のエンドポイントURLがあり、制限付きAPIキーではありません。Stripeに登録したすべてのエンドポイントはすべてのイベントを受け取り、各アプリはそのコピーを独立して処理します。アプリに追加されていないプロダクトのイベントは無視されるため、プロダクトセットが異なるアプリは安全に共存できます。ただし、同じStripeプロダクトと価格が複数のアプリに追加されている場合、それぞれのアプリがそのトランザクションを記録します。
...
`variation_id` は、Stripe Subscription（`sub_...`）および Checkout Session（`ses_...`）オブジェクトのメタデータからのみ読み取られます：
...
Stripe イベントの転送と分析に使用できるインテグレーション：
- [Adjust](adjust)
- [Amplitude](amplitude/)
- [AppsFlyer](appsflyer)
- [Webhook](webhook)
...
Adjustには Stripeのフィールドがありません。AdaptyはStripeの購入を、ユーザーがアプリをインストールしたプラットフォームのアプリトークンを使用してAdjustに送信します。詳細は[Webでの購入（StripeおよびPaddle）](adjust#web-purchases-stripe-and-paddle)をご覧ください。
