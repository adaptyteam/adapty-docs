---
title: "フローとペイウォールビルダーでテキストとリストを追加・スタイル設定する"
description: "Adapty のフローとペイウォールビルダーでタイトル、サブタイトル、段落、リストを追加・スタイル設定し、ブランドに合わせたユーザー体験を実現する方法を説明します。"
metadataTitle: "テキスト | フローとペイウォール | Adapty ドキュメント"
---

タイトル、段落、リストをワンクリックで追加し、ブランドに合わせてスタイルを設定できます。また、[動的変数](onboarding-variables)を使ってユーザーごとにコンテンツをパーソナライズすることも可能です。

<AIPromo title="見出しをプロンプト一つで書き換える。">
  AIエディターに文章の書き換えや無料トライアルの追記、画面上のすべてのテキストを一括で短縮するよう指示できます。
</AIPromo>
...
3. [**Open URL**](onboarding-actions#open-url) アクションの遷移先を設定します。
    ...
    :::important
    空の Open URL アクションは[プレビューと公開をブロックします](flow-common-issues#a-flow-wont-preview-or-publish)。
    :::
...
:::tip
別の言語の条件付きテキストを編集するには、エディタ下部でアクティブなロケールを切り替えてください。言語は先に **Localizations** パネルで追加してください — [フロー＆ペイウォールビルダーでロケールを追加する](add-paywall-locale-in-adapty-paywall-builder)を参照してください。
:::
