...
2. `openWebPaywall(paywall)` — ペイウォールによってURLを生成しますが、プロダクトデータはURLに追加しません。SDK v4では、取得したフローのペイウォールバリアントである `AdaptyFlowPaywall` を受け取ります。インデックスアクセスの前に `flow.paywalls` が空でないことを確認してください（例：`flow.paywalls[0]`）。
:::
