...
### サブスクリプションステータスの更新を受け取る \{#listening-for-subscription-status-updates\}
...
:::note
SDK 4.0 では、リスナーインターフェースが C# の I プレフィックス規則に従っています。`AdaptyEventListener` の代わりに `IAdaptyEventListener` を実装してください。メソッドは変わりません。詳細は[移行ガイド](migration-to-unity-sdk-v4)を参照してください。
:::
