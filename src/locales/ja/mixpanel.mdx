---
title: "Mixpanel"
description: "強力なサブスクリプション分析のためにMixpanelとAdaptyを連携する。"
metadataTitle: "Mixpanel インテグレーション | Adapty ドキュメント"
---

import AccessLevelUpdatedEventFeed from '@site/src/locales/ja/reusable/AccessLevelUpdatedEventFeed.mdx';


[Mixpanel](https://mixpanel.com/home/) は強力なプロダクト分析サービスです。イベント駆動型のトラッキングソリューションにより、プロダクトチームはさまざまなプラットフォームにおけるユーザー獲得・コンバージョン・リテンション戦略の最適化に関する貴重なインサイトを得ることができます。

このインテグレーションを使用すると、Adapty のすべてのイベントを Mixpanel に取り込むことができます。その結果、サブスクリプションビジネスと顧客の行動についてより包括的なインサイトを得られます。Adapty は、ストアからの[サブスクリプションイベント](events)を一か所で追跡できる完全なデータセットを提供します。Adapty を使えば、サブスクライバーの行動を簡単に把握し、好みを理解した上で、ターゲットを絞った効果的なコミュニケーションを実現できます。

## Mixpanel インテグレーションの設定方法 \{#how-to-set-up-mixpanel-integration\}

1. Adapty ダッシュボードの [Integrations -> Mixpanel](https://app.adapty.io/integrations/mixpanel) ページを開きます。
2. トグルを有効にして、**Mixpanel Token** を入力します。Adapty は iOS、Android、Stripe、Paddle の各プラットフォームに対して、**Production** と **Sandbox** のトークンを個別に管理しています。データを受け取りたいプラットフォームのみ入力してください。トークンが設定されていないプラットフォームからはイベントが送信されません。
3. **Mixpanel Data Residency** を Mixpanel プロジェクトの設定に合わせて選択します。このフィールドは必須で、デフォルトは **US** です。`api.mixpanel.com` エンドポイントを使用する場合は **US** を、`api-eu.mixpanel.com` を使用する場合は **Europe** を選択してください。
