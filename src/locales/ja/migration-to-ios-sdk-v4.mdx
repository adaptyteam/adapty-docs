...
## キッズモード: 独立パッケージから Package Trait への移行 \{#kids-mode-separate-products-replaced-by-a-package-trait\}

v3 では、[キッズモード](kids-mode)を有効にするには専用の **Adapty_KidsMode** および **AdaptyUI_KidsMode** パッケージプロダクトを選択してインポートを変更する必要がありました。`4.0.0-beta.2` 以降、これらのプロダクトは廃止されました。キッズモードは通常の Adapty パッケージ上に `KidsMode` という Swift package trait として実装され、これを有効にすることで SDK 全体から IDFA および AdSupport がコンパイル時に除外されます。

移行手順:
1. **Choose Package Products** ウィンドウで、**Adapty_KidsMode** と **AdaptyUI_KidsMode** の代わりに、通常の **Adapty** と **AdaptyUI** プロダクトを選択します。

2. `KidsMode` トレイトを有効にします。Xcode 26.4 以降を使用している場合は、プロジェクトの **Package Dependencies** ビューで AdaptySDK-iOS 依存関係に対して有効にしてください。`Package.swift` で Adapty を依存関係として追加する場合（`swift-tools-version` 6.1 以降が必要）は、そちらで有効にします：
...
3. 通常のモジュールに戻してインポートし直してください:
   ...
   ```
:::note
Xcode 26.4より前のバージョンでは、UIからXcodeプロジェクトのトレイトを有効にできません。その場合は、`KidsMode`トレイトを有効にしてAdaptyに依存する小さなローカルSwiftパッケージを作成し、アプリターゲットがそのパッケージに依存するようにしてください。
:::
...
## didFinishPurchase が必須になりました \{#didfinishpurchase-is-now-required\}

v3 では `didFinishPurchase` は省略可能で、実装しなかった場合は購入成功後にペイウォールが自動で閉じられていました。`4.0.0-beta.2` 以降、この自動クローズの実装は削除されました。これにより、購入成功後もフローを継続できるようになっています。たとえば、フローの残りの画面を表示し続けることができます。購入後の動作はご自身で決定してください。画面を閉じるか、何もしないでフローを継続させるかを選べます。
