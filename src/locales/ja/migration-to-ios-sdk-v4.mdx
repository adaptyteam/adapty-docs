...
description: "ペイウォール API をフロー API に置き換えることで Adapty iOS SDK v4.0 に移行します。フロー API は、旧ビルダーのペイウォールに加えてフローもレンダリングします。"
metadataTitle: "Adapty iOS SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty iOS SDK 4.0 ではフローが導入され、ペイウォール API の名称が変更されました。新しい API はフローと連携しており、旧ビルダーのペイウォールにも引き続き対応しています。Adapty ダッシュボード側での設定変更は不要です。
...
Adapty iOS SDK 4.0 では CocoaPods のサポートが廃止されました。SDK のインストールには [Swift Package Manager](sdk-installation-ios#install-adapty-sdk) をご利用ください。また、v4 のパッケージマニフェストが Swift tools 6.2 を宣言しているため、**Xcode 26** 以降が必要です。

プロジェクトでまだ CocoaPods を使用している場合は、`Podfile` から `Adapty` および `AdaptyUI` pod を削除し、`pod install` を実行してクリーンアップした後、Xcode で **File → Add Package Dependency** から `https://github.com/adaptyteam/AdaptySDK-iOS.git` を使ってパッケージを追加してください。
...
## App Store プロモート購入処理の一時的な削除 \{#app-store-promoted-in-app-purchases-temporarily-removed\}

StoreKit 2 への移行の一環として、Adapty iOS SDK 4.0 では App Store プロモートのアプリ内課金を処理する API が削除されました。`shouldAddStorePayment(for:)` デリゲートメソッドと、それが受け取る `AdaptyDeferredProduct` 型は 4.0 では利用できません。このような購入は引き続き完了し、Adapty がトランザクションを記録しますが、4.0 ではアプリがそれをインターセプトまたは遅延させる手段がありません。

:::warning
この削除は4.0にのみ適用されます。この処理はSDK 4.1で新しいAPIとともに復活します。アプリがApp Storeのプロモーション済みアプリ内課金をインターセプトしている場合は、3.xから直接4.1に移行してください。[App StoreからのIn-app purchases](making-purchases#in-app-purchases-from-the-app-store)を参照してください。
:::
...
返される型が `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` から `AdaptyFlow` / `AdaptyUI.FlowConfiguration` に変わります。`locale` パラメータはフェッチ呼び出しから `getFlowConfiguration` に移動します。`AdaptyFlow` は `paywalls` も公開しており、これはフローのペイウォールバリエーションを `AdaptyFlowPaywall` の値として返します。Web ペイウォールのメソッドはこれらのいずれかを受け取ります。
...
フォールバックファイルの形式は [SDK v4 で変更されました](fallback-flows)。**[Placements](https://app.adapty.io/placements)** > **Fallbacks** から新しいファイルをダウンロードし、アプリにバンドルしてください。

## ペイウォールの表示を記録する \{#tracking-paywall-views\}

### logShowPaywall(_:) → logShowFlow(_:)

`logShowPaywall` は `logShowFlow` に名称変更され、`AdaptyPaywall` の代わりに `AdaptyFlow` を受け取るようになりました。イベントは引き続き同じバリエーションに対して記録されるため、既存のファネルや A/B テストの指標はダッシュボードを変更せずにそのまま機能します。
...
v3 と同様に、Adapty がレンダリングするフローやペイウォールを表示する際はこのメソッドを呼び出す必要はありません。Adapty がそれらのビューを自動的にトラッキングします。
...
:::note
SDK 4.1 ではこのメソッドが `updateExternalAttribution(_:provider:)` に、型が `AdaptyExternalAttributionProvider` に改名されました。古い名前は 4.1 で非推奨なしに削除されています。4.1 へ直接移行する場合は [v4.1 への移行](migration-to-ios-sdk-41) を参照してください。
:::
