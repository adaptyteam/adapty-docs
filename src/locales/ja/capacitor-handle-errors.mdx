---
title: "Capacitor SDKでエラーを処理する"
description: "Capacitor SDKでエラーを処理します。"
...
:::tip
**デバッグ前に詳細ログを有効にしてください。** ほとんどの `AdaptyError` は、StoreKit、Play Billing、ネットワーク、またはバックエンドのエラーをラップしています。詳細ログを有効にすると（`adapty.setLogLevel({ logLevel: 'verbose' })` — [ログ設定](sdk-installation-capacitor#logging)を参照）、ラップされたエラーがコンソールに出力され、実際の原因がわかります。`AdaptyError` の `detail` プロパティはログレベルに関係なく値が入っていますが、詳細ログを使うとコンソールにも表示されます。
:::
...
### ネットワークエラーへの対処 \{#handle-network-errors\}
```typescript showLineNumbers
