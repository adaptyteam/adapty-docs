---
title: "プロダクトの追加"
description: "アプリ内課金プロダクトやサブスクリプションをAdaptyに追加し、App Store、Google Play、Stripe、Paddle、またはカスタムストアの登録情報とリンクします。"
metadataTitle: "プロダクトの追加 | Adapty ドキュメント"
---

:::tip
Adaptyをプログラムで設定しますか？このステップは[Developer CLI](developer-cli-quickstart)を使って完了できます。
:::

Adaptiyの主要機能を使い始める前に、販売する各プロダクトを追加し、サポートするすべてのストアまたは決済プラットフォームに紐付ける必要があります。この設定により、ユーザーのデバイスにプロダクトを配信し、後からアナリティクスで追跡できるようになります。

Adaptiyでは、アプリが販売するものはすべて**プロダクト**です。同じアイテムがApp Store、Google Play、またはStripeに存在する場合、Adaptiyの単一プロダクトにまとめることができます。一度設定すれば、すべてのプラットフォームを一か所から管理できます。
