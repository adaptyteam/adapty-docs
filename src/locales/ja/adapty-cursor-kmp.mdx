---
title: "AIの支援でKotlin MultiplatformアプリにAdaptyを統合する"
description: "Cursor、Context7、ChatGPT、Claude、またはその他のAIツールを使って、Kotlin MultiplatformアプリにAdaptyを統合するためのステップバイステップガイド。"
metadataTitle: "AIの支援によるKotlin MultiplatformへのAdapty統合 | Adapty ドキュメント"
displayed_sidebar: sdkkmp
---

このガイドでは、AI コーディングツールを使って Kotlin Multiplatform アプリに Adapty を段階的に統合する方法を説明します。適切な Adapty ドキュメントを正しい順序でツールに渡すだけです。
...
4. **フローまたはペイウォールを作成し、プレースメントに割り当てる**: Adapty ダッシュボードでフロー（UIを自分で構築する場合はペイウォール）を作成し、**Placements** ページでプレースメントに割り当てます。コード内では、プレースメント ID が `Adapty.getFlow("YOUR_PLACEMENT_ID")` に渡す文字列になります。
...
コーディングを始めるのに必須ではありませんが、インテグレーションが成熟するにつれて必要になります：

- **A/B テスト**: **Placements** ページで設定します。コードの変更は不要です。
  [A/B テスト](ab-tests)
- **追加フローとプレースメント**: 異なるプレースメント ID で `getFlow` の呼び出しを追加します。
...
- [**Adapty フロー & ペイウォールビルダー**](adapty-flow-builder): Adaptyのノーコードビルダーでフローを作成し、SDKが自動的にレンダリングします。
- [**手動で作成したペイウォール**](kmp-making-purchases): 自分でペイウォールUIをコードで構築しつつ、プロダクトの取得と購入処理にはAdaptyを使用します。
- [**オブザーバーモード**](observer-vs-full-mode): 既存の購入インフラをそのまま使い、Adaptyはアナリティクスとインテグレーションにのみ活用します。

どれを選べばいいか迷っている場合は、[クイックスタートの比較表](kmp-quickstart-paywalls)をご覧ください。
...
進めながらサンドボックスで各購入をテストしましょう — 最後まで待たないでください。セットアップ手順については[サンドボックスで購入をテストする](test-purchases-in-sandbox)を参照してください。
...
**ガイド:**
- [フローとペイウォールビルダーで購入を有効にする（クイックスタート）](kmp-quickstart-paywalls)
- [フローとその設定を取得する](kmp-get-pb-paywalls)
- [フローを表示する](kmp-present-paywalls)
- [フローイベントを処理する](kmp-handling-events)
- [ボタンアクションに応答する](kmp-handle-paywall-actions)
...
:::tip[チェックポイント]
- **期待される結果:** 設定したプロダクトが表示されたペイウォールが表示される。プロダクトをタップするとサンドボックス購入ダイアログが表示される。
