...
アプリの既製キーワードプールを取得します。これらはダッシュボードでキャンペーンを設定する際に Ads Manager が使用するものと同じプールです。
...
| フラグ | 必須 | 説明 |
|---|---|---|
| `--adam-id` | はい | [`asa apps list`](#adapty-asa-apps-list) から取得した、アプリの Apple App Store ID（`adam_id`） |
| `--type` | はい | `brand`、`generic`、または `competitor` |
| `--country` | いいえ | 2文字のISO国コード。複数指定する場合は繰り返します。デフォルト：プール内のすべての国 |

`--type` フラグでプールを選択します:

| タイプ | キーワード |
|---|---|
| `brand` | アプリのブランド用語、その表記バリエーション、およびブランドを含むオーガニック用語。出力にはブランド用語自体も含まれます |
| `generic` | アプリおよび上位オーガニック競合他社がランク付けしている非ブランド用語（既知のブランド用語はすべて除外）。`relevance_tier: top_organic` は、アプリがすでにオーガニックトップ10にランクインしている用語を示します |
| `competitor` | [AI Growth Advisor](autopilot-analysis#select-competitors) でアプリ向けに選択した各競合他社のブランド用語（競合他社ごとに1プール）。競合他社を選択するまでは空です |

各キーワードには、国、人気度、スコア、および中央値のオーガニックランクが含まれています。入札額やマッチタイプは含まれていません。それらは自分で選択し、[`asa keywords add`](#adapty-asa-keywords-add) でキーワードを追加してください。

プールのキーワードを使用する前に、プールの `status` を確認してください：

| ステータス | 意味 |
|---|---|
| `ready` | プールが構築済みです |
| `building` | プールはまだ構築中です。約1分後に再試行してください |
| `empty` | プールは構築済みですが、キーワードがありません |
| `failed` | 構築に失敗しました。後で再試行してください |

最初の `brand` または `generic` の呼び出しでは、リクエスト中にプールが構築されるため、数十秒かかる場合があります。このコマンドは1社あたり1分間に10回の呼び出し制限があり、`brand` または `generic` のビルドは1社あたり同時に1つしか実行できません。その処理中に2回目のビルドを実行しようとすると、`Retry-After: 5` とともに `429 cli_analytics_busy` が返されます。
...
CLIは指定したルールのJSONをそのまま保存しますが、1つ例外があります。**Add as keyword** アクションの設定はフラグから取得されます。各ルールタイプの詳細については[オートメーション](ads-manager-automations)を、ルールファイルの作成方法については[オートメーションルールの実行](developer-cli-ads-manager#run-automation-rules)を参照してください。

### キーワードとして追加アクションのフラグ \{#add-keyword-flags\}

`automations create` と `automations update` は、`add-as-keyword-to` アクションの `actions[0].params` を設定するフラグを共有しています。これは[検索語句ルール](ads-manager-automations-search-terms)の **Add as keyword**、[キーワードルール](ads-manager-automations-keyword-rules)の **Add as keyword to…** に対応します。いずれかのフラグを渡すと `params` オブジェクト全体が置き換わるため、このアクションには手書きの JSON は不要です。

| フラグ | JSONフィールド | 説明 |
|---|---|---|
| `--target-ad-group` | `targets.internal_ids` | キーワードを受け取る広告グループ（UUID）、繰り返し指定可。対象の広告グループが指定されていないルールは何もしない |
| `--match-type` | `match_type` | `BROAD` または `EXACT` |
| `--cpt-bid-type` | `cpt_bid.type` | 新しいキーワードの入札額の取得元：`ad_group_default_bid`、`set_to`、`search_term_current_cpt`、または `keyword_current_bid` |
| `--cpt-bid` | `cpt_bid.value` | `set_to` の場合は入札額そのもの。`search_term_current_cpt` または `keyword_current_bid` の場合は、コピーする入札額へのパーセンテージ上乗せ分。`ad_group_default_bid` では使用不可 |
| `--negate` | `negate` | 検索語句をネガティブキーワードとしても追加する。独自の `ad-group` または独自の `campaign` に追加。`--no-negate` を指定した場合は追加しない。検索語句ルールのみ |
| `--skip-enable-duplicates` | `skip_enable_duplicate_keywords` | 対象の広告グループにすでに存在するキーワードを有効化するのではなくスキップする。検索語句ルールのみ |
| `--pause-original` | `pause_in_original_ad_group` | ソースキーワードを元の広告グループで一時停止する。キーワードルールのみ |

`--cpt-bid-type` と `--match-type` はデフォルト値がないため、ルールファイル自体に `params` が含まれていない場合は両方必須です。

これらのフラグのうち4つはルールの `operate_with` フィールドに依存します。`--negate`、`--no-negate`、`--skip-enable-duplicates` は `search-term` ルールに属し、`--pause-original` は `targeting-keyword` ルールに属します。対応しないセットのフラグを指定した場合、終了コード `2` と適用可能なフラグの一覧が表示されます。

:::warning
このアクションの `params` はフラグから取得してください。異なるアクションを持つルールから取得しないでください。APIは `params` を名前付きバリアントではなく形状で解決するため、別のアクションに属するキーが含まれると、そのアクションが選択されて残りがすべて破棄されます。結果として `200` が返されますが、実際にはどの広告グループにもキーワードを追加しないルールが保存されてしまいます。
:::
...
`--file` は必須で、ファイルにはアクションを1つだけ含める必要があります（APIはルールごとにアクション1つ、条件1つを保持します）。

[add-as-keyword アクションフラグ](#add-keyword-flags)も受け付けており、`actions[0].params` を設定または上書きします。
...
自動化ルールを変更します。停止、名前の変更、またはルールの一部を差し替えることができます。
...
| `--name` | 新しいルール名 |
| `--file` | 変更する内容を記載したJSONファイル。`-` を指定すると標準入力から読み込む |

`--start` と `--stop` は同時に指定できません。ここで渡すファイルには `internal_id` を含めないでください。

[add-as-keyword アクションフラグ](#add-keyword-flags)も使用できます。いずれかを渡すと、この呼び出しは読み取り→変更→書き込みの操作になります。CLI がルールを読み取り、`actions[0].params` を再構築し、`actions` リスト全体を返送します。これは API が `actions` をマージするのではなく置き換えるためです。読み取りと書き込みの間にダッシュボードで行われた編集は上書きされます。
...
| `--metric` | はい | 指標名。繰り返し指定可能 |
| `--app` | いいえ | 対象アプリを絞り込む。繰り返し指定可能 |
| `--campaign` | いいえ | 対象キャンペーンを絞り込む。繰り返し指定可能 |
| `--ad-group` | いいえ | 対象広告グループを絞り込む。繰り返し指定可能 |
| `--group-by` | いいえ | 行を `country`、`day`、`week`、`month`、`quarter`、または `year` で集計する。繰り返し指定可能 |
| `--by-days` | いいえ | コホート指標の更新ウィンドウ（日数）。繰り返し指定可能。1回の呼び出しで最大16件。省略するとダッシュボードのデフォルト値が使用される |
| `--order-by` | いいえ | 並び替えに使用する指標またはフィールド |
| `--order-by-day` | いいえ | 指定した更新ウィンドウでコホート指標をランク付けする。`--by-days` で指定した値のいずれかを指定する必要がある |
| `--order` | いいえ | `asc` または `desc`。デフォルト：`desc` |

[ページネーションフラグ](#global-flags)に対応しています。[リストフィルター](#list-filters)のうち、`--app`、`--campaign`、`--ad-group`のみ使用できます。`--search`および`--status`は使用できません。それ以外の条件で絞り込む場合は、スコープを絞った`list`コマンドで取得したIDと行を照合してください。

各行は1つのエンティティを表し、サーバー側で集計されて`--order-by`の順に並べられます。上位N件を取得したい場合は1回の呼び出しで完結します。結果をページングして合計するのではなく、`--order-by`と`--page-size N`を設定してください。

```bash
adapty asa metrics --entity campaign --date-from 2026-07-01 --date-to 2026-07-31 --order-by spend --page-size 5
```

`--metric` には、Ads Manager が追跡する指標名をダッシュボードの表記で指定します。例えば `spend`、`taps`、`gross_roas` などです。全リストと各指標の計算方法については [指標](adapty-ads-manager-metrics) を参照してください。存在しない名前を指定した場合、有効な名前一覧がエラーに表示されます。

すべての指標はページング処理の前にエンティティレベル全体で集計されるため、カタログ全体ではなく読み取る列だけを指定してください。`subscribers` と `paid_subscribers` はエンティティごとにユニークなプロファイルをカウントするため、スペンドとインストールのみの呼び出しと比べて約17倍のコストがかかります。`arppu` と `arpas` はこれらから導出されるため同様です。どの `--entity` でも、これら4つの指標名を使用する場合は `--campaign` または `--ad-group` で呼び出し範囲を絞り込む必要があります。

```bash
adapty asa metrics --entity keyword --date-from 2026-07-01 --date-to 2026-07-31 --metric arpas --campaign <campaign-id>
```

これら2つのフィルターのどちらかがないと、呼び出しは `422 cli_metric_scope_too_wide` で失敗します。スコープを絞ることは、指標の呼び出しを高速化する最も安価な方法です。コストはページのサイズではなく、集計されるエンティティの数に依存するためです。

レポート期間の長さは、最も細かい `--group-by` の値によって上限が決まります。より長い期間をレポートするには、リクエストを複数に分割するのではなく、グループ化を粗くしてください:
...
日単位では最大4週間までカバーできます。より長い期間が必要な場合は `week` 粒度で指定してください。1回のリクエストで半年分のデータを取得できます。

各ページは最大5000件のブレークダウン行に制限されており、1行はエンティティ × 国 × 期間に相当します。制限を超えるページは `422 cli_response_too_large` エラーで失敗します。グルーピングを粗くするか、日付範囲を狭めるか、`--page-size` を下げてください。`day` 単位でグルーピングしている場合、エラーメッセージに適切なページサイズが示されるため、自分で計算した値ではなくその値でリトライしてください。`metrics overview` はエンティティごとのブレークダウンを返さないため、この制限は適用されません。

`ltv` 指標は存在しません。ライフタイムバリューはコホート指標であり、更新ウィンドウで読み取られます。そのため、day-7 や day-90 の値を取得するには `--by-days` を使用します。

```bash
adapty asa metrics --entity campaign --date-from 2026-07-01 --date-to 2026-07-31 --metric roas --by-days 7 --by-days 90
```

`--order-by-day` はそれらのウィンドウのいずれかで行をランク付けし、1 回の呼び出しで day-90 ROAS の上位キャンペーンを返します。
...
| ステータス | コード | 意味 |
|---|---|---|
| `402` | `ads_manager_subscription_required` | 会社に有効な Ads Manager サブスクリプションがない |
| `404` | — | エンティティが存在しないか、別の会社に属している |
| `409` | `cli_idempotency_in_progress` | 同じ冪等性キーを持つ書き込みが実行中 |
| `422` | `cli_idempotency_key_reuse` | 同じ冪等性キーが異なるリクエストボディで使用された |
| `422` | `cli_metric_scope_too_wide` | `--campaign` または `--ad-group` なしでプロファイルカウント指標がリクエストされた |
| `422` | `cli_response_too_large` | `metrics` ページが 5000 行のブレークダウン行を超える |
| `429` | `cli_analytics_busy` | アナリティクスプールがビジー状態、またはその会社の別のキーワードプールビルドが実行中。待機時間は `Retry-After` ヘッダーに記載 |
