...
`buildReactNativeFromSource` は **Expo SDK 57 以降で必要**です。Expo SDK 57 はプリビルド済みの React Native フレームワークを同梱していますが、フレームワークがダイナミックになると他のパッケージがそのヘッダーにアクセスできなくなるため、`expo-updates` や `@expo/ui` で `'React/RCTBridge.h' file not found` のようなエラーが発生して iOS ビルドが失敗します。React Native をソースからビルドすることでこの競合を回避できますが、iOS ビルドの時間は長くなります。Expo SDK 56 以前はこのオプションを省略できます。
...
完全な移行手順については、[Adapty React Native SDK を v4 へ移行する](migration-to-react-native-sdk-v4)を参照してください。
