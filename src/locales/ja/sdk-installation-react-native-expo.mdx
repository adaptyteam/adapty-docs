...
| iOS | 15.0 以降 |
| Swift | 6.2 以降（Xcode 26 同梱） |

:::info
3.17.0 以降、Adapty React Native SDK はデフォルトで Billing Library v8 を使用します。バージョン 3.15.0〜3.15.6 は v7 を使用しており、Android ビルドの別の依存関係が Billing Library を v8 に引き上げた場合にも動作します。

特定の Billing Library バージョンとの互換性は、Google がそのバージョンで導入したすべての機能を Adapty がサポートすることを意味しません。新しい Google Play の課金機能を採用する前に、[Play Store のプロダクト](android-products)をご確認ください。
:::
