...
Adapty React Native SDKにはiOS 15.0以上が必要です。

iOSビルドには**Swift 6.0**以降が必要です。[キッズモード](kids-mode-react-native)には**Swift 6.1**以降が必要です。

:::info
Adapty React Native SDK 4.0.3以降はGoogle Play Billing Library v8に対応しています。

Billing Libraryのバージョンへの対応は、GoogleがそのバージョンでリリースしたすべてのBilling機能をAdaptyがサポートすることを意味しません。新しいGoogle Playの課金機能を採用する前に、[Play StoreのProduct](android-products)をご確認ください。
:::
...
:::important
v4 以降、Adapty React Native SDK はネイティブ依存関係の CocoaPods によるインストールをサポートしなくなりました。v4 以降（[フロー & ペイウォールビルダー](adapty-flow-builder) 向け）が必要な場合は、以下の [Adapty SDK 4.0: Swift Package Manager を有効にする](#adapty-sdk-40-enable-swift-package-manager) を参照してください。
:::
...
### Adapty SDK 4.0: Swift Package Manager を有効化する \{#adapty-sdk-40-enable-swift-package-manager\}
...
iOS ビルドで `'React/RCTBridge.h' file not found` が発生した場合は、[トラブルシューティング項目](#react-header-not-found)を参照してください。

完全な移行手順については、[Adapty React Native SDK を v4 に移行する](migration-to-react-native-sdk-v4)を参照してください。
...
#### `'React/RCTBridge.h' file not found` で iOS ビルドが失敗する \{#react-header-not-found\}

[Adapty SDK 4.0 で動的フレームワークを有効化](#adapty-sdk-40-enable-swift-package-manager)すると、`expo-updates`、`@expo/ui`、または Objective-C ソースを含む他のパッケージで iOS ビルドが失敗する場合があります:
