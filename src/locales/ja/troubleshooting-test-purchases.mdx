...
## サンドボックス購入が「未承認」として拒否される \{#the-sandbox-purchase-is-refused-as-unauthorized\}

ストアに `You are not authorized to make purchases of this in-app in sandbox at this time` のようなアラートが表示される場合、拒否しているのは Adapty ではなく Apple です。テストデバイスと App Store Connect で以下の点を確認してください。

- **デバイスでアプリ内課金が許可されている**: **設定** > **スクリーンタイム** > **コンテンツとプライバシーの制限** > **App Store** で、**アプリ内課金**が**許可**に設定されている必要があります。Appleはこの設定について[Turn off Apple In-App Purchases from Screen Time](https://support.apple.com/en-us/102470)で説明しています。
- **サンドボックスアカウントが正しい場所でサインインしている**: メインのApple アカウント設定ではなく、[**設定** > **Developer** > **Sandbox Apple Account**](test-purchases-in-sandbox#step-4-switch-to-sandbox-test-account)からサインインしてください。Appleの指示では、「設定のSandboxサインインオプションを使用し、メインのApple アカウント設定は使わない」とされています。
- **テスターのメールアドレスが既存のApple アカウントと重複していない**: Appleは、重複したアドレスを使うと「テスト中にエラーが発生する」と述べています。テスターのメールアドレスは作成後に変更できないため、別のアドレスで[新しいテスターを作成](test-purchases-in-sandbox#step-1-create-sandbox-test-account-in-app-store-connect)してください。
