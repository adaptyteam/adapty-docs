...
:::info
アプリ内課金のテストには実機を使用することをおすすめします。シミュレーターでもサンドボックス購入は実行できますが、支払いダイアログや生体認証プロンプトを含むすべてのフローを完全にテストするには実機が必要です。
:::

アプリ内課金をテストする方法は主に2つあります：

- **Xcodeでビルドしてテストデバイスで実行する**：開発者やQAエンジニアに便利です。
- **TestFlightでサンドボックステストアカウントを使用する**：それ以外の方に適しています。
...
1. 親プロファイルを削除する。
2. テストデバイスからアプリを削除する。
3. [TestFlightからアプリをダウンロードする](#step-3-download-the-app-from-testflight)。
4. [サンドボックステストアカウントに切り替える](#step-4-switch-to-sandbox-test-account)。
5. [購入履歴をクリアする](#step-5-clear-purchase-history)。
6. [アプリを開いてテスト購入を行う](#step-7-make-test-purchase)。
...
1. テストデバイスからアプリを削除する。
2. [TestFlight からアプリをダウンロードする](#step-3-download-the-app-from-testflight)。
3. [サンドボックステストアカウントに切り替える](#step-4-switch-to-sandbox-test-account)。
4. [購入履歴をクリアする](#step-5-clear-purchase-history)。
5. [アプリを開いてテスト購入を行う](#step-7-make-test-purchase)。
