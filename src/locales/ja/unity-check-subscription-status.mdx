...
:::note
SDK 4.1 では、リスナーインターフェースが C# の I プレフィックス規約に従っており、`AdaptyEventListener` の代わりに `IAdaptyEventListener` を実装します。また、このインターフェースにはメソッドが1つ追加されており、`OnReceivePromotedPurchase` が必要です。詳細は[移行ガイド](migration-to-unity-sdk-v4)を参照してください。
:::
