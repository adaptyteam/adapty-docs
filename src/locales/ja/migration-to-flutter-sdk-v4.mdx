...
description: "ペイウォール API をフロー API に置き換えて Adapty Flutter SDK v4.0 に移行します。Flow Builder と Paywall Builder の両方に対応しています。"
metadataTitle: "Adapty Flutter SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty Flutter SDK 4.0 ではフローが導入され、それに伴いペイウォール API が改名されました。新しい API は新しい Flow Builder と既存のペイウォールビルダーの両方に対応しており、Adapty ダッシュボード側での設定変更は不要です。
...
メソッド名を変更し、`flow` パラメータで `AdaptyFlow` を渡します。その他のパラメータ（`loadTimeout`、`preloadProducts`、`customTags`、`customTimers`、`customAssets`、`productPurchaseParams`）は変更なく、ビューメソッドの `present`、`dismiss`、`showDialog` も同様です。

:::note
`customTimers` は引き続き存在しますが、レガシーペイウォールビルダーのペイウォールにのみ影響します。フローの[カウントダウンタイマー](flow-timer)はフロービルダーで設定した動作に従って実行されるため、ここで渡した値はフローでは無視されます。
:::
