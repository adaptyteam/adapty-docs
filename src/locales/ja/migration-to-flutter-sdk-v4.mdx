...
description: "ペイウォール API をフロー API に置き換えて Adapty Flutter SDK v4.0 (beta) へ移行します。Flow Builder と Paywall Builder の両方に対応しています。"
metadataTitle: "Adapty Flutter SDK v4.0 (beta) への移行 | Adapty ドキュメント"
---

Adapty Flutter SDK 4.0 (beta) ではフローが導入され、それに伴いペイウォール API の名称が変更されました。新しい API は新しい Flow Builder と既存のペイウォールビルダーの両方で動作します。Adapty ダッシュボード側での設定変更は不要です。
...
v4.0 はプレリリース版のため、`pubspec.yaml` に正確なバージョンを固定してください — pub はキャレット範囲でプレリリースを解決しません:
