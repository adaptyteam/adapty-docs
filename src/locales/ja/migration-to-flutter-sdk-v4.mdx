---
title: "Adapty Flutter SDK v4.0 への移行"
description: "ペイウォール APIをフロー APIに置き換えてAdapty Flutter SDK v4.0へ移行します。フロー APIは、旧ビルダーのペイウォールに加えてフローもレンダリングします。"
metadataTitle: "Adapty Flutter SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty Flutter SDK 4.0 ではフローが導入され、それに伴いペイウォール API が名称変更されました。新しい API はフローに対応しており、旧ビルダーのペイウォールでも引き続き動作します。Adapty ダッシュボード側での設定変更は不要です。
