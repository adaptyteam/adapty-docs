...
description: "Adapty を使用して React Native アプリでフローとペイウォールをユーザーに表示します。"
metadataTitle: "React Native でのフローとペイウォールの表示 | Adapty ドキュメント"

---

<SDKv4>

<MethodPromo method="getFlow" label="フローとペイウォールを表示する" />

フローや旧ペイウォールビルダーで作成したペイウォールは、ユーザーに表示するためにモバイルアプリのコードでレンダリングを実装する必要はありません。フローには、表示する内容と表示方法の両方が含まれています。

始める前に、以下を確認してください：

1. [フロー](paywall-builder-templates)または[ペイウォール](create-paywall)を作成済みであること。
2. [プレースメント](placements)に追加済みであること。
3. [フローをフェッチしてビューを準備済みである](react-native-get-pb-paywalls)こと。

:::warning

このガイドは、SDK v4.0 以降が必要な **Adapty がレンダリングするフローおよびペイウォール**専用です。リモートコンフィグペイウォールの表示方法については異なります。

- **リモートコンフィグペイウォール**の表示については、[リモートコンフィグで設計したペイウォールをレンダリングする](present-remote-config-paywalls)を参照してください。
...
このガイドは、**Adapty SDK 3.x** 向けの旧ペイウォールビルダーで作成されたペイウォールのみを対象としています。SDK v3.0 以降が必要です。SDK 2.x 以前向けに作成されたペイウォールやリモートコンフィグペイウォールの表示方法は異なります。

- **リモートコンフィグペイウォール**の表示については、[リモートコンフィグで設計したペイウォールのレンダリング](present-remote-config-paywalls)を参照してください。

:::

Adapty React Native SDK には、ペイウォールを表示する 2 つの方法があります。

- **React コンポーネント**: 組み込みコンポーネントとして、アプリのアーキテクチャやナビゲーションシステムに統合できます。
