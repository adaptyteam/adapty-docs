---
title: "OneSignal"
description: "プッシュ通知によるエンゲージメントを向上させるために、OneSignal を Adapty と統合します。"
metadataTitle: "OneSignal 統合ガイド | Adapty ドキュメント"
---

import AccessLevelUpdatedEventFeed from '@site/src/locales/ja/reusable/AccessLevelUpdatedEventFeed.mdx';


[OneSignal](https://onesignal.com/) は、プッシュ通知、メール、SMS、アプリ内メッセージングを提供する主要なカスタマーエンゲージメントプラットフォームです。Adapty と OneSignal を連携することで、すべてのサブスクリプションイベントを一か所で確認でき、それらのイベントに基づいた自動コミュニケーションをトリガーできます。

Adapty を使用すると、複数のストアにわたる[サブスクリプションイベント](events)を追跡し、ユーザーの行動を分析して、よりターゲットを絞ったコミュニケーションに活用できます。このインテグレーションにより、OneSignal ダッシュボード内でサブスクリプションイベントを監視し、[獲得キャンペーン](https://documentation.onesignal.com/docs/en/automated-messages)にマッピングできます。
...
| :------------------------- | :----------------------------------------------------------- |
| スケジュール | リアルタイム更新 |
| データ方向 | 一方向：AdaptyからOneSignalサーバーへ |
| Adapty連携ポイント | <ul><li>モバイルアプリコード内のOneSignalおよびAdapty SDK</li><li>Adaptyサーバー</li></ul>|
...
すべてのデバイス間でユーザー識別子の一貫性を維持し、ユーザーのIDが変更された場合は常にOneSignalを更新することが重要です。これにより、ユーザーのアクティビティとサブスクリプションのトラッキングが簡素化され、一貫したメッセージングが確保されるとともに、より正確な分析とより良いユーザー体験が実現されます。詳細については、OneSignalの[外部ユーザーIDドキュメント](https://documentation.onesignal.com/docs/en/users)を参照してください。
