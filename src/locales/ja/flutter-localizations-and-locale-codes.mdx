...
- **ビルダーで作成されたフロー**: `createFlowView` はロケールパラメータを受け取らず、SDKはデバイスのロケールを読み取りません。フローは[デフォルトロケール](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)でレンダリングされます。iOSでは、フローに英語のローカライゼーションがある場合、`en` で表示されます。
- **カスタム（リモートコンフィグ）ペイウォール**: `getFlow` は設定済みのすべてのローカライゼーションを `flow.remoteConfigs` として返します。各エントリには `locale` コードと設定内容（`data` 文字列、またはパースされた `dictionary`）が含まれます。ユーザーに合うエントリを選択し、独自のフォールバックを実装してください：
