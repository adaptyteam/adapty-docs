...
コードがどのローカライゼーションにも一致しない場合、フローはその[デフォルトロケール](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale)でサイレントにレンダリングされます。SDKはエラーを返したり、警告をログに記録したりしません。

コードが一致した場合、Adaptyはそのローカライゼーションをデフォルトのローカライゼーションとマージします。一致したローカライゼーションで定義されていない文字列やアセットは、デフォルトのローカライゼーションから取得されます。

ロケールコードを省略することは、フローのデフォルトローカライズを要求することとは異なります。SDKは固定の`en`を代入します。デフォルトロケールが`de`のフローは、`en`ローカライズが存在する場合は`en`で表示され、存在しない場合にのみ`de`にフォールバックします。
