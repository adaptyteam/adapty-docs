...
---
Adaptyがインストールをキャンペーンに紐付けると、`onInstallationDetailsSuccess` コールバックを通じてアプリにアトリビューションデータが返されます。このデータを使って、インストールを促したチャネルやキャンペーンに基づいてユーザー体験をパーソナライズできます。
...
すべてのフィールドは任意です。オーガニックインストールの場合や、アトリビューションを特定できなかった場合、`payload` フィールドに `attribution` オブジェクトは含まれません。
