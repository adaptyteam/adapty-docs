...
description: "Adapty SDKを使用して、Kotlin MultiplattformアプリでオンボーディングのデータをSaveして活用する方法。"
metadataTitle: "オンボーディング | Kotlin Multiplatform SDK | Adapty ドキュメント"
---

<OnboardingDeprecation />

ユーザーがクイズの質問に回答したり、入力フィールドにデータを入力したりすると、`onboardingViewOnStateUpdatedAction` メソッドが呼び出されます。このフィールドタイプをコード内で保存または処理することができます。
