...
* **App name** <Inline id="app.svg" alt="App" />: このフローが属するアプリを識別します。
* **All flows** <Inline id="flows.svg" alt="Flows" />: このアプリのすべてのフロー一覧を開きます。
* **フローの名前変更**: フロー名の横にある鉛筆アイコン <Inline id="pencil.svg" alt="Pencil" /> をクリックして名前を変更します。
* **表示モード切り替え**: デザインビュー <Inline id="cursor.svg" alt="Cursor" /> と[リモートコンフィグビュー](customize-flow-with-remote-config)<Inline id="remote-config.svg" alt="Remote Config" /> を切り替えます。
* **元に戻す／やり直す**: 矢印アイコンをクリックして変更を元に戻す <Inline id="undo.svg" alt="Undo" /> か、やり直す <Inline id="redo.svg" alt="Redo" /> ことができます。⌘Z / Ctrl+Z でも元に戻せます。
* **下書き保存／公開**: **Save draft** をクリックするとライブ配信せずに進捗を保存できます（⌘ / Ctrl+S）。ドロップダウン <Inline id="collapse-layer.svg" alt="Open dropdown" /> を開くと [**Publish**](builder-save-publish) ボタンにアクセスできます。フローを[プレースメント](create-placement)に追加できるのは公開後のみです。
