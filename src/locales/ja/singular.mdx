---
title: "Singular"
description: "SingularをAdaptyと連携して、マーケティングとサブスクリプションデータを分析します。"
...
[Singular](https://www.singular.net/) は主要なモバイル計測パートナー（MMP）プラットフォームの一つで、マーケティングキャンペーンのデータを収集・可視化します。これにより、企業はキャンペーンのパフォーマンスを追跡できます。

Adapty は、ストアからの[サブスクリプションイベント](events)を一元管理できる完全なデータセットを提供します。Adapty を使えば、サブスクライバーの行動を簡単に把握し、ニーズを理解した上で、ターゲットを絞った効果的なコミュニケーションが可能になります。この統合により、Singular でサブスクリプションイベントをトラッキングし、キャンペーンが生み出す収益を正確に分析できます。

Adapty は、インテグレーションで設定されたすべてのサブスクリプションイベントを Singular に送信できます。これにより、Singular ダッシュボード内でこれらのイベントをトラッキングできるようになります。このインテグレーションは、広告キャンペーンの効果を評価するうえで役立ちます。
...
| パラメーター                | タイプ    | 説明                           |
|:--------------------------|:--------|:--------------------------------------|
| `is_revenue_event`        | Boolean | `true`（イベントに収益が含まれる場合）。 |
| `amt`                     | Float   | 収益額。                       |
| `cur`                     | String  | 通貨コード。                        |
| `purchase_product_id`     | String  | ストアのプロダクト ID。        |
| `purchase_transaction_id` | String  | 元のトランザクション ID。              |
