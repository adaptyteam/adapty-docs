...
プロファイル一覧では、以下の方法で特定のユーザーを検索できます：

- **Profile ID**: Adapty がユーザーに割り当てた内部識別子（Adapty ID とも呼ばれます）。
- **Customer user ID**: アプリ側でユーザーに設定した識別子（設定済みの場合）。
- **Email**: カスタム属性として送信されたユーザーのメールアドレス（送信済みの場合）。
- **Transaction ID**: 購入時にストアから発行されたトランザクション ID。

検索は値の完全一致で機能します。部分一致はできません。また、前後に余分なスペースがあると検索結果が返されません。識別子全体をそのままコピー＆ペーストしてください。カスタムユーザーID、メールアドレス、トランザクションIDは大文字・小文字も区別されます。

任意の行をクリックすると、そのユーザーの完全なプロファイルを表示できます。
...
| プロパティ | 説明 |
| ---------------- | ------------------------------------------------------------ |
| Customer user ID | あなたのシステムにおけるエンドユーザーの識別子。 |
...
| IP country | エンドユーザーの直近の IP アドレスから判定された国。 |
| Store country | ストアから報告されたエンドユーザーの App Store または Google Play アカウントの国。Adapty はストアのトランザクションとともにこの情報を受け取るため、購入履歴のないユーザーでは空になります。 |
| OS | エンドユーザーが使用しているオペレーティングシステム。 |
| Device | エンドユーザーに表示されるデバイスのモデル名。 |
| Install date | ユーザーが Adapty に初めて記録された日付：<ul><li>ユーザーが作成された日付。</li><li>Adapty を導入する前にアプリをインストールしていたユーザーの場合、インストール日は最初のトランザクションの日付になります。</li><li>該当する場合は、過去データのインポート時に提供された日付。</li></ul> |
...
プロファイルの **Attributes** セクションでは、SDK または API を通じて設定されたカスタム属性を確認できます。また、**Add attribute** ボタンを使って手動で属性を追加することもできます。
...
属性を変更または削除するには、その属性の横にある三点メニューをクリックして **Edit** または **Delete** を選択します。属性を削除しても、影響を受けるのはそのプロファイルのみです。同じ属性を持つ他のプロファイルには影響しません。
