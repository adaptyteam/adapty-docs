...
ロケールコードは、Adaptyがフローのローカライズを選択するときと、カスタムペイウォール用のリモートコンフィグを読み取るときに使用されます。

ロケールコードはプラットフォームによって異なる複雑なものになりがちです。そのため、Adaptyはサポートするすべてのプラットフォームで共通の内部標準を採用しています。この標準を理解することで、ユーザーがどのローカライズを受け取るかを予測できます。
...
Adapty がユーザーのロケールに一致するローカライズを検索する際、以下の手順が実行されます：
...
この方法により、`'pt_BR'`、`pt-BR`、`pt-br` はすべて同じローカライゼーションに解決されます。
