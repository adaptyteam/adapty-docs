...
description: "Unityアプリでフローやペイウォールからのボタンアクションを処理します。"
...
Adapty フロー & ペイウォールビルダーでフローを構築する場合、ボタンを適切に設定することが重要です。
...
Adapty のペイウォールビルダーを使用してペイウォールを作成する場合、ボタンを適切に設定することが重要です。

1. 旧ペイウォールビルダーでボタンを追加し、既存のアクションまたはカスタムアクション ID を割り当てます。
2. 割り当てた各アクションを処理するコードをアプリに記述します。

このガイドでは、コード内でカスタムアクションおよび既存のアクションを処理する方法を説明します。

:::warning
**購入と復元のみ自動的に処理されます。** ペイウォールを閉じたり、リンクを開いたりといった他のボタンアクションはすべて、アプリのコード側で適切な処理を実装する必要があります。
:::
