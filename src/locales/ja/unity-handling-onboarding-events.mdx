...
:::note
SDK 4.1 では、リスナーインターフェースが C# の I プレフィックス規則に従います。`AdaptyOnboardingsEventsListener` の代わりに `IAdaptyOnboardingsEventsListener` を実装してください。メソッドは変わりません。詳細は[移行ガイド](migration-to-unity-sdk-v4)を参照してください。
:::
