...
description: "Adaptyを使用してUnityでオンボーディング関連のイベントを処理します。"
metadataTitle: "UnityでのオンボーディングイベントのハンドリングI | Adapty ドキュメント"
...
:::warning
**オンボーディングはSDK v4で非推奨となり、将来のリリースで削除される予定です。** バグ修正や改善は行われません。代わりに[フロー](unity-get-pb-paywalls)を使用してください。オンボーディングはWebView内で動作しますが、フローはデバイス上でネイティブにレンダリングされるため、よりスムーズなアニメーション、一貫したネイティブの外観、高速な読み込み、WebViewへの依存がなくなります。詳細は[フロー＆ペイウォールの取得](unity-get-pb-paywalls)と[フロー＆ペイウォールの表示](unity-present-paywalls)をご覧ください。
:::

始める前に、以下を確認してください。

1. [Adapty Unity SDK](sdk-installation-unity) 3.14.0 以降をインストール済みであること。
2. [オンボーディングを作成済み](create-onboarding)であること。
3. オンボーディングを[プレースメント](placements)に追加済みであること。

ビルダーで設定したオンボーディングは、アプリが応答できるイベントを生成します。これらのイベントへの対応方法については、以下をご覧ください。

Unity アプリのオンボーディング画面で発生するプロセスを制御・監視するには、`AdaptyOnboardingsEventsListener` インターフェースを実装してください。

:::note
SDK 4.0 では、リスナーインターフェースが C# の I-prefix 規約に従っています。`AdaptyOnboardingsEventsListener` の代わりに `IAdaptyOnboardingsEventsListener` を実装してください。メソッドは変更されていません。詳細は[移行ガイド](migration-to-unity-sdk-v4)を参照してください。
:::
