...
:::important
このガイドでは、購入・復元・プロダクト選択・レンダリングのイベント処理について説明します。ビューのクローズとリンクのオープンは、デフォルトの `flowViewDidPerformAction` 実装によって処理されます。これらをオーバーライドする場合やカスタムボタンアクションを処理する場合は、[ボタンアクションの処理に関するガイド](flutter-handle-paywall-actions)をご覧ください。
:::
...
これらのイベントの処理方法については、[フロー画面のビューを追跡する](flutter-flow-screen-views)を参照してください。

フローはユーザーが入力・選択した値も報告します。それらの読み取り方法については、[フローからのデータを処理する](flutter-flow-input)を参照してください。
...
[ペイウォールビルダー](adapty-paywall-builder)で設定されたペイウォールは、購入や復元のために追加のコードを書く必要はありません。ただし、アプリが応答できるいくつかのイベントを生成します。これらのイベントには、ボタンの押下（閉じるボタン、URL、プロダクト選択など）や、ペイウォール上で行われた購入関連アクションの通知が含まれます。これらのイベントへの対応方法については、以下をご覧ください。

:::warning

このガイドは、Adapty SDK v3.0以降が必要な**旧ビルダーで作成されたペイウォール**のみを対象としています。
