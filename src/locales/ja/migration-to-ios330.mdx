---
title: "Adapty iOS SDK v3.3への移行"
description: "より良いパフォーマンスと新しいマネタイズ機能のためにAdapty iOS SDK v3.3に移行します。"
metadataTitle: "Adapty iOS SDK v3.3への移行 | Adapty ドキュメント"
---

Adapty SDK 3.3.0はメジャーリリースであり、いくつかの改善が含まれています。ただし、移行作業が必要になる場合があります。

1. `Adapty.Configuration` を `AdaptyConfiguration` に変更する。
2. `getViewConfiguration` メソッドを `getPaywallConfiguration` に変更する。
3. SwiftUI から `didCancelPurchase` パラメータと `paywall` パラメータを削除し、`viewConfiguration` パラメータを `paywallConfiguration` に変更する。
4. `AdaptyDelegate` メソッドから `defermentCompletion` パラメータを削除して、App Store からのプロモーション アプリ内課金の処理方法を更新する。
5. `getProductsIntroductoryOfferEligibility` メソッドを削除する。
6. Adjust、AirBridge、Amplitude、AppMetrica、Appsflyer、Branch、Facebook Ads、Firebase and Google Analytics、Mixpanel、OneSignal、Pushwoosh のインテグレーション設定を更新する。
7. オブザーバーモードの実装を更新する。
...
以下のようにモバイルアプリのコードを更新してください。完全なコード例については、[AppsFlyer インテグレーションの SDK 設定](appsflyer#configure-your-app-code)をご参照ください。
