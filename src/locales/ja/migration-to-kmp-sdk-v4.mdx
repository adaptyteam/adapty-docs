...
## App Storeのプロモートアプリ内課金 \{#app-store-promoted-in-app-purchases\}

SDK 4.1では、[App StoreのプロダクトページでプロモートされたアプリURL課金](kmp-making-purchases#in-app-purchases-from-the-app-store)をiOSアプリ内で受け取れるようになりました。以前のバージョンではこのような購入は自動的に完了し、アプリ側で処理を横取りする方法はありませんでした。4.1からは購入がコードの処理を待つ形になったため、プロモート購入を扱っていなかった場合でも対応が必要です。
