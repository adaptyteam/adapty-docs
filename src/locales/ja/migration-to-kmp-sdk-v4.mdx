...
`adapty-kmp-ui`モジュールは、Compose Multiplatformレイヤー（`view.present()`）でフローやペイウォールをレンダリングする場合にのみ必要です。完全なセットアップについては、[SDKのインストール](sdk-installation-kotlin-multiplatform)を参照してください。

基盤となるネイティブAdapty SDKは両プラットフォームで4.0.0にバンプされ、自動的に解決されます — ビルドの変更は不要です。iOSのデプロイメントターゲットは**15.0**のまま、このリリースでは変更ありません。
