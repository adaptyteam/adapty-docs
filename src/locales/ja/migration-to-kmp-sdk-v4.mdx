...
:::note
`customTimers` は引き続き存在しますが、レガシーのペイウォールビルダーのペイウォールにのみ影響します。フローの[カウントダウンタイマー](flow-timer)はフロー＆ペイウォールビルダーで設定した動作で動作するため、ここで渡した値はフローに無視されます。
:::
...
IDに一致するレイアウトがない場合、フローはビュー設定なしで読み込まれます。このパラメータはオプションで、デフォルト値は`null`なので、既存の呼び出しには影響しません。
...
## オンボーディング API の廃止予定 \{#onboarding-api-deprecation\}

レガシーのオンボーディング API は、[フロー & ペイウォールビルダー](adapty-flow-builder)に移行する形で v4 で非推奨となりました。現在も動作しますが、将来のリリースで削除される予定です。オンボーディングをフロー & ペイウォールビルダーへ移行する計画を立ててください。
