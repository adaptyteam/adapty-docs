---
title: "Paddleとの初期連携"
description: "PaddleをAdaptyと連携して、サブスクリプション決済をシームレスに処理します。"
metadataTitle: "Paddle連携ガイド | Adapty ドキュメント"
keywords: ['paddle']
---

Adaptyは、[Paddle](https://www.paddle.com/)を通じて行われたWeb決済およびサブスクリプションをトラッキングすることで、web2appのサブスクリプションフローをサポートしています。

この連携では、Webで開始された購入を対象とし、ストアのアプリ内課金とあわせて、モバイルアプリのアクセスおよびアナリティクスと同期します。

以下のシナリオで役立ちます：

- アプリ内課金とウェブサイト購入のサブスクリプションデータを1つのシステムに集約する
- ウェブサイトで購入したユーザーに、モバイルアプリの有料機能へのアクセスを付与する
- すべての販売チャネルのアナリティクスとサブスクリプションデータを1つのダッシュボードで確認する

:::note
Appleは現在、米国のApp Storeアプリに外部決済システムへのリンクを含めることを許可していますが、アプリはアプリ内課金と外部オプションを並行して提供する必要がある場合があります。お住まいの地域とアプリカテゴリに適用される最新のApp Storeガイドラインをご確認ください。
:::

:::note
このインテグレーションは、Paddle のウェブ購入のトラッキングと同期に特化しています。アプリからユーザーをウェブチェックアウトに誘導する場合は、Adapty の[ウェブペイウォール](web-paywall)をご利用ください。
:::

Paddle インテグレーションを設定するには、以下の手順に従ってください。
...
## Paddle データをさらに活用する \{#get-more-from-your-paddle-data\}

:::important
Paddle イベントをインテグレーションで使用するには、ユーザーが少なくとも一度 App Store／Google Play アカウントでアプリにログインしている必要があります。
:::

Paddle とのインテグレーションが完了すると、Adapty はすぐにインサイトを提供できる状態になります。Paddle データをさらに活用するには、追加の Adapty インテグレーションを設定して Paddle イベントを転送することで、すべてのサブスクリプション分析を Adapty ダッシュボードに一元化できます。

Paddle のイベントを転送・分析するために利用できるインテグレーション:
- [Amplitude](amplitude)
- [AppsFlyer](appsflyer)
...
Amplitude、Firebase / Google Analytics、Mixpanel はプラットフォームごとに別々の認証情報を管理しています。インテグレーションページの **Paddle** フィールドに入力してください。Paddle のイベントは iOS や Android の認証情報を使用しません。
