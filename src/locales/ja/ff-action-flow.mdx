---
title: "ステップ 1. ペイウォールデータを表示するフローを作成する"
description: "Adapty のペイウォールとプロダクトデータを独自のペイウォールページで取得する FlutterFlow アクションフローを構築します。"
metadataTitle: "FlutterFlow でペイウォールデータを表示するフローを作成する | Adapty ドキュメント"
---

:::important
FlutterFlow プラグインを使用する場合、Adapty ペイウォールビルダーで作成したペイウォールは使用できません。FlutterFlow で独自のペイウォールページを実装し、Adapty に接続する必要があります。
:::

Adapty ライブラリを FlutterFlow プロジェクトの依存関係として追加したら、次は **Adapty のペイウォールとプロダクトデータを取得し、FlutterFlow でデザインしたペイウォールに表示するフロー**を構築します。

まず Adapty からペイウォールデータを受け取る必要があります。Adapty のペイウォールをリクエストし、次にそれに紐づくプロダクトをリクエストし、最後にデータが正常に受信できたかを確認します。成功した場合はプロダクトのタイトルと価格をペイウォールページに表示し、失敗した場合はエラーメッセージを表示します。

先に進む前に、以下の作業が完了していることを確認してください:

1. Adapty ダッシュボードで[ペイウォールを少なくとも1つ作成し、プロダクトを少なくとも1つ追加している](create-paywall)こと。
2. Adapty ダッシュボードで[プレースメントを少なくとも1つ作成し](create-placement)、[そこにペイウォールを追加している](add-audience-paywall-ab-test)こと。
