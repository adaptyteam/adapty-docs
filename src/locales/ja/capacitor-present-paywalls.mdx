...
description: "Capacitor アプリで Adapty を使用してユーザーにフローとペイウォールを表示します。"
metadataTitle: "Capacitor でのフローとペイウォールの表示 | Adapty ドキュメント"
---

<SDKv4>

フロービルダーでフローまたはペイウォールを作成した場合、モバイルアプリのコードでそれをレンダリングしてユーザーに表示することを心配する必要はありません。このようなフローには、表示する内容とその表示方法の両方が含まれています。

始める前に、以下を確認してください：

1. [フローまたはペイウォールを作成](create-paywall)済みであること。
2. [プレースメント](placements)に追加済みであること。
3. [フローを取得してビューを準備](capacitor-get-pb-paywalls)済みであること。

:::warning

このガイドは**フローおよびペイウォールビルダーのペイウォール**専用であり、SDK v4.0 以降が必要です。フローの表示方法は、リモートコンフィグのペイウォールとは異なります。

- **リモートコンフィグのペイウォール**を表示する場合は、[リモートコンフィグで設計したペイウォールのレンダリング](present-remote-config-paywalls-capacitor)を参照してください。
