...
description: "Adapty を使用して Capacitor アプリでフローとペイウォールをユーザーに表示します。"
metadataTitle: "Capacitor でのフローとペイウォールの表示 | Adapty ドキュメント"
---

<SDKv4>

フローまたは旧ペイウォールビルダーで作成したペイウォールがある場合、モバイルアプリのコードでレンダリングしてユーザーに表示する必要はありません。このようなフローには、表示する内容とその表示方法の両方が含まれています。

開始する前に、以下を確認してください：

1. [フロー](paywall-builder-templates)または[ペイウォール](create-paywall)を作成済みである。
2. それを[プレースメント](placements)に追加済みである。
3. [フローを取得してビューを準備](capacitor-get-pb-paywalls)済みである。

:::warning

このガイドは、SDK v4.0 以降が必要な **Adapty がレンダリングするフローとペイウォール**専用です。フローの表示プロセスは、リモートコンフィグペイウォールとは異なります。

- **リモートコンフィグペイウォール**の表示については、[リモートコンフィグで設計したペイウォールをレンダリングする](present-remote-config-paywalls-capacitor)を参照してください。
...
ペイウォールビルダーを使ってペイウォールをカスタマイズした場合、モバイルアプリのコードでレンダリングしてユーザーに表示する手間はありません。このようなペイウォールには、表示する内容と表示方法の両方が含まれています。

:::warning
このガイドは、旧ペイウォールビルダーで作成されたペイウォール専用です。リモートコンフィグペイウォールの表示方法は異なります。**リモートコンフィグペイウォール**の表示については、[リモートコンフィグで設計されたペイウォールのレンダリング](present-remote-config-paywalls-capacitor)を参照してください。
:::
