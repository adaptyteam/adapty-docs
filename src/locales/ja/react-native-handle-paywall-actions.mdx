---
title: "フローアクションへの応答 - React Native"
description: "Adapty を使用して React Native のフローやペイウォールのボタンアクションを処理し、アプリの収益化を改善します。"
metadataTitle: "フローアクションへの応答 - React Native | Adapty ドキュメント"
...
Adapty のフロー & ペイウォールビルダーでフローを構築する場合、ボタンを適切に設定することが重要です。

1. [ビルダーにボタンを追加](paywall-buttons)して、既存のアクションを割り当てるか、カスタムアクション ID を作成します。
2. 割り当てた各アクションを処理するコードをアプリに記述します。

このガイドでは、カスタムアクションおよび既存アクションをコードで処理する方法を説明します。

:::warning
**購入、復元、フロー／ペイウォールのクローズ、URL オープンは自動的に処理されます。** デフォルトの動作を設定するか、カスタムアクションに対する応答を実装できます。
:::
...
モーダル表示の場合は、カスタムアクションハンドラーを実装してください：
...
2. 割り当てた各アクションを処理するコードをアプリ内に記述します。

このガイドでは、カスタムアクションおよびあらかじめ定義されたアクションをコードで処理する方法について説明します。

:::warning
**購入、復元、ペイウォールのクローズ、URLの開封のみが自動的に処理されます。** その他のボタンアクションはすべて、アプリのコードで適切なレスポンス実装が必要です。
:::
