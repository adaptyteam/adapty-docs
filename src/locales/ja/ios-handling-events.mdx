...
description: "iOSアプリでフローとペイウォールのイベントを処理する方法。"
...
| **flowConfiguration**                     | 必須     | フローまたはペイウォールのビジュアル詳細を含む `AdaptyUI.FlowConfiguration` オブジェクト。詳細は[フローとペイウォールの取得](get-pb-paywalls)を参照してください。                                                                                  |
| **didFinishPurchase**                     | 必須     | `Adapty.makePurchase()` が正常に完了したときに呼び出されます。フローは自動的に閉じないため、ここでプレゼンテーションバインディングを `false` に設定するか、何もせずにフローを購入後も継続させてください。                                           |
...
| **didReceiveError**                       | 必須     | フローでレンダリングエラーまたはフロースクリプトのランタイムエラー（例：JavaScript の例外、`AdaptyUIError` コード `4105`）が発生したときに呼び出されます。レンダリングエラーの場合は[Adapty サポートにお問い合わせください](mailto:support@adapty.io)。 |
...
| **didPerformAction**                      | 任意     | ユーザーがボタンをクリックしたときに呼び出されます。`close` と `openURL` の2つのアクション ID が事前定義されており、それ以外はビルダーで設定できるカスタムアクションです。                                                                           |
| **didSelectProduct**                      | 任意     | ユーザーまたはシステムによって購入するプロダクトが選択されたときに呼び出されます。                                                                                                                                                                |
...
| **didFailLoadingProducts**                | 任意     | プロダクトの読み込み中にエラーが発生したときに呼び出されます。`true` を返すと読み込みを再試行します。                                                                                                                                              |
...
UIKit アプリでは、`AdaptyFlowControllerDelegate` プロトコルを通じてイベントを処理します。`AdaptyFlowController` と `AdaptyFlowControllerDelegate` のセットアップ方法については、[フローとペイウォールの表示 - iOS](ios-present-paywalls) を参照してください。

このプロトコルには13のメソッドが宣言されており、そのうち4つはデフォルト実装がなく、準拠する際に必ず実装する必要があります：`didFinishPurchase`、`didFailPurchase`、`didFinishRestoreWith`、`didFailRestoreWith`です。残りのメソッドはデフォルトの何もしない実装を持っており、カスタム動作が必要な場合にオーバーライドできます。メソッドは以下に目的別にグループ化されています。
...
`didFinishPurchase` と `didFailPurchase` にはデフォルトの実装がないため、必ず実装する必要があります。購入が成功してもコントローラーは自動的に閉じられません。適切なタイミングで `controller.dismiss(animated:)` を呼び出すか、複数画面のフローで購入後も続けて進む場合はそのまま何もしなくても構いません。
...
### Web決済ナビゲーション \{#web-payment-navigation\}
...
Webによる決済ナビゲーションが完了したとき（成功・失敗を問わず）に呼び出されます。
...
これらのイベントの処理方法については、[フロー画面のビューを追跡する](ios-flow-screen-views)を参照してください。

フローはユーザーが入力・選択した値もレポートします。詳細は[フローからのデータを処理する](ios-flow-input)を参照してください。
...
:::important
このガイドでは、購入・復元・プロダクト選択・ペイウォールレンダリングに関するイベント処理を説明します。ボタン操作（ペイウォールを閉じる、リンクを開くなど）の処理も実装する必要があります。詳細は[ボタンアクションの処理に関するガイド](handle-paywall-actions)を参照してください。
:::

[ペイウォールビルダー](adapty-paywall-builder)で設定されたペイウォールは、購入や復元のために追加のコードを必要としません。ただし、アプリが反応できるいくつかのイベントが生成されます。それらのイベントには、ボタンの押下（閉じるボタン、URL、プロダクト選択など）や、ペイウォールで行われた購入関連アクションの通知が含まれます。これらのイベントへの対応方法については、以下をご覧ください。

このガイドは、Adapty SDK v3.0以降が必要な**旧ビルダーで作成されたペイウォール**のみを対象としています。

:::tip

実際のモバイルアプリへのAdapty SDK統合の具体例を見たい場合は、[サンプルアプリ](sample-apps)をご確認ください。ペイウォールの表示、購入処理、その他の基本的な機能を含む完全なセットアップを紹介しています。
...
モバイルアプリのペイウォール画面で発生するプロセスを制御・監視するには、`AdaptyPaywallControllerDelegate` のメソッドを実装します。
...
この場合はペイウォール画面を閉じることをお勧めします。

Observer モードでは呼び出されません。詳細は [iOS - Observer モードでペイウォールビルダーのペイウォールを表示する](ios-present-paywall-builder-paywalls-in-observer-mode) を参照してください。
...
通常の状況ではこのようなエラーは発生しないはずです。もし遭遇した場合は、ぜひご連絡ください。
