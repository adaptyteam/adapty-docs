...
## プロファイルの更新は連続して行うと制限される \{#repeated-profile-updates-are-throttled\}

インテグレーションの識別子やプロファイルの属性は、アクティベーション完了後に一度だけ送信し、値が変わったときのみ更新してください。アプリがプロファイルを更新しすぎると、Adapty バックエンドはそのエンドポイントへのリクエストをスロットリングします。スロットリングが発生すると、リクエストは [`#2004 serverError`](ios-sdk-error-handling#network-errors) で失敗し、サーバーが指定した再試行インターバルが終了するまで、SDK はそのエンドポイントへの追加呼び出しをブロックします。スロットリングはエンドポイント単位で行われるため、他の SDK の呼び出しは引き続き機能します。

## `updateProfile` 直後のプレースメント取得 \{#fetching-a-placement-right-after-updateprofile\}

カスタム属性を設定するために `updateProfile` を呼び出した直後に同じセッションでプレースメントを取得（`getPaywall`）する場合は、取得前に `updateProfile` の完了を await してください。同時に実行すると、SDK がまだ反映されていない属性からプレースメントのセグメントハッシュを計算してしまい、フェッチが解決するまでに再試行のラウンドトリップが発生することがあります。再試行自体が失敗した場合は、[iOS SDK のエラーハンドリング](ios-sdk-error-handling#general-errors)を参照してください。
