...
`adapty-bom` のバージョンを `4.0.0-beta.2` に設定してプロジェクトを同期してください。BOM が対応する `android-sdk` と `android-ui` のバージョンを自動的に解決します。依存関係の宣言については、[Adapty SDK のインストール](sdk-installation-android)を参照してください。
