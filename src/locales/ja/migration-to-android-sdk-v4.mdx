---
title: "Adapty Android SDK を v4.0 へ移行する"
description: "ペイウォール API をフロー API に置き換えることで Adapty Android SDK v4.0 へ移行します。フロー API は従来のビルダーペイウォールに加え、フローもレンダリングします。"
metadataTitle: "Adapty Android SDK v4.0 への移行 | Adapty ドキュメント"
---

Adapty Android SDK 4.0 ではフローが導入され、ペイウォール API の名称が変更されました。新しい API はフローに対応しており、旧ビルダーのペイウォールも引き続きそのまま動作します。Adapty ダッシュボード側での設定変更は不要です。
...
`adapty-bom` のバージョンを `4.0.1`（またはそれ以降）に設定し、プロジェクトを同期してください。BOM が対応する `android-sdk` および `android-ui` のバージョンを自動的に解決します。依存関係の宣言については、[Adapty SDK のインストール](sdk-installation-android) を参照してください。
...
- **オンボーディングは非推奨になりました。** `AdaptyUI.getOnboardingView` と `AdaptyUI.getOnboardingConfiguration` は 4.0 で `@Deprecated` としてマークされています — オンボーディングを[フロー & ペイウォールビルダー](adapty-flow-builder)で構築したフローに移行してください。
...
フェッチの戻り値の型が `AdaptyPaywall` から `AdaptyFlow` に変わり、設定ローダーは `AdaptyUI.getViewConfiguration` から `AdaptyUI.getFlowConfiguration`（`AdaptyUI.LocalizedViewConfiguration` の代わりに `AdaptyUI.FlowConfiguration` を返す）に改名されました。`locale` パラメーターはフェッチ呼び出しから `getFlowConfiguration` に移動しました。`AdaptyFlow` は `paywalls`（フローのペイウォールバリエーション、`AdaptyFlowPaywall` の値として）も公開しています。ウェブペイウォールのメソッドはこれらのいずれかを受け取ります。
...
`locale` は `getFlowConfiguration` でオプションのままです。省略すると、ビューは `en` でレンダリングされます。フローに `en` がない場合は、フローのデフォルトのローカライズでレンダリングされます。詳しくは[ローカライズとロケールコード](android-localizations-and-locale-codes)をご覧ください。
...
フォールバックファイルの形式は [SDK v4 で変更されました](fallback-flows)。新しいファイルを **[Placements](https://app.adapty.io/placements)** > **Fallbacks** からダウンロードして、アプリにバンドルしてください。

## フローのビュー数トラッキング \{#tracking-flow-views\}

### logShowPaywall → logShowFlow

`logShowPaywall` は `logShowFlow` に名前が変更され、`AdaptyPaywall` の代わりに `AdaptyFlow` を受け取るようになりました。イベントは引き続き同じバリエーションに対して記録されるため、既存のファネルや A/B テストの指標はダッシュボードを変更することなく引き続き機能します。
...
v3 と同様に、Adapty がレンダリングするフローやペイウォールを表示する際は、このメソッドを呼び出す必要はありません — Adapty がそれらのビューを自動的にトラッキングします。
...
定義済みのソース: `AdaptyAttributionSource.APPLE_ADS`、`.ADJUST`、`.APPSFLYER`、`.BRANCH`、`.TENJIN`。その他のソースは、文字列から構築します: `AdaptyAttributionSource("your_source")`。

:::note
SDK 4.1 では、このメソッド名が `updateExternalAttribution` に、型名が `AdaptyExternalAttributionProvider` に変更されています。旧名称は 4.1 で非推奨化なしに削除されています。4.1 に直接移行する場合は、[v4.1 への移行](migration-to-android-sdk-41)を参照してください。
:::
