---
title: "App Store 連携の設定"
description: "シームレスなサブスクリプション追跡のために App Store との接続を設定します。"
...
このセクションでは、iOSアプリのApp StoreとAdaptyを接続する方法について説明します。これは、サブスクリプションアナリティクスの表示と購入の検証に必要です。初回オンボーディング中、またはAdapty ダッシュボードの **App Settings** から後で設定を完了できます。

このページの各認証情報は、データ交換の異なる部分を解放します。[Adaptyがストアとデータをどのように同期するか](store-sync#what-adapty-reads-from-the-stores)では、各認証情報が扱うデータを確認できます。

モバイルアプリとAdaptyの連携はオンボーディング時に設定済みかもしれませんが、後から **App settings** で変更することもできます。

:::danger
設定の変更は、Adapty SDKをインストールしたモバイルアプリがリリースされるまでのサンドボックスフェーズ中であれば安全に行えます。リリース後に変更すると、アプリの購入フローが壊れる可能性があります。

App Store Connectでキーが無効化、削除、またはローテーションされた場合は、気づいた時点ですぐに交換用のキーをアップロードしてください。アップロードするまで、そのキーに依存するすべての機能が停止したままになります。詳しくは[アップデートが届かなくなった場合](store-sync#if-updates-stop-arriving)を参照してください。
:::
...
:::note
App Store Connect には、フィールドがどちらも **Issuer ID** と **Key ID** と表示される2種類のキーがあります。手順2〜3では、**Integrations** → **In-App Purchase** タブの **In-App Purchase** キーを使用します。**Team keys** タブの App Store Connect API キーは別のキーで、それ自体のIDを持っています。これは[手順6](#step-6-add-app-store-connect-api-key)で追加します。
:::
...
:::note
これは、[ステップ 2〜3](#step-2-provide-issuer-id-and-key-id) で追加したアプリ内課金キーとは別のキーペアです。**Team keys** タブでキーを生成し、そこから **Issuer ID** と **Key ID** をコピーしてください。アプリ内課金キーの値はここでは使用できません。
:::

1. App Store Connectで、[**Users and Access > Integrations > Team keys**](https://appstoreconnect.apple.com/access/integrations/api) を開き、**+** をクリックします。
