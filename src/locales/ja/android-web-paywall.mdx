...
1. `openWebPaywall(product)` — ペイウォールに基づいてURLを生成し、プロダクトのデータもURLに追加します。
2. `openWebPaywall(paywall)` — ペイウォールに基づいてURLを生成しますが、プロダクトのデータはURLに追加しません。SDK v4 では `AdaptyFlowPaywall`（フェッチしたフローのペイウォールバリアント）を受け取ります。インデックスアクセスの前に `flow.paywalls` が空でないことを確認してください（例：`flow.paywalls[0]`）。
:::
