---
title: "AppsFlyer"
description: "高度なモバイルアトリビューショントラッキングのために AppsFlyer と Adapty を連携する。"
...
[AppsFlyer](https://www.appsflyer.com/) は、モバイルアトリビューションとマーケティング分析の主要プラットフォームです。マーケティングキャンペーンのデータを収集・整理するサードパーティサービスとして、各キャンペーンのパフォーマンスを一元管理できます。

Adapty は、ストアからの[サブスクリプションイベント](events)を一か所で追跡できる完全なデータセットを提供しています。Adapty を使えば、サブスクライバーの行動を簡単に把握し、好みを理解した上で、ターゲットを絞った効果的なコミュニケーションを取ることができます。このインテグレーションにより、AppsFlyer でサブスクリプションイベントを追跡し、キャンペーンが生み出す収益を正確に分析できます。

Adapty と AppsFlyer のインテグレーションは、主に 2 つの方法で機能します。

1. **AppsFlyer からアトリビューションデータを受信する**
   [アプリのコードで AppsFlyer のアトリビューションを Adapty に送信するよう設定](appsflyer#connect-your-app-to-appsflyer)すると、Adapty は AppsFlyer からアトリビューションデータを受信し始めます。このデータはユーザーのプロファイルページで簡単に確認できます。
...
2. **AppsFlyer へのサブスクリプションイベントの送信**
   Adapty は、インテグレーションで設定されたすべてのサブスクリプションイベントを AppsFlyer に送信できます。これにより、AppsFlyer ダッシュボード内でこれらのイベントを追跡できるようになります。このインテグレーションは、広告キャンペーンの効果を評価するのに役立ちます。
...
### AppsFlyer とアプリを連携する \{#connect-your-app-to-appsflyer\}

上記の手順を完了したら、`updateExternalAttribution` メソッド（SDK バージョン 4.1 以前は `updateAttribution`）を呼び出してアトリビューションデータを保存し、`Adapty.setIntegrationIdentifier()` を使ってインテグレーション識別子を設定してください。

AppsFlyer SDKを初期化し、そのUIDコールバックを受け取ってからAdaptyでユーザーを識別してください。そうしないと、`appsflyer_id`がアクティベーション時に作成された一時的な匿名Adaptyプロファイルに紐付けられ、識別済みプロファイルに引き継がれないことがあります。その場合、AppsFlyerの収益転送が無音で失敗します。
