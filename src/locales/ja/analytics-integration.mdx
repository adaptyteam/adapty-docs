...
Adaptyは、[サブスクリプションイベント](events)を[Amplitude](amplitude)、[Mixpanel](mixpanel)、[AppMetrica](appmetrica)などの分析サービスに送信します。また、[webhook](webhook)連携を使用してサーバーにイベントを送信することもできます。イベントの送信はAdaptyが自動で行うため、手動で送信する必要はありません。Adapty ダッシュボードで連携を設定するだけです。

Adaptyは以下のサードパーティ分析サービスとの連携をサポートしています：
...
:::note
お使いのアナリティクスプロバイダーが見当たりませんか？

ぜひお知らせください！[機能リクエストを作成する](https://adapty.featurebase.app/en?b=6979f233ebd3cffd4f425ba0)と、追加を検討します。
:::

## イベントのプロパティ \{#event-properties\}

WebhookイベントはJSON形式で送信されます。すべてのイベントは同じ構造に従いますが、フィールドはイベントタイプ、ストア、および設定内容によって異なります。
...
| **currency**                  | str           | ローカル通貨（デフォルトは USD）。                             |
| **price_usd**                 | float         | Apple/Google の手数料控除前のプロダクト価格。収益。           |
| **proceeds_usd**              | float         | Apple/Google の手数料控除後のプロダクト価格。純収益。         |
| **net_revenue_usd**           | float         | USD での純収益（Apple/Google の手数料および税金控除後の収入）。空の場合あり。 |
| **price_local**               | float         | ローカル通貨での Apple/Google の手数料控除前のプロダクト価格。収益。 |
| **proceeds_local**            | float         | ローカル通貨での Apple/Google の手数料控除後のプロダクト価格。純収益。 |
...
| **vendor_product_id**         | str           | Apple App Store、Google Play ストア、または Stripe のプロダクト ID。 |
| **base_plan_id**              | str           | Google Play ストアの[ベースプラン ID](https://support.google.com/googleplay/android-developer/answer/12154973) または Stripe の[価格 ID](https://docs.stripe.com/products-prices/how-products-and-prices-work#use-products-and-prices)。 |
| **event_datetime**            | ISO 8601 date | イベントの日時。                                              |
| **store**                     | str           | _app_store_ または _play_store_。                            |
| **trial_duration**            | str           | トライアル期間の日数。"{} days" の形式で送信されます（例：「7 days」）。 |
...
| **consecutive_payments**      | int           | ユーザーが中断なくサブスクリプションを継続している期間数。現在の期間を含む。 |
| **rate_after_first_year**     | bool          | 1 年間継続して更新された後に、サブスクリプションが低率手数料（通常 15%）の対象となるかを示すブール値。手数料率はプログラムの適格性と国によって異なります。詳細は[ストア手数料と税金](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue)をご覧ください。 |
| **promotional_offer_id**      | str           | Adapty ダッシュボードのプロダクトセクションに記載されているプロモーションオファーの ID。 |
...
| **profile_country**           | str           | プロファイルの IP アドレスをもとに Adapty が判定した国。      |
| **profile_total_revenue_usd** | float         | プロファイルの総収益（返金分を含む）。                        |
| **variation_id**              | uuid          | 購入が行われたペイウォールの一意の ID。                       |
| **access_level_id**           | str           | 有料アクセスレベルの ID。                                     |
| **is_active**                 | bool          | プロファイルで有料アクセスレベルが有効かどうかを示すブール値。 |
...
| **renewed_at**                | ISO 8601 date | 有料アクセスの現在の期間が最後に開始した日時。初回更新までは `activated_at` と同じ値になります。 |
| **expires_at**                | ISO 8601 date | 有料アクセスが終了する日時。                                  |
...
:::warning
外部アナリティクスをブロックすると、カスタム[webhook](webhook)を含む**すべて**のインテグレーションへのイベント配信が、このユーザーに対して停止されます。バックエンドがwebhookイベントを使ってこのユーザーのアクセスを管理している場合は使用しないでください。

この設定は遡及的には適用されません。設定前に作成されたイベントは引き続き配信されます。また、Adapty自身のアナリティクスには影響しません。ユーザーのトランザクションは引き続きダッシュボードのチャートに反映されます。
:::
