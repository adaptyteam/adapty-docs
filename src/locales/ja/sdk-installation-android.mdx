...
description: "サブスクリプションアプリ向けに、Android に Adapty SDK をインストールするステップバイステップガイドです。"
...
最小 SDK 要件: `minSdkVersion 21`

:::info
デフォルトでは、Adapty SDK 4.0.2 以降は Google Play Billing Library v8 で動作します。それ以前のバージョンの SDK はデフォルトで v7.0.0 で動作し、Billing Library v8.x までに対応しています。より新しいバージョンを強制使用したい場合は、手動で[依存関係を追加](https://developer.android.com/google/play/billing/integrate#dependency)してください。

Billing Library のバージョンとの互換性は、Adapty がそのバージョンで Google が導入したすべての機能をサポートしていることを意味しません。新しい Google Play の課金機能を採用する前に、[Google Play のプロダクト](android-products) をご確認ください。
:::
...
Adapty SDK バージョン 4.1 以降、[Adapty アトリビューション](user-acquisition)はデフォルトで無効になっています。Adapty モジュールを有効化する際に、`adaptyAttributionEnabled` を `true` に設定すると、SDK がインストールを登録してインストールの詳細をアプリに配信できます。
...
`AdaptyUI.configureMediaCache` を使用して、デフォルトのキャッシュサイズと有効期間を上書きできます。この設定はオプションです。このメソッドを呼び出さない場合は、デフォルト値（ディスクサイズ 100MB、有効期間 7 日間）が使用されます。
