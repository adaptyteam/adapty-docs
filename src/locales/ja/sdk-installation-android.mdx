...
description: "サブスクリプションアプリ向けに、Android へ Adapty SDK をインストールする手順を解説します。"
...
- **Core Adapty**: AdaptyをアプリでGAするために必要な基本SDKです。
- **AdaptyUI**: このモジュールは、旧ビルダーのペイウォールに加え、フローもレンダリングします。AdaptyUIはコアモジュールと同時に自動的に有効化されます。
...
SDKの最小要件：`minSdkVersion 21`

:::info
Adapty SDKにはGoogle Play Billing Libraryが同梱されているため、アプリに別途追加する必要はありません。

3.xラインの3.17.3以降、および4.xラインの4.0.2以降では、Adapty Android SDKはデフォルトでBilling Library v8を使用します。3.15.0〜3.17.2および4.0.0〜4.0.1のバージョンはv7を使用しており、ビルド内の別の依存関係がBilling Libraryをv8に引き上げた場合でも動作します。

Billing Library のバージョンとの互換性は、Adapty がそのバージョンで Google が導入したすべての機能をサポートすることを意味しません。新しい Google Play の課金機能を導入する前に、[Play Store でのプロダクト](android-products)を参照してください。
:::
...
:::important
他のAdapty SDKメソッドを呼び出す前に、`Adapty.activate`の完了を待ってください。完全なシーケンスについては、[Android SDKの呼び出し順序](android-sdk-call-order)をご覧ください。
:::

次に、アプリでペイウォールを設定します：

- [Flow & ペイウォールビルダー](adapty-flow-builder)を使用する場合は、[クイックスタート](android-quickstart-paywalls)に従ってください。
...
[フロー & ペイウォールビルダー](adapty-flow-builder)を使用する場合、AdaptyUI モジュールが必要です。コアモジュールを有効化すると自動的に有効化されるため、追加の操作は不要です。
