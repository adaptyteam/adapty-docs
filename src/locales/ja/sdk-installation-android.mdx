...
:::info
3.x ラインの 3.17.3 以降、4.x ラインの 4.0.2 以降では、Adapty Android SDK はデフォルトで Billing Library v8 を使用します。バージョン 3.15.0〜3.17.2 および 4.0.0〜4.0.1 は v7 を使用しており、ビルド内の別の依存関係が Billing Library を v8 に引き上げた場合にも動作します。

特定の Billing Library バージョンとの互換性は、Adapty がその中で Google が導入したすべての機能をサポートすることを意味するわけではありません。新しい Google Play の課金機能を採用する前に、[Play Store のプロダクト](android-products) を参照してください。
:::
