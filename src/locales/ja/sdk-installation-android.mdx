...
最小SDKバージョン要件: `minSdkVersion 21`

<InstallationPrereqs />

## 依存関係 \{#dependencies\}

Adapty SDK は以下の Google Play Billing Library バージョンに対応しています：

| Adapty SDK バージョン | Billing Library バージョン |
|---|---|
| 4.0.2 以降 | v8 |
| 4.0.0〜4.0.1 | デフォルトは v7、他の依存関係により v8 に上がる場合あり |
| 3.x ライン上の 3.17.3 以降 | v8 |
| 3.15.0〜3.17.2 | デフォルトは v7、他の依存関係により v8 に上がる場合あり |

:::note
特定の Billing Library バージョンに対応しているからといって、Google がそのバージョンで導入したすべての機能を Adapty がサポートしているわけではありません。新しい Google Play の課金機能を採用する前に、[Play ストアのプロダクト](android-products) をご確認ください。
:::

## Adapty SDK のインストール \{#install-adapty-sdk\}

依存関係のセットアップ方法を選択してください：
