...
description: "Developer CLI を使って Adapty アカウントをエンドツーエンドでセットアップする — アプリの作成からライブプレースメントまで、数コマンドで完了。"
metadataTitle: "Developer CLI クイックスタート | Adapty ドキュメント"
keywords: ['cli', 'developer cli', 'mcp']
---

:::link
AI アシスタントを使っていますか？LLM が CLI を操作できるよう支援する [Adapty CLI スキル](https://github.com/adaptyteam/adapty-cli/tree/main/skills/adapty-cli) が利用可能です。
:::
...
同じプロダクトを Stripe または Paddle を通じてウェブでも販売している場合は、同じコマンドにそれらの ID を追加してください。詳細は完全なリファレンスの [products create](developer-cli-reference#adapty-products-create) を参照してください。
