...
Adapty CLI を使えば、アプリの設定をすべてコマンドラインから行えます。ターミナルツールや [MCP クライアント](https://github.com/adaptyteam/adapty-cli/tree/main/skills/adapty-cli)を好む場合は、[ダッシュボードのクイックスタート](integrate-payments)の代わりに使用してください。

:::note
Adapty を App Store Connect および Google Play に接続するには、ダッシュボードでの初回セットアップが一度だけ必要です。詳細はステップ 3 で説明します。
:::

完了すると、アプリ、アクセスレベル、プロダクト、ペイウォール、プレースメントがすべて [Adapty ダッシュボード](https://app.adapty.io)に表示されます。

Claude Cowork で CLI を実行する場合は、開始前に Adapty のドメインを許可してください。詳しくは[Cowork で CLI を実行する](#run-the-cli-in-cowork)をご覧ください。
...
## Cowork で CLI を実行する \{#run-the-cli-in-cowork\}

Claude Cowork はコマンドを、許可リストに登録されたドメインにのみアクセスできるサンドボックス内で実行します。Adapty を許可しない限り、CLI はサインインもアカウントの読み取りもできません。自分のターミナルから CLI を実行する場合は、このセクションをスキップしてください。

`adapty.io` と `*.adapty.io` の両方を許可してください。ほとんどの許可リスト実装では、ワイルドカードは頂点ドメイン（apex domain）をカバーしません。CLI 自体は `api.adapty.io` にアクセスし、作業中に Adapty のドキュメントを読むエージェントは頂点ドメインにアクセスします。
...
2. **Code execution and file creation** をオンにします。これをオンにするまで、**Allow network egress** ブロックは表示されません。
3. **Allow network egress** をオンにします。
4. **Domain allowlist** で **Package managers only** を選択します。
5. **Additional allowed domains** に `adapty.io` を追加し、次に `*.adapty.io` を追加します。
...
もう一方のアクセスモード、**All domains** を選択すると、サンドボックスはあらゆるサイトにアクセスできるようになります。この場合、Adapty のドメインを追加する必要はありません。

### 企業向けワークスペース \{#corporate-workspaces\}

ワークスペースの管理者が許可リストを管理しています。両ドメインをワークスペースに追加するよう管理者に依頼してください。
...
- [Adapty SDK を統合する](quickstart-sdk) — アプリに SDK を追加して、プレースメントを取得・表示します。
- 異なるユーザー [セグメント](segments) を異なるペイウォールにルーティングする — コマンドリファレンスの [`placements update`](developer-cli-reference#adapty-placements-update) と [`segments list`](developer-cli-reference#adapty-segments-list) を参照してください。
