...
:::warning
このセクションは[オブザーバーモード](observer-vs-full-mode)のみに適用されます。オブザーバーモードで作業していない場合は、[フローとペイウォールの表示](react-native-present-paywalls)を参照してください。
:::

:::info
この機能には Adapty React Native SDK 4.0 以降が必要です。以前はネイティブの iOS および Android SDK のみで利用可能でした。アップグレードするには[マイグレーションガイド](migration-to-react-native-sdk-v4)を参照してください。
:::
