...
<TabItem value="flow-builder" label="Flow Builder を使用する" default>

:::important
Flow Builder は現在、iOS、Android、React Native、および Capacitor SDK v4 以降をサポートしています。その他のプラットフォームへの対応は近日公開予定です。
:::

フローとは、プロダクトが直接埋め込まれた1つまたは複数の画面です。[Flow Builder](adapty-flow-builder) でデザインできます — コードは不要です。
Adapty SDK はフローを各プラットフォームでネイティブにレンダリングします。アプリが `getFlow` を呼び出すと、SDK が画面を表示し、購入を処理し、イベントをレポートします。別途 UI コードを書く必要も、ペイウォールを並行してメンテナンスする必要もありません。
