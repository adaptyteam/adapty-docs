...
- **フロー & ペイウォールビルダー（推奨）**: 購入フロー全体をノーコードで構築できるビジュアルエディター。Adapty SDK がネイティブにレンダリングするため、UI コードを書く必要はありません。
- **マニュアルペイウォール**: ペイウォールを作成してプロダクトを紐づけ、UI はアプリコードで自分実装します。
- **Adapty ペイウォールビルダー（レガシー）**: ノーコードのペイウォールエディター。

どちらの方法も最終的には同じです。作成したものを[プレースメント](placements)に紐づけます。プレースメントは、アプリが実行時に呼び出し、適切なユーザーに適切なコンテンツを取得するための仕組みです。

<Tabs groupId="purchase-setup" queryString>
<TabItem value="flow-builder" label="フローとペイウォールビルダーを使用する" default>

:::important
フローには Adapty SDK v4.0 以降が必要です。
:::

フローとは、プロダクトが直接埋め込まれた1つ以上の画面のことです。[フロー＆ペイウォールビルダー](adapty-flow-builder)でデザインでき、コードは不要です。

Adapty SDK は各プラットフォームでフローをネイティブにレンダリングします。アプリが `getFlow` を呼び出すと、SDK が画面を表示し、購入を処理し、イベントをレポートします。別途 UI コードを書く必要も、ペイウォールを個別に管理する必要もありません。

## 1. フローを作成する \{#1-build-the-flow\}

1. Adapty のメインメニューから [**Flows**](https://app.adapty.io/flows) に移動します。
2. **Create flow** をクリックしてフローをデザインします。

[Adapty フロー & ペイウォールビルダー](adapty-flow-builder) について詳しく学びましょう。

<ZoomImage id="new-flow.webp" />

以下のテンプレートガイドでは、よくあるパターンをステップごとに説明しています：

<CustomDocCardList ids={['basic-paywall-screen', 'paywall-with-tabs', 'paywall-features-per-product', 'onboarding-flow-tutorial']} />

フローを保存して公開したら、プレースメントへの組み込みに進みましょう。
