...
description: "Unity アプリでフローとペイウォールのイベントを処理します。"
...
[Flow & Paywall Builder](adapty-flow-builder)で設定したフローとペイウォールは、購入や復元のために追加のコードは必要ありません。ただし、アプリが対応できるいくつかのイベントが発生します。これらのイベントには、ボタン操作（閉じるボタン、URL、プロダクト選択など）や、購入関連のアクションに関する通知が含まれます。これらのイベントへの対応方法については、以下をご覧ください。
...
詳細は[フロー画面ビューのトラッキング](unity-flow-screen-views)を参照してください。

また、フローはユーザーが入力・選択した値も報告します。詳細は[フローからのデータ処理](unity-flow-input)を参照してください。
...
[ペイウォールビルダー](adapty-paywall-builder)で設定されたペイウォールは、購入や復元のために追加のコードを必要としません。ただし、アプリが応答できるイベントが発生します。これらのイベントには、ボタン操作（閉じるボタン、URL、プロダクトの選択など）や、ペイウォール上での購入関連アクションに関する通知が含まれます。これらのイベントへの対応方法については、以下をご覧ください。

:::warning
このガイドは、**旧ビルダーで作成されたペイウォール**のみを対象としており、Adapty SDK v3.3.0 以降が必要です。
:::
