...
:::important
**このガイドは、カスタムペイウォールを実装する開発者向けです。** 購入を最も簡単に有効にする方法をお探しの場合は、[Adapty フロー & ペイウォールビルダー](react-native-quickstart-paywalls)をご利用ください。フロー & ペイウォールビルダーでは、ノーコードのビジュアルエディターでフローを作成でき、Adapty がすべての購入ロジックを自動的に処理します。また、アプリを再公開することなく、さまざまなデザインをテストできます。
:::
