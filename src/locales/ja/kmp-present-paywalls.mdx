...
フローや旧ペイウォールビルダーで作成したペイウォールは、ユーザーに表示するためにモバイルアプリのコードでレンダリングする必要はありません。このようなフローやペイウォールには、表示する内容と表示方法の両方が含まれています。

:::warning

このガイドは、Adapty がレンダリングするフローとペイウォールを対象としています。リモートコンフィグペイウォールや [Observer モード](observer-vs-full-mode) では手順が異なります。
...
ペイウォールビルダーを使ってペイウォールをカスタマイズした場合、モバイルアプリのコードでそれをレンダリングしてユーザーに表示することを気にする必要はありません。このようなペイウォールには、ペイウォール内に表示すべき内容とその表示方法の両方が含まれています。

:::warning

このガイドは**旧ビルダーで作成されたペイウォール**専用です。リモートコンフィグペイウォールと[オブザーバーモード](observer-vs-full-mode)では、ペイウォールの表示方法が異なります。
