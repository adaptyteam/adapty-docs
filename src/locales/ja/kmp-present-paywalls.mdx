...
KMP インターフェースのデフォルトメソッドは Swift では `@required` になるため、Swift から `AdaptyUIFlowsEventsObserver` を直接実装することはできません。まず `iosMain` でオープンな基底クラスを宣言してください：
...
次に、Swift でサブクラス化し、必要なメソッドのみをオーバーライドします:
