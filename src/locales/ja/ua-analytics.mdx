...
Adapty Attribution は、キャンペーンのパフォーマンスとユーザー行動を測定するための包括的な**指標**を提供します。これらの指標は標準値として利用できるほか、主要な指標はユーザーグループの時系列分析のための**コホート指標**としても提供されます。

:::note
[Stripe](ua-stripe)、[Paddle](ua-paddle)、[FunnelFox](ua-funnelfox) などのウェブ購入は、Adapty のメイン分析ではなく、Adapty Attribution 分析にのみ表示されます。アプリのインストールイベントがないため、Adapty は初回トライアル開始日または購入日でコホートを分類します。その日付を含む日付範囲を選択してください。プラットフォームでフィルタリングする場合は、ウェブファネルを含めてください。そうしないとウェブ購入が非表示になります。
:::

指標の完全なリストは[こちら](ua-metrics)をご覧ください。
