...
- **レート制限**: APIキーごとに適用され、アプリごとに割り当てられるため、すべてのアプリに共通する数値はありません。詳細は[レート制限](https://adapty.io/docs/ja/ss-authorization.md#rate-limits)を参照してください。これはバースト制限ではなく持続レートであり、一部のエンドポイントはさらに低い上限が設定されています。[createVirtualCurrencyTransaction](https://adapty.io/docs/ja/api-adapty/operations/createVirtualCurrencyTransaction.md)はデフォルトでアプリごとに1分あたり600リクエストまで許可されています。`429`が返された場合はバックオフ付きでリトライし、`Retry-After`ヘッダーが存在する場合はその値を守るようエージェントに設定してください。
- **アプリ固有のキー**: 各キーは1つのアプリにのみ有効です。アプリごとに対応するキーを使用してください。
- **識別子の必須指定**: すべてのリクエストに`adapty-customer-user-id`または`adapty-profile-id`のいずれかが必要です。
