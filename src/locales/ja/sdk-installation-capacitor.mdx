...
iOSのビルドには **Xcode 26** 以降が必要です。ネイティブ iOS SDK は Swift tools 6.2 でビルドされています。Android の最低 SDK バージョンは 24 です。

:::info
Adapty SDK には Google Play Billing Library が同梱されているため、アプリに別途追加する必要はありません。

3.17.0 以降、Adapty Capacitor SDK はデフォルトで Billing Library v8 を使用します。3.15.0〜3.16.3 はv7を使用しますが、Android ビルドの他の依存関係が Billing Library を v8 に引き上げた場合も動作します。

Compatibility with a Billing Library version doesn't mean Adapty supports every feature Google introduced in it. Before you adopt a new Google Play billing capability, see [Google Play ストアのプロダクト](android-products).
:::
...
iOS では、Adapty SDK はネイティブの Adapty SDK を **Swift Package Manager のみ** 経由で取得します。CocoaPods の podspec は削除されました（[CocoaPods のスペックリポジトリは 2026 年 12 月に読み取り専用になります](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)）。アプリの iOS プロジェクトは Capacitor の SPM 連携を使用する必要があります：
...
- 既存のアプリの場合は、[Capacitor の既存プロジェクトでの SPM 使用ガイド](https://capacitorjs.com/docs/ios/spm#using-spm-in-an-existing-capacitor-project)に従って、iOS プロジェクトを CocoaPods から SPM に移行してください。

v3 からアップグレードする場合は、API の変更点の完全なリストについて [Adapty Capacitor SDK を v4.1 に移行する](migration-to-capacitor-sdk-v4)を参照してください。
...
:::important
他のAdapty SDKメソッドを呼び出す前に、`activate`の完了を待ってください。完全な順序については、[Capacitor SDKの呼び出し順序](capacitor-sdk-call-order)を参照してください。
:::

:::tip
開発環境でのアクティベーションエラーを回避するには、[ヒント](#development-environment-tips)を参照してください。
:::

次に、アプリでペイウォールを設定します：

- [フロー＆ペイウォールビルダー](adapty-flow-builder)を使用する場合は、[クイックスタート](capacitor-quickstart-paywalls)に従ってください。
- 独自のペイウォールUIを構築する場合は、[カスタムペイウォールのクイックスタート](capacitor-quickstart-manual)を参照してください。

## Adapty SDKのAdaptyUIモジュールを有効にする \{#activate-adaptyui-module-of-adapty-sdk\}

[Flow & Paywall Builder](adapty-flow-builder)を使用する予定がある場合は、AdaptyUIモジュールが必要です。コアモジュールを有効にすると自動的に設定されるため、追加の作業は必要ありません。
...
[Adapty アトリビューション](user-acquisition)を使用する場合は、SDK をアクティベートする際に `adaptyAttributionEnabled` を `true` に設定してください。デフォルト値は `false` です。このパラメータを指定しない場合、SDK はインストールを登録せず、インストールの詳細をアプリに送信しません。SDK バージョン 4.1 未満では、Adapty アトリビューションは自動的に有効化されます。
