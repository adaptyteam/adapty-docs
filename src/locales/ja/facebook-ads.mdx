---
title: "Facebook Ads"
description: "Adapty と Facebook Ads を連携して、サブスクリプションマーケティングを効果的に行いましょう。"
...
Facebook Ads インテグレーションを使用すると、Meta Analytics でアプリの統計を簡単に確認できます。Adapty は Meta Ads Manager にイベントを送信し、サブスクリプションに基づいた類似オーディエンスを作成して広告効果を高めるのに役立ちます。これにより、サブスクリプションから広告がどれだけの収益を生み出しているかを正確に把握できます。

Adapty と Facebook Ads の連携は次のように機能します。Adapty は、連携で設定されたすべてのサブスクリプションイベントを Facebook Ads に送信します。この連携は、広告キャンペーンの効果を評価するのに役立ちます。
...
## 高度なマッチング（Conversions API） \{#advanced-matching-conversions-api\}

Facebook CAPIは、上記のFacebook Ads連携とは別の独立した連携で、独自の認証情報を持ち、Adapty ダッシュボードで個別に設定します。有効にすると、AdaptyはMetaのConversions APIに高度なマッチング用のハッシュ化されたIDペイロードを送信します。顧客のAdaptyプロファイルから、AdaptyはSHA-256でハッシュ化して次の情報を送信します：メールアドレス、電話番号、氏名、誕生日、性別、国。Facebookのクリック識別子およびブラウザ識別子（`fbc`、`fbp`）とIPアドレス/ユーザーエージェントは、他のMeta CAPI連携と同様にハッシュ化されずに送信されます。
