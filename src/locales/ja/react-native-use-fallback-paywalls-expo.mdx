---
title: "ExpoプロジェクトでフォールバックペイウォールをUseする"
description: "react-native-adapty configプラグインを使ってExpo React NativeプロジェクトでフォールバックペイウォールをConfigureする。"
metadataTitle: "React NativeでフォールバックFlowを使用する | Adapty ドキュメント"
keywords: ['フォールバックペイウォール', 'expo', 'react native', 'configプラグイン', 'prebuild', 'オフラインペイウォール']
---

:::important
このガイドは**Expoプロジェクト**を対象としています。
**純粋なReact Native（Expo非使用）**を利用している場合は、[純粋なReact Nativeのフォールバックガイド](react-native-use-fallback-paywalls-pure)を参照してください。
:::
