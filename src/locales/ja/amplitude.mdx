---
title: "Amplitude"
description: "ユーザー行動のインサイトを向上させるために、AmplitudeとAdaptyを連携しましょう。"
metadataTitle: "Amplitude連携 | Adapty ドキュメント"
---

import AccessLevelUpdatedEventFeed from '@site/src/locales/ja/reusable/AccessLevelUpdatedEventFeed.mdx';


[Amplitude](https://amplitude.com/) は強力なモバイル分析サービスです。Adapty と連携することで、イベントを Amplitude に簡単に送信し、ユーザーの行動を把握して、より賢い意思決定が行えます。

Adapty はストアからの[サブスクリプションイベント](events)を一元管理できる完全なデータセットを提供し、それを Amplitude アカウントに送信します。これにより、Amplitude 上でユーザーの行動と支払い履歴を紐付け、プロダクトの意思決定に役立てることができます。

### Amplitude インテグレーションの設定方法 \{#how-to-set-up-amplitude-integration\}

Adapty では、Apple、Stripe、Paddle のサンドボックス環境や Google テストアカウントからの**本番**イベントと**テストイベント**を、それぞれ別のフローで設定できます。

- 本番イベントには、Amplitude ダッシュボードの **Production** API キーを入力します。iOS、Android、Stripe、Paddle ごとに個別の API キーを使用してください。
- テストイベントには、必要に応じて **Sandbox** フィールドを使用してください。

Amplitude インテグレーションを設定するには：
...
| フィールド | 説明 |
   | ---------------------------------------------------- | ------------------------------------------------------------ |
   | **Amplitude iOS/ Android/ Stripe/ Paddle API key** | iOS/ Android/ Stripe/ Paddle の Amplitude **API Key** を Adapty に入力します。Amplitude の **Project settings** から確認できます。詳しくは [Amplitude のドキュメント](https://amplitude.com/docs/apis/authentication)をご覧ください。テスト時は **Sandbox** キーを使用し、テスト成功後に **Production** キーへ切り替えてください。販売しているプラットフォームのみ入力してください。キーが未入力のプラットフォームからはイベントが送信されません。 |
