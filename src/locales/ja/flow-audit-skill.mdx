...
description: "flow-audit スキルをインストールして、AIコーディングツールがフローを公開する前に本番環境への準備が整っているかどうかをチェックできるようにします。"
metadataTitle: "フロー監査エージェントスキル | フロー＆ペイウォールビルダー | Adapty ドキュメント"
---

flow-audit スキルはフローについて一つの問いに答えます：本番環境への準備はできているか？AIコーディングツールをフローに向けると、[Adapty CLI](developer-cli-quickstart) を通じて設定を読み取り、プロダクトカタログと照合し、自分で解釈しなければならないリストではなく、判定結果を返します。
