---
title: "AppMetrica"
description: "より詳細なサブスクリプション分析のために AppMetrica と Adapty を統合する。"
metadataTitle: "AppMetrica 統合 | Adapty ドキュメント"
---

import AccessLevelUpdatedEventFeed from '@site/src/locales/ja/reusable/AccessLevelUpdatedEventFeed.mdx';


[AppMetrica](https://appmetrica.yandex.com/about)は、ユーザーの行動をトラッキングし、モバイルアプリのパフォーマンスをリアルタイムで分析できる無料の分析ツールです。AppMetricaをAdaptyと連携することで、サブスクリプション指標とユーザーエンゲージメントについてより深いインサイトを得ることができます。
...
| パラメーター            | 型    | 説明                                                                                                                                                                   |
|:---------------------|:--------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `post_api_key`       | String  | AppMetrica Post API キー。                                                                                                                                                 |
| `application_id`     | String  | AppMetrica アプリケーション ID。                                                                                                                                               |
| `revenue_event_type` | String  | 収益イベントのタイプ（例："subscription_renewed"、"refund"、"intro_started"）。[AppMetrica イベントマッピング](#events-and-tags)を参照してください。                             |
| `price`              | Float   | 収益金額（収益計算設定に基づく）。                                                                                                                                                  |
| `currency`           | String  | 通貨コード（例："USD"）。                                                                                                                                                  |
| `product_id`         | String  | ストアのプロダクト ID。                                                                                                                                                |
| `quantity`           | Integer | 常に 1。                                                                                                                                                                     |
| `transaction_id`     | String  | ストアのトランザクション ID。                                                                                                                                                         |
| `payload`            | String  | イベントの詳細を含む URL エンコードされた JSON 文字列。30KB を超えた場合は、最も重要なデータを保持するために、重要度の低いフィールドから順に削除して自動的にトリミングされます。 |
| `os_name`            | String  | "ios" または "android"。                                                                                                                                                           |
| `profile_id`         | String  | AppMetrica プロファイル ID（設定されている場合）、それ以外の場合はカスタマーユーザー ID（利用可能な場合）。                                                                                                    |
| `appmetrica_device_id` | String  | AppMetrica デバイス ID ハッシュ。`profile_id` が利用できない場合にのみ送信されます。                                                                                                        |
| `session_type`       | String  | 常に "foreground"。                                                                                                                                                          |
| `ios_ifa`            | String  | **iOS のみ**。広告主向け ID。                                                                                                                                             |
| `ios_ifv`            | String  | **iOS のみ**。ベンダー向け ID。                                                                                                                                                 |
| `google_aid`         | String  | **Android のみ**。Google 広告 ID。                                                                                                                                      |
