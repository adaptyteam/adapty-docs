...
Adapty Android SDK 4.0 is a pre-release, so you must pin the exact version instead of a dynamic range — set the `adapty-bom` version to `4.0.0-beta.1` and sync the project. The BOM resolves the matching `android-sdk` and `android-ui` versions for you. See [Install Adapty SDK](sdk-installation-android) for the dependency declarations.
...
`setIntegrationIdentifier(key, value)` is replaced by a method that takes one or more `AdaptyIntegrationIdentifier` values. Use the predefined `Key` constants instead of raw string keys:
...
Replace each old key string with its `Key` constant:
