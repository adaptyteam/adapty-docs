...
description: "Migrate to Adapty Android SDK v4.0 by replacing paywall APIs with flow APIs, which render flows in addition to old builder paywalls."
metadataTitle: "Migrating to Adapty Android SDK v4.0 | Adapty Docs"
---

Adapty Android SDK 4.0 introduces flows and renames the paywall APIs accordingly. The new APIs work with flows, and they still work with paywalls from the old builder — no setup changes are required on the Adapty Dashboard side.
...
- **Onboardings are deprecated.** `AdaptyUI.getOnboardingView` and `AdaptyUI.getOnboardingConfiguration` are marked `@Deprecated` in 4.0 — migrate onboardings to flows built in the [Flow & Paywall Builder](adapty-flow-builder).
...
:::note
SDK 4.1 renames this method to `updateExternalAttribution` and the type to `AdaptyExternalAttributionProvider`. The old names are removed in 4.1 without deprecation. If you migrate to 4.1 directly, see [Migrate to v4.1](migration-to-android-sdk-41).
:::
