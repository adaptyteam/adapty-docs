...
The returned types change from `AdaptyPaywall` / `AdaptyUI.LocalizedViewConfiguration` to `AdaptyFlow` / `AdaptyUI.FlowConfiguration`, and the `locale` parameter moves out of the fetch call and onto `getFlowConfiguration`:
