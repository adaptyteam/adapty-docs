...
:::warning
Adapty's Android SDK is built against Google Play Billing Library 8, but it doesn't use every capability the library exposes. Before you adopt a new Google Play product model or billing capability, email [support@adapty.io](mailto:support@adapty.io) to confirm Adapty works with it — some of these changes can't be reverted once saved in Play Console.
:::
