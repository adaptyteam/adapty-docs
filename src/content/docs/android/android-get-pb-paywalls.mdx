...
metadataTitle: "Get Flows & Paywalls - Android | Adapty Docs"
...
- **Adapty SDK installed**: See the [Android SDK installation guide](sdk-installation-android).
...
:::tip
To warm several placements at once, call [`preloadFlows`](android-optimize-paywall-fetching#preload-placements) (Android SDK 4.1+). It caches the placement JSON only, so you still fetch the view configuration to get the layout and images.
:::
...
- **`true`** — the placement was designed in the **Flow & Paywall Builder** (a flow) or the **old Paywall Builder** (a paywall). Adapty renders the UI for you. Continue with the steps below to fetch the view configuration and [present the flow or paywall](android-present-paywalls).
...
:::warning
Paywalls built in the SDK 3.x Paywall Builder require Android SDK version 3.0 or higher.
:::
...
:::note
If you support multiple languages, add a localization to your paywall. For the codes to use, see [Localizations and locale codes](android-localizations-and-locale-codes).
:::
