...
- **`true`** — the placement is a flow or an old Paywall Builder paywall. Adapty renders the UI for you. Continue with the steps below to fetch the view configuration and [present the flow or paywall](android-present-paywalls).
...
:::note
If you are using multiple languages, learn how to add a [flow localization](add-paywall-locale-in-adapty-paywall-builder) and how to use locale codes correctly [here](android-localizations-and-locale-codes).
:::
...
Please be aware that this topic refers to paywalls created with the old Paywall Builder. To use the Flow & Paywall Builder, update your SDK to version 4. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-android) topic.
...
To ensure optimal performance, it's crucial to retrieve the paywall and its [view configuration](android-get-pb-paywalls#fetch-the-view-configuration-of-a-paywall-created-with-the-old-paywall-builder) as early as possible, allowing sufficient time for images to download before presenting them to the user.
...
Check whether the fetched paywall has a `ViewConfiguration`. If it does, the old Paywall Builder created the paywall, and the Adapty SDK renders it for you — follow the steps below. If it doesn't, you render the paywall yourself; see [remote config paywalls](present-remote-config-paywalls).
...
:::note
If you are using multiple languages, learn how to add a [localization](add-paywall-locale-in-adapty-paywall-builder) and how to use locale codes correctly [here](android-localizations-and-locale-codes).
:::
