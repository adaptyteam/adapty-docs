...
keywords: ['getFlow', 'getPaywall', 'getViewConfiguration', 'getFlowForDefaultAudience']
rank: 95
---
...
AdaptyUI.getViewConfiguration(flow, loadTimeout = 10.seconds) { result ->
    when(result) {
        is AdaptyResult.Success -> {
...
AdaptyUI.getViewConfiguration(flow, TimeInterval.seconds(10), result -> {
    if (result instanceof AdaptyResult.Success) {
        AdaptyUI.LocalizedViewConfiguration flowConfiguration =
