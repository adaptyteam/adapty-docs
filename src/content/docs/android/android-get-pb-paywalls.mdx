...
keywords: ['getFlow', 'getPaywall', 'getFlowConfiguration', 'getViewConfiguration', 'getFlowForDefaultAudience']
rank: 95
---
...
<TabItem value="kotlin" label="Kotlin" default>
```kotlin showLineNumbers
Adapty.getFlowForDefaultAudience("YOUR_PLACEMENT_ID", locale = "en") { result ->
    when (result) {
        is AdaptyResult.Success -> {
            val flow = result.value
            // the requested flow
...
<TabItem value="java" label="Java" default>
```java showLineNumbers
Adapty.getFlowForDefaultAudience("YOUR_PLACEMENT_ID", "en", result -> {
    if (result instanceof AdaptyResult.Success) {
        AdaptyFlow flow = ((AdaptyResult.Success<AdaptyFlow>) result).getValue();
        // the requested flow
