...
- **`true`** — the placement is a flow, or a paywall that Adapty renders for you. Continue with the steps below to fetch the view configuration and [present the flow or paywall](android-present-paywalls).
- **`false`** — the placement is a manually implemented paywall. [Handle it as a manually implemented paywall](present-remote-config-paywalls-android).
...
| **customLayoutId** | <p>optional</p><p>default: `null`</p><p>SDK 4.1+</p> | The custom ID of a layout defined in the Flow & Paywall Builder. Pass it to render that specific layout instead of the one the SDK picks automatically from the device type and screen size. If no layout matches the ID, the flow loads without a view configuration. |
...
:::warning
The legacy Paywall Builder works with Android SDK version 3.0 or higher.
:::

Please be aware that this topic refers to Paywall Builder-customized paywalls. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-android) topic.
...
After fetching the paywall, check if it includes a `ViewConfiguration`, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the `ViewConfiguration` is present, treat it as a Paywall Builder paywall; if not,  [handle it as a manually implemented paywall](present-remote-config-paywalls).
