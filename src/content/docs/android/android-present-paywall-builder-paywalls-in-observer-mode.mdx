...
<details>
   ...
   1. Set up initial integration of Adapty [with Google Play](initial-android).
   2. Install and configure Adapty SDK. Make sure to set the `observerMode` parameter to `true`. Refer to the [Android SDK installation guide](sdk-installation-android).
...
1. Implement the `AdaptyUiObserverModeHandler`. The interface is the same as in SDK v3 — observer mode itself does not change between paywall and flow rendering:
...
2. Display the flow using `AdaptyUI.getFlowView()` or `AdaptyFlowView` directly, passing the observer mode handler:
...
| **variationId** | required | The string identifier of the variation. You can get it using `variationId` property  of the [`AdaptyPaywall`](https://android.adapty.io/adapty/com.adapty.models/-adapty-paywall/)   object. |
