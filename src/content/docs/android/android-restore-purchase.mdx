...
|---------|-----------|
| **Profile** | <p>An [`AdaptyProfile`](https://android.adapty.io/adapty/com.adapty.models/-adapty-profile/) object. This model contains info about access levels, subscriptions, and non-subscription purchases.</p><p>Сheck the **access level status** to determine whether the user has access to the app.</p> |

:::important
Starting from Adapty SDK version 4.1, `restorePurchases` completes successfully even when there is nothing to restore, and returns the current profile. Always check the access level status in the returned profile instead of treating the absence of an error as a restored purchase. In earlier versions, the method failed with `NO_PURCHASES_TO_RESTORE` in this case.
:::
...
Starting from Adapty SDK version 4.1, which uses Google Play Billing Library 8, the Play Store no longer returns expired subscriptions or consumed one-time purchases. To let users recover those across devices and reinstalls, [identify them](android-identifying-users) with `Adapty.identify`. Adapty then restores the purchase history from its backend instead of from the store.
