...
2. Get the view configuration using the `getViewConfiguration` method. The view configuration contains the UI elements and styling needed to display the flow.
...
Adapty.getFlow("YOUR_PLACEMENT_ID") { result ->
    if (result is AdaptyResult.Success) {
        ...
        AdaptyUI.getViewConfiguration(flow) { configResult ->
