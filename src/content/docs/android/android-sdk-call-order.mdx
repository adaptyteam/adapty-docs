...
metadataTitle: "Android SDK Call Order | Adapty Docs"
---
...
| 3    | `Adapty.setIntegrationIdentifier(AdaptyIntegrationIdentifier.appsflyerId(uid)) { error -> ... }`               | After step 2, before any user-action call                                              | Required so MMP IDs land on the correct profile. Pass a list of identifiers to set several MMPs in one call. |
...
If you call `updateProfile` to set custom attributes and then fetch a placement (`getPaywall`) in the same session, await `updateProfile` before the fetch. Firing them together can make the SDK compute the placement's segment hash from attributes that haven't finished propagating, which adds a retry round-trip before the fetch resolves. See [Handle errors in Android SDK](android-sdk-error-handling) if the retry itself fails.
