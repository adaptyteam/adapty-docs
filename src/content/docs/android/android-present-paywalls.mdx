...
description: "Present paywall and flows to users in your Android app."
metadataTitle: "Display Paywalls & Flows - Android | Adapty Docs"
---
...
To get the `AdaptyUI.FlowConfiguration` object used below, see [Get paywalls and flows](android-get-pb-paywalls).

## Present paywalls and flows

In order to display the visual paywall or flow on the device screen, you must first configure it. To do this, call the method `AdaptyUI.getFlowView()` or create the `AdaptyFlowView` directly:
...
...
flowView.showFlow(flowConfiguration, products, eventListener, insets, personalizedOfferResolver, tagResolver, timerResolver);
```
...
In order to display the visual paywall or flow on the device screen with Jetpack Compose, use this composable function:
...
| :---------------------------- | :------- |:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **flowConfiguration**         | required | Supply an `AdaptyUI.FlowConfiguration` object containing visual details of the paywall or flow. Use the `AdaptyUI.getFlowConfiguration(flow)` method to load it. Refer to [Get paywalls and flows](android-get-pb-paywalls) topic for more details.                                       |
...
| **eventListener**             | optional | Provide an `AdaptyUiFlowEventListener` to observe paywall and flow events. Extending `AdaptyUiDefaultFlowEventListener` is recommended for ease of use. Refer to [Handle paywall & flow events](android-handling-events) topic for more details.                                          |
| **insets**                    | optional | <p>Insets are the spaces around the paywall or flow that prevent tapable elements from getting hidden behind system bars.</p><p>Default: `UNSPECIFIED`. See [Change paywall insets](#change-paywall-insets) below.</p>                                                                    |
| **personalizedOfferResolver** | optional | To indicate personalized pricing ([read more](https://developer.android.com/google/play/billing/integrate#personalized-price)), implement `AdaptyUiPersonalizedOfferResolver`.                                                                                                            |
| **tagResolver**               | optional | Use `AdaptyUiTagResolver` to resolve custom tags within the paywall text. Refer to [Custom tags in Paywall Builder](custom-tags-in-paywall-builder) topic for more details.                                                                                                              |
...
Insets are the spaces around the paywall or flow that prevent tapable elements from getting hidden behind system bars. By default, Adapty will automatically adjust the insets, which works great for edge-to-edge paywalls and flows.

If your paywall isn't edge-to-edge, you might want to set custom insets:

- If neither the status bar nor the navigation bar overlap with the `AdaptyFlowView`, use `AdaptyPaywallInsets.NONE`.
...
flowView.onReceiveSystemBarsInsets { insets ->
    val flowInsets = AdaptyPaywallInsets.vertical(insets.top, 0)
    flowView.showFlow(
           flowConfiguration,
           products,
		   eventListener,
...
ViewCompat.setOnApplyWindowInsetsListener(flowView, (view, insets) -> {
    ...
    AdaptyPaywallInsets flowInsets =
                AdaptyPaywallInsets.of(systemBarInsets.top, 0);
    flowView.showFlow(flowConfiguration, products, eventListener, flowInsets, personalizedOfferResolver, tagResolver, timerResolver);
...
To use developer-defined timers in your mobile app, create a `timerResolver` object — a dictionary or map that pairs custom timers with the string values that will replace them when the paywall or flow is rendered. Here's an example:
...
To use custom tags in your mobile app, create a `tagResolver` object — a dictionary or map that pairs custom tags with the string values that will replace them when the paywall or flow is rendered. Here's an example:
...
We recommend creating and populating the `tagResolver` right before presenting your paywall or flow. Once it's ready, pass it to the AdaptyUI method you use for presenting the paywall or flow.
