...
| :---------------------------- | :------- |:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **flowConfiguration**         | required | Supply an `AdaptyUI.LocalizedViewConfiguration` object containing visual details of the flow. Use the `AdaptyUI.getViewConfiguration(flow)` method to load it. Refer to [Fetch the view configuration](android-get-pb-paywalls#fetch-the-view-configuration) topic for more details.                                                                                                 |
...
| **eventListener**             | optional | Provide an `AdaptyUiEventListener` to observe flow events. Extending AdaptyUiDefaultEventListener is recommended for ease of use. Refer to [Handle flow & paywall events](android-handling-events)  topic for more details.                                                                                                                                                                                                                                       |
...
ViewCompat.setOnApplyWindowInsetsListener(flowView, (view, insets) -> {
    ...
    AdaptyFlowInsets flowInsets =
                AdaptyFlowInsets.of(systemBarInsets.top, 0);
    flowView.showFlow(flow, products, flowConfiguration, flowInsets, productTitleResolver);
