...
metadataTitle: "Display Flows & Paywalls - Android | Adapty Docs"
---
...
If you've created a flow, or a paywall in the old Paywall Builder, you don't need to worry about rendering it in your mobile app code to display it to the user. Such a flow or paywall contains both what should be shown within it and how it should be shown.

:::warning

This guide covers flows and paywalls rendered by Adapty. The process differs for manually implemented paywalls and [Observer mode](observer-vs-full-mode).

- For a **manually implemented paywall**, see [Render a manual paywall with remote config](present-remote-config-paywalls).
- For presenting **Observer mode paywalls**, see [Android - Present flows in Observer mode](android-present-paywall-builder-paywalls-in-observer-mode)
...
| **insets**                    | optional | <p>Insets are the spaces around the flow that prevent tapable elements from getting hidden behind system bars.</p><p>Default: `Unspecified` which means Adapty will automatically adjust the insets, which works great for edge-to-edge flows. </p><p>If your flow isn’t edge-to-edge, you might want to set custom insets. For how to do that, read in the [Change flow insets](android-present-paywalls#change-flow-insets) section below.</p> |
...
This guide covers only paywalls built in the old Paywall Builder for **Adapty SDK 3.x**. They require SDK v3.0. Presenting a paywall built for SDK 2.x or earlier, a remote config paywall, or a paywall in [Observer mode](observer-vs-full-mode) works differently.

- For presenting **remote config paywalls**, see [Render a manual paywall with remote config](present-remote-config-paywalls).
