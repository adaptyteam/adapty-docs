---
title: "Handle paywall & flow events - Android"
...
keywords: ['event', 'AdaptyUiFlowEventListener', 'AdaptyUiEventListener', 'onActionPerformed', 'onProductSelected', 'onPurchaseStarted', 'onPurchaseFinished', 'onPurchaseFailure', 'onRestoreSuccess', 'onRestoreFailure', 'onLoadingProductsFailure', 'onRenderingError', 'onFinishWebPaymentNavigation']
---
...
If you need to control or monitor processes that take place on the paywall or flow screen, implement the `AdaptyUiFlowEventListener` methods.

If you would like to leave the default behavior in some cases, you can extend `AdaptyUiDefaultFlowEventListener` and override only those methods you want to change.

Below are the defaults from `AdaptyUiDefaultFlowEventListener`.
...
public override fun onAppear(context: Context) {}

public override fun onDisappear(context: Context) {}
```

### User actions

```kotlin showLineNumbers title="Kotlin"
public override fun onActionPerformed(
...
- `Close` — default behavior dismisses the flow. Override to keep the controller on screen or perform extra cleanup.
- `OpenUrl(url)` — default behavior opens the URL with an `Intent.ACTION_VIEW`.
...
In v4 the product carries full offer information — eligibility is determined automatically. There is no separate `AdaptyPaywallProductWithoutDeterminingOffer` type.
...
If the purchase finishes (success, cancellation, or pending), this method will be invoked:
...
The default behavior dismisses the flow on success unless the purchase was cancelled — override only if you need custom post-purchase logic.
...
Check whether the returned `AdaptyProfile` contains your desired access level before dismissing the flow.
...
### Rendering and product-loading errors
...
If you return `true`, AdaptyUI will repeat the request after 2 seconds — useful for transient network failures.

If an error occurs during the interface rendering, this method will be invoked. In this case, [contact Adapty Support](mailto:support@adapty.io).
...
This method is invoked after an attempt to open a [web paywall](web-paywall) for a specific product, whether successful or failed:
