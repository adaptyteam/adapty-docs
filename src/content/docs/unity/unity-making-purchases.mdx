...
**Looking for step-by-step guidance?** Check out the [quickstart guide](unity-implement-paywalls-manually) for end-to-end implementation instructions with full context.
:::
...
| Parameter                    | Presence | Description                                                                                            |
| :--------------------------- | :------- |:-------------------------------------------------------------------------------------------------------|
| **subscriptionUpdateParams** | required | an [`AdaptySubscriptionUpdateParameters`](https://unity.adapty.io/class_adapty_s_d_k_1_1_adapty_subscription_update_parameters.html) object. |
...
:::info
Your app receives promoted in-app purchases from SDK version 4.1, on **iOS 16.4** or later. Below iOS 16.4, the callback never fires and promoted purchases complete on their own. This is an iOS-only feature: the callback never fires on Android.
:::

When a user starts a purchase from your App Store product page, the SDK hands you the product through `OnReceivePromotedPurchase` and waits. Completing the purchase is up to your app: pass the product to `Adapty.MakePromotedPurchase`. Because you control when that happens, you can show your own screen before the purchase starts.
...
`IAdaptyEventListener` includes the required `OnReceivePromotedPurchase(AdaptyPromotedProduct)` method, paired with `Adapty.MakePromotedPurchase` to complete a purchase you received:
...
:::warning
`OnReceivePromotedPurchase` is the purchase. An empty implementation drops a purchase the user already started: the App Store hands the product to your app and nothing else completes it.
:::
