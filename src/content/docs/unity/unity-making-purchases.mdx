...
:::info
Promoted in-app purchases are supported starting from SDK version 4.1 and require **iOS 16.4** or later. On devices below iOS 16.4, they don't reach your app.
:::

When a user initiates a purchase from your App Store product page and the transaction carries over to your app, the SDK completes it automatically — the Apple purchase system screen shows right away, and Adapty processes the transaction like any other purchase.
...
For handling promoted purchases in your code, `IAdaptyEventListener` includes the required `OnReceivePromotedPurchase(AdaptyPromotedProduct)` method, paired with `Adapty.MakePromotedPurchase` to complete a purchase you received:
...
The SDK doesn't deliver promoted purchases to this method yet — it completes them automatically instead — so you can't yet defer a promoted purchase to show something custom first. Implement the method anyway (it's required by the interface); an empty body is a valid implementation.
