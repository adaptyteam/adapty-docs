...
- **AdaptyUI**: This module renders flows, in addition to old builder paywalls.
...
Adapty SDK supports iOS 13.0+, but requires iOS 15.0+ to work with paywalls created in the paywall builder. Adapty SDK 4.1 — which adds support for [flows](adapty-flow-builder) — requires **iOS 15.0+** for the whole app: a build validator in the Unity Editor stops the iOS build if the deployment target is lower. Building for iOS with SDK 4.x also requires **Xcode 26** or later — the native iOS SDK it adds as a Swift package is built with Swift tools 6.2.

:::info
Adapty SDK brings its own copy of the Google Play Billing Library — you don't add it to your app.

From 3.17.0, Adapty Unity SDK uses Billing Library v8 by default. Versions 3.15.0 through 3.15.2 use v7, and also work when another dependency in your Android build raises the Billing Library to v8.
...
SDK 4.1 — which adds support for [flows](adapty-flow-builder) — is the first stable release of the 4.x line. To install it via the Unity Package Manager, append the version tag to the Git URL:

```
https://github.com/adaptyteam/AdaptySDK-Unity.git?path=/Packages/com.adapty.unity-sdk#4.1.0
```

If you install via the Unity package, download `adapty-unity-plugin-4.1.0.unitypackage` from the [4.1.0 release](https://github.com/adaptyteam/AdaptySDK-Unity/releases/tag/4.1.0).
...
- Update the External Dependency Manager to **1.2.188 or later** — earlier versions don't support Swift Package Manager dependencies. This is the version SDK 4.1 declares as its peer dependency, so Unity warns you if your project has an older one.
- The CocoaPods steps above (`iOS Resolver -> Install Cocoapods`, opening `Unity-iPhone.xcworkspace`) apply to SDK 3.x only. On SDK 4.1, EDM adds the Swift package to the generated Xcode project automatically.
- Set the iOS deployment target to **15.0 or later**. A build validator in the Unity Editor stops the iOS build otherwise.

See the [migration guide](migration-to-unity-sdk-v4) for the full list of changes in the 4.x line.
...
- If you use the [Flow & Paywall Builder](adapty-flow-builder), first [activate the AdaptyUI module](#activate-adaptyui-module-of-adapty-sdk) below, then follow the [quickstart](unity-quickstart-paywalls).
- If you build your own paywall UI, see the [quickstart for custom paywalls](unity-quickstart-manual).

## Activate AdaptyUI module of Adapty SDK

If you plan to use the [Flow & Paywall Builder](adapty-flow-builder) and have installed AdaptyUI module, you need AdaptyUI to be active. You can activate it during the configuration:
...
If you use [Adapty Attribution](user-acquisition), set `SetAdaptyAttributionEnabled` to `true` when activating the SDK. The default value is `false`: without this parameter, the SDK doesn't register installs or deliver installation details to your app. In SDK versions below 4.1, Adapty Attribution is enabled automatically.
