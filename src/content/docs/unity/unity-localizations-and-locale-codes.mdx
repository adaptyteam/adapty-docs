...
- **Flow Builder and Paywall Builder paywalls**: Adapty resolves the localization automatically from the device and the localizations you configured in the builder, so `CreateFlowView` needs no locale code. To choose the localization yourself, [override it when you create the view](#override-the-localization-of-a-flow).
...
To render a flow or paywall with a specific localization instead of the one Adapty resolves from the device, pass the locale code to `SetLocale` when you create the view:
