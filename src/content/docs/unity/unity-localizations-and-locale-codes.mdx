...
- **Flow Builder and Paywall Builder paywalls**: the SDK doesn't read the device locale, so resolve it in your app and [pass it when you create the view](#choose-the-localization-of-a-flow). The locale code is optional — omit it and the flow renders in `en`, or in its default locale when the flow has no `en` localization.
