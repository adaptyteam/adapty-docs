...
After fetching the flow or paywall, load its view configuration and create the view in one step with the `CreateFlowView` method. There is no separate flag to check: if the placement was designed in the **Flow Builder** (a flow) or the **Paywall Builder** (a paywall), `CreateFlowView` returns the view ready for presentation. If the placement is a custom paywall with no Builder UI, `CreateFlowView` returns an error — [handle it as a remote config paywall](present-remote-config-paywalls-unity).
...
:::warning
The new Paywall Builder works with Unity SDK version 3.3.0 or higher.
:::
