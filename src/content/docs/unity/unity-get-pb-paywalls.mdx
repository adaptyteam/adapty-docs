...
After fetching the flow or paywall, load its view configuration and create the view in one step with the `CreateFlowView` method. There is no separate flag to check: if the placement is a flow or a paywall with a view configuration, `CreateFlowView` returns the view ready for presentation. If the placement is a manually implemented paywall, `CreateFlowView` returns an error — [handle it as a manually implemented paywall](present-remote-config-paywalls-unity).

:::important
Make sure to [publish the flow](builder-save-publish). A flow in Draft status has no view configuration to retrieve.
:::
...
After [you designed the visual part for your paywall](adapty-paywall-builder) with the legacy Paywall Builder in the Adapty Dashboard, you can display it in your mobile app. The first step in this process is to get the paywall associated with the placement and its view configuration as described below.

:::warning
The legacy Paywall Builder works with Unity SDK version 3.3.0 or higher.
:::

Please be aware that this topic refers to Paywall Builder-customized paywalls. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-unity) topic.
...
After fetching the paywall, check if it includes a `ViewConfiguration`, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the `ViewConfiguration` is present, treat it as a Paywall Builder paywall; if not, [handle it as a manually implemented paywall](present-remote-config-paywalls-unity).
