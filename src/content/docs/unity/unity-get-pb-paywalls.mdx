---
title: "Fetch old Paywall Builder paywalls and their configuration in Unity SDK"
description: "Learn how to retrieve old Paywall Builder paywalls in Adapty for better subscription control in your Unity app."
...
Please be aware that this topic refers to paywalls created with the old Paywall Builder. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-unity) topic.
...
To ensure optimal performance, it's crucial to retrieve the paywall and its [view configuration](unity-get-pb-paywalls#fetch-the-view-configuration-of-a-paywall-created-with-the-old-paywall-builder) as early as possible, allowing sufficient time for images to download before presenting them to the user.
...
Check whether the fetched paywall has a `ViewConfiguration`. If it does, the old Paywall Builder created the paywall, and the Adapty SDK renders it for you — follow the steps below. If it doesn't, you render the paywall yourself; see [remote config paywalls](present-remote-config-paywalls-unity).
