---
title: "Enable purchases with Flow & Paywall Builder in Unity SDK"
description: "Quickstart guide to enabling in-app purchases with the Adapty Flow & Paywall Builder."
...
- [**Flows**](adapty-flow-builder) – screen sequences that present products to users, built in the no-code Flow & Paywall Builder. The SDK retrieves them via `GetFlow`. If you'd rather build the UI in your own code, use a paywall instead — see [Implement paywalls manually](unity-quickstart-manual).
...
|---|---|---|
| Adapty Flow & Paywall Builder | ✅ Easy | You [create a complete, purchase-ready flow in the no-code builder](quickstart-paywalls). Adapty automatically renders it and handles all the complex purchase flow, receipt validation, and subscription management behind the scenes. |
...
:::important
**The steps below show how to implement a flow created in the Adapty Flow & Paywall Builder.**

If you'd rather build the paywall UI yourself, see [Implement paywalls manually](unity-quickstart-manual).
:::

To display a flow created in the Adapty Flow & Paywall Builder, in your app code, you only need to:
...
3. [Create a flow](paywall-builder-templates) and [add products to it](paywall-product-block).
...
To get a flow created in the Adapty Flow & Paywall Builder, you need to:
...
:::important
To get the view, [publish the flow](builder-save-publish). A flow with unpublished edits has the **Dirty** status, and its placement keeps serving the last published version.
:::
