...
Use `PlayFabId` from the `LoginResult` callback — PlayFab's permanent per-player identifier, stable across sessions and devices.
...
PlayFabClientAPI.LoginWithCustomID(
    ...
    (LoginResult result) => {
        // PlayFabId is stable — use it, not the player's display name or email
        Adapty.Identify(result.PlayFabId, (error) => {
            if (error == null) {
                // successful identify
            }
        });
...
If your game supports anonymous play with later social sign-in, call `Adapty.Identify()` when the account is linked — not at guest session start. This applies to Apple Game Center and Google Play Games account linking. See [Allow purchases without login](unity-quickstart-identify#allow-purchases-without-login) for how Adapty handles the profile switch.

You can also use a Google Play Games player ID or Apple Game Center player ID directly as `customerUserId` in games that do not use a separate backend auth service.
...
// Generate once per user with Guid.NewGuid(), persist (e.g. PlayerPrefs), and reuse
var appAccountToken = Guid.Parse("00000000-0000-0000-0000-000000000000");
var builder = new AdaptyConfiguration.Builder("YOUR_API_KEY")
