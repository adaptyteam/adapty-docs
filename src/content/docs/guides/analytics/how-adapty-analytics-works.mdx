...
- Third-party [attribution services](attribution-integration) (Appsflyer, Adjust, Branch, etc.) share data on traffic sources and ad campaigns. If you set up [Adapty Attribution](adapty-user-acquisition), Adapty can handle your ad campaign data on its own, bypassing this step.
...
You can enable [Adapty Attribution](adapty-user-acquisition) to expand Adapty's analytics capabilities with ad campaign data. It will help you discover correlations between ad campaign spending and user behavior.
...
These delays affect how soon an event appears, not how long a chart keeps changing afterwards. Most charts date an event to the day it happened, so that day's total is final as soon as the event arrives. Cohort-based metrics work differently: [conversion rates](analytics-conversion), [cohorts](analytics-cohorts), and [retention](analytics-retention) group users by an action they took, then keep measuring what those users do afterwards. The metric for a past date keeps rising for weeks or months. [Why past values change](analytics-conversion#why-past-values-change) covers what that means for reporting.
