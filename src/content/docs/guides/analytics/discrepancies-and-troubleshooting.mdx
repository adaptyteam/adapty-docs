...
Cohort-based metrics group users by an action they took — an install, a paywall view, a trial start, or a first payment — then keep measuring what those users do afterwards. [Conversion rates](analytics-conversion), [cohorts](analytics-cohorts), [retention](analytics-retention#how-do-we-calculate-retention), and [LTV](ltv) all work this way; the [metric comparison table](metric-comparison-table) marks which metrics do and which don't. A user who installs in March and subscribes in July raises March's "Install to paid" rate in July.

Comparing a two-year-old month against last month therefore compares a settled number against a provisional one. Read each period after the same amount of elapsed time instead.

[Install to trial](analytics-conversion#install---trial) settles within days, while [Install to paid](analytics-conversion#install---paid) and [Trial to paid](analytics-conversion#trial---paid) keep rising for months. [Compare months in your reports](analytics-conversion#compare-months-in-your-reports) explains how long to wait before comparing.
...
## Metric and event mismatches

A mismatch between a chart total and the number of events your integration received is expected, not a delivery failure. The names line up, but the definitions don't.

### New subscriptions metric vs the `subscription_started` event

The [New subscriptions](reactivated-subscriptions) metric and the `subscription_started` [integration event](events) count different things. Expect the "new subscriptions" metric to exceed the number of `subscription_started` events.

| What differs | New subscriptions metric | `subscription_started` event |
|---|---|---|
| What it reports | How many subscriptions started in a period | That a paid subscription began, as it happens |
| What it's for | Track how many new payers you gained | Record the purchase in your analytics, MMP, or own backend |
| Timing | Counts transactions by purchase date in your reporting timezone | Sent when the purchase happens |
| Trial conversions | Counted | Not sent — Adapty sends `trial_converted` instead |
| Retracted transactions | Excluded once Adapty retracts them | Already sent, and never recalled |
| Profiles the SDK never identified | Their purchases are counted | Absent from the Event Feed |

### Active subscriptions metric vs the `subscription_active` event

The [Active subscriptions](active-subscriptions) metric and the `subscription_active` [integration event](events) differ in what they're for, not just in what they count.

| What differs | Active subscriptions metric | `subscription_active` event |
|---|---|---|
| What it reports | How many subscriptions are alive at the end of each period | That a subscription survived its first days |
| What it's for | Track the size and growth of your subscriber base | Forward to an MMP to bid for users who stay |
| Timing | Recalculated at the end of every period | Sent a configured number of days after the subscription starts, from 1 to 90 |
| Repeats | Yes, every period | No — Adapty checks once and doesn't look again |
| Trial conversions | Counted once the trial converts to paid | Never sent — the timer starts only at `subscription_started`, and a conversion sends `trial_converted` |
| Cancelled renewals | Excluded | Still sent, as long as the user has access |
| Refunds | Removed, including retroactively from past dates | Not sent if the refund comes first; never recalled if it comes later |
| Availability | Always calculated | Sent only by integrations where you turned it on |

### Active trials metric vs the `trial_active` event

The [Active trials](active-trials) metric and the `trial_active` [integration event](events) differ in what they're for, not just in what they count.

| What differs | Active trials metric | `trial_active` event |
|---|---|---|
| What it reports | How many free trials haven't expired | That a trial survived its first days |
| What it's for | See how many trials are running | Optimize MMP bidding for trial users who stay |
| Timing | Recalculated at the end of every period | Sent a configured number of days after the trial starts, from 1 to 90 |
| Repeats | Yes, every period | No — Adapty checks once and doesn't look again |
| Trial conversions | Counted until the trial converts, then dropped in the next period | Never sent for a trial that already converted |
| Availability | Always calculated | Sent only by integrations where you turned it on |

The two agree on cancellation: a user who turns off auto-renewal stays in the metric, and the event is still sent, until the trial expires.

## A filtered total exceeds the unfiltered total

With a **Period** filter applied, adding a product filter or grouping by product changes what a period means. Without a product involved, **Activation** counts the first payment in a subscription chain. With one, it counts the first payment for each product in that chain, so a user who switched products contributes an Activation for each. Neither view double-counts a transaction; they count different populations.

Which one to use depends on the question: all products with no grouping answers "how many new payers did we acquire", the product-filtered view answers "how many users started this product for the first time". Removing the **Period** filter reconciles both against the unfiltered total.

## Paywall view conversion rates dropped

If your app uses [Flow Builder](adapty-flow-builder), the [Paywall view -> Trial](analytics-conversion#paywall-view---trial) and [Paywall view -> Paid](analytics-conversion#paywall-view---paid) rates are lower than they used to be, across your whole history.

A flow screen shown to a user now counts as a paywall view. Flow screen views previously reached [flow metrics](flow-metrics) alone, so these two rates divided your purchases by a paywall-view count that left every flow screen out. The rates you see now are the accurate ones; the values they replaced were too high.

Each user counts once per date, however many screens of a flow they went through, and whether or not they also saw a paywall built in the [Paywall Builder](adapty-paywall-builder).
