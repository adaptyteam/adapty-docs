...
* **Access restoration**
    ...
    If you built your paywall with the [Flow & Paywall Builder](adapty-flow-builder), the restore button automatically triggers the restoration process. If you [implemented a manual paywall](ios-implement-paywalls-manually), add code that fires the [restorePurchases](restore-purchase) method. Adapty will restore the user's access level, **unless** you use the SDK in [observer mode](observer-vs-full-mode).
