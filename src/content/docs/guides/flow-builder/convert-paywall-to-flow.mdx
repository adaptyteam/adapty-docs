...
To convert a paywall into a flow, open the paywall's overview page and click **Move to new builder**. This button only appears for paywalls made with the old Paywall Builder.
...
Some legacy Paywall Builder features have no flow equivalent, so a converted paywall won't always match the original exactly.

Adapty completes the conversion regardless. The **Import review** panel opens with the flow and lists what changed, with a fix for each. Tick them off as you handle them.

<ZoomImage id="import-review.webp" width="500px" alt="Import review panel listing what changed during the conversion, with a checkbox for each item" />
