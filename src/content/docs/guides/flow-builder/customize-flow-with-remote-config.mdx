...
:::important
This guide covers remote config for flows. For classic paywalls created without the builder, see [Design paywall with remote config](customize-paywall-with-remote-config).
:::
...
To open the remote config for a flow, click the <Inline id="remote-config.svg" alt="Remote Config" /> icon above the screen preview in the flow builder.

<ZoomImage id="flow-remote-config-access.webp" width="300px" alt="Open remote config in the builder editor" />
...
:::warning
If the remote config contains invalid JSON, you can neither **save** nor **publish** the flow. See [Save & publish flows](builder-save-publish#troubleshooting) for the full list of issues that block previewing and publishing.
:::
...
Next, [create a placement](create-placement) and add the flow to it. Then render the flow in your app: [iOS](present-remote-config-paywalls) or [Android](present-remote-config-paywalls-android).
