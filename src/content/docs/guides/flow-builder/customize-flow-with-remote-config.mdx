...
description: "Customize your flow with a remote config JSON payload."
...
:::important
This guide covers remote config for Flow & Paywall Builder. For classic paywalls created without Flow & Paywall Builder, see [Design paywall with remote config](customize-paywall-with-remote-config).
:::
...
To open the remote config for a flow, click the <Inline id="remote-config.svg" alt="Remote Config" /> icon above the screen preview in the builder.
...
:::warning
If the remote config contains invalid JSON, you can neither **save** nor **publish** the flow. See [Common issues](flow-common-issues#a-flow-wont-preview-or-publish) for the full list of issues that block previewing and publishing.
:::
...
Next, [create a placement](create-placement) and add the flow to it. Then render the flow in your app: [iOS](present-remote-config-paywalls), [Android](present-remote-config-paywalls-android), [React Native](present-remote-config-paywalls-react-native), [Flutter](present-remote-config-paywalls-flutter), [Unity](present-remote-config-paywalls-unity), [Kotlin Multiplatform](present-remote-config-paywalls-kmp), or [Capacitor](present-remote-config-paywalls-capacitor).
