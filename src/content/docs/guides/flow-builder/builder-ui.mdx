...
<ZoomImage id="flow-builder-overview.webp" width="700px" alt="Flow Builder interface with a paywall preview" />
...
* **Close** <Inline id="close.svg" alt="Close" />: Exit the flow editor and return to the flows page.
* **App name** <Inline id="app.svg" alt="App" />: Identifies the app the flow belongs to.
* **All flows** <Inline id="flows.svg" alt="Flows" />: Open the list of all flows for this app.
* **Flow status**: The icon to the left of the flow name indicates the current [flow status](builder-save-publish#flow-status):
    - **Draft** <Inline id="pencil.svg" alt="Draft" />
    - **Publishing** (spinning loader)
    - **Failed** <Inline id="flow-failed.svg" alt="Failed" />
    - or **Live** <Inline id="flow-live.svg" alt="Live" />. 
* **Rename the flow**: Click the flow name to rename it. Several flows can have the same name — [give each new flow a unique one](paywall-builder-templates#create-flow).
* **View mode toggle**: Switch between the design view <Inline id="cursor.svg" alt="Cursor" /> and the [remote config view](customize-flow-with-remote-config)<Inline id="remote-config.svg" alt="Remote Config" />.
* **Undo/Redo**: Click the arrow icons to undo <Inline id="undo.svg" alt="Undo" /> or redo <Inline id="redo.svg" alt="Redo" /> your flow changes. You can also use ⌘Z / Ctrl+Z to undo.
* **Save draft / Publish**: Click **Save draft** to save your progress without going live (⌘ / Ctrl+S). Open the dropdown <Inline id="collapse-layer.svg" alt="Open dropdown" /> to access the [**Publish**](builder-save-publish) button. You can only add your flow to a [placement](create-placement) after publishing.
...
<ZoomImage id="builder-floating-controls.webp" width="400px" alt="Floating controls above the device preview: screen name pill, Toggle animations, and Add element" />

- **Screen name** — a pill with the name of the current screen.
- **Toggle animations** <Inline id="fill-video.svg" alt="Toggle animations" /> — turns element animation previews on or off; they run continuously until turned off again. Only visible when the active screen contains at least one [animation](builder-styling#animation). Does not affect the visibility of the animations on the live device.
- **Add element** <Inline id="plus.svg" alt="Plus" /> — open the [element library](builder-elements) at the current screen. Equivalent to the **+** at the top of the Screens and Layers panel — useful when the panel is collapsed.
...
* **Screen orientation**: Toggle between portrait <Inline id="portrait.svg" alt="Portrait" /> and landscape <Inline id="landscape.svg" alt="Landscape" /> modes to preview your flow in different orientations.
*  **Color scheme**: Switch between light <Inline id="light-mode.svg" alt="Light mode" /> and dark <Inline id="dark-mode.svg" alt="Dark mode" /> modes to see how your design adapts to different themes.
...
:::link
Learn more: [Layout and positioning](manage-paywall-ui-elements), [Styles and appearance](builder-styling)
:::
...
If you click the plus <Inline id="plus.svg" alt="Plus" /> button, the left panel displays the list of available UI elements and their variations. Click an entry to add it to the current screen as a new layer.
...
The products <Inline id="products.svg" alt="Products" /> button opens the product list. It shows which products are assigned to each screen in your flow.
...
:::info
Learn more:
- [Styles and appearance](builder-styling)
...
The styles <Inline id="styles.svg" alt="Styles" /> button opens Saved Styles.
...
The brackets <Inline id="variables.svg" alt="Variables" /> button opens Variables.
...
### Localization
:::link
Main article: [Localization](add-flow-remote-config-locale)
:::
...
The sparkle <Inline id="sparkle.svg" alt="AI Editor" /> button opens the AI Editor chat. 

- Describe a change to copy or design, and Adapty applies it to the current screen. Use the screen selector dropdown to change the work surface.
- Open the <Inline id="library.svg" alt="Prompts library" /> **Prompts library** to select from one of our prompt templates.
- Click **Stop** <Inline id="stop.svg" alt="Stop" /> to cancel the prompt as it's being processed.

#### Retry, rate, or copy

Every response has a row of actions:

- <Inline id="retry.svg" alt="Retry" /> **Retry** runs the same prompt again and adds a new response.
- <Inline id="thumbs-up.svg" alt="Thumbs up" /> **Thumbs up** and <Inline id="thumbs-down.svg" alt="Thumbs down" /> **Thumbs down** allow you to rate the quality of the response.
- <Inline id="copy.svg" alt="Copy" /> **Copy** copies the response text.

#### Limitations

The AI editor restyles, rewrites, and rearranges the current screen. It can't do the following:

- **Generate or fetch media** — add images and video yourself, then ask the AI to place or restyle them.
- **Accept attachments** — no reference images or files in the chat; describe the change in text.
- **Manage [variables](#variables) and set up conditional logic** — set these up yourself.
- **Set up [interactions](#interactions-properties)** — connect navigation, purchases, and links manually.
- **Change products** — it won't edit your catalog or choose which plans to show; it only restyles the product cards already on the screen.
- **Edit other screens** — it works only on the screen you have open.

Some of the listed capabilities may become available in future releases.
