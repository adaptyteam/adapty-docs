...
metadataTitle: "AI Editor | Flows & paywalls | Adapty Docs"
keywords: ['ai editor', 'ai', 'flows', 'flow builder', 'paywall', 'prompts']
---

The AI Editor is a chat panel inside the Flow & Paywall Builder. Describe a change in plain language, and Adapty rewrites, restyles, or rearranges the screen you're looking at. Adapty then tells you what changed, and one click takes it back.
...
- **The canvas locks while the AI works**: You can't select or change elements on the screen until the prompt is processed. You can always interrupt the generation.
...
The AI chat cannot generate images, but it can work with images that you attach <Inline id="image-plus.svg" alt="Attach image" />. A single message can include up to 5 images: JPEG, PNG, and WebP files, up to 5 MB each.

- **Place the attached image**: Attach the image and say where it goes, for example `Use this as the hero image` or `Make this the background`.
- **Build a screen from a screenshot**: Attach a screenshot of a reference paywall and prompt `Build a screen like this`. The AI builds a similar screen, subject to the generator's [limitations](#what-the-ai-editor-cant-do).
- **Debug the screen**: Attach a screenshot to illustrate what's broken, and describe how.
...
- **Generate or fetch media**: The AI can't create images or download them from a link. It only uses images you [attach to the chat](#work-with-images). When it rebuilds a screen from a screenshot, it leaves image slots empty. The chat doesn't accept videos, so [add them manually](custom-media).
