...
metadataTitle: "Migrate paywalls to flows | Flows & paywalls | Adapty Docs"
---
...
1. **[Build your flow](#build-your-flow)**: Convert an existing paywall in one click, or create a flow from a template; no developer needed.
...
4. **[Update the SDK](#update-the-sdk)**: Upgrade to the Adapty SDK v4, fetch the flow from its placement, and verify a sandbox purchase. This is the main developer task.

### Build your flow

**If your paywall was built in the legacy Paywall Builder, convert it rather than rebuilding it.** The **Move to new builder** action recreates it as a draft flow in one click — layout, copy in every locale, and products with prices as variables — and your original stays live while you edit and test. See [Convert a paywall into a flow](convert-paywall-to-flow).

To build something new instead, pick the route that fits:

- **[Start from a template](paywall-builder-templates)**: A starter template or a blank flow in the Flow & Paywall Builder, with no code.
- **[Use the agent skill](flow-generator-skill)**: Your own AI coding tool authors and edits flows from the terminal — useful for quizzes, branching, and adding locales in bulk.

Reference while you build:

- **[Flow & Paywall Builder documentation](adapty-flow-builder)**: Walks you through the builder and what you can create.
...
You can preview the flow on a real device without touching the app. Download the Adapty mobile app for [iOS](https://apps.apple.com/us/app/adapty/id6739359219) or [Android](https://play.google.com/store/apps/details?id=io.adapty.dashboard). Then, in the Flow & Paywall Builder, click **Test on device**, choose a locale, and scan the QR code with your device. This shows the real screens, branching, copy, and design.
...
You can continue running A/B tests with flows. Run a [Regular A/B test](ab-tests) across flow variants on a flow placement, or a [Crossplacement A/B test](ab-test-types) across several flow placements. You can't put a flow and a paywall into one A/B test, Regular or Crossplacement: they are different content types with different metrics.

#### Create the placements with an agent skill

With more than a handful of placements to move, the [migrate-placements skill](migrate-placements-skill) creates them from your own AI coding tool instead. It reads every placement in your app through the [Adapty CLI](developer-cli-quickstart), groups them by the paywall each one serves, asks per paywall where the flow comes from, then creates one flow placement per paywall placement in scope and hands back the old-to-new ID mapping and the `getFlow` call your app has to ship.
...
With your flow placement ready, point the app at it. Flows render only on the Adapty SDK v4 and later. Upgrade the SDK and fetch the flow from your new placement with `getFlow`. See the v4 migration guide for your platform — [iOS](migration-to-ios-sdk-v4), [Android](migration-to-android-sdk-v4), [React Native](migration-to-react-native-sdk-v4), [Flutter](migration-to-flutter-sdk-v4), [Unity](migration-to-unity-sdk-v4), [Kotlin Multiplatform](migration-to-kmp-sdk-v4), or [Capacitor](migration-to-capacitor-sdk-v4) — for specific upgrade steps.

Once the flow is wired in, verify it like any other purchase flow: run it on a device or simulator and make a sandbox purchase to confirm that products, the purchase, and the access level all work. See the testing guide for your platform: [iOS](ios-test), [Android](testing-on-android), [React Native](react-native-test), [Flutter](flutter-test), [Unity](unity-test), [Kotlin Multiplatform](kmp-test), or [Capacitor](capacitor-test).
