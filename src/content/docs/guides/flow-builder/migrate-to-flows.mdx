...
You can keep A/B testing with flows: run a [Regular A/B test](ab-tests) across flow variants on a flow placement, or a [Crossplacement A/B test](ab-test-types) across several flow placements. Comparing a new flow against your old paywall is still a cohort comparison, not a single test — a Crossplacement test can't mix placement kinds, and the two live on different placement types.
