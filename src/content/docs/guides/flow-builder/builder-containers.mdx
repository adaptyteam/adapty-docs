...
- **Footer**: a panel pinned to the bottom, outside the scroll area
- **Cover**: a hero image or video pinned to the top, behind the scrolling content
...
<ZoomImage id="flow-builder-footer-scroll.webp" width="300px" alt="Footer pinned to the screen bottom while content scrolls" float="right" />
...
<ZoomImage id="flow-cover-in-action.webp" width="300px" alt="A Cover selected on the canvas, with a photo across the top of the screen and the content starting below it" float="right" />

A **Cover** is a panel that displays a hero image or video on the top part of the screen. It reaches into the device's top safe area, behind the status bar and the notch, even if the **Ignore Safe Area** option is off. As the user scrolls, the screen content slides over the cover, which stays in place.

Only one cover is allowed per screen. You cannot duplicate an existing cover, or add a new one.

### Properties

Set **Fill** to a solid color, gradient, image, or video. See [Images, videos, and icons](custom-media) for the list of accepted file types and size limits.

To replace the cover's image or video at runtime from your app code, set a [custom media ID](custom-media#custom-media-id).

<ZoomImage id="cover-settings.webp" width="700px" alt="The Design panel for a selected Cover, with Fill set to Image and the Use custom media ID checkbox below it" />
