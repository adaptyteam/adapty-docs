...
A cover, the screen background, and media elements all have different use cases.

- **Use a Cover** for media that fills the top part of the screen and stays in place. The content starts below the cover, but hides it as it scrolls up.
- **Use a [screen background](paywall-head-picture)** for media that fills the whole viewport and stays in place, always below other screen elements.
- **Use a [Media element](custom-media)** for media that scrolls away with the rest of the content.
- **Use a media element with [fixed positioning](manage-paywall-ui-elements#fixed)** for media that stays fully visible while the content scrolls underneath it. A fixed element reserves no space, so the content starts at the top of the screen.
