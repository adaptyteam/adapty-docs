...
:::warning
The file you upload is **for editor preview only**. To render the font on a user's device, your app must include the same file in its bundle. Review [Custom font limitations](#custom-font-limitations) for other constraints before you finish setup.
:::
...
- **Bundling**: Adapty keeps the uploaded file for editor preview only. [Include the font file in your app bundle](#add-the-font-files-to-your-apps-bundle) — without it, the SDK falls back to SF Pro (iOS) or Roboto (Android) at runtime.
- **Font variations (weights and italics)**: The SDK doesn't synthesize bold or italic, and it can't pick a weight out of a variable font. Upload a separate font for each weight (`Inter`, `Inter Bold`, `Inter Bold Italic`) and switch via the **Font** dropdown, not the **Bold**/**Italic** toggles.
- **Deletion**: Deleting a font silently rewrites all references to the system font across draft and published flows.
- **Unique names**: The **Font name in Builder**, **iOS font name**, and **Android font name** must each be unique within an app.
