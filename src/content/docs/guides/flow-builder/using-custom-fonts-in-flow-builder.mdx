...
If you need fonts outside of the default selection, you can add a custom font.

Adapty registers one style per custom font. Split a variable font (one file with multiple weights) into per-style files before upload. Each variant needs its own iOS and Android name. For [text styling](onboarding-text), switch variants via the font dropdown — the weight, bold, and italic controls don't apply.
