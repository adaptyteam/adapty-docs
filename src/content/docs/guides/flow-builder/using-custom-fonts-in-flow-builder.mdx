...
0. If the font is variable, split it into single-style files with unique names. The weight, bold, and italic controls don't apply to custom fonts. Adapty only registers one style per custom font file. To [style the text](onboarding-text), switch the font to the correct variation.
