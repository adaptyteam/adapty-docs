...
This means Adapty's native A/B tests can't bridge the two: a Regular A/B test only compares variants on a single placement, and a Crossplacement A/B test works across placements of one kind only, so it can't hold a flow placement and a paywall placement together. Comparing a new flow against your existing paywall is a cohort comparison, not a single native test.
