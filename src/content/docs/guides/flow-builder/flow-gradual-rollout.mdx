...
This means Adapty's native A/B tests can't bridge the two:
- A Regular A/B test only compares variants on a single placement
- A Crossplacement A/B test works across placements of one kind only, so it can't run on a flow placement and a paywall placement at the same time. 

Comparing a new flow against your existing paywall is a cohort comparison, not a single native test.
