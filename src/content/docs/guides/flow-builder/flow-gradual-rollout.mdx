...
description: "Test a new flow on part of your audience while your existing Paywall Builder paywall keeps serving the rest, using your own traffic split instead of a native Adapty A/B test."
metadataTitle: "Roll out a flow gradually alongside your paywall | Flow Builder | Adapty Docs"
---

If you have a live Paywall Builder paywall driving revenue, you may want to test a new flow with a small share of your audience before switching everyone over.

Flows and Paywall Builder paywalls are different content types, so each lives on its own placement type — a flow placement and a paywall placement never share traffic.
...
:::note
Firebase Remote Config is one common way to do this — add a parameter (for example, `flow_rollout_percentage`) and read it in your app to pick the placement. This isn't an Adapty feature; it's a tool you configure on your own, separate from [Adapty Remote Config](customize-paywall-with-remote-config), which configures flow and paywall content itself.
:::
