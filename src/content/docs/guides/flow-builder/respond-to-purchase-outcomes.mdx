...
metadataTitle: "Respond to purchase outcomes | Flows & paywalls | Adapty Docs"
---
...
To fetch the post-purchase flow from your app, assign it to its own placement. Your flow's placement can't serve it too — a placement serves one flow per audience.
...
To open the post-purchase flow without a wait, fetch it alongside the paywall, rather than inside the handler. Request its UI configuration too — that call is what starts caching the flow's images in the background.

On [iOS](ios-optimize-paywall-fetching#preload-placements) and [Android](android-optimize-paywall-fetching#preload-placements), `preloadFlows` (SDK 4.1+) warms several placements in one call. It caches the placement JSON only, so pair it with a `ReturnCacheDataElseLoad` fetch policy and still request the UI configuration ahead of time.
