...
metadataTitle: "Element states | Flow Builder | Adapty Docs"
keywords: ['flow builder', 'element states', 'disabled state', 'selectable', 'input states']
---

import ZoomImage from '@site/src/components/ZoomImage.astro';

Interactive elements in the Flow Builder change their look based on user actions: a tapped quiz option becomes **Selected**, a focused input becomes **Active**. Some states are condition-driven — for example, you can **disable** a button. Style each state separately to give users visual feedback without app code.
...
In a single-choice group, selecting one element de-selects the others. Multi-choice and toggle groups let several be Selected at the same time. See [group types](flow-selectable-elements#group-types).
