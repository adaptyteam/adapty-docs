...
- **One vocabulary across your own tools**: Every event your app forwards names the screen the same way, so a funnel, a cohort, and a dashboard filter agree without a lookup table.
...
- **Rename before the flow goes live**: Events already collected keep the old ID. A funnel that spans the change shows two half-filled screens, and renaming back does not merge them. Adapty's flow metrics splits the same way, because it groups screen rows by ID — the renamed screen gets a second row, under the same label as the first.
...
- **Adapty's flow metrics won't show these names**: [Flow metrics](flow-metrics) labels a screen `<position>. <caption>` from the flow's published version, so it reads `9. Paywall A` whatever the screen's ID is. The IDs here reach your own analytics only.
