...
metadataTitle: "Flow Audit Agent Skill | Flow Builder | Adapty Docs"
---

The flow-audit skill answers one question about a flow: is it ready for production? Point your AI coding tool at a flow and it reads the config through the [Adapty CLI](developer-cli-quickstart), runs 23 checks across six families, and ends in a verdict — not a list you have to interpret yourself.
...
- **The Adapty CLI**: v0.8.1 or later, authenticated. flow-audit reads the flow and your product catalog through the CLI, so it needs a version new enough to have the commands it depends on, logged in to the app the flow belongs to.

## What it checks

flow-audit runs 23 checks grouped into six families, all against one fetched snapshot — the flow's config and your product catalog:
...
4. **Run the checks.** All 23 checks, across six families, against that snapshot alone.
5. **Report.** A verdict, not a list.

For a 7-screen flow, the checks themselves take about two seconds of CLI and checker time. A full run — including your AI tool reading the flow and writing up the report — usually finishes in under a minute.
...
- [flow-generator skill](flow-generator-skill) — the skill that makes the fix once flow-audit tells you what's wrong.
- [Save & publish flows](builder-save-publish) — where a flow's status comes from, and what publishing does.
- [Common issues](flow-common-issues) — the separate list of what stops a flow from previewing or publishing at all.
- [Preview flows](paywall-device-compatibility-preview) — check what flow-audit can't see, like a placement attachment or a branch that only fires on tap.
