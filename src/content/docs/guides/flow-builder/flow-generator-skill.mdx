...
metadataTitle: "Flow Generator Agent Skill | Flows & paywalls | Adapty Docs"
---
...
:::important
The flow-generator skill is experimental, and it saves changes directly to flows in your dashboard. Run it against a draft or a copy first, and review every change in the Flow & Paywall Builder before you publish.
:::
...
The skill ships in the `adapty-skills` repository alongside `adapty-integration`, `flow-audit`, `migrate-placements`, `ads-manager`, `paywall-teardown`, `onboarding-teardown`, and others. Each of these commands installs every skill in the repository.
...
<Tabs>
  ...
  <TabItem value="codex" label="Codex">
    ...
    Then start a new thread, so Codex picks up the skills.
  </TabItem>
...
1. **Open the flow in the Flow & Paywall Builder.** If you already had the flow open in a browser tab, refresh the page — the Flow & Paywall Builder doesn't pick up a write made through the CLI.
...
4. **Publish.** Click **Publish to Live** in the top right of the Flow & Paywall Builder, or run [`adapty flows publish`](developer-cli-reference#adapty-flows-publish) from the terminal. A flow that isn't in a [placement](create-placement) still reaches nobody, and a Draft flow can't be added to one — so publish first. See [Save & publish flows](builder-save-publish).

If the result is wrong, restore from the backup: the skill copies the config before its first edit and names the file in its approval message. Ask it to write that file back, or fix the flow by hand in the Flow & Paywall Builder. Either way a saved draft changes nothing for your users.
...
The skill uploads an image from your computer and binds the returned URL into the flow, so you don't add media in the Flow & Paywall Builder by hand. It needs a **file path**, not a picture: when you paste or attach an image into the chat, your AI tool receives the pixels but no file, so there's nothing to upload.
...
Formats and size limits match a manual upload in the Flow & Paywall Builder — see [Images, videos, and icons](custom-media). SVG is the one exception: it fails to upload, so the skill draws single-color glyphs inline instead.
...
Adapty also has an [AI Editor](flow-ai-editor) inside the Flow & Paywall Builder. The difference isn't which one has more features — it's whose environment the work happens in.
...
| What it can see | The screen you have open, plus the images you attach | Anything your AI tool can reach |
...
- **Publishing stays yours**: The skill stops at a saved draft and never publishes. Publish it yourself in the Flow & Paywall Builder, or with [`adapty flows publish`](developer-cli-reference#adapty-flows-publish).
...
- **Video**: The skill places a media element where the video belongs, so the layout is already right when you swap in the file in the Flow & Paywall Builder.
- **Fonts**: The skill creates the text styles your design calls for and points them at a placeholder face. Upload the font in the Flow & Paywall Builder and assign it to those styles.
...
- **Any locale but the one it draws**: The preview ignores the flow's default locale and the order of the locale list, so a translation can't be verified visually at all. Switch locale in the Flow & Paywall Builder and look for text that overflows its container.
...
- [flow-audit skill](flow-audit-skill) — read-only check on whether the draft this skill leaves you is ready for production, before you publish it.
...
- [Add a locale in Adapty Flow & Paywall Builder](add-paywall-locale-in-adapty-paywall-builder) — the same localization work by hand.
