...
Then run `/reload-plugins` to activate the skills. For other AI coding tools, follow [flow-generator's install steps](flow-generator-skill#install-the-skill) — same repo, same commands. Those routes copy the skills into place instead of installing a plugin, so re-run them to pick up a skill added since your last install.
