...
Then run `/reload-plugins` to activate the skills. For other AI coding tools, follow [flow-generator's install steps](flow-generator-skill#install-the-skill) — same repo, same commands. Codex installs a plugin the way Claude Code does; the other routes copy the skills into place, so re-run those to pick up a skill added since your last install.
