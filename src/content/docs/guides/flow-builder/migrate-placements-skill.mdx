...
:::warning
A placement can't be deleted. The skill shows you every placement ID it proposes and waits for your approval, because an ID created by mistake stays in your app.
:::
...
Proposed IDs follow your existing ones — `main` becomes `main-flow` — and each is verified against every placement ID already in the app, because a collision would be permanent.
...
The skill looks for existing flows before it builds anything, and matches them to your paywalls **by name alone** — nothing in Adapty records which paywall a flow came from. So every candidate it finds is a proposal for you to confirm, and a flow you renamed won't be found at all. Confirm against your [flow list](https://app.adapty.io/flows): a flow created by mistake can't be deleted from the CLI either.
...
Before anything is created, the skill prints a block for you to read and answer. It names your app, the number of placements it's about to create, and the full old-to-new table — never an abbreviated count, because the middle column is the part that becomes permanent.
...
- **Build a second flow for a paywall that has one**: Not without you declining to reuse the one that's there, because the duplicate would be a permanent row.
