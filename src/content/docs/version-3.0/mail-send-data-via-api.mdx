...
If the same person also reaches Adapty Mail through the Adapty SDK, add `customer_user_id` — your own user ID, the one you pass to `Adapty.identify()`. Adapty Mail matches on it and keeps both sources on one profile, instead of creating a duplicate that mails the person twice.
...
Identify the profile by `external_profile_id`, `customer_user_id`, or `email` — send at least one. Erasure is final: a later save carrying the same identifiers doesn't recreate the profile.

To make a retry safe, identify the profile by `external_profile_id` or `customer_user_id`. Repeating a deletion that carried only `email` returns `404`, because erasure clears the stored address. See the [Delete profile](api-mail/operations/deleteProfile) reference for details.
