...
:::note
The `KidsMode` trait is available starting from SDK version `4.0.0-beta.2`. Starting with SDK 4.0, the SDK is installed via Swift Package Manager only — CocoaPods is no longer supported.
:::
