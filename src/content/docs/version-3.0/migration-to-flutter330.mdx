---
title: "Migrate Adapty Flutter SDK to v3.3"
...
To verify specifically whether the available offer is a free trial rather than a paid discount, read `product.subscription?.offer?.phases` and look for `paymentMode == AdaptyPaymentMode.freeTrial` — see [Fetch products](fetch-paywalls-and-products-flutter#fetch-products).
