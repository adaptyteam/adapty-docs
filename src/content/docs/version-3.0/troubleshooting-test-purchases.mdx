...
Refer to the error list for your platform: [for iOS](ios-sdk-error-handling), [for Android](android-sdk-error-handling), [for React Native](react-native-troubleshoot-purchases), [Flutter](error-handling-on-flutter-react-native-unity),  and [Unity](unity-troubleshoot-purchases) and follow our recommendations to resolve the issue.

## The sandbox purchase is refused as unauthorized

The store shows an alert such as `You are not authorized to make purchases of this in-app in sandbox at this time`. Apple refuses the purchase, not Adapty. Verify the following on the test device and in App Store Connect:

- **In-app purchases are allowed on the device**: In **Settings** > **Screen Time** > **Apps & Websites** > **App Store**, **In-app Purchases** should be set to **Allowed**. Apple describes the setting in [Turn off Apple In-App Purchases from Screen Time](https://support.apple.com/en-us/102470).
- **The sandbox account is signed in to the right slot**: Sign in under [**Settings** > **Developer** > **Sandbox Apple Account**](test-purchases-in-sandbox#step-4-switch-to-sandbox-test-account), not under your main Apple Account. Apple's instruction is to "use the Sandbox sign-in option in Settings, not your main Apple Account settings."
- **The tester's email address isn't an existing Apple Account**: Apple states that reusing one "will cause errors during testing." You can't edit a tester's email address after creating it, so [create a new tester](test-purchases-in-sandbox#step-1-create-sandbox-test-account-in-app-store-connect) with a different address.
...
4. Ensure you are using a sandbox account – not a [local StoreKit configuration file](local-sk-files). If you have used a local StoreKit configuration file for testing before, ensure you are not using it in the current build. 
...
For more details, see [Profile creation](how-profiles-work#profile-creation).
...
For more details, check out the [Apple Developer documentation](https://developer.apple.com/documentation/storekit/testing-in-app-purchases-with-sandbox).
