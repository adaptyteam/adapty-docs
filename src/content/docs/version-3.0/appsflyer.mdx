...
1. **Receiving attribution data from AppsFlyer**  
      Once you've [set up sending Appsflyer attribution to Adapty in your app code](appsflyer#connect-your-app-to-appsflyer), Adapty will start receiving attribution data from AppsFlyer. You can easily access and view this data on the user's profile page.
...
5. For both iOS and Android, open the [AppsFlyer site](https://www.appsflyer.com/home) and log in. 
...
After you complete the steps described above, call the `updateExternalAttribution` method (named `updateAttribution` before SDK version 4.1) to save the attribution data, and use the `Adapty.setIntegrationIdentifier()` to set the integration identifier. 

Initialize the AppsFlyer SDK and wait for its UID callback before calling identifying users in Adapty. Otherwise `appsflyer_id` lands on a brief anonymous Adapty profile created during activation, and isn't always transferred to the identified profile. When that happens, AppsFlyer revenue forwarding silently fails.
...
See the [migration guide](switch-from-appsflyer-s2s-api-2-to-3) or replace the credentials with the valid ones from [here](https://hq1.appsflyer.com/security-center/api-tokens).
