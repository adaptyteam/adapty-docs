...
Initialize the AppsFlyer SDK and wait for its UID callback before calling `Adapty.activate`. Otherwise `appsflyer_id` lands on a brief anonymous Adapty profile created during activation, and isn't always transferred to the identified profile. When that happens, AppsFlyer revenue forwarding silently fails.
