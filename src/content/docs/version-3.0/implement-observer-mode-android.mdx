...
1. Render [a manual paywall with remote config](present-remote-config-paywalls-android) as usual. For flows & paywalls built in the Flow & Paywall Builder, see [Present flows in Observer mode](android-present-paywall-builder-paywalls-in-observer-mode).
2. [Associate flows & paywalls](report-transactions-observer-mode-android) with purchase transactions. 