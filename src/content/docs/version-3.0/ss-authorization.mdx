...
For [virtual currency](virtual-currencies) balances, either identifier resolves to a single profile — balances are stored per profile, not per customer user ID.
