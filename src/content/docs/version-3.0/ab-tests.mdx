...
* Only new users can participate — those who have not seen a single Adapty paywall and whose app never called the `getPaywall` SDK method. Adapty cannot guarantee a consistent paywall chain for other users.
...
* Once a user receives a crossplacement paywall, they see it for 90 days, even after you stop the test. Use the **[Cross-placement variation stickiness](general#9-cross-placement-variation-stickiness)** setting to control the duration of this assignment.
