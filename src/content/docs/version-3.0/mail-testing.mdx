...
- **Sending domain**: Your domain is verified in **Settings → DNS**. See [Set up your sending domain](mail-sending-domain).
...
- **A live entry point**: User data is actually reaching Adapty Mail. On the Adapty [entry point](mail-entry-points), that means the integration is enabled in **Settings → Project**; on the API, that means you're posting profiles and transaction events.
