...
1. Turn it on when configuring the Adapty SDK by setting the `observerMode` parameter to `true`. Follow the setup instructions for [Kotlin Multiplatform](sdk-installation-kotlin-multiplatform).
...
1. Render [a manual paywall with remote config](present-remote-config-paywalls-kmp) as usual. For flows & paywalls built in the Flow & Paywall Builder, see [Present flows in Observer mode](kmp-present-flows-in-observer-mode).
2. [Associate flows & paywalls](report-transactions-observer-mode-kmp) with purchase transactions.
