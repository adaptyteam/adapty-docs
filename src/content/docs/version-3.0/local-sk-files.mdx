...
1. [Create a product in Adapty](quickstart-products) and assign it a **App Store product ID**.
...
However, you must remember that this kind of testing is local, so no changes will appear on the Adapty dashboard. Before launching your app in the production environment, we recommend you test [working with profiles](ios-quickstart-identify) using the [sandbox environment](test-purchases-in-sandbox).
