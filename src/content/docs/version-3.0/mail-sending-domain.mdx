...
The setup wizard has three screens: enter the domain, confirm the generated subdomains, and add the DNS records. All three live in **Settings → DNS**.
...
- **Automatic polling**: Polling starts 1 minute after you submit, then doubles each round — 2, 4, 8, 16 min — before capping at 32 min. It continues until records are found or the 7-day window closes.
- **Manual check**: Click **Check Verification** to force an immediate check. There's a 10-second cooldown between manual checks — triggering too fast returns *"Verification check is on cooldown."*
...
- **7-day deadline**: If verification doesn't complete in 7 days, the identity is marked **Failed**. Your DNS records stay in your registrar — re-enter the domain in **Settings → DNS** to start a new window.
...
Your current tier and daily limit are shown in **Settings → DNS**.
...
| "Verification check is on cooldown"                                            | Wait 10 seconds between manual checks. Automatic polling continues in the background.                                     |
