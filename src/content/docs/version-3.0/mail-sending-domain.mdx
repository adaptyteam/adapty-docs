...
1. To register a domain, click **Add domain**.
2. In the **Domain** field, type your apex domain, for example `yourapp.com`.
3. In the **Sending subdomain** field, type the prefix your emails go out from, or leave it empty to use `mail`. The hint under the field spells out the resulting host: *Emails will be sent from mail.yourapp.com*.
4. Click **Add domain**. Adapty Mail validates the input, registers the domain, and adds a card for it with every DNS record the domain needs.

   <ZoomImage id="mail-email-domains.webp" width="500px" alt="Add domain form with Domain and Sending subdomain fields" />

5. Add every record on the card to your DNS at your registrar, exactly as shown. The card groups them by what they authenticate — [Which records you add](#which-records-you-add) covers each group and which ones are required. To take the whole set at once, click **Export** and pick **CSV**, **GoDaddy zone file**, or **Cloudflare zone file**.
6. Click **Check** to verify right away, or leave the card — Adapty Mail polls DNS on its own.
