...
Use quizzes to present users with predefined choices. Unlike inputs, quizzes don't have typed fields — users select from the options you define. Use them to collect preferences, segment users, or branch the flow based on their answers. Your app can also [receive the answers through the SDK](ios-flow-input) and process them internally or on your back-end.
