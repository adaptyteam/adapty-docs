...
1. Turn it on when configuring the Adapty SDK by setting the `observerMode` parameter to `true`. Follow the setup instructions for [Capacitor](sdk-installation-capacitor#activate-adapty-module-of-adapty-sdk).
...
1. Render [a manual paywall with remote config](present-remote-config-paywalls-capacitor) as usual. For flows & paywalls built in the Flow & Paywall Builder, see [Present flows in Observer mode](capacitor-present-flows-in-observer-mode).
2. [Associate flows & paywalls](report-transactions-observer-mode-capacitor) with purchase transactions. 
