...
:::important
If you render the paywall yourself (without [Adapty's no-code builders](adapty-flow-builder)), call [`logShowFlow` (iOS SDK v4+) / `logShowPaywall`](present-remote-config-paywalls#track-paywall-view-events) when you display Flow/Paywall A. Without it, Adapty cannot count views in the test. Do not log a view for Flow/Paywall B, since it is never shown.
:::
