...
Apple returns a stable user identifier in the `user` field of `ASAuthorizationAppleIDCredential`. Apple provides this value only during the first authorization — persist it immediately to the Keychain, because it is not returned on subsequent sign-ins.
...
func authorizationController(
    ...
    let appleUserId = credential.user // Apple returns this only once — persist it

    Task {
        do {
            try await Adapty.identify(appleUserId)
        } catch {
            // handle the error
        }
    }
...
<TabItem value="swift-callback" label="Swift-Callback">
...
func authorizationController(
    ...
    Adapty.identify(appleUserId) { error in
        if let error {
            // handle the error
        }
    }
...
Use `uid` from `FirebaseAuth` — not the email address. Firebase UIDs are permanent and unique per user; email addresses can be changed or transferred between accounts.
...
```swift showLineNumbers
// Generate once per user, persist to Keychain, and reuse on subsequent launches
...
let configurationBuilder =
    AdaptyConfiguration
        .builder(withAPIKey: "PUBLIC_SDK_KEY")
        .with(customerUserId: "YOUR_USER_ID", withAppAccountToken: appAccountToken)
...
do {
    try await Adapty.identify("YOUR_USER_ID", withAppAccountToken: appAccountToken)
} catch {
    // handle the error
}
```
...
// Or when identifying a user:
Adapty.identify("YOUR_USER_ID", withAppAccountToken: appAccountToken) { error in
    if let error {
        // handle the error
    }
}
