...
With Adapty, you can create a paywall or [flow](adapty-flow-builder) with a button that redirects your app users to their browser for payment. Then, when they return to your app after a successful purchase, the subscription activates.
...
- If you are using the Flow & Paywall Builder, you only need to [add a new button](#step-2a-add-a-web-purchase-button) that will use the link you've provided to track purchases and send the data back to Adapty.
- If you are using the SDK, you must set up the [`openWebPaywall`](#step-2b-call-the-sdk-method) method to handle web paywalls.
...
To create a cancellation funnel, in the web paywall editor, open the **Cancellation** tab. Retention actions are available only in cancellation funnels, so you can't add them to the funnel that serves your web paywall. For the funnel itself, see [FunnelFox's guide to cancellation funnels](https://funnelfox.com/docs/dashboard/funnels#cancellation-funnels).

<ZoomImage id="cancellation-funnel.webp" width="700px" alt="Cancellation tab in the web paywall editor" />

### Get the subscriber's FunnelFox profile ID

A cancellation funnel identifies the subscriber by their FunnelFox profile ID. After a web subscription purchase, FunnelFox writes that ID to the Adapty [profile](profiles-crm) as the `fnlfx_profile_id` custom attribute, together with `fnlfx_funnel_id`, `fnlfx_session_id`, `vendor_profile_id`, and `vendor`.
