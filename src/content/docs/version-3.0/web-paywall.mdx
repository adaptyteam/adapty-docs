...
- **Paywall Builder**: 3.6.1 or later (iOS), 3.15 or later (Android and cross-platform)
...
A web paywall is a unique URL generated for each of your in-app paywalls. It opens in the browser for payment and works with different payment providers (Stripe, Paddle, and others), supporting both a simple page with an Apple Pay button and more complex flows with upsells.
...
6. When they return to the app, Adapty SDK starts **tracking whether the profile was updated** by the purchase.
7. Adapty gets information about the purchase event, records the purchase as an analytics event, and monitors it for any updates.

## Step 1. Create a web paywall

1. To enable external payments for an existing paywall, [duplicate](duplicate-paywalls) it so you can show it only to your target user segment and show the old one to all other users. To start from scratch, [create](create-paywall) a new paywall.
...
:::tip
Use our [quickstart guide](web-paywall-configuration) to launch a working web paywall.
:::
