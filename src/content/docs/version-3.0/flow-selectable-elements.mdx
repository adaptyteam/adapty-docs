...
metadataTitle: "Selectable elements and groups | Flows & paywalls | Adapty Docs"
---
...
- [Use default selectable elements](#default-selectable-elements) — quiz options, products, tabs, toggles, and checkboxes are selectable out of the box
...
- **Toggles and checkboxes**: Trial toggles, toggles, and checkboxes each belong to a group and get a selected state. See [Toggles and checkboxes](builder-toggles).
...
- **[Your app](ios-flow-input)**: Handle the SDK's analytics callback to receive an array with the currently selected options whenever the user interacts with your quiz. Process the selection in your app, or on a back-end server.

For product groups, you can also set the selection instead of only reading it. A [Select product](onboarding-actions#select-product) action moves the selection to a specific product element in a group on the same screen.
