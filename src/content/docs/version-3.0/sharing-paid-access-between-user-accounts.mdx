...
If you disable paid access sharing, Adapty ties the product to the active [customer ID](identifying-users#set-customer-user-id-on-configuration) at the time of the purchase, and does not share the access level with any other customer profiles. This policy allows for strict 1-to-1 product distribution.
...
* If two or more profiles share the same access level, the purchase is attributed to the [parent profile](how-profiles-work#parent-and-inheritor-profiles).
* Access level inheritance does not impact installation statistics. To determine how Adapty counts installations, you can select one of the two available [installation definitions](installs#counting-modes) on the settings page.
