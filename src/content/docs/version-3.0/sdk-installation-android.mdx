...
:::important
Adapty Android SDK 4.0 is a pre-release. Gradle does not select pre-release versions through dynamic version ranges (like `+` or `latest.release`), so you must pin the exact version. Set the `adapty-bom` version to the 4.0 pre-release — for example `io.adapty:adapty-bom:4.0.0-beta.1`, or `adaptyBom = "4.0.0-beta.1"` in `libs.versions.toml`. The BOM resolves the matching `android-sdk` and `android-ui` versions for you. See [Migrate Adapty Android SDK to v4](migration-to-android-sdk-v4).
:::
