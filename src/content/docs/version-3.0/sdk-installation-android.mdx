...
From 3.17.3 on the 3.x line and from 4.0.2 on the 4.x line, Adapty Android SDK uses Billing Library v8 by default. Versions 3.15.0 through 3.17.2 and 4.0.0 through 4.0.1 use v7, and also work when another dependency in your build raises the Billing Library to v8.
...
- If you use the [Flow & Paywall Builder](adapty-flow-builder), follow the [quickstart](android-quickstart-paywalls).
...
Starting from Adapty SDK version 4.1, [Adapty Attribution](user-acquisition) is disabled by default. When activating the Adapty module, set `adaptyAttributionEnabled` to `true` to let the SDK register installs and deliver installation details to your app.
