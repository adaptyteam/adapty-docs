...
:::tip
To fill that cache before the user needs the flow, call [`preloadFlows`](android-optimize-paywall-fetching#preload-placements) (Android SDK 4.1+) for several placements at once. A later `getFlow` reads the preloaded copy only with `AdaptyPlacementFetchPolicy.ReturnCacheDataElseLoad`.
:::
