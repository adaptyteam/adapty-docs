...
:::note
In v4, the `locale` parameter has moved out of `getFlow` and onto `getViewConfiguration` (used only when rendering with AdaptyUI). For custom paywalls, all available locales are returned together in `flow.remoteConfigs` — pick the locale that matches the user's device or your app's setting.
:::
