...
Adapty Flutter SDK requires **iOS 15.0+**, **Xcode 26+**, and **Flutter 3.32.0+** (Dart 3.8.0+). See [Swift Package Manager (iOS)](#swift-package-manager-ios) below for the installation specifics.
...
|---|---|
| 4.0.4 and later | v8 |
| 4.0.0–4.0.3 | v7 by default, v8 if another dependency raises it |
...
:::important
These steps require **Flutter 3.32.0+** (Dart 3.8.0+). The plugin pulls the native iOS SDK through Swift Package Manager — see [Swift Package Manager (iOS)](#swift-package-manager-ios) for the one-time setup.
:::
...
The plugin pulls the native iOS SDK through **Swift Package Manager**. If you are on Flutter 3.32–3.43, enable Swift Package Manager support once:
...
:::note
This applies only if the Adapty iOS SDKs install as CocoaPods in your project. When they install as Swift packages, a `SWIFT_VERSION` override in `post_install` doesn't reach them.
:::
