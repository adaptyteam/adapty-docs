...
Adapty Flutter SDK 4.0 — which adds support for [flows](adapty-flow-builder) — raises the minimums to **iOS 15.0+**, **Xcode 26+**, and **Flutter 3.32.0+** (Dart 3.8.0+). See [Adapty SDK 4.0](#adapty-sdk-40-swift-package-manager) below for the installation specifics.
...
|---|---|
| 3.17.0 and later | v8 |
| 3.15.0–3.15.5 | v7 by default, v8 if another dependency raises it |
...
:::important
The steps below install the latest stable SDK (3.x). If you need v4 — required for the [Flow & Paywall Builder](adapty-flow-builder) and used by the [quickstart](flutter-quickstart-paywalls) — follow [Adapty SDK 4.0: Swift Package Manager](#adapty-sdk-40-swift-package-manager) below instead.
:::
...
Add Adapty Flutter SDK 4.0 — which adds support for [flows](adapty-flow-builder) — to your `pubspec.yaml`:
...
Starting with v4, the native iOS SDK is no longer distributed through CocoaPods — the plugin pulls it through **Swift Package Manager** only ([CocoaPods' spec repo goes read-only in December 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). If you are on Flutter 3.32–3.43, enable Swift Package Manager support once:
...
- If you use the [Flow & Paywall Builder](adapty-flow-builder), first [activate the AdaptyUI module](#activate-adaptyui-module-of-adapty-sdk) below, then follow the [quickstart](flutter-quickstart-paywalls).
- If you build your own paywall UI, see the [quickstart for custom paywalls](flutter-quickstart-manual).

## Activate AdaptyUI module of Adapty SDK

If you plan to use the [Flow & Paywall Builder](adapty-flow-builder) and have [installed AdaptyUI module](sdk-installation-flutter#install-adapty-sdk), you also need to activate AdaptyUI:
...
The module is activated automatically with the Adapty SDK. If Adapty doesn't render your screens and you want to deactivate the AdaptyUI module, pass `withActivateUI(false)` during activation.
...
If you use [Adapty Attribution](user-acquisition), call `withAdaptyAttributionEnabled(true)` when activating the SDK. The default value is `false`: without this parameter, the SDK doesn't register installs or deliver installation details to your app. In SDK versions below 4.1, Adapty Attribution is enabled automatically.
