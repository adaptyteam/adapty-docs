...
Adapty Flutter SDK 4.0 (beta) — which adds [Flow Builder](adapty-flow-builder) support — raises the minimums to **iOS 15.0+**, **Xcode 26+**, and **Flutter 3.32.0+** (Dart 3.8.0+). See [Adapty SDK 4.0 (beta)](#adapty-sdk-40-beta-swift-package-manager) below for the installation specifics.
...
:::important
The steps below install the latest stable SDK (3.x). If you need v4 (beta) — required for the [Flow Builder](adapty-flow-builder) and used by the [quickstart](flutter-quickstart-paywalls) — follow [Adapty SDK 4.0 (beta): Swift Package Manager](#adapty-sdk-40-beta-swift-package-manager) below instead, because pub doesn't resolve pre-releases through caret ranges.
:::
...
Adapty Flutter SDK 4.0 — which adds [Flow Builder](adapty-flow-builder) support — is a pre-release, so pin the exact version in `pubspec.yaml` (pub doesn't resolve pre-releases through caret ranges):
