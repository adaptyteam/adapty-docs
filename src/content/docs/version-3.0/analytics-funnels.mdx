...
We collect data for this step from all your paywalls that use the `logShowFlow()` (iOS SDK v4+) / `logShowPaywall()` method. So please be sure to send every paywall view to Adapty using this method as described in the [docs](present-remote-config-paywalls#track-paywall-view-events).  
