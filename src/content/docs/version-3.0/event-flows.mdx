...
One action often produces several events. For example, a first purchase sends both **Subscription started** and **Access level updated**. The diagrams below show what Adapty sends when a customer subscribes, cancels, or reactivates, and in what order.

## How to read the diagrams

- **Timing.** Apple charges for a subscription several hours before it starts or renews. Showing that gap would add a step to every diagram without changing which events you receive, so the diagrams put the charge and the start together.
- **Order.** Events from one action arrive at the same moment. A diagram has to draw them in some order, but your [Event Feed](event-feed) may list them in a different one, and neither is more correct.
- **Coverage.** A store notification triggers every event shown. [Synthetic events](events#synthetic-events) don't appear: Adapty sends them on a timer, and the delay differs per integration, so they have no fixed place in a sequence.
...
<ZoomImage id="Subscription_Rejoin_Flow.webp" width="700px" alt="Subscription Rejoin Flow diagram" />
...
<ZoomImage id="Subscription_Paused_Flow.webp" width="700px" alt="Subscription Paused Flow diagram" />
...
<ZoomImage id="Subscription_Reactivation_Flow_after_Expired_Trial.webp" width="700px" alt="Subscription Reactivation Flow after Expired Trial diagram" />
...
When a <InlineTooltip tooltip="Customer User ID">[iOS](identifying-users#set-customer-user-id-on-configuration), [Android](android-identifying-users#setting-customer-user-id-on-configuration), [React Native](react-native-identifying-users#setting-customer-user-id-on-configuration), [Flutter](flutter-identifying-users#setting-customer-user-id-on-configuration), and [Unity](unity-identifying-users#setting-customer-user-id-on-configuration)</InlineTooltip> attempts to restore or extend a subscription already tied to a different <InlineTooltip tooltip="Customer User ID">[iOS](identifying-users#set-customer-user-id-on-configuration), [Android](android-identifying-users#setting-customer-user-id-on-configuration), [React Native](react-native-identifying-users#setting-customer-user-id-on-configuration), [Flutter](flutter-identifying-users#setting-customer-user-id-on-configuration), and [Unity](unity-identifying-users#setting-customer-user-id-on-configuration)</InlineTooltip>, Adapty's **Sharing paid access between user accounts** setting controls how access is managed. The flow will vary depending on the selected option.

:::note
For Apple Family Sharing transactions (`in_app_ownership_type=FAMILY_SHARED`), only the **Access level updated** event fires — the per-product subscription events below do not. See [Apple Family Sharing](apple-family-sharing) for the full event matrix.
:::
