...
- **Which experiment variant earned more?** Use Adapty's revenue events as the metric for a PostHog experiment on any product change. Adapty's [A/B tests](ab-tests) cover paywalls and onboardings.
...
- **How do subscribers retain compared to free users?** Build cohorts and retention curves on derived access status — see [How to tell a subscriber from a free user](#how-to-tell-a-subscriber-from-a-free-user).
...
Mismatched IDs cause data fragmentation — see [One user appears as several persons](#one-user-appears-as-several-persons-in-posthog).
...
1. Sign in to the deployment that hosts your data — [US Cloud](https://us.posthog.com/) or [EU Cloud](https://eu.posthog.com/) — and select the project you want to connect.
...
- **No access level or subscription status.** The `access_level_updated` event is exclusive to [webhook integrations](webhook), so Adapty sends PostHog no field describing what the user currently has access to — see [How to tell a subscriber from a free user](#how-to-tell-a-subscriber-from-a-free-user).
...
- [Events don't appear in PostHog](#events-dont-appear-in-posthog)
- [`access_level_updated` shows as failed in the Event Feed](#access_level_updated-shows-as-failed-in-the-event-feed)
- [One user appears as several persons in PostHog](#one-user-appears-as-several-persons-in-posthog)
- [Revenue in PostHog doesn't match Adapty Analytics](#revenue-in-posthog-doesnt-match-adapty-analytics)
- [Events appear in PostHog before they happen](#events-appear-in-posthog-before-they-happen)
- [No country or city data on Adapty events](#no-country-or-city-data-on-adapty-events)
- [How to tell a subscriber from a free user](#how-to-tell-a-subscriber-from-a-free-user)
- [Paywall views don't appear in PostHog](#paywall-views-dont-appear-in-posthog)
...
PostHog cannot undo most splits after the fact — see [Match Adapty's ID to PostHog's](#match-adaptys-id-to-posthogs).
