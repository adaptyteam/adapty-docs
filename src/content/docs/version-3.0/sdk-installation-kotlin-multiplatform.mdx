...
When building your iOS host app, you may see Swift 6 compilation errors on Adapty pod targets. Typical symptoms include `@Sendable` mismatches in `AdaptyUIBuilderLogic`, missing `Sendable` conformance on Adapty types, or actor isolation errors.
...
The most common cause is a `post_install` hook in your iOS app's `Podfile` (typically `iosApp/Podfile`) that rewrites `SWIFT_VERSION` for every pod target:

```ruby showLineNumbers title="iosApp/Podfile"
...
Then run `pod install` from the iOS app directory and rebuild.

To verify, open `Pods.xcodeproj` in your iOS app's `Pods/` folder, select the `Adapty` pod target → **Build Settings** → **Swift Language Version**. It should be **Swift 6**.