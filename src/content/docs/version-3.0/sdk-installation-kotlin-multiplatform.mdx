...
- **AdaptyUI** (`io.adapty:adapty-kmp-ui`): This module renders the screens Adapty draws for you, with the Compose Multiplatform rendering layer (`view.present()`). If your project does not use Compose Multiplatform, you can use [`createNativePaywallView`](kmp-present-paywalls#without-compose-multiplatform) and [`createNativeOnboardingView`](kmp-present-onboardings#without-compose-multiplatform) from the core module instead.
...
- If you use the [Flow & Paywall Builder](adapty-flow-builder), first [activate the AdaptyUI module](#activate-adaptyui-module-of-adapty-sdk) below, then follow the [Paywall Builder quickstart](kmp-quickstart-paywalls).
