...
:::important
Adapty Kotlin Multiplatform SDK 4.0 is a pre-release. Gradle does not select pre-release versions through dynamic version ranges (like `+` or `latest.release`), so you must pin the exact version — for example `io.adapty:adapty-kmp:4.0.0-beta.1`, or `adapty-kmp = "4.0.0-beta.1"` in `libs.versions.toml`. See [Migrate Adapty Kotlin Multiplatform SDK to v4](migration-to-kmp-sdk-v4).
:::
