...
- **AdaptyUI** (`io.adapty:adapty-kmp-ui`): This module is needed if you use the [Adapty Paywall Builder](adapty-paywall-builder) with the Compose Multiplatform rendering layer (`view.present()`). If your project does not use Compose Multiplatform, you can use [`createNativePaywallView`](kmp-present-paywalls#without-compose-multiplatform) and [`createNativeOnboardingView`](kmp-present-onboardings#without-compose-multiplatform) from the core module instead.
...
:::important
Adapty Kotlin Multiplatform SDK 4.0 is a pre-release. Gradle does not select pre-release versions through dynamic version ranges (like `+` or `latest.release`), so you must pin the exact version — for example `io.adapty:adapty-kmp:4.0.1-beta.1`, or `adapty-kmp = "4.0.1-beta.1"` in `libs.versions.toml`. See [Migrate Adapty Kotlin Multiplatform SDK to v4](migration-to-kmp-sdk-v4).
:::
...
- If you use [Adapty Paywall Builder](adapty-paywall-builder), first [activate the AdaptyUI module](#activate-adaptyui-module-of-adapty-sdk) below, then follow the [Paywall Builder quickstart](kmp-quickstart-paywalls).
- If you build your own paywall UI, see the [quickstart for custom paywalls](kmp-quickstart-manual).

## Activate AdaptyUI module of Adapty SDK

If you plan to activate the **AdaptyUI** module to use the [Adapty Paywall Builder](kmp-present-paywalls), make sure to set `.withActivateUI(true)` in your configuration.
