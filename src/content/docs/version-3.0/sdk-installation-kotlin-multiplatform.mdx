...
- **AdaptyUI** (`io.adapty:adapty-ui-kmp`): This module is needed if you use the [Adapty Paywall Builder](adapty-paywall-builder) with the Compose Multiplatform rendering layer (`view.present()`). If you display paywalls and onboardings using [native platform views](kmp-present-paywalls#embed-in-a-native-view), this dependency is not required — native view support is built into the core module.
