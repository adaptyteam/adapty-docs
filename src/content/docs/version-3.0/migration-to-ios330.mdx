---
title: "Migrate Adapty iOS SDK to v3.3"
...
To verify specifically whether the available offer is a free trial rather than a paid introductory discount, read `product.subscriptionOffer` and verify that `price` is `0` — see [Fetch products](fetch-paywalls-and-products#fetch-products).
