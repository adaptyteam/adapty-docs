...
Once support imports a transaction's `apple_original_transaction_id` or `google_purchase_token`, Adapty matches future App Store and Play server notifications for that transaction to the same profile automatically — the user doesn't need to open the app first.
:::

:::warning
If a webhook or other integration grants rewards (for example, virtual currency) on a subscription event, enable **Exclude Historical Events** for that integration in [integration settings](configuration) before you start the import. Without it, imported historical purchases re-fire integration events, and a reward-granting webhook grants the reward a second time for a purchase the user already has. See [Migrate integrations to Adapty](migrate-integrations-to-adapty#exclude-historical-events).
:::
...
:::tip
If you're migrating from RevenueCat, you can send the RevenueCat export file directly — no conversion needed. See [RevenueCat's documentation](https://www.revenuecat.com/docs/integrations/scheduled-data-exports) for export instructions.
:::
