...
## Before you start: verify the paywall has products

The same error occurs when the paywall your placement resolves to has no products attached — the SDK then requests an empty product list, and no App Store check can fix that. In the Adapty Dashboard, open the paywall shown in this placement and confirm its **Products** section lists the products you expect. If it does, continue with Step 1.
