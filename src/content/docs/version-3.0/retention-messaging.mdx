---
title: "Retention Messaging"
description: "Show subscribers a custom message inside Apple's Cancel Subscription screen. You create it in Adapty, and Adapty delivers it to Apple automatically."
metadataTitle: "Apple Retention Messaging | Adapty Docs"
---

<CustomDocCardList ids={['retention-messaging-setup', 'retention-messaging-create']} />

Retention Messaging shows subscribers a custom message on the iOS Cancel Subscription screen, before they confirm a cancellation. Use it to persuade churning subscribers to stay.

A default message gives every subscriber a reason to stay. A promo message adds a concrete incentive: a discount, a free trial, or an opportunity to switch to a more fitting plan. Apple shows each subscriber the message that matches their product and language.

You create, localize, and test messages in the Adapty Dashboard. Adapty delivers them to Apple automatically, with no app release. 

<ZoomImage id="retention-messaging-api.webp" width="700px" alt="Retention Messaging page in the Adapty Dashboard with the messages table and Create Default and Create Message buttons" />

## Quickstart

Before you start, you need:

- Access to [Apple's Retention Messaging program](https://developer.apple.com/documentation/retentionmessaging).
- An [App Store Connect API key](app-store-connection-configuration) in Adapty.

Then:

1. [Connect Adapty to the Retention Messaging API in sandbox mode](retention-messaging-setup).
2. [Create a default sandbox message](retention-messaging-create) and purchase the product it covers in sandbox mode.
3. Pass Apple's connectivity test.
4. Switch to production mode.
5. Create your production messages — first, default messages, then promo messages — and submit each for Apple's review.
