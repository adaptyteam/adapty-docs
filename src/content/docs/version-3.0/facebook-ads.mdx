...
Facebook CAPI is a separate integration from the Facebook Ads integration described above, with its own credentials, and you configure it independently in the Adapty Dashboard. If you enable it, Adapty forwards a hashed identity payload to Meta's Conversions API for advanced matching. From the customer's Adapty profile, Adapty SHA-256-hashes and sends: email, phone number, first and last name, birthday, gender, and country. Facebook click and browser identifiers (`fbc`, `fbp`) and IP address/user agent travel unhashed, the same as any Meta CAPI integration.
