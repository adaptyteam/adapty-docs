...
import FilterMetricsByInstallDate from '@site/src/components/reusable/FilterMetricsByInstallDate.mdx';
...
:::warning
Make sure to send paywall views to Adapty using the `.logShowFlow()` (iOS SDK v4+) / `.logShowPaywall()` method. Otherwise, paywall views will not be accounted for in the metrics and conversions will be irrelevant.
:::