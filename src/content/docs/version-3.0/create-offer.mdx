...
In the [Flow & Paywall Builder](adapty-flow-builder), an offer is attached to a product on a Products element. First add the product element and assign products to it — see [Set up purchases](paywall-product-block).

To attach an offer:

1. On the canvas, select the product card that should show the offer.
2. In the right panel, under **Product**, select the product, then pick the offer from the offer dropdown. The dropdown reads **No offer** until you pick one, and it lists offers only for products that have them.
