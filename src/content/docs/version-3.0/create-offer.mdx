...
1. [Add offers to products in Adapty using the offer IDs from stores.](#1-add-offer-to-product-in-adapty)
...
Offers apply to subscriptions only: products with a **Lifetime**, **Non-Subscriptions** or **Consumables** duration can't carry one. To discount a one-time product, [create a second product at the discounted price and target it with audiences](#how-adapty-works-with-offers) instead.
...
In the [Flow & Paywall Builder](adapty-flow-builder), you pair an offer with a product in the **Products** panel or on a product card.

#### In the Products panel

- **When you add the product**: Click **+**, select the product, and pick the offer under **Offer**. Cards that use the product afterwards get this offer pre-selected. The full steps are in [Add products in the Products panel](paywall-product-block#add-products-in-the-products-panel).
- **For a product already on the screen**: Open the product entry's context menu and select [**Offer**](paywall-product-block#product-actions). Every card, text variable, and action that uses the product switches to the new offer.

#### On a product card

1. On the canvas, select the product card that should show the offer.
2. In the right panel, under **Product**, select the product, then pick the offer from the offer dropdown. If you added the product with an offer in the **Products** panel, that offer is pre-selected. A different offer adds a second entry for the product to the panel.
...
:::info
You can change products and offers on a paywall while it's in the **Draft** state, or within 30 days of creating it if it has no transactions yet. After that Adapty rejects the change, and the dashboard disables the product fields and explains why. Swapping one offer for another on a product that's already on the paywall counts as such a change.

To change products or offers on a paywall past that point, [duplicate it](duplicate-paywalls) and configure the copy. Flows don't have this restriction — you can change their products and offers at any time.
:::
