---
title: "Render a manual paywall with remote config in Kotlin Multiplatform SDK"
description: "Read remote config values and render your own paywall screen in the Adapty Kotlin Multiplatform SDK."
metadataTitle: "Remote config in a manual paywall | Kotlin Multiplatform SDK | Adapty Docs"
...
After you've done with displaying the paywall, continue with setting up a purchase flow. When the user makes a purchase, simply call `.makePurchase()` with the product from your flow. For details on the`.makePurchase()` method, read [Making purchases](kmp-making-purchases).
...
:::warning
Make sure to [record the paywall view event](present-remote-config-paywalls-kmp#track-paywall-view-events-1) as described below, allowing Adapty analytics to capture information for funnels and A/B tests.
:::
