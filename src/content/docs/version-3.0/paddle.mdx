...
:::note
This integration focuses on tracking and syncing Paddle web purchases. If you need to send users from the app to a web checkout, use Adapty [web paywalls](web-paywall).
:::
...
2. The `email` from the Paddle Customer object (see [Paddle docs](https://developer.paddle.com/paddle-js/methods/paddle-checkout-open/#parameters))
3. The Paddle Customer ID in the `ctm-...` format (see [Paddle docs](https://developer.paddle.com/paddle-js/methods/paddle-checkout-open/#parameters))
...
Adjust has no Paddle fields: Adapty sends a Paddle purchase to Adjust with the app token of the platform the buyer installed your app on. See [Web purchases (Stripe and Paddle)](adjust#web-purchases-stripe-and-paddle).
