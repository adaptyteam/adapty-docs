...
- **Per-user overrides**: A refund preference and a consent value for a specific user. When set, they take priority over the app-level defaults. Set the preference from the [Dashboard](#set-refund-behavior-for-a-specific-user-in-the-dashboard), the [SDK](#set-a-refund-preference-for-a-user), or the [server-side API](api-adapty/operations/setRefundSaverSettings) — all three write the same value. Consent updates come from the [SDK](#update-a-users-consent) or the API.
...
A prorated refund is the middle option between declining and granting. It works on the App Store only. You return the unused part of the purchase and keep the rest, instead of risking the whole payment on a decline. Prorated refunds fit purchases the user has partly used — a coin pack with half the coins spent, or a yearly plan canceled in the second month. Google Play's refund reviews accept decline, approve, or no preference and nothing in between, so Refund Saver can't ask for a partial refund there. You can still issue one by hand from Play Console Order management.
...
<ZoomImage id="refund-saver-list.webp" width="700px" alt="Refund Saver section showing the Saved by Refund Saver, Refunds requested, and Win rate cards above a table of answered refund requests" />
...
Use the SDK for choices the user makes in your own UI — a consent prompt, a preference toggle. The Dashboard and the [server-side API](api-adapty/operations/setRefundSaverSettings) write the same two values. Android has no equivalent methods — the multi-platform frameworks write these values on iOS only.
