...
## How a mixed balance is spent

A currency's balance can hold credits from more than one source at once — for example, an expiring subscription grant and a non-expiring one-time-product grant. When a spend debits the balance, Adapty draws down expiring credits first, oldest grant first, and reaches non-expiring credits only once no expiring credits remain. This is why a subscription lapsing resets only the unspent portion that came from that subscription — credits granted by a one-time product stay in the balance.
