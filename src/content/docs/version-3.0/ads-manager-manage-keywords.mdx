...
- The **Metrics** tab shows a chart with CPT Bid and Spend over time. A marker appears at each point where the bid was changed — hover over a marker to see the exact bid amount and spend on that date. Use this to correlate bid changes with shifts in performance: if impressions dropped or CPA spiked after a change, the marker pinpoints when it happened.
