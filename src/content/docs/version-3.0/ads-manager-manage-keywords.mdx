---
title: "Manage keywords in Adapty Ads Manager"
description: "Add and manage Apple Ads keywords, negative keywords, and SKAG keywords in Adapty Ads Manager."
metadataTitle: "Manage keywords | Adapty Ads Manager | Adapty Docs"
keywords: ['adapty ads manager', 'adapty search ads', 'apple ads', 'asa']
---
import ZoomImage from '@site/src/components/ZoomImage';

Adapty Ads Manager has a two-way integration with Apple Ads: you get near real-time performance data, and you can create and edit keywords directly from the Adapty dashboard in a much more convenient way than in the native UI.

If you create a keyword in the native Apple Ads dashboard, it will automatically appear in Adapty Ads Manager within 24 hours.

In addition to [exploring comprehensive analytics](adapty-ads-manager-analytics), you can manage all your keyword settings:
...
:::tip
To find underperforming keywords across your account — for example, keywords with spend but no conversions — ask the [AI Agent](ads-manager-ai-agent).
:::

## What are keywords

Keywords are the search terms that trigger your ads to appear in App Store search results. They are organized within [ad groups](ads-manager-create-ad-group), which belong to [campaigns](ads-manager-create-campaign). This hierarchical structure allows you to organize and manage your advertising strategy effectively.
...
:::note
Keywords in **Maximize Conversions** campaigns don't use bids — bidding is managed automatically by the campaign's CPA target. The **CPT bid** field doesn't apply to them.
:::
...
4. In the **Add keywords** dialog, enter keywords in the **Keywords list** field. If you have a comma-delimited file with keywords, you can paste its contents so Adapty Ads Manager uploads all the keywords in bulk.
...
3. Enter negative keywords in the **Keywords list** field. If you have a comma-delimited file with keywords, you can paste its content, so Adapty Ads Manager uploads all the keywords in bulk.
...
5. Enter keywords in the **Keywords list** field. If you have a comma-delimited file with keywords, you can paste its content, so Adapty Ads Manager uploads all the keywords in bulk.
...
:::note
Edits to keywords made directly in Apple Ads sync automatically to Adapty Ads Manager, but may take time to show in Adapty Ads Manager.
:::
...
3. Click the **Actions** dropdown and select one of the following options:
   ...
   - **Create segment from keywords**: Create an audience segment from the selected keywords
   - **Copy keywords**: Copy the selected keyword names to your clipboard
   - **Edit CPT bids**: Edit the CPT bids for the selected keywords. You can edit them in different ways:
     ...
     - **Set to average CPT**: Use the CPT (cost-per-tap) metric to align the bid with it. Set a multiplier coefficient. For example, set the multiplier to 0.9 when performance is below expectations, or to 1.1 when it’s outperforming.
     - **Set to average CPA**: Use the CPA (cost-per-acquisition) metric to align the bid with it. Set a multiplier coefficient.

The **Negative keywords** tab includes a limited set of actions:

- **Add as keywords**
- **Add as negative keywords**
- **Add as SKAG**
- **Delete keywords** are available.
...
To export the keywords table as a CSV, click the download icon above the table and select **Export current page** or **Export all pages**.

**Export all pages** downloads all keywords across all pages in a single file. A progress modal tracks the download; you can cancel it at any time. Two optional filters are available:

- **Enabled only**: Include only active keywords.
- **With spend ≥**: Include only keywords with spend above a specified threshold.
- **Group by country**: Break down each keyword row by country.
...
To view bid history for a keyword, click the **chart icon** next to it in the table. A panel opens with two tabs: **Metrics** and **Bid History**.

- The **Metrics** tab shows a chart with metrics over time. You can add and remove metrics the same way as in keyword-level charts — click **+** to add, or clear checkboxes to hide them. A marker appears at each point where the bid was changed — hover over a marker to see the exact bid amount on that date. Use this to correlate bid changes with shifts in performance: if a metric dropped or spiked after a change, the marker pinpoints when it happened.
- The **Bid History** tab lists each bid change: its date, type, previous and new values, and what caused it — a manual change or an automation rule (shown with the rule's ID).
