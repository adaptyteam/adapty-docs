...
Use **Last 28 days** to track weekly subscription products.
...
<ZoomImage id="c07addf-CleanShot_2023-07-10_at_20.59.372x.webp" width="700px" alt="Period comparison on an Adapty analytics chart" />
...
Switch to the table view to inspect the chart's underlying data in a tabular format. Use it to read exact values or work with the data outside Adapty.

### Export data to CSV

Click the **Export** button to download the chart's underlying data as a CSV file. Adapty offers the same data through [the API](export-analytics-api) — both methods produce identical files.
