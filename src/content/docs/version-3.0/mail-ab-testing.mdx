...
Each variation is a full campaign. Variations can differ in copy, hero images, tone, sequence length, or delay timing between emails. What they share is the [purpose](mail-create-campaign) — a test compares like with like, so every campaign in it is written for the same moment of the customer lifecycle. The A/B test itself doesn't expose those as dials; you create the campaigns separately and add them as variations.
