...
2. Turn it on when configuring the Adapty SDK by setting the `observerMode` parameter to `true`. Follow the setup instructions for [iOS](sdk-installation-ios#activate-adapty-module-of-adapty-sdk), [Android](sdk-installation-android#activate-adapty-module-of-adapty-sdk), [React Native](sdk-installation-reactnative), [Flutter](sdk-installation-flutter#activate-adapty-module-of-adapty-sdk), [Kotlin Multiplatform](sdk-installation-kotlin-multiplatform#activate-adapty-sdk), and [Unity](sdk-installation-unity#activate-adapty-module-of-adapty-sdk).
...
In Observer mode, paywalls built in Adapty's no-code builders require you to:

- present them in Observer mode ([iOS](ios-present-paywall-builder-paywalls-in-observer-mode), [Android](android-present-paywall-builder-paywalls-in-observer-mode), [Flutter](flutter-present-flows-in-observer-mode), [Capacitor](capacitor-present-flows-in-observer-mode));
- [associate each transaction with its paywall](report-transactions-observer-mode) when you report purchases.