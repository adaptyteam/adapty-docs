...
| **discount_amount_local**     | Float         | The amount subtracted from the standard price for the transaction. `0` when there's no offer, and for a <InlineTooltip tooltip="free trial">`price_*` already reports the standard price, so nothing is subtracted</InlineTooltip>. `null` when an offer applied but the standard price is unknown, including App Store pay-up-front offers. |
...
| **original_price_local**      | Float         | The product's standard, non-discounted renewal price before the Apple/Google cut, in local currency. Equals `price_local` for a full-price purchase; `null` for a discounted purchase when the store reports no reliable standard price (for example, auto-renew is off, the renewal still carries an offer, or a product change is pending). |
