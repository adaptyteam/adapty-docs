...
| **attributions** | JSON | [Attribution data](webhook-event-types-and-fields#attributions). Included if **Send Attribution** is enabled in [Webhook settings](https://app.adapty.io/integrations/customwebhook). |
...
| **event_datetime** | ISO 8601 | The effective (business) time of the event, such as the purchase date for a purchase or the expiration date for an expiration — not when Adapty received or sent the event. [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html) format (e.g., `2020-07-10T15:00:00.000000+0000`). See the note below on ordering. |
...
| **base_plan_id**              | String        | [Base plan ID](https://support.google.com/googleplay/android-developer/answer/12154973) in the Google Play Store or [price ID](https://docs.stripe.com/products-prices/how-products-and-prices-work#use-products-and-prices) in Stripe.                                                                                                                                  |
...
| **discount_amount_local**     | Float         | The discount applied to the transaction: the standard price minus the amount actually charged, before the Apple/Google cut, in local currency. `0` for a full-price purchase. For a free trial, equals the full standard price (`original_price_local`), since nothing is charged. `null` when an offer applied but the standard price is unknown (see `original_price_local`). Always `null` for App Store pay-up-front offers: the single up-front charge covers multiple billing periods, so it can't be compared with the per-period standard price. |
| **discount_amount_usd**       | Float         | The value of `discount_amount_local` in USD. |
...
| **original_price_local**      | Float         | The product's standard, non-discounted price before the Apple/Google cut, in local currency. For subscriptions, this is the renewal price. Equals `price_local` for a full-price purchase and always equals `price_local` for one-time purchases, as stores don't report a separate standard price for them. `null` for a discounted purchase when the store reports no reliable standard price (for example, auto-renew is off, the renewal still carries an offer, or a product change is pending). |
| **original_price_usd**        | Float         | Same as `original_price_local`, in USD. |
...
| **price_local**               | Float         | The amount charged for the transaction before the Apple/Google cut, in local currency. `null` for free trials, since nothing is charged.                                                                                                                                                                                                                         |
| **price_usd**                 | Float         | The amount charged for the transaction before the Apple/Google cut, in USD. `null` for free trials, since nothing is charged.                                                                                                                                                                                                                                    |
...
| **store**                     | String        | Store where the product was bought. Standard values: **app_store**, **play_store**, **stripe**, **paddle**. <br/>If you set [custom store transactions](api-adapty/operations/setTransaction) using the server-side API, the value from the **store** parameter is used.                                                                 |
| **store_country**             | String        | The country sent to us by the app store.                                                                                                                                                                                                                                                                                                                         |
| **store_offer_category**      | String        | Applied offer category. Possible values are `introductory`, `promotional`, `offer_code`, `win_back`, `retention`.                                                                                                                                                                                                                                                                            |
...
| **vendor_product_id**         | String        | <p>Product ID in the Apple App Store, Google Play Store, or Stripe.</p><p>If access was granted without a real store transaction, `vendor_product_id` will be one of:</p><ul><li>`adapty_server_side_product` — granted via the [server-side API](api-adapty/operations/grantAccessLevel).</li><li>`adapty_dashboard_product` — [granted manually](give-access-level-to-specific-customer) in the Adapty Dashboard.</li><li>`adapty_promotion` — legacy.</li></ul> |
...
- `subscription_initial_purchase` (also referred to as `subscription_started` — same event)
...
#### `non_subscription_purchase` example payload

`non_subscription_purchase` follows the same structure as subscription events but reflects a one-time or consumable purchase. Subscription-only fields don't apply: `cancellation_reason`, `will_renew`, `is_in_grace_period`, `is_refund`, `is_lifetime`, and `trial_duration` are absent. `subscription_expires_at` is present but `null`. The tax and revenue fields (`net_revenue_*`, `proceeds_*`, `tax_amount_*`) are included.

<details>
<summary>Example payload (click to expand)</summary>
...
If you have enabled [sharing access levels](general#6-sharing-paid-access-between-user-accounts), the **access level updated** event will be sent for all profiles sharing the access level.
...
| **renewed_at**                     | ISO 8601 date | Date and time when the current period of paid access most recently started. Until the first renewal, it matches `activated_at`. |
...
| **subscription_expires_at**        | ISO 8601 date | The expiration date of the subscription. In this event it holds the same value as `expires_at`: during a grace period, the end of the grace period; after a refund or cancellation, the cancellation date. |
| **transaction_id**                 | String        | Unique identifier for a transaction.                         |
...
| **vendor_product_id**              | String        | <p>Product ID in the store (Apple/Google/Stripe).</p><p>If access was granted without a real store transaction, `vendor_product_id` will be one of:</p><ul><li>`adapty_server_side_product` — granted via the [server-side API](api-adapty/operations/grantAccessLevel).</li><li>`adapty_dashboard_product` — [granted manually](give-access-level-to-specific-customer) in the Adapty Dashboard.</li><li>`adapty_promotion` — legacy.</li></ul> |
