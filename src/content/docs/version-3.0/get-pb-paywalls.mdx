...
| :-------- | :---------- |
| Flow   | An `AdaptyFlow` object containing the placement, identifiers (`id`, `variationId`), name, its paywall variations (`paywalls`), remote configs, and a `hasViewConfiguration` flag indicating whether the flow includes a view configuration. To fetch the actual products for pre-loading, custom UI, or programmatic checks, call `getPaywallProducts(flow:)`. |
