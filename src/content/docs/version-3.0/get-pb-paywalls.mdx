...
| :-------- | :---------- |
| Flow   | An `AdaptyFlow` object with a list of product IDs, the paywall or flow identifier, remote configs, and several other properties. |

## Fetch the view configuration

After fetching the paywall or flow, check if it includes a view configuration, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the view configuration is present, treat it as a Paywall Builder paywall; if not, [handle it as a remote config paywall](present-remote-config-paywalls).
