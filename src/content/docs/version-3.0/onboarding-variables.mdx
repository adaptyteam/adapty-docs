...
Variables let you display dynamic content in your flows — product pricing, offer details, or values that change based on user input. Use them to control element visibility, personalize screen content, or route users to different screens based on their choices.
...
Element variables capture the user's choices — what they selected in quizzes, which tab they are on, and whether they switched some toggle on.

Element variable types are different for different group types:
...
<ZoomImage id="insert-var.webp" width="500px" alt="Inserting a variable via an interaction" />
