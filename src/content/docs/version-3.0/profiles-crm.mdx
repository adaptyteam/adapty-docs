...
| IP country       | Country of your end user, determined by their most recent IP address. |
| Store country    | Country of your end user's App Store or Google Play account, as reported by the store. Adapty receives it with store transactions, so it's empty for users without purchases. |
...
:::note
Virtual currency balances are not shared or transferred between profiles like access levels. Each balance stays on a single profile — see [Balances, profiles, and devices](virtual-currency-balance#balances-profiles-and-devices).
:::
