...
- **Social proof**: User count, rating, rating count, press mentions, key metrics. The App Store rating is a count-weighted average across 20 major storefronts — Apple publishes no worldwide figure, and a single storefront undercounts by orders of magnitude.
- **Social links**: Twitter, Instagram, TikTok, YouTube, Facebook, LinkedIn.
- **Legal**: Terms URL, privacy URL, support email.
- **Reviews**: User reviews extracted from the store source — content, author, rating, source. Review text and author names stay exactly as written, in their original language, so a brand profile in English can still hold Spanish or Turkish reviews.
- **Pain points**: Pain statements grounded in reviews or social signal.
- **FAQ**: Questions and answers used in email content and web paywall sections.
...
When Adapty Mail builds the brand, it copies your app icon onto its own storage instead of hot-linking it from the store's CDN, so an email body doesn't pull images from a third-party host. At send time it rewrites that URL once more, to the `go.` host of the [sending domain](mail-sending-domain#2-add-the-dns-records) carrying the email — so recipients load your logo and hero images from your own domain.
