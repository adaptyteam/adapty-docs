---
title: "Respond to flow actions - iOS"
...
If you are building flows or paywalls in Adapty's no-code builders, it's crucial to set up buttons properly:

1. Add a [button in the old Paywall Builder](paywall-buttons) and assign it either a pre-existing action or create a custom action ID.
...
:::warning
**Only flow/paywall closures and URL opening are handled automatically.** All other button actions require proper response implementation in the app code.
:::

:::note
The iOS SDK can respond to system permission requests, such as push notifications or camera access, through an `AdaptySystemRequestsHandler`. Flows don't trigger these requests yet, so you don't need to handle them for now.
:::
...
:::info
In the iOS SDK, the `close` action triggers closing the flow or paywall by default. However, you can override this behavior in your code if needed. For example, closing one flow might trigger opening another.
:::
...
If you are building paywalls using the old Paywall Builder, it's crucial to set up buttons properly:
...
1. In the old Paywall Builder, add a button and assign it the **Close** action.
...
:::tip
If you want to add a group of links (e.g., terms of use and purchase restoration), add a **Link** element in the old Paywall Builder and handle it the same way as buttons with the **Open URL** action.
:::

To add a button that opens a link from your paywall (e.g., **Terms of use** or **Privacy policy**):

1. In the old Paywall Builder, add a button, assign it the **Open URL** action, and enter the URL you want to open.
2. In your app code, implement a handler for the `openUrl` action that opens the received URL in a browser.
...
1. In the old Paywall Builder, add a button and assign it the **Login** action.
...
1. In the old Paywall Builder, add a button, assign it the **Custom** action, and assign it an ID.
2. In your app code, implement a handler for the action ID you've created.
