...
If you are building paywalls or flows using the Adapty Paywall Builder or Flow Builder, it's crucial to set up buttons properly:
...
:::warning
**Only paywall/flow closures and URL opening are handled automatically.** All other button actions require proper response implementation in the app code.
:::

## Close paywalls and flows

To add a button that will close your paywall or flow:

1. In the builder, add a button and assign it the **Close** action.
2. In your app code, implement a handler for the `close` action.

:::info
In the iOS SDK, the `close` action triggers closing the paywall or flow by default. However, you can override this behavior in your code if needed. For example, closing one paywall might trigger opening another.
:::
...
.flow(
    ...
    didPerformAction: { action in
        switch action {
        case .close:
            flowPresented = false // dismiss the paywall or flow
        default:
            break
        }
    },
...
## Open URLs from paywalls and flows
...
To add a button that opens a link from your paywall or flow (e.g., **Terms of use** or **Privacy policy**):
...
For example, if you have another set of subscription offers or one-time purchases, you can add a button that will display another paywall or flow:
    ...
    didPerformAction: { action in
        ...
        case let .custom(id):
            if id == "openNewPaywall" {
                // Display another paywall or flow
            }
        default:
            break
        }
