...
The Create page lists the placeholders you can use, and ticks each one it finds in the URL you paste. These are the ones to reach for:
...
The page also offers a placeholder per profile source, so it may list sources your project doesn't send data from. A placeholder the recipient has no value for resolves to an empty string, so it never reaches your page as literal text.
