...
:::important
To ensure accurate web paywall analytics tracking in Adapty, you need to [add your products](product) along with their corresponding Stripe/Paddle/other payment provider product IDs in Adapty.
:::
...
In **Settings > Domains**, select your primary payment provider to use for domain verification. Then, verify your paywall domains with the respective provider:
...
2. Add `app.funnelfox.com` and your personal paywall subdomain (it will look like `paywalls-....fnlfx.com`). To find your subdomain, go to **Settings > Domains** and copy the **Hosted subdomain** value.
...
Follow the [FunnelFox Billing integration instructions](https://funnelfox.com/docs/billing/integration-billing-funnelfox).

**SolidGate**:
1. In your Solidgate Dashboard, go to **Developers > Apple Pay Domains**.
2. Click **+ Add new domain** and paste your project domain (from **Settings > Domains** in FunnelFox). Add your custom domain too, if applicable.
3. To use Apple Pay in preview mode, also add `http://app.funnelfox.com/`.
...
That's it! Use this link to [proceed with the setup](web-paywall#step-2-trigger-the-paywall).
