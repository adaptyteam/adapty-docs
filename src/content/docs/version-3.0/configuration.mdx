...
<ZoomImage id="20bf659-CleanShot_2023-08-22_at_13.26.562x.webp" width="700px" alt="Integration settings that apply to every event sent through the integration" />
...
It's important to note that event name customization is available for certain integrations, while for others, the event names are set and cannot be modified. Additionally, with certain integrations such as [Airbridge](airbridge#configure-events-and-tags) for example, you have the flexibility to associate multiple event names with a single Adapty event. Check the full list of the Events offered by Adapty [here](events).

<ZoomImage id="c79f5cd-screencapture-app-adapty-io-integrations-pushwoosh-2023-08-22-13_31_07.webp" width="700px" alt="Event groups with a toggle and an event name field for each event" />
...
### Set the timer for synthetic events

:::warning
Keep the **Trial still active** delay shorter than the trial. On a 7-day trial with a 7-day delay, the trial has already converted or expired by the time Adapty checks, so the event never arrives.
:::

[Synthetic events](events#synthetic-events) arrive on a timer, not in response to a change. Enter the number of days in the field next to the event's toggle. Adapty waits that long, then sends the event if the trial or subscription is still running.

The field accepts 1 to 90 days. **Trial still active** starts at 1 day and **Subscription still active** at 7. Each integration keeps its own delay, so two destinations can receive the same event days apart.
