...
metadataTitle: "Fetch a manual paywall | Unity SDK | Adapty Docs"
...
:::note
In v4, `GetFlow` has no `locale` parameter. When you render a flow with `CreateFlowView`, the localization is resolved automatically. For manually implemented paywalls, all available locales are returned together in `flow.RemoteConfigs` — pick the locale that matches the user's device or your app's setting. See [Localizations and locale codes](unity-localizations-and-locale-codes) for details.
:::
