...
- [Expo dev build sample](https://github.com/adaptyteam/AdaptySDK-React-Native/tree/master/examples/FocusJournalExpo) for full functionality including real purchases and the Flow & Paywall Builder
...
Expo Go doesn't support custom native modules, so you can only use it with [**mock mode**](#set-up-mock-mode-for-expo-go--expo-web) for UI/logic development (no real purchases and no AdaptyUI / no-code-builder rendering).
:::

1. Install Adapty SDK (this also installs `@adapty/core` automatically):
   ```sh
...
- It **doesn't render** paywalls or onboardings created in the **Flow & Paywall Builder (AdaptyUI)**.
...
To test real purchases and paywalls rendered by the Flow & Paywall Builder, use an Expo Dev Client / production build where mock mode is automatically disabled.
::::
