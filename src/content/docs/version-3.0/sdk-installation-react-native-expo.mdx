...
If you’re using **pure React Native (without Expo)**, follow the [React Native installation guide](sdk-installation-react-native-pure) instead.
:::
...
:::important
Adapty React Native SDK v4 and later ship their native iOS SDKs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) as Swift packages. CocoaPods still installs them in an Expo project, but Swift packages link dynamically, so your build needs dynamic frameworks — see [Enable dynamic frameworks for Adapty SDK v4](#enable-dynamic-frameworks).
:::
...
### Enable dynamic frameworks for Adapty SDK v4 \{#enable-dynamic-frameworks\}

React Native SDK 4.0 — which adds support for [flows](adapty-flow-builder) — requires **React Native 0.75 or later**. Install the SDK:

```sh
npx expo install react-native-adapty@^4.0.0
```

v4 pulls the native iOS SDKs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) as Swift packages instead of CocoaPods sub-dependencies ([CocoaPods' spec repo goes read-only in December 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Swift packages link dynamically, which in Expo you enable with the [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/) plugin. Add it to `app.json` (or `app.config.js`):
...
When building for iOS, you may see an error about the **minimum iOS version** or deployment target. Adapty requires **iOS 15.0+**.
...
With [dynamic frameworks enabled for Adapty SDK v4](#enable-dynamic-frameworks), the iOS build may fail in `expo-updates`, `@expo/ui`, or another package with Objective-C sources:
