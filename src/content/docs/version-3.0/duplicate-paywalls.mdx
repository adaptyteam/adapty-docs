...
Duplicating a paywall is how you change the products or offers on a paywall Adapty no longer lets you edit — see [Add offer to paywall](create-offer#add-offer-to-paywall) for when that restriction applies. Duplicating also keeps the original paywall's analytics intact when you make smaller changes to a paywall that's already live in your app. You can then use the duplicates to replace the original paywalls in some or all placements as needed.
