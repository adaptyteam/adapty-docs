...
- **Font family**: Choose a typeface. For details on working with custom fonts, see the [guide](using-custom-fonts-in-flow-builder).
- **Weight**: Set the font weight. This setting does not apply to custom fonts.
...
:::warning
Built-in fonts come from the device's own library. Different devices include different fonts and render them differently. Select **System** to display SF Pro on iOS and Roboto on Android. To consistently display the same font across devices, use a [custom font](using-custom-fonts-in-flow-builder) instead.
:::
