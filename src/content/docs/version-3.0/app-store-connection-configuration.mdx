...
Each credential on this page unlocks a different part of the exchange. [How Adapty syncs data with the stores](store-sync#what-adapty-reads-from-the-stores) shows which data each one carries.
...
IF a key is revoked, deleted, or rotated in App Store Connect, upload the replacement as soon as you notice: everything it powers stays broken until you do. See [If updates stop arriving](store-sync#if-updates-stop-arriving).
:::
