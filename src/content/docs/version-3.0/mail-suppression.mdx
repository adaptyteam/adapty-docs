...
**The inbox button.** On a marketing email, Adapty Mail sends the `List-Unsubscribe: <URL>, <mailto:>` header together with `List-Unsubscribe-Post: List-Unsubscribe=One-Click`. Gmail and Yahoo require these of bulk senders (RFC 8058), and clients that support them show an unsubscribe button next to the sender name. This URL has no confirmation page: the first request to it suppresses the profile, whatever the method — RFC 8058 specifies POST, and some providers send GET instead.

A [transactional email](mail-flows#transactional-triggers) carries neither header. It's owed to the recipient rather than sent on their consent, and the button the header draws sits next to the sender name — one tap on a confirmation request would block the address for good.
