---
title: "Google Cloud Storage in Adapty Attribution"
description: "Integrate Google Cloud Storage with Adapty Attribution for secure user acquisition data storage."
metadataTitle: "Google Cloud Storage | Adapty Attribution | Adapty Docs"
...
Adapty Attribution's integration with Google Cloud Storage allows you to store user acquisition campaign data securely in one central location. You will be able to save your campaign performance data, attribution data, and user acquisition events to your Google Cloud Storage bucket as .csv files.  
To set up this integration, you will need to follow a few simple steps in the Google Cloud Console and Adapty Attribution Dashboard.

:::note
Schedule

Adapty Attribution exports one set of files per calendar day to Google Cloud Storage. The export scheduler runs three times a day — at **02:00**, **04:00**, and **06:00 UTC**. Each run either creates the previous day's export job, if it doesn't exist yet, or retries it if an earlier run failed. Your data therefore usually lands after the 02:00 run, and the later runs are there to recover from failures rather than to add data.

Each file will contain data for the events created for the entire previous calendar day in UTC. For example, the data exported on March 8th will contain all the events created on March 7th from 00:00:00 to 23:59:59 in UTC.
:::

:::warning
The scheduled export only runs for companies on a paid plan or in trial. If your company is neither, the scheduler skips it and writes no files, even though the integration stays enabled in the dashboard. If your bucket is empty and the configuration looks correct, check your plan status first.
:::
...
In order for Adapty Attribution to upload raw data reports to your designated bucket, the service account's key is required, as well as write access to the corresponding bucket. By providing the service account's key and granting write access to the bucket, you allow Adapty Attribution to securely and efficiently transfer the raw data reports from its platform to your storage environment.
...
2. Next, create a new service account for the Adapty Attribution by clicking on the "+ CREATE SERVICE ACCOUNT" button
...
6. Go to the Google Cloud Storage's[ Buckets](https://console.cloud.google.com/storage/browser) page and either select an existing bucket or create a new one to store the user acquisition data reports from Adapty Attribution
...
11. After passing these steps have successfully completed the necessary setup steps in the Google Cloud Console! The final step involves entering the bucket's name and downloading the JSON file for use in Adapty Attribution
...
### Step 2. Configure integration in Adapty Attribution
...
3. Fill out the required fields to build a connection between Google Cloud Storage and Adapty Attribution:

| Field                                     | Description                                                                                                                                                                                                 |
|:------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Google Cloud service account key file** | The downloaded private [JSON key file](ua-google-cloud-storage#step-1-create-google-cloud-storage-credentials).                                                                       |
...
In addition to the automatic event data export to Google Cloud Storage, Adapty Attribution also provides a manual file export functionality. With this feature, you can select a specific date for the user acquisition data and export it to your GCS bucket manually. This allows you to have greater control over the data you export and when you export it.

## Table structure

In Google Cloud Storage integration, Adapty Attribution provides a table to store historical data for installation events. The table contains information about the user profile, revenue and proceeds, and the origin store, among other data points.
...
| `os_version`             | Major OS version                          |
| `app_version`            | App version reported by the Adapty SDK    |
| `sdk_version`            | Adapty SDK version                        |
...
| `click_time`             | Timestamp of the attributed click, in Unix seconds. Empty when no click is attributed to the install. |