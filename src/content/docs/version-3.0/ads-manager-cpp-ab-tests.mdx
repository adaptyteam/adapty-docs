---
title: "CPP A/B Tests in Adapty Search Ads"
description: "Compare custom product pages in Apple Ads and find the best performer."
metadataTitle: "CPP A/B Tests | Adapty Search Ads | Adapty Docs"
keywords: ['adapty search ads', 'apple ads', 'asa', 'cpp', 'a/b test', 'custom product page']
---

CPP A/B Tests let you compare custom product pages (CPPs) against each other inside Apple Ads. You select 2 to 4 product pages, and [Adapty Search Ads](adapty-ads-manager) rotates traffic between them, collects performance data, and tells you which page converts best.
...
- **Adapty Search Ads is connected**: Follow the [setup guide](adapty-ads-manager-get-started) if you haven't done so.
- **The source ad group has traffic**: The ad group you test must be at least 28 days old and have impressions, taps, and installs over that period. Adapty Search Ads uses this history to estimate the test duration and the required sample size.
- **You have at least one custom product page**: Create CPPs in App Store Connect first. Adapty Search Ads reads them automatically.
...
Configure how the test runs. Adapty Search Ads calculates the **Calculated Test Duration**, **Start Time**, and **End Time** automatically — the values update whenever you change one of the three settings below.
...
Until each variant has comparable impressions, Adapty Search Ads doesn't highlight a winner. While the data is still coming in, you'll see a banner above the table: **Winner highlighting is paused — variants don't have comparable impressions yet.**
...
For metric definitions, see [Metrics in Adapty Search Ads](adapty-ads-manager-metrics).
...
Adapty Search Ads uses the **Ad Group Switch** method:
...
4. Adapty Search Ads collects per-variant impressions, taps, and downloads, and tracks how close each variant is to a statistically meaningful sample.
...
- **Variants don't switch on a fixed clock**: The switch interval is a baseline, but Adapty Search Ads adjusts timing so each variant collects a fair share of impressions. A variant may stay active longer than one slot if it's behind on impressions.
