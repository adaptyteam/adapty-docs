---
title: "CPP A/B Tests in Adapty Ads Manager"
description: "Compare custom product pages in Apple Ads and find the best performer."
metadataTitle: "CPP A/B Tests | Adapty Ads Manager | Adapty Docs"
keywords: ['adapty ads manager', 'adapty search ads', 'apple ads', 'asa', 'cpp', 'a/b test', 'custom product page']
---

CPP A/B Tests let you compare custom product pages (CPPs) against each other inside Apple Ads. You select 2 to 4 product pages, and [Adapty Ads Manager](adapty-ads-manager) rotates traffic between them, collects performance data, and tells you which page converts best.
...
- **Adapty Ads Manager is connected**: Follow the [setup guide](adapty-ads-manager-get-started) if you haven't done so.
- **The source ad group has traffic**: The ad group you test must be at least 28 days old and have impressions, taps, and installs over that period. Adapty Ads Manager uses this history to estimate the test duration and the required sample size.
- **You have at least one custom product page**: Create CPPs in App Store Connect first. Adapty Ads Manager reads them automatically.
...
Configure how the test runs. Adapty Ads Manager calculates the **Calculated Test Duration**, **Start Time**, and **End Time** automatically — the values update whenever you change one of the three settings below.
...
Until each variant has comparable impressions, Adapty Ads Manager doesn't highlight a winner. While the data is still coming in, you'll see a banner above the table: **Winner highlighting is paused — variants don't have comparable impressions yet.**
...
For metric definitions, see [Metrics in Adapty Ads Manager](adapty-ads-manager-metrics).
...
Adapty Ads Manager uses the **Ad Group Switch** method:
...
4. Adapty Ads Manager collects per-variant impressions, taps, and downloads, and tracks how close each variant is to a statistically meaningful sample.
...
- **Variants don't switch on a fixed clock**: The switch interval is a baseline, but Adapty Ads Manager adjusts timing so each variant collects a fair share of impressions. A variant may stay active longer than one slot if it's behind on impressions.
