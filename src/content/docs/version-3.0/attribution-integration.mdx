...
3. **Set the attribution data**:
    ...
    **Parameters:**
    ...
    - `provider` (required; named `source` before SDK version 4.1): attribution provider. Use the custom value if your attribution provider does not support [integrated attribution](#integrated-attribution) — `.custom` on iOS, `AdaptyExternalAttributionProvider.CUSTOM` on Android and Kotlin Multiplatform, `AdaptyExternalAttributionProvider.Custom` on Unity, `AdaptyExternalAttributionProvider.custom` on Flutter, and the string `'custom'` on Capacitor and React Native. Before SDK version 4.1, pass the string `"custom"`.
