...
3. **Set the attribution data**:
    ...
    ```typescript showLineNumbers
    await adapty.updateAttribution(attribution, 'custom');
    ```
    ...
    :::note
    On Flutter and React Native, the method is still named `updateAttribution` and the provider is a plain string — SDK 4.1 has not shipped for those platforms yet.
    :::
