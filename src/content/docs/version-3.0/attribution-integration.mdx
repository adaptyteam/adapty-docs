...
* Filter Adapty [subscription charts](charts) by attribution
...
:::tip
Enable [Adapty Attribution](user-acquisition) for a complete overview of your application's economy.

Adapty Attribution is an easy-to-configure web dashboard that consolidates data from different sources to detect efficient user acquisition strategies.
:::
...
2. **Integrate the Adapty SDK.** Some platforms require extra code to set attribution data.
...
Adjust, AppsFlyer, Branch, and Singular can also receive [synthetic events](events#synthetic-events) — a signal that a trial or subscription survived its first days. For Meta and TikTok campaigns, Adapty Attribution offers [Qualified trials](ua-facebook#qualified-trial) instead.

:::note
If you want Adapty to expand the list, [create a feature request](https://adapty.featurebase.app/en?b=6979f233ebd3cffd4f425ba0) and express your interest in a particular service.
:::
...
2. **Create a dictionary with the attribution data you received.**
    ...
    **Example**:
    ...
    Unity takes the attribution data as a serialized JSON string, so build the dictionary and serialize it:
...
3. **Set the attribution data**:
    ...
    Pass the attribution dictionary to the `updateExternalAttribution` method (named `updateAttribution` before SDK version 4.1). Once Adapty has saved the attribution value, you cannot override it:
    ...
    ```dart showLineNumbers
    await Adapty().updateAttribution(attribution, source: 'custom');
    ```
    ...
    :::note
    On Flutter, where SDK 4.1 isn't available, the method is still named `updateAttribution` and the provider is a plain string.
    :::

    :::info
    `appliedExternalAttributionProviders` only reports attribution from Apple Ads. Attribution from other providers is visible on user profiles and available in segment filters, but not yet here.
    :::

    **Parameters:**

    - `attribution` (required): dictionary with attribution data.
    - `provider` (required; named `source` before SDK version 4.1): attribution provider. Use the custom value if your attribution provider does not support [integrated attribution](#integrated-attribution) — `.custom` on iOS, `AdaptyExternalAttributionProvider.CUSTOM` on Android and Kotlin Multiplatform, `AdaptyExternalAttributionProvider.Custom` on Unity, and the string `'custom'` on Capacitor. Before SDK version 4.1, and on Flutter and React Native, pass the string `"custom"`.
