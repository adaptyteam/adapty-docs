...
3. **Set the attribution data**:
    ...
    **Parameters:**
    ...
    - `provider` (required; named `source` before SDK version 4.1): attribution provider. Use `.custom` (SDK 4.1 and later) if your attribution provider does not support [integrated attribution](#integrated-attribution). In earlier versions, pass the string `"custom"`.
