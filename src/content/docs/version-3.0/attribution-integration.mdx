...
3. **Set the attribution data**:
    ...
    Pass the attribution dictionary to the `updateExternalAttribution` method (named `updateAttribution` before SDK version 4.1). Once you set the attribution value, you cannot override it:
    ...
    **Parameters:**
    ...
    - `provider` (required; named `source` before SDK version 4.1): attribution provider. Pass any custom string, for example `"custom"`, if your attribution provider does not support [integrated attribution](#integrated-attribution).
