...
- **Rate limit**: Enforced per API key and assigned per app, so no single figure applies to every app — see [Rate limits](https://adapty.io/docs/ss-authorization.md#rate-limits). It is a sustained rate rather than a per-minute budget, and some endpoints are capped lower: [createVirtualCurrencyTransaction](https://adapty.io/docs/api-adapty/operations/createVirtualCurrencyTransaction.md) allows 600 requests per minute per app by default. Tell your agent to retry with backoff on `429` and to honor `Retry-After` when it's present.
