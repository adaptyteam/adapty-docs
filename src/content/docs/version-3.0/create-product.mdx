...
The price you enter here can't be changed after creation, and a price change in the store console doesn't come back to Adapty. See [Product and price discrepancies](store-sync).

:::tip
You can also create products programmatically using the [Developer CLI](developer-cli-reference#adapty-products-create).
:::
...
4. Enter the following data:
    ...
    - **Subscription duration**: select the duration of the subscription from the list. You can’t change it after the product is created.
        ...
        - **Non-Subscriptions**: For one-time products that have no duration. A non-subscription purchase doesn’t grant an access level — it appears in the `nonSubscriptions` array of the user profile, and your app decides what to unlock.
        - **Consumables**: Consumable items can be purchased multiple times and can be used up during the life of the application. Examples are in-game currency and extras. A consumable purchase doesn’t grant an access level either. To grant an access level from a one-time purchase, use **Lifetime**.
...
10. For iOS, configure the introductory offer – free trial – by selecting its **Free duration** from the dropdown. For this initial setup, you can add an introductory free trial. Once the main product is approved by the stores, you can [add more offers](offers) (e.g., promotional, win-back) by linking their existing IDs from your store console.
...
:::important
Intro offers don't sync automatically with Google Play. Unlike App Store, Google Play has no separate "introductory offer" type — free trials and discounted offers are all configured as **offers** on a base plan. [Create the offer in Google Play Console and link it to your Adapty product](google-play-offers).
:::
...
- If you haven't configured introductory/promotional offers yet, you can [do it](offers) now.
- If you don't want to do it or have already done it, proceed with [setting up paywalls](quickstart-paywalls) to enable in-app purchases.
...
- Read how you can [edit products](edit-product) later.
