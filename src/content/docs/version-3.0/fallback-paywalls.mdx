...
:::important
This article covers fallbacks for Adapty SDK v3 and earlier. On SDK v4, a single fallback file includes flow and paywall data — see [Fallback flows](fallback-flows). The old fallback file format is incompatible with SDK v4, so download the correct version after upgrading.
:::
...
* Fallback paywalls don't support [internationalization](paywall-localization). The file always uses the `en` locale, so offline users only see English. [Upgrade to SDK v4](migrate-to-flows#update-the-sdk) to lift that limit — its fallback file carries every locale.
