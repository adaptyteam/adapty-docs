...
1. Render [a manual paywall with remote config](present-remote-config-paywalls-react-native) as usual. For flows & paywalls built in the Flow & Paywall Builder, see [Present flows in Observer mode](react-native-present-flows-in-observer-mode).
2. [Associate flows & paywalls](report-transactions-observer-mode-react-native) with purchase transactions. 