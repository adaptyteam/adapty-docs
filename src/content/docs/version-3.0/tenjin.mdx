...
Use the `Adapty.updateExternalAttribution()` SDK method (named `updateAttribution()` before SDK version 4.1) to retrieve attribution data from Tenjin, and pass it on to Adapty. 
...
### Some events show as failed in the Event Feed

Tenjin accepts only the events listed in [Configure events and tags](#configure-events-and-tags). Every other event still gets a row in the Event Feed, marked as a failed delivery with the tooltip "The event type is not supported for integration." That includes `access_level_updated`, which you won't find in the **Events names** section at all, because it's exclusive to the [webhook integration](webhook).