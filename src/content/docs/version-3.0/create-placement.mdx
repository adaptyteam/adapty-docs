...
:::info
Although the placement creation process is similar for flows, paywalls, and onboardings, you can't create one placement that serves more than one type — each placement type processes different metrics. The type is fixed once the placement exists, so serving a flow where you previously served a paywall means a new flow placement with a new placement ID.
:::
...
4. Enter a **Placement ID**. You'll use this ID in the Adapty SDK to call the placement's [flows](adapty-flow-builder), [paywalls](paywalls), [onboardings](onboardings), and [A/B tests](ab-tests). Placement IDs are unique across every placement in the app, whatever its type, and you cannot edit one later. They are also case-sensitive: `onboarding` and `Onboarding` are two separate placements. Your app must request the exact ID you enter here.
