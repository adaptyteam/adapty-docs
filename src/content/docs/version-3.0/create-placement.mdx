...
4. Enter a **Placement ID**. You'll use this ID in the Adapty SDK to call the placement's [flows](adapty-flow-builder), [paywalls](paywalls), [onboardings](onboardings), and [A/B tests](ab-tests). Placement IDs are unique across every placement in the app, whatever its type, and you cannot edit one later.
