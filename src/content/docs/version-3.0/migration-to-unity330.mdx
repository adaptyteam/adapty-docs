...
To verify specifically whether the available offer is a free trial rather than a paid discount, read `product.Subscription?.Offer?.Phases` and look for `PaymentMode == AdaptyPaymentMode.FreeTrial` — see [Fetch products](fetch-paywalls-and-products-unity#fetch-products).
...
1. Delete processing of the Canceled purchase event.
...
Update your mobile app code as shown below. For the complete code example, check out the [SDK configuration for AppsFlyer integration](appsflyer#configure-your-app-code).
