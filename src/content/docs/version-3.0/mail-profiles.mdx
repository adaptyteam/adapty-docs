...
- **Adapty SDK**: The SDK collects emails and purchases from your app. See [Connect Adapty to Adapty Mail](mail-collect-emails).
...
Adapty Mail links every email, click, and purchase to a profile. It recognizes the profile by the sending system's own ID first, then `customer_user_id`, then email address, so the same person arriving from two sources lands on one profile. The Profiles page is read-only. You can view profiles and unsubscribe them, but you can't create, edit, or delete them. The source app or the API owns that data.
