...
:::note
If a subscription also grants a virtual currency, the two behave differently across a user's profiles: the access level can be shared or transferred between profiles, but the virtual currency balance stays on a single profile. See [Balances, profiles, and devices](virtual-currency-balance#balances-profiles-and-devices).
:::
