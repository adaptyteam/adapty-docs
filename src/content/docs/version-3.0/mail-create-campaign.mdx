...
Adapty Mail generates a full email sequence — copy, design, hero images, and subject lines — from your app's store metadata. No copywriting or design work required. The send schedule comes from the campaign's purpose rather than the AI, and every delay stays editable. The campaign saves as a draft; it starts delivering only once you attach it to a [flow](mail-create-flow).

:::tip
The steps below create a marketing campaign. The **Transactional** tab on the **Campaigns** page holds campaigns written for a single system job instead — confirming an address, getting a buyer into the app, or handing them the page where a subscription is managed. Each transactional purpose leads to its own kind of [destination](mail-destinations), and the one for confirming an address is written for you by [double opt-in](mail-double-opt-in).
:::

## Before you start

- **Saved web paywall**: Every campaign must link to a web paywall. The backend rejects campaigns without one. See [Create a web paywall](mail-get-started#6-create-a-web-paywall) if you don't have one yet.
...
2. In the **New marketing campaign** dialog, select the moment of the customer lifecycle the campaign is written for, then click **Continue**. The five purposes are the [marketing flow triggers](mail-flows#marketing-triggers): **Never purchased**, **Renewal cancelled**, **Expired**, **Billing issue**, and **Refunded**.
3. Set the campaign name.
4. In the **Email Destination (Web Paywall)** dropdown, select the web paywall you want emails to link to. The dropdown offers only web paywalls, because that's the [destination kind](mail-destinations#which-kind-a-campaign-needs) a marketing campaign leads to.
...
7. Click **Generate**. Generation usually takes a few minutes. The backend times out after 5 minutes if it can't complete — retry if that happens.
