...
:::tip
To fill that cache before the user needs the flow, call [`preloadFlows`](ios-optimize-paywall-fetching#preload-placements) (iOS SDK 4.1+) for several placements at once. A later `getFlow` reads the preloaded copy only with `fetchPolicy: .returnCacheDataElseLoad`.
:::
