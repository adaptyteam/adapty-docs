...
4. Select each product card and assign it a product in the dropdown in the **Design** panel.
   ...
   :::important
   A product element without an attached product [blocks previewing and publishing](flow-common-issues#a-flow-wont-preview-or-publish). Assign a product or delete the element.
   :::

   <ZoomImage id="product-card.webp" width="500px" alt="Adding a product element to the canvas" />

5. (Optional) To apply an offer when a user buys from that card, pick the offer from the offer dropdown. See [Add offer to flow](create-offer#add-offer-to-flow).

The **Bottom Sheet** preset puts the product list inside a slide-up panel. To control when that panel opens, see [Bottom Sheet](builder-containers#bottom-sheet).
...
:::important
If you delete a product group and replace it with a new one, check that every action and variable targets the new group. References left pointing to the deleted group [block previewing and publishing](flow-common-issues#a-flow-wont-preview-or-publish).
:::
...
A restore button with no action attached, or a product missing from your catalog, renders exactly like a working screen. Before you publish, the [flow-audit skill](flow-audit-skill) can check every screen in the flow for both.
...
## Change the selected product from another element

Users normally change the selection by tapping a product card. To move the selection from somewhere else on the screen, add a [Select product](onboarding-actions#select-product) action and point it at the target product card.

To set up the action:

1. Select the element that should change the selection — a button, a toggle, or any other element.
2. Open the **Interactions** tab in the right panel and click **Add trigger**.
...
The action works only with product cards in a product group on the same screen. To select a different product depending on the user's choice, wrap the action in a [conditional action](onboarding-actions#conditional-actions).
