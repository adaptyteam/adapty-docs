...
- **Historical events reach Firebase at a user's first launch.** Adapty doesn't resend events it processed before you enabled the integration. But when an existing subscriber first launches a build with the Adapty SDK, Adapty ingests their full store history and forwards it like any other event — and Firebase maps initial purchases, renewals, trial conversions, and one-time purchases all onto the GA4 `purchase` event, so Adapty sends each backfilled renewal with revenue attached. (Past data also lives in Adapty's [S3](s3-exports) / [GCS](google-cloud-storage) exports, but importing it into Firebase isn't part of this integration.)
...
4. Configure how Adapty forwards revenue and user data. The four controls share one row in the dashboard:
   ...
   - **Report user's currency** toggle: When on, Adapty reports each sale in the currency the buyer paid in. When off, it reports in USD.
...
- **First-launch history import**: When an existing subscriber first launches a build with the Adapty SDK, Adapty forwards their whole store history to Firebase, sending every past renewal as a GA4 `purchase` with revenue. Firebase can report far more revenue than Adapty for the affected period. See [Limitations](#limitations).
