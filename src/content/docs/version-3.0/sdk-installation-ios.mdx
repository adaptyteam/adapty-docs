...
:::important
Adapty iOS SDK 4.0 is a pre-release. Swift Package Manager does not resolve beta versions through the **Up to Next Major Version** (`from:`) rule, so you must pin the exact version. In Xcode, set the **Dependency Rule** to **Exact Version** and enter `4.0.0-beta.2`. In `Package.swift`, use `.exact("4.0.0-beta.2")`. See [Migrate Adapty iOS SDK to v4](migration-to-ios-sdk-v4).
:::
