...
- **AdaptyUI**: This optional module renders flows, in addition to old builder paywalls.
...
Adapty iOS SDK requires iOS 15.0 or later. Adapty SDK 4.0 and later also requires **Xcode 26** or later to build, since its Swift package manifest declares Swift tools 6.2.

:::important
Adapty SDK 3.15.7+ is required when building with Xcode 26.4 or later.
:::
...
3. In the **Choose Package Products** window, select the modules you need:
   - **Adapty** (core module)
   - **AdaptyUI** (optional - only if Adapty renders your screens)
   :::note
   Note:
     - To enable [Kids Mode](kids-mode) in SDK 3.x, select **Adapty_KidsMode** instead of **Adapty**. In SDK 4.0 and later, select the regular modules — Kids Mode is enabled via the `KidsMode` package trait instead.
...
:::important
Wait for `activate` to resolve before calling any other Adapty SDK method. See [Call order in iOS SDK](ios-sdk-call-order) for the full sequence.
:::

Now set up paywalls in your app:

- If you use the [Flow & Paywall Builder](adapty-flow-builder), first [activate the AdaptyUI module](#activate-adaptyui-module-of-adapty-sdk) below, then follow the [quickstart](ios-quickstart-paywalls).
- If you build your own paywall UI, see the [quickstart for custom paywalls](ios-quickstart-manual).

## Activate AdaptyUI module of Adapty SDK

If you plan to use the [Flow & Paywall Builder](adapty-flow-builder) and have [installed AdaptyUI module](sdk-installation-ios#install-adapty-sdk), you also need to activate AdaptyUI.
...
:::tip
Optionally, when activating AdaptyUI, you can [override default caching settings for paywalls](#media-cache-configuration-for-paywalls-in-adaptyui).
:::
...
### App Store messages handling

:::info
This parameter is available starting from SDK version 4.2.
:::

By default, StoreKit shows App Store messages — price increase consent requests, billing issue notices, and win-back offers — on its own while your app is open. To choose when your app shows them, set `storeMessagesHandling` to `.manual`. The default value is `.auto`.
...
In manual mode, StoreKit doesn't show messages on its own, so your app must call `showStoreMessages`. See how in [Control App Store messages](ios-store-messages).
...
This happens because Tuist generates Xcode projects from SPM packages but doesn't preserve the `swift-tools-version` setting the SDK declares (`6.0` in SDK 3.x, `6.2` from SDK 4.0). As a result, some Adapty targets (`Adapty`, `AdaptyUI`, `AdaptyUIBuilder`) compile with Swift 5 rules while others use Swift 6, creating cross-module `@Sendable` mismatches.
