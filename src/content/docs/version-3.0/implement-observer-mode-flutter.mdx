...
:::tip
In SDK v4, you can also present Adapty-rendered flows and paywalls in Observer mode: register an `AdaptyUIObserverModeResolver` to perform the purchase or restore with your own code when a user taps the corresponding button. See [Handle purchases in observer mode](flutter-handling-events#handle-purchases-in-observer-mode).
:::