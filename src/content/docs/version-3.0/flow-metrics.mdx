...
Adapty collects a series of metrics to help you measure the performance of your flows. Unlike paywall metrics, flow metrics include completion tracking, so you can see how many of the users who start a flow reach the end of it. All metrics are updated in real-time, except for views, which are updated once every several minutes. This document outlines the metrics available, their definitions, and how they are calculated.

:::important
Flow revenue is calculated from all transactions that occurred after the flow has been shown.
:::

Flow metrics are available in the [flow list](filter-flows), which gives you an overview of all your flows' performance. This consolidated view presents aggregated metrics for each flow, allowing you to compare their effectiveness and identify areas for improvement.
...
:::tip
For metrics this page doesn't compute, such as MRR or active subscriptions, [filter the Charts page by flow](#filter-analytics-by-flow).
:::

## Metrics controls

The system displays metrics based on the selected time period and organizes them as an expandable tree in the left-side column. In the placement-based view, the levels are placement, audience, A/B test, flow variation, screen, and product. The audience-based view drops the placement level.

A screen row shows the revenue, purchases, and trials attributed to that screen. Its **Views** and **Completions** are not a per-screen funnel: **Views** counts only the users who started the flow on that screen, and **Completions** only those who ended it there. Every screen in between reports zero for both.
...
- Filter by: Attribution (reported by, status, channel, campaign, ad group, ad set, creative), Country, Store.
...
The number of unique completions divided by the number of unique views. Use this metric to compare how many users finish one flow against another, or one variation against another.
