...
The Adapty React Native SDK requires iOS 15.0+.
...
Adapty React Native SDK v4 — which adds support for [flows](adapty-flow-builder) — requires **React Native 0.75 or later**.

For iOS, you can install the SDK with CocoaPods or with Swift Package Manager. CocoaPods is the default. React Native 0.87 added Swift Package Manager as an experimental integration, and Adapty React Native SDK supports it from v4.1.
...
Then set up your iOS project:

<Tabs groupId="ios-package-manager" queryString>

<TabItem value="cocoapods" label="CocoaPods" default>

v4 pulls the native iOS SDKs (`Adapty`, `AdaptyUI`, `AdaptyPlugin`) through Swift Package Manager instead of CocoaPods sub-dependencies ([CocoaPods' spec repo goes read-only in December 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). SPM requires dynamic frameworks — add the following to your `ios/Podfile` target:
...
Swift Package Manager requires **React Native 0.87 or later** and **Adapty React Native SDK 4.1 or later**. React Native ships its Swift Package Manager integration as a preview and doesn't recommend it for production yet.

To integrate Adapty through Swift Package Manager, in your project root, run:

```sh showLineNumbers title="Shell"
cd ios && npx react-native spm
```

Adapty React Native SDK ships a `Package.swift` manifest, so React Native resolves the native iOS SDKs on its own. There is nothing to add to your Xcode project.
