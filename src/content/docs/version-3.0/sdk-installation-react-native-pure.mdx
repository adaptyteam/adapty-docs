...
:::important
Starting from v4, the Adapty React Native SDK no longer supports CocoaPods installation of its native dependencies. If you need v4 or later (for the [Flow & Paywall Builder](adapty-flow-builder)), follow [Adapty SDK 4.0: enable Swift Package Manager](#adapty-sdk-40-enable-swift-package-manager) below instead.
:::
...
React Native SDK 4.0 — which adds [Flow & Paywall Builder](adapty-flow-builder) support — requires **React Native 0.75 or later**. Install the SDK:
...
- It **doesn't render** flows or paywalls that Adapty draws for you (AdaptyUI).
