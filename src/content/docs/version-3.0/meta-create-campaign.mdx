...
4. For [Adapty Attribution](adapty-user-acquisition), you won't need to complete the full installation of the pixel. So, when asked about the integration, you can just click **x** in the setup window, and your pixel will still appear on the list after you refresh the page.
...
6. Adapty lets you conduct comprehensive [A/B tests](ab-tests). However, you can also enable A/B tests in Meta Ads if needed. Read more about A/B tests in Meta Ads Manager [here](https://www.facebook.com/business/help/1159714227408868).
...
3. In the **Performance goal** field, select the goal that matches how you drive traffic:
   - **Maximize number of conversions**: Optimizes on the purchases and trials Adapty reports back to your Meta dataset. Needs the dataset and conversion event in the next two steps.
   - **Maximize number of landing page views**: Use this if you have a landing page.
   - **Maximize number of link clicks**: Use this with a smart link that navigates users directly to the store.

   <ZoomImage id="performance-goal.webp" />

4. If you selected **Maximize number of conversions**, in the **Dataset** field, select the dataset you've created on [Step 2](#step-2-add-meta-pixel). For the other two goals, skip to step 6.
5. Select a **Conversion event**. In our case, it is probably going to be **Purchase** or **Start trial**. Don't worry if you see a warning that your dataset doesn't have any events yet – that just means that your dataset is new.
   ...
   For Adapty to report purchases and trials into that dataset, connect the pixel to your Adapty campaign and map the events — see [Add campaigns](ua-facebook#step-2-add-campaigns) and [Map events](ua-facebook#step-3-map-events).
...
4. In the **Destination** section, keep **Website** selected as **Main destination**. Then, fill in the **Website URL** field and the **URL parameters** field in the **Tracking** section. Both values come from your [web campaign in Adapty Attribution](ua-facebook), and they depend on where the ad sends users:
   - **App Store or Google Play**: In **Website URL**, paste `https://api-ua.adapty.io/api/v1/attribution/click`. In **URL parameters**, paste the rest of the **Click link**, after the `?`.
   - **Landing page or web funnel**, such as FunnelFox: In **Website URL**, paste your page's URL. In **URL parameters**, paste the **Ad URL parameters** value. See [Tag your ad URLs for web funnels](ua-tag-ad-urls).
