...
4. In the **Destination** section, keep **Website** selected as **Main destination**. In the **Website URL** field, paste `https://api-ua.adapty.io/api/v1/attribution/click`. In [Adapty Attribution](adapty-user-acquisition), [create a web campaign](ua-facebook) and paste the **Click link** content after `https://api-ua.adapty.io/api/v1/attribution/click` to the **URL parameters** field in the **Tracking** section.

   If your ad sends users to a landing page or web funnel, such as FunnelFox, paste that page's URL in **Website URL** instead, and add Adapty's tracking parameters as described in [Tag your ad URLs for web funnels](ua-tag-ad-urls).
