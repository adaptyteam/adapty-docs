...
- You display a paywall created in the Flow & Paywall Builder
- You have implemented your own paywall and use the `makePurchase` method inside it to handle purchases
- You use Adapty in the observer mode (either with the Flow & Paywall Builder or your custom paywall)
...
- [ ] Your app [displays the paywall](ios-present-paywalls) from the same placement you will ship.
    - [ ] Paywall displays on the screen. If loading takes too much time (e.g., if you or your users deal with unstable internet), consider [adjusting your fetch policy](get-pb-paywalls#fetch-a-paywall-created-with-the-old-paywall-builder).
...
- [ ] Store review requirements:
    - [ ] The **Restore purchases** button is on the paywall. You can add it in the Flow & Paywall Builder, and it will process purchase restorations automatically when tapped.
