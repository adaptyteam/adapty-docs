...
:::important
This guide covers remote config for manually implemented paywalls. For flows, see [Customize flow with remote config](customize-flow-with-remote-config).
:::

A paywall remote config is a custom JSON payload attached to a paywall. Use it to define titles, images, fonts, colors, and anything else your screen reads at runtime.
...
Now it's time to [create a placement](create-placement) and add the paywall to it. After that, you can <InlineTooltip tooltip="render a manual paywall with remote config">[iOS](present-remote-config-paywalls), [Android](present-remote-config-paywalls-android), [React Native](present-remote-config-paywalls-react-native), [Flutter](present-remote-config-paywalls-flutter), [Unity](present-remote-config-paywalls-unity), [Kotlin Multiplatform](present-remote-config-paywalls-kmp), and [Capacitor](present-remote-config-paywalls-capacitor)</InlineTooltip> in your mobile app.