...
- **Paywalls are rendered by the SDK**: Superwall renders paywalls inside a `WKWebView`. Adapty renders flows and paywalls natively — fonts, product info, and buttons are drawn by the SDK.
...
With Adapty's no-code builders, both SDKs process purchases automatically inside the paywall UI — **you can skip this step**.
