...
For manually implemented paywalls, Superwall requires a `PurchaseController` implementation. Adapty does not: replace `PurchaseController.purchase` with `Adapty.makePurchase` and `PurchaseController.restorePurchases` with `Adapty.restorePurchases`. The SDK handles validation on its own.
