...
Adapty Mail includes its own **flows**, unrelated to Adapty's in-app flows. A flow turns a saved campaign into scheduled deliveries. It decides which campaign a user receives when a **trigger** fires, and a **segment** decides which users qualify. Adapty Mail evaluates every flow whenever a matching event fires — no polling, no cron, no manual launch.

The **Flows** page splits into a **Marketing** tab and a **Transactional** tab. Marketing flows sell, so a purchase ends them; transactional flows carry a message owed once, such as a confirmation request or a way into the app the recipient just bought. The two classes run side by side: a transactional flow and a marketing sequence can be underway for the same recipient at the same time, and neither holds the other back.

## Marketing triggers

Adapty Mail ships with five fixed marketing triggers, each with its own flow view under **Flows → Marketing**:
...
A marketing flow runs one at a time per recipient: while one sequence is underway, no second one starts, whatever else the recipient does. It ends when the recipient buys, which is what every marketing sequence is sent for.

## Transactional triggers

Transactional flows start from a moment rather than a standing, and each carries a message owed exactly once. Two triggers fire them:

- **Profile created**: A profile appears in the project.
- **Purchased**: Money arrives from that profile for the first time — a subscription starts or a one-time purchase lands. Renewals don't count, since the buyer isn't new to the product.

The **Transactional** tab holds three flow views, because the **Purchased** trigger carries two flows that differ in what their email is for. Each view accepts campaigns of its own purpose, and each purpose leads to its own [destination kind](mail-destinations):

| Flow | Trigger | What its email does | Destination |
| --- | --- | --- | --- |
| **Double opt-in** | Profile created | Asks the recipient to confirm their address. Created and maintained by the [double opt-in](mail-double-opt-in) setting. | Web Paywall |
| **App activation** | Purchased | Takes the buyer into the app they just paid for. | Deep Link |
| **Cancellation page** | Purchased | Hands the buyer the page where the purchase is reviewed, changed, or cancelled. | Cancellation Page |

Both **Purchased** flows are evaluated on the same event and neither stands in for the other, so a buyer with both configured receives both.

A transactional flow runs once per recipient, counted against the flow rather than the campaign it picked — a recipient whose segment changed doesn't earn a second copy. Only the double opt-in flow stops early, as soon as the address is confirmed; the other two have nothing to wait for and run to the end of their sequence.
...
Every flow view, marketing and transactional alike, holds a list of **segment → campaign** (or segment → A/B test) rows, ordered by priority. When a user hits the trigger, Adapty Mail:
