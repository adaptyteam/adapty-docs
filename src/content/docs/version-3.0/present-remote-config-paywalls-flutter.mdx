...
To get a remote config of a paywall, access the flow's `remoteConfig` property (the first remote config) or pick a specific localization from `remoteConfigs`, and extract the needed values. See [Localizations and locale codes](flutter-localizations-and-locale-codes) for picking the right localization.

```dart showLineNumbers
try {
  final flow = await Adapty().getFlow(placementId: 'YOUR_PLACEMENT_ID');
  final String? headerText = flow.remoteConfig?.dictionary?['header_text'] as String?;
