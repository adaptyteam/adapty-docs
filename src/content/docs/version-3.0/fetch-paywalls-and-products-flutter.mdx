...
:::note
In v4, `getFlow` doesn't take a `locale` parameter. For custom paywalls, all available localizations are returned in the flow's remote configs (`flow.remoteConfigs`) — pick the one that matches the user's device or app setting. See [Localizations and locale codes](flutter-localizations-and-locale-codes).
:::
...
To address this, you can use the `getFlowForDefaultAudience` method, which fetches the flow of the specified placement for the **All Users** audience. However, it's crucial to understand that the recommended approach is to fetch the flow by the `getFlow` method, as detailed in the [Fetch flow information](fetch-paywalls-and-products-flutter#fetch-flow-information) section above.
...
If you're willing to accept these drawbacks to benefit from faster flow fetching, use the `getFlowForDefaultAudience` method as follows. Otherwise, stick to the `getFlow` described [above](fetch-paywalls-and-products-flutter#fetch-flow-information).
:::
