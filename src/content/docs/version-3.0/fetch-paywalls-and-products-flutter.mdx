...
metadataTitle: "Fetch a manual paywall | Flutter SDK | Adapty Docs"
...
:::note
In v4, `getFlow` doesn't take a `locale` parameter. For manually implemented paywalls, all available localizations are returned in the flow's remote configs (`flow.remoteConfigs`) — pick the one that matches the user's device or app setting. See [Localizations and locale codes](flutter-localizations-and-locale-codes).
:::
