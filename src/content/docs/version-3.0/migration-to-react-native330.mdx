...
To verify specifically whether the available offer is a free trial rather than a paid discount, read `product.subscription?.offer?.phases` and look for `paymentMode === 'free_trial'` — see [Fetch products](fetch-paywalls-and-products-react-native#fetch-products).
