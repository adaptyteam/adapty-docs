...
1. Check whether the integration supports this event type. The following events do not reach all integrations:
    ...
    - [Synthetic events](events#synthetic-events) (`trial_active`, `subscription_active`): Adjust, Amplitude, AppMetrica, AppsFlyer, Branch, Facebook Analytics, Firebase, Mixpanel, OneSignal, PostHog, Singular, SplitMetrics, and the webhook.
...
- The user turned auto-renewal off before the delay elapsed. Their access runs to the end of the paid period, but Adapty skips the send. Look for `trial_renewal_cancelled` or `subscription_renewal_cancelled` instead.
