...
metadataTitle: "Fetch a manual paywall | React Native SDK | Adapty Docs"
...
:::note
In v4, `getFlow` no longer takes a `locale` parameter. For manually implemented paywalls, all available locales are returned in the flow's remote config (`flow.remoteConfigs`)—pick the one that matches the user's device or app setting.
:::
