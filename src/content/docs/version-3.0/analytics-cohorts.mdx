...
import CohortLengthAndTimeFrame from '@site/src/components/reusable/CohortLengthAndTimeFrame.mdx';
...
<ZoomImage id="cfc961c-cohorts_general.webp" width="700px" alt="Cohort analysis table" />

<ZoomImage id="00bf923-cohorts_monthly.webp" width="700px" alt="Monthly cohort table" />
...
<ZoomImage id="4d5ffdd-cohorts.webp" width="700px" alt="Cohorts by renewals or by days" />
...
3. The time period we're working with is the last 6 months, and the cohort length is 1 month (mark #3).
...
## Cohort length and time frame

<CohortLengthAndTimeFrame />

<ZoomImage id="e20b676-cohorts_period.webp" width="700px" alt="Cohort length and time frame controls" />
...
By default, Adapty builds cohorts using data from all purchases. You can filter by product duration, specific products, country, store, paywall, segment, and attribution data.

Of these, only **Country, Store, Segment,** and **Attribution** narrow the cohort's install count and any metric calculated from that count, like ARPU. Filtering by **Product duration, Product,** or **Paywall** still narrows Revenue, Payers, and the other purchase-based metrics, but the install count keeps every user who installed in the period — so ARPU can read artificially low, since its revenue numerator shrinks while its install-count denominator doesn't.
...
<ZoomImage id="7f8484d-cohorts_absolute.webp" width="700px" alt="Cohort metrics shown as absolute values" />
...
One important aspect of revenue calculation in cohorts is the inclusion of store commission fees and taxes (which can vary based on the user's store account country). Adapty currently supports commission fee and taxes calculation for both App Store and Play Store in cohort analytics.
For more details on how Adapty calculates taxes and commissions in its analytics, please refer to our [documentation](controls-filters-grouping-compare-proceeds#display-gross-or-net-revenue).
...
The actual commission deducted varies based on multiple factors, including eligibility for programs like the [Small Business Program](app-store-small-business-program) (15%), reduced rates for long-term subscriptions (15% after one year of renewal), country-specific rates, and standard rates (up to 30%).
...
Two rules are universal for every cohort metric:

- A refund is dated to the day it was issued, not to the original purchase date. A refund affects a cohort only once that refund date falls inside the selected time frame.
- A refund never moves a user out of their cohort and never changes the install count. Cohort membership is fixed when the user installs the app.

Beyond these rules, the effect of a refund depends on the metric and on the [view mode](#cohorts-by-renewals-or-by-days). Read down the column for the mode you use.

| Metric | By renewals | By days |
| --- | --- | --- |
| Installs (cohort size) | Not affected. The user stays in their cohort. | Same as "By renewals". |
| Subscriptions | Refunded subscriptions still count. | A refund removes the subscription from the count. |
| Payers | Refunded paying users still count. | A refund removes the user from the count, even if they made other successful payments. |
| Revenue | The refunded amount is subtracted from the renewal-period column where the payment was originally counted. | The refunded amount is subtracted from the refund day onward. |
| ARPU | Revenue / installs. A refund lowers revenue; the install count never changes. | Same as "By renewals". |
| ARPPU | Revenue / paying users. A refund can lower both revenue and the number of paying users, so ARPPU can move more sharply than revenue alone. | Same as "By renewals". |
| ARPAS | Revenue / active subscribers. A refund lowers revenue; the subscriber count doesn't change. | Same as "By renewals". |
| Retention | Not affected. Counts trial and purchase events, not refunds. | Same as "By renewals". |

A refund subtracts the refunded amount from revenue regardless of the active revenue accounting mode.

### Conversion rate and ARPPU

Refunds don't affect a conversion rate based on installs, because the install count never changes. A conversion rate based on paying users is different. Refunds lower it in the **by days** view, but not in the **by renewals** view.

In the **by renewals** view, the Revenue and Payers columns each show one period on its own. The ARPPU column doesn't. Each ARPPU cell adds up everything from the cohort's first period up to the column's period. So it always covers several periods at once, and leaves out refunded users. Because of this, dividing a single period's Revenue by its paying users won't reproduce the ARPPU shown.

**Example.** A user installs in February and buys a subscription, then receives a full refund in April. Viewing the February cohort **by days**:

- Time frame February–March, before the refund enters the window: the user counts as 1 paying user, and their revenue is included in full.
- Time frame February–June, after the refund enters the window: the user counts as 0 paying users, and their revenue drops to 0.

In both time frames, the February install count and Retention stay the same.

[How metrics handle refunds](refund-events#how-metrics-handle-refunds) compares the same rules across MRR, the revenue charts, and data exports.
...
These predictions are based on historical cohort growth patterns, using the app's own data when enough history is available and cross-app averages otherwise. For detailed documentation on Adapty's prediction model, please refer to our [Prediction documentation](predicted-ltv-and-revenue).
