---
title: "Get flows & paywalls - Kotlin Multiplatform"
description: "Fetch flows and paywalls from Adapty in your Kotlin Multiplatform app."
metadataTitle: "Get Flows & Paywalls - Kotlin Multiplatform | Adapty Docs"
...
- **Adapty SDK installed**: See the [Kotlin Multiplatform SDK installation guide](sdk-installation-kotlin-multiplatform).
...
To ensure optimal performance, it's crucial to retrieve the flow or paywall and its [view configuration](kmp-get-pb-paywalls#fetch-the-view-configuration) as early as possible, allowing sufficient time for images to download before presenting them to the user.
...
| **fetchPolicy** | default: `AdaptyPaywallFetchPolicy.Default` | <p>`fetchPolicy` sets which layer the SDK reads first, not whether it can use the cache. By default the SDK goes to the server first and returns cached data if that request fails. We recommend this variant because it ensures your users always get the most up-to-date data.</p><p></p><p>However, if you believe your users deal with unstable internet, consider `AdaptyPaywallFetchPolicy.ReturnCacheDataElseLoad`, which reverses that order — it reads the cache first and goes to the server only when nothing is cached. Users might not get the absolute latest data, but they'll experience faster loading times, no matter how patchy their internet connection is. The cache is updated regularly, so it's safe to use it during the session to avoid network requests.</p><p></p><p>A third policy, `AdaptyPaywallFetchPolicy.ReturnCacheDataIfNotExpiredElseLoad(maxAgeMillis)`, sits between the two: it reads the cache first while the cached copy is younger than `maxAgeMillis`, and goes to the server once it is older.</p><p></p><p>Note that the cache remains intact upon restarting the app and is only cleared when the app is reinstalled or through manual cleanup.</p> |
...
:::warning
In color and gradient assets, the alpha channel comes **last**: write `#RRGGBB` or `#RRGGBBAA`, never `#AARRGGBB`. An alpha-first string isn't rejected — it's read as `#RRGGBBAA`, so the flow renders in the wrong color with no error.
:::
...
| **fetchPolicy** | default: `AdaptyPaywallFetchPolicy.Default` | <p>`fetchPolicy` sets which layer the SDK reads first, not whether it can use the cache. By default the SDK goes to the server first and returns cached data if that request fails. We recommend this variant because it ensures your users always get the most up-to-date data.</p><p></p><p>However, if you believe your users deal with unstable internet, consider `AdaptyPaywallFetchPolicy.ReturnCacheDataElseLoad`, which reverses that order — it reads the cache first and goes to the server only when nothing is cached. Users might not get the absolute latest data, but they'll experience faster loading times, no matter how patchy their internet connection is. The cache is updated regularly, so it's safe to use it during the session to avoid network requests.</p><p></p><p>A third policy, `AdaptyPaywallFetchPolicy.ReturnCacheDataIfNotExpiredElseLoad(maxAgeMillis)`, sits between the two: it reads the cache first while the cached copy is younger than `maxAgeMillis`, and goes to the server once it is older.</p><p></p><p>Note that the cache remains intact upon restarting the app and is only cleared when the app is reinstalled or through manual cleanup.</p><p></p><p>Adapty SDK stores paywalls locally in two layers: regularly updated cache described above and [fallback paywalls](fallback-paywalls). We also use CDN to fetch paywalls faster and a stand-alone fallback server in case the CDN is unreachable. This system is designed to make sure you always get the latest version of your paywalls while ensuring reliability even in cases where internet connection is scarce.</p> |
...
:::note
If you support multiple languages, add a localization to your paywall. For the codes to use, see [Localizations and locale codes](kmp-localizations-and-locale-codes).
:::
