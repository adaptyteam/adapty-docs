...
| :-------- | :---------- |
| Flow   | An `AdaptyFlow` object containing the placement, identifiers (`id`, `variationId`), name, and remote configs (`remoteConfigs` — a list with one entry per locale). To fetch the actual products for pre-loading, custom UI, or programmatic checks, call `getPaywallProducts(flow)`. |
