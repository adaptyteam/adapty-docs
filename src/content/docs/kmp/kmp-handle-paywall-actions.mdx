---
title: "Respond to flow actions - Kotlin Multiplatform"
description: "Handle button actions from flows and paywalls in your Kotlin Multiplatform app."
...
:::warning
**Only purchases, restorations, and flow/paywall closures are handled automatically.** All other button actions, such as opening links, require proper response implementation in the app code.
:::
...
If you are using [`createNativeFlowView`](kmp-present-paywalls#without-compose-multiplatform), calling `view.dismiss()` has no effect — the view is embedded in your layout, not presented through the KMP stack. Remove the view from your layout and call `dispose()` on it instead.
...
If you are using [`createNativePaywallView`](kmp-present-paywalls#without-compose-multiplatform), calling `view.dismiss()` has no effect — the view is embedded in your layout, not presented through the KMP stack. Remove the view from your layout and call `dispose()` on it instead.
