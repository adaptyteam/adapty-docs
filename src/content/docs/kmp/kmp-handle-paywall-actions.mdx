...
If you are using [native platform views](kmp-present-paywalls#embed-in-a-native-view) (`createNativePaywallView`), calling `view.dismiss()` has no effect — the view is embedded in your layout, not presented through the KMP stack. Remove the native view from your layout and call `dispose()` on it instead.
