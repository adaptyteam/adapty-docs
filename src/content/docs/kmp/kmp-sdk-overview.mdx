...
1. [Install & configure SDK](sdk-installation-kotlin-multiplatform): Add the SDK as a dependency to your project and activate it in the code.
2. [Enable purchases through flows](kmp-quickstart-paywalls): Set up the purchase flow so users can buy products. To build your own UI, see [Implement paywalls manually](kmp-quickstart-manual) instead.
3. [Check the subscription status](kmp-check-subscription-status): Automatically check the user's subscription state and control their access to paid content.
4. [Identify users (optional)](kmp-quickstart-identify): Associate users with their Adapty profiles to ensure their data is stored consistently across devices.
...
2. **Flow or paywall** - Products bundled with configuration, attached to a placement. Two flavors:
   ...
   - **[Paywall](paywalls)** - No visual config; you build the UI in your own code and call `makePurchase` yourself. See [Implement paywalls manually](kmp-quickstart-manual).
