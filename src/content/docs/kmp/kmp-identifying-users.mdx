...
Use `uid` from `Firebase.auth.currentUser` — not the email address. Firebase UIDs are permanent and unique per user; email addresses can be changed or transferred between accounts.
...
// Must be called from a coroutine scope, e.g. viewModelScope.launch { ... }
Firebase.auth.authStateChanged.collect { user ->
    val uid = user?.uid
    if (uid != null) {
        // Use uid, not user.email — emails can change
        Adapty.identify(uid).onSuccess {
            // successful identify
        }.onError { error ->
            // handle the error
        }
    } else {
        Adapty.logout().onError { error ->
            // handle the error
        }
    }
}
