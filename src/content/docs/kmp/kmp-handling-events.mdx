...
Flows and paywalls configured with the [Flow & Paywall Builder](adapty-flow-builder) or [Paywall Builder](adapty-paywall-builder) don't need extra code to make and restore purchases. However, they generate some events that your app can respond to. Those events include button presses (close buttons, URLs, product selections, and so on) as well as notifications on purchase-related actions. Learn how to respond to these events below.
...
Kotlin Multiplatform hands you the event parameters as a raw JSON string, so decode `paramsJsonString` before you read any of them.
...
:::warning
This guide is for **legacy Paywall Builder paywalls** only.
:::
