---
title: "Kotlin Multiplatform - Present old Paywall Builder paywalls"
...
This guide is for **old Paywall Builder paywalls** only. The process for presenting paywalls differs for paywalls designed with remote config paywalls and [Observer mode](observer-vs-full-mode).
...
To display a paywall, use the `view.present()` method on the `view` created by the [`createPaywallView`](kmp-get-pb-paywalls#fetch-the-view-configuration-of-a-paywall-created-with-the-old-paywall-builder) method. Each `view` can only be used once. If you need to display the paywall again, call `createPaywallView` one more to create a new `view` instance.
