...
- **Standalone screen**: Adapty presents the paywall as a full-screen view on top of your existing UI.
...
:::note
`AdaptyUI` and `createNativePaywallView` are part of the core `io.adapty:adapty-kmp` module. The separate `io.adapty:adapty-ui-kmp` Gradle dependency — which provides the Compose Multiplatform rendering layer for `view.present()` — is not needed for native views.
:::

To embed a paywall as a native view, call `createNativePaywallView`. It returns an `AdaptyNativePaywallView` that you add to your layout:
...
<TabItem value="native" label="Native view">
```kotlin showLineNumbers title="Kotlin Multiplatform (Android)"
import com.adapty.kmp.AdaptyUI
