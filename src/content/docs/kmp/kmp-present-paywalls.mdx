...
metadataTitle: "Display Flows & Paywalls - Kotlin Multiplatform | Adapty Docs"
---
...
- For a **manually implemented paywall**, see [Render a manual paywall with remote config](present-remote-config-paywalls-kmp).
...
This guide is for **paywalls built in the old builder** only. The process for presenting paywalls differs for remote config paywalls and [Observer mode](observer-vs-full-mode).

For presenting **remote config paywalls**, see [Render a manual paywall with remote config](present-remote-config-paywalls-kmp).
