...
In SDK v4, the `paywall` parameter is replaced by a `flowPaywall` parameter taking an `AdaptyFlowPaywall` — one of the paywall variations in `flow.paywalls`. See the [migration guide](migration-to-kmp-sdk-v4).
:::
