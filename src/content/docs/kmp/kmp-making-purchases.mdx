...
:::info
Your app receives promoted in-app purchases from SDK version 4.1, on **iOS 16.4** or later. Below iOS 16.4, the listener never fires and promoted purchases complete on their own. This is an iOS-only feature: on Android the listener never fires either, and `makePromotedPurchase` returns an `AdaptyErrorCode.DEVELOPER_ERROR`.
:::

When a user starts a purchase from your App Store product page and the transaction carries over to your app, the SDK hands you the product through an `OnPromotedPurchaseListener`. Completing the purchase is up to your app: pass the product to `makePromotedPurchase`. Because you control when that happens, you can show your own screen before the purchase starts.

To support promoted purchases, register the listener and complete the purchase from it:
...
:::warning
Without a registered listener, a promoted purchase never completes: the App Store hands the product to your app and waits. Passing `null` to `setOnPromotedPurchaseListener` also stops promoted purchases from working.
:::

Register the listener at app startup, right after `Adapty.activate`. A promoted purchase usually cold-launches the app, so it often reaches the SDK before your registration runs. The SDK holds one such purchase and delivers it as soon as you register, keeping only the most recent one, and writes a console warning each time it holds a purchase.
...
`AdaptyPromotedProduct` carries `vendorProductId`, `localizedTitle`, `localizedDescription`, `price`, `regionCode`, `isFamilyShareable`, and a `subscription` of type `AdaptyProductSubscription`.
