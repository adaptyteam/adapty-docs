...
:::info
Promoted in-app purchases are supported starting from SDK version 4.1 and require **iOS 16.4** or later. On devices below iOS 16.4, they don't reach your app. This is an iOS-only feature: on Android the listener never fires, and `makePromotedPurchase` returns an `AdaptyErrorCode.DEVELOPER_ERROR`.
:::
