...
:::info
Promoted in-app purchases are supported starting from SDK version 4.1 and require **iOS 16.4** or later. On devices below iOS 16.4, they don't reach your app. The listener never fires on Android.
:::
...
To take over completion yourself — for example to show your own screen first — set an `OnPromotedPurchaseListener` and pass the product to `makePromotedPurchase`:
...
:::warning
While your listener is set, the SDK stops completing promoted purchases for you. If your listener never calls `makePromotedPurchase`, the purchase never happens: the App Store hands the product to your app and waits. Pass `null` to `setOnPromotedPurchaseListener` to restore the automatic behavior.
:::
