...
Always `await` `identify` before calling other SDK methods. Concurrent calls produce `#3006 profileWasChanged` or land on the anonymous profile. See [Call order in Capacitor SDK](capacitor-sdk-call-order).
