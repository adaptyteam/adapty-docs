...
Please be aware that this topic refers to flows and paywalls that Adapty renders. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-capacitor) topic.
...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](capacitor-present-paywalls). If the placement is a custom paywall with no Builder UI, [handle it as a remote config paywall](present-remote-config-paywalls-capacitor) instead.
