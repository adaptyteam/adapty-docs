...
Please be aware that this topic refers to flows and Paywall Builder-customized paywalls. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-capacitor) topic.
...
If the placement was designed in the **Flow Builder** or the **Paywall Builder**, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](capacitor-present-paywalls). If the placement is a custom paywall with no Builder UI, [handle it as a remote config paywall](present-remote-config-paywalls-capacitor) instead.
...
| **customLayoutId**   | <p>optional</p><p>default: `undefined`</p><p>SDK 4.1+</p> | The custom ID of a layout defined in the Flow Builder. Pass it to render that specific layout instead of the one the SDK picks automatically from the device type and screen size. If no layout matches the ID, the flow loads without a view configuration. |
