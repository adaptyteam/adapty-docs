...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](capacitor-present-paywalls). `createFlowView` throws when the flow has no view configuration — catch that error and [handle it as a manually implemented paywall](present-remote-config-paywalls-capacitor) instead.
...
Please be aware that this topic refers to Paywall Builder-customized paywalls. For guidance on fetching remote config paywalls, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-capacitor) topic.
