...
Please be aware that this topic refers to flows and paywalls that Adapty renders. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-capacitor) topic.
...
| :-------- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Flow   | An `AdaptyFlow` object with the flow's identifiers (`id`, `variationId`), name, placement, its paywall variations (`paywalls`), any remote configs (`remoteConfigs`), and — in SDK 4.1.1 and later — a `hasViewConfiguration` flag. |
...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](capacitor-present-paywalls). If the placement is a manually implemented paywall, [handle it as a manually implemented paywall](present-remote-config-paywalls-capacitor) instead.

In SDK 4.1.1 and later, the `hasViewConfiguration` flag on the flow tells the two apart before you create the view:
...
On 4.0 and 4.1.0, the flag is absent, and `createFlowView` throws an `AdaptyError` for a flow with no view configuration.
...
| **customLayoutId**   | <p>optional</p><p>default: `undefined`</p><p>SDK 4.1+</p> | The custom ID of a layout defined in the Flow & Paywall Builder. Pass it to render that specific layout instead of the one the SDK picks automatically from the device type and screen size. If no layout matches the ID, the flow loads without a view configuration. |
...
Please be aware that this topic refers to Paywall Builder-customized paywalls. For guidance on fetching a manually implemented paywall, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-capacitor) topic.
...
After fetching the paywall, check if it includes a `ViewConfiguration`, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the `ViewConfiguration` is present, treat it as a Paywall Builder paywall; if not, [handle it as a manually implemented paywall](present-remote-config-paywalls-capacitor).
