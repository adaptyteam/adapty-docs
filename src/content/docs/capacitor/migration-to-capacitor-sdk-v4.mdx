...
- `onAnalytics: (name, params)` — analytics events a flow emits, starting with a screen view for every screen the user opens.
- `onRequestPermission: (permission, customArgs)` — a permission a flow asks the user for. Unlike the other handlers, it returns a `Promise<FlowPermissionResponse>`, so the flow waits for your answer before continuing.
