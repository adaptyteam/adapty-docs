...
The 4.x line introduces flows and renames the paywall APIs accordingly. The new APIs work with both flows and legacy Paywall Builder paywalls — no setup changes are required on the Adapty Dashboard side. On top of that, 4.1 makes Adapty Attribution opt-in, renames the external attribution method, changes the fallback file format, and adds App Store promoted in-app purchases.
...
:::note
`customTimers` still exists, but it only affects legacy Paywall Builder paywalls. A flow's [countdown timer](flow-timer) runs on the behavior set in the Flow & Paywall Builder, so a flow ignores whatever you pass here.
:::
