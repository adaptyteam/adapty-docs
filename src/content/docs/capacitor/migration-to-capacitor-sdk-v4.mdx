...
The 4.x line introduces flows and renames the paywall APIs accordingly. The new APIs work with both the new Flow Builder and the existing Paywall Builder — no setup changes are required on the Adapty Dashboard side. On top of that, 4.1 renames the external attribution method, makes Adapty Attribution opt-in, changes the fallback file format, and adds App Store promoted in-app purchases.

:::note
Coming from the 4.0 beta? Replace the pinned beta version with the [4.1.0 install](#installation), then only four sections apply: [the renamed external attribution method](#renamed-external-attribution-method), [Adapty Attribution is disabled by default](#adapty-attribution-is-disabled-by-default), [fallback files](#fallback-files), and [App Store promoted in-app purchases](#app-store-promoted-in-app-purchases).
:::
...
npm install @adapty/capacitor@4.1.0
```
...
Only the names change. The provider is the same string as before (`'adjust'`, `'appsflyer'`, `'branch'`, `'tenjin'`, `'apple_search_ads'`, `'custom'`), and attribution data is still a plain object.

`AdaptyProfile.appliedAttributionSources` is unchanged — it keeps its name and its `AttributionSource[]` type. Code that reads it, such as the [Apple Ads targeting flow](capacitor-show-aa-targeted-paywall), needs no edit.
