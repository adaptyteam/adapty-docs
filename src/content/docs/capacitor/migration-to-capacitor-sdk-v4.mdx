...
The [fallback file](fallback-flows) format changed in 4.0, and again in 4.1.1. Download the file again from **[Placements](https://app.adapty.io/placements)** > **Fallbacks** and bundle it in your app, even if you already downloaded one for a 4.0 beta.
...
:::note
`customTimers` still exists, but it only affects paywalls from the old builder. A flow's [countdown timer](flow-timer) runs on the behavior set in the Flow & Paywall Builder, so a flow ignores whatever you pass here.
:::
