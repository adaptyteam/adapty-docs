---
title: "Migrate Adapty Capacitor SDK to v4.1.1"
description: "Migrate to Adapty Capacitor SDK v4.1.1: replace paywall APIs with flow APIs, adopt the renamed attribution APIs, enable Adapty Attribution explicitly, and re-download your fallback file."
metadataTitle: "Migrating to Adapty Capacitor SDK v4.1.1 | Adapty Docs"
---
...
The 4.x line introduces flows and renames the paywall APIs accordingly. The new APIs work with both the new Flow Builder and the existing Paywall Builder — no setup changes are required on the Adapty Dashboard side. On top of that, 4.1.1 makes Adapty Attribution opt-in, renames the external attribution method, changes the fallback file format, and adds App Store promoted in-app purchases.
...
:::warning
If you use [Adapty Attribution](user-acquisition) and update to SDK 4.1.1 without opting in, it breaks silently — installs stop registering, and nothing warns you.
:::

In earlier versions, the SDK registered installs for [Adapty Attribution](user-acquisition) automatically. Starting from SDK version 4.1.1, this is off by default: the SDK doesn't register installs, the `'onInstallationDetailsSuccess'` and `'onInstallationDetailsFail'` events never fire, and `getCurrentInstallationStatus` returns the `not_available` status.
...
The [fallback file](fallback-flows) format changed in the 4.x line, and again in 4.1.1. Download the file again from **[Placements](https://app.adapty.io/placements)** > **Fallbacks** and bundle it in your app, even if you already downloaded one for a 4.0 beta.
...
`hasViewConfiguration` keeps its name and meaning on the flow. `requestLocale` remains on `AdaptyOnboarding` — only the flow model drops it.
...
SDK 4.1.1 adds support for [in-app purchases promoted on your App Store product page](capacitor-making-purchases#in-app-purchases-from-the-app-store). This is a new feature, not a migration step: the SDK completes promoted purchases on its own, so you get the behavior without writing any code.
