...
When your users respond to a quiz question or input their data into an input field, the `onStateUpdated` method will be invoked. You can save or process the field type in your code.
...
If you want to immediately link the input data with the user profile and avoid asking them twice for the same info, you need to [update the user profile](capacitor-setting-user-attributes) with the input data when handling the action.
...
2. Handle the quiz responses based on their IDs and [set custom attributes](capacitor-setting-user-attributes) for users.
...
5. [Display a paywall](capacitor-paywalls) for the placement in your app code. If your onboarding has a button that opens a paywall, implement the paywall code as a [response to this button's action](capacitor-handling-onboarding-events#opening-a-paywall).