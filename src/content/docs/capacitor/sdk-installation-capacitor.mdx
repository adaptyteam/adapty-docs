...
:::important
The steps below install Adapty SDK 3.x. SDK v4 (beta) — required for the [Flow & Paywall Builder](adapty-flow-builder) and used by the [quickstart](capacitor-quickstart-paywalls) — is installed differently: follow [Adapty SDK 4.0 (beta)](#adapty-sdk-40-beta) below, or see the [migration guide](migration-to-capacitor-sdk-v4).
:::
...
Capacitor SDK 4.0 — which adds [Flow & Paywall Builder](adapty-flow-builder) support — is a pre-release. Install the exact version (npm doesn't resolve pre-releases through caret/tilde ranges), then sync:
