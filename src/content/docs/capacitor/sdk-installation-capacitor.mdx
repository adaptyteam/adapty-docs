...
Building for iOS with Adapty SDK v4 requires **Xcode 26** or later — the native iOS SDK it uses is built with Swift tools 6.2. The iOS 15.0+, Capacitor 8, and Android minSdk 24 requirements are the same as for SDK 3.16+.

:::info
Adapty Capacitor SDK 4.1 and later works with Google Play Billing Library v8.
...
:::important
The steps below install Adapty SDK 3.x. SDK 4.1 — required for the [Flow Builder](adapty-flow-builder) and used by the [quickstart](capacitor-quickstart-paywalls) — is installed differently: follow [Adapty SDK 4.1](#adapty-sdk-41) below, or see the [migration guide](migration-to-capacitor-sdk-v4).
:::
...
Capacitor SDK 4.1 — which adds [Flow Builder](adapty-flow-builder) support — is the first stable release of the 4.x line. Install it, then sync:
...
:::note
This applies to CocoaPods-based projects on **SDK 3.x**. SDK 4.1 installs on iOS through Swift Package Manager only (there is no `Podfile`) and requires iOS 15.0 — set your deployment target to 15.0 in Xcode.
:::
...
:::note
This applies to CocoaPods-based projects on **SDK 3.x**. SDK 4.1 installs the native SDKs through Swift Package Manager, so there is no `Podfile` to adjust.
:::
