...
Building for iOS requires **Xcode 26** or later — the native iOS SDK is built with Swift tools 6.2. On Android, the minimum SDK is 24.

:::info
Adapty SDK brings its own copy of the Google Play Billing Library — you don't add it to your app.

From 3.17.0, Adapty Capacitor SDK uses Billing Library v8 by default. Versions 3.15.0 through 3.16.3 use v7, and also work when another dependency in your Android build raises the Billing Library to v8.
...
On iOS, Adapty SDK pulls the native Adapty SDKs through **Swift Package Manager only** — the CocoaPods podspec was removed ([CocoaPods' spec repo goes read-only in December 2026](https://blog.cocoapods.org/CocoaPods-Specs-Repo/)). Your app's iOS project must use Capacitor's SPM integration:
...
Upgrading from v3? See [Migrate Adapty Capacitor SDK to v4.1](migration-to-capacitor-sdk-v4) for the full list of API changes.
...
- If you use the [Flow & Paywall Builder](adapty-flow-builder), follow the [quickstart](capacitor-quickstart-paywalls).
- If you build your own paywall UI, see the [quickstart for custom paywalls](capacitor-quickstart-manual).

## Activate AdaptyUI module of Adapty SDK

If you plan to use the [Flow & Paywall Builder](adapty-flow-builder), you need the AdaptyUI module. It is done automatically when you activate the core module; you don't need to do anything else.
...
:::note
This applies to CocoaPods-based projects on **SDK 3.x**. SDK 4 installs on iOS through Swift Package Manager only (there is no `Podfile`) and requires iOS 15.0 — set your deployment target to 15.0 in Xcode.
:::
...
:::note
This applies to CocoaPods-based projects on **SDK 3.x**. SDK 4 installs the native SDKs through Swift Package Manager, so there is no `Podfile` to adjust.
:::
