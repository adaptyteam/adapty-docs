...
On iOS, v4 pulls the native Adapty SDKs through **Swift Package Manager only** — the CocoaPods podspec was removed ([CocoaPods' spec repo goes read-only in December 2026](https://blog.cocoapods.org/CocoaPods-Support-Plans/)). Your app's iOS project must use Capacitor's SPM integration:
...
- For existing apps, migrate the iOS project from CocoaPods to SPM by following [Capacitor's official guide](https://capacitorjs.com/docs/ios/spm).
