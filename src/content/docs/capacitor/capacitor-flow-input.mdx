...
- **Save answers and preferences**: Track what the user picked so your app can act on it later — for example, [write it to their Adapty profile](capacitor-setting-user-attributes) as custom attributes.
...
The `onAnalytics` callback delivers all analytics events from a flow, including [screen views](capacitor-flow-screen-views).
