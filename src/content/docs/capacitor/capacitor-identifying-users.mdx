...
Use `uid` from the Firebase Auth state observer — not the email address. Firebase UIDs are permanent and unique per user; email addresses can be changed or transferred between accounts.
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        try {
            await adapty.identify({ customerUserId: user.uid }); // Use uid, not user.email
        } catch (error) {
            // handle the error
        }
    } else {
