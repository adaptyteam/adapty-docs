...
On first launch, an Apple Ads user briefly sees the default paywall before it's replaced. If you present paywalls built in Adapty's no-code builders, decide whether re-presenting is acceptable, or apply the upgrade only before the paywall is shown. Tune `timeoutMs` to how long you're willing to keep listening — attribution that's coming usually arrives within a few seconds of launch.
