...
- **Flows built in the builder**: the SDK doesn't read the device locale, so resolve it in your app and pass it as the `locale` option of `createFlowView`. It's optional — omit it and the flow renders in `en`, or in its [default locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) when the flow has no `en` localization. When you request a localization the flow doesn't have, the view falls back to the flow default without an error, and any strings the chosen localization lacks come from the default one.
