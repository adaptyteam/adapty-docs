...
- **Flows built in the builder**: the SDK doesn't read the device locale, so resolve it in your app and pass it as the `locale` option of `createFlowView`. It's optional — omit it and the flow renders in `en`, or in its [default locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) when the flow has no `en` localization.
  ...
  `view.locale` reports the localization the view was built with. Both the `locale` option and `view.locale` require Capacitor SDK 4.0.1-beta.1 or later, and `view.locale` is `undefined` on earlier versions. The [`onAppeared` handler](capacitor-handling-events#the-view-argument) reports the same value.
