...
To start a keyword list without research, get a ready-made pool for your app. Pass the app's `adam_id` from `adapty asa apps list` and the pool type — `brand`, `generic`, or `competitor`:

```bash
adapty asa keywords recommend --adam-id <adam-id> --type generic --country US
```

The pool has no bids or match types. Choose those yourself, then add the keywords as described in [Add keywords in bulk](#add-keywords-in-bulk). If the output shows `status: building`, retry in about a minute. See [Ads Manager commands](developer-cli-ads-manager-reference#adapty-asa-keywords-recommend) for the pool types and limits.
...
`--metric` is required and repeatable, and takes the names that Ads Manager tracks. See [Metrics](adapty-ads-manager-metrics) for the full list. Every metric you name is computed across the whole entity level before the page is cut, so ask for the columns you read rather than the whole catalog.

`--app`, `--campaign`, and `--ad-group` scope the report, and scoping it is the cheapest way to make a call faster, because cost follows the number of entities aggregated rather than the size of the page. Four metrics count unique profiles per entity — `subscribers`, `paid_subscribers`, `arppu`, and `arpas` — and those require a campaign or ad group filter at any entity level:
...
Metrics and the search terms list share one analytics budget per company: 5 calls per minute, and at most 2 in any 10 seconds. Ask a narrow question once rather than polling.

Two caps apply to a report. The reporting period is capped by how finely you group it — 28 days at `--group-by day`, 90 days with no period grouping, 180 by week, 365 by month — and each page is capped at 5000 breakdown rows, counting one row per entity × country × period. So widen a report by coarsening `--group-by`, not by splitting it into more calls.
...
The CLI stores the JSON you give it, so the fastest way to get a valid [automation rule](ads-manager-automations) file is to build one rule in the dashboard, then read it back:
...
### Build an Add as keyword action from flags

One action is the exception to hand-written JSON. **Add as keyword**, which promotes a search term or copies a keyword into another ad group, takes its target ad groups, bid, and match type from flags:
...
The same flags on `automations update` repair a rule already stored with the wrong shape. The CLI reads the rule, rebuilds the action from scratch, and writes it back, keeping only the stored settings that fit an **Add as keyword** action:
...
Whatever the broken rule was missing has to come from a flag. See [Ads Manager commands](developer-cli-ads-manager-reference#add-keyword-flags) for the whole set.
