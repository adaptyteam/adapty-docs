...
Publication is asynchronous. The response reports `status: publishing`. To learn when the flow is live, poll [`flows get`](#adapty-flows-get) until the status becomes `published` or `publication_failed`.
...
The response contains an `audiences` array. Each entry carries `content_type` (`paywall` or `flow`), `segment_ids`, `priority`, and either `paywall_id` or `flow_id`. The default audience has `segment_ids: []` and the highest priority value (last evaluated). See [Audiences shape](#audiences-shape).
...
A flow can be attached only after it has been published: a `draft` flow, or a flow whose first publication is still in progress, is rejected with HTTP 400 `Flow must be published before placing in a placement.` To publish a flow, use [`flows publish`](#adapty-flows-publish) or the [Flow & Paywall Builder](builder-save-publish).
