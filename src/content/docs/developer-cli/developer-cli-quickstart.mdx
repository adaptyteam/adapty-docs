...
The command asks for confirmation. Publication runs in the background, so check the status with `adapty flows get --app <app-id> <flow-id>` until it reads `published`. A placement accepts only a published flow.
