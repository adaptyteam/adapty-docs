...
If you implement your purchase screens yourself, you list what to sell in a [paywall](paywalls): which products, in what order, and with which offers. Your app fetches the paywall and renders the screen. Flows work differently: products attach to the flow itself.
...
- [Implement the paywall in your app](ios-implement-paywalls-manually) — fetch it by placement ID and implement the screen yourself.
...
The CLI creates paywalls, which only work with screens you implement yourself. To build a screen Adapty renders, create a [flow](adapty-flow-builder) in the dashboard and attach products to the flow itself.
