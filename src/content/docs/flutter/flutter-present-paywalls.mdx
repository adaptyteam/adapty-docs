...
If you've designed a flow or paywall in the builder, you don't need to worry about rendering it in your mobile app code to display it to the user. Such a flow or paywall contains both what should be shown and how it should be shown.
...
This guide is for **legacy builder paywalls** only which require SDK v3.2.0 or later. The process for presenting paywalls differs for paywalls designed with different versions of Paywall Builder and manually implemented paywalls.

- For a **manually implemented paywall**, see [Render a manual paywall with remote config](present-remote-config-paywalls-flutter).
