...
If you've created a flow, or a paywall in the old Paywall Builder, you don't need to worry about rendering it in your mobile app code to display it to the user. Such a flow or paywall contains both what should be shown and how it should be shown.

:::warning

This guide is for flows and paywalls rendered by Adapty. For presenting **Remote config paywalls**, see [Render paywall designed by remote config](present-remote-config-paywalls-flutter).
...
Starting from SDK 4.1, pass `destroy: false` to keep the view in memory instead:

```dart showLineNumbers title="Flutter"
await view.dismiss(destroy: false);
```

Present that view again and the user returns to the screen they left, with the state the flow had built up. A view kept this way is held until you dismiss it with `destroy: true`.
...
This guide covers only paywalls built in the old Paywall Builder for **Adapty SDK 3.x**. They require SDK v3.2.0 or later. Presenting a paywall built for SDK 2.x or earlier, or a remote config paywall, works differently.
