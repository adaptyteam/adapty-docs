...
metadataTitle: "Optimize Paywall Fetching | Flutter SDK | Adapty Docs"
---
...
The advice below uses the v3 method names. In SDK v4, `getPaywall` is renamed to `getFlow` and the fetch policy type to `AdaptyFlowFetchPolicy` — every rule applies unchanged.
