...
:::info
In Adapty Flutter SDK 4.0.4 and later, `restorePurchases()` completes successfully even when there is nothing to restore: it returns the current profile, so check the access level status in it. In earlier SDK versions, the method fails with the `noPurchasesToRestore` error on Android in this case.
:::
