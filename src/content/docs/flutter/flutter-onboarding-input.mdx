...
<Details>
<summary>Property shapes for each params type (click to expand)</summary>
...
If you want to immediately link the input data with the user profile and avoid asking them twice for the same info, you need to [update the user profile](flutter-setting-user-attributes) with the input data when handling the action.
...
2. Handle the quiz responses based on their IDs and [set custom attributes](flutter-setting-user-attributes) for users.
...
5. [Display a paywall](flutter-paywalls) for the placement in your app code. If your onboarding has a button that opens a paywall, implement the paywall code as a [response to this button's action](flutter-handling-onboarding-events#opening-a-paywall).