...
This guide walks through integrating Adapty into your Flutter app step by step with an AI coding tool — you feed it the right Adapty docs in the right order.
...
4. **Create a flow or paywall and a placement**: In the Adapty Dashboard, create a flow (or a paywall if you'll build the UI yourself), then assign it to a placement on the **Placements** page. In code, the placement ID is the string you pass to `Adapty().getFlow()`.
...
- [**Manually created paywalls**](flutter-quickstart-manual): You build your own paywall UI in code but still use Adapty to fetch products and handle purchases.
...
**Guides:**
- [Enable purchases with Flow & Paywall Builder (quickstart)](flutter-quickstart-paywalls)
- [Fetch flows and their configuration](flutter-get-pb-paywalls)
- [Display flows](flutter-present-paywalls)
- [Handle flow events](flutter-handling-events)
...
- [Render a manual paywall with remote config](present-remote-config-paywalls-flutter)
