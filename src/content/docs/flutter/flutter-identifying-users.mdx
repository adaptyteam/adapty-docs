...
Use `uid` from `FirebaseAuth.instance.currentUser` — not the email address. Firebase UIDs are permanent and unique per user; email addresses can be changed or transferred between accounts.
...
FirebaseAuth.instance.authStateChanges().listen((user) async {
    if (user != null) {
        try {
            await Adapty().identify(user.uid); // Use uid, not user.email
        } on AdaptyError catch (adaptyError) {
            // handle the error
        }
    } else {
...
If your app uses anonymous sign-in followed by account linking, call `identify()` when Firebase links the anonymous account — not when the anonymous session starts. See [Allow purchases without login](flutter-quickstart-identify#allow-purchases-without-login) for how to handle purchase continuity across the profile switch.
