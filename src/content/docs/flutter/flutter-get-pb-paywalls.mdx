...
Please be aware that this topic refers to flows and paywalls that Adapty renders. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-flutter) topic.
...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you — the `hasViewConfiguration` property of the fetched flow is `true`. Create the view with `createFlowView`, then [present the flow or paywall](flutter-present-paywalls). If the placement is a manually implemented paywall (`hasViewConfiguration` is `false`), [handle it as a manually implemented paywall](present-remote-config-paywalls-flutter) instead.
...
:::warning
The legacy Paywall Builder works with Flutter SDK version 3.3.0 or higher.
:::

Please be aware that this topic refers to Paywall Builder-customized paywalls. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-flutter) topic.
...
After fetching the paywall, check if it includes a `ViewConfiguration`, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the `ViewConfiguration` is present, treat it as a Paywall Builder paywall; if not, [handle it as a manually implemented paywall](present-remote-config-paywalls-flutter).
