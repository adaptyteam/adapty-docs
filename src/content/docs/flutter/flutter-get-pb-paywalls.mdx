...
Please be aware that this topic refers to flows and paywalls created in Adapty's no-code builders. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-flutter) topic.
...
:::warning
Minimum supported SDK version: v3.3.0
:::

Please be aware that this topic refers to paywalls created with the old Paywall Builder. To use the Flow & Paywall Builder, update your SDK to version 4. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-flutter) topic.
...
To ensure optimal performance, it's crucial to retrieve the paywall and its [view configuration](flutter-get-pb-paywalls#fetch-the-view-configuration-of-a-paywall-created-with-the-old-paywall-builder) as early as possible, allowing sufficient time for images to download before presenting them to the user.
...
Check whether the fetched paywall has a `ViewConfiguration`. If it does, the old Paywall Builder created the paywall, and the Adapty SDK renders it for you — follow the steps below. If it doesn't, you render the paywall yourself; see [remote config paywalls](present-remote-config-paywalls-flutter).
...
To address this, you can use the `getPaywallForDefaultAudience`  method, which fetches the paywall of the specified placement for the **All Users** audience. However, it's crucial to understand that the recommended approach is to fetch the paywall by the `getPaywall` method, as detailed in the [Fetch Paywall Information](flutter-get-pb-paywalls#fetch-a-paywall-created-with-the-old-paywall-builder) section above.
