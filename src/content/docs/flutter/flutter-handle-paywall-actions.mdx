...
:::warning
**Purchases, restorations, closing, and opening URLs are handled automatically** by the default `flowViewDidPerformAction` implementation. All the other button actions, such as logging in or opening another flow, require implementing proper responses in the app code.
:::
...
If you want to open URLs yourself — for example, through the `url_launcher` plugin — override the handler:
