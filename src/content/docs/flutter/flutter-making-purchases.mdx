...
**Looking for step-by-step guidance?** Check out the [quickstart guide](flutter-implement-paywalls-manually) for end-to-end implementation instructions with full context.
:::
...
| Parameter                    | Presence | Description                                                                                             |
| :--------------------------- | :------- |:--------------------------------------------------------------------------------------------------------|
| **parameters** | required | an `AdaptyPurchaseParameters` object with its `subscriptionUpdateParams` field set to an [`AdaptyAndroidSubscriptionUpdateParameters`](https://pub.dev/documentation/adapty_flutter/latest/adapty_flutter/AdaptyAndroidSubscriptionUpdateParameters-class.html) object. |
...
## Promoted in-app purchases from the App Store

:::info
Promoted in-app purchases are supported from SDK version 4.1, on **iOS 16.4** or later. Below iOS 16.4, on Android, and in observer mode, `didReceivePromotedPurchaseStream` never emits.
:::
...
To complete promoted purchases yourself — for example to show your own screen first — subscribe to `didReceivePromotedPurchaseStream` and pass the product to `makePromotedPurchase`:
...
:::warning
While anything is subscribed to the stream, the SDK stops completing promoted purchases for you. If your handler never calls `makePromotedPurchase`, the purchase never happens: the App Store hands the product to your app and waits. The SDK doesn't step in when your handler throws either.
:::
...
Who completes the purchase is decided for every event, so a stream you listen to only while a screen is mounted hands promoted purchases back to the SDK once that screen is gone. You can subscribe before `activate`, which is how you catch a purchase the store delivers at launch.
