...
:::warning
Promoted in-app purchases reach your app from SDK version 4.1 on, and your app completes them. On SDK 4.0 the SDK completed them on its own. From 4.1, a promoted purchase that nothing listens for is dropped, so add the listener below when you update. See [Migrate to v4.1](migration-to-flutter-sdk-41#promoted-in-app-purchases-now-wait-for-your-app).
:::

When a user starts a purchase from your App Store product page and the transaction carries over to your app, the SDK delivers the product to `didReceivePromotedPurchaseStream`. Subscribe to the stream and pass the product to `makePromotedPurchase`:
...
Subscribe during app startup, right after `activate`, so the listener exists before a promoted purchase can arrive. `didReceivePromotedPurchaseStream` is a broadcast stream that doesn't replay: a product delivered while nothing is listening is dropped.
