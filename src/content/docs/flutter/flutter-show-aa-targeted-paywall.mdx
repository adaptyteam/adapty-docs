...
description: "Wait briefly for Apple Ads attribution before showing the paywall on first launch in Flutter, falling back to the default audience on timeout. Uses AdaptyProfile.appliedExternalAttributionProviders."
...
You need:
- Adapty Flutter SDK **4.1** or later. On 4.0.x, the profile property is named `appliedAttributionSources` and its values are `AdaptyAttributionSource` — see [Migrate to v4.1](migration-to-flutter-sdk-41). On 3.17.0–3.x, flows are also fetched with `getPaywall`/`getPaywallForDefaultAudience` and the returned type is `AdaptyPaywall` — see [Migrate to v4.0](migration-to-flutter-sdk-v4).
- Apple Ads configured for the app in Adapty. See [Apple Ads](apple-search-ads).

## How it works

After `Adapty().activate()`, the SDK requests Apple Ads attribution from Apple in the background and forwards the result to Adapty's backend. When AA becomes the active attribution source for the profile, the SDK delivers an updated `AdaptyProfile` to your `didUpdateProfileStream` listener, with `AdaptyExternalAttributionProvider.appleAds` in its `appliedExternalAttributionProviders` list.

On first launch, this gives you two outcomes to handle:

1. **Attribution lands within your timeout.** Call `getFlow` — Adapty resolves the request against the Apple Ads audience and returns the targeted paywall.
2. **The timeout elapses first.** Show the default-audience paywall instead, so users without Apple Ads attribution aren't left waiting. `getFlowForDefaultAudience` returns it without waiting for segmentation.

`appliedExternalAttributionProviders` can be empty. That means one of the following:
...
In all three cases, `getFlowForDefaultAudience` is safe to call — it returns the default-audience paywall regardless of the profile state.

:::important
The wait only applies to the first launch. Once Apple Ads attribution has been recorded, it's stored on the profile permanently. On every subsequent launch, the cached profile already carries `AdaptyExternalAttributionProvider.appleAds` in `appliedExternalAttributionProviders`, so the attribution path resolves right away and `getFlow` returns the Apple-Ads-segmented paywall without any delay.
:::

## Implementation

On first launch, wait for `AdaptyExternalAttributionProvider.appleAds` and apply a hard timeout — if Apple Ads attribution never arrives, those users still need to see a paywall.
...
3. **Watch for `AdaptyExternalAttributionProvider.appleAds` in `appliedExternalAttributionProviders`.** When it appears, load the paywall with `getFlow` — Adapty returns the AA-segmented variant:
...
- If attribution lands within `timeout`, it returns the segmented paywall via `getFlow`.
- If `timeout` elapses first, it returns the prefetched default-audience paywall via `getFlowForDefaultAudience`.
