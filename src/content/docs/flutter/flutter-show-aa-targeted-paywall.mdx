...
:::important
`appliedAttributionSources` only reports Apple Ads. Attribution from other providers is visible on user profiles and available in segment filters, but not yet here.
:::
...
:::note
The examples use the 3.x API names. On SDK 4.0 and later, paywalls are fetched with `getFlow`/`getFlowForDefaultAudience` and the returned type is `AdaptyFlow`. See [Migrate Adapty Flutter SDK to v4.0](migration-to-flutter-sdk-v4). The profile property keeps the name `appliedAttributionSources` on every Flutter version.
:::
...
`appliedAttributionSources` can be empty. That means one of the following:
...
In all three cases, `getPaywallForDefaultAudience` is safe to call — it returns the default-audience paywall regardless of the profile state.
