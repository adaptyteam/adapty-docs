...
description: "Migrate to Adapty Flutter SDK v4.0 by replacing paywall APIs with flow APIs, compatible with both flows and legacy builder paywalls."
metadataTitle: "Migrating to Adapty Flutter SDK v4.0 | Adapty Docs"
---

Adapty Flutter SDK 4.0 introduces flows and renames the paywall APIs accordingly. The new APIs work with both flows and legacy builder paywalls — no setup changes are required on the Adapty Dashboard side.
...
The returned type changes from `AdaptyPaywall` to `AdaptyFlow`, and you no longer pass a `locale` — when you render a flow, the localization is resolved automatically; for manually implemented paywalls, all configured locales are returned in `flow.remoteConfigs`:
