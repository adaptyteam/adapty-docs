...
description: "Migrate to Adapty Flutter SDK v4.0 by replacing paywall APIs with flow APIs, which render flows in addition to old builder paywalls."
metadataTitle: "Migrating to Adapty Flutter SDK v4.0 | Adapty Docs"
---

Adapty Flutter SDK 4.0 introduces flows and renames the paywall APIs accordingly. The new APIs work with flows, and they still work with paywalls from the old builder — no setup changes are required on the Adapty Dashboard side.

:::note
This guide covers the 3.x → 4.0 migration. SDK 4.1 changes a few more APIs on top of it — Adapty Attribution becomes opt-in, the external attribution methods are renamed, and the fallback file format changes again. After you finish here, see [Migrate to v4.1](migration-to-flutter-sdk-41).
:::
...
Kids Mode also requires you to disable IP address collection in the Adapty Dashboard — see [Kids Mode](kids-mode-flutter) for the full setup.
...
- The new optional `flowViewDidReceiveAnalyticEvent` callback reports analytics events from a flow, starting with a screen view for every screen a user opens. See [Track flow screen views](flutter-flow-screen-views).

v4 also adds capabilities you can opt into:

- `AdaptyUI().setObserverModeResolver(...)` with an `AdaptyUIObserverModeResolver` — drive purchases and restores initiated from flows while the SDK runs in [Observer mode](implement-observer-mode-flutter). Previously this was available only in the native iOS and Android SDKs. See [Present flows in Observer mode](flutter-present-flows-in-observer-mode).
- `AdaptyUI().setSystemRequestsHandler(...)` with an `AdaptyUISystemRequestsHandler` — reserved for system requests from a flow (OS permission prompts and App Store review requests). Flows don't trigger these requests yet, so you don't need to register a handler.
...
- **View errors**: `flowViewDidReceiveError` is required, and dismissal is up to your implementation. If your v3 integration relied on the view being dismissed automatically on rendering errors, call `view.dismiss()` in this callback.
...
The legacy onboarding API is deprecated in v4.0 in favor of the [Flow & Paywall Builder](adapty-flow-builder). It still works, and your IDE flags the deprecated symbols through their `@Deprecated` annotations — there are no runtime warnings. These symbols will be removed in a future release, so plan migration of your onboardings to the Flow & Paywall Builder.
