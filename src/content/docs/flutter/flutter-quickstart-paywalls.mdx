---
title: "Enable purchases with Flow & Paywall Builder in Flutter SDK"
...
- [**Flows**](adapty-flow-builder) – screen sequences that present products to users, built in the no-code Flow & Paywall Builder. The SDK retrieves them via `getFlow`. If you'd rather build the UI in your own code, use a paywall instead — see [Implement paywalls manually](flutter-quickstart-manual).
...
:::important
To get the view configuration, [publish the flow](builder-save-publish). A flow with unpublished edits has the **Dirty** status, and its placement keeps serving the last published version.
:::
