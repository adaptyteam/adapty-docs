...
In SDK v4, the `paywall` parameter takes an `AdaptyFlowPaywall` — a paywall variation of the fetched flow. Check that `flow.paywalls` isn't empty before indexing into it, for example `flow.paywalls[0]`.
:::
