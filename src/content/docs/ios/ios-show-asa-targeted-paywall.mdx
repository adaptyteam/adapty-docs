...
3. **Check `appliedAttributionSources` on every profile update.** When the array contains `.appleAds`, request the paywall — Adapty will return the ASA-segmented variant.
...
4. **Apply a timeout** of 3–5 seconds. If `.appleAds` never appears, call `getPaywall(placementId:)` anyway when the timer fires. Configure a [fallback paywall](fallback-paywalls) for the placement so the user is never stuck on the splash screen.
