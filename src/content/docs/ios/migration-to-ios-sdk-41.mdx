...
description: "Migrate to Adapty iOS SDK v4.1: enable Adapty Attribution explicitly, adopt the renamed external attribution APIs, and re-download your fallback file."
...
| Promoted in-app purchases completed automatically, with no way to intercept them | `didReceivePromotedPurchase(_:)` delegate method and `AdaptyPromotedProduct` hand completion to your app |
...
SDK 4.1 restores the hook for App Store promoted in-app purchases, which [4.0 removed](migration-to-ios-sdk-v4#app-store-promoted-in-app-purchases-temporarily-removed). On 4.0, such a purchase still completed and Adapty recorded the transaction; your app just couldn't intercept it. 4.1 keeps that default, so this is a new capability rather than a migration step.
...
:::warning
The new mechanism is built on StoreKit 2 and requires **iOS 16.4** or later. The 3.x `shouldAddStorePayment` method worked on earlier iOS versions. Below iOS 16.4, `didReceivePromotedPurchase` never fires and promoted purchases complete without reaching your app.
:::
