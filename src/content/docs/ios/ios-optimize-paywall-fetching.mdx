...
`preloadFlows` writes placement JSON into the SDK cache ahead of time — one request per placement, and nothing else. Despite the name, it doesn't fetch the flow's UI layout or its media.

A later `getFlow` reads that cache only when you pass `fetchPolicy: .returnCacheDataElseLoad` or `.returnCacheDataIfNotExpiredElseLoad(maxAge:)`. The default policy, `.reloadRevalidatingCacheData`, goes to the network on every call, so a preload paired with a default-policy fetch costs one extra request and saves nothing — the warmed copy only serves as an offline fallback.

Use it when you know which placements the session will need but don't want to show them yet — for example, right after `activate` and `identify` resolve, for the paywall behind a button the user hasn't tapped.
