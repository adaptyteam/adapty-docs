...
`getFlowConfiguration` awaits the layout, so on a cold cache that call still costs a round trip after a preload. Once the SDK has the layout, it starts caching the images independently of the call: it doesn't hold up the screen, and there's no callback, delegate method, or error that reports when it finishes.
