...
:::info
These methods are available starting from iOS SDK version 4.1.
:::
...
- `loadTimeout` (optional): timeout in seconds for the whole batch, not per placement. Defaults to 5 seconds.
...
- If a placement can't be fetched for the user's audience, the SDK automatically falls back to the default-audience variation for that placement.
