...
description: "Migrate to Adapty iOS SDK v4.0 by replacing paywall APIs with flow APIs, which render flows in addition to old builder paywalls."
metadataTitle: "Migrating to Adapty iOS SDK v4.0 | Adapty Docs"
---

Adapty iOS SDK 4.0 introduces flows and renames the paywall APIs accordingly. The new APIs work with flows, and they still work with paywalls from the old builder — no setup changes are required on the Adapty Dashboard side.
...
Adapty iOS SDK 4.0 drops CocoaPods support. Install the SDK with [Swift Package Manager](sdk-installation-ios#install-adapty-sdk). v4 also requires **Xcode 26** or later, since its package manifest declares Swift tools 6.2.
...
## App Store promoted purchase handling temporarily removed \{#app-store-promoted-in-app-purchases-temporarily-removed\}

As part of the StoreKit 2 migration, Adapty iOS SDK 4.0 removes the API for handling App Store promoted in-app purchases: the `shouldAddStorePayment(for:)` delegate method and the `AdaptyDeferredProduct` type it receives are unavailable in 4.0. Such purchases still complete and Adapty records the transaction — 4.0 gives your app no way to intercept or defer them.

:::warning
This removal only applies to 4.0 — the handling returns in SDK 4.1 with a new API. If your app intercepts promoted in-app purchases, migrate from 3.x directly to 4.1. See [In-app purchases from the App Store](making-purchases#in-app-purchases-from-the-app-store).
:::
...
The returned types change from `AdaptyPaywall` / `AdaptyUI.PaywallConfiguration` to `AdaptyFlow` / `AdaptyUI.FlowConfiguration`. The `locale` parameter moves out of the fetch call and onto `getFlowConfiguration`: `AdaptyFlow` also exposes `paywalls` — the flow's paywall variations, as `AdaptyFlowPaywall` values. The web paywall methods take one of these.
...
:::note
SDK 4.1 renames this method to `updateExternalAttribution(_:provider:)` and the type to `AdaptyExternalAttributionProvider`. The old names are removed in 4.1 without deprecation. If you migrate to 4.1 directly, see [Migrate to v4.1](migration-to-ios-sdk-41).
:::
