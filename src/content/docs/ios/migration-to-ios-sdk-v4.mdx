...
description: "Migrate to Adapty iOS SDK v4.0 by replacing paywall APIs with flow APIs, compatible with both flows and paywall-builder paywalls."
metadataTitle: "Migrating to Adapty iOS SDK v4.0 | Adapty Docs"
---

Adapty iOS SDK 4.0 introduces flows and renames the paywall APIs accordingly. The new APIs work with both flows and paywall-builder paywalls — no setup changes are required on the Adapty Dashboard side.
...
As in v3, you do not need to call this method when displaying flows or paywalls rendered by [Adapty's no-code builders](adapty-flow-builder) — Adapty tracks those views automatically.
