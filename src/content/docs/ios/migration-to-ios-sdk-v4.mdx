...
In v3, you enabled [Kids Mode](kids-mode) by selecting the separate **Adapty_KidsMode** and **AdaptyUI_KidsMode** package products and renaming your imports. Starting from `4.0.0-beta.2`, these products are removed. Kids Mode is now a Swift package trait named `KidsMode` on the regular Adapty package — enabling it compiles out IDFA and AdSupport across the whole SDK.
...
In v3, `didFinishPurchase` was optional: if you didn't implement it, the paywall dismissed itself after a successful purchase. Starting from `4.0.0-beta.2`, this default auto-dismiss implementation is removed so that a flow can continue after a successful purchase — for example, to show the remaining screens of your flow. You now decide what happens after a purchase: dismiss the screen, or do nothing to let the flow continue.
