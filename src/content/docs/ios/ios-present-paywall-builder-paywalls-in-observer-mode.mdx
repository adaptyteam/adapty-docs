...
<details>
   ...
   4. [Configure paywalls or flows in the builders](create-paywall) and assign products to them.
   5. [Create placements and assign your paywalls or flows to them](create-placement).
...
1. Implement the `AdaptyObserverModeResolver` object. The protocol is the same as in SDK v3 — observer mode itself does not change between paywall and flow rendering:
...
2. Create a flow configuration object, passing your resolver as the `observerModeResolver:` parameter:
   ...
   | :----------------------- | :------- | :----------------------------------------------------------------------------------------------------------------------- |
   | **forFlow**              | required | An `AdaptyFlow` object obtained via `Adapty.getFlow(placementId:)`. See [Get paywalls and flows](get-pb-paywalls). |
...
3. Initialize the flow controller using `AdaptyUI.flowController(with:delegate:)`:
   ...
   | Parameter                  | Presence | Description                                                                                                                                      |
   | :------------------------- | :------- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
   | **flowConfiguration**      | required | An `AdaptyUI.FlowConfiguration` object containing visual details of the flow. See [Get paywalls and flows](get-pb-paywalls).                     |
   | **delegate**               | required | An `AdaptyFlowControllerDelegate` to listen to flow events. See [Handle paywall & flow events](ios-handling-events).                             |
