...
description: "Present flows and Paywall Builder paywalls in Observer mode in your iOS app while handling purchases with your own code."
...
:::warning
This section refers to [Observer mode](observer-vs-full-mode) only. If you do not work in the Observer mode, refer to the [iOS - Display flows & paywalls](ios-present-paywalls) topic instead.
:::

:::tip
Build the flow itself in [the Flow Builder](adapty-flow-builder).
:::
...
<details>
   ...
   1. Set up initial integration of Adapty [with the App Store](initial_ios).
   2. Install and configure Adapty SDK. Make sure to set the `observerMode` parameter to `true`. Refer to the [iOS SDK installation guide](sdk-installation-ios#activate-adapty-module-of-adapty-sdk).
   ...
   6. [Fetch flows and their configuration](get-pb-paywalls) in your mobile app code.
...
1. Implement the `AdaptyObserverModeResolver` object. The protocol is the same as in SDK v3 — observer mode itself does not change between flow and paywall rendering:
