...
`didFailPurchase` is the only purchase event without a default implementation. `didFinishPurchase` defaults to dismissing the controller on success; override only if you need custom post-purchase logic.
