...
In SwiftUI, when displaying the paywall, you also need to handle events. `didFailPurchase`, `didFinishRestore`, `didFailRestore`, and `didReceiveError` are required. When testing, you can just copy the code from the snippet below to log these events.
...
.flow(
    ...
    didReceiveError: { error in paywallPresented = false }
)
```
</TabItem>
