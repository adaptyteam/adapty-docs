---
title: "Show an AA-targeted paywall on first launch in iOS SDK"
description: "Wait for Apple Ads attribution before requesting the paywall on iOS using AdaptyProfile.appliedExternalAttributionProviders."
...
Even with an empty array, `getFlow` is safe to call — Adapty resolves the request against whichever audience matches the current profile state, typically the default audience.

:::important
The wait only applies to **first launch**. Once Apple Ads attribution has been recorded, it's stored on the profile permanently. On every subsequent launch, the cached profile already carries `.appleAds` in `appliedExternalAttributionProviders`, `didLoadLatestProfile` fires with that value immediately, and `getFlow` returns the Apple-Ads-segmented paywall without any delay.
:::
...
`ProfileObserver` is a reusable singleton that publishes profile updates from `AdaptyDelegate`. `FlowLoader.getFlowOrDefault` runs the race using a structured-concurrency `TaskGroup`:

- If attribution lands within `timeout`, it returns the segmented paywall via `getFlow(placementId:)`.
- If `timeout` elapses first, it returns the prefetched default-audience paywall via `getFlowForDefaultAudience(placementId:)`.
