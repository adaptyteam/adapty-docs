...
:::important
This guide covers event handling for purchases, restorations, product selection, and flow rendering. You can also set up button handling (closing the flow, opening links, custom actions, etc.). See our [guide on handling button actions](react-native-handle-paywall-actions) for details.
:::

Flows and paywalls built in the Flow & Paywall Builder don't need extra code to make and restore purchases. However, they generate some events that your app can respond to. Those events include button presses (close buttons, URLs, product selections, and so on) as well as notifications on purchase-related actions taken on the flow. Learn how to respond to these events below.
...
The `view` argument requires React Native SDK 4.0.3 or later. Of all the flow handlers, only `onAppeared` receives a description of the view itself — a `FlowEventView` object with these fields:
...
| **locale**         | The [flow localization](add-paywall-locale-in-adapty-paywall-builder) the view was built with. It differs from the locale you requested when the flow has no such localization. Read it to align the rest of your screen with the language the flow rendered in. See [Use localizations and locale codes](react-native-localizations-and-locale-codes). |
...
:::warning
This guide covers only **paywalls built in the old builder**, which require Adapty SDK v3.0 or later.
:::
