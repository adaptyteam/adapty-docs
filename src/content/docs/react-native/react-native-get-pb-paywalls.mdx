...
Please be aware that this topic refers to flows and paywalls created in Adapty's no-code builders. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-react-native) topic.
...
If the placement was designed in Adapty's no-code builders, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](react-native-present-paywalls). If the placement is a custom paywall with no Builder UI, [handle it as a remote config paywall](present-remote-config-paywalls-react-native) instead.
...
:::warning
Minimum supported SDK version: v3.0
:::

Please be aware that this topic refers to paywalls created with the old Paywall Builder. To use the Flow & Paywall Builder, update your SDK to version 4. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-react-native) topic.
...
To ensure optimal performance, it's crucial to retrieve the paywall and its [view configuration](react-native-get-pb-paywalls#fetch-the-view-configuration-of-a-paywall-created-with-the-old-paywall-builder) as early as possible, allowing sufficient time for images to download before presenting them to the user.
...
Check whether the fetched paywall has a `ViewConfiguration`. If it does, the old Paywall Builder created the paywall, and the Adapty SDK renders it for you — follow the steps below. If it doesn't, you render the paywall yourself; see [remote config paywalls](present-remote-config-paywalls-react-native).
...
:::note
If you are using multiple languages, learn how to add a [localization](add-paywall-locale-in-adapty-paywall-builder) and how to use locale codes correctly [here](react-native-localizations-and-locale-codes).
:::
