...
To ensure optimal performance, it's crucial to retrieve the flow or paywall and its [view configuration](react-native-get-pb-paywalls#fetch-the-view-configuration) as early as possible, allowing sufficient time for images to download before presenting them to the user.
