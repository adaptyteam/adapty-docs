...
Please be aware that this topic refers to flows and paywalls designed in Adapty's builders. If you are implementing your paywalls manually, please refer to the [Fetch paywalls and products for remote config paywalls in your mobile app](fetch-paywalls-and-products-react-native) topic.
...
| :-------- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Flow   | An `AdaptyFlow` object with the flow's identifiers (`id`, `variationId`), name, placement, its paywall variations (`paywalls`), any remote configs (`remoteConfigs`), and — in SDK 4.1 and later — a `hasViewConfiguration` flag. |
...
If the placement was designed in the **Flow & Paywall Builder** or the **old Paywall Builder**, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](react-native-present-paywalls). If the placement is a custom paywall with no Builder UI, [handle it as a remote config paywall](present-remote-config-paywalls-react-native) instead.

In SDK 4.1 and later, the `hasViewConfiguration` flag on the flow tells the two apart before you create the view:
...
On 4.0, the flag is absent, and `createFlowView` throws an `AdaptyError` for a flow with no view configuration.
...
| **customLayoutId**   | <p>optional</p><p>default: `undefined`</p><p>SDK 4.1+</p> | Reserved for a Flow & Paywall Builder capability that isn't released yet. Until the builder can define layout IDs, any value you pass matches no layout and `createFlowView` throws an `AdaptyError`. Leave it unset. |
...
:::warning
Paywalls built in the SDK 3.x Paywall Builder require React Native SDK version 3.0 or higher.
:::
...
:::note
If you support multiple languages, add a localization to your paywall. For the codes to use, see [Localizations and locale codes](react-native-localizations-and-locale-codes).
:::
