...
keywords: ['getFlow', 'getPaywall', 'getPaywallConfiguration', 'getViewConfiguration', 'createFlowView', 'createPaywallView', 'getFlowForDefaultAudience', 'getPaywallForDefaultAudience']
rank: 90
---
...
| :-------- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Flow   | An `AdaptyFlow` object with a list of product IDs, the flow/paywall identifier, remote config, and a `hasViewConfiguration` flag indicating whether the flow includes a view configuration. |
...
- **`true`** — the placement was designed in the **Flow Builder** (a flow) or the **Paywall Builder** (a paywall). Adapty renders the UI for you. Continue with the steps below to create the view and [present the flow or paywall](react-native-present-paywalls).
- **`false`** — the placement is a custom paywall with no Builder UI. [Handle it as a remote config paywall](present-remote-config-paywalls-react-native).

In React Native SDK, directly call the `createFlowView` method without manually fetching the view configuration first.
...
// for the Adapty SDK < 3.14 – import {createFlowView} from 'react-native-adapty/dist/ui';
