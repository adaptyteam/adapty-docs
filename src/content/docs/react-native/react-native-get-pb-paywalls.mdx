...
Please be aware that this topic refers to flows and paywalls that Adapty renders. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-react-native) topic.
...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](react-native-present-paywalls). `createFlowView` throws when the flow has no view configuration — catch that error and [handle it as a manually implemented paywall](present-remote-config-paywalls-react-native) instead.
...
Please be aware that this topic refers to Paywall Builder-customized paywalls. If you are implementing your paywalls manually, please refer to the [Fetch a manual paywall and its products](fetch-paywalls-and-products-react-native) topic.
