...
If the placement is a flow or a paywall with a view configuration, Adapty renders the UI for you. Create the view with `createFlowView`, then [present the flow or paywall](react-native-present-paywalls). If the placement is a manually implemented paywall, [handle it as a manually implemented paywall](present-remote-config-paywalls-react-native) instead.
...
:::warning
The legacy Paywall Builder works with React Native SDK version 3.0 or higher.
:::
...
After fetching the paywall, check if it includes a `ViewConfiguration`, which indicates that it was created using Paywall Builder. This will guide you on how to display the paywall. If the `ViewConfiguration` is present, treat it as a Paywall Builder paywall; if not, [handle it as a manually implemented paywall](present-remote-config-paywalls-react-native).
