...
| wrongParam           | 3001 | This error indicates that some of your parameters are not correct. <br/> If you're using the old Paywall Builder and can't display a paywall because of this error, toggle on **Show on device** in that builder.<br/> Another possible reason for this issue is that the local [fallback](fallback-paywalls) file version doesn't match the SDK version. Download a new file in the dashboard. |
