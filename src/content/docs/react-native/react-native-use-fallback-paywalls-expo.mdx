...
metadataTitle: "Using Fallback Paywalls in Expo | Adapty Docs"
...
The Adapty SDK reads the fallback file from the **native** bundle — an iOS resource inside the `.app` package, or an entry under `android/app/src/main/assets/`. In an Expo project, `npx expo prebuild` regenerates those directories on every run, so you can't drop the files in manually. The `react-native-adapty` config plugin wires the file into the native bundle for you.
