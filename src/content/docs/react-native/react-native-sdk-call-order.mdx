...
| 3    | `adapty.updateAttribution(data, source, networkUserId)` for each MMP                                          | After step 2, before any user-action call                                              | Required so MMP IDs land on the correct profile.                                               |
