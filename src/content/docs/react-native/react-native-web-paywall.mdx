...
2. `openWebPaywall(paywall)` that generates URLs by paywall without adding the product data to URLs. In SDK v4 this takes an `AdaptyFlowPaywall` — a paywall variation of the fetched flow. Check that `flow.paywalls` isn't empty before indexing into it, for example `flow.paywalls[0]`.
:::
