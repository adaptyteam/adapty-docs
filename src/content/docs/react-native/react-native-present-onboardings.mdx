...
1. You have installed [Adapty React Native SDK](sdk-installation-reactnative) 3.8.0 or later.
...
:::note
On Android, we recommend additional configuration for `AdaptyOnboardingView` to avoid a visual rendering artifact. See [System UI overlaps onboarding content on Android](#system-ui-overlaps-onboarding-content-on-android).
:::
...
- **Android**: Create `adapty_onboarding_placeholder_view.xml` in `res/layout` and define a placeholder there. [Learn more](android-present-onboardings#add-smooth-transitions-between-the-splash-screen-and-onboarding).
...
### System UI overlaps onboarding content on Android
...
When using `AdaptyOnboardingView` on Android, system UI elements such as the status bar and navigation bar may appear on top of the paywall content. To prevent this, add the following boolean resource to your app:
...
Once you've presented your onboarding, you'll want to [handle user interactions and events](react-native-handling-onboarding-events). Learn how to handle onboarding events to respond to user actions and track analytics.
