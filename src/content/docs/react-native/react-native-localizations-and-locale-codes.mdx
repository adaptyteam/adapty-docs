...
- **Flows built in the builder**: `createFlowView` takes no locale parameter, and the SDK doesn't read the device locale. The flow renders in its [default locale](add-paywall-locale-in-adapty-paywall-builder#set-the-default-locale) — on iOS, in `en` when the flow has an English localization. To confirm which localization Adapty applied, read `view.locale` on the controller `createFlowView` returns. It requires React Native SDK 4.0.2 or later and is `undefined` on earlier versions.
