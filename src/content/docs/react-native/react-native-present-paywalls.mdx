...
:::note
On Android, we recommend additional configuration for `AdaptyPaywallView` to avoid a visual rendering artifact. See [System UI overlaps paywall content on Android](#system-ui-overlaps-paywall-content-on-android).
:::
...
When using `AdaptyPaywallView` on Android, system UI elements such as the status bar and navigation bar may appear on top of the paywall content. To prevent this, add the following boolean resource to your app:
