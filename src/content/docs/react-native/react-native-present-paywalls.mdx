...
If you've created a flow, or a paywall in the old Paywall Builder, you don't need to worry about rendering it in your mobile app code to display it to the user. Such a flow contains both what should be shown within it and how it should be shown.

Before you start, ensure that:

1. You have created a [flow](paywall-builder-templates) or a [paywall](create-paywall).
...
This guide is for **flows and paywalls rendered by Adapty** only, which require SDK v4.0 or later. The process for presenting flows differs for manually implemented paywalls.
...
This guide covers only paywalls built in the old Paywall Builder for **Adapty SDK 3.x**. They require SDK v3.0 or later. Presenting a paywall built for SDK 2.x or earlier, or a remote config paywall, works differently.
