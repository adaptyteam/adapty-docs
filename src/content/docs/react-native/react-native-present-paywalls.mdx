...
:::note
On Android, if the flow does not extend behind the status bar, a visual overlay may appear at its top. We recommend you to turn it off for your flows. See [Visual overlay at the top of the flow (Android)](#visual-overlay-at-the-top-of-the-flow-android).
:::
...
To display a flow as a standalone screen, use the `view.present()` method on the `view` created by the [`createFlowView`](react-native-get-pb-paywalls#fetch-the-view-configuration-of-paywall-designed-using-paywall-builder) method. Each `view` can only be used once. If you need to display the flow again, call `createFlowView` one more time to create a new `view` instance.
...
const productPurchaseParams = flow.productIdentifiers.map((productId) => {
  let params = {};
...
### Visual overlay at the top of the flow (Android)

:::note
This setting is only available in bare React Native projects.
...
If `AdaptyFlowView` does not extend behind the status bar, a visual overlay may still appear at its top. To remove it, add the following boolean resource to your app:
...
Note that the changes apply globally for all flows in your app.
