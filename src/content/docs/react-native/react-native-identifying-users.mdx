...
Use `uid` from `auth().currentUser` — not the email address. Firebase UIDs are permanent and unique per user; email addresses can be changed or transferred between accounts.
