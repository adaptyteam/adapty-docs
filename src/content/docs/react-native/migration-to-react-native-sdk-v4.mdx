...
| `products` | `flow.variations[i].productIdentifiers` | Product identifiers now live on each flow variation, not on the flow. |
...
| _(new)_ | `variations: AdaptyFlowPaywall[]` | Each entry is one paywall variation in the flow. |
...
+ const ids = flow.variations[0].productIdentifiers;
```
...
+ await adapty.openWebPaywall(flow.variations[0]);
```
...
- **`ProductReference`**: Use `AdaptyProductIdentifier`, read from `flow.variations[i].productIdentifiers`.
...
`lockMethodsUntilReady` is deprecated and now on by default. Remove it from your `activate` call — keeping it has no effect and triggers a deprecation warning:
...
The flat Android subscription-update shape is deprecated. Move `oldSubVendorProductId` and `prorationMode` into a nested `subscriptionUpdateParams` object, and keep `isOfferPersonalized` at the top level. See [Make purchases](react-native-making-purchases) for the full example.
...
- **`dismiss()`**: The presented view is now destroyed, not just hidden. Call `createFlowView` again to present the flow once more.
- **Embedded `AdaptyFlowView`**: An embedded view is dismissed by unmounting it. Returning `true` from a handler does not close an embedded view — change your own state instead, for example in `onCloseButtonPress`.
...
The legacy onboarding API is deprecated in v4.0 in favor of the [Flow Builder](adapty-flow-builder). It still works but emits deprecation warnings. Plan migration of your onboardings to the Flow Builder.
