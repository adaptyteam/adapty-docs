...
:::info
Your app can take over promoted in-app purchases from SDK version 4.1, on **iOS 16.4** or later. Below iOS 16.4, and on Android, the `'onPromotedPurchaseReceived'` event never fires.
:::
...
Listeners are additive: if you register several, each one receives the product, and none of them completes the purchase for the others.
