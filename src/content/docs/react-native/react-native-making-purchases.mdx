...
**Looking for step-by-step guidance?** Check out the [quickstart guide](react-native-implement-paywalls-manually) for end-to-end implementation instructions with full context.
:::
...
| Parameter  | Presence | Description                                                  |
| :--------- | :------- | :----------------------------------------------------------- |
| **params** | required | an object of the [`MakePurchaseParamsInput`](https://react-native.adapty.io/types/makepurchaseparamsinput) type. |
...
:::warning
While your listener is registered, the SDK stops completing promoted purchases for you. If your handler never calls `makePromotedPurchase`, the purchase never happens: the App Store hands the product to your app and waits. Call `subscription.remove()` to restore the automatic behavior.
:::

`makePromotedPurchase` takes no purchase parameters — a promoted product comes from the App Store rather than a paywall, so it carries no paywall context. It returns the same `AdaptyPurchaseResult` as `makePurchase`.

Listeners are additive: if you register several, each one receives the product, and none of them completes the purchase for the others. Calling `adapty.removeAllListeners()` drops them all, and the SDK's automatic completion resumes in its place — register the listener again if your app still owns completion.
