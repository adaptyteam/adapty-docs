...
**Reason**: You may be calling `logShowFlow` (React Native SDK v4+) / `logShowPaywall` in your code, which duplicates the view count if you're using the Paywall Builder or the Flow & Paywall Builder. For flows and paywalls built with these tools, analytics are tracked automatically, so you don't need to use this method.

**Solution**: Ensure you are not calling `logShowFlow` (React Native SDK v4+) / `logShowPaywall` in your code if you're using the Paywall Builder or the Flow & Paywall Builder.
