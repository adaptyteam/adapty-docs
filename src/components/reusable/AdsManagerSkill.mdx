...
Claude Cowork runs the CLI in a sandbox that blocks every domain outside its allowlist. Allow the Adapty domains before you install the plugin there — see [Run the CLI in Cowork](developer-cli-quickstart#run-the-cli-in-cowork). A Cowork task also starts on a clean machine, so the CLI and its login don't carry over between sessions — the plugin's `adapty-cli-setup` skill installs and authenticates the CLI again at the start of each one.
