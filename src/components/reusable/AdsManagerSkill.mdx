...
Both plugin routes bring the `/asa-*` commands along with the skills — ready-made prompts for common jobs, like `/asa-review` for the weekly check-in and `/asa-harvest` for turning search terms into keywords and negatives. Claude Code exposes them as slash commands; Codex loads them as skills you ask for by name.
