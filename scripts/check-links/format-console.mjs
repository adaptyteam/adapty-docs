export function printConsoleSummary({ uniqueErrors, uniqueWarnings, manualCheckList, whitelistedWarnings = [] }) {
  ...
  const mdExtensions = uniqueWarnings.filter(w => w.severity === 'md-extension').length;
  ...
  console.log(`Stale links: ${uniqueWarnings.length} (${internalRedirects} internal redirects, ${redirects} redirects, ${anchors} missing anchors, ${mdExtensions} .md extensions)`);
