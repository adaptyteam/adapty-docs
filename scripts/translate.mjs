 ...
 *                                                                                               # When a sidebar JSON is in --only-files, the script also
 *                                                                                               # auto-translates any article it references that has no
 *                                                                                               # translation yet in the current locale — fills the gap
 *                                                                                               # left when an article is added to a sidebar after it was
 *                                                                                               # first committed.
...
const onlyDocIds = onlyFilePaths
  ? new Set(
      onlyFilePaths
        .filter((p) => p.startsWith("src/content/docs/") && p.endsWith(".mdx"))
        .map((p) => path.basename(p, ".mdx")),
    )
  : null;
const onlySidebarNames = onlyFilePaths
  ? new Set(
      onlyFilePaths
        .filter(
          (p) => p.startsWith("src/data/sidebars/") && p.endsWith(".json"),
        )
...
const onlySpecIds = onlyFilePaths
  ? new Set(
      onlyFilePaths
        .filter((p) => /^src\/api-reference\/specs\/[^./]+\.yaml$/.test(p))
...
const onlyReusableIds = onlyFilePaths
  ? new Set(
      onlyFilePaths
        .filter(
          (p) => p.startsWith("src/components/reusable/") && /\.mdx?$/.test(p),
        )
        .map((p) => path.basename(p).replace(/\.mdx?$/, "")),
    )
  : null;
...
if ((flagResume || fileId || fileIds || sidebarName || platform) && !lang) {
  console.error(
    "[translate] --lang <code> is required when using --resume, --file, --ids, --sidebar, or --platform",
  );
  process.exit(1);
}
...
const syncMode =
  !flagBatch &&
  (fileId != null || fileIds != null || flagIncremental || flagSync);
...
function buildSystemPrompt(
  targetLanguage,
  metadataTitleSuffix = "| Adapty Docs",
) {
...
async function main() {
  ...
  if (!apiKey && !flagDryRun) {
    ...
    console.error(
      "[translate] ANTHROPIC_API_KEY environment variable is required.",
    );
    process.exit(1);
  }
  ...
  if (flagResume) {
    let resumeBatchId =
      resumeArgValue && !resumeArgValue.startsWith("--")
        ? resumeArgValue
        : null;
    if (!resumeBatchId) {
      try {
        ...
        console.log(
          `[translate] Auto-reading batch ID from .translate-batch-id: ${resumeBatchId}`,
        );
      } catch {
        console.error(
          `[translate] --resume given but no batch ID provided and ${BATCH_ID_FILE} not found.`,
        );
        process.exit(1);
      }
    }
  ...
  if (flagMigrateHashes) {
    if (!lang) {
      console.error("[migrate-hashes] --lang is required");
      process.exit(1);
    }
    const hashesDir = path.resolve(LOCALES_BASE, lang, ".hashes");
  ...
  if (
    ...
    onlySpecIds.size === 0 &&
    onlyReusableIds.size === 0
  ) {
    console.log(
      "[translate] --only-files: no translatable files in diff — nothing to do.",
    );
    return;
  }
  ...
  if (langs.length === 0) {
    console.log(
      "[translate] No locale directories found in src/locales/ — nothing to do.",
    );
    return;
  }

  // --only-files sidebar gap-fill: when a sidebar JSON is in the diff, the
  // workflow's `--only-files` filter would otherwise miss any article newly
  // added to it (the article MDX isn't in the diff). Collect every article
  // ID referenced by an in-diff sidebar so the per-locale loop below can
  // queue the ones that lack a translation in that locale.
  // Built once outside the loop; the missing-translation check is per locale.
  let sidebarReferencedIds = null;
  let allSourceMdxIds = null;
  if (onlySidebarNames && onlySidebarNames.size > 0) {
    sidebarReferencedIds = new Set();
    ...
    for (const sidebarBaseName of onlySidebarNames) {
      try {
        const sidebar = JSON.parse(
          await fs.readFile(
            path.join(SIDEBARS_DIR, `${sidebarBaseName}.json`),
            "utf-8",
          ),
        );
        collectIds(sidebar);
      } catch {
        /* malformed/missing sidebar — skip silently */
      }
    }
    if (sidebarReferencedIds.size > 0) {
      const allMdx = await collectMdxFiles(DOCS_DIR);
      allSourceMdxIds = new Set(
        allMdx.map((f) => path.basename(f, ".mdx")),
      );
    }
  }
  ...
  for (const currentLang of langs) {
    ...
    const metadataTitleSuffix =
      METADATA_TITLE_SUFFIXES[currentLang] ?? "| Adapty Docs";
    const systemPrompt =
      buildSystemPrompt(targetLanguage, metadataTitleSuffix) + glossary;
    ...
    if (!flagApiSpecs) {
      ...
      // Expand onlyDocIds with sidebar gap-fills for this locale: any article
      // referenced by a changed sidebar that has a source MDX but no
      // translation yet under src/locales/<currentLang>/. Already-translated
      // articles are never re-translated here — only true gaps.
      let effectiveDocIds = onlyDocIds;
      if (sidebarReferencedIds && allSourceMdxIds) {
        const gapIds = [];
        for (const id of sidebarReferencedIds) {
          if (!allSourceMdxIds.has(id)) continue; // broken sidebar ref — skip
          if (onlyDocIds && onlyDocIds.has(id)) continue; // already in diff
          try {
            await fs.access(path.join(localesDir, `${id}.mdx`));
            // exists — already translated, leave it alone
          } catch {
            gapIds.push(id);
          }
        }
        if (gapIds.length > 0) {
          effectiveDocIds = new Set([...(onlyDocIds ?? []), ...gapIds]);
          console.log(
            `${tag} Sidebar gap-fill: queuing ${gapIds.length} untranslated article(s) referenced by changed sidebar(s): ${gapIds.join(", ")}`,
          );
        }
      }
  ...
  if (hadErrors) {
    ...
    const failurePct =
      articlesAttempted > 0 ? articlesFailed / articlesAttempted : 1;
...
async function translateForLang(
  client,
  lang,
  localesDir,
  hashesDir,
  ...
  if (fileId) {
    ...
    const notFound = fileIds.filter(
      (id) => !files.some((f) => path.basename(f, ".mdx") === id),
    );
  ...
  for (const file of files) {
    ...
    if (flagIncremental) {
      ...
      if (storedHash === currentHash) {
        ...
        let cachedData = null;
        try {
          cachedData = JSON.parse(await fs.readFile(hashFilePath, "utf-8"));
        } catch {
          /* ok */
        }
      ...
      if (!storedHash) {
        ...
        if (!explicitlyChanged) {
          try {
            ...
            /* no translation exists → fall through and translate */
          }
        }
      }
        ...
        if (flagIncremental) {
          ...
          if (content.length > 40000) {
            ...
            if (response.stop_reason === "max_tokens") {
              throw new Error(
                `output truncated at max_tokens limit — increase max_tokens or split the file`,
              );
            }
...
function makeSeedEntry(enContent, zhContent) {
  const cH =
    "sha256:" + crypto.createHash("sha256").update(enContent).digest("hex");
  const pH =
    "sha256:" +
    crypto
      .createHash("sha256")
      .update(stripCodeBlocks(enContent))
      .digest("hex");
  const enCode = extractCodeBlocks(enContent);
  const zhCode = extractCodeBlocks(zhContent);
  const codeUnchanged =
    enCode.length === zhCode.length && enCode.every((b, j) => b === zhCode[j]);
  return {
    contentHash: codeUnchanged
      ? cH
      : "sha256:" + crypto.createHash("sha256").update(zhContent).digest("hex"),
    proseHash: pH,
...
async function seedSectionCache(file, localesDir, hashesDir, lang) {
  ...
  const zhHeadSecs = splitIntoSections(existingTranslation, {
    paragraphFallback: false,
  });
  if (engHeadSecs.length !== zhHeadSecs.length) {
    console.warn(
      `  ⚠ ${basename}: seed skipped — heading count mismatch (en=${engHeadSecs.length}, locale=${zhHeadSecs.length}); next edit will retranslate the whole file`,
    );
    return;
  }
  ...
  const zhByHeadId = new Map(
    dedupEngHead.map((s, i) => [s.id, dedupZhHead[i].content]),
  );
  ...
  if (paraSkipReasons.length > 0) {
    console.warn(
      `  ⚠ ${basename}: seed skipped para-chunks for ${paraSkipReasons.length} heading section(s) [${paraSkipReasons.slice(0, 3).join(", ")}${paraSkipReasons.length > 3 ? ", …" : ""}] — en/locale block-count mismatch; next prose edit in those sections will retranslate them whole`,
    );
  }
  ...
  const headingCount = Object.keys(seeded).filter(
    (id) => !/-p[0-9a-f]{8}$/.test(id),
  ).length;
  ...
  await fs.writeFile(
    hashFile,
    JSON.stringify({ fileHash: currentHash, sections: seeded }),
    "utf-8",
  );
  console.log(
    `  ⟳ ${basename}: seeded ${headingCount} heading + ${paraCount} para-chunk sections`,
  );
}
...
async function translateFileWithSections(
  ...
  if (!storedData?.sections) {
    ...
    try {
      ...
      const engHeadSecs = deduplicateSectionIds(
        splitIntoSections(content, { paragraphFallback: false }),
      );
      const zhHeadSecs = deduplicateSectionIds(
      ...
      if (engHeadSecs.length !== zhHeadSecs.length) {
        ...
        if (Object.keys(seeded).length > 0) {
          ...
          console.log(
            `  ↺ ${basename}: seeded section cache from existing translation`,
          );
        }
      }
    } catch {
  ...
  for (const section of sections) {
    ...
    const proseHash =
      ...
      crypto
        .createHash("sha256")
        .update(stripCodeBlocks(section.content))
        .digest("hex");
    ...
    if (cached?.contentHash === contentHash) {
      ...
      const reason = !cached
        ? "no cache"
        : `prose changed (cached=${cached.proseHash?.slice(0, 12)}, current=${proseHash.slice(0, 12)})`;
      ...
      try {
        ...
        await fs.writeFile(
          hashFile,
          JSON.stringify({ fileHash: fHashPartial, sections: newSections }),
          "utf-8",
        );
      ...
      if (response.stop_reason === "max_tokens") {
        ...
        throw new Error(
          `section '${section.id}' output truncated at max_tokens limit`,
        );
      }
  ...
  if (cachedCount > 0 && (apiCallCount > 0 || patchCount > 0))
    parts.push(`${cachedCount} cached`);
  console.log(
    `  ✓ ${basename} (${parts.length ? parts.join(", ") : "all cached"})`,
  );
}
...
async function buildSectionPlan(file, lang, localesDir, hashesDir) {
  ...
  if (!storedData?.sections) {
    ...
    try {
      ...
      /* no existing translation — falls through */
    }
  }
  ...
  for (const section of sections) {
    ...
    if (cached?.contentHash === contentHash) {
      decisions.push({
        ...
        contentHash,
        proseHash,
        cachedTranslation: cached.translation,
      });
    } else if (cached?.proseHash === proseHash && cached?.translation) {
      const patched = patchCodeBlocks(
        cached.translation,
        extractCodeBlocks(section.content),
      );
      decisions.push({
 ...
 */
async function translateBatchSections(
  client,
  files,
  systemPrompt,
  localesDir,
  ...
  for (const file of files) {
    try {
      ...
      console.error(
        `  ✗ ${path.basename(file, ".mdx")}: plan failed: ${err.message}`,
      );
      hadErrors = true;
    }
  }
  ...
  if (flagDryRun) {
    console.log(
      `${tag} Dry run — not submitting. Batch entries that would be sent:`,
    );
    for (const e of allEntries) {
      const meta = customIdLookup[e.custom_id];
      console.log(
        `  ${meta.basename}::${meta.sectionId}  (custom_id: ${e.custom_id})`,
      );
    }
    return;
  }
  ...
  if (allEntries.length > 0) {
    console.log(
      `${tag} Submitting batch of ${allEntries.length} section(s)...`,
    );
    const batch = await client.messages.batches.create({
      requests: allEntries.map((e) => ({
    ...
    for await (const r of results) {
      if (r.result.type === "succeeded") {
        if (r.result.message.stop_reason === "max_tokens") {
          ...
          console.error(
            `  ✗ ${meta?.basename}::${meta?.sectionId} (${r.custom_id}): output truncated at max_tokens`,
          );
          hadErrors = true;
          continue;
        }
  ...
  for (const plan of plans) {
    ...
    if (!allOk) {
      console.error(
        `  ✗ ${plan.basename}: some sections failed — file left unwritten`,
      );
      articlesFailed++;
      continue;
    }
...
async function translateBatch(
  ...
  if (largeFiles.length > 0) {
    console.log(
      `${tag} ${largeFiles.length} large file(s) will be translated section-by-section (sync):`,
    );
  ...
  for (let c = 0; c < chunks.length; c++) {
    ...
    console.log(
      `${tag} Submitting batch of ${requests.length} requests${chunkLabel}...`,
    );
    ...
    console.log(
      `${tag} Use --resume <batchId> to retrieve results if this process is interrupted.`,
    );

    const fileMap = Object.fromEntries(
      chunk.map((f) => [path.basename(f, ".mdx"), f]),
    );
    await waitAndRetrieve(client, batchId, fileMap, localesDir, tag, lang);
  }
}

async function waitAndRetrieve(
  client,
  batchId,
  fileMap,
  localesDir,
  ...
  for await (const result of results) {
    ...
    if (result.result.type === "succeeded") {
      if (result.result.message.stop_reason === "max_tokens") {
        console.error(
          `  ✗ ${basename}: output truncated at max_tokens limit — increase max_tokens or split the file`,
        );
        errors++;
        continue;
      }
...
async function resumeBatch(client, batchId, lang, localesDir, hashesDir) {
  ...
  let batch = await client.messages.batches.retrieve(batchId);
  console.log(
    `${tag} Resuming batch ${batchId} — status: ${batch.processing_status}`,
  );
  ...
  const fileMap = Object.fromEntries(
    allSourceFiles.map((f) => [path.basename(f, ".mdx"), f]),
  );
  ...
  try {
    ...
    for (const e of entries) {
      ...
      reusableMap[basename] = {
        name: e.name,
        full: path.join(REUSABLE_DIR, e.name),
        basename,
      };
    }
  } catch {
  ...
  for await (const result of results) {
    ...
    if (result.result.type === "succeeded") {
      if (result.result.message.stop_reason === "max_tokens") {
        console.error(
          `  ✗ ${label}: output truncated at max_tokens limit — increase max_tokens or split the file`,
        );
        errors++;
        continue;
      }
      ...
      if (isReusable) {
        ...
        if (file) {
          ...
          await fs.writeFile(
            path.join(reusableLocalesDir, `${basename}.md`),
            postProcessTranslation(text, lang),
            "utf-8",
          );
        }
      } else {
        ...
        if (sourceFile) {
          ...
          await fs.mkdir(localesDir, { recursive: true });
          await fs.writeFile(
            path.join(localesDir, `${customId}.mdx`),
            postProcessTranslation(text, lang),
            "utf-8",
          );
        }
 ...
 */
async function rebuildSidebarLabels(
  sidebarFiles,
  sidebarHashesDir,
  localesDir,
) {
  ...
  try {
    merged = JSON.parse(
      await fs.readFile(path.join(localesDir, "_sidebar-labels.json"), "utf-8"),
    );
  } catch {
  ...
  for (const file of sidebarFiles) {
    ...
    try {
      const data = JSON.parse(
        await fs.readFile(path.join(sidebarHashesDir, `${name}.json`), "utf-8"),
      );
      if (data.labels) {
        for (const [key, value] of Object.entries(data.labels)) {
          // Handle both new format { en, translation } and legacy format string
          const translation =
            typeof value === "object" ? value.translation : value;
          if (translation) merged[key] = { value: translation };
        }
      }
    } catch {
      /* sidebar not yet translated — skip */
    }
  }
  await fs.mkdir(localesDir, { recursive: true });
  await fs.writeFile(
    path.join(localesDir, "_sidebar-labels.json"),
...
async function writeReusableResult(
  file,
  text,
  lang,
  reusableLocalesDir,
...
async function translateReusableForLang(
  client,
  lang,
  localesDir,
  hashesDir,
  ...
  if (onlyReusableIds) {
    ...
    if (files.length === 0) {
      console.log(
        `${tag} No matching reusable snippets from --only-files — skipping.`,
      );
      return;
    }
  }
  ...
  for (let c = 0; c < chunks.length; c++) {
    ...
    console.log(
      `${tag} Submitting reusable batch of ${requests.length} requests${chunkLabel}...`,
    );
    const batch = await client.messages.batches.create({ requests });
    const batchId = batch.id;
    ...
    const fileMap = Object.fromEntries(
      chunk.map((f) => [`reusable_${f.basename}`, f]),
    );
    ...
    for await (const result of results) {
      ...
      if (result.result.type === "succeeded") {
        if (result.result.message.stop_reason === "max_tokens") {
          console.error(
            `  ✗ reusable:${file.name}: output truncated at max_tokens limit`,
          );
          errors++;
          hadErrors = true;
          continue;
        ...
        console.error(
          `  ✗ reusable:${file.name}: ${JSON.stringify(result.result)}`,
        );
        errors++;
        hadErrors = true;
      }
...
async function translateSidebarsForLang(
  client,
  lang,
  localesDir,
  hashesDir,
  ...
  if (sidebarName) {
    const match = allSidebarFiles.find(
      (f) => path.basename(f, ".json") === sidebarName,
    );
    if (!match) {
      console.error(`${tag} No sidebar found with name: ${sidebarName}`);
      console.error(
        `  Available: ${allSidebarFiles.map((f) => path.basename(f, ".json")).join(", ")}`,
      );
      process.exit(1);
    }
    sidebarFiles = [match];
  } else if (onlySidebarNames) {
    sidebarFiles = allSidebarFiles.filter((f) =>
      onlySidebarNames.has(path.basename(f, ".json")),
    );
    if (sidebarFiles.length === 0) {
      console.log(
        `${tag} No matching sidebars from --only-files — skipping sidebars.`,
      );
      return;
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (cachedData.labels) {
          const labelMap = Object.fromEntries(
            labelEntries.map((e) => [e.key, e.label]),
          );
          isOldFormat = Object.values(cachedData.labels).some(
            (v) => typeof v === "string",
          );
          ...
          if (isOldFormat) {
            ...
            if (!fileUnchanged) {
              console.log(
                `  ↺ sidebar:${name}: first incremental run after format migration — all labels retranslated once`,
              );
            }
        ...
        /* no cache — all labels need translation */
      }
      ...
      const response = await client.messages.create({
        ...
        messages: [
          {
            role: "user",
            content: JSON.stringify(toTranslateEntries.map((e) => e.label)),
          },
        ],
      });
      if (response.stop_reason === "max_tokens") {
      ...
      if (
        ...
        translated.length !== toTranslateEntries.length
      ) {
        throw new Error(
          `label count mismatch: expected ${toTranslateEntries.length}, got ${translated.length}`,
        );
      }
      ...
      console.log(
        `  ✓ sidebar:${name} (${toTranslateEntries.length}/${labelEntries.length} label(s) translated)`,
      );
      anyUpdated = true;
    } catch (err) {
...
async function migrateHashes(hashesDir) {
  ...
  const hashFiles = (await fs.readdir(hashesDir).catch(() => [])).filter(
    (f) => f.endsWith(".json") && f !== "_sidebar-labels.json",
  );
  ...
  for (const hashFileName of hashFiles) {
    ...
    const sourceFile = sourceFiles.find(
      (f) => path.basename(f, ".mdx") === articleId,
    );
    if (!sourceFile) {
      skipped++;
      continue;
 ...
 */
async function translateApiSpecBatchSections(
  client,
  spec,
  systemPrompt,
  apiHashesDir,
  ...
  let localeDoc = null;
  try {
    localeDoc = yaml.load(await fs.readFile(localePath, "utf-8"));
  } catch {
    /* first run */
  }
  ...
  console.log(
    `${tag} api-spec:${spec.basename}: ${stats.hit} cached, ${stats.send} to translate.`,
  );
  ...
  if (stats.send > 0) {
    ...
    while (current.processing_status !== "ended") {
      const counts = current.request_counts ?? {};
      console.log(
        `    Status: ${current.processing_status} — succeeded: ${counts.succeeded ?? "?"}, errored: ${counts.errored ?? "?"}`,
      );
      await sleep(30_000);
      current = await client.messages.batches.retrieve(batch.id);
    }
    ...
    for await (const r of results) {
      ...
      if (r.result.type === "succeeded") {
        if (r.result.message.stop_reason === "max_tokens") {
          console.error(
            `  ✗ api-spec:${spec.basename} ${sectionId} (${r.custom_id}): truncated at max_tokens`,
          );
          hadErrors = true;
          continue;
        }
  ...
  for (const decision of decisions) {
    if (decision.kind === "hit") {
      ...
      newSectionsCache[decision.section.id] = {
        contentHash: decision.contentHash,
        translation: decision.cachedTranslation,
      };
    } else if (newTranslations[decision.section.id]) {
      newSectionContents[decision.section.id] =
        newTranslations[decision.section.id];
      newSectionsCache[decision.section.id] = {
        contentHash: decision.contentHash,
        translation: newTranslations[decision.section.id],
      };
    }
...
async function translateApiSpecsForLang(
  client,
  lang,
  localesDir,
  hashesDir,
  ...
  if (onlySpecIds) {
    ...
    if (specFiles.length === 0) {
      console.log(
        `${tag} No matching API specs from --only-files — skipping specs.`,
      );
      return;
    }
  }
  ...
  for (const spec of toTranslate) {
    try {
      if (flagBatch && flagIncremental) {
        await translateApiSpecBatchSections(
          client,
          spec,
          systemPrompt,
          apiHashesDir,
        ...
        if (finalMessage.stop_reason === "max_tokens") {
          throw new Error(
            `API spec output truncated at max_tokens limit — spec may be too large`,
          );
        }
        const outputPath = path.join(
          API_SPECS_DIR,
  ...
  for (const [topKey, topVal] of Object.entries(doc)) {
    if (topKey === "paths" && topVal && typeof topVal === "object") {
      ...
      for (const [bucket, bucketVal] of Object.entries(topVal)) {
        if (
          bucketVal &&
          typeof bucketVal === "object" &&
          !Array.isArray(bucketVal)
        ) {
          for (const [name, val] of Object.entries(bucketVal)) {
            sections.push({
              id: `components.${bucket}::${name}`,
              content: yaml.dump(
                { components: { [bucket]: { [name]: val } } },
                { lineWidth: -1, noRefs: true },
              ),
            });
          }
...
function mergeYamlSections(originalDoc, existingLocaleDoc, newSectionContents) {
  ...
  for (const topKey of Object.keys(originalDoc)) {
    if (
      topKey === "paths" &&
      originalDoc.paths &&
      typeof originalDoc.paths === "object"
    ) {
      merged.paths = {};
      ...
      typeof originalDoc.components === "object"
    ) {
      merged.components = {};
        ...
        if (
          ...
          for (const name of Object.keys(bucketVal)) {
            ...
            if (newSectionContents[sectionId]) {
              ...
              merged.components[bucket][name] =
                existingLocaleDoc.components[bucket][name];
            } else {
...
async function getAllSidebarIds() {
  ...
  for (const entry of entries) {
    ...
    try {
      const sidebar = JSON.parse(
        await fs.readFile(path.join(SIDEBARS_DIR, entry.name), "utf-8"),
      );
      extract(sidebar);
    } catch {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isOverloaded && attempt < maxRetries) {
        ...
        console.warn(
          `  ⚠ API overloaded (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${delay / 1000}s...`,
        );
        await sleep(delay);
        continue;
      }
