...
|---|---|
| "web purchase doesn't unlock the app", "web2app access not granted", "bought on the site, no premium in the app" | `stripe` / `paddle`, but establish which half broke first. Adapty can only link a web purchase to a profile by an ID passed *at checkout time* — Stripe `metadata`, Paddle `custom_data`, both keyed `customer_user_id` — and the app must then call `.activate()`/`.identify()` with that same value. A missing ID on either side produces this symptom. **Third origin, added 2026-09-11: `funnelfox`.** There the reader passes nothing at checkout — FunnelFox sets the ID itself, the buyer's checkout email by default — so the break is always on the app side. |
| "installed the app from the funnel link but the user isn't Pro", "FunnelFox purchase didn't grant access", "Custom entitlement" | `funnelfox`. Two distinct causes, both seen in support. (1) The install link carries no identity, so the app must call `.identify()` with the ID FunnelFox used — signing in with the checkout email is what makes it work, and a deferred deep link is what avoids the sign-in. (2) Access level resolution is two-level: the integration settings carry a default, and a checkout element's **Custom entitlement** overrides it for purchases through that element (confirmed by product 2026-09-16). A funnel can hold several checkout elements (Wallet button, accordion checkout), each with its own field; an empty one falls back to the default. Republish the funnel after changing either. |
| "FunnelFox revenue shows the wrong duration", "web purchase is uncategorized in analytics" | `funnelfox`. Unlike `stripe` / `paddle`, a matching Adapty product is **not** required for the purchase to be recorded — FunnelFox reports it over the Server-side API regardless. Without one Adapty has no subscription duration, so analytics can misreport the duration or leave the purchase uncategorized. Recommendation, not a prerequisite (product, 2026-09-16). |
| "do we also need the Adapty–Stripe integration", "double-counted web revenue with FunnelFox" | `funnelfox`. In a Stripe/Paddle → FunnelFox → Adapty chain the processor does **not** need its own Adapty connection. Connect it only when the customer also sells through that processor outside their funnels; Adapty deduplicates transactions arriving from both sources (product, 2026-09-16). |
...
| "Amazon Appstore", "Huawei AppGallery", "our own store", "direct sales" | `custom-store`. Defined by absence: Adapty neither validates nor processes the purchase, so the article names no specific store. Note `payment-integrations` (the "Web" entry page) lists Stripe, Paddle, and — since 2026-09-11 — FunnelFox; custom stores are still not reachable from it. |
