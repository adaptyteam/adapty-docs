...
- **`funnelfox` was written 2026-09-11 with two questions deliberately left out of the article**, because neither could be settled from a source. (1) When a FunnelFox funnel bills through the customer's *own* Stripe or Paddle account, does the Adapty↔Stripe/Paddle connection still need to be set up alongside the FunnelFox one? Support told one customer no; that customer only got access working after adding it ([Pylon 1178](https://app.usepylon.com/issues?issueNumber=1178)). (2) Whether the webhook's **Exclude historical events** setting still drops `subscription_started` for FunnelFox purchases — FunnelFox delivered profile creation *after* the purchase, so the purchase read as historical, and unlike the App Store and Google Play no SDK guarantees that ordering for a web source ([Pylon 2359](https://app.usepylon.com/issues?issueNumber=2359), closed February 2026 pending a dev ticket, never confirmed fixed). Both are worth an owner answer; the second is the one that silently breaks integrations.
