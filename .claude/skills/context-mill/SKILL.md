...
| File | Owner | Carries |
|---|---|---|
| `docs-map.jsonl` | script | Mechanical per-document facts: title, headings, symbols, sidebars, links, three narrow hashes (`content_hash`, `shape_hash`, `api_hash`). Regenerated by `npm run mill`. Never hand-edit. **Local-only (gitignored)** — a fresh clone has none until you run `npm run mill:map`. |
...
| `npm run mill:map` | Rebuilds `docs-map.jsonl` only and touches no committed file. Run it before reading or searching the map. |
