...
:::warning
If you A/B-test paywalls built outside the paywall builder, be sure to [log paywall views](present-remote-config-paywalls#track-paywall-view-events) with `.logShowPaywall()`. Without this call, Adapty can’t calculate views, so conversion stats will be wrong.
:::
...
Onboarding A/B tests are available only on the Pro & Enterprise plans and require the native iOS or Android SDK version 3.8.0 or higher.
