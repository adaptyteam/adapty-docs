...
* Installed Android Studio on your machine
* Have a basic understanding of Kotlin and Android development
* Set up an Adapty account with access to the Adapty dashboard
* Set up your app in Google Play Console with at least one in-app purchase configured
* Downloaded the starter project [here](https://github.com/ferrousguy/movie-mania-kotlin-multiplatform)
  * The *main* branch of this repo contains the finished version of the project, which is perfect for referencing as we go along. If you'd rather follow along with the steps below, switch to the *starter* branch first.

## Installing the Kotlin Multiplatform Adapty SDK

1. Open your project's `build.gradle.kts` file.
2. Add the Adapty SDK dependency:
...
1. Create a Kotlin file named `Constants.kt` in the sample project. This file is used to store constants used throughout the app. Add the following Adapty-related constants:

```kotlin showLineNumbers
object AppConstants {
    object Adapty {
        const val apiKey = "API key goes here"
        const val accessLevelID = "premium"
        const val placementID = "on_launch"
    }
}
```
...
2. Open your Application class and import the Adapty frameworks:

```kotlin showLineNumbers
import com.adapty.Adapty
import com.adapty.AdaptyConfig
```

3. Add the Adapty initialization in your Application's `onCreate` method:
...
override fun onCreate() {
    ...
    Adapty.activate(
        applicationContext,
        AdaptyConfig.Builder(AppConstants.Adapty.apiKey)
...
We're going to create a class that will be used throughout the app as the single-source-of-truth on the user's status as a paying customer and what access they have purchased.

To manage Adapty interactions, create a new Kotlin file called `AdaptyHelper.kt`:

```kotlin showLineNumbers
import com.adapty.Adapty
import com.adapty.AdaptyProfile
import com.adapty.AdaptyPurchaseResult

class AdaptyHelper {
    private var customerProfile: AdaptyProfile? = null
    var subscriptionActive: Boolean = false
        private set
    
    val activeAccessLevel: AdaptyProfile.AccessLevel?
        get() = customerProfile?.accessLevels?.get(AppConstants.Adapty.accessLevelID)?.takeIf { it.isActive }
    
    suspend fun refreshCustomerProfile() {
        try {
            customerProfile = Adapty.getProfile()
            subscriptionActive = customerProfile?.accessLevels?.get(AppConstants.Adapty.accessLevelID)?.isActive == true
        } catch (e: Exception) {
            // Handle error
        }
    }
    
    fun wasPurchased(result: AdaptyPurchaseResult) {
        when (result) {
            is AdaptyPurchaseResult.Success -> {
                customerProfile = result.profile
                subscriptionActive = result.profile.accessLevels[AppConstants.Adapty.accessLevelID]?.isActive == true
            }
            else -> {
                // Handle other cases
            }
    ...
    fun wasRestored(profile: AdaptyProfile) {
        customerProfile = profile
    }
}
```
...
To display a paywall, you can use the following code:
...
3. Enter your *Issuer ID* and *Key ID* from App Store Connect.

4. Upload your private key .p8 file from App Store Connect.

5. Add you *App Store Connect shared secret*.

6. Copy the URL for App Store server notifications and paste it into App Store Connect. Find directions [here](enable-app-store-server-notifications).
...
7. Navigate to the **Products** section in the left-side navigation bar and click on the tab called **Access Levels**. A default access level has been created for you (`premium`). You can also create your own here to group different sets of functionality together in your app.

8. Click the other tab called **Products** in this section, and create new products linked to App Store Connect subscriptions.
...
9. Click on **Paywalls** in the left-side navigation, and create a new paywall using the button in the upper right.
...
10. Click the **Builder** tab and choose from our available templates. You'll want to pick a template with at least two product areas as we have two subscriptions to show.

11. Once chosen, you can customize the template with marketing copy, images, and style and layout changes to make it look just right for your needs. Any changes you make to the design will be reflected in the preview design to the right.
    <Zoom>
...
12. If you're happy with the design of the paywall, click **Save & publish** at the bottom to save all your changes.
