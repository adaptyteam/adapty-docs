...
1. You set up your endpoint:
   ...
   3. And after that to  [response](set-up-webhook-integration#your-servers-verification-response) to it for Adapty server to learn the connection is successfully established.
   4. To [receive following standard event requests and process them](set-up-webhook-integration#adapty-standard-event-request).

2. [You configure the integration](set-up-webhook-integration#configure-webhook-integration-in-the-adapty-dashboard) in the Adapty Dashboard. Within this step, you can optionally [map Adapty events with your event names](set-up-webhook-integration#choose-events-to-send-and-map-event-names). We recommend to fist use the Sandbox environment to test how your server processes Adapty events and after you make sure it does it as expected, switch to the production environment 
3. Adapty sends a verification request to your server.
4. Your server sends a verification response to Adapty.
5. After Adapty server receives the verification responce in the expected format, it's ready to send standard event requests.
